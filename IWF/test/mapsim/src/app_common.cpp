 ...
 * LOG: Revision 1.1.2.2  2013/03/18 09:43:34  jvikas
 * LOG: MAP Simulator changes.
 * LOG:
 ...
 *
 ****************************************************************************/
#ident "$Id: app_common.cpp,v 1.1.2.2 2013/03/18 09:43:34 jvikas Exp $"
...
TCAPMsgs::GenerateLoad(ITS_HANDLE handle)
{
   ...
   while(1)
   {
         ...
         {
              SendBeginDlg(handle, TRANSPORT_INSTANCE(handle), ITS_FALSE);
                              ...
                              ITS_POINTER callData)
{
   ...
   gtt->addrInd = gtt->addrInd | (SCCP_CPA_ROUTE_SSN | SCCP_CPA_HAS_PC |
                        SCCP_CPA_HAS_SSN);
       gtt->dpc = oPC;
       gtt->ssn = oSN;
 
       /*cout <<endl<<"===> Enter DPC for GTT:";
       cin >> gtt->dpc;
       gtt->ssn = 0x05;*/
