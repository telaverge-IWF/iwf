 ...
 * LOG: Revision 1.1.1.1.2.3.4.1.12.4  2015/05/25 05:11:54  jkchaitanya
 * LOG: Cap-Ro test tool enhancement to support automatic user configuration mode for automation purpose, Review 4059
 * LOG:
 * LOG: Revision 1.1.1.1.2.3.4.1.12.3  2014/12/16 05:40:13  jkchaitanya
 * LOG: changes merged from TSL branch
 * LOG:
 * LOG: Revision 1.1.1.1.2.3.4.1.10.1  2014/12/10 10:48:55  kpnithin
 * LOG: changes for bug 5688
 * LOG:
 * LOG: Revision 1.1.1.1.2.3.4.1  2013/12/31 04:44:52  millayaraja
 ...
 *
 ****************************************************************************/
#ident "$Id: app_callbacks.cpp,v 1.1.1.1.2.3.4.1.12.4 2015/05/25 05:11:54 jkchaitanya Exp $"
...
void ENGINE_PreFunc(void)
{
   ...
   APP_TRACE_DEBUG(("Added SCCP GTT Callback..."));
#endif

#ifndef USE_ANSI_SCCP   
#if 1
   CALLBACK_AddCallback(SCCP_GlobalTitleTranslator,
                        TCAPUtils::RegisterSCCPGTTCallback, NULL);
   APP_TRACE_DEBUG(("Added SCCP GTT Callback..."));
#endif
#endif
...
void WP_Dispatch(ThreadPoolThread* thr, void* arg)
{
   ...
   while(!work->GetExit())
   {
       if(work->GetNextEvent(evt) == ITS_SUCCESS)
       {
           ...
           }else if(evt.GetSource() == ITS_TIMER_SRC)
           {    
               APP_TRACE_DEBUG(("WPTQ_PostNextEvent: TIMER SRC"));
               ByteArray octData;
               STimerContext timerCtxtData;
               ITS_TimerData *td; 
               td = (ITS_TimerData*)evt.GetData();
               memcpy(&timerCtxtData, td->context, sizeof(STimerContext));
               tcapMsgs[APP_CCITT]->HandleTimeout(timerCtxtData);
           }
           else
           {
