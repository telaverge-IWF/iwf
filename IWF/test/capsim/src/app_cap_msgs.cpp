...
Json::Value parseCapConfig()
{
    ...
    if (jsonFile == NULL )
    {
        APP_TRACE_ERROR(("CAPSIM_IWF_JSON is not exported"));
        exit(0);
    }
    APP_TRACE_DEBUG(("CAPSIM_IWF_JSON=%s",jsonFile));
...
AsnObject* BuildApplyChargingReportinTcContAgain()
{
    ...
    if(!isNull && isAVPEnabled)
    {
        ...
        //NOTE: As of now only for all the callflows, CallActive is false in ACR_in_TcCont_Again
...
AsnObject* BuildApplyChargingReportinTcCont()
{
    ...
    if(!isNull && isAVPEnabled)
    {
        ...
        //Default CallActive true in all the callflows 1,4,6,7,8,9,11
...
AsnObject* BuildApplyChargingReportinTcEnd()
{
    ...
    if(!isNull && isAVPEnabled)
    {
        ...
        //NOTE:CallActive set to false in TC_END case
        cap_v3::CAMEL_CallResult::TimeDurationChargingResult::CallActive ca(false);
        tdcr.SetCallActive(ca);
...
AsnObject* BuildEventReportSMSArg()
{
    ...
    if(optionfromserver == 13)
    {
        cfg = &(*m_jsonRoot1)["SMS_CALL_FLOWS"]["TestCase_13"]["EventReportSMSArg"];
    }
    else if(optionfromserver == 14)
    {
        cfg = &(*m_jsonRoot1)["SMS_CALL_FLOWS"]["TestCase_14"]["EventReportSMSArg"];
    }
...
AsnObject* BuildEventReportBCSMArginTcContAgain()
{
    ...
    //NOTE: Assume that this ERB in Again is by default set to O_disconnet 
    if(optionfromserver)
    {
        //NOTE: For callflow 1 EventTypeBCSM is set to O_disconnet & EventSpecificInformationBCSM is set to ODisconnectSpecificInfo
        cap_v3::EventTypeBCSM etb(9);
                    ...
                    else
                    {
                        ...
                        if(!isNull && isAVPEnabled)
                        {
                            ...
                            if(!isNull && isAVPEnabled)
                            {
                                ...
                                std::string dest = ((*cfg)["eventSpecificInformationBCSM"]["oAnswerSpecificInfo"]["destinationAddress"]["Value"].asString());                            if(dest.length() > MAX_CALLED_PARTY_NUMBER_DIGITS)
                                {    
...
AsnObject* BuildEventReportBCSMArginTcCont()
{
    ...
    if(optionfromserver == 2)
    {
        //NOTE: For callflow 1 EventTypeBCSM is set to O_CalledPartyBusy & EventSpecificInformationBCSM is set to OCalledPartyBusySpecificInfo
        cap_v3::EventTypeBCSM etb(5);
        erb->SetEventTypeBCSM(etb);
    ...
    else if(optionfromserver == 3)
    {
        //NOTE: For callflow 1 EventTypeBCSM is set to O_NoAnswer & EventSpecificInformationBCSM is set to ONoAnswerSpecificInfo
        cap_v3::EventTypeBCSM etb(6);
        erb->SetEventTypeBCSM(etb);
    ...
    else if(optionfromserver)
    {
        //NOTE: For callflow 1,4,6,7,8,9 EventTypeBCSM is set to O_Answer & EventSpecificInformationBCSM is set to OAnswerSpecificInfo
        cap_v3::EventTypeBCSM etb(7);
        erb->SetEventTypeBCSM(etb);
...
AsnObject* BuildEventReportBCSMArginTcEnd()
{
    ...
    //NOTE: Assume that this ERB in Again is by default set to O_disconnet & ODisconnectSpecificInfo
    ...
    if(!isNull && isAVPEnabled)
    {
        ...
        if(!isNull && isAVPEnabled)
        {
            ...
            else
            {
                ...
                if(!isNull && isAVPEnabled)
                {
                    cap_v3::LocationInformation::CellGlobalIdOrServiceAreaIdOrLAI::LAIFixedLength lai(MAPUtils::StringToVector((*cfg)["locationInformationMSC"]["cellGlobalIdOrServiceAreaIdOrLAI"]["laiFixedLength"]["Value"].asString()));
...
AsnObject* BuildInitialDPArg()
{
    ...
    if(!isNull && isAVPEnabled)
    {
        ...
        if(!isNull && isAVPEnabled)
        {
            ...
            {
                isNull = (*cfg)["locationInformation"]["cellGlobalIdOrServiceAreaIdOrLAI"]["laiFixedLength"]["Value"].isNull();
