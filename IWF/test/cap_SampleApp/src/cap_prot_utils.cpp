...
const std::string CAPUtils::sDigits("0123456789ABCDEF");

#ifdef GCC_LESS_THAN_4_3
using namespace std::tr1;
const CAPUtils::MccWith3DigitMnc CAPUtils::sMccWith3DigitMnc = InitMccWith3DigitMnc();
#else
// gcc flag -std=c++0x need for this type of initialization
const CAPUtils::MccWith3DigitMnc CAPUtils::sMccWith3DigitMnc = {
...
int CAPUtils::StringToTBCD(const std::string &str, uint8_t *tbcd, int len, unsigned pad, bool no_rev)
{
    int slen = str.size();
    int shalf = slen >> 1;
    int bIdx, sIdx; 
...
void CAPUtils::TBCDToString(const uint8_t *tbcd, int len, std::string &str, bool odd, bool rev)
{
...
void CAPUtils::IpStringToGSNAddress(const std::string &ipStr, its::ByteArray &gsnAddr)
{
    // GSN Address format 
...
/*diameter::Address CAPUtils::GSNAddressToDiaAddress(
...
void CAPUtils::ByteArrayToHexString(const std::vector<byte>& arr, std::string& hexStr)
{
    unsigned val = 0;
...
void CAPUtils::ExtractMncMcc(const std::vector<byte>& imsi, unsigned& mcc, unsigned& mnc)
{
...
void CAPUtils::ExtractMncMcc(const std::string& imsi, unsigned& mcc, unsigned& mnc)
{
    unsigned mncDigits = 2;
    unsigned *val = &mcc;
    mcc = 0;
