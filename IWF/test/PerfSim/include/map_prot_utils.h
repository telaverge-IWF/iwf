#ifndef __MAP_PROT_UITLS_h__
#define __MAP_PROT_UITLS_h__

#include "map_prot_defines.h"
...
#define MAP_FixedArraySize(_array) \
        (sizeof(_array) / sizeof(_array[0]))

#define MAP_FixedArrayToVector(_array) \
            std::vector<byte>(_array, _array + MAP_FixedArraySize(_array))
...
class MAPUtils
{
 public:
    ...
    static void TBCDToString(const uint8_t *tbcd, int len, std::string &str, bool odd = false);
