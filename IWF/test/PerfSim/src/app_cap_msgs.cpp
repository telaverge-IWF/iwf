...
void ParseCAPConfiguration()
{
    ...
    if(!((*m_jsonRoot1)["LocalGSUTimer"]["Value"].isNull()))
    {
        LocalGSUTimer = (*m_jsonRoot1)["LocalGSUTimer"]["Value"].asInt();
        GSUTimer = LocalGSUTimer;
    }

    if(!((*m_jsonRoot1)["NumberOfACRTrueMsgsToSend"]["Value"].isNull()))
    {
        NumberOfACRTrueMsgsToSend = (*m_jsonRoot1)["NumberOfACRTrueMsgsToSend"]["Value"].asInt();
    }
}
...
Json::Value parseCapConfig()
{
    ...
    if (jsonFile == NULL )
    {
        APP_TRACE_ERROR(("CAP_IWFPERF_CONFIG_FILE is not exported"));
        exit(0);
    }
    APP_TRACE_DEBUG(("CAP_IWFPERF_CONFIG_FILE=%s",jsonFile));
