 ...
 ****************************************************************************
 *
 *  ID: $Id: readISUPMsg.c,v 9.1 2005/03/23 12:52:05 cvsadmin Exp $
 *
 ...
 * LOG: Begin PR5
 * LOG:
 * LOG: Revision 1.2  2000/09/18 18:52:07  labuser
 * LOG: Change the order of reading the bytes from the data file for
 * LOG: UserServiceInformation param
 * LOG:
 * LOG: Revision 1.1  2000/09/08 14:34:16  labuser
 * LOG: cbascon: now have correct directory structure
 * LOG:
 * LOG: Revision 3.1  2000/08/15 23:59:34  mmiers
 * LOG:
 * LOG: Begin round 4.
 * LOG:
 * LOG: Revision 2.13  2000/07/14 16:40:20  labuser
 * LOG: Modified code for transaction_id as 4 octets instead of int
 * LOG:
 * LOG: Revision 2.12  2000/07/07 16:12:05  hxing
 * LOG: Type of transaction_id has been changed from ITS_UINT to ITS_OCTET[4]
 * LOG:
 * LOG: Revision 2.11  2000/05/17 14:08:55  labuser
 * LOG: *** empty log message ***
 * LOG:
 * LOG: Revision 2.10  2000/05/16 15:11:33  labuser
 * LOG: *** empty log message ***
 * LOG:
 * LOG: Revision 2.9  2000/01/27 14:56:46  labuser
 * LOG: cbascon: Clean up code.
 * LOG:
 * LOG: Revision 2.8  2000/01/11 22:33:52  labuser
 * LOG: Add read function for OutgoingTrunkGroupNumber parameter.
 * LOG:
 * LOG: Revision 2.7  2000/01/11 18:11:20  labuser
 * LOG: Almost done with pass thru messages.
 * LOG:
 * LOG: Revision 2.6  1999/12/27 22:33:29  cbascon
 * LOG: *** empty log message ***
 * LOG:
 * LOG: Revision 2.5  1999/12/27 21:51:32  cbascon
 * LOG: Clean up code.
 * LOG:
 * LOG: Revision 2.4  1999/12/27 19:45:17  cbascon
 * LOG: TransmissionMediumRequirement parameter is CCITT only.
 * LOG:
 * LOG: Revision 2.3  1999/12/27 19:01:48  cbascon
 * LOG: Added functions for reading and printing parameters for ACM, ANM, CPG,
 * LOG: REL, RLC, CFN, CON, SAM, UCIC, RSC for CCITT only.
 * LOG:
 * LOG: Revision 2.2  1999/12/21 19:24:11  cbascon
 ...
 * LOG:
 * LOG: Revision 2.0  1999/12/03 23:21:36  mmiers
 * LOG:
 * LOG: Begin third iteration.
 * LOG:
 * LOG: Revision 1.3  1999/11/24 19:27:05  cbascon
 * LOG: Fixed some bugs which caused core dumps.
 * LOG:
 * LOG: Revision 1.2  1999/11/15 23:25:25  cbascon
 * LOG: Apparently for fscanf to store a value using %x cannot be an unsigned
 * LOG: char in Solaris. Worked fine on NT.
 * LOG:
 * LOG: Revision 1.1  1999/11/14 23:15:47  cbascon
...
ReadMessageInPAMPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    {
        printf("ISUPTEST: msgTypeInPAM plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
int ReadAccessDeliveryInfoPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if (ie->param_length != plen)
    {
        printf("ISUPTEST: AccessDeliveryInformation plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadAutoCongestionLvlPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    {
        printf("ISUPTEST: AutomaticCongestionLevel, plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadBackwardCallIndPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: BackwardCallIndicator plen != %d \n", plen);
        ret = !ITS_SUCCESS; 
    }
    else
    {
...
ReadBusinessGrpPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    {
        printf("ISUPTEST: BusinessGroup, GroupId field len != %d \n",
               fieldOctLenBusGrpId);
        return !ITS_SUCCESS;
    }
    ...
    {
        printf("ISUPTEST: BusinessGroup, SubGroupId field len != %d \n",
               fieldOctLenSubGrpId);
        return !ITS_SUCCESS;
    }
...
ReadCallDiversionInfoPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: CallDiversionInformation plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadCallHistoryInfoPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: CallHistoryInformation maxplen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadCallReferencePar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if (ci_nOctet != ciLen)
    {
        printf("ISUPTEST: CallReference: CallIdentity field != %d octets",
               ciLen);
        return !ITS_SUCCESS;
    }
    ...
    if (pc_nOctet != pcLen)
    {
        printf("ISUPTEST: Point code octet length != %d", pcLen);
        return !ITS_SUCCESS;
    }

    GetOctetsFromStr(&(ie->param_data.callRef.point_code[0]), (int)pc_nOctet);
...
ReadCallingPartyCategoryPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: CallingPartyCategory plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadCallingPartyNumPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    {
        printf("ISUPTEST: CallingPartyNumber: no. of digits octets > %d \n",
...
ReadCarrierIdPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    {
        printf("ISUPTEST: CarrierId plen > %d \n", maxplen);
        return !ITS_SUCCESS;
    }
...
ReadCarrierSelectionInfoPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != maxplen)
    {
        printf("ISUPTEST: CarrierSelectionInfo plen > %d \n", maxplen);
        return !ITS_SUCCESS;
    }
...
ReadCircuitGrpCharIndPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != maxplen)
    {
        printf("ISUPTEST: CircuitGroupCharacteristicIndicator plen != %d \n",
               maxplen);
        return !ITS_SUCCESS;
    }
...
ReadCircuitGrpSupervisionMsgTypIndPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: CircuitGroupSupervisionMsgTypeInd plen != %d \n",
               plen);
        ret = !ITS_SUCCESS;
    }
    else
...
ReadCircuitIdNamePar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    {
        printf("ISUPTEST: CircuitIdentificationName trunkNumDigits != %d \n",
               maxTrunkNumLen);
        return !ITS_SUCCESS;
    }
    ...
    {
        printf("ISUPTEST: CircuitIdentificationName officeA len != %d \n",
               maxTrunkNumLen);
        return !ITS_SUCCESS;
    }
...
ReadCircuitValRespIndPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    {
        printf("ISUPTEST: CircuitValidationResponseIndicator plen != %d \n",
               maxplen); 
        return !ITS_SUCCESS;
    }
...
ReadClosedUserGroupInterlockCodePar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    {
        printf("ISUPTEST: ClosedUserGroupInterlockCode digits len != %d \n",
               dLen);
        return !ITS_SUCCESS;
    }
    ...
    {
        printf("ISUPTEST: ClosedUserGroupInterlockCode code length != %d \n",
               cLen);
        return !ITS_SUCCESS;
    }
...
ReadConnectedNumPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    {
        printf("ISUPTEST: ConnectedNumber: no. of digits octets > %d \n",
               MAX_NB_OCTETS_DIGITS);
        ret = !ITS_SUCCESS;
    }
    else
...
ReadConnectRequestPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != maxplen)
    {
        printf("ISUPTEST: ConnectionRequest plen > %d \n", maxplen);
        return !ITS_SUCCESS;
    }
    ...
    {
        printf("ISUPTEST: ConnectionRequest call identity len != %d \n",
               ciLen);
        return !ITS_SUCCESS;
    }
    ...
    {
        printf("ISUPTEST: ConnectionRequest pt code len != %d \n", pcLen);
        return !ITS_SUCCESS;
    }

    GetOctetsFromStr(&(ie->param_data.connectionRequest.point_code[0]),
                     (int)lenPointCode);
...
ReadContinuityIndPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: ContinuityIndicator, plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadEchoControlInfoPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: EchoControlInformation plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadEventInfoPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: EventInformation plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadFacilityIndPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: FacilityIndicator plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadForwardCallIndPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: ForwardCallIndicators plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadGenericDigitsPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    {
        printf("ISUPTEST: GenericDigits max num of octets > %d \n",
               maxNoOctets);
        ret = !ITS_SUCCESS;
    }
    else
...
ReadGenericNotificationPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if (ie->param_length != plen)
    {
        printf("ISUPTEST: GenericNotification plen != %d", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadGenericReferencePar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    while (nOctetsToRead > 0);
    {
        ...
        {
            printf("ISUPTEST: GenericReference, reference len > %d \n",
                   maxRefOctets);
            return !ITS_SUCCESS;
        }
        ...
        if (nOctetsToRead < 0)
        {
            printf("ISUPTEST: GenericReference, missing data in file \n");
            ret = !ITS_SUCCESS;
        }
    }
...
ReadHopCounterPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    printf("\n");
    printf("ISUPTEST: HopCounter reserved param not read. \n");
...
ReadInfoIndPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: InformationIndicator plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadInfoReqIndPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: InformationRequestIndicator plen != %d \n",
               plen);
        ret = !ITS_SUCCESS;
    }
    else
...
ReadJurisdictionInfoPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != maxplen)
    {
        printf("ISUPTEST: Jurisdiction plen > %d \n", maxplen);
        return !ITS_SUCCESS;
    }
...
ReadLocationNumberPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    {
        printf("ISUPTEST: LocationNumber no. of octets for digits > %d \n",
               MAX_NB_OCTETS_DIGITS);
        ret = !ITS_SUCCESS;
    }
    else
...
int ReadMCIDRequestIndPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: MCIDRequestIndicator plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
int ReadMCIDResponseIndPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: MCIDResponseIndicator plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadMLPPPrecedencePar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != maxplen)
    {
        printf("ISUPTEST: MLPPPrecedence plen != %d \n", maxplen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadNatureOfConnectionIndPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: NatureOfConnection plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadNetworkSpecificFacilityPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    {
        /* 1b - 1m) network id */
        GetOctetsFromStr(&nNetwId, 1);
        if ((int)nNetwId > maxNoNetwId)
        {
            printf("ISUPTEST: No. of network ids > %d \n", maxNoNetwId);
            return !ITS_SUCCESS;
        }

        GetOctetsFromStr(&(ie->param_data.networkSpecificFacility. \
                           ntwrk_id[0]), (int)nNetwId);
    }
    ...
    {
        printf("ISUPTEST: NetworkSpecificFacility no. of facility > %d \n",
                maxNoNetwFacility);
        return !ITS_SUCCESS;
    }
...
ReadNetworkTransportPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    {
        printf("ISUPTEST: NetworkTransport plen > %d \n", maxplen);
        return !ITS_SUCCESS;
    }
...
ReadNotificationIndPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != maxplen)
    {
        printf("ISUPTEST: NotificationIndicator plen > %d \n", maxplen);
        return !ITS_SUCCESS;
    }
...
ReadOptBackwardCallIndPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: OptionalBackwardCallIndicator plen != %d \n",
               plen);
        ret = !ITS_SUCCESS;
    }
    else 
...
ReadOptionalForwardCallIndPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: OptionalForwardCallIndicator plen != %d \n",
               plen);
        ret = !ITS_SUCCESS;
    }
    else
...
ReadOrigLineInfoPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != maxplen)
    {
        printf("ISUPTEST: OriginatingLineInfo plen > %d \n", maxplen);
        return !ITS_SUCCESS;
    }
...
ReadParamCompInfoPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    GetOctetsFromStr(&ie->param_data.paramCompatInfo.param_info[i].paramName,
                     (int)ie->param_length);
...
ReadPropagationDelayCounterPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: PropagationDelayCounter plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
int ReadRedirectionNumRestrictPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: RedirectionNumberRestriction plen != %d \n",
               plen);
        ret = !ITS_SUCCESS;
    }
    else
...
ReadRemoteOperationsPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length > 1)
    {
        ...
        {
            printf("ISUPTEST: RemoteOperations components length > %d \n",
                   maxCompLen);
            return !ITS_SUCCESS;
        }
...
ReadServiceActivationPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length > maxplen)
    {
        printf("ISUPTEST: ServiceActivation plen > %d \n", maxplen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadServiceCodePar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: ServiceCode plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadSignallingPointCodePar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: SignallingPointCode plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadSpecialProcessReqPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: SpecialProcessingRequest plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadSuspendResumeIndPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: SuspendResumeIndicator plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadTransactionReqPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    {
        printf("ISUPTEST: TransactionRequest SCCP addr len > %d \n",
               SCCP_MAX_ADDR_LEN);
        result = !ITS_SUCCESS;
    }
...
ReadTransmissionMediumRequirementPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: TransmissionMediumRequirement plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadTransmissionMediumRequirementPrimePar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: TransmissionMediumRequirementPrime plen != %d \n",
               plen);
        ret = !ITS_SUCCESS;
    }
    else
...
ReadTransmissionMediumUsedPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: TransmissionMediumUsed plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
{
    int        varplen = 1;
    ITS_OCTET  lenLayer1 = 0x00;
    ITS_OCTET  lenLayer2 = 0x00;
    ITS_OCTET  lenLayer3 = 0x00;
...
/* TBD: Fix me */
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: UserTeleserviceInformation plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadUserToUserIndPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length != plen)
    {
        printf("ISUPTEST: UserToUserIndicator plen != %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
...
ReadUserToUserInfoPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length > maxplen)
    {
        printf("ISUPTEST: UserToUserInfo: param len > %d \n", maxplen);
        ret = !ITS_SUCCESS;
    }
    else
    {
        ...
        {
            printf("ISUPTEST: UserToUserInfo: info len > %d \n", maxplen - 1);
            ret = !ITS_SUCCESS;
        }
        else
        {
            ...
            int *ieCount)
{
    ...
    int        parCode = 0;
    int        nFields = 0;
    char       *p = NULL, typeOfInfo, buf[100] = "";
    FILE       *fp = NULL;
    ITS_OCTET  mType = 0;

    printf("ISUPTEST: start ReadISUPMsg(). \n");
    *ieCount = 0;                    /* no. of IEs */
    ...
    if ( fp )
    {
        printf("ISUPTEST: %s opened. \n", filename);
    }
    else
    {
        printf("ISUPTEST: Failed to open %s. \n", filename);
        return !ITS_SUCCESS;
    }
    ...
    printf("ISUPTEST: Found message data.\n"); 
    while ( !feof(fp) )
    {
        ...
        printf("ISUPTEST: parCode = 0x%02x \n", parCode);
        switch ( parCode )
        {
...
*/
        ...
        default:
            printf("ISUPTEST: Unrecognized parameter. \n");
        }
    }
