 ...
 * LOG: Start PR6.
 * LOG:
 * LOG: Revision 1.2  2001/08/16 15:47:19  lbana
 * LOG: resolve symbols from global symbol table, and fix compiler errors in thread api
 * LOG:
 * LOG: Revision 1.1  2001/05/04 18:30:28  mmiers
 ...
 * LOG: Begin PR5
 * LOG:
 * LOG: Revision 1.4  2001/04/26 15:14:57  mmiers
 ...
 * LOG:
 * LOG: Revision 1.1  2001/03/05 23:55:23  mmiers
 * LOG: New file for this platform.
 * LOG:
 * LOG: Revision 1.1  2001/02/08 22:53:15  mmiers
 * LOG: Finish up the platforms.  VxWorks is left, but that's a new
 * LOG: implementation anyway.
 * LOG:
...
#ident "$Id: its_dlsym.c,v 9.1 2005/03/23 12:53:46 cvsadmin Exp $"
...
extern DLSYM_Link  *DLSYM_Links;

/*
 * System global symbol table ID
 */
...
Create(ITS_Object obj, va_list args)
{
    ...
    DLSYM_MAP(manager) = DLSYM_Links;
    /* Symbol table id is the same for all loaded modules. */
    DLSYM_HANDLE(manager) = sysSymTbl;
...
Resolve(DLSYM_Manager *manager, const char *symName)
{
    ...
    for (links = DLSYM_MAP(manager); links->symName != NULL; links++)
    {
        if (strcmp(links->symName, symName) == 0)
        {
            if(symFindByName(DLSYM_HANDLE(manager), links->symName, 
                              (char**)&links->value, 0) != ERROR) 
            {
                return ((ITS_DLSYM)links->value);
            }
        }
    }
