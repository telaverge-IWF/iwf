 ...
 * LOG: Start PR6.
 * LOG:
 * LOG: Revision 4.1  2001/05/04 18:30:28  mmiers
 * LOG: Move these back.  Support can't be dynamically loaded due to
 * LOG: WIN32.
 * LOG:
 * LOG: Revision 4.3  2001/05/04 15:03:06  mmiers
 * LOG: Allow NULL DLL names.
 * LOG:
 * LOG: Revision 4.2  2001/05/01 18:41:35  mmiers
 * LOG: Correct signature.
 * LOG:
 ...
 * LOG: Start debugging.
 * LOG:
 * LOG: Revision 1.2  2001/03/08 23:18:29  mmiers
 ...
 * LOG:
 * LOG: Revision 1.2  2001/02/08 22:48:56  mmiers
...
#ident "$Id: its_dlsym.c,v 9.1 2005/03/23 12:53:42 cvsadmin Exp $"
...
Create(ITS_Object obj, va_list args)
{
    ...
    if (dllName != NULL)
    {
        DLSYM_HANDLE(manager) = LoadLibrary(dllName);
    }
    else
    {
        DLSYM_HANDLE(manager) = GetModuleHandle(NULL);
    }
    ...
    if (node == NULL)
    {
        FreeLibrary(DLSYM_HANDLE(manager));
...
{
    return GetProcAddress(DLSYM_HANDLE(manager), symName);
}
