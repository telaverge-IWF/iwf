 ...
 * LOG:
 * LOG: Revision 3.1  2000/08/16 00:05:59  mmiers
 * LOG:
 * LOG: Begin round 4.
 * LOG:
 * LOG: Revision 1.1  2000/01/06 02:45:13  mmiers
 * LOG:
 * LOG: Get the platform name right for HPUX.
 * LOG:
 * LOG: Revision 1.1  1999/12/16 16:16:16  mmiers
 * LOG: Sanitize the build environment a bit.
 * LOG:
 * LOG: Revision 2.0  1999/12/03 23:27:59  mmiers
 * LOG:
 * LOG: Begin third iteration.
 * LOG:
 * LOG: Revision 1.10  1999/10/12 19:00:31  mmiers
 * LOG:
 * LOG:
 ...
 * LOG:
 * LOG: Revision 1.6  1999/08/23 01:25:47  mmiers
 * LOG:
 * LOG:
 * LOG: Correct function linkage.
 * LOG:
 * LOG: Revision 1.5  1999/08/22 01:20:08  mmiers
 * LOG:
 * LOG:
...
#ident "$Id: its_serv.c,v 9.1 2005/03/23 12:53:43 cvsadmin Exp $"
...
SERVICE_RunAsService()
{
    ...
    else if (pid == 0)
    {
         ...
         * set signal handlers
         */
        action.sv_handler = _Handler;
        action.sv_flags = SA_RESTART;
        action.sv_mask = 0;
        sigvector(SIGKILL, &action, NULL);
        sigvector(SIGTERM, &action, NULL);
        sigvector(SIGSTOP, &action, NULL);
        sigvector(SIGCONT, &action, NULL);
        sigvector(SIGHUP, &action, NULL);
        action.sv_handler = _ControlHandler;
        sigvector(SIGINT, &action, NULL);

        action.sv_handler = _CoreHandler;
        sigvector(SIGSEGV, &action, NULL);
        sigvector(SIGILL, &action, NULL);
        sigvector(SIGQUIT, &action, NULL);
        sigvector(SIGFPE, &action, NULL);
        sigvector(SIGBUS, &action, NULL);
        sigvector(SIGXCPU, &action, NULL);
        sigvector(SIGXFSZ, &action, NULL);
