 ...
 * LOG: Begin round 4.
 * LOG:
 * LOG: Revision 1.1  2000/01/06 02:45:11  mmiers
 * LOG:
 * LOG: Get the platform name right for HPUX.
 * LOG:
 * LOG: Revision 1.1  1999/12/16 16:16:14  mmiers
 * LOG: Sanitize the build environment a bit.
 * LOG:
 * LOG: Revision 2.0  1999/12/03 23:27:35  mmiers
 * LOG:
 * LOG: Begin third iteration.
 * LOG:
 * LOG: Revision 1.5  1999/02/25 22:57:10  mmiers
 * LOG:
 * LOG:
 * LOG: Update for C++ compilation.
 * LOG:
 * LOG: Revision 1.4  1998/11/09 23:08:02  rsonak
 * LOG: ReadFIFO, not DeleteFIFO
 * LOG:
 * LOG: Revision 1.3  1998/09/23 23:12:59  jrao
 * LOG: Finish adding trace.
 * LOG:
 * LOG: Revision 1.2  1998/09/23 22:17:39  jrao
 * LOG: Add alpha support, fix HPUX support.
 * LOG:
 * LOG: Revision 1.1  1998/09/22 16:01:30  jrao
...
#ident "$Id: its_fifo.c,v 9.1 2005/03/23 12:53:43 cvsadmin Exp $"
...
FIFO_CreateFifo(ITS_FIFO *f, char *name, ITS_FIFO_MODE mode)
{
    ...
    {
        ITS_TRACE_ERROR(("FIFO_CreateFIFO: mkfifo failed errno %d\n", errno));
    ...
    if (mode == ITS_FIFO_READER)
    {
        ...
        {
            ITS_TRACE_ERROR(("FIFO_CreateFIFO: open failed errno %d\n",
                             errno));
...
FIFO_PeekFifo(ITS_FIFO *f, char *buf, int size)
{
    ...
    if (select(*f + 1, (int *)&rset, NULL, NULL, &tv) > 0)
    {
        return (read(*f, buf, size) == size ? ITS_SUCCESS : ITS_ERCVFAIL);
    }
