 ...
 ****************************************************************************
 *
 * LOG: $Log: crlicense.c,v $
 * LOG: Revision 9.3.34.1.12.1.6.1.2.1  2014/12/04 12:45:37  pramana
 * LOG: Added help strings in create license tool for iwf translators
 * LOG:
 * LOG: Revision 9.3.34.1.12.1.6.1  2014/09/19 07:18:53  jkchaitanya
 * LOG: changes for translator specific license validation and alarm implementation in IWF application
 * LOG:
 * LOG: Revision 9.3.34.1.12.1  2014/02/12 10:43:23  jsarvesh
 * LOG: Changes done for TPS Based Licensing
 * LOG:
 * LOG: Revision 9.3.34.1  2011/04/13 07:37:34  nvijikumar
 ...
 * LOG: RANAP is added.
 * LOG:
 * LOG: Revision 9.2  2005/04/06 09:41:57  mmanikandan
 * LOG: Bug Fix Propagation for -e option.
 * LOG:
 ...
 * LOG:
 * LOG: Revision 7.2.2.2.6.1.8.1  2004/12/16 03:30:36  randresol
 * LOG: Add JAPAN variant implementation
 * LOG:
 * LOG: Revision 7.2.2.2.6.1.4.1  2004/10/25 20:39:59  randresol
 * LOG: Start Implementation for Japan SS7
 * LOG:
 * LOG: Revision 7.2.2.2.6.1  2004/07/26 06:54:50  mmanikandan
 * LOG: License support for BSSAP.
 * LOG:
 * LOG: Revision 7.2.2.2  2003/07/07 12:15:44  mmanikandan
 * LOG: License capability is added for IS652, IS653, IS826, CAMEL, WIN, JSTD.
 * LOG:
 * LOG: Revision 7.2.2.1  2003/06/16 11:13:03  sjaddu
 ...
 * LOG: Begin PR6.2
 * LOG:
 * LOG: Revision 6.3  2002/07/03 16:51:38  mmiers
...
#ident "$Id: crlicense.c,v 9.3.34.1.12.1.6.1.2.1 2014/12/04 12:45:37 pramana Exp $"
 ...
 ****************************************************************************
 *  Purpose:
 *      Calculate the date from the given string. The format of the string
 *      is "yyyymmddhhmmss". 
 *
 *  Input Parameters:
 *      string for which the date to be calulated.
 *
 ...
 *  --------------------------------------------------------------------------
 *  Mani   06-04-2005    EXCEL branch   Bug - Date expiration mentioned with 
 *                                     -e option is not working is propagated.
 *
 ****************************************************************************/
...
static void
getMsgRate(char *string)
{
    license.msg_rate = atoi(string);
}
static void
...
handleLongOptions(char *option, char *second, int *argc)
{
    ...
    else if (strcmp(option, "camel") == 0)
    {
        LICENSE_SET_CAPABILITY(ITS_LIC_CAP_CAMEL, license);
    }
    else if (strcmp(option, "is652") == 0)
    {
        LICENSE_SET_CAPABILITY(ITS_LIC_CAP_IS652, license);
    }
    else if (strcmp(option, "is653") == 0)
    {
        LICENSE_SET_CAPABILITY(ITS_LIC_CAP_IS653, license);
    }
    else if (strcmp(option, "is826") == 0)
    {
        LICENSE_SET_CAPABILITY(ITS_LIC_CAP_IS826, license);
    }
    else if (strcmp(option, "win") == 0)
    {
        LICENSE_SET_CAPABILITY(ITS_LIC_CAP_WIN, license);
    }
    else if (strcmp(option, "jstd") == 0)
    {
        LICENSE_SET_CAPABILITY(ITS_LIC_CAP_JSTD, license);
    }
    else if (strcmp(option, "bssap") == 0)
    {
        LICENSE_SET_CAPABILITY(ITS_LIC_CAP_BSSAP, license);
    }
    else if (strcmp(option, "diameter") == 0)
    {
        LICENSE_SET_CAPABILITY(ITS_LIC_CAP_DIAMETER, license);
    }
    else if (strcmp(option, "dcca") == 0)
    {
        LICENSE_SET_CAPABILITY(ITS_LIC_CAP_DIA_DCCA, license);
    }
    else if (strcmp(option, "ranap") == 0)
    {
        LICENSE_SET_CAPABILITY(ITS_LIC_CAP_RAN, license);
    }
...
main(int argc, char **argv)
{
    ...
    if (argc == 1)
    {
        ...
        printf("          [--lnp --is651 --camel --is652 --is653 --is826 --win --jstd --bssap]\n");
        printf("          [--bssap --ranap]\n");
        printf("          [--diameter]\n");
        printf("          [--dcca]\n");
        printf("          [--iwf [caprotsl|capro|maps6a|s6amap|all]]\n");
        printf("\n"); 
        printf("  where:\n");
        ...
        printf("          --is634 enable IS634 application protocol\n");
        ...
        printf("          --is651 enable IS651 application protocol\n");
        printf("          --camel enable CAMEL application protocol\n");
        printf("          --is652 enable IS652 application protocol\n");
        printf("          --is653 enable IS653 application protocol\n");
        printf("          --is826 enable IS826 application protocol\n");
        printf("          --win enable WIN application protocol\n");
        printf("          --jstd enable JSTD36 application protocol\n");
        printf("          --bssap enable BSSAP application protocol\n");
        printf("          --diameter enable Diameter application protocol\n");
        printf("          --dcca enable DCCA application protocol\n");
        printf("          --ranap enable RANAP application protocol\n");
        printf("          --iwf enable IWF translator(s)\n");
        return (0);
    }

    memset(license.capabilities, 0, sizeof(license.capabilities));
