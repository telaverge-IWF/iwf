 ...
 * LOG:
 * LOG: Revision 1.5  1998/10/21 04:30:46  mmiers
 ...
 * LOG:
 * LOG: Revision 1.3  1998/10/20 23:32:52  mmiers
...
#ident "$Id: ode2oid.c,v 9.1 2005/03/23 12:53:02 cvsadmin Exp $"
...
preloadcache (char *str)
{
    ...
    while ((io = getisobject ()) != NULL)
    {
        if (strcmp (str, io->io_descriptor) == 0)
        {
            if ((cp->oid = oid_cpy (&io->io_identity)) == NULL ||
...
ode2oid (char *descriptor)
{
    ...
    cpn->ref = 1;
    if ((cpn->oid = oid_cpy (&io->io_identity)) == NULL ||
        (cpn->descriptor = (char *)malloc ((unsigned) (strlen (descriptor) + 1)))
         == NULL)
    {
        if (cpn->oid)
        {
