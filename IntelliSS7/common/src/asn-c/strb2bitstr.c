 ...
 * LOG: Begin PR7.
 * LOG:
 * LOG: Revision 5.2  2001/10/31 23:06:51  mmiers
 ...
 * LOG:
 * LOG: Revision 2.0  1999/12/03 23:26:48  mmiers
 * LOG:
 * LOG: Begin third iteration.
 * LOG:
 * LOG: Revision 1.1  1998/10/27 01:16:11  mmiers
 ...
 * LOG:
 * LOG: Revision 1.4  1998/10/21 02:41:20  mmiers
...
#ident "$Id: strb2bitstr.c,v 9.1 2005/03/23 12:53:04 cvsadmin Exp $"

PE
strb2bitstr (char *cp, int len, PElementClass classOf, PElementID id)
{
     int    i, j, bit, mask;
     PE	    p;

    if ((p = pe_alloc (classOf, PE_FORM_PRIM, id)) == NULL)
    {
    	return NULL;
    }

    p = prim2bit (p);
    if (len > 0 && pbit_off (p, len - 1) == NOTOK)
    {
no_mem:
        pe_free (p);
