 ...
 * LOG:
 * LOG: Revision 1.2  1998/10/21 17:48:17  mmiers
...
#ident "$Id: str2vec.c,v 9.1 2005/03/23 12:53:04 cvsadmin Exp $"
	...
	*mask = 0;
	...
	if (brk > 0)
	{
		...
		*s++ = 0;
        ...
        {
	    while (isspace((ITS_OCTET) * s) || *s == comma)
            {
		*s++ = 0;
            }
        }
	...
	if (brk > 0)
	{
        ...
        {
	    for (s++; *s != 0 && !isspace((ITS_OCTET) * s) && *s != comma; s++)
            {
		continue;
            }
        }
