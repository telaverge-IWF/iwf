 ...
 * LOG: Compile with CC.
 * LOG:
 * LOG: Revision 1.1  1998/10/27 01:15:23  mmiers
 ...
 * LOG:
 * LOG: Revision 1.5  1998/10/21 04:30:43  mmiers
...
#ident "$Id: num2prim.c,v 9.1 2005/03/23 12:53:02 cvsadmin Exp $"

PE
num2prim (integer i, PElementClass pclass, PElementID id)
{
     integer mask, sign, n;
     PElementData dp;
     PE	    pe;
    ...
    sign = i >= 0 ? i : i ^ (-1);
    mask = ((integer)0x1ff) << (((n = sizeof i) - 1) * 8 - 1);
    while (n > 1 && (sign & mask) == 0)
    {
	mask >>= 8, n--;
    }

    if ((pe->pe_prim = PEDalloc (n)) == NULL)
    {
	pe_free (pe);
