 ...
 * LOG:
 * LOG: Revision 1.1  1998/10/27 01:16:06  mmiers
 ...
 * LOG:
 * LOG: Revision 1.5  1998/10/21 17:48:42  mmiers
 * LOG: Remove last vestiges of ISODE.
 * LOG:
 * LOG: Revision 1.4  1998/10/21 04:31:05  mmiers
 * LOG: Cleaning up pepsy.
 * LOG:
 * LOG: Revision 1.3  1998/10/21 02:41:16  mmiers
 * LOG: More cleanup.  Eliminated most of the extra pollution headers.
 * LOG:
 * LOG: Revision 1.2  1998/10/20 23:33:09  mmiers
...
#ident "$Id: sprintoid.c,v 9.1 2005/03/23 12:53:04 cvsadmin Exp $"

int
sprintoid (OID oid, char *buffer, int len)
{
    int    i;
    unsigned int  *ip;
    char  *bp, *cp;
    ...
    for (ip = oid->oid_elements, i = oid->oid_nelem, cp = "";
         ...
         ip++, cp = ".")
    {
        sprintf (tmp, "%s%u", cp, *ip);
        
        if ((int)(strlen(tmp) + (bp - buffer)) > len)
        {
