 ...
 * LOG:
 * LOG: Revision 1.9  1998/10/21 19:21:57  mmiers
 * LOG: Get the names right.
 * LOG:
 * LOG: Revision 1.8  1998/10/21 17:48:16  mmiers
 * LOG: Remove last vestiges of ISODE.
 * LOG:
 * LOG: Revision 1.7  1998/10/21 04:30:06  mmiers
 * LOG: Cleaning up pepsy.
 * LOG:
 * LOG: Revision 1.6  1998/10/20 23:31:13  mmiers
 * LOG: Cleanup, make this stuff reentrant.
 * LOG:
 *
 *****************************************************************************/
/*
...
#ident "$Id: general.c,v 9.1 2005/03/23 12:53:02 cvsadmin Exp $"

void
enqueue(struct qbuf* elem, struct qbuf* pred)
{
    if (pred->qb_forw)
    {
        pred->qb_forw->qb_back = elem;
    }
    elem->qb_forw = pred->qb_forw;
    elem->qb_back = pred;
    pred->qb_forw = elem;
}

void
dequeue(struct qbuf* elem)
{
    if (elem->qb_forw)
    {
        elem->qb_forw->qb_back = elem->qb_back;
    }
    elem->qb_back->qb_forw = elem->qb_forw;
}
