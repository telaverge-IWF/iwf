...
#ident "$Id: AsnSetOf.cpp,v 9.3.64.1 2014/09/16 09:34:53 jsarvesh Exp $"
...
int
AsnSetOf::EncodeAPER(Octets& octs,
                      const AsnOptions& asnOptions,
                      const AsnDescObject& description)
{
    bool extBit = false;
    ...
    if((_elementList.size() < lb) || (ub !=0 && extBit == false && (_elementList.size() > ub)))
    {
	ASN_THROW_ENCODE_ERROR(ASN_E_INVALID_SET_OF_SIZE);
    }
...
int 
AsnSetOf::DecodeAPER(Octets& octs,
                      const AsnOptions& asnOptions,
                      const AsnDescObject& description)
{
    bool extBit = false;
    ...
    if (!extBit)
    {
		...
		if(len > ub)
		{
		    ASN_THROW_DECODE_ERROR(ASN_E_INVALID_SET_OF_SIZE, octs);
		}
    ...
    //for semi constraint type, lb can be non zero & for len-lb ==64K, padding byte (0) skipped
    if((len-lb) == K64)
    {
	octs.GetNextOctet();
    }
