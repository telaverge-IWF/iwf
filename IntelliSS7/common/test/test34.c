...
static void
blockPrint (FILE *fp, const ITS_OCTET *block, int blockBits, const char *tag)
{
    int i;
...
static void
HexToBin (ITS_OCTET *binBlock, const ITS_OCTET *hexBlock, int blockLen)
{
    int j = 0, t;
    while (blockLen > 0)
    {
...
static void
rijndaelVKKAT (FILE *fp, int keyLength, int blockLength)
{
    int i, j, r;
    ITS_OCTET block[4*MAXBC];
    ITS_OCTET keyMaterial[320];
    ITS_OCTET byteVal = (ITS_OCTET)'8';
    ITS_BlockKey keyInst;
    ITS_BlockCipher cipherInst;
    ...
    for (i = 0; i < keyLength; i++)
    {
        ...
        r = AES_MakeKey(&keyInst, DIR_ENCRYPT, keyLength, keyMaterial);
        if (ITS_SUCCESS != r)
        {
            fprintf(stderr,"AES_MakeKey error %d\n",r);
            exit(-1);
        }
        fprintf (fp, "\nI=%d\n", i+1);
        fprintf (fp, "KEY=%s\n", keyMaterial);
        ...
        r = AES_CipherInit (&cipherInst, BLOCK_MODE_ECB, NULL);
        if (ITS_SUCCESS != r) {
            fprintf(stderr,"AES_CipherInit error %d\n",r);
            exit(-1);
        }
        r = AES_BlockEncrypt(&cipherInst, &keyInst, block, blockLength, block);
        if (blockLength != r) {
            fprintf(stderr,"AES_BlockEncrypt error %d\n",r);
            exit(-1);
        }
        blockPrint (fp, block, blockLength, "CT");
        /* now check decryption: */
        keyInst.blockLen = blockLength;
        AES_MakeKey(&keyInst, DIR_DECRYPT, keyLength, keyMaterial);
        AES_BlockDecrypt(&cipherInst, &keyInst, block, blockLength, block);
        for (j = 0; j < blockLength/8; j++) {
            assert (block[j] == 0);
        }
        /* undo changes for the next iteration: */
        keyMaterial[i/4] = (ITS_OCTET)'0';
        byteVal =
            (byteVal == '8') ?	'4' :
        (byteVal == '4') ?	'2' :
        (byteVal == '2') ?	'1' :
...
static void rijndaelVTKAT (FILE *fp, int keyLength, int blockLength)
{
	...
	memset (keyMaterial, '0', keyLength/4);
	keyInst.blockLen = blockLength;
	AES_MakeKey(&keyInst, DIR_ENCRYPT, keyLength, keyMaterial);
	fprintf (fp, "KEY=%s\n", keyMaterial);
	for (i = 0; i < blockLength; i++) {
		...
		blockPrint (fp, block, blockLength, "PT");
		cipherInst.blockLen = blockLength;
		AES_CipherInit (&cipherInst, BLOCK_MODE_ECB, NULL);
		AES_BlockEncrypt(&cipherInst, &keyInst, block, blockLength, block);
		blockPrint (fp, block, blockLength, "CT");
	}
#ifdef TRACE_KAT_MCT
	printf (" done.\n");
...
static void rijndaelTKAT (FILE *fp, int blockLength, int keyLength, FILE *in)
{
	int i, j;
	unsigned int s;
	ITS_OCTET block[4*MAXBC], block2[4*MAXBC];
	ITS_OCTET keyMaterial[320];
	ITS_BlockKey keyInst;
	ITS_BlockCipher cipherInst;
	...
	for(i = 0; i < 64; i++) {
		...
		cipherInst.blockLen = blockLength;
		AES_CipherInit (&cipherInst, BLOCK_MODE_ECB, NULL);
		AES_BlockEncrypt(&cipherInst, &keyInst, block, blockLength, block2);
	...
	for(i = 64; i < 128; i++) {
		...
		cipherInst.blockLen = blockLength;
		AES_CipherInit (&cipherInst, BLOCK_MODE_ECB, NULL);
		AES_BlockDecrypt(&cipherInst, &keyInst, block, blockLength, block2);
...
static void rijndaelIVKAT (FILE *fp, int keyLength, int blockLength)
{
	...
	AES_CipherInit (&cipherInst, BLOCK_MODE_ECB, NULL);
	for(i = 1; i < ROUNDS; i++) {
		AES_CipherUpdateRounds (&cipherInst, &keyInst, block, blockLength/8, block2, i);
		sprintf(format,"CT%d",i);
		blockPrint (fp, block2, blockLength, format);
	}
	AES_CipherUpdateRounds (&cipherInst, &keyInst, block, blockLength, block2, ROUNDS);
	blockPrint (fp, block2, blockLength, "CT");
	...
	AES_CipherInit (&cipherInst, BLOCK_MODE_ECB, NULL);
	for(i = 1; i < ROUNDS; i++) {
		AES_CipherUpdateRounds (&cipherInst, &keyInst, block2, blockLength,block, ROUNDS-i);
		sprintf(format,"PT%d",i);
		blockPrint (fp, block, blockLength, format);
	}
	AES_CipherUpdateRounds (&cipherInst, &keyInst, block2, blockLength, block, 0);
	blockPrint (fp, block, blockLength, "PT");
...
static void rijndaelECB_MCT (FILE *fp, const char *initKey, int keyLength,
	const char *initBlock, int blockLength, ITS_OCTET direction)
{
	int i, j;
	ITS_OCTET inBlock[4*MAXBC], outBlock[4*MAXBC], binKey[4*MAXKC];
	ITS_OCTET keyMaterial[320];
	ITS_BlockKey keyInst;
	ITS_BlockCipher cipherInst;
	...
	for (i = 0; i < REPS; i++) {
		...
		fprintf (fp, "KEY=%s\n", keyMaterial);
		keyInst.blockLen = blockLength;
		AES_MakeKey(&keyInst, direction, keyLength, keyMaterial);
		...
		if (direction == DIR_ENCRYPT) {
			for (j = 0; j < 10000; j++) {
				...
				AES_BlockEncrypt(&cipherInst, &keyInst, inBlock, blockLength, outBlock);
			}
		} else {
			for (j = 0; j < 10000; j++) {
				memcpy (inBlock, outBlock, blockLength/8);
				AES_BlockDecrypt(&cipherInst, &keyInst, inBlock, blockLength, outBlock);
			}
		}
...
static void rijndaelCBC_MCT (FILE *fp, const char *initKey, int keyLength,
	const char *initIV, const char *initBlock, int blockLength, ITS_OCTET direction)
{
	int i, j, r, t;
	ITS_OCTET inBlock[256/8], outBlock[256/8], binKey[256/8], cv[256/8];
	ITS_OCTET keyMaterial[320];
	ITS_OCTET iv[64+1];
	ITS_BlockKey keyInst;
	ITS_BlockCipher cipherInst;
	...
	for (i = 0; i < REPS; i++) {
		...
		fprintf (fp, "KEY=%s\n", keyMaterial);
		keyInst.blockLen = blockLength;
		r = AES_MakeKey(&keyInst, direction, keyLength, keyMaterial);
		if (ITS_SUCCESS != r) {
			fprintf(stderr,"AES_MakeKey error %d\n",r);
			exit(-1);
		}
		...
		if (direction == DIR_ENCRYPT) {
			for (j = 0; j < 10000; j++) {
				...
				cipherInst.blockLen = blockLength;
				r = AES_CipherInit (&cipherInst, BLOCK_MODE_CBC, iv);
				if (ITS_SUCCESS != r) {
					fprintf(stderr,"AES_CipherInit error %d\n",r);
					exit(-1);
				}
				r = AES_BlockEncrypt(&cipherInst, &keyInst, inBlock, blockLength, outBlock);
				if (blockLength != r) {
					fprintf(stderr,"AES_BlockEncrypt error %d\n",r);
					exit(-1);
				}
			...
			for (j = 0; j < 10000; j++) {
				...
				cipherInst.blockLen = blockLength;
				AES_CipherInit (&cipherInst, BLOCK_MODE_CBC, iv);
				AES_BlockDecrypt(&cipherInst, &keyInst, inBlock, blockLength, outBlock);
