...
int TEST_SendGSMMAPInvoke(int, ITS_HANDLE);
int TEST_DecodeGSMMAPResult( TCAP_CPT*, ITS_HANDLE);
int TEST_WaitForResponse(ITS_HANDLE);
...
Dispatch(void *arg)
{
	...
	dispatch_handle = ITS_Initialize(ITS_SCCP | ITS_TCAP | ITS_TRANSPORT_FIFO, 2);
...
main(int argc, char** argv)
{
    ...
    int result, cmdInput = 0;

    printf("\nBegin GSM MAP V4 send example...\n");
    ...
    result = TEST_SendGSMMAPInvoke(cmdInput, handle);
    if (result == ITS_SUCCESS)
    {
    ...
    printf("\nEnd of GSM MAP V4 send example.\n");
    ...
    while(!stop)
    {
        ...
        if (TCAP_MSG_TYPE(&event) == ITS_TCAP_DLG)
        {
            ...
            case TCAP_PT_TC_UNI_CCITT:
                printf("\nUnidirectional dialogue Received.\n\n");
                /* Should be processed by complete code. */
                break;
            ...
            case TCAP_PT_TC_CONTINUE_CCITT:
                printf("\nContinue dialogue Received.\n\n");
                /* Should be processed by complete code. */
                break;

            case TCAP_PT_TC_P_ABORT_CCITT:
                printf("\nP-Abort dialogue Received.\n\n");
                /* Should be processed by complete code. */
                break;

            case TCAP_PT_TC_U_ABORT_CCITT:
                printf("\nU-Abort dialogue Received.\n\n");
                /* Should be processed by complete code. */
                break;

            case TCAP_PT_TC_NOTICE_CCITT:
                printf("\nNotice dialogue Received.\n\n");
                stop = 1;
                /* Should be processed by complete code. */
                break;
...
int
TEST_SendGSMMAPInvoke(int cmdInput, ITS_HANDLE handle)
{
    int ret = 0, c;
    TCAP_CPT cpt;
    ITS_EVENT event;    
    ...
    cpt.u.invoke.timeout = 100;  /* Set timeout of waiting for response */
