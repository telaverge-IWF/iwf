 ...
 *----------------------------------------------------------------------------*
 *
 *  ID: $Id: test_send_single_wt_main.c,v 9.1 2005/03/23 12:51:35 cvsadmin Exp $
 *
 *----------------------------------------------------------------------------*/
...
main(int argc, char** argv)
{
    ...
    APPL_SetName("test_send_single_wt");
...
TEST_DecodeGSMMAPResult( TCAP_CPT *cpt, ITS_HANDLE handle )
{
    ...
    switch(cpt->u.result.operation.data[2])
    {
        ...
        case (MAP_OP_SEND_IDENTIFICATION):
        case (MAP_OP_RESTORE_DATA):
        case (MAP_OP_ACTIVATE_TRACE_MODE):
        case (MAP_OP_DEACTIVATE_TRACE_MODE):
...
BuildOperationInsertSubscriberDataArg(ITS_OCTET** octetArray, size_t* size)
{
    ...
    if (res == ITS_E_ASN_ENCODE_ERROR)
    {
        ...
        encodeExtendedErrorText = 
            ASN_EncodeError_GetExtendedErrorText(encodeError);
