...
AsnDescObject* KeyStatus::GetStaticDescription()
{
    ...
    else
    {
        ...
        staticDesc.SetTypeName("KeyStatus");
        result = &staticDesc;
        initialized = true;
        Asn::Unlock();
        return result;
...
long KeyStatus::Get(const std::string& valueName) const
{
    if (valueName == "old")
    {
        return 0;
    }
    else if (valueName == "new")
    {
        return 1;
    }
    else
...
std::string KeyStatus::Get(long value) const
{
    if (value == 0)
    {
        return "old";
...
std::string KeyStatus::GetExt(long value) const
{
    return "";
}
