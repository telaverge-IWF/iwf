//
// Generated by IntelliAsnCC ASN.1 compiler (C, C++, Java).
//
// Do not edit!
//
//
//
// (C) 2000, 2001 IntelliNet Technologies Inc. All Rights Reserved.
//
//


#include <map_v13_job_type.h>

#include <AsnDescEnumerated.h>


using namespace its;
using namespace map_v13;



AsnDescObject* JobType::GetStaticDescription()
{
    static volatile bool initialized = false;
    static AsnDescObject* result = NULL;
    static AsnDescEnumerated staticDesc;

    if (initialized)
    {
        return result;
    }

    Asn::Lock();

    if (initialized)
    {
        Asn::Unlock();
        return result;
    }
    else
    {
        staticDesc.SetCloneForFactory(new JobType(&staticDesc));
        staticDesc.TagAddLevel(new Tag(Tag::ASN_CLASS_UNIVERSAL, Tag::ASN_PRIMITIVE, 10));
        staticDesc._valueSet.insert(0);
        staticDesc._valueSet.insert(1);
        staticDesc._valueSet.insert(2);
        staticDesc._valueSet.insert(3);
        static AsnDescEnumerated::NamedNumber namedNumberList[] = {{"immediate-MDT-only", 0}, {"logged-MDT-only", 1}, {"trace-only", 2}, {"immediate-MDT-and-trace", 3}, {NULL, 0}};
        staticDesc.SetNamedNumberList(namedNumberList);
        static AsnDescEnumerated::NamedNumber namedNumberExtList[] = {{NULL, 0}};
        staticDesc.SetNamedNumberExtList(namedNumberExtList);
        staticDesc.SetTypeName("JobType");
        result = &staticDesc;
        initialized = true;
        Asn::Unlock();
        return result;
    }
}

long JobType::Get(const std::string& valueName) const
{
    if (valueName == "immediate-MDT-only")
    {
        return 0;
    }
    else if (valueName == "logged-MDT-only")
    {
        return 1;
    }
    else if (valueName == "trace-only")
    {
        return 2;
    }
    else if (valueName == "immediate-MDT-and-trace")
    {
        return 3;
    }
    else
    {
        bool invalid_value_name = false;
        ITS_ASSERT(invalid_value_name);

        return 0;
    }
}

std::string JobType::Get(long value) const
{
    if (value == 0)
    {
        return "immediate-MDT-only";
    }
    else if (value == 1)
    {
        return "logged-MDT-only";
    }
    else if (value == 2)
    {
        return "trace-only";
    }
    else if (value == 3)
    {
        return "immediate-MDT-and-trace";
    }
    else
    {
        bool invalid_value = false;
        ITS_ASSERT(invalid_value);

        return "";
    }
}

std::string JobType::GetExt(long value) const
{
    return "";
}





