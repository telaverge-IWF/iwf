...
#include <map_v13_resource_unavailable_reason.h>
...
AsnDescObject* ResourceUnavailableReason::GetStaticDescription()
{
    ...
    else
    {
        ...
        staticDesc.SetTypeName("ResourceUnavailableReason");
        result = &staticDesc;
        initialized = true;
        Asn::Unlock();
        return result;
...
long ResourceUnavailableReason::Get(const std::string& valueName) const
{
    if (valueName == "shortTermResourceLimitation")
    {
        return 0;
    }
    else if (valueName == "longTermResourceLimitation")
    {
        return 1;
    }
    else
...
std::string ResourceUnavailableReason::Get(long value) const
{
    if (value == 0)
    {
...
std::string ResourceUnavailableReason::GetExt(long value) const
{
    return "";
}
