//
// Generated by IntelliAsnCC ASN.1 compiler (C, C++, Java).
//
// Do not edit!
//
//
//
// (C) 2000, 2001 IntelliNet Technologies Inc. All Rights Reserved.
//
//


#include <map_v9_reason.h>

#include <AsnDescEnumerated.h>


using namespace its;
using namespace map_v9;



AsnDescObject* Reason::GetStaticDescription()
{
    static volatile bool initialized = false;
    static AsnDescObject* result = NULL;
    static AsnDescEnumerated staticDesc;

    if (initialized)
    {
        return result;
    }

    Asn::Lock();

    if (initialized)
    {
        Asn::Unlock();
        return result;
    }
    else
    {
        if (CheckLicense(ITS_LIC_CAP_GSMMAP) != ITS_TRUE)
        {
            Asn::Unlock();
            exit(0);
        }

        staticDesc.SetCloneForFactory(new Reason(&staticDesc));
        staticDesc.TagAddLevel(new Tag(Tag::ASN_CLASS_UNIVERSAL, Tag::ASN_PRIMITIVE, 10));
        staticDesc._valueSet.insert(0);
        staticDesc._valueSet.insert(1);
        staticDesc._valueSet.insert(2);
        staticDesc._valueSet.insert(3);
        staticDesc._valueSet.insert(4);
        static AsnDescEnumerated::NamedNumber namedNumberList[] = {{"noReasonGiven", 0}, {"invalidDestinationReference", 1}, {"invalidOriginatingReference", 2}, {"encapsulatedAC-NotSupported", 3}, {"transportProtectionNotAdequate", 4}, {NULL, 0}};
        staticDesc.SetNamedNumberList(namedNumberList);
        static AsnDescEnumerated::NamedNumber namedNumberExtList[] = {{NULL, 0}};
        staticDesc.SetNamedNumberExtList(namedNumberExtList);
        staticDesc.SetTypeName("Reason");
        result = &staticDesc;
        initialized = true;
        Asn::Unlock();
        return result;
    }
}

long Reason::Get(const std::string& valueName) const
{
    if (valueName == "noReasonGiven")
    {
        return 0;
    }
    else if (valueName == "invalidDestinationReference")
    {
        return 1;
    }
    else if (valueName == "invalidOriginatingReference")
    {
        return 2;
    }
    else if (valueName == "encapsulatedAC-NotSupported")
    {
        return 3;
    }
    else if (valueName == "transportProtectionNotAdequate")
    {
        return 4;
    }
    else
    {
        bool invalid_value_name = false;
        ITS_ASSERT(invalid_value_name);

        return 0;
    }
}

std::string Reason::Get(long value) const
{
    if (value == 0)
    {
        return "noReasonGiven";
    }
    else if (value == 1)
    {
        return "invalidDestinationReference";
    }
    else if (value == 2)
    {
        return "invalidOriginatingReference";
    }
    else if (value == 3)
    {
        return "encapsulatedAC-NotSupported";
    }
    else if (value == 4)
    {
        return "transportProtectionNotAdequate";
    }
    else
    {
        bool invalid_value = false;
        ITS_ASSERT(invalid_value);

        return "";
    }
}

std::string Reason::GetExt(long value) const
{
    return "";
}





