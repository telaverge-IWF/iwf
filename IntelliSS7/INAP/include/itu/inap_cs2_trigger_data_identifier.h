//
// Generated by IntelliAsnCC ASN.1 compiler (C, C++, Java).
//
// Do not edit!
//
//
//
// (C) 2000, 2001 IntelliNet Technologies Inc. All Rights Reserved.
//
//


#if !defined(_INAP_CS2_TRIGGER_DATA_IDENTIFIER_H_)
#define _INAP_CS2_TRIGGER_DATA_IDENTIFIER_H_

#if _MSC_VER >= 1000
#pragma once
#endif // _MSC_VER >= 1000

#include <Asn.h>
#include <AsnObject.h>
#include <AsnDescObject.h>
#include <AsnError.h>

#include <AsnSequence.h>
#include <AsnSequenceOf.h>
#include <AsnEnumerated.h>
#include <AsnChoice.h>

#include <inap_cs2_event_type_bcsm.h>
#include <inap_cs2_profile_identifier.h>
#include <inap_cs2_extension_field.h>



namespace inap_cs2 {



class TriggerDataIdentifier : public its::AsnSequence
{
public:


////////// Begin Nested Class(es) //////////



    class Extensions : public its::AsnSequenceOf
    {
    public:


    ////////// Begin Nested Class //////////


            typedef inap_cs2::ExtensionField ExtensionField;


    ////////// End Nested Class //////////

        Extensions() : its::AsnSequenceOf(false)
        {
            Initialize();
        }

        Extensions(its::Octets& octets) : its::AsnSequenceOf(false)
        {
            Initialize();
            Decode(octets);
        }

        Extensions(const Extensions& rhs) : its::AsnSequenceOf(rhs)
        {
            // Nothing to do.
        }

        Extensions(its::AsnDescObject* description) : its::AsnSequenceOf(description)
        {
            // Nothing to do.
        }

    protected:

        void Initialize()
        {
            _description = GetStaticDescription();
        }

    public:

        static its::AsnDescObject* GetStaticDescription();

        static its::AsnDescObject* GetExtensionFieldStaticDescription();

    public:

        virtual ~Extensions()
        {
            // Nothing to do.
        }

        virtual its::AsnObject* Clone() const
        {
            return new Extensions(*this);
        }

        virtual std::string GetName() const
        { return "Extensions"; }

        void AddElement(ExtensionField* element)
        {
            ITS_REQUIRE(element != NULL);
            element->AsnObject::SetDescription(GetExtensionFieldStaticDescription());
            AsnSequenceOf::AddElement(element);
        }

        void AddElement(const ExtensionField& element)
        {
            its::AsnObject& asnObject = const_cast<ExtensionField&>(element);
            asnObject.SetDescription(GetExtensionFieldStaticDescription());
            AsnSequenceOf::AddElement(element);
        }

        const ExtensionField& ElementAt(int index) const
        {
            return static_cast<const ExtensionField&> (AsnSequenceOf::ElementAt(index));
        }

    };


        typedef inap_cs2::EventTypeBCSM EventTypeBCSM;


        typedef inap_cs2::ProfileIdentifier ProfileIdentifier;


////////// End Nested Class(es) //////////

    TriggerDataIdentifier() : its::AsnSequence(3, false)
    {
        Initialize();
    }

    TriggerDataIdentifier(its::Octets& octets) : its::AsnSequence(3, false)
    {
        Initialize();
        Decode(octets);
    }

    TriggerDataIdentifier(const TriggerDataIdentifier& rhs) : its::AsnSequence(rhs)
    {
        // Nothing to do.
    }

    TriggerDataIdentifier(its::AsnDescObject* description) : its::AsnSequence(3, description)
    {
        // Nothing to do.
    }

protected:

    void Initialize()
    {
        _description = GetStaticDescription();
    }

public:

    static its::AsnDescObject* GetStaticDescription();

    static its::AsnDescObject* GetTriggerIDStaticDescription();
    static its::AsnDescObject* GetProfileIdentifierStaticDescription();
    static its::AsnDescObject* GetExtensionsStaticDescription();

public:

    virtual ~TriggerDataIdentifier()
    {
        // Nothing to do.
    }

    virtual its::AsnObject* Clone() const
    {
        return new TriggerDataIdentifier(*this);
    }

    virtual std::string GetName() const
    { return "TriggerDataIdentifier"; }

    void SetTriggerID(EventTypeBCSM* element)
    {
        ITS_REQUIRE(element != NULL);
        element->AsnObject::SetDescription(GetTriggerIDStaticDescription());
        AddElement(0, element);
    }

    void SetTriggerID(const EventTypeBCSM& element)
    {
        its::AsnObject& asnObject = const_cast<EventTypeBCSM&>(element);
        asnObject.SetDescription(GetTriggerIDStaticDescription());
        AddElement(0, element);
    }

    const EventTypeBCSM& GetTriggerID() const
    {
        return static_cast<const EventTypeBCSM&> (ElementAt(0));
    }

    void SetProfileIdentifier(ProfileIdentifier* element)
    {
        ITS_REQUIRE(element != NULL);
        element->AsnObject::SetDescription(GetProfileIdentifierStaticDescription());
        AddElement(1, element);
    }

    void SetProfileIdentifier(const ProfileIdentifier& element)
    {
        its::AsnObject& asnObject = const_cast<ProfileIdentifier&>(element);
        asnObject.SetDescription(GetProfileIdentifierStaticDescription());
        AddElement(1, element);
    }

    const ProfileIdentifier& GetProfileIdentifier() const
    {
        return static_cast<const ProfileIdentifier&> (ElementAt(1));
    }

    void SetExtensions(Extensions* element)
    {
        ITS_REQUIRE(element != NULL);
        element->AsnObject::SetDescription(GetExtensionsStaticDescription());
        AddElement(2, element);
    }

    void SetExtensions(const Extensions& element)
    {
        its::AsnObject& asnObject = const_cast<Extensions&>(element);
        asnObject.SetDescription(GetExtensionsStaticDescription());
        AddElement(2, element);
    }

    bool OptionExtensions() const
    {
        return Contains(2);
    }

    const Extensions& GetExtensions() const
    {
        ITS_REQUIRE(OptionExtensions());
        return static_cast<const Extensions&> (ElementAt(2));
    }

};



} // namespace.

#endif    // !defined(_INAP_CS2_TRIGGER_DATA_IDENTIFIER_H_)

