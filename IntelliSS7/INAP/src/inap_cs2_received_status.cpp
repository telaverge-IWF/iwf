...
AsnDescObject* ReceivedStatus::GetStaticDescription()
{
    ...
    else
    {
        ...
        staticDesc.SetTypeName("ReceivedStatus");
        result = &staticDesc;
        initialized = true;
        Asn::Unlock();
        return result;
...
long ReceivedStatus::Get(const std::string& valueName) const
{
    if (valueName == "messageComplete")
    {
        return 0;
    }
    else if (valueName == "messageInterrupted")
    {
        return 1;
    }
    else if (valueName == "messageTimeOut")
    {
        return 2;
    }
    else
...
std::string ReceivedStatus::Get(long value) const
{
    if (value == 0)
    {
...
std::string ReceivedStatus::GetExt(long value) const
{
    return "";
}
