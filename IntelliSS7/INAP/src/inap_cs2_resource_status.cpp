...
AsnDescObject* ResourceStatus::GetStaticDescription()
{
    ...
    else
    {
        ...
        staticDesc.SetTypeName("ResourceStatus");
        result = &staticDesc;
        initialized = true;
        Asn::Unlock();
        return result;
...
long ResourceStatus::Get(const std::string& valueName) const
{
    if (valueName == "busy")
    {
        return 0;
    }
    else if (valueName == "idle")
    {
        return 1;
    }
    else
...
std::string ResourceStatus::Get(long value) const
{
    if (value == 0)
    {
        return "busy";
...
std::string ResourceStatus::GetExt(long value) const
{
    return "";
}
