...
void		Memb_init()
{
	...
	if( Wide_network )
	{
        ...
        /* HLD: changed timeouts. */
        /* Reduce these timeout durations */
#if 0
...
#endif
        Token_timeout.sec  =   0; Token_timeout.usec  = 500000;
        Hurry_timeout.sec  =   0; Hurry_timeout.usec  = 200000;

        Alive_timeout.sec  =   0; Alive_timeout.usec  = 100000;
        Join_timeout.sec   =   0; Join_timeout.usec   = 100000;
        Rep_timeout.sec    =   0; Rep_timeout.usec    = 250000;
        Seg_timeout.sec    =   0; Seg_timeout.usec    = 200000;
        Gather_timeout.sec =   0; Gather_timeout.usec = 500000;
        Form_timeout.sec   =   0; Form_timeout.usec   = 500000;
        Lookup_timeout.sec =   6; Lookup_timeout.usec = 0;       
        /* HLD end */
	}
...
static	void	Create_form1()
{
	...
	*holes_procs_ptr = My.id;
	...
	if( F_members.num_pending > 0 )
	{
        ...
        if(ret < 0)
        {
            Alarm(EXIT, "Send to self failed, error %d\n", errno);
        }
	}
