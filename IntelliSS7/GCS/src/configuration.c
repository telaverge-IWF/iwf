...
int		Conf_init( char *file_name, char *my_name )
{
	 ...
	 */
        ...
        strcpy(configfile_location, SPREAD_ETCDIR);
        strcat(configfile_location, "/gcs_daemon.ini");

	if (NULL != (yyin = fopen(file_name,"r")) )
                Alarm( PRINT, "Conf_init: using file: %s\n", file_name);
	if (yyin == NULL) 
		if (NULL != (yyin = fopen("./gcs_damemon.ini", "r")) )
                        Alarm( PRINT, "Conf_init: using file: ./gcs_daemon.ini\n");
	if (yyin == NULL)
