...
static	void	Sess_accept( mailbox mbox, int domain, void *dummy )
{
	...
	for( i=10; i <= 200; i+=5 )
	{
	    ...
	    ret= getsockopt( Sessions[MAX_SESSIONS].mbox, SOL_SOCKET, SO_SNDBUF, (void *)&on, (socklen_t * )&onlen );
	    if( on < i*1024 ) break;
	    Alarm( NONE, "Sess_accept: set sndbuf %d, ret is %d\n", on, ret );

	    onlen = sizeof(on);
	    ret= getsockopt( Sessions[MAX_SESSIONS].mbox, SOL_SOCKET, SO_RCVBUF, (void *)&on, (socklen_t * )&onlen );
	    if( on < i*1024 ) break;
