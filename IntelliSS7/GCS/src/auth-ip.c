 ...
 * gcs_daemon_ip_access.ini file can connect.
 */

/* HLD: replaced access ip configuration file name by gcs_daemon_ip_access.ini. */
...
void ip_init(void)
{
    ...
    sprintf(file_name, "gcs_daemon_ip_access.ini");
    ...
    if (fp == NULL) 
        if (NULL != (fp = fopen("./gcs_daemon_ip_access.ini", "r")) )
            Alarm( PRINT, "ip_init: using file: ./gcs_daemon_ip_access.ini\n");
    if (fp == NULL)
        if (NULL != (fp = fopen("/etc/spread.access_ip", "r")) )
            Alarm( PRINT, "ip_init: using file: /etc/gcs_daemon_ip_access.ini\n");
    if (fp == NULL)
    {
