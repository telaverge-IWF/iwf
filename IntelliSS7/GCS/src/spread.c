...
/* HLD: end. */
    ...
    /* HLD: changed initial info box. */
        Alarm(PRINT, "/===========================================================================\\\n");
    Alarm(PRINT, "| Group Communication System (GCS) Daemon.                                  |\n");
    Alarm(PRINT, "|                                                                           |\n");
    Alarm(PRINT, "| Copyright (c) 2006 IntelliNet Technologies.                               |\n");
    Alarm(PRINT, "|                                                                           |\n");
...
#ifndef WIN32
    /* Launch the ethMonitorThread and wait till it initializes */
...
{
    Alarmp( SPLOG_FATAL, SYSTEM, 
           "Usage: spread\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n",
           "\t[-l y/n]          : print log",
           "\t[-n <proc name>]  : force computer name",
           "\t[-c <file name>]  : specify configuration file",
           "\t[-i <iface name>] : interface name to be monitored",
           "\t Multiple interfaces can be specified Ex: -i eth0 -i eth1",
           "\t[-f <floatingIP>] : Floating IP for multiple interfaces",
           "\t[-m <monDuration>]: Monitor duration in ms for NIC failure",
           "\t[-p <server port>]: Port on which nicmon communicates");
}

static	void	Invalid_privilege_decrease(char *user, char *group)
{
    Alarmp( SPLOG_FATAL, SECURITY, "Spread: FAILED privilege drop to user/group "
           "'%s/%s' (defined in gcs_daemon.ini or spread_params.h)\n",
           user, group );
}
...
static	void	Usage(int argc, char *argv[])
{
	...
	while( --argc > 0 )
	{
		...
		if( !strncmp( *argv, "-n", 2 ) )
		{
            ...
            /* Check if its an argument for nicmonitor */
            if(ITS_ParseEthMonArg(&argc, &argv) < 0)
                Print_help();
		}
#endif
	}
