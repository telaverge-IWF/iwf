...
/* HLD: replaced configuration file name by gcs_daemon.ini. */
...
int main( int argc, char *argv[] )
{
    ...
    /* HLD: changed initial info box. */
        Alarm(PRINT, "/===========================================================================\\\n");
    Alarm(PRINT, "| Group Communication System (GCS) Monitor.                                 |\n");
    Alarm(PRINT, "|                                                                           |\n");
    Alarm(PRINT, "| Copyright (c) 2001 IntelliNet Technologies.                               |\n");
    Alarm(PRINT, "|                                                                           |\n");
    Alarm(PRINT, "| This product uses software developed by Spread Concepts LLC for use in    |\n");
    Alarm(PRINT, "| the Spread toolkit.                                                       |\n");
    Alarm(PRINT, "|                                                                           |\n");
    Alarm(PRINT, "| For more information about Spread see \"http://www.spread.org\".            |\n");
    Alarm(PRINT, "|                                                                           |\n");
    Alarm(PRINT, "| Copyright (c) 1993-2001 Spread Concepts LLC. All rights reserved.         |\n");
    Alarm(PRINT, "|                                                                           |\n");
    Alarm(PRINT, "|                                                                           |\n");
    Alarm(PRINT, "|                                                                           |\n");
        Alarm(PRINT, "| Version %d.%02d.%02d                                                           |\n",
...
static	void	Report_message(mailbox fd, int dummy, void *dummy_p)
{
	...
	int	ret1,ret2;
    int proc_number;
/* HLD: switch to array (for multiple procs status requests). */
#define MAX_MONITOR_PROCS 32
static  int32   last_mes[MAX_MONITOR_PROCS];
static  int32   last_aru[MAX_MONITOR_PROCS];
static  int32   last_sec[MAX_MONITOR_PROCS];
    ...
    proc_number = Conf_proc_by_id( GlobalStatus.my_id, &p );
    if (proc_number < 0 || proc_number >= MAX_MONITOR_PROCS - 1)
    {
        proc_number = MAX_MONITOR_PROCS - 1;
    }
	...
	printf("Delta Mes: %7d\tDelta Pack: %7d\tDelta sec  : %7d\n",
                        GlobalStatus.message_delivered - last_mes[proc_number], GlobalStatus.aru - last_aru[proc_number], GlobalStatus.sec - last_sec[proc_number]);
	printf("==================================\n");
    ...
    if (proc_number >= 0 && proc_number < MAX_MONITOR_PROCS - 1)
    {
            last_mes[proc_number] = GlobalStatus.message_delivered;
            last_aru[proc_number] = GlobalStatus.aru;
            last_sec[proc_number] = GlobalStatus.sec;
    }
}
...
static	void	Usage(int argc, char *argv[])
{
	...
	strcpy( Config_file, "gcs_daemon.ini" );
