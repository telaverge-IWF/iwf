 ...
 *                                                                          *
 ****************************************************************************
 *  ID: $Id: itu-tcap_codec.c,v 9.8.44.1.2.1 2014/09/16 09:34:53 jsarvesh Exp $
 *
 *  LOG: $Log: itu-tcap_codec.c,v $
 *  LOG: Revision 9.8.44.1.2.1  2014/09/16 09:34:53  jsarvesh
 *  LOG: Changes done for removing warnings
 *  LOG:
 *  LOG: Revision 9.8.46.1  2014/09/15 07:20:40  jsarvesh
 *  LOG: Changes done for removing Warnings
 *  LOG:
 *  LOG: Revision 9.8.44.1  2014/08/22 10:13:53  jkchaitanya
 *  LOG: changes for increasing the size of tcap component for encoding
 *  LOG:
 *  LOG: Revision 9.8  2008/04/22 12:20:38  kramesh
 *  LOG: Propagated Bridgeport Issue 1751. Incorrect decode for indefinite form.
 *  LOG:
 *  LOG: Revision 9.7  2008/04/22 09:58:13  kramesh
 *  LOG: Propagated TCS issue 1024. 1-4 Byte TID for ITU TCAP.
 *  LOG: Propagated Personeta issue 1749/1715.Wrong destination TRID
 *  LOG: in outgoing message.
 *  LOG:
 *  LOG: Revision 9.6  2007/01/10 11:15:02  yranade
 ...
 *  --------------------------------------------------------------------------
 *  kramesh   22-04-2008  Bug ID:1749/1715        Propagated Personeta Issue.
 *                                                Wrong Destination Trid in 
 *                                                Outgoing Message.
 *  kramesh   22-04-2008   Bug ID:1024            Propagated TCS issue.
 *                                                1-4 Byte TID for ITU TCAP.
 *                                       
 ****************************************************************************/
static int
 ...
 ****************************************************************************/
SS7DLLAPI int
TCAP_CvtDLGToData_CCITT(TCAP_DLG *dlg, ITS_OCTET *tp, ITS_UINT *tpLen,
                        ITS_OCTET *cpList, ITS_UINT cptsLen,
 ...
 ****************************************************************************/
SS7DLLAPI int
TCAP_CvtCPTToData_CCITT(TCAP_CPT *cpt, ITS_OCTET *cp, ITS_UINT *cpLen)
{
 ...
 * kramesh   22-04-2008  Issue: 1751            Incorrect decode for 
 *                                              indefinite form
 ****************************************************************************/
SS7DLLAPI int
                        ...
                        ITS_UINT cTag, ITS_USHORT cLen)
{
    ...
    case TCPPT_TC_INVOKE:
        ...
        if ((data - cp) < cLen)
        {
            ...
            cpt->u.invoke.param.data = (ITS_OCTET*)malloc((tagLen + tLen + len) * sizeof(ITS_OCTET));
    ...
    case TCPPT_TC_RESULT_NL:
        ...
        if ((data - cp) < cLen)
        {
            ...
            tLen = TCAP_GetLength(tmp, &len, stop, ITS_TRUE);
            tmp += tLen;
            cpt->u.result.param.data = (ITS_OCTET*)malloc((tagLen + tLen + len) * sizeof(ITS_OCTET));
    ...
    case TCPPT_TC_U_ERROR:
        ...
        if ((data - cp) < cLen)
        {
            ...
            tLen = TCAP_GetLength(tmp, &len, stop, ITS_TRUE);
            tmp += tLen;
            cpt->u.error.param.data = (ITS_OCTET*)malloc((tagLen + tLen + len) * sizeof(ITS_OCTET));
	...
	if((CCITT_TCAP_Variant == CCITT_TCAP_ETSI) &&
	  ((*data < TCAP_PROB_CODE_MIN_CCITT) ||
	  (*data > TCAP_PROB_CODE_MAX_CCITT)))
	{
	    return (TCPPROB_SPEC_GEN_BADLY_STRUCT_COMP);
	}
    ...
    */
    ...
    case DIALOG_RESPONSE_TAG:
        ...
        *isResponse = ITS_TRUE;
	    ...
	    if ((CCITT_TCAP_Variant == CCITT_TCAP_ETSI) &&
	       ((len < TCAP_PROB_CODE_MIN_CCITT) ||
	       (len > TCAP_PROB_CODE_MAX_CCITT)))
	    {
	        return (ITS_FALSE);
	    }
                        ...
                        ITS_USHORT len, int variant)
{
    ...
    case TCPPT_TC_U_ABORT:
        ...
        */
	    ...
	    if((variant == CCITT_TCAP_ETSI) &&
	      ((*tmp < TCAP_PROB_CODE_MIN_CCITT) ||
	      (*tmp > TCAP_PROB_CODE_MAX_CCITT)))
	    {
	        return (ITS_EPROTERR);
	    }
