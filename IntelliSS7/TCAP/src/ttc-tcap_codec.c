 ...
 *                                                                          *
 ****************************************************************************
 *  ID: $Id: ttc-tcap_codec.c,v 9.2 2008/06/24 07:20:32 ssingh Exp $
 *
 *  LOG: $Log: ttc-tcap_codec.c,v $
 *  LOG: Revision 9.2  2008/06/24 07:20:32  ssingh
 *  LOG: porting Issue: 1751 fix from ITU module
 *  LOG:
 ...
 ****************************************************************************/
SS7DLLAPI int
TCAP_CvtDLGToData_TTC(TCAP_DLG *dlg, ITS_OCTET *tp, ITS_UINT *tpLen,
                        ITS_OCTET *cpList, ITS_UINT cptsLen,
 ...
 ****************************************************************************/
SS7DLLAPI int
TCAP_CvtCPTToData_TTC(TCAP_CPT *cpt, ITS_OCTET *cp, ITS_UINT *cpLen)
{
                        ...
                        ITS_UINT cTag, ITS_USHORT cLen)
{
	...
	if((TTC_TCAP_Variant == TTC_TCAP_ETSI) &&
	  ((*data < TCAP_PROB_CODE_MIN_TTC) ||
	  (*data > TCAP_PROB_CODE_MAX_TTC)))
	{
	    return (TCPPROB_SPEC_GEN_BADLY_STRUCT_COMP);
	}
        ...
        ITS_BOOLEAN *isResponse)
{
    ...
    */
    ...
    case DIALOG_REQUEST_TAG:
		...
		if ((TTC_TCAP_Variant == TTC_TCAP_ETSI) &&
		   ((count < TCAP_USER_INFO_MIN_TTC) ||
		   (count > TCAP_USER_INFO_MAX_TTC)))
		{
		    return (ITS_FALSE);
		}
    ...
    case DIALOG_RESPONSE_TAG:
        ...
        *isResponse = ITS_TRUE;
	    ...
	    if ((TTC_TCAP_Variant == TTC_TCAP_ETSI) &&
	       ((len < TCAP_PROB_CODE_MIN_TTC) ||
	       (len > TCAP_PROB_CODE_MAX_TTC)))
	    {
	        return (ITS_FALSE);
	    }
                        ...
                        ITS_USHORT len, int variant)
{
    ...
    case TCPPT_TC_U_ABORT:
        ...
        */
	    ...
	    if((variant == TTC_TCAP_ETSI) &&
	      ((*tmp < TCAP_PROB_CODE_MIN_TTC) ||
	      (*tmp > TCAP_PROB_CODE_MAX_TTC)))
	    {
	        return (ITS_EPROTERR);
	    }
