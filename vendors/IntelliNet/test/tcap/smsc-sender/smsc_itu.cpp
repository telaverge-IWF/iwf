 ...
 ****************************************************************************
 *
 *  ID: $Id: smsc_itu.cpp,v 9.1 2007/01/11 12:43:43 yranade Exp $
 *
 * LOG: $Log: smsc_itu.cpp,v $
 * LOG: Revision 9.1  2007/01/11 12:43:43  yranade
 * LOG: Merge changes from 6.5.3 and Lucent branches
 * LOG:
 * LOG: Revision 1.1.2.1  2006/12/23 10:05:39  randresol
...
#ident "$Id: smsc_itu.cpp,v 9.1 2007/01/11 12:43:43 yranade Exp $"
...
its::TaskQueueTransport *tcapEventQueue_CCITT;
...
#define LOCAL_PC    0x6789
#define LOCAL_SSN   11 

#define REMOTE_PC   0x2345
#define REMOTE_SSN  4
...
static void ShutDownNode_CCITT();
...
ApplicationSelector(ITS_HDR *h, ITS_EVENT *ev)
{
   // instruct Accelero to route all inbound message to TCAP instance 
      ...
      if (h->type == ITS_TCAP_CCITT)
      {
          return (TCAP_RECEIVE_QUEUE_ID_CCITT);
      }
/*
      else if (h->type == ITS_TCAP_TCC)
      {
          return (TCAP_RECEIVE_QUEUE_ID_TCC);
      }
      else if (h->type == ITS_TCAP_PRC)
      {
          return (TCAP_RECEIVE_QUEUE_ID_CCITT);
      }
*/
...
/*
    tcapEventQueue_CCITT = new TaskQueueTransport("Tcap Events CCITT",
                                          TCAP_RECEIVE_QUEUE_ID_CCITT,
...
*/
    DBC_AddDoCommand(TestCases_CCITT);
...
extern "C" void SendContinue_CCITT()
{
...
*/
    ...
    TCAP_Component::Send(ISS7_CCITT_Stack, &continueDlg, &invokeCpt);

    TCAP_Component::Send(ISS7_CCITT_Stack, &continueDlg, &resultCpt);

    TCAP_Dialogue::Send(ISS7_CCITT_Stack, &continueDlg);
...
extern "C" TESTDLLAPI void
TrafficGenerator_CCITT(void *arg)
{
    int i;
...
extern "C" TESTDLLAPI void 
TCAP_SS7PostInit_CCITT(TPOOL_THREAD *thr, ITS_HANDLE h)
{
    tcapEventQueue_CCITT = new TaskQueueTransport("Tcap Events CCITT",
                                          TCAP_RECEIVE_QUEUE_ID_CCITT,
                                          ITS_TRANSPORT_TQUEUE);
...
TCAP_SS7PostNextEvent_CCITT(TPOOL_THREAD *thr, ITS_EVENT *ev)
{
    int i;
...
extern "C" TESTDLLAPI void
MTP3Mgmt_CCITT(ITS_POINTER object, ITS_POINTER userData, ITS_POINTER callData)
{
...
extern "C" TESTDLLAPI void
TcapEventHandler_CCITT(void *arg)
{
    ITS_UINT i;
    ITS_Event event ; 
    ITS_INT res = 0;
    ...
    while (TRANSPORT_GetNextEvent((TRANSPORT_Control *)tcapEventQueue_CCITT->GetTC(), &event.GetEvent()) == ITS_SUCCESS)
    {
...
*/
        ...
        if (tContext[currentDialogueID].beginReceived && recvCpt && recvCpt->GetLast() )
        {
            if (shortTr) /* short transaction: BEGIN-END */
            {
                ...
                TCAP_Component::Send(ISS7_CCITT_Stack, &endDlg, &resultCpt);
...
void
TestCases_CCITT(DBC_Server *dbc, const char *cmdLine)
{    
 ...
 ****************************************************************************/
extern "C" TESTDLLAPI void
ApplicationMembershipChangeCb_CCITT(
    ITS_POINTER object,
