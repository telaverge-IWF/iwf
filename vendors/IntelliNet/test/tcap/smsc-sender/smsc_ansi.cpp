 ...
 ****************************************************************************
 *
 *  ID: $Id: smsc_ansi.cpp,v 9.1 2005/03/23 12:55:25 cvsadmin Exp $
 *
 * LOG: $Log: smsc_ansi.cpp,v $
...
#ident "$Id: smsc_ansi.cpp,v 9.1 2005/03/23 12:55:25 cvsadmin Exp $"
...
extern "C" void SendBegin()
{
    ...
    TCAP_AllocateDialogueId(ISS7_ANSI_Stack, &dialogueID);
...
*/
    ...
    TCAP_Component::Send(ISS7_ANSI_Stack, &beginDlg, &invokeCpt);

    TCAP_Dialogue::Send(ISS7_ANSI_Stack, &beginDlg);
...
*/
    ...
    TCAP_Component::Send(ISS7_ANSI_Stack, &continueDlg, &invokeCpt);

    TCAP_Component::Send(ISS7_ANSI_Stack, &continueDlg, &resultCpt);

    TCAP_Dialogue::Send(ISS7_ANSI_Stack, &continueDlg);
...
*/
        ...
        if (tContext[currentDialogueID].beginReceived && recvCpt && recvCpt->GetLast() )
        {
            if (shortTr) /* short transaction: BEGIN-END */
            {
                ...
                TCAP_Component::Send(ISS7_ANSI_Stack, &endDlg, &resultCpt);
