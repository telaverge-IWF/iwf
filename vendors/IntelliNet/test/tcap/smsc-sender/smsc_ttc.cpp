 ...
 ****************************************************************************
 *
 *  ID: $Id: smsc_ttc.cpp,v 9.2 2007/01/11 12:43:43 yranade Exp $
 *
 * LOG: $Log: smsc_ttc.cpp,v $
 * LOG: Revision 9.2  2007/01/11 12:43:43  yranade
 * LOG: Merge changes from 6.5.3 and Lucent branches
 * LOG:
 * LOG: Revision 9.1.10.1  2006/12/23 10:05:39  randresol
...
#ident "$Id: smsc_ttc.cpp,v 9.2 2007/01/11 12:43:43 yranade Exp $"
...
its::TaskQueueTransport *tcapEventQueue_TTC;
...
ApplicationSelector(ITS_HDR *h, ITS_EVENT *ev)
{
   // instruct Accelero to route all inbound message to TCAP instance 
      ...
      if (h->type == ITS_TCAP_TTC)
      {
          return (TCAP_RECEIVE_QUEUE_ID_TTC);
      }
/*
      else if (h->type == ITS_TCAP_CCITT)
      {
...
*/
    tcapEventQueue_TTC = new TaskQueueTransport("Tcap Events TTC",
                                          TCAP_RECEIVE_QUEUE_ID_TTC,
                                          ITS_TRANSPORT_TQUEUE);
    /* create mutex */
/*
...
*/
 //   DBC_AddDoCommand(TestCases_CCITT);
    DBC_AddDoCommand(TestCases_TTC);
...
extern "C" void SendContinue_TTC()
{
...
TCAP_SS7PostNextEvent_TTC(TPOOL_THREAD *thr, ITS_EVENT *ev)
{
    int i;
...
extern "C" TESTDLLAPI void
MTP3Mgmt_TTC(ITS_POINTER object, ITS_POINTER userData, ITS_POINTER callData)
{
...
TcapEventHandler_TTC(void *arg)
{
    ...
    while (TRANSPORT_GetNextEvent((TRANSPORT_Control *)tcapEventQueue_TTC->GetTC(), &event.GetEvent()) == ITS_SUCCESS)
    {
...
void
TestCases_TTC(DBC_Server *dbc, const char *cmdLine)
{    
 ...
 ****************************************************************************/
extern "C" TESTDLLAPI void
ApplicationMembershipChangeCb_TTC(
    ITS_POINTER object,
