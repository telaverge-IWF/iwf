 ...
 *                                                                          *
 ****************************************************************************
 * ID:$Id: ISUP_PrintCic.C,v 9.1 2005/03/23 12:55:18 cvsadmin Exp $
 * LOG:$Log: ISUP_PrintCic.C,v $
...
ValidateCicRange(const char* cmdLine, ITS_USHORT* startcic, ITS_USHORT* endcic)
{
    ...
    len = (ITS_USHORT)strlen(start);
    tmp = start;
    while( isdigit((int)*tmp) ) tmp++;
    *tmp = 0;
    if( len == strlen(start) )
    {
        *startcic = (ITS_USHORT)atoi(start);
    }
    else
    {
        printf("\nInvalid command given: \"%s\"\n",cmdLine);
    ...
    len = (ITS_USHORT)strlen(end);
    tmp = end;
    while( isdigit((int)*tmp) ) tmp++;
    *tmp = 0;
    if( len == strlen(end) )
    {
        *endcic = (ITS_USHORT)atoi(end);
    }
    else
    {
        printf("\nInvalid command given: \"%s\"\n",cmdLine);
...
#if 0
        /*
         * Looks like we have a problem with ISUP_ValidateCIC
         * use pcic for the timebeing!
         */

        ret = ISUP_ValidateCIC(ISS7_CCITT_Stack,
                              (ITS_OCTET)sio,
                              (ITS_UINT)opc,
                              (ITS_UINT)dpc,
                              (ITS_USHORT)i,
...
PrintCICRecInfo(ITS_UINT opc, ITS_UINT dpc, ITS_UINT sio, ITS_USHORT cic)
{
    ...
    /* Convert cic, opc, dpc to pcic */
    ISUP_PCICFromParts( &pcic, opc, dpc, (ITS_OCTET) sio, cic );
              ...
              ITS_USHORT startcic, ITS_USHORT endcic)
{
    ...
    for(i = startcic; i <= endcic; i++)
    {
        ...
        /* Convert cic, opc, dpc to pcic */
        ISUP_PCICFromParts( &pcic, opc, dpc, (ITS_OCTET)sio, i );
 ...
 ****************************************************************************/
int
PrintCICPegsInfo(ITS_USHORT opc, ITS_USHORT dpc, ITS_USHORT sio, ITS_USHORT cic)
{
    ISUP_PCIC pcic;
    ISUP_CIC_INFO *cic_rec = NULL;
    int i;
    ...
    {
        printf("%d\t%s\t%d\n", i, ISUP_GetCICPegDesc(i), 
                               PEG_GetPeg(&cic_rec->ISUP_CIC_PEGS, i));
    }
...
#if defined(CCITT)
        printf("%d\t%s\t%d\n", i, ISUP_GetPegDesc(i), 
                               PEG_GetPeg(CCITT_ISUP_Pegs, i));
#elif defined (ANSI)
/* TODO: */
/*        printf("%d\t%s\t%d\n", i, ISUP_GetPegDesc(i), 
                               PEG_GetPeg(ANSI_ISUP_Pegs, i)); */
#endif
    }
 ...
 ****************************************************************************/
int
PrintDestPegsInfo(ITS_USHORT opc, ITS_USHORT dpc, ITS_USHORT sio)
{
    int ret = 0;
    int i;
    MTP3_HEADER hdr;
    ...
    hdr.sio = (ITS_OCTET)sio;

    ret = ISUP_FindOpcDpc(hdr, &config_info);
    if (ret != ITS_SUCCESS)
    {
    ...
    printf("\n======================================================================\n");
    printf("Dest Pegs for opc %d dpc %d:\n",opc, dpc);
    printf("----------------------\n");

    for (i = 0; i < PEG_ISUP_DEST_NUM_PEGS; i++)
    {
        printf("%d\t%s\t%d\n", i, ISUP_GetDestPegDesc(i), 
                   PEG_GetPeg(&config_info.opc_dpc_stat->ISUP_DEST_PEGS, i));
    }
