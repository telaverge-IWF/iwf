 ...
 ****************************************************************************
 *
 *  ID: $Id: readISUPMsg.c,v 9.2 2006/01/09 06:40:06 omayor Exp $
 *
 ...
 * LOG: Begin PR6.5
 * LOG:
 * LOG: Revision 8.2  2005/03/21 13:54:19  cvsadmin
 * LOG: PR6.4.2 Source Propagated to Current
 * LOG:
 * LOG: Revision 7.3  2002/10/25 16:11:58  ssingh
 * LOG: Put some comments as fixing BUG 148.
 * LOG:
 * LOG: Revision 7.2  2002/10/10 10:23:01  sjaddu
 * LOG: Changed ISUP_PRM_USER_SERVICE_INFORM_PRIME to ISUP_PRM_USER_SERVICE_INFO_PRIME.
 * LOG:
 * LOG: Revision 7.1  2002/08/26 22:10:38  mmiers
 * LOG: Begin PR6.2
 * LOG:
 * LOG: Revision 1.5  2002/08/01 16:01:46  ssharma
 * LOG: Merge from ISUP_PR7 branch.
 * LOG:
 * LOG: Revision 1.4.2.4  2002/07/31 19:13:48  ssingh
 * LOG: Changes done to support ANSI95 related new params.
 * LOG:
 * LOG: Revision 1.4.2.3  2002/07/06 06:16:21  hbalimid
 * LOG: Took care of all the warnings.
 * LOG:
 * LOG: Revision 1.4.2.2  2002/07/03 20:13:31  ssharma
 * LOG: Bug fix for unrecognized param.
 * LOG:
 * LOG: Revision 1.4  2002/06/23 23:27:27  hbalimid
 * LOG: ITU97 specific parameters in Place.
 * LOG:
 * LOG: Revision 1.3  2002/06/18 20:23:08  sjaddu
 * LOG: Added mtp3 label case for ANSI.
 * LOG:
 ...
 * LOG: Start PR5.
 * LOG:
 * LOG: Revision 1.3  2001/02/05 23:10:58  hxing
 * LOG: Remove warnings
 * LOG:
 ...
 * LOG:
 * LOG: Revision 3.2  2000/10/12 14:29:17  labuser
 * LOG: Core dump problem fixed
 * LOG:
 * LOG: Revision 3.1  2000/08/16 00:11:13  mmiers
 * LOG:
 * LOG: Begin round 4.
 * LOG:
 * LOG: Revision 1.7  2000/08/01 19:52:11  hxing
 * LOG: Continue testing
 * LOG:
 * LOG: Revision 1.6  2000/07/31 21:39:16  hxing
 * LOG: Testing more ies and more messages
 * LOG:
 * LOG: Revision 1.5  2000/07/28 15:04:42  hxing
 * LOG: Testing
 * LOG:
 * LOG: Revision 1.4  2000/07/27 23:00:19  hxing
 * LOG: Testing with more optional IEs
 * LOG:
 * LOG: Revision 1.3  2000/07/24 21:24:07  hxing
 * LOG: Fix bug with ie translation
 * LOG:
 * LOG: Revision 1.2  2000/07/21 19:16:08  hxing
 * LOG: Type of transaction_id is changed to ITS_OCTET[4] from ITS_UINT
 * LOG:
...
ReadRedirectionNumPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    if ((int)ie->param_length > aiLen+npLen)
    {
        GetOctetsFromStr(&nDigits, 1);
        nOctDigits = NumOfOctets4Digits(nDigits);
...
/*********************** ISUP_ITU_99 Parameters ****************************/
                                                                                                                             
/*.implementation:public
 ****************************************************************************
 *  Purpose:
 *      This function reads the Application Transport Parameter
 *
 *  Input Parameters:
 *      buf - string read from a file containing parameter data
 *      ie - a pointer to an information element structure to store data read
...
#if defined(CCITT)
int
ReadApplicationTransportPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    int plen = 1;                                                                                                                              
    /* Set IE information */
    ie->param_id = ISUP_PRM_APPLLICATION_TRANSPORT;
    ie->param_length = (ITS_OCTET)strtol(strtok(buf, " \t"), NULL, BASE_16);
                                                                                                                             
    if ((int)ie->param_length <  plen)
    {
        printf("ReadISUPMsg: Appl Trns par plen < %d \n", plen);
        ret = !ITS_SUCCESS;
    }
    else
    {
        GetOctetsFromStr(&(ie->param_data.applicationTransport.appl_cntx_ident), 1);
        if(ie->param_length ==  plen)
        return ret;
                                                                                                                             
        GetOctetsFromStr(&(ie->param_data.applicationTransport.sni_rci), 1);
        if(ie->param_length ==  plen+1)
        return ret;
                                                                                                                             
        GetOctetsFromStr(&(ie->param_data.applicationTransport.apm_seg_ind), 1);
        if(ie->param_length ==  plen+2)
        return ret;
                                                                                                                             
        GetOctetsFromStr(&(ie->param_data.applicationTransport.seg_local_ref), 1);
        if(ie->param_length ==  plen+4)
        return ret;

        /* read in no. of digits */
        GetOctetsFromStr(&nDigits, 1);
        nOctets = NumOfOctets4Digits(nDigits);
        GetOctetsFromStr(&(ie->param_data.applicationTransport.enc_appl_info[0]), nDigits);
                                                                                                                             
        /* GetOctetsFromStr(&nDigits, 1);
        GetOctetsFromStr(&(ie->param_data.applicationTransport.enc_appl_info[0]), (int)nDigits); */
    }                                                                                                                              
    return ret;
}
#endif
 ...
 ****************************************************************************
 *  Purpose:
 *      This function reads the CCNR Possible Indication Ind Parameter
 *
 *  Input Parameters:
 *      buf - string read from a file containing parameter data
 *      ie - a pointer to an information element structure to store data read
...
#if defined(CCITT)
int
ReadCcnrPossibleIndPar(char *buf, ITS_ISUP_IE *ie)
{
    int ret = ITS_SUCCESS;
    int plen = 1;             /* param len = 1 octet */
                                                                                                                             
    /* Set IE information */
    ie->param_id = ISUP_PRM_CCNR_POSSIBLE_IND;
    ie->param_length = (ITS_OCTET)strtol(strtok(buf, " \t"), NULL, BASE_16);
    ...
    {
        GetOctetsFromStr(&(ie->param_data.ccnrPossibleInd.posbl_ind), plen);
    }
 ...
 ****************************************************************************
 *  Purpose:
 *      This function reads the Pivot Capability Parameter
 *
 *  Input Parameters:
 *      buf - string read from a file containing parameter data
 *      ie - a pointer to an information element structure to store data read
...
#if defined(CCITT)
int
ReadPivotCapabilityPar(char *buf, ITS_ISUP_IE *ie)
{
    int ret = ITS_SUCCESS;
    int plen = 1;             /* param len = 1 octet */
                                                                                                                             
    /* Set IE information */
    ie->param_id = ISUP_PRM_PIVOT_CAPABILITY;
    ie->param_length = (ITS_OCTET)strtol(strtok(buf, " \t"), NULL, BASE_16);
    ...
    {
        GetOctetsFromStr(&(ie->param_data.pivotCapability.pvt_cablt), plen);
    }
 ...
 ****************************************************************************
 *  Purpose:
 *      This function reads the Pivot routing Indicator Parameter
 *
 *  Input Parameters:
 *      buf - string read from a file containing parameter data
 *      ie - a pointer to an information element structure to store data read
...
#if defined(CCITT)
int
ReadPivotRoutingIndPar(char *buf, ITS_ISUP_IE *ie)
{
    int ret = ITS_SUCCESS;
    int plen = 1;             /* param len = 1 octet */
                                                                                                                             
    /* Set IE information */
    ie->param_id = ISUP_PRM_PIVOT_ROUTING_IND;
    ie->param_length = (ITS_OCTET)strtol(strtok(buf, " \t"), NULL, BASE_16);
    ...
    {
        GetOctetsFromStr(&(ie->param_data.pivotRoutingInd.pvt_rt_ind), plen);
    }
 ...
 ****************************************************************************
 *  Purpose:
 *      This function reads the Called Directory Number Parameter
 *
 *  Input Parameters:
 *      buf - string read from a file containing parameter data
 *      ie - a pointer to an information element structure to store data read
...
#if defined(CCITT)
int
ReadCalledDirNumPar(char *buf, ITS_ISUP_IE *ie)
{
    int ret = ITS_SUCCESS;
    int nOctets = 0;
    ITS_OCTET nDigits = 0x00;
    ...
    /* Set IE information */
    ie->param_id = ISUP_PRM_CALLED_DIR_NUM;
    ie->param_length = (ITS_OCTET)strtol(strtok(buf, " \t"), NULL, BASE_16);
    ...
    {
        GetOctetsFromStr(&(ie->param_data.calledDirNum.nature_addr_ind), 1);
        GetOctetsFromStr(&(ie->param_data.calledDirNum.num_plan_ind), 1);                                                                                                                              
        GetOctetsFromStr(&nDigits,1);
        nOctets = NumOfOctets4Digits(nDigits);
        GetOctetsFromStr(&(ie->param_data.calledDirNum.addr_signal[0]), nOctets);
    }                                                                                                                              
    return ret;                                                                                                                              
}
#endif
 ...
 ****************************************************************************
 *  Purpose:
 *      This function reads the Original Called IN number Parameter
 *
 *  Input Parameters:
 *      buf - string read from a file containing parameter data
 *      ie - a pointer to an information element structure to store data read
...
#if defined(CCITT)
int
ReadOriginalCalledInNumPar(char *buf, ITS_ISUP_IE *ie)
{
    int ret = ITS_SUCCESS;
    int nOctets = 0;
    ITS_OCTET nDigits = 0x00;
    int plen = 2;                                                                                                                              
    /* Set IE information */
    ie->param_id = ISUP_PRM_ORIGINAL_CALLED_IN_NUM;
    ie->param_length = (ITS_OCTET)strtol(strtok(buf, " \t"), NULL, BASE_16);
    ...
    {
        GetOctetsFromStr(&(ie->param_data.originalCalledInNum.addr_ind), 1);
        GetOctetsFromStr(&(ie->param_data.originalCalledInNum.subfields4), 1);                                                                                                                              
        GetOctetsFromStr(&nDigits,1);
        nOctets = NumOfOctets4Digits(nDigits);
        GetOctetsFromStr(&(ie->param_data.originalCalledInNum.dgts[0]), nOctets);
    }                                                                                                                              
    return ret;
}
#endif
 ...
 ****************************************************************************
 *  Purpose:
 *      This function reads the Called geodetic location Parameter
 *
 *  Input Parameters:
 *      buf - string read from a file containing parameter data
 *      ie - a pointer to an information element structure to store data read
...
#if defined(CCITT)
int
ReadCalledGeodeticLocationPar(char *buf, ITS_ISUP_IE *ie)
{
    int ret = ITS_SUCCESS;
    int nOctets = 0;
    ITS_OCTET nDigits = 0x00;
    int plen = 1;                                                                                                                              
    /* Set IE information */
    ie->param_id = ISUP_PRM_CALLING_GEODETIC_LOCATION;
    ie->param_length = (ITS_OCTET)strtol(strtok(buf, " \t"), NULL, BASE_16);
    ...
    {
        GetOctetsFromStr(&(ie->param_data.calledGeodeticLocation.lpri_screen), 1);
        if(ie->param_length == plen)
        return ret;
        GetOctetsFromStr(&(ie->param_data.calledGeodeticLocation.type_shape), 1);
        GetOctetsFromStr(&nDigits,1);
        nOctets = NumOfOctets4Digits(nDigits);
        GetOctetsFromStr(&(ie->param_data.calledGeodeticLocation.shape_desc[0]), nOctets);
    }                                                                                                                              
    return ret;
}
#endif
 ...
 ****************************************************************************
 *  Purpose:
 *      This function reads the HTR Information Parameter
 *
 *  Input Parameters:
 *      buf - string read from a file containing parameter data
 *      ie - a pointer to an information element structure to store data read
...
#if defined(CCITT)
int
ReadHtrInfoPar(char *buf, ITS_ISUP_IE *ie)
{
    int ret = ITS_SUCCESS;
    int nOctets = 0;
    ITS_OCTET nDigits = 0x00;
    int plen = 2;                                                                                                                              
    /* Set IE information */
    ie->param_id = ISUP_PRM_HTR_INFORMATION;
    ie->param_length = (ITS_OCTET)strtol(strtok(buf, " \t"), NULL, BASE_16);
    ...
    {
        GetOctetsFromStr(&(ie->param_data.htrInfo.nature_addr_ind), 1);
        GetOctetsFromStr(&(ie->param_data.htrInfo.num_plan_ind), 1);
        if (ie->param_length == plen)
        return ret;
        GetOctetsFromStr(&nDigits,1);
        nOctets = NumOfOctets4Digits(nDigits);
        GetOctetsFromStr(&(ie->param_data.htrInfo.addr_signal[0]), nOctets);
    }                                                                                                                              
    return ret;
}
#endif
 ...
 ****************************************************************************
 *  Purpose:
 *      This function reads the Network routing number Parameter
 *
 *  Input Parameters:
 *      buf - string read from a file containing parameter data
 *      ie - a pointer to an information element structure to store data read
...
#if defined(CCITT)
int
ReadNetworkRoutingNumPar(char *buf, ITS_ISUP_IE *ie)
{
    int ret = ITS_SUCCESS;
    int nOctets = 0;
    ITS_OCTET nDigits = 0x00;
    int plen = 2;                                                                                                                              
    /* Set IE information */
    ie->param_id = ISUP_PRM_NETWORK_ROUTING_NUM;
    ie->param_length = (ITS_OCTET)strtol(strtok(buf, " \t"), NULL, BASE_16);
    ...
    {
        GetOctetsFromStr(&(ie->param_data.networkRoutingNum.indicators), 1);
                                                                                                                             
        GetOctetsFromStr(&nDigits,1);
        nOctets = NumOfOctets4Digits(nDigits);
        GetOctetsFromStr(&(ie->param_data.networkRoutingNum.addr_signal[0]), nOctets);
    }                                                                                                                              
    return ret;
}
#endif
 ...
 ****************************************************************************
 *  Purpose:
 *      This function reads the Query on release capability Parameter
 *
 *  Input Parameters:
 *      buf - string read from a file containing parameter data
 *      ie - a pointer to an information element structure to store data read
...
#if defined(CCITT)
int
ReadQueryOnReleaseCapabilityPar(char *buf, ITS_ISUP_IE *ie)
{
    int ret = ITS_SUCCESS;
    int plen = 1;             /* param len = 1 octet */
                                                                                                                             
    /* Set IE information */
    ie->param_id = ISUP_PRM_QUERY_ON_RELEASE_CAPABILITY;
    ie->param_length = (ITS_OCTET)strtol(strtok(buf, " \t"), NULL, BASE_16);
    ...
    {
        GetOctetsFromStr(&(ie->param_data.queryOnReleaseCapability.qor_cap), plen);
    }
 ...
 ****************************************************************************
 *  Purpose:
 *      This function reads the Pivot status Parameter
 *
 *  Input Parameters:
 *      buf - string read from a file containing parameter data
 *      ie - a pointer to an information element structure to store data read
...
#if defined(CCITT)
int
ReadPivotStatusPar(char *buf, ITS_ISUP_IE *ie)
{
    int ret = ITS_SUCCESS;
    int plen = 1;             /* param len = 1 octet */
                                                                                                                             
    /* Set IE information */
    ie->param_id = ISUP_PRM_PIVOT_STATUS;
    ie->param_length = (ITS_OCTET)strtol(strtok(buf, " \t"), NULL, BASE_16);
    ...
    {
        GetOctetsFromStr(&(ie->param_data.pivotStatus.pvt_stat), plen);
    }
 ...
 ****************************************************************************
 *  Purpose:
 *      This function reads the Pivot counter Parameter
 *
 *  Input Parameters:
 *      buf - string read from a file containing parameter data
 *      ie - a pointer to an information element structure to store data read
...
#if defined(CCITT)
int
ReadPivotCounterPar(char *buf, ITS_ISUP_IE *ie)
{
    int ret = ITS_SUCCESS;
    int plen = 1;             /* param len = 1 octet */
                                                                                                                             
    /* Set IE information */
    ie->param_id = ISUP_PRM_PIVOT_COUNTER;
    ie->param_length = (ITS_OCTET)strtol(strtok(buf, " \t"), NULL, BASE_16);
    ...
    {
        GetOctetsFromStr(&(ie->param_data.pivotCounter.pvt_cntr), plen);
    }
 ...
 ****************************************************************************
 *  Purpose:
 *      This function reads the Pivot routing forward information Parameter
 *
 *  Input Parameters:
 *      buf - string read from a file containing parameter data
 *      ie - a pointer to an information element structure to store data read
...
#if defined(CCITT)
int
ReadPivotRoutingFrwdInfoPar(char *buf, ITS_ISUP_IE *ie)
{
    int ret = ITS_SUCCESS;
    int nOctets = 0;
    ITS_OCTET nDigits = 0x00;
    ...
    /* Set IE information */
    ie->param_id = ISUP_PRM_PIVOT_ROUTING_FRWD_INFO;
    ie->param_length = (ITS_OCTET)strtol(strtok(buf, " \t"), NULL, BASE_16);
                                                                                                                             
    GetOctetsFromStr(&(ie->param_data.pivotRoutingFrwdInfo.info[0].type_tag), 1);
    if (ie->param_length == 1)
    return ret;
    GetOctetsFromStr(&(ie->param_data.pivotRoutingFrwdInfo.info[0].type_len), 1);
                                                                                                                             
    GetOctetsFromStr(&nDigits, ie->param_data.pivotRoutingFrwdInfo.info[0].type_len);
    nOctets = NumOfOctets4Digits(nDigits);
                                                                                                                             
    GetOctetsFromStr(&(ie->param_data.pivotRoutingFrwdInfo.info[0].type_val[0]), nOctets);                                                                                                                              
    return ret;
}
#endif
/*.implementation:public
 ****************************************************************************
 *  Purpose:
 *      This function reads the Pivot routing backward information Parameter
 *
 *  Input Parameters:
 *      buf - string read from a file containing parameter data
 *      ie - a pointer to an information element structure to store data read
...
#if defined(CCITT)
int
ReadPivotRoutingBkwdInfoPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    /* Set IE information */
    ie->param_id = ISUP_PRM_PIVOT_ROUTING_BKWD_INFO;
    ie->param_length = (ITS_OCTET)strtol(strtok(buf, " \t"), NULL, BASE_16);
                                                                                                                             
    GetOctetsFromStr(&(ie->param_data.pivotRoutingBkwdInfo.info[0].type_tag), 1);
    if (ie->param_length == 1)
    return ret;
    GetOctetsFromStr(&(ie->param_data.pivotRoutingBkwdInfo.info[0].type_len), 1);
                                                                                                                             
    GetOctetsFromStr(&nDigits, ie->param_data.pivotRoutingBkwdInfo.info[0].type_len);
    nOctets = NumOfOctets4Digits(nDigits);
                                                                                                                             
    GetOctetsFromStr(&(ie->param_data.pivotRoutingBkwdInfo.info[0].type_val[0]), nOctets);                                                                                                                              
    return ret;
 ...
 ****************************************************************************
 *  Purpose:
 *      This function reads the Redirect capability Parameter
 *
 *  Input Parameters:
 *      buf - string read from a file containing parameter data
 *      ie - a pointer to an information element structure to store data read
...
#if defined(CCITT)
int
ReadRedirectCapabilityPar(char *buf, ITS_ISUP_IE *ie)
{
    int ret = ITS_SUCCESS;
    int plen = 1;             /* param len = 1 octet */
                                                                                                                             
    /* Set IE information */
    ie->param_id = ISUP_PRM_REDIRECT_CAPABILITY;
    ie->param_length = (ITS_OCTET)strtol(strtok(buf, " \t"), NULL, BASE_16);
    ...
    {
        GetOctetsFromStr(&(ie->param_data.redirectCapability.rd_cap), plen);
    }
 ...
 ****************************************************************************
 *  Purpose:
 *      This function reads the  Redirect counter Parameter
 *
 *  Input Parameters:
 *      buf - string read from a file containing parameter data
 *      ie - a pointer to an information element structure to store data read
...
#if defined(CCITT)
int
ReadRedirectCounterPar(char *buf, ITS_ISUP_IE *ie)
{
    int ret = ITS_SUCCESS;
    int plen = 1;             /* param len = 1 octet */
                                                                                                                             
    /* Set IE information */
    ie->param_id = ISUP_PRM_REDIRECT_COUNTER;
    ie->param_length = (ITS_OCTET)strtol(strtok(buf, " \t"), NULL, BASE_16);
    ...
    {
        GetOctetsFromStr(&(ie->param_data.redirectCounter.rd_cntr), plen);
    }
 ...
 ****************************************************************************
 *  Purpose:
 *      This function reads the  Redirect status Parameter
 *
 *  Input Parameters:
 *      buf - string read from a file containing parameter data
 *      ie - a pointer to an information element structure to store data read
...
#if defined(CCITT)
int
ReadRedirectStatusPar(char *buf, ITS_ISUP_IE *ie)
{
    int ret = ITS_SUCCESS;
    int plen = 1;             /* param len = 1 octet */
                                                                                                                             
    /* Set IE information */
    ie->param_id = ISUP_PRM_REDIRECT_STATUS;
    ie->param_length = (ITS_OCTET)strtol(strtok(buf, " \t"), NULL, BASE_16);
    ...
    {
        GetOctetsFromStr(&(ie->param_data.redirectStatus.rd_stat), plen);
    }
 ...
 ****************************************************************************
 *  Purpose:
 *      This function reads the  Redirect forward information Parameter
 *
 *  Input Parameters:
 *      buf - string read from a file containing parameter data
 *      ie - a pointer to an information element structure to store data read
...
#if defined(CCITT)
int
ReadRedirectFrwdInfoPar(char *buf, ITS_ISUP_IE *ie)
{
    ...
    /* Set IE information */
    ie->param_id = ISUP_PRM_REDIRECT_FRWD_INFO;
    ie->param_length = (ITS_OCTET)strtol(strtok(buf, " \t"), NULL, BASE_16);
                                                                                                                             
    GetOctetsFromStr(&(ie->param_data.redirectFrwdInfo.info[0].type_tag), 1);
    if (ie->param_length == 1)
    return ret;
    GetOctetsFromStr(&(ie->param_data.redirectFrwdInfo.info[0].type_len), 1);
                                                                                                                             
    GetOctetsFromStr(&nDigits, ie->param_data.redirectFrwdInfo.info[0].type_len);
    nOctets = NumOfOctets4Digits(nDigits);
                                                                                                                             
    GetOctetsFromStr(&(ie->param_data.redirectFrwdInfo.info[0].type_val[0]), nOctets);                                                                                                                              
    return ret;
 ...
 ****************************************************************************
 *  Purpose:
 *      This function reads the Redirect backward information Parameter
 *
 *  Input Parameters:
 *      buf - string read from a file containing parameter data
 *      ie - a pointer to an information element structure to store data read
...
#if defined(CCITT)
int
ReadRedirectBkwdInfoPar(char *buf, ITS_ISUP_IE *ie)
{
    int ret = ITS_SUCCESS;
    int nOctets = 0;
    ITS_OCTET nDigits = 0x00;
    ...
    /* Set IE information */
    ie->param_id = ISUP_PRM_REDIRECT_BKWD_INFO;
    ie->param_length = (ITS_OCTET)strtol(strtok(buf, " \t"), NULL, BASE_16);
                                                                                                                             
    GetOctetsFromStr(&(ie->param_data.redirectBkwdInfo.info[0].type_tag), 1);
    if (ie->param_length == 1)
    return ret;
    GetOctetsFromStr(&(ie->param_data.redirectBkwdInfo.info[0].type_len), 1);
                                                                                                                             
    GetOctetsFromStr(&nDigits, ie->param_data.redirectBkwdInfo.info[0].type_len);
    nOctets = NumOfOctets4Digits(nDigits);
                                                                                                                             
    GetOctetsFromStr(&(ie->param_data.redirectBkwdInfo.info[0].type_val[0]), nOctets);                                                                                                                              
    return ret;
 ...
 ****************************************************************************
 *  Purpose:
 *      This function reads the Number portability forward Information
 *
 *  Input Parameters:
 *      buf - string read from a file containing parameter data
 *      ie - a pointer to an information element structure to store data read
...
#if defined(CCITT)
int
ReadNumPortabilityFrwdInfoPar(char *buf, ITS_ISUP_IE *ie)
{
    int ret = ITS_SUCCESS;
    int plen = 0;             /* param len = 1 octet */
                                                                                                                             
    /* Set IE information */
    ie->param_id = ISUP_PRM_NUM_PORTABILTY_FRWD_INFO;
    ie->param_length = (ITS_OCTET)strtol(strtok(buf, " \t"), NULL, BASE_16);
    ...
    else if(ie->param_length == plen+1)
    {
        GetOctetsFromStr(&(ie->param_data.numPortabilityFrwdInfo.num_port_fwd_inf), plen);
    }
    else
    {
       printf("ReadISUPMsg: Appl Trns par plen > %d \n", plen+1);
...
/* ISUP_ITU_99 Parameters end here*/
...
*/
        ...
        case ISUP_PRM_APPLLICATION_TRANSPORT:
            ReadApplicationTransportPar(buf, &(ie[ieIndex]));
            break;
                                                                                                                             
        case ISUP_PRM_CCNR_POSSIBLE_IND:
            ReadCcnrPossibleIndPar(buf, &(ie[ieIndex]));
            break;
                                                                                                                             
        case ISUP_PRM_PIVOT_CAPABILITY:
            ReadPivotCapabilityPar(buf, &(ie[ieIndex]));
            break;
                                                                                                                             
        case ISUP_PRM_PIVOT_ROUTING_IND:
            ReadPivotRoutingIndPar(buf, &(ie[ieIndex]));
            break;
                                                                                                                             
        case ISUP_PRM_CALLED_DIR_NUM:
            ReadCalledDirNumPar(buf, &(ie[ieIndex]));
            break;
                                                                                                                             
        case ISUP_PRM_ORIGINAL_CALLED_IN_NUM:
            ReadOriginalCalledInNumPar(buf, &(ie[ieIndex]));
            break;
                                                                                                                             
        case ISUP_PRM_CALLING_GEODETIC_LOCATION:
            ReadCalledGeodeticLocationPar(buf, &(ie[ieIndex]));
            break;
                                                                                                                             
        case ISUP_PRM_HTR_INFORMATION:
            ReadHtrInfoPar(buf, &(ie[ieIndex]));
            break;
                                                                                                                             
        case ISUP_PRM_NETWORK_ROUTING_NUM:
            ReadNetworkRoutingNumPar(buf, &(ie[ieIndex]));
            break;

        case ISUP_PRM_QUERY_ON_RELEASE_CAPABILITY:
            ReadQueryOnReleaseCapabilityPar(buf, &(ie[ieIndex]));
            break;
                                                                                                                             
        case ISUP_PRM_PIVOT_STATUS:
            ReadPivotStatusPar(buf, &(ie[ieIndex]));
            break;
                                                                                                                             
        case ISUP_PRM_PIVOT_COUNTER:
            ReadPivotCounterPar(buf, &(ie[ieIndex]));
            break;
                                                                                                                             
        case ISUP_PRM_PIVOT_ROUTING_FRWD_INFO:
            ReadPivotRoutingFrwdInfoPar(buf, &(ie[ieIndex]));
            break;
                                                                                                                             
        case ISUP_PRM_PIVOT_ROUTING_BKWD_INFO:
            ReadPivotRoutingBkwdInfoPar(buf, &(ie[ieIndex]));
            break;
                                                                                                                             
        case ISUP_PRM_REDIRECT_CAPABILITY:
            ReadRedirectCapabilityPar(buf, &(ie[ieIndex]));
            break;
                                                                                                                             
        case ISUP_PRM_REDIRECT_COUNTER:
            ReadRedirectCounterPar(buf, &(ie[ieIndex]));
            break;
                                                                                                                             
        case ISUP_PRM_REDIRECT_STATUS:
            ReadRedirectStatusPar(buf, &(ie[ieIndex]));
            break;
                                                                                                                             
        case ISUP_PRM_REDIRECT_FRWD_INFO:
            ReadRedirectFrwdInfoPar(buf, &(ie[ieIndex]));
            break;
                                                                                                                             
        case ISUP_PRM_REDIRECT_BKWD_INFO:
            ReadRedirectBkwdInfoPar(buf, &(ie[ieIndex]));
            break;

        case ISUP_PRM_NUM_PORTABILTY_FRWD_INFO:
            ReadNumPortabilityFrwdInfoPar(buf, &(ie[ieIndex]));
            break;
