 ...
 ****************************************************************************
 *
 *  ID: $Id: isup_cmn.h,v 9.1 2005/03/23 12:55:20 cvsadmin Exp $
 *
...
#ident "$Id: isup_cmn.h,v 9.1 2005/03/23 12:55:20 cvsadmin Exp $"
...
#define ISUP_MSG_DATA(ev)   ((ev)->data[1 + sizeof(ITS_USHORT)])
...
#define SET_RAS_STATUS(x,b,v)    ((x)->status[(b)/8] &= ~(1 << (7-((b) % 8))), \
                                  (x)->status[(b)/8] |= ((v) << (7-((b) % 8))))
...
#define ISUP_ANSI_GENERIC_95_STRING                 "ANSI_GENERIC_95"
...
/*
 * for a circuit group, which cics do i control (none, all, or a portion)
 */
typedef enum
{
    ISUP_CKGP_CTRL_NONE,
...
SS7DLLAPI ISUP_CKGP_CTRL    ISUP_StringToCkgpCtrl(const char *ckgp_ctrl);
SS7DLLAPI const char *      ISUP_CkgpCtrlToString(ISUP_CKGP_CTRL ckgp_ctrl);
 ...
 * the call to stop. 
 */
#define ITS_ISUP_REL_GLARE          1
#define ITS_ISUP_BAD_ADDR           2
#define ITS_ISUP_CIC_BUSY           3
#define ITS_ISUP_CIC_HWBLOCK        4
#define ITS_ISUP_CIC_MNBLOCK        5
#define ITS_ISUP_BAD_RNG_STATUS     6
...
extern VSS7DLLAPI ITS_Class               isupITU97_Class;
