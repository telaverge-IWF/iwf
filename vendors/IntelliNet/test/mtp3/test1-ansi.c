 ...
 ****************************************************************************
 *
 *  ID: $Id: test1-ansi.c,v 9.1 2005/03/23 12:55:22 cvsadmin Exp $
 *
 * LOG: $Log: test1-ansi.c,v $
...
#ident "$Id: test1-ansi.c,v 9.1 2005/03/23 12:55:22 cvsadmin Exp $"
...
TrafficGenerator(TPOOL_THREAD *thr)
{
    ...
    for (i = 0; ; i++)
    {
        ...
        if (sendTraffic)
        {
            ...
            MTP3_RL_SET_OPC_VALUE(mtp3.label, 0x000008);
            if ( sndCnt & 0x00000001)
            {
                MTP3_RL_SET_DPC_VALUE(mtp3.label, 0x000009);
            }
            else
            {
                MTP3_RL_SET_DPC_VALUE(mtp3.label, 0x000102);
            }
        ...
        if (sendTraffic)
        {
	        ...
	        ITS_OCTET buf[NUM_PAYLOAD_BYTES];
                ...
                MTP3_HDR_SET_SIO(mtp3, MTP3_NIC_INTERNATIONAL | MTP3_SIO_SCCP);
                ...
                {
                    MTP3_RL_SET_DPC_VALUE(mtp3.label, 0x0102);
                }
                MTP3_RL_SET_SLS(mtp3.label, sndCnt);
...
{    
    ...
    else if (DBC_Cmd(dbc, "stopt", "Stop Traffic", "<>", "<>"))
    {
    ...
    else if (DBC_Cmd(dbc, "dumpls", "Dump Link Sets", "<>", "<>"))
    {
        char *dump = NULL;
