 ...
 ****************************************************************************
 *
 *  ID: $Id: ansi-sccp.c,v 9.11.6.7.36.1 2014/09/17 07:01:58 jsarvesh Exp $
 *
 * LOG: $Log: ansi-sccp.c,v $
 * LOG: Revision 9.11.6.7.36.1  2014/09/17 07:01:58  jsarvesh
 * LOG: Changes done for removing warnings
 * LOG:
 * LOG: Revision 9.11.6.7.30.1  2014/09/15 07:23:32  jsarvesh
 * LOG: Changes done for removing Warnings
 * LOG:
 * LOG: Revision 9.11.6.7  2010/07/14 08:59:48  ssingh
 * LOG: Fix for SMLC bug #2891
 * LOG:
 * LOG: Revision 9.11.6.6  2010/07/14 04:44:03  ssingh
 * LOG: Fix for SMLC bug #2891
 * LOG:
 * LOG: Revision 9.11.6.5  2010/06/07 10:52:03  ssingh
 * LOG: issue #2979; dynamically added concern route commiting into Routing SubSys
 * LOG:
 * LOG: Revision 9.11.6.4  2010/05/25 08:25:42  chandrashekharbs
 * LOG: SMLC::Redundancy, Fix for bug #2927
 * LOG:
 * LOG: Revision 9.11.6.3  2010/05/17 16:43:52  nvijikumar
 * LOG: Redundancy::Issue  Wiz ID:2887, Message going out from Standby node
 * LOG:
 * LOG: Revision 9.11.6.2  2010/05/03 13:23:48  chandrashekharbs
 * LOG: SMLC::Redundancy::UT fixes, Wix ID #2718
 * LOG:
 * LOG: Revision 9.11.6.1  2009/12/21 03:46:41  rajeshak
 * LOG: Removing CTF_TraceEvent function call in vendors
 * LOG:
 * LOG: Revision 9.11  2008/07/18 05:13:18  ssingh
 * LOG: SRS ID:: ACC-CG-GEN-R0070Warning removal for Solaris Compilation
 * LOG:
 * LOG: Revision 9.10  2008/06/04 10:30:24  ssingh
 * LOG: ALU porting, B-0100LX-LUSG-00 (june 04,2008 base)
 * LOG:
 * LOG: Revision 9.9  2008/05/01 13:20:04  kramesh
 ...
 * LOG:
 * LOG: Revision 9.8  2008/04/15 05:03:17  kramesh
 * LOG: Merged Persistent Branch Changes to current.
 * LOG:
 * LOG: Revision 9.7.2.1  2007/04/24 13:59:25  pspl
 * LOG: Fixed an issue with reassembly timer in ansi (Rahul)
 * LOG:
 * LOG: Revision 9.7  2007/02/01 13:25:18  yranade
 * LOG: 2nd round of merge
 * LOG:
 * LOG: Revision 9.6  2007/01/30 06:24:46  yranade
 ...
 * LOG:
 * LOG: Revision 9.4.4.11  2008/02/13 08:08:01  ssodhi
 ...
 * LOG:
 * LOG: Revision 9.4.4.10  2007/10/05 11:28:15  mshanmugam
 * LOG: ported the fix for issue 6172
 * LOG:
 * LOG: Revision 9.4.4.9  2007/05/15 14:19:19  raghavendrabk
 * LOG: Changed the function names - DeleteDupData & DeleteCPCData
 * LOG:
 * LOG: Revision 9.4.4.8  2007/04/27 05:41:40  raghavendrabk
 * LOG: Alarm Format Changes for Family handling in OAM
 * LOG:
 * LOG: Revision 9.4.4.7  2007/04/16 11:55:38  raghavendrabk
 * LOG: Changed function names - BuildCPCData and BuildDupData
 * LOG:
 * LOG: Revision 9.4.4.6  2007/04/10 15:25:13  raghavendrabk
 * LOG: Check Validation for SCCP_ANSI_TraceData
 * LOG:
 * LOG: Revision 9.4.4.5  2007/02/05 06:14:19  clin
 ...
 * LOG:
 * LOG: Revision 9.4.4.4  2007/02/02 13:58:23  rdangi
...
#ident "$Id: ansi-sccp.c,v 9.11.6.7.36.1 2014/09/17 07:01:58 jsarvesh Exp $"
...
void BuildCPCData_ANSI();
void BuildDupData_ANSI();
void DeleteCPCData_ANSI();
void DeleteDupData_ANSI();
#if defined(__cplusplus)
}
#endif /* defined(__cplusplus) */
                               ...
                               ITS_CTXT *cref)
{
    ...
    if (DSM_LockTableLocal(DSM_SccpFtGroupId, DSM_TABLE_ANSI_SCCP_CTXT) != ITS_SUCCESS)
    {
    ...
    for (i = 0; i < 65536; i++ )
    {
        ...
        if (error == ITS_ENOTFOUND)
        {
            ...
            newCtxt = (SCCP_CONN_CTXT *)DSM_AllocNoLockLocal(DSM_SccpFtGroupId,
                                                        DSM_TABLE_ANSI_SCCP_CTXT, 
                                                        (ITS_OCTET *)&nextUnique, 
...
CheckForCSS_ANSI(ROUTE_Info *linfo)
{
    ...
    {
        if (DSM_LockTableLocal(DSM_SccpFtGroupId, DSM_TABLE_ANSI_SCCP_LPC)
                                                                != ITS_SUCCESS)
        {
        ...
        if (error != ITS_SUCCESS)
        {
           DSM_UnlockTableLocal(DSM_SccpFtGroupId, DSM_TABLE_ANSI_SCCP_LPC);
    ...
    else
    {
        ...
        if (DSM_LockTableLocal(DSM_SccpFtGroupId, DSM_TABLE_ANSI_SCCP_CPC)
                                                                != ITS_SUCCESS)
        {
            SCCP_ERROR(("Can't lock table\n"));
        ...
        if (error != ITS_SUCCESS)
        {
           DSM_UnlockTableLocal(DSM_SccpFtGroupId, DSM_TABLE_ANSI_SCCP_CPC);
...
SCCP_SendSCMG_ANSI(SCCPConcernedSSInfo *data)
{
     ...
     * Lock the tables we need
     */
    if (DSM_LockTableLocal(DSM_SccpFtGroupId, DSM_TABLE_ANSI_SCCP_LPC) != ITS_SUCCESS)
    {
        SCCP_ERROR(("Can't lock table\n"));
        return (ITS_EBADMUTEX);
    }
    ...
    for (tinfo = rinfo; tinfo != NULL; tinfo = tinfo->next)
    {
                           ...
                           (tinfo->basic.dpc == data->cpc))
        {
            ...
            error = SendSCMG_ANSI(&hdr, &scmg, tinfo->basic.criteria.ssn);
...
ISCCP_Initialize_ANSI(ITS_SS7_HANDLE handle)
{
    ...
    for (i = 0; i < numThreads; i++)
    {
                                ...
                                NULL, ITS_TRUE) != ITS_SUCCESS)
        {
            ...
            /* SCCP_Alarm_ANSI(21, __FILE__, __LINE__, "family=%s", "ANSI"); */
 ...
 *  Name      Date        Reference               Description
 *  --------------------------------------------------------------------------
 *  Balaji   SEP-08-2006  Bug ID: 653        Fix for ual routing failure at 
 *                                           standby node   
 *  
 ****************************************************************************/
THREAD_RET_TYPE
           ...
           == ITS_SUCCESS)
    {
        ...
        default:
            ...
            default:
                ...
                SCCP_Alarm_ANSI(234, __FILE__, __LINE__,
                                "family=%s:mType=%x", "ANSI", SCCP_MSG_TYPE(&sccpEvent));
            }
        }
...
SCCP_Console_ANSI(DBC_Server *dbc, const char *cmdLine)
{
                     ...
                     "<point code> <ni> <ssn> [<linkset>]"))
    {
        ...
        SCCP_Alarm_ANSI(101, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x", "ANSI", pc, ssn);
    }
    ...
    else if (DBC_Cmd(dbc, "sa_set_lss", "SCCP Set Subsystem Entry", 
                     "<sspc> <ssn> <linkset>",
    ...
    else if (DBC_Cmd(dbc, "sa_set_rss", "SCCP Set Subsystem Entry",
                     "<sspc> <ssn> <linkset>",
                     "<point code> <ssn> <linkset>"))
                                                                                                                             
    {
        ...
        if (ret != ITS_SUCCESS)
        {
         ...
         sprintf(buf, "********* SCCP Subsystem Entry Set********\n");
         SCCP_Alarm_ANSI(101, __FILE__, __LINE__, " PC %x, SSN %d", pc, ssn);
...

    {
        ...
        SCCP_Alarm_ANSI(103, __FILE__, __LINE__, "family=%s:ps=%x", "ANSI", pc);
        DBC_AppendText(dbc, buf);
...

    {
        ...
        SCCP_Alarm_ANSI(106, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x", "ANSI", cpc, cssn);
        DBC_AppendText(dbc, buf);
...

    {
        ...
        SSTCStopSST_ANSI(&hdr, cpc, cssn);

        sprintf(buf, "****** SCCP Concerned Subsystem Entry Removed*****\n");
        SCCP_Alarm_ANSI(107, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x", "ANSI", cpc, cssn);
        DBC_AppendText(dbc, buf);
...
INTELLINET_SCCP_Commit_ANSI(FILE* fp)
{
    ...
    for (count = 0; count < (int)ANSI_SCCP_Timers.numTimers; count++)
    {
        fprintf(fp,"\t\t\t\t\t%s = \"%ld\"\n",SCCP_ANSI_Timers[count],
                                             (long)(ANSI_SCCP_Timers.timers[count]));
    }
 ...
 ****************************************************************************
 *  Purpose:
 *      Adds a remote SSN entry. The check for dpc-ni route is removed 
 *      for TCS SMIM.
 *
 *  Input Parameters:
 *      ITS_UINT        pc
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT
SCCP_OSS_SetRemoteSSN_ANSI(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn, ITS_OCTET linkset)
{
    ITS_INT             ret;
    SCCPSubSystemInfo   info;
 ...
 ****************************************************************************
 *  Purpose:
 *      Removes a remote SSN entry. The check for dpc-ni route is removed 
 *      for TCS SMIM.
 *
 *  Input Parameters:
 *      ITS_UINT        pc
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT
SCCP_OSS_RemRemoteSSN_ANSI(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn, ITS_OCTET linkset)
{
    ITS_INT             ret;
    SCCPSubSystemInfo   info;
    ...
    SCCP_Alarm_ANSI(100, __FILE__, __LINE__, " PC %x, SSN %d", pc, ssn);
