 ...
 ****************************************************************************
 *
 *  ID: $Id: itu-smh.c,v 9.5.8.2 2010/04/08 12:41:02 ssingh Exp $
 *
 * LOG: $Log: itu-smh.c,v $
 * LOG: Revision 9.5.8.2  2010/04/08 12:41:02  ssingh
 * LOG: SMLC-6327; temporary fix (commenting sls recalculation) for ITU stack
 * LOG: SLS value for outgoing link selection in MessageForRouting(..) function
 * LOG:
 ...
 * LOG:
  ...
  * LOG: ALU porting, B-0100LX-LUSG-00 (june 04,2008 base)
  * LOG:
 * LOG: Revision 9.3.8.16  2008/04/10 12:20:33  ssodhi
 * LOG: Fixed warnings
 * LOG:
 * LOG: Revision 9.3.8.15  2008/03/28 11:01:43  ssodhi
 * LOG: Issue 376
 * LOG:
 * LOG: Revision 9.3.8.14  2008/02/27 15:31:21  nvijikumar
 * LOG: Fix for PBN Id: 540 Overload Control
 * LOG:
 * LOG: Revision 9.3.8.13  2008/02/15 14:42:37  skatta
 * LOG: Fix for issue 536
 * LOG:
 * LOG: Revision 9.3.8.12  2008/01/30 15:06:34  ssodhi
 * LOG: Fix for issue #376
 * LOG:
 * LOG: Revision 9.3.8.11  2008/01/17 10:38:00  ssodhi
 * LOG: Call function Send_MTP2_to_MTP3_Msg_ToPeer only if msg is management msg
 * LOG:
 * LOG: Revision 9.3.8.10  2008/01/14 10:07:01  ssodhi
 * LOG: Code changes for Accelero Performance Enhancement - ALU BSG Project
 * LOG:
 * LOG: Revision 9.3.8.8.4.4  2008/01/04 07:43:53  ssodhi
 * LOG: Revision 9.3.8.8.4.3  2008/01/02 07:19:44  ssodhi
 * LOG: Revision 9.3.8.8  2007/11/16 12:10:54  mshanmugam
 * LOG: to avoid more CPU utilisation in MTP3, we are not calling MTP3_TRACE_ENTRY when DEBUG is OFF
 * LOG:
 * LOG: Revision 9.3.8.7  2007/07/22 13:03:37  raghavendrabk
 * LOG: Fix for Issue ID:6172
 * LOG:
 * LOG: Revision 9.3.8.6  2007/05/18 09:55:06  raghavendrabk
 * LOG: Changes for Alarm Optimisation
 * LOG:
 * LOG: Revision 9.3.8.5  2007/04/30 09:20:47  raghavendrabk
 * LOG: Resolved internode issue for parallel stack
 * LOG:
 * LOG: Revision 9.3.8.4  2007/02/02 14:23:29  raghavendrabk
...
#ident "$Id: itu-smh.c,v 9.5.8.2 2010/04/08 12:41:02 ssingh Exp $"
 ...
 ****************************************************************************/
int
SMH_Main_CCITT(MTP3_SubSystems src, MTP3_SubSystems dest,
               SMH_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    MTP3_TRACE_ENTRY_CCITT(MTP3_SMH, src, trigger, sif, len,
                           linkSet, linkCode);
    ...
    case SMH_TRIGGER_L2_L3_MSG:
        ...
        case L2_L3_IN_SERVICE:
            return LSAC_Main_CCITT(MTP3_L2,
                                   SLM_TRIGGER_L2_IN_SERVICE,
                                   sif, len,
                                   linkSet, linkCode);

        case L2_L3_REMOTE_PROCESSOR_OUT:
            MTP3_WARNING(("SMH_Main_CCITT: L2_L3_REMOTE_PROCESSOR_OUT: linkSet:%u linkCode:%u\n",
			  linkSet, linkCode));
            return LSAC_Main_CCITT(MTP3_L2,
                                   SLM_TRIGGER_L2_RPO,
                                   sif, len,
        ...
        case L2_L3_OUT_OF_SERVICE:
            MTP3_WARNING(("SMH_Main_CCITT: L2_L3_OUT_OF_SERVICE: linkSet:%u linkCode:%u\n",
			  linkSet, linkCode));
            return LSAC_Main_CCITT(MTP3_L2,
        ...
        case L2_L3_BSNT:
            return TCOC_Main_CCITT(MTP3_L2,
                                   STM_TRIGGER_BSNT,
                                   sif, len,
                                   linkSet, linkCode);
        ...
        case L2_L3_DATA:
            return HMDC_Main_CCITT(MTP3_L2, trigger,
                                   sif, len,
                                   linkSet, linkCode);
        ...
        case L2_L3_CONGESTION_ABATES:
            return TSRC_Main_CCITT(MTP3_L2,
        ...
        default:
            ...
            MTP3_Alarm_CCITT(2406, __FILE__, __LINE__, "type=%s:family=%s", sif[0], "CCITT");
    ...
    case SMH_TRIGGER_L4_L3_MSG:
        return HMRT_Main_CCITT(MTP3_L4, trigger,
                               sif, len,
                               linkSet, linkCode);

    default:
        PEG_IncrPeg(CCITT_MTP3_Pegs,PEG_MTP3_MSG_RECEIVED_INERROR);
        MTP3_CRITICAL(("SMH_Main_CCITT: SMH_Triggers Unknown Type\n"));
        abort();
    }
 ...
 ****************************************************************************/
int
HMDC_Main_CCITT(MTP3_SubSystems src, SMH_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    MTP3_HEADER *mtp3a;
    ...
    MTP3_TRACE_ENTRY_CCITT(MTP3_SMH_HMDC, src, trigger, sif, len,
                           linkSet, linkCode);
             ...
             MTP3_NIC_RESERVED)
    {
        ...
        else
        {
            ...
            MTP3_Alarm_CCITT(2407, __FILE__, __LINE__, "family=%s", "CCITT");
                                          ...
                                          ITS_SS7_DEFAULT_SLS)) != ITS_SUCCESS)
    {
        ...
        MTP3_Alarm_CCITT(2408, __FILE__, __LINE__, "up=%x:family=%s",up, "CCITT");
    ...
    if (isLocal)
    {
        return HMDT_Main_CCITT(MTP3_SMH_HMDC,
    ...
    {
        return HMRT_Main_CCITT(MTP3_SMH_HMDC,
 ...
 ****************************************************************************/
int
HMDT_Main_CCITT(MTP3_SubSystems src, SMH_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    SNMM_MESSAGE *snmm;
    ...
    MTP3_TRACE_ENTRY_CCITT(MTP3_SMH_HMDT, src, trigger, sif, len,
                          linkSet, linkCode);
    ...
    case MTP3_SIO_SNMM: /* signalling network management */
        ...
        case SNMM_HC0_UFC: /* user flow control */
            return STM_Main_CCITT(MTP3_SMH_HMDT, MTP3_STM,
                                  STM_TRIGGER_L2_MSG,
                                  sif, len,
                                  linkSet, linkCode);
        ...
        case SNMM_HC0_RSM: /* route set test */
            return SRM_Main_CCITT(MTP3_SMH_HMDT, MTP3_SRM,
                                  SRM_TRIGGER_L2_MSG,
                                  sif, len, linkSet, linkCode);
            
        case SNMM_HC0_DLM: /* data link connection */
            return SLM_Main_CCITT(MTP3_SMH_HMDT, MTP3_SLM,
                                  SLM_TRIGGER_L2_MSG,
                                  sif, len,
                                  linkSet, linkCode);
            
        default:
            MTP3_DEBUG(("HMDT_Main_CCITT: Invalid HCO\n"));
    ...
    case MTP3_SIO_SLTM_SPEC:
        return SLT_Main_CCITT(MTP3_SMH_HMDT, MTP3_SLT,
                              SLT_TRIGGER_L2_MSG,
                              sif, len,
                              linkSet, linkCode);
    ...
    default:
        ...
        if (dest != ITS_INVALID_SRC && dest != ITS_NO_SRC)
        {
                                     ...
                                     sizeof(ITS_OCTET)))) != ITS_SUCCESS)
            {
                ...
                MTP3_Alarm_CCITT(2501, __FILE__, __LINE__, "pc=%x:family=%s", dest, "CCITT");
            ...
            PEG_IncrPeg(CCITT_MTP3_Pegs, PEG_MTP3_MSU_RX);
        ...
        else
        {
            ...
            ret = HMRT_Main_CCITT(MTP3_SMH_HMDT,
                                  SMH_TRIGGER_MSG_FOR_ROUTING,
                                  ev.data, ev.len,
                                  linkSet, linkCode);
            ITS_EVENT_TERM(&ev);
                  ...
                  ITS_OCTET linkSet, ITS_OCTET linkCode)
{
...
#if 0
    /* FIXME:: commented as of now for fixing SMLC-2637
     * Need to find out; why was this being placed here!!
     */
    sls = (sls << 1) | ((sls >> 3) & 1);
    sls += 1;
#endif
    ...
    if (dest == ITS_NO_SRC || dest == ITS_INVALID_SRC)
    {
        ...
        {
            if ((rc = ROUTE_FindDestination(dpc, ni, FAMILY_ITU)) == NULL)
            {
                MTP3_DEBUG(("FindDestination failed.\n"));

                MTP3_Alarm_CCITT(2411, __FILE__, __LINE__, "type=%s:pc=%x:ni=%x:family=%s", 
                                 "remote", dpc, ni, "CCITT");

                MTP3_PC_SET_VALUE(pc, dpc);

                return RTPC_Main_CCITT(MTP3_SMH_HMRT,
            ...
            if (rc->status == DESTINATION_UNAVAILABLE)
            {
                ...
                MTP3_Alarm_CCITT(2412, __FILE__, __LINE__,"type=%s:pc=%x:ni=%x:family=%s","remote",dpc, ni, "CCITT");                                    
    ...
    else
    {
        if ((rc = ROUTE_FindDestination(dpc, ni, FAMILY_ITU)) == NULL)
        {
            ...
            MTP3_Alarm_CCITT(2411, __FILE__, __LINE__, "type=%s:pc=%x:ni=%x:family=%s", "remote", dpc, ni, "CCITT");
        ...
        if (rc->status == DESTINATION_RESTRICTED)
        {
            ...
            MTP3_Alarm_CCITT(2413, __FILE__, __LINE__, "type=%s:pc=%x:ni=%x:family=%s","remote", dpc, ni, "CCITT");

            RTRC_Main_CCITT(MTP3_SMH_HMRT,
        ...
        PEG_IncrPeg(CCITT_MTP3_Pegs, PEG_MTP3_MSU_TX);
        /*PEG_IncrPeg(&ln->MTP3_LINK_PEGS, PEG_LINK_MSU_TX);*/

        rc = ROUTE_FindDestination(dpc, ni, FAMILY_ITU);
        PEG_IncrPeg(&rc->MTP3_DPC_PEGS,PEG_MTP3_DPC_SIF_TX);

        return HMCG_Main_CCITT(MTP3_SMH_HMRT,
                      ...
                      ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ...
    if (ROUTE_ParseRouteMatchInfoArgs(&rinfo, dpc,
                                      ...
                                      sls) != ITS_SUCCESS)
    {
        MTP3_Alarm_CCITT(2502, __FILE__, __LINE__,
    ...
    if (dest == ITS_NO_SRC || dest == ITS_INVALID_SRC)
    {
        MTP3_Alarm_CCITT(2414, __FILE__, __LINE__, "type=%s:pc=%x:ni=%x","remote",dpc,ni);
        /* discard if it isn't */
        MTP3_Alarm_CCITT(2502, __FILE__, __LINE__,
                         "type=%s:pc=%x:ni=%x:family=%s",
                         "remote", dpc, ni, "CCITT");
                         ...
                         ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ...
    if (ROUTE_ParseRouteMatchInfoArgs(&rinfo, dpc,
                                      ...
                                      sls) != ITS_SUCCESS)
    {
        MTP3_Alarm_CCITT(2503, __FILE__, __LINE__,
                         "type=%s:pc=%x:ni=%x:family=%s", "remote", dpc, ni, "CCITT");

        /* can't recover, can't continue */
        return (ITS_EPROTERR);
    }

    d = ROUTE_FindDestination(dpc, ni, FAMILY_ITU);
    dest = ROUTE_SelectRouteContextInfo(&rinfo);
    ...
    if (SNMM_HC0_GET(*snmm) == SNMM_HC0_MIM)
    {
        ...
        if (SNMM_HC1_GET(*snmm) == SNMM_HC1_LFU)
        {
            ...
            return HMCG_Main_CCITT(MTP3_SMH_HMRT, SMH_TRIGGER_L3_L2_MSG,
                                   sif, len,
                                   linkSet,
                                   linkCode,
                                   rinfo.rkey);
    ...
    if (dest == ITS_NO_SRC || dest == ITS_INVALID_SRC)
    {
        MTP3_Alarm_CCITT(2415, __FILE__, __LINE__, "type=%s:pc=%x:ni=%x:family=%s","remote",dpc,ni, "CCITT");
    ...
    else if (SNMM_HC0_GET(*snmm) == SNMM_HC0_ECM)
    {
        ...
        {
            MTP3_Alarm_CCITT(2515, __FILE__, __LINE__, "ls=%x:link=%x:family=%s",
                   ...
                   ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ...
    if (ROUTE_ParseRouteMatchInfoArgs(&rinfo, dpc,
                                      ...
                                      sls) != ITS_SUCCESS)
    {
        MTP3_Alarm_CCITT(2504, __FILE__, __LINE__,
                        "type=%s:pc=%x:ni=%x:family=%s",
                         "remote",dpc,ni, "CCITT");
        /* can't recover, can't continue */
        return (ITS_EPROTERR);
    ...
    if (dest == ITS_NO_SRC || dest == ITS_INVALID_SRC)
    {
        MTP3_Alarm_CCITT(2416, __FILE__, __LINE__, "type=%s:pc=%x:ni=%x","remote",dpc,ni);
 ...
 ****************************************************************************/
int
HMRT_Main_CCITT(MTP3_SubSystems src, SMH_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    MTP3_TRACE_ENTRY_CCITT(MTP3_SMH_HMRT, src, trigger, sif, len,
                          linkSet, linkCode);
 ...
 ****************************************************************************/
int
HMCG_Main_CCITT(MTP3_SubSystems src, SMH_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode,
               ITS_UINT whichEntry)
{
    ...
    MTP3_TRACE_ENTRY_CCITT(MTP3_SMH_HMCG, src, trigger, sif, len,
                          linkSet, linkCode);
    ...
    if ((rc = ROUTE_FindDestination(dpc, ni, FAMILY_ITU)) == NULL)
    {
        ...
        MTP3_Alarm_CCITT(2419, __FILE__, __LINE__, "type=%s:pc=%x:ni=%x:family=%s","remote",dpc,ni, "CCITT");
    ...
    if (pri < rc->entries[whichEntry]->congLevel)
    {
        ...
        ret = MTP2_SendData_CCITT(linkSet, linkCode,
                                  L3_L2_DATA, sif, len);
 ...
 ****************************************************************************/
int
MTP2_SendData_CCITT(ITS_OCTET linkSet, ITS_OCTET linkCode,
                   ITS_OCTET command,
                   ITS_OCTET *data, ITS_USHORT len)
{
    ...
    if (command != L3_L2_DATA)
    {
        ...
        if (TRACE_IsLevelOn(MTP3_CCITT_TraceData, MTP3_TRACE_DEBUG))
        {
            ...
            {
                TRACE_RAW(MTP3_CCITT_TraceData, MTP3_TRACE_DEBUG, ("  NO DATA\n"));
            }
        }
    }
    ...
    if ((ln = LINK_FindLinkNoLock(linkSet, linkCode)) == NULL)
    {
        ...
        MTP3_Alarm_CCITT(2420, __FILE__, __LINE__, "ls=%x:link=%x:family=%s", 
                         linkSet, linkCode, "CCITT");
    ...
    if (TRACE_IsLevelOn(MTP3_CCITT_TraceData, MTP3_TRACE_EVENT))
    {
        ITS_UINT gmask = 0;
        ITS_USHORT src = ITS_SCCP_CCITT_SRC;
        ITS_INT output = 0;
 ...
 ****************************************************************************/
int
MTP2_ReceiveData_CCITT(ITS_EVENT *ev,
                      ITS_OCTET *linkSet, ITS_OCTET *linkCode,
                      ITS_OCTET *data, ITS_USHORT *len)
{
    ...
    *data = L2_MSG_TYPE(ev);
    ...
    if (TRACE_IsLevelOn(MTP3_CCITT_TraceData, MTP3_TRACE_EVENT))
    {
        ...
        //CTF_TraceEvent(ev->src, ITS_MTP3_CCITT_SRC,
        //    output, ev);
    }
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
MTP3_INodeEvent_CCITT(TPOOL_THREAD *thr, ITS_EVENT *ev)
{
    ...
    case L3_L2_LOCAL_PROCESSOR_REC:
    {
        ...
        MTP2_SendData_CCITT(L2_LINK_SET(ev),
                            L2_LINK_CODE(ev),
                            L2_MSG_TYPE(ev),
                            L2_LINK_DATA(ev),
                            (ITS_USHORT)(ev->len -
