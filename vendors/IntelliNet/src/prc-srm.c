 ...
 ****************************************************************************
 *
 *  ID: $Id: prc-srm.c,v 1.4 2008/06/17 06:39:08 nvijikumar Exp $
 *
 * LOG: $Log: prc-srm.c,v $
 ...
 * LOG: Add prc files
 * LOG:
 * LOG: Revision 1.1.4.3  2007/05/18 10:03:10  raghavendrabk
 * LOG: Changes for Alarm Optimisation
 * LOG:
 * LOG: Revision 1.1.4.2  2007/02/06 12:02:14  ssingh
...
#ident "$Id: prc-srm.c,v 1.4 2008/06/17 06:39:08 nvijikumar Exp $"
 ...
 ****************************************************************************/
int
SRM_Main_PRC(MTP3_SubSystems src, MTP3_SubSystems dest,
              ...
              ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_SRM, src, trigger, sif, len,
                           linkSet, linkCode);
    ...
    if (trigger == SRM_TRIGGER_L2_MSG)
    {
        ...
        case SNMM_HC0_FCM: /* transfer controlled/congestion */
            ...
            case SNMM_HC1_RCT: /* route congestion test */
                return RCAT_Main_PRC(src, SRM_TRIGGER_CONG_TEST,
                                       sif, len,
                                       linkSet, linkCode);

            case SNMM_HC1_TFC: /* transfer controlled */
                return RTCC_Main_PRC(src, SRM_TRIGGER_TRANSFER_CONTROL,
                                       sif, len,
                                       linkSet, linkCode);
            default:
 ...
 ****************************************************************************/
ITS_OCTET
MTP3_FindAdjacent_PRC(ITS_OCTET ni, ITS_UINT pc,
                        ITS_OCTET linkSet)
{
 ...
 ****************************************************************************/
int
RTPC_Main_PRC(MTP3_SubSystems src, SRM_Triggers trigger,
                ITS_OCTET *sif, ITS_USHORT len,
                ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_SRM_RTPC, src, trigger, sif, len,
                           linkSet, linkCode);
    ...
    case SRM_TRIGGER_TRANSFER_PROHIBIT:      /* from HMDT */
        ...
        linkSet = MTP3_FindAdjacent_PRC(ni, pc2, linkSet);

        rc = ROUTE_FindDestination(pc2, ni, FAMILY_CHINA);
        if (rc)
        {
        ...
        if (linkSet == ITS_SS7_DEFAULT_LINK_SET)
        {
            MTP3_ERROR(("%08x might think we're adjacent, but we don't.\n",
                         pc2));
            MTP3_Alarm_PRC(2448, __FILE__, __LINE__, "pc=%x:family=%s",pc2, "PRC"); 
    ...
    case SRM_TRIGGER_MSG_FOR_INACCESS_SP:    /* from HMRT */
                                      ...
                                      ITS_FALSE)) == NULL)
        {
            ...
            rc = ROUTE_FindDestination(pc1, ni, FAMILY_CHINA);
            if (rc)
            {
             ...
             * available route.
             */
            ret = HMRT_Main_PRC(MTP3_SRM_RTPC,
    ...
    case SRM_TRIGGER_NEW_STP_DEST:           /* from TSRC */
        ...
        if (rc->entries[whichEntry]->t8 == TIMER_BAD_SERIAL)
        {
            ...
            rc = ROUTE_FindDestination(pc1, ls->ni, FAMILY_CHINA);
            if (rc)
            {
                PEG_IncrPeg(&rc->MTP3_DPC_PEGS, PEG_MTP3_DPC_TFP_TX);
            }
 ...
 ****************************************************************************/
int
RTAC_Main_PRC(MTP3_SubSystems src, SRM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    MTP3_HEADER *mtp3 = (MTP3_HEADER *)&sif[1];
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_SRM_RTAC, src, trigger, sif, len,
                           linkSet, linkCode);
    ...
    case SRM_TRIGGER_TRANSFER_ALLOW:         /* from HMRT */
        ...
        PEG_IncrPeg(PRC_MTP3_Pegs, PEG_MTP3_TFA_RECEIVED);
        ...
        if (linkSet == ITS_SS7_DEFAULT_LINK_SET)
        {
            ...
            MTP3_Alarm_PRC(2451, __FILE__, __LINE__, "pc=%x:family=%s",pc2, "PRC");
    ...
    case SRM_TRIGGER_DEST_ACCESS:            /* from TSRC */
        ...
        for (pc2 = 0; pc2 < ITS_SS7_DEFAULT_LINK_SET; pc2++)
        {
            ...
            ret = HMRT_Main_PRC(MTP3_SRM_RTAC,
                                 SMH_TRIGGER_ROUTE_MGMT_MSG,
                                 ev.data, ev.len,
                                 ITS_SS7_DEFAULT_LINK_SET,
                                 ITS_SS7_DEFAULT_LINK_CODE);
 ...
 ****************************************************************************/
int
RSRT_Main_PRC(MTP3_SubSystems src, SRM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    MTP3_HEADER *mtp3 = (MTP3_HEADER *)&sif[1];
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_SRM_RSRT, src, trigger, sif, len,
                           linkSet, linkCode);
    ...
    case SRM_TRIGGER_START_ROUTE_SET_TEST:   /* from RTRC: RTPC: TSRC */
        ...
        if (rc->entries[whichEntry]->testState == ROUTE_TEST_IDLE)
        {
            if (src == MTP3_STM_TSRC)
            {
                ...
                /* USE any available route */
                ret = HMRT_Main_PRC(MTP3_SRM_RSRT,
                                     SMH_TRIGGER_ROUTE_MGMT_MSG,
                                     ev.data, ev.len,
                                     ITS_SS7_DEFAULT_LINK_SET,
                                     ITS_SS7_DEFAULT_LINK_CODE);
        ...
        if (STPFunction_PRC())
        {
            if (!SPRestarting_PRC())
            {
                ...
                if (linkSet == ITS_SS7_DEFAULT_LINK_SET)
                {
                    ...
                    MTP3_Alarm_PRC(2456, __FILE__, __LINE__, "pc=%x:family=%s",pc2, "PRC");
    ...
    case SRM_TRIGGER_ACCESSIBILITY_DATA:     /* from TSRC */
        ...
        if (rc->entries[whichEntry]->testState == ROUTE_TEST_WAIT_2)
        {
                      ...
                      onlyThroughY))
            {
                ...
                ret = RTPC_Main_PRC(MTP3_SRM_RSRT,
                     ...
                     ri && ri->priority != 0)
            {
                ...
                ret = RTRC_Main_PRC(MTP3_SRM_RSRT,
            ...
            else
            {
                ...
                ret = RTAC_Main_PRC(MTP3_SRM_RSRT,
 ...
 ****************************************************************************/
int
RTRC_Main_PRC(MTP3_SubSystems src, SRM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    MTP3_HEADER *mtp3 = (MTP3_HEADER *)&sif[1];
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_SRM_RTRC, src, trigger, sif, len,
                           linkSet, linkCode);
    ...
    case SRM_TRIGGER_TRANSFER_RESTRICT:      /* from HMDT */
        ...
        if (linkSet == ITS_SS7_DEFAULT_LINK_SET)
        {
            ...
            MTP3_Alarm_PRC(2459, __FILE__, __LINE__, "pc=%x:family=%s",pc2, "PRC");
    ...
    case SRM_TRIGGER_MSG_FOR_RESTRICTED_DEST: /* from HMRT, TSRC */
        ...
        if (!(rc->entries[whichEntry]->flags & ROUTE_SENT_TFR_TWICE))
        {
            ...
            else
            {
                ...
                ret = HMRT_Main_PRC(MTP3_SRM_RTRC,
                                     SMH_TRIGGER_ROUTE_MGMT_MSG,
                                     ev.data, ev.len,
                                     ITS_SS7_DEFAULT_LINK_SET,
                                     ITS_SS7_DEFAULT_LINK_CODE);
 ...
 ****************************************************************************/
int
RTCC_Main_PRC(MTP3_SubSystems src, SRM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    MTP3_HEADER *mtp3 = (MTP3_HEADER *)&sif[1];
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_SRM_RTCC, src, trigger, sif, len,
                           linkSet, linkCode);
    ...
    case SRM_TRIGGER_TRANSFER_CONTROL:       /* from HMDT */
        ...
        PEG_IncrPeg(PRC_MTP3_Pegs, PEG_MTP3_TFC_RECEIVED);
        ...
        if ((rc = ROUTE_FindBestMatch(&rinfo, &whichEntry,
                                      ...
                                      ITS_FALSE)) != NULL)
        {
            ret = TRCC_Main_PRC(MTP3_SRM_RTCC,
        ...
        else
        {
            ...
            MTP3_Alarm_PRC(2477, __FILE__, __LINE__,"type=%s:pc=%x:ni=%x:family=%s","remote",pc1,ni, "PRC");
        }
        break;

    case SRM_TRIGGER_USER_PART_UNAVAIL:      /* from HMDT */
        return TSFC_Main_PRC(MTP3_SRM_RTCC,
                              STM_TRIGGER_UPU,
                              sif, len,
                              linkSet, linkCode);
    ...
    case SRM_TRIGGER_SEND_TRANSFER_CONTROL:  /* from HMCG */
        ...
        if (isLocal)
        {
            ...
            ret = TSFC_Main_PRC(MTP3_SRM_RTCC,
                                 STM_TRIGGER_DEST_CONGESTION_STATUS,
                                 ev.data, ev.len,
                                 ITS_SS7_DEFAULT_LINK_SET,
                                 ITS_SS7_DEFAULT_LINK_CODE);
 ...
 ****************************************************************************/
int
RCAT_Main_PRC(MTP3_SubSystems src, SRM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    MTP3_HEADER *mtp3 = (MTP3_HEADER *)&sif[1];
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_SRM_RCAT, src, trigger, sif, len,
                           linkSet, linkCode);
    ...
    case SRM_TRIGGER_START_CONG_TEST:        /* from TRCC */
                                      ...
                                      ITS_FALSE)) != NULL)
        {
            ...
            /* send via any route */
            ret = HMRT_Main_PRC(MTP3_SRM_RCAT,
                                 SMH_TRIGGER_ROUTE_MGMT_MSG,
                                 ev.data, ev.len,
                                 ITS_SS7_DEFAULT_LINK_SET,
                                 ITS_SS7_DEFAULT_LINK_CODE);
    ...
    case SRM_TRIGGER_T16:                    /* from timer control */
                                      ...
                                      ITS_FALSE)) != NULL)
        {
            ...
            ret = TRCC_Main_PRC(MTP3_SRM_RCAT,
