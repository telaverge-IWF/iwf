 ...
 ****************************************************************************
 *
 *  ID: $Id: ttc-cco.c,v 9.2 2008/06/04 10:38:48 ssingh Exp $
 *
 * LOG: $Log: ttc-cco.c,v $
 ...
 * LOG:
 * LOG: Revision 1.1.2.3  2007/02/08 14:38:41  raghavendrabk
 * LOG: Changed alarm format to put family at beginning
 * LOG:
 * LOG: Revision 1.1.2.2  2007/02/02 14:15:46  raghavendrabk
...
#ident "$Id: ttc-cco.c,v 9.2 2008/06/04 10:38:48 ssingh Exp $"
 ...
 ****************************************************************************/
int
CCOHandleAPPMsg_TTC(ITS_EVENT *event)
{
    /*
     * function has been updated to handle TTC 97. You will notice
     * no check for TTC variant here, since it was done previously in
    ...
    case TCPPT_TC_U_CANCEL:
        ...
        cptCtxt = TCAP_FindInvokeContext_TTC(did, &cpt.u.invoke.invoke_id, 
                                               INV_OUTGOING);
        ...
        if (!cptCtxt->inUse)
        {
            ...
            case TCPPT_TC_INVOKE:
                ...
                if (cpt.u.invoke.correlation_id.len != 0)
                {
                    ...
                    corCtxt = TCAP_FindInvokeContext_TTC(did,
                    ...
                    if (corCtxt == NULL || !corCtxt->inUse)
                    {
                        ...
                        PEG_IncrPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_CMP_DISCARDED);
                        ...
                        TCAP_Alarm_TTC(1401, __FILE__, __LINE__,
                                       "family=%s:Dialog-Id=%x", "TTC", did); 

                        return (ITS_SUCCESS);
                    }
                }
                /* Send to DHA (just enqueue it) */
                if (DHAHandleCCOMsg_TTC(event) != ITS_SUCCESS)
                {
                ...
                else
                {
                    ...
                    PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_INVOKE_SENT);
                    PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_CMP_SENT);
            ...
            case TCAP_PT_TC_TIMER_RESET:
                ...
                /* Discarding Reset Timer Request for non-existent invoke
                   operation. */
                TCAP_Alarm_TTC(1403, __FILE__, __LINE__,
                               "family=%s:Dialog-Id=%x", "TTC", did); 
        ...
        else /* context is inUse */
        {
            ...
            case TCPPT_TC_U_CANCEL:
                ...
                if (DHAHandleCCOCancelMsg_TTC(did,
            ...
            case TCAP_PT_TC_TIMER_RESET:
                ...
                if (DHAHandleCCOResetTimerMsg_TTC(cptCtxt) != ITS_SUCCESS)
                {
    ...
    case TCPPT_TC_U_ERROR:
        ...
        else /* we were waiting on the application */
        {
            ...
            case TCPPT_TC_U_ERROR:
                ...
                {
                    PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_ERROR_SENT);
                    PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_CMP_SENT);
                    PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_MSG_SENT); 
                    tcapOmImage[omBuf.activeOm].appMsgSent++;
                    tcapOmImage[omBuf.activeOm].appCmpSent++;
            ...
            case TCPPT_TC_RESULT_L:
                ...
                {
                    PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_RESULT_SENT);
                    PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_CMP_SENT);
                    PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_MSG_SENT);
                    tcapOmImage[omBuf.activeOm].appMsgSent++;
                    tcapOmImage[omBuf.activeOm].appCmpSent++;
    ...
    case TCPPT_TC_R_REJECT:
        ...
        cptCtxt = TCAP_FindInvokeContext_TTC(did, &cpt.u.invoke.invoke_id,
                                               INV_OUTGOING);
        /*
         * In that case it is tricky to find the invoke context. In TTC
        ...
        if (cptCtxt && (cptCtxt->inUse)) /* if NULL try INV_INCOMING */
        {
             ...
             * try INCOMING. Remember that in TTC we can have 2 invokeCtxt
            ...
            if (cptCtxt->state == ITS_CPT_OP_AWAIT_REJECT)
            {
                ...
                */
                ...
                {
                    PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_REJECT_SENT);
                    PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_CMP_SENT);
                    PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_MSG_SENT);
                    tcapOmImage[omBuf.activeOm].appMsgSent++;
                    tcapOmImage[omBuf.activeOm].appCmpSent++;
        ...
        if (!cptCtxt->inUse) /* can't find the operation we're sending to */
        {
            ...
            /* Outgoing Component - Discarding Component -
               Rejection Sent by stack. */
            TCAP_Alarm_TTC(1401, __FILE__, __LINE__, "family=%s:Dialog-Id=%x", "TTC", 
                           did);
        }
        else
        {
 ...
 ****************************************************************************/
int
CCOHandleDHAMsg_TTC(ITS_CTXT did, TCAP_CPT *cpt)
{
    TCAP_INVOKE_ENTRY *cptCtxt = NULL, *corCtxt = NULL;
    ITS_EVENT ev;
    ITS_HDR hdr;
    ...
    if (TCAP_VendorComponentSanityCheck(cpt) != ITS_SUCCESS)
    {
        ...
        TCAP_Alarm_TTC(1404, __FILE__, __LINE__,
                       "family=%s:Dialog-Id=%x:ComponentType=%x",  "TTC", did, 
                       cpt->ptype); 
    ...
    switch (cpt->ptype)
    {
        ...
        cptCtxt = TCAP_FindInvokeContext_TTC(did,
                                               &cpt->u.invoke.invoke_id,
                                               INV_INCOMING);
         if (did && (cptCtxt == NULL))
        {
                ...
                if (cpt->u.invoke.correlation_id.len != 0)
                {
                    ...
                    if (corCtxt == NULL || !corCtxt->inUse)
                    {
                        ...
                        __TCAP_SendAppReject_TTC(did, cpt,
                         ...
                         PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_CMP_DISCARDED); 
                         
                        /* Incoming Component - Discarding Component -
                           Rejection Sent by stack. */
                        TCAP_Alarm_TTC(1406, __FILE__, __LINE__,
                                       "family=%s:Dialog-Id=%x", "TTC", did); 
                    }
                }
            }
        ...
        else  /* context is not NULL */
        {
            ...
            /* Incoming Component - Discarding Component -
               Rejection Sent by stack. */
            TCAP_Alarm_TTC(1406, __FILE__, __LINE__, "family=%s:Dialog-Id=%x", "TTC", 
                           did); 
                     ...
                     if (TIMERS_GetTimerValue(&TTC_TCAP_Timers, TCAP_T_reject))
                     {
                         ...
                         if ((cptCtxt->timer =
                                 TIMERS_StartSharedTimer(ITS_TCAP_SRC,
                                                         TIMERS_GetTimerValue(&TTC_TCAP_Timers,
                                                         TCAP_T_reject),
                                                         cptCtxt,
    ...
    default:
        ...
        /* Incoming Component - Discarding Component -
           Rejection Sent by stack. */
        TCAP_Alarm_TTC(1406, __FILE__, __LINE__, "family=%s:Dialog-Id=%x", "TTC", did); 
 ...
 ****************************************************************************/
int
CCOHandleTimeout_TTC(ITS_EVENT *event)
{
    ...
    sharedTx = TCAP_FindTransactionContext_TTC(ie.ltid,
                                                 tidKey, &tidKeyLen);
    if (sharedTx)
    {
       localTx = TCAP_CopyDSMTxToLocalTx_TTC(sharedTx);
    }
    else
    {
    ...
    tmp = TCAP_FindInvokeContextWithTimer_TTC(ie.did,
                                                ie.invoke_id, 
                                                td->serial);                                                
    if (tmp == NULL)
    {
    ...
    TCAP_FreeInvokeContext_TTC(tmp);

    /* copy local ctxt in the DSM */
    memcpy(sharedTx, localTx, sizeof(TCAP_TRANS_ENTRY));

    ret = DSM_Commit(DSM_TcapFtGroupId, DSM_TABLE_TTC_TCAP_DLG,
    ...
    if (ie.state != ITS_CPT_OP_AWAIT_REJECT)
    {
        ...
        PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_LOCAL_CANCEL);
