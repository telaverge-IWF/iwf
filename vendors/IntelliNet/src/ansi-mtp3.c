 ...
 ****************************************************************************
 *
 *  ID: $Id: ansi-mtp3.c,v 9.9.8.1 2010/06/09 20:20:57 omayor Exp $
 *
 * LOG: $Log: ansi-mtp3.c,v $
 * LOG: Revision 9.9.8.1  2010/06/09 20:20:57  omayor
 * LOG: Committing fixes for TCS-SMLC Issues:2863, 2864, 2865, 2870, T15, T16 logic
 * LOG:
 * LOG: Revision 9.9  2008/06/04 10:30:24  ssingh
 * LOG: ALU porting, B-0100LX-LUSG-00 (june 04,2008 base)
 * LOG:
 * LOG: Revision 9.8  2008/05/25 17:16:17  kramesh
 * LOG: Whizible Bug Id 915. Automatic link alignment.
 * LOG:
 ...
 * LOG:
 * LOG: Revision 9.4  2007/01/30 06:24:46  yranade
 * LOG: Get things to compile.
 * LOG:
 * LOG: Revision 9.3.6.9  2008/02/13 09:39:42  ssodhi
 * LOG: In MTP3_TransportPostInit Function, used ITS_TERM_EVENT to free
 * LOG: the event (to fix Purify Observations).
 * LOG:
 * LOG: Revision 9.3.6.8  2007/11/16 12:06:48  mshanmugam
 * LOG: to avoid more CPU utilisation in MTP3, we are not calling MTP3_TRACE_ENTRY when DEBUG is OFF
 * LOG:
 * LOG: Revision 9.3.6.7  2007/05/18 09:46:43  raghavendrabk
 * LOG: Changes for Alarm Optimisation
 * LOG:
 * LOG: Revision 9.3.6.6  2007/04/10 15:24:28  raghavendrabk
 * LOG: Check Validation for MTP3_ANSI_TraceData
 * LOG:
 * LOG: Revision 9.3.6.5  2007/02/05 05:56:58  clin
 * LOG: for MTP3_Resume, need to check each entry in ROUTE_Info, only care about
 * LOG: REMOTE_ROUTE entry
 * LOG:
 * LOG: Revision 9.3.6.4  2007/02/02 14:06:57  raghavendrabk
 * LOG: Parallel Stack changes for Alarms and Statistics
 * LOG:
 * LOG: Revision 9.3.6.3  2007/01/15 15:30:17  ssingh
 * LOG: taking back trigger given to OAM for mtp3 start
 * LOG:
 * LOG: Revision 9.3.6.2  2006/11/22 09:12:00  ssingh
 * LOG: moving mtp3 start to OAM/Application
 * LOG:
 * LOG: Revision 9.3.6.1  2006/10/07 13:27:28  ssingh
 * LOG: MTP3 Start in PostInit, #3505
 * LOG:
...
#ident "$Id: ansi-mtp3.c,v 9.9.8.1 2010/06/09 20:20:57 omayor Exp $"
                     ...
                     count++, tmp = DSMQUEUE_NEXT(link->linkSet->routes, tmp) )
                {
                    ...
                    if (firstUp)
                    {
                        if (rinfo->dest)
                        {
                            for (index = 0; index < rinfo->dest->numEntries; index++)
                            {
                                if (rinfo->dest->entries[index]->basic.type
                                                              == REMOTE_ROUTE)
                                {
                                    if (rinfo->dest->entries[index]->linkSet->linkSet
                                        != link->linkSet->linkSet &&
                                        rinfo->dest->entries[index]->linkSet->isUp)
                                    {
                                        firstUp = ITS_FALSE;
...
HandleTimerEvent(ITS_TimerData *td)
{
    ...
    case MTP3_T15:
        ...
        ret = TRCC_Main_ANSI(MTP3_TIMER, STM_TRIGGER_T15,
                             ev.data, ev.len,
                             ctxt.data.routeInfo.linkSet,
                             ITS_SS7_DEFAULT_LINK_SET);
    ...
    case MTP3_T25:
        ...
        ret = TSRC_Main_ANSI(MTP3_TIMER, STM_TRIGGER_T25,
                             ev.data, ev.len,
                             ctxt.data.routeInfo.linkSet,
                             ITS_SS7_DEFAULT_LINK_SET);
        ITS_EVENT_TERM(&ev);
        ...
        default:
            ...
            else
            {
                                          ...
                                          buf, &len) != ITS_SUCCESS)
                {
                    ...
                    MTP3_Alarm_ANSI(2005, __FILE__, __LINE__, NULL);
                }
                else
                {
 ...
 *  Name    Date         Reference       Description
 *  ------------------------------------------------------------------------
 *  ssingh  07-10-2006   BugId: 3505     porting Delayed link alignment fix
 *                                       from ITU to ANSI
 *  kramesh 05-25-2008   BugId: 915      Automatic link alignment in ANSI.
 ****************************************************************************/
int 
