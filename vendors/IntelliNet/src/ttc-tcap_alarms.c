 ...
 ****************************************************************************
 *
 *  ID:$Id: ttc-tcap_alarms.c,v 9.1 2007/01/11 12:43:41 yranade Exp $
 *
 * LOG: $Log: ttc-tcap_alarms.c,v $
 ...
 ****************************************************************************/
void
TCAP_Alarm_TTC(int which, const char *file, const int line,
                 char *format, ...)
{
    ...
    ITS_C_ASSERT(which >= TTC_TCAP_ALARM_START &&
                 which <= TTC_TCAP_ALARM_END);

    fmt = TTC_TCAP_Alarms[which - TTC_TCAP_ALARM_START];

    if (fmt->severity > TCAP_AlarmLevel_TTC)
    {
        return;
    }
