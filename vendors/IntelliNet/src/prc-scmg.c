 ...
 ****************************************************************************
 *
 *  ID: $Id: prc-scmg.c,v 1.7 2008/07/18 05:39:51 ssingh Exp $
 *
 * LOG: $Log: prc-scmg.c,v $
 * LOG: Revision 1.7  2008/07/18 05:39:51  ssingh
 * LOG: SDS ID:: ID :: D0240 Warning removal for Solaris Compilation
 * LOG:
 * LOG: Revision 1.6  2008/06/24 11:31:05  ssingh
 * LOG: fix for unit testing issue #1008
 * LOG:
 ...
 * LOG:
 * LOG: Revision 1.2.2.1  2007/05/18 06:47:41  pspl
 ...
 * LOG: Add prc files
 * LOG:
 * LOG: Revision 1.1.2.7  2007/07/22 13:26:12  raghavendrabk
 * LOG: Propagated the fix given for Issue ID:6172
 * LOG:
 * LOG: Revision 1.1.2.6  2007/07/06 14:19:46  ssingh
 * LOG: Fix to stop sending SST when the Node is Standby
 * LOG:
 * LOG: Revision 1.1.2.5  2007/04/30 11:17:56  sbhaskaran
 * LOG: Fix for stopping the Backup Node to send SCMG messages
 * LOG:
 * LOG: Revision 1.1.2.4  2007/02/13 04:05:04  raghavendrabk
 * LOG: Changed alarm format to put family at beginning
 * LOG:
 * LOG: Revision 1.1.2.3  2007/02/06 12:02:13  ssingh
 * LOG: Fixing issue:: Accelero 6.7:: PRC links are not aligning (#5340)
 * LOG:
 * LOG: Revision 1.1.2.2  2007/02/02 14:32:28  raghavendrabk
...
#ident "$Id: prc-scmg.c,v 1.7 2008/07/18 05:39:51 ssingh Exp $"
...
extern void BuildCPCData_PRC();
extern void BuildDupData_PRC();
 ...
 ****************************************************************************/
int
SendSCMG_PRC(MTP3_HEADER* mtp3, SCCP_SCMG* scmg, ITS_UINT toSSN)
{
    ITS_SCCP_IE lies[32];
    ITS_OCTET addrInd;
    ...
    if (ITS_RedundancyClass != NULL)
    {
        ...
        {
            SCCP_DEBUG(("SendSCMG_PRC: Backup node should not send SCMG msgs\n"));
            return (ITS_SUCCESS);
        }
    }

    SCCP_DEBUG(("SendSCMG_PRC: type %02x ssn %02x pc %08x\n",
    ...
    return SCLCHandleSCMGMsg_PRC(mtp3, SCCP_MSG_UDT, lies, 4);
}
...
SSTCStartSST_PRC(MTP3_HEADER* mtp3, ITS_UINT pc, ITS_OCTET ssn)
{
    ...
    skey = DSM_MGMT_REC_PRC_SCCP_SST_SERIAL;
 ...
 ****************************************************************************/
void
SSTCStopSST_PRC(MTP3_HEADER* mtp3, ITS_UINT pc, ITS_OCTET ssn)
{
    SST_Key key;
    ROUTE_Info* tmp;
    ITS_UINT size, skey;
 ...
 ****************************************************************************/
ITS_BOOLEAN
SCMGIsSSTPending_PRC(ROUTE_Info *rinfo)
{
    ITS_INT error;
    SST_Key key;
    ROUTE_Info* tmp;
...
BCSTSendSSA(MTP3_HEADER* mtp3, SCCP_SCMG* oscmg, ITS_BOOLEAN fromCPC)
{
    ...
    if (!fromCPC)
    {
        ...
        for (tinfo1 = rinfo; rinfo != NULL; rinfo = rinfo->next)
        {
            ...
            {
                SendSCMG_PRC(&hdr, &scmg, rinfo->basic.criteria.ssn);
            }
            else if (rinfo->basic.style == ROUTE_DPC_SIO_SSN_RANGE ||
                     rinfo->basic.style == ROUTE_DPC_OPC_SIO_SSN_RANGE)
            {
                SendSCMG_PRC(&hdr, &scmg, rinfo->basic.criteria.range.begin);
            }
        }
    ...
    else
    {
        ...
        for (tinfo1 = linfo; linfo != NULL; linfo = linfo->next)
        {
            ...
            for (tinfo2 = rinfo; rinfo != NULL; rinfo = rinfo->next)
            {
                ...
                SendSCMG_PRC(&hdr, &scmg, localSSN);
            }
        }
                        ...
                        ITS_OCTET ni, ITS_UINT pc, ITS_OCTET ssn)
{
    ...
    MTP3_RL_SET_OPC_VALUE(hdr.label, 
                          MTP3_PC_GET_VALUE_PRC(uos->pointCode));
    ...
    skey = DSM_MGMT_REC_PRC_SCCP_SOR_SERIAL;
    if ((serial = DSM_FindNoLock(DSM_SccpFtGroupId,
                                 DSM_TABLE_MGMT_DATA,
                                 (ITS_OCTET *)&skey,
                                 sizeof(ITS_UINT),
...
SCMG_SendSOGToDuplicate(MTP3_HEADER *mtp3, SCCP_SCMG *sor)
{
    ...
    SendSCMG_PRC(&resp, &scmg, sor->ssn);
 ...
 *
 *  Notes:
 *      There's a comment in SCRCHandleSCLCMsg_PRC that explains why
...
SendSST(ITS_POINTER data, void *in, void *out)
{
    ...
    {
        SendSCMG_PRC(&tmp->sourceHdr, &scmg,
 ...
 ****************************************************************************/
int
SCMGSSPCHandleMsgForProhibitedSSN_PRC(MTP3_HEADER* mtp3, ITS_OCTET mType,
                                        ...
                                        ITS_OCTET reason)
{
    ...
    if (SCCP_IsSSNEquipped(ni, pc, ssn))
    {
        ...
        return SendSCMG_PRC(&resp, &scmg, toSSN);
    }
 ...
 ****************************************************************************/
int
SCMGHandleSCLCMsg_PRC(MTP3_HEADER* mtp3, ITS_OCTET mType,
                        ITS_SCCP_IE* ies, int ieCount)
{
    int i, error;
    SCCP_SCMG scmg;
    ...
    if (data == NULL || clp == NULL)
    {
        ...
        SCCP_Alarm_PRC(450, __FILE__, __LINE__, "family=%s", "PRC");
    ...
    case SCCP_SCMG_SS_ALLOWED:
        ...
        PEG_IncrPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_SSA_RCVD);

        SCCP_Alarm_PRC(443, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                       "PRC", MTP3_RL_GET_OPC_VALUE(mtp3->label), scmg.ssn);
        ...
        SSTCStopSST_PRC(mtp3, pc, scmg.ssn);

        if (isAllowed)
        {
            /* Already in the allowed state SSN=%x. */
            SCCP_Alarm_PRC(446, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                           "PRC", pc, scmg.ssn);
        ...
        SCCP_Alarm_PRC(455, __FILE__, __LINE__,"family=%s:pc=%x:ssn=%x",
                       "PRC", MTP3_RL_GET_OPC_VALUE(mtp3->label), scmg.ssn);
        break;
    ...
    case SCCP_SCMG_SS_PROHIBIT:
        ...
        PEG_IncrPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_SSP_RCVD);

        /* Q752 Implementation */
            SsnOmImage[omBuf.activeOm].SCCPSSNPegs[ssn].ssn = ssn;
            SsnOmImage[omBuf.activeOm].SCCPSSNPegs[ssn].messageSSPRcvd++;

        SCCP_Alarm_PRC(444, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                       "PRC", MTP3_RL_GET_OPC_VALUE(mtp3->label), scmg.ssn);
        ...
        if (!isAllowed)
        {
            ...
            SCCP_Alarm_PRC(447, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                           "PRC", pc, scmg.ssn);
            return (ITS_SUCCESS);
        }
        ...
        SSTCStartSST_PRC(mtp3, pc, scmg.ssn);
        ...
        /* State change to Prohibted State SSN=%x */
         SCCP_Alarm_PRC(456, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x", "PRC", pc,
                        scmg.ssn);
    ...
    case SCCP_SCMG_SS_STATUS_TEST:
        ...
        skey = DSM_MGMT_REC_PRC_SCCP_SST_IGNORE_SERIAL;
        if ((serial = DSM_FindNoLock(DSM_SccpFtGroupId,
                                     DSM_TABLE_MGMT_DATA,
                                     (ITS_OCTET *)&skey,
                                     sizeof(ITS_UINT),
    ...
    case SCCP_SCMG_SS_CONGESTED:
        ...
        if (!isAllowed)
        {
            SCCP_Alarm_PRC(448, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                           "PRC", pc, scmg.ssn);
            return (ITS_SUCCESS);
        }
    ...
    case SCCP_SCMG_SS_UIS:
        ...
        if (!isLocal)
        {
            SCCP_Alarm_PRC(454, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                           "PRC", pc, scmg.ssn);
            /* Discarding Message - Invalid or Corrupted. */
        ...
        if (isAllowed)
        {
            SCCP_Alarm_PRC(451, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                           "PRC", pc, scmg.ssn);
    ...
    case SCCP_SCMG_SS_UOS:
    {
        ...
        if (!isLocal)
        {
            SCCP_Alarm_PRC(453, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                           "PRC", pc,scmg.ssn);
            /* How the heck did we get this */
        ...
        if (!isAllowed)
        {
            ...
            SCCP_Alarm_PRC(452, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                           "PRC", MTP3_RL_GET_OPC_VALUE(mtp3->label), scmg.ssn);
    ...
    default:
        ...
        SCCP_Alarm_PRC(449, __FILE__, __LINE__, "family=%s", "PRC");
        break;
    }
                                        ...
                                        sizeof(ITS_UINT), &error)) != NULL)
            {
                for (tinfo2 = rinfo; rinfo != NULL; rinfo = rinfo->next)
                {
                    if (rinfo->basic.dpc == pc)
                    {
                        if (rinfo->basic.style == ROUTE_DPC_OPC_SIO_SSN)
                        {
                            SSTCStopSST_PRC(mtp3, pc, rinfo->basic.criteria.ssn);
                        ...
                        else if (rinfo->basic.style == ROUTE_DPC_OPC_SIO_SSN_RANGE)
                        {
                            ...
                            for (cnt = rinfo->basic.criteria.range.begin;
                                 ...
                                 cnt++)
                            {
                                SSTCStopSST_PRC(mtp3, pc, (ITS_OCTET)cnt);
                                        ...
                                        sizeof(ITS_UINT), &error)) != NULL)
            {
                for (tinfo2 = rinfo; rinfo != NULL; rinfo = rinfo->next)
                {
                    if (rinfo->basic.dpc == pc)
                    {
                        if (rinfo->basic.style == ROUTE_DPC_OPC_SIO_SSN)
                        {
                            SSTCStartSST_PRC(mtp3, pc, rinfo->basic.criteria.ssn);
                        }
                        else if (rinfo->basic.style == ROUTE_DPC_OPC_SIO_SSN_RANGE)
                        {
                            ITS_USHORT cnt;
                            ...
                            for (cnt = rinfo->basic.criteria.range.begin;
                                 ...
                                 cnt++)
                            {
                                SSTCStartSST_PRC(mtp3, pc, (ITS_OCTET)cnt);
                            }
                        }
                    }
                }
 ...
 ****************************************************************************/
int
SCMGHandleMTP3Msg_PRC(MTP3_HEADER* mtp3, ITS_OCTET mType,
                        ITS_OCTET* data, ITS_USHORT len)
{
    ...
    case MTP3_MSG_PAUSE:
        ...
        SCCP_Alarm_PRC(437, __FILE__, __LINE__, "family=%s:pc=%x",
                       "PRC", MTP3_RL_GET_OPC_VALUE(mtp3->label));

        if (len != sizeof(MTP3_PAUSE_RESUME))
        {
           SCCP_Alarm_PRC(439, __FILE__, __LINE__, "family=%s", "PRC");
           break;
        }
        ...
        PEG_IncrPeg(PRC_SCCP_Pegs, PEG_MTP3_MSG_PAUSE);
    ...
    case MTP3_MSG_RESUME:
        ...
        SCCP_Alarm_PRC(436, __FILE__, __LINE__, "family=%s:pc=%x",
                       "PRC", MTP3_RL_GET_OPC_VALUE(mtp3->label));

        /* Incr Resume Indication Counter for SCCP */
        PEG_IncrPeg(PRC_SCCP_Pegs, PEG_MTP3_MSG_RESUME);
    ...
    case MTP3_MSG_STATUS:
        ...
        SCCP_Alarm_PRC(438, __FILE__, __LINE__, "family=%s:pc=%x",
                       "PRC", MTP3_RL_GET_OPC_VALUE(mtp3->label));
        ...
        PEG_IncrPeg(PRC_SCCP_Pegs, PEG_MTP3_MSG_STATUS);
