 ...
 ****************************************************************************
 *
 *  ID: $Id: ttc-tcap.c,v 9.2.8.1 2009/12/21 03:46:44 rajeshak Exp $
 *
 * LOG: $Log: ttc-tcap.c,v $
 * LOG: Revision 9.2.8.1  2009/12/21 03:46:44  rajeshak
 * LOG: Removing CTF_TraceEvent function call in vendors
 * LOG:
 * LOG: Revision 9.2  2008/06/04 10:38:48  ssingh
 * LOG: ALU porting, B-0100LX-LUSG-00 (june 04,2008 base)
 * LOG:
 * LOG: Revision 9.1  2007/01/11 12:43:41  yranade
 * LOG: Merge changes from 6.5.3 and Lucent branches
 * LOG:
 * LOG: Revision 1.1.2.4  2007/04/10 15:33:39  raghavendrabk
 * LOG: Check Validation for TCAP_TTC_TraceData
 * LOG:
 * LOG: Revision 1.1.2.3  2007/02/26 10:25:12  raghavendrabk
 * LOG: Family filled in OAM API
 * LOG:
 * LOG: Revision 1.1.2.2  2007/02/02 14:21:17  raghavendrabk
 * LOG: Parallel Stack changes for Alarms and Statistics
 * LOG:
 * LOG: Revision 1.1.2.1  2006/12/23 10:05:38  randresol
...
#ident "$Id: ttc-tcap.c,v 9.2.8.1 2009/12/21 03:46:44 rajeshak Exp $"
...
TQUEUETRAN_Manager *__TCAP_TTC_RcvQueue;
MLIST_Manager* __TCAP_TTC_primQueue = NULL;
...
static ITS_CHAR* TCAP_TTC_Timers[] =
{
...
TRACE_Data* TCAP_TTC_TraceData = NULL;

THREAD_RET_TYPE
TTC_TCAPThread(void *arg);
...
TIMERS_DECL_SUBSYS(TTC_TCAP, timers);

static ITS_UINT pegs[PEG_TCAP_NUM_PEGS];
static PEGS_DECL_SUBSYS(TTC_TCAP, pegs);
PEG_Manager *TTC_TCAP_Pegs = &TTC_TCAP_PEGS;
static void DumpAllTCAPPegs(DBC_Server *dbc);
static ITS_BOOLEAN behaviors[TCAP_NUM_BEHAVIORS];
BEHAVIORS_DECL_SUBSYS(TTC_TCAP, behaviors);
 ...
 */
static TCAP_MgmtClassRec TCAP_MgmtClassRec_TTC =
{
    /* core part */
    {
...
ITSSS7DLLAPI ITS_Class TCAP_MgmtClass_TTC =
    (ITS_Class)&TCAP_MgmtClassRec_TTC;
...
void TCAP_ResetOm_TTC();
void TCAP_GetOm_TTC(tcapPegs *);
#if defined(__cplusplus)
}
#endif /* defined(__cplusplus) */

void TCAP_ResetOm_MML_TTC();
void TCAP_GetOm_MML_TTC(DBC_Server *);
...
InitSettings()
{
    BEHAVIORS_SetBehavior(&TTC_TCAP_Behaviors, TCAP_B_use_reserved_NI,
                          ITS_FALSE);

    /*
     * initialize timer values
     */
    TIMERS_SetTimerValue(&TTC_TCAP_Timers, TCAP_T_reject, 5);

    /*
     * determine tracing from INI files
     */
    if (TRANSPORT_RES(__TCAP_TTC_RcvQueue) != NULL)
    {
        char value[ITS_PATH_MAX];
        RESFILE_Manager *res = TRANSPORT_RES(__TCAP_TTC_RcvQueue);
        char *name = TRANSPORT_NAME(__TCAP_TTC_RcvQueue);
        ...
        if (RESFILE_GetKeyValueOf(res, name, TCAP_REJECT_TIMEOUT_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&TTC_TCAP_Timers, TCAP_T_reject,
                                 atoi(value));
        }
        ...
        if (RESFILE_GetKeyValueOf(res, name, TCAP_USE_RESERVED_NI_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            BEHAVIORS_SetBehavior(&TTC_TCAP_Behaviors,
        ...
        if (RESFILE_GetKeyValueOf(res, name, TCAP_VARIANT_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TCAP_SetVariant_TTC(atoi(value));
        }
                                  ...
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            ...
            {
                TRACE_SetOutputOnOffByName(TCAP_TTC_TraceData,
            ...
            if (std || fil || sys)
            {
                TRACE_SetLevelOnOffByName(TCAP_TTC_TraceData,
 ...
 ****************************************************************************/
static void
TCAP_StateChangeCallback_TTC(ITS_POINTER object,
 ...
 ****************************************************************************/
int
ITCAP_Initialize_TTC(ITS_SS7_HANDLE handle)
{
    SS7_LinkPtr link;
    DSM_Queue   *rinfoQ;
    DSMQUEUE_Entry  *temp;
    ...
    if ((TCAP_TTC_TraceData = TRACE_Constructor("TTC-TCAP",
    ...
    if ((__TCAP_TTC_RcvQueue = TQUEUETRAN_CreateTransport("TTC-TCAP",
                                                            ITS_TCAP_TTC_SRC,
                                                            ITS_TCAP_TTC |
                                                            ITS_SCCP_TTC |
                                                            ITS_SCCP_ANSI)) == NULL)
    {
        ...
        TCAP_Alarm_TTC(1413, __FILE__, __LINE__, "family=%s", "TTC");        
  
        TCAP_Term_TTC();

        TRACE_Destructor(TCAP_TTC_TraceData);
    ...
    if ((__TCAP_TTC_primQueue = MQUEUE_Create()) == NULL)
    {
        ...
        TQUEUETRAN_DeleteTransport(__TCAP_TTC_RcvQueue);
                                           ...
                                           sizeof(ITS_THREAD))) == NULL)
    {
        ...
        MQUEUE_Delete(__TCAP_TTC_primQueue);
    ...
    {
        if (THREAD_CreateThread(&tcapThread[i], 0, TTC_TCAPThread,
    ...
    for (link = TRANSPORT_SS7_INFO(__TCAP_TTC_RcvQueue).linkInfo;
         link != NULL;
         link = link->next)
    {
             ...
             temp != NULL; count++, temp = DSMQUEUE_NEXT(rinfoQ, temp))
        {
            ...
            for (tmprinfo = rinfo; tmprinfo != NULL; tmprinfo = rinfo->next)
            {
                ...
                {
                    INTELLINET_SendUIS((ITS_HANDLE)__TCAP_TTC_RcvQueue,
    ...
    TCAP_Alarm_TTC(1414, __FILE__, __LINE__, "family=%s", "TTC");

    if ((dbClass = ITS_FindFeature(DBC_CLASS_NAME)) != NULL)
    {
        DBC_AddDoCommand(TCAP_Console_TTC);
    }
    ...
    if (redunClass != NULL)
    {
        ...
        if (REDUNDANCY_CONFIGURED_TYPE(redunClass)  == ITS_ACTIVE_STANDBY)
        {
            ...
            CALLBACK_AddCallback(REDUNDANCY_CALLBACK_MGR(redunClass),
                                 TCAP_StateChangeCallback_TTC, NULL);
 ...
 ****************************************************************************/
int
TCAP_HandleGcsEvent_TTC(ITS_EVENT *evt)
{
    ITS_USHORT nodeId, ndId;
    ITS_CTXT did;

    TCAP_DEBUG(("TCAP_HandleGcsEvent_TTC\n"));

    nodeId = DSM_GetNodeId();

    if (evt->data[0] == TCAP_NODE_ID)
    {
        TCAP_DEBUG(("TCAP_HandleGcsEvent_TTC: Node Id Event\n"));
    ...
    {
        TCAP_DEBUG(("TCAP_HandleGcsEvent_TTC: Allocate DID Event\n"));
        ndId = evt->data[1];
        did = ITS_GET_CTXT(&evt->data[2]);

        if (ndId != nodeId)
        {
            if (TCAP_AllocateThisDID_TTC(did) == ITS_SUCCESS)
            {
                TCAP_DEBUG(("TCAP_HandleGcsEvent_TTC: Succesfully Allocated DID %d\n", did));
            }
            else
            {
                TCAP_ERROR(("TCAP_HandleGcsEvent_TTC: Unable to Allocate DID %d\n", did));
            }
        }
    }
 ...
 ****************************************************************************/
void
ITCAP_Terminate_TTC(ITS_SS7_HANDLE handle)
{
    ITS_EVENT ev;
    void *status;
    SS7_LinkPtr link;
    ...
    TCAP_Alarm_TTC(1415, __FILE__, __LINE__, "family=%s", "TTC");
    ...
    if ((dbClass = ITS_FindFeature(DBC_CLASS_NAME)) != NULL)
    {
        DBC_RemDoCommand(TCAP_Console_TTC);
    }
            ...
            for (tmprinfo = rinfo; tmprinfo != NULL; tmprinfo = rinfo->next)
            {
                ...
                if (rinfo->basic.type == LOCAL_ROUTE &&
                    (rinfo->basic.style == ROUTE_DPC_SIO_SSN ||
                     rinfo->basic.style == ROUTE_DPC_OPC_SIO_SSN))
                {
                    INTELLINET_SendUOS((ITS_HANDLE)__TCAP_TTC_RcvQueue,
                                       (ITS_OCTET)(rinfo->basic.criteria.sio &
                                                   ROUTE_NI_MASK),
                                       rinfo->basic.dpc,
                                       (ITS_OCTET)rinfo->basic.criteria.ssn);
    ...
    ITS_EVENT_INIT(&ev, ITS_TCAP_TTC_SRC, 0);

    for (i = 0; i < numThreads; i++)
    {
        TRANSPORT_PutEvent(ITS_TCAP_TTC_SRC, &ev);
    }
    ...
    assert(__TCAP_TTC_RcvQueue != NULL);
 ...
 ****************************************************************************
 *  Purpose:
 *      This function is the entry point for the TTC TCAP thread.
 *
 *  Input Parameters:
 *      None.
 *
...
TTC_TCAPThread(void *arg)
{
    ...
    if (sizeof(TCAP_INVOKE_ENTRY) > ITS_MAX_TIMER_CONTEXT)
    {
        ...
        TCAP_Alarm_TTC(1416, __FILE__, __LINE__, "family=%s", "TTC");
    ...
    TRANSPORT_IS_ACTIVE(__TCAP_TTC_RcvQueue) = ITS_TRUE;

    while (cont &&
           TRANSPORT_CLASS_GET_NEXT_EVENT(ITS_OBJ_CLASS(__TCAP_TTC_RcvQueue))
                                                        (__TCAP_TTC_RcvQueue,
                                                        &tcapEvent)
           == ITS_SUCCESS)
    {
        ...
        case ITS_TCAP_DID_REQ:
            TCAP_HandleGcsEvent_TTC(&tcapEvent);
            break;

        case ITS_GCS_EVENT_FROM_STACK:
        case ITS_SCCP_SRC:
            TCAP_DEBUG(("TCAP_Main: process SCCP event.\n"));
            TCOHandleSCCPMsg_TTC(&tcapEvent);

            if ((tcapEvent.src == ITS_SCCP_SRC) &&
                (TRACE_IsLevelOn(TCAP_TTC_TraceData, TCAP_TRACE_EVENT)))
            {

                ITS_INT output = 0;
                if(TRACE_IsOutputOn(TCAP_TTC_TraceData,
                                      TCAP_TRACE_EVENT,
                                      0))
                {
                    output += 1;
                ...
                //CTF_TraceEvent(tcapEvent.src, ITS_TCAP_TTC_SRC,
                //        output, &tcapEvent); 
            }
        ...
        case ITS_TCAP_SRC:
            ...
            TRANSPORT_IS_ACTIVE(__TCAP_TTC_RcvQueue) = ITS_FALSE;
        ...
        case ITS_TIMER_SRC:
            ...
            CCOHandleTimeout_TTC(&tcapEvent);
            break;
        ...
        default:
            ...
            {
                DHAHandleAPPMsg_TTC(&tcapEvent);
            }
            else if (TCAP_MSG_TYPE(&tcapEvent) == ITS_TCAP_CPT)
            {            
                CCOHandleAPPMsg_TTC(&tcapEvent);
            }
            else
            {
                ...
                TCAP_Alarm_TTC(1417, __FILE__, __LINE__, "family=%s", "TTC");
            }
            break;
        }
    }
...
TCAP_GetGeneralCfg_TTC()
{
    ...
    ret->traceOn = (TRACE_IsLevelOn(TCAP_TTC_TraceData,
                                    TCAP_TRACE_DEBUG) &&
                   (TRACE_IsOutputOn(TCAP_TTC_TraceData,
                                      TCAP_TRACE_DEBUG,
                                      0) ||
                    TRACE_IsOutputOn(TCAP_TTC_TraceData,
                                      TCAP_TRACE_DEBUG,
                    ...
                    TRACE_IsOutputOn(TCAP_TTC_TraceData,
                                      TCAP_TRACE_DEBUG,
                                      2))) ||
                   (TRACE_IsLevelOn(TCAP_TTC_TraceData,
                                    TCAP_TRACE_WARNING) &&
                   (TRACE_IsOutputOn(TCAP_TTC_TraceData,
                                      TCAP_TRACE_WARNING,
                                      0) ||
                   ...
                   (TRACE_IsLevelOn(TCAP_TTC_TraceData,
                    ...
                    (TRACE_IsOutputOn(TCAP_TTC_TraceData,
                     ...
                     TRACE_IsOutputOn(TCAP_TTC_TraceData,
                                      TCAP_TRACE_EVENT,
                                      2)));
    ret->alarmLevel = TCAP_AlarmLevel_TTC;
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
TCAP_SetGeneralCfg_TTC(TCAPGeneralCfg *data)
{
    if(TCAP_TTC_TraceData == NULL)
    {
      return !ITS_SUCCESS;
    }

    TCAP_AlarmLevel_TTC = data->alarmLevel;
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
TCAP_SetLocalSSEntry_TTC(TCAPSubSystemInfo *data)
{
 ...
 ****************************************************************************/
void
TCAP_Console_TTC(DBC_Server *dbc, const char *cmdLine)
{
    ...
    if (DBC_Cmd(dbc, "tt_get_gen_cfg", "TCAP Get General Config", "", ""))
    {
        TCAPGeneralCfg *cfg;
        cfg =  TCAP_GetGeneralCfg_TTC();
        ...
        if ( cfg->traceOn)
        {
           if (TRACE_IsLevelOn(TCAP_TTC_TraceData,
                                    TCAP_TRACE_DEBUG))
           {
               strcpy(buf, "        Debug     = ");
               if (TRACE_IsOutputOn(TCAP_TTC_TraceData,
    ...
    else if (DBC_Cmd(dbc, "tt_set_gen_cfg", "TCAP Set General Config", 
                      "<al> <tl>",
                     "<alarm level> <On / Off>"))
    {
        TCAPGeneralCfg cfg;
        ...
        ret =  TCAP_SetGeneralCfg_TTC(&cfg);
        if (ret != ITS_SUCCESS)
        {
            return;
        }
    ...
    else if (DBC_Cmd(dbc, "tt_get_tmr", "TCAP Get Timers", "", ""))
    {
        int value = 0;
        if (TIMERS_GetTimerValue(&TTC_TCAP_Timers, TCAP_T_reject))
        {
            value = TIMERS_GetTimerValue(&TTC_TCAP_Timers, TCAP_T_reject);
        }
    ...
    else if (DBC_Cmd(dbc, "tt_set_tmr", "TCAP Set Timers", 
                     "<tr>", "<Treject>"))
    {
        int tr = 0;
        ...
        TIMERS_SetTimerValue(&TTC_TCAP_Timers, TCAP_T_reject, tr);

        sprintf(buf, "    TCAP_T_reject   Timer Set \n");
        DBC_AppendText(dbc, buf);

    }
    else if (DBC_Cmd(dbc, "tt_get_behavior", "TCAP Get Behaviors", 
                     "", ""))
    {
        if (BEHAVIORS_GetBehavior(&TTC_TCAP_Behaviors, TCAP_B_use_reserved_NI))
        {
    ...
    else if (DBC_Cmd(dbc, "tt_set_behavior", "TCAP Set Behaviors", 
                     "<rni>", ""))
    {
        int rni;
    ...
    else if (DBC_Cmd(dbc, "tt_get_gen_peg", "Get Pegs", "<Peg Id> ", "<Peg#> <>"))
    {
        ...
        {
            /* Invoke function TCAP_GetOm_MML_TTC() */
            TCAP_GetOm_MML_TTC(dbc);
        ...
        if (pgNum > PEG_TCAP_TTC_PEGS_END)
        {
        ...
        value = PEG_GetPeg(TTC_TCAP_Pegs, pgNum);

        sprintf(buf, "value is %d\n", value);
        DBC_AppendText(dbc, buf);
    }
    else if (DBC_Cmd(dbc, "tt_clr_gen_peg", "Clear Peg", "<peg Id>", "<Peg#> <>"))
    {
        char pgName[ITS_PATH_MAX];
        ITS_INT pgNum;
        ...
        if((strncmp(pgName, "ALL", 3) == 0)||
           ...
           (atoi(pgName) == ALL_TCAP_PEGS))
        {
            /* Invoke function TCAP_ResetOm_MML_TTC() */
            TCAP_ResetOm_MML_TTC();
        
            PEG_ClearBlock(TTC_TCAP_Pegs);

            sprintf(buf, "Clearing All TTC TCAP Pegs for Now\n");
            DBC_AppendText(dbc, buf);
            return;
        }
        else
        ...
        PEG_ClearPeg(TTC_TCAP_Pegs, pgNum);
    }
} 
...
DumpAllTCAPPegs(DBC_Server *dbc)
{
    ...
    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_MSG_SENT],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_MSG_SENT));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_MSG_DISCARDED],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_MSG_DISCARDED));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_CMP_SENT],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_CMP_SENT));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_CMP_DISCARDED],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_CMP_DISCARDED));

    DBC_AppendText(dbc, buf);
   
    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_DLG_SENT],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_DLG_SENT));

    DBC_AppendText(dbc, buf);
   
    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_REMOTE_DLG_RCVD],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_REMOTE_DLG_RCVD));

    DBC_AppendText(dbc, buf);
   
    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_REMOTE_CMP_RCVD],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_REMOTE_CMP_RCVD));

    DBC_AppendText(dbc, buf);
   
    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_REMOTE_CMP_DISCARDED],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_REMOTE_CMP_DISCARDED));

    DBC_AppendText(dbc, buf);
   
    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_REMOTE_MSG_RCVD],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_REMOTE_MSG_RCVD));

    DBC_AppendText(dbc, buf);
  
    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_REMOTE_UNKNOWN_MSG_RCVD],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_REMOTE_UNKNOWN_MSG_RCVD));

    DBC_AppendText(dbc, buf);
   
    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_REMOTE_MSG_DISCARDED],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_REMOTE_MSG_DISCARDED));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_ACTIVE_DIALOGUE],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_ACTIVE_DIALOGUE));

    DBC_AppendText(dbc, buf);
   
    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_ACTIVE_TRANSACTIONS],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_ACTIVE_TRANSACTIONS));

    DBC_AppendText(dbc, buf);
   
    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_PREARRANGED_END_SENT],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_PREARRANGED_END_SENT));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_INVALID_SCCP_RCVD],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_INVALID_SCCP_RCVD));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_REMOTE_SCCP_CLASS_0_RCVD],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_REMOTE_SCCP_CLASS_0_RCVD));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_REMOTE_SCCP_CLASS_1_RCVD],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_REMOTE_SCCP_CLASS_1_RCVD));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_REMOTE_UNI_RCVD],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_REMOTE_UNI_RCVD));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_SCCP_CLASS_0_SENT],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_SCCP_CLASS_0_SENT));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_SCCP_CLASS_1_SENT],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_SCCP_CLASS_1_SENT));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_UNI_SENT],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_UNI_SENT));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_ABORT_SENT],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_ABORT_SENT));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_REMOTE_ABORT_RCVD],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_REMOTE_ABORT_RCVD));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_REJECT_SENT],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_REJECT_SENT));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_REMOTE_REJECT_RCVD],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_REMOTE_REJECT_RCVD));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_LOCAL_CANCEL],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_LOCAL_CANCEL));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_DLG_SEND_REMOTE_FAIL],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_DLG_SEND_REMOTE_FAIL));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_DLG_SEND_APP_FAIL],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_DLG_SEND_APP_FAIL));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_INVOKE_SENT],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_INVOKE_SENT));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_RESULT_SENT],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_RESULT_SENT));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_ERROR_SENT],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_ERROR_SENT));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_REMOTE_INVOKE_RCVD],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_REMOTE_INVOKE_RCVD));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_REMOTE_RESULT_RCVD],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_REMOTE_RESULT_RCVD));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_REMOTE_ERROR_RCVD],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_REMOTE_ERROR_RCVD));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_OP_CLASS_1_SENT],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_OP_CLASS_1_SENT));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_OP_CLASS_2_SENT],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_OP_CLASS_2_SENT));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_OP_CLASS_3_SENT],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_OP_CLASS_3_SENT));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_OP_CLASS_4_SENT],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_OP_CLASS_4_SENT));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_BEGIN_SENT],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_BEGIN_SENT));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_REMOTE_BEGIN_RCVD],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_REMOTE_BEGIN_RCVD));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_CONT_SENT],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_CONT_SENT));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_APP_END_SENT],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_APP_END_SENT));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_REMOTE_CONT_RCVD],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_REMOTE_CONT_RCVD));

    DBC_AppendText(dbc, buf);

    sprintf(buf,"%-40s :%d\n",tcapPegStr[PEG_TCAP_REMOTE_END_RCVD],
                              PEG_GetPeg(TTC_TCAP_Pegs,PEG_TCAP_REMOTE_END_RCVD));
 ...
 ****************************************************************************/
int
INTELLINET_TCAP_Commit_TTC(FILE* fp)
{
    int count = 0;
    TRACE_Data* td = TCAP_TTC_TraceData;
    ...
    for (count = 0; count < (int)TTC_TCAP_Timers.numTimers; count++)
    {
        fprintf(fp,"\t\t\t\t\t%s = \"%d\"\n",TCAP_TTC_Timers[count],
                                             TTC_TCAP_Timers.timers[count]);
    }

    fprintf(fp,"\t\t\t\t/>\n");
    fprintf(fp,"\t\t\t\t<BehaviorControl\n");
    fprintf(fp,"\t\t\t\t\tuse_reserved_NI = \"%s\"\n",
            (BEHAVIORS_GetBehavior(&TTC_TCAP_Behaviors,
...
TCAP_OSS_GetGeneralCfg_TTC(TCAP_GeneralCfg* genCfg)
{
    if (genCfg == NULL)
        return ITS_ENOMEM;

    genCfg->alarmLevel = (MGMT_AlarmLevel)TCAP_AlarmLevel_TTC;

    genCfg->debugTrace = TRACE_IsLevelOn(TCAP_TTC_TraceData, TCAP_TRACE_DEBUG);
    if (genCfg->debugTrace)
    {
        genCfg->debugTraceOutput = 0;
        if (TRACE_IsOutputOn(TCAP_TTC_TraceData, TCAP_TRACE_DEBUG, 0))
        {
            genCfg->debugTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(TCAP_TTC_TraceData, TCAP_TRACE_DEBUG, 1))
        {
            genCfg->debugTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(TCAP_TTC_TraceData, TCAP_TRACE_DEBUG, 2))
        {
    ...
    genCfg->warningTrace = TRACE_IsLevelOn(TCAP_TTC_TraceData, TCAP_TRACE_WARNING);
    if (genCfg->warningTrace)
    {
        genCfg->warningTraceOutput = 0;
        if (TRACE_IsOutputOn(TCAP_TTC_TraceData, TCAP_TRACE_WARNING, 0))
        {
            genCfg->warningTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(TCAP_TTC_TraceData, TCAP_TRACE_WARNING, 1))
        {
            genCfg->warningTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(TCAP_TTC_TraceData, TCAP_TRACE_WARNING, 2))
        {
    ...
    genCfg->errorTrace = TRACE_IsLevelOn(TCAP_TTC_TraceData, TCAP_TRACE_ERROR);
    if (genCfg->errorTrace)
    {
        genCfg->errorTraceOutput = 0;
        if (TRACE_IsOutputOn(TCAP_TTC_TraceData, TCAP_TRACE_ERROR, 0))
        {
            genCfg->errorTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(TCAP_TTC_TraceData, TCAP_TRACE_ERROR, 1))
        {
            genCfg->errorTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(TCAP_TTC_TraceData, TCAP_TRACE_ERROR, 2))
        {
    ...
    genCfg->criticalTrace = TRACE_IsLevelOn(TCAP_TTC_TraceData, TCAP_TRACE_CRITICAL);
    if (genCfg->criticalTrace)
    {
        genCfg->criticalTraceOutput = 0;
        if (TRACE_IsOutputOn(TCAP_TTC_TraceData, TCAP_TRACE_CRITICAL, 0))
        {
            genCfg->criticalTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(TCAP_TTC_TraceData, TCAP_TRACE_CRITICAL, 1))
        {
            genCfg->criticalTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(TCAP_TTC_TraceData, TCAP_TRACE_CRITICAL, 2))
        {
    ...
    genCfg->eventTrace = TRACE_IsLevelOn(TCAP_TTC_TraceData, TCAP_TRACE_EVENT);
    if (genCfg->eventTrace)
    {
        genCfg->eventTraceOutput = 0;
        if (TRACE_IsOutputOn(TCAP_TTC_TraceData, TCAP_TRACE_EVENT, 0))
        {
            genCfg->eventTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(TCAP_TTC_TraceData, TCAP_TRACE_EVENT, 1))
        {
            genCfg->eventTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(TCAP_TTC_TraceData, TCAP_TRACE_EVENT, 2))
        {
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
TCAP_OSS_SetGeneralCfg_TTC(TCAP_GeneralCfg* genCfg)
{
    if (genCfg == NULL)
        return ITS_ENOMEM;

    TCAP_AlarmLevel_TTC = genCfg->alarmLevel;

    if (genCfg->debugTrace)
    {
        TRACE_SetLevelOnOffByName(TCAP_TTC_TraceData, TCAP_DEBUG_STRING,  ITS_TRUE);
        ...
        if (genCfg->debugTraceOutput & TRACE_TO_NONE)
        {
            ...
            TRACE_SetLevelOnOffByName(TCAP_TTC_TraceData, TCAP_DEBUG_STRING,  ITS_FALSE);
        }
    }

    if (genCfg->warningTrace)
    {
        TRACE_SetLevelOnOffByName(TCAP_TTC_TraceData, TCAP_WARNING_STRING,  ITS_TRUE);
        ...
        if (genCfg->warningTraceOutput & TRACE_TO_NONE)
        {
            ...
            // Set trace level to false
            TRACE_SetLevelOnOffByName(TCAP_TTC_TraceData, TCAP_WARNING_STRING,  ITS_FALSE);
        }
    }

    if (genCfg->errorTrace)
    {
        TRACE_SetLevelOnOffByName(TCAP_TTC_TraceData, TCAP_ERROR_STRING,  ITS_TRUE);
        ...
        if (genCfg->errorTraceOutput & TRACE_TO_NONE)
        {
            ...
            // Set trace level to false
            TRACE_SetLevelOnOffByName(TCAP_TTC_TraceData, TCAP_ERROR_STRING,  ITS_FALSE);
        }
    }

    if (genCfg->criticalTrace)
    {
        TRACE_SetLevelOnOffByName(TCAP_TTC_TraceData, TCAP_CRITICAL_STRING,  ITS_TRUE);
        ...
        if (genCfg->criticalTraceOutput & TRACE_TO_NONE)
        {
            ...
            // Set trace level to false
            TRACE_SetLevelOnOffByName(TCAP_TTC_TraceData, TCAP_CRITICAL_STRING,  ITS_FALSE);
        }
    }

    if (genCfg->eventTrace)
    {
        TRACE_SetLevelOnOffByName(TCAP_TTC_TraceData, TCAP_EVENT_STRING,  ITS_TRUE);
        ...
        if (genCfg->eventTraceOutput & TRACE_TO_NONE)
        {
            ...
            // Set trace level to false
            TRACE_SetLevelOnOffByName(TCAP_TTC_TraceData, TCAP_EVENT_STRING,  ITS_FALSE);
        }
    }
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
TCAP_OSS_GetBehaviors_TTC(TCAP_Behaviors* behaviors)
{
    if (behaviors == NULL)
        return ITS_ENOMEM;

    behaviors->useReservedNI = BEHAVIORS_GetBehavior(&TTC_TCAP_Behaviors, TCAP_B_use_reserved_NI);
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
TCAP_OSS_SetBehavior_TTC(ITS_UINT behId, ITS_BOOLEAN behValue, TCAP_Behaviors* behaviors)
{
    if (behId == TCAP_ALL_BEHAVIORS)
    {
        ...
        BEHAVIORS_SetBehavior(&TTC_TCAP_Behaviors, TCAP_B_use_reserved_NI, behaviors->useReservedNI);
    }
    else
    {
        ...
        {
            BEHAVIORS_SetBehavior(&TTC_TCAP_Behaviors, behId, behValue);
        }
    }
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
TCAP_OSS_GetPegs_TTC(TCAP_Pegs_TTC* pegs)
{
    if (pegs == NULL)
        return ITS_ENOMEM;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
TCAP_OSS_ClearPeg_TTC(ITS_UINT pegId)
{
    ...
    {
        if (pegId > PEG_TCAP_TTC_PEGS_END)
        {
        ...
        {
            PEG_ClearPeg(TTC_TCAP_Pegs, pegId);
        }
    }
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
TCAP_OSS_GetTimers_TTC(TCAP_Timers* timers)
{
    if (timers == NULL)
        return ITS_ENOMEM;

    timers->reject = TIMERS_GetTimerValue(&TTC_TCAP_Timers, TCAP_T_reject);
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
TCAP_OSS_SetTimer_TTC(ITS_UINT timerId, ITS_UINT timerValue, TCAP_Timers* timers)
{
    if (timerId == TCAP_ALL_TIMERS)
    {
        ...
        TIMERS_SetTimerValue(&TTC_TCAP_Timers, TCAP_T_reject, timers->reject);
    }
    else
    {
        ...
        {
            TIMERS_SetTimerValue(&TTC_TCAP_Timers, timerId, timerValue);
        }
    }
 ...
 ****************************************************************************/
void TCAP_GetOm_TTC(tcapPegs *OmTcapData)
{
    ...
    tcapOmImage[omBuf.activeOm].family = FAMILY_JAPAN;
...
/****************************************************************************
 *  Purpose:
 *      This API is invoked by the tt_clr_gen_peg() MML command. This will
 *      clear the ACTIVE TCAP OM Buffer.
 *
...
/****************************************************************************
 *  Purpose:
 *      This API is invoked by the tt_get_gen_peg() MML command.  This will
 *      clear the INACTIVE TCAP OM Buffer, toggle the ACTIVE/INACTIVE
...
void TCAP_GetOm_MML_TTC(DBC_Server *dbc)
{
    char buf[32000];
    char buf1[32000];
