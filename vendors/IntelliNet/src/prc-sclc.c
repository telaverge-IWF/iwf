 ...
 ****************************************************************************
 *
 *  ID: $Id: prc-sclc.c,v 1.5 2008/07/18 05:39:51 ssingh Exp $
 *
 * LOG: $Log: prc-sclc.c,v $
 * LOG: Revision 1.5  2008/07/18 05:39:51  ssingh
 * LOG: SDS ID:: ID :: D0240 Warning removal for Solaris Compilation
 * LOG:
 * LOG: Revision 1.4  2008/06/17 06:39:07  nvijikumar
 ...
 * LOG:
 * LOG: Revision 1.1.2.2  2007/02/02 14:32:11  raghavendrabk
...
#ident "$Id: prc-sclc.c,v 1.5 2008/07/18 05:39:51 ssingh Exp $"
 ...
 ****************************************************************************/
int
SCLCHandleRoutingFailure_PRC(MTP3_HEADER* mtp3, ITS_OCTET mType,
                               ...
                               ITS_OCTET reason)
{
    ...
    case SCCP_MSG_UDT:
        ...
        return SCRCHandleSCLCMsg_PRC(&resp, SCCP_MSG_UDTS, lies, 4);
    ...
    case SCCP_MSG_XUDT:
        ...
        return SCRCHandleSCLCMsg_PRC(&resp, SCCP_MSG_XUDTS, lies, 5);
    ...
    case SCCP_MSG_LUDT:
        ...
        /* Q752 Implementation */
        /* LUDTS Sent */
        SccpGenOmImage_PRC[omBuf.activeOm].SCCPGenPegs.messageLUDTSSent++;
        ...
        return SCRCHandleSCLCMsg_PRC(&resp, SCCP_MSG_LUDTS, lies, 5);
 ...
 ****************************************************************************/
int
SCLCHandleSCRCMsg_PRC(MTP3_HEADER* mtp3, ITS_OCTET mType,
                        ITS_SCCP_IE* ies, int ieCount,
                        ITS_UINT pc, ITS_OCTET ssn)
{
    ...
    printf("Intellinet Routing SCLCHandleSCRCMsg_PRC %d \n", ssn);
    ...
    {
        return SCMGHandleSCLCMsg_PRC(mtp3, mType, ies, ieCount);
    }
    ...
    if (hc) 
    {
        if (hc->count < 0 || hc->count > SCCP_MAX_HOP_COUNT) 
        {
            ...
            SCLCHandleRoutingFailure_PRC(mtp3, mType, ies, 
                                           ieCount, 
    ...
    {
    case SCCP_MSG_UDT:
        PEG_IncrPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_UDT_RCVD);

        /* Q752 Implementation */
            SccpGenOmImage_PRC[omBuf.activeOm].SCCPGenPegs.messageUDTRcvd++;
    ...
    case SCCP_MSG_XUDT:
       PEG_IncrPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_XUDT_RCVD);
 
       /* Q752 Implementation */
           SccpGenOmImage_PRC[omBuf.activeOm].SCCPGenPegs.messageXUDTRcvd++;
       ...
       for (i = 0; i < ieCount; i++)
       {
            if (ies[i].param_id == SCCP_PRM_SEGMENTATION)
            {
                ...
                if (ies[i].param_data.segmentation.flags & SCCP_SEG_IS_FIRST)
                {
                    ...
                    if( SCCP_AddSegmentRef_PRC(dsmKey, keyLen,  maxDataLen, 
                                                 ...
                                                 != ITS_SUCCESS)
                    {
                         ...
                         PEG_IncrPeg(PRC_SCCP_Pegs,
                                               PEG_SCCP_REMOTE_REASSEMBLE_ERR);

                         /* Q752 Implementation */
                             SccpGenOmImage_PRC[omBuf.activeOm].SCCPGenPegs.reassemblyErrNoReassemblySpace++;  /* 7.12 */
    ...
    case SCCP_MSG_UDTS:
        PEG_IncrPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_UDTS_RCVD);

        /* Q752 Implementation */ 
            SccpGenOmImage_PRC[omBuf.activeOm].SCCPGenPegs.messageUDTSRcvd++;
    ...
    case SCCP_MSG_XUDTS:
        PEG_IncrPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_XUDTS_RCVD);
 
        /* Q752 Implementation */ 
            SccpGenOmImage_PRC[omBuf.activeOm].SCCPGenPegs.messageXUDTSRcvd++;
    ...
    case SCCP_MSG_LUDT:
        PEG_IncrPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_LUDT_RCVD);

        /* Q752 Implementation */
            SccpGenOmImage_PRC[omBuf.activeOm].SCCPGenPegs.messageLUDTRcvd++;
    ...
    case SCCP_MSG_LUDTS:
        PEG_IncrPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_LUDTS_RCVD);

        /* Q752 Implementation */
            SccpGenOmImage_PRC[omBuf.activeOm].SCCPGenPegs.messageLUDTSRcvd++;
 ...
 ****************************************************************************/
int
SCLCHandleSCMGMsg_PRC(MTP3_HEADER* mtp3, ITS_OCTET mType,
                        ITS_SCCP_IE* ies, int ieCount)
{
    switch (mType)
    {
    case SCCP_MSG_UDT:
        return SCRCHandleSCLCMsg_PRC(mtp3, SCCP_MSG_UDT, ies, ieCount);

    case SCCP_MSG_XUDT:
        return SCRCHandleSCLCMsg_PRC(mtp3, SCCP_MSG_XUDT, ies, ieCount);

    case SCCP_MSG_UDTS:
        return SCRCHandleSCLCMsg_PRC(mtp3, SCCP_MSG_UDTS, ies, ieCount);

    case SCCP_MSG_XUDTS:
        return SCRCHandleSCLCMsg_PRC(mtp3, SCCP_MSG_XUDTS, ies, ieCount);

    case SCCP_MSG_LUDT:
        return SCRCHandleSCLCMsg_PRC(mtp3, SCCP_MSG_LUDT, ies, ieCount);

    case SCCP_MSG_LUDTS:
        return SCRCHandleSCLCMsg_PRC(mtp3, SCCP_MSG_LUDTS, ies, ieCount);
 ...
 ****************************************************************************/
int
SCLCHandleAPPMsg_PRC(ITS_EVENT* event)
{
    ITS_SCCP_IE ies[32];
    int ieCount;
    int ret, i;
    ...
    {
    case SCCP_MSG_UDT:
        PEG_IncrPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_UDT_SENT);
        
        /* Q752 Implementation */ 
             SccpGenOmImage_PRC[omBuf.activeOm].SCCPGenPegs.messageUDTSent++;
        ...
        if (mtp3 == NULL)
        {
            ...
            SCCP_Alarm_PRC(633, __FILE__, __LINE__, "family=%s:mType=%s",
                           "PRC", "UDT");
    ...
    case SCCP_MSG_XUDT:
        PEG_IncrPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_XUDT_SENT);

        /* Q752 Implementation */
            SccpGenOmImage_PRC[omBuf.activeOm].SCCPGenPegs.messageXUDTSent++;
    ...
    case SCCP_MSG_UDTS:
        PEG_IncrPeg(PRC_SCCP_Pegs,   PEG_SCCP_USER_UDTS_SENT);

        /* Q752 Implementation */
            SccpGenOmImage_PRC[omBuf.activeOm].SCCPGenPegs.messageUDTSSent++;
    ...
    case SCCP_MSG_XUDTS:
        PEG_IncrPeg(PRC_SCCP_Pegs,   PEG_SCCP_USER_XUDTS_SENT);

        /* Q752 Implementation */
            SccpGenOmImage_PRC[omBuf.activeOm].SCCPGenPegs.messageXUDTSSent++;
    ...
    case SCCP_MSG_LUDT:
        PEG_IncrPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_LUDT_SENT);

        /* Q752 Implementation */
            SccpGenOmImage_PRC[omBuf.activeOm].SCCPGenPegs.messageLUDTSent++;
        ...
        if(dataLen > SCCP_LUDT_MAX)             
        {
            ...
            {
               SccpGenOmImage_PRC[omBuf.activeOm].SCCPGenPegs.segmentationErrSegFailed++; 
               return (ITS_EMISSMAND); 
            }  
            ...
            if (totalSegment > SCCP_MAX_ALLOWED_SEGMENT)
            {
                ...
                PEG_IncrPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_SEGMENTATION_ERR);
            ...
            for (i = 0;i < totalSegment; i++)
            {
                ...
                ret = SCRCHandleSCLCMsg_PRC(mtp3, SCCP_MSG_XUDT, ies, ieCount);
        ...
        {
            ret = SCRCHandleSCLCMsg_PRC(mtp3, SCCP_MSG_LUDT, ies, ieCount);
    ...
    case SCCP_MSG_LUDTS:
        PEG_IncrPeg(PRC_SCCP_Pegs,   PEG_SCCP_USER_LUDTS_SENT);
 ...
 ****************************************************************************/
int
SCLCHandleReasmTimer_PRC(ITS_TimerData* td)
{
    SCCP_REASM_CTXT ctxt;
