 ...
 ****************************************************************************
 *
 *  ID: $Id: ttc-tcap_context.c,v 9.3 2008/06/24 07:51:15 ssingh Exp $
 *
 * LOG: $Log: ttc-tcap_context.c,v $
 * LOG: Revision 9.3  2008/06/24 07:51:15  ssingh
 * LOG: porting of Revision 9.3.8.4 changes from ITU module
 * LOG:
 ...
 * LOG:
 * LOG: Revision 1.1.2.2  2007/02/02 14:21:39  raghavendrabk
...
#ident "$Id: ttc-tcap_context.c,v 9.3 2008/06/24 07:51:15 ssingh Exp $"
...
TCAP_TRANS_ENTRY*   LocalTxTable[MAX_NUM_DIALOGS];
...
TCAP_Init_TTC()
{
    ...
    if (!splitTIDS)
    {
        ...
        if (DSM_LockTable(DSM_TcapFtGroupId, DSM_TABLE_TTC_DIDS) == ITS_SUCCESS)
        { 
                ...
                if (nextLTid != NULL)
                {
                    ...
                    *nextLTid = -1;
                    ...
                    if (ret != ITS_SUCCESS)
                    {
                        ...
                        DSM_UnlockTable(DSM_TcapFtGroupId, DSM_TABLE_TTC_DIDS);
 ...
 ****************************************************************************/
TCAP_TRANS_ENTRY *
TCAP_AllocateLocalTx_TTC(ITS_CTXT did)
{
    TCAP_TRANS_ENTRY *tx = NULL;

    if ((tx = (TCAP_TRANS_ENTRY *)calloc(1, sizeof(TCAP_TRANS_ENTRY))) == NULL)
    {
        TCAP_CRITICAL(("AllocateLocalTx_TTC: NO MEMORY for LocalTx for did %d\n",
                      did));
    ...
    {
        TCAP_ERROR(("AllocateLocalTx_TTC: LocalTx already exists for did %d\n",
                      did));
    ...
    TCAP_DEBUG(("AllocateLocalTx_TTC: Allocated LocalTx for did %d\n", did));   
    return tx;
}
 ...
 ****************************************************************************/
TCAP_TRANS_ENTRY*
TCAP_FindLocalTx_TTC(ITS_CTXT did)
{
 ...
 ****************************************************************************/
int 
TCAP_AllocateThisDID_TTC(ITS_CTXT nextDID)
{
    ...
    if (DSM_LockTable(DSM_TcapFtGroupId, DSM_TABLE_TTC_DIDS) != ITS_SUCCESS)
    {
        TCAP_ERROR(("TCAP_AllocateThisDID_TTC: cannot aquire "
                    "mutex !!! \n"));
    ...
    else
    {
        ...
        else
        {
            ...
            TCAP_Alarm_TTC(1455, __FILE__, __LINE__, "family=%s", "TTC");
    ...
    if (ret == ITS_SUCCESS)
    {
        localTx = TCAP_AllocateLocalTx_TTC(nextDID);
        if (localTx == NULL)
        {
            TCAP_ERROR(("TCAP_AllocateThisDID_TTC: Cannot Allocate "
                        "Local Tx for did %d.\n", nextDID));
        ...
        TCAP_DEBUG(("TCAP_AllocateThisDID_TTC: Allocated "
                    "Local Tx for did %d.\n", nextDID));

        ret = AllocateLTIDinTx_TTC(localTx);
...
TCAP_AllocateSharedDID(ITS_CTXT *did)
{
    ...
    if (DSM_LockTable(DSM_TcapFtGroupId, DSM_TABLE_TTC_DIDS) != ITS_SUCCESS)
    {
        TCAP_ERROR(("TCAP_AllocateDialogueId_TTC: cannot aquire "
                    "mutex !!! \n"));
        /* CRITICAL - Unable to Lock Table. */
        ...
        else
        {
            ...
            if (didPtr != NULL)
            {
                *didPtr = (TCAP_TRANS_ENTRY *)TCAP_DID_BEING_USED;
                ...
                PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_ACTIVE_DIALOGUE);
            }
            else
            {
                TCAP_CRITICAL(("DSM_Alloc Failled, Error Code %d\n", error));
 ...
 ****************************************************************************/
int
INTELLINET_AllocateDID_TTC(ITS_SS7_HANDLE handle,
                             ITS_CTXT *did)
{
    int ret = ITS_SUCCESS;
    ITS_CTXT i = 0;
    ...
    if (!didAllocated)
    {
        TCAP_CRITICAL(("INTELLINET_AllocateDID_TTC: Cannot Allocate DID"));
    ...
    localTx = TCAP_AllocateLocalTx_TTC(*did);
    if (localTx == NULL)
    {
        TCAP_ERROR(("INTELLINET_AllocateDID_TTC: Cannot Allocate "
                    "Local Tx for did %d.\n", *did));
    ...
    TCAP_DEBUG(("INTELLINET_AllocateDID_TTC: Allocated "
                "Local Tx for did %d.\n", *did));
 ...
 ****************************************************************************/
int
TCAP_GetNextIDID_TTC(ITS_CTXT *idid)
{
    int ret = ITS_SUCCESS;
 ...
 ****************************************************************************/
TCAP_TRANS_ENTRY *
TCAP_AllocTransactionContext_TTC(ITS_UINT ltid, ITS_OCTET *key, int *klen)
{
    TCAP_TRANS_ENTRY *tx = NULL;
    int error = 0;
    ITS_OCTET nextLTidKey[sizeof(ITS_UINT)];
    ...
    if (tx == NULL)
    {
        TCAP_CRITICAL(("TCAP_AllocTransactionContext_TTC: DSM_Alloc " 
                       "failed, error code: %d\n", error));        
        /* Unable to Allocate Memory for Transaction Context in Table. */
 ...
 ****************************************************************************/
int
AllocateLTIDinTx_TTC(TCAP_TRANS_ENTRY* tx)
{
    ITS_UINT nextLTid = 0;
    ...
    */
    ...
    TCAP_DEBUG(("TCAP_AllocateLocalTx_TTC: DID <---> TID : %d <---> %u\n",
                tx->did, nextLTid));
 ...
 ****************************************************************************/
int
TCAP_PopulateLocalTx_TTC(TCAP_TRANS_ENTRY *tx, ITS_CTXT did,
                           ...
                           SCCP_ADDR *cdp, SCCP_ADDR *clp)
{
    ...
    {
        TCAP_CRITICAL(("TCAP_PopulateLocalTx_TTC: I-DID is ZERO!!!!\n"));

        return ITS_ENOMEM;

#if 0
        if (TCAP_GetNextIDID_TTC(&did) != ITS_SUCCESS)
        {
            return ITS_ENOMEM;
        }
#endif
    ...
    if (tx != NULL)
    {
        ...
        PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_ACTIVE_TRANSACTIONS);
    }
    else
    {
        TCAP_CRITICAL(("TCAP_PopulateTransactionTx_TTC: Tx is NULL\n"));
 ...
 ****************************************************************************/
int
TCAP_FreeLocalTx_TTC(ITS_CTXT did)
{
    TCAP_TRANS_ENTRY *localTx = NULL;
    ...
    if ((localTx = TCAP_FindLocalTx_TTC(did)))
    {
        ...
        for (i = 0; i < localTx->num_CptList; i++)
        {
            if (CptList != NULL)
            {
                for (j = 0; j < TCAP_CTX_MAX_CPT_ENTRIES; j++)
                {
                    ...
                    {
                        TCAP_DEBUG(("TCAP_FreeLocalTx_TTC: Cancelling running timer "
                ...
                if (i > 0)
                {
                    ...
                    if (ret != ITS_SUCCESS)
                    {
                        TCAP_ERROR(("TCAP_FreeLocalTx_TTC: DSM_Free "
                                    "failed for did %d, error code: %d\n", did, ret));
                    }
                    else
                    {
                        TCAP_DEBUG(("TCAP_FreeLocalTx_TTC: DSM_Free Cpt List %d "
                                    "for did %d\n", i, did));
                    }
                }
                else
        ...
        TCAP_DEBUG(("FreeLocalTx_TTC: Freed LocalTx for did %d\n", did));
    }
    else
    {
        TCAP_DEBUG(("FreeLocalTx_TTC: Cannot find LocalTx for did %d\n", did));
        return ITS_ENOTFOUND;
    }

    /* Empty the queue */
    if (DHAHandleDIDClean_TTC(did) == ITS_SUCCESS)
    { 
        TCAP_DEBUG(("FreeLocalTx_TTC: Emptied the component queue for did %d\n", 
                     did));
    }
    else
    {
        TCAP_ERROR(("FreeLocalTx_TTC: Cannot empty the component queue "
                    "for did %d\n", 
                     did));
    }
 ...
 ****************************************************************************/
TCAP_TRANS_ENTRY *
TCAP_FindTransactionContext_TTC(ITS_UINT tid, ITS_OCTET *key, int *klen)
{
    TCAP_TRANS_ENTRY *tx = NULL;
    int error = 0;
 ...
 ****************************************************************************/
TCAP_TRANS_ENTRY *
TCAP_CopyDSMTxToLocalTx_TTC(TCAP_TRANS_ENTRY *sharedTx)
{
    TCAP_TRANS_ENTRY *localTx = NULL;

    if ((localTx = TCAP_FindLocalTx_TTC(sharedTx->did)) == NULL)
    {
        localTx = TCAP_AllocateLocalTx_TTC(sharedTx->did);
        if (localTx == NULL)
        {
            TCAP_ERROR(("TCAP_CopyDSMTxToLocalTx_TTC: Cannot allocate local context"
                         "for did %d.\n", sharedTx->did));
            return NULL;
        }
        ...
        TCAP_DEBUG(("TCAP_CopyDSMTxToLocalTx_TTC: Copied DSMTx to LocalTx"
                    "for did %d.\n", sharedTx->did));
    }
    else
    {
         ...
         */
#if 0
        TCAP_WARNING(("TCAP_CopyDSMTxToLocalTx_TTC: Found existing local "
 ...
 ****************************************************************************/
void 
TCAP_FindAndFreeTransactionContext_TTC(ITS_UINT ltid, ITS_OCTET* tidKey, int* tidKeyLen)
{
    TCAP_TRANS_ENTRY *sharedTx = NULL;

    sharedTx = TCAP_FindTransactionContext_TTC(ltid,
                                                 tidKey, tidKeyLen);
    if (sharedTx == NULL)
    {
        TCAP_DEBUG(("TCAP_FindAndFreeTransactionContext_TTC: Cannot Find DSM Tx for tid"
                    "%u.\n", ltid));
    }
    else
    {
        TCAP_DEBUG(("TCAP_FindAndFreeTransactionContext_TTC: Freeing DSM Tx for ltid"
                    "%u.\n", ltid));
 ...
 ****************************************************************************/
void
TCAP_FreeDSMTransactionContext_TTC(TCAP_TRANS_ENTRY *trCtxt)
{
    int error = 0, ret = ITS_SUCCESS; 
    ...
    if (!splitDIDS && trCtxt->did < TCAP_DIR_BIT)
    {
        ...
        if (didPtr != NULL)
        {
            *didPtr = NULL;
            ...
            if (ret != ITS_SUCCESS)
            {
                TCAP_ERROR(("TCAP_FreeTransactionContext_TTC: DSM_Commit "
                            "failed for did %d, error code: %d\n", did, ret));
        ...
        {
            TCAP_WARNING(("TCAP_FreeTransactionContext_TTC: DSM_Find "
                          "failed for did %d, error code: %d\n", did, error));
 ...
 ****************************************************************************/
int
INTELLINET_FetchAddrs_TTC(ITS_SS7_HANDLE handle,
                            ITS_CTXT did,
                            SCCP_ADDR *oaddr, SCCP_ADDR *daddr)
{
    ...
    if (tx != NULL)
    {
        ...
        if (ret != ITS_SUCCESS)
        {
            TCAP_ERROR(("TCAP_FetchAddrs_TTC: DSM_Commit failed, error "
                        "code %d\n", ret));
 ...
 ****************************************************************************/
int
INTELLINET_SetAddrs_TTC(ITS_HANDLE handle, ITS_CTXT did,
                          SCCP_ADDR *oaddr, SCCP_ADDR *daddr)
{
    ...
    if (txPtr!= NULL)
    {
        ...
        if (ret != ITS_SUCCESS)
        {
            TCAP_ERROR(("TCAP_SetAddrs_TTC: DSM_Commit failed, error "
                        "code %d\n", ret));
        ...
        if (tx != NULL)
        {
            ...
            if (ret != ITS_SUCCESS)
            {
                TCAP_ERROR(("TCAP_SetQOS_TTC: DSM_Commit failed, error "
                            "code %d\n", ret));
                return ret;
            }
        }
        ...
        {
            TCAP_ERROR(("TCAP_SetAddrs_TTC: Could not find Trans Ctxt, tid: %u\n",
                        tid));
    ...
    else
    {
        ...
        TCAP_ERROR(("TCAP_SetAddrs_TTC: Could not find DID entry, DID: %d\n",
                     did));
 ...
 ****************************************************************************/
int
INTELLINET_FetchQOS_TTC(ITS_SS7_HANDLE handle,
 ...
 ****************************************************************************/
int
INTELLINET_SetQOS_TTC(ITS_SS7_HANDLE handle,
                        ITS_CTXT did, DLG_QOS *qos)
{
    TCAP_TRANS_ENTRY *tx, **txPtr = NULL;
    int error = 0, ret = ITS_SUCCESS;
    ...
    if (txPtr != NULL)
    {
        ...
        {
            TCAP_ERROR(("TCAP_SetQOS_TTC: Could not find Trans Ctxt, tid: %u\n",
                        tid));
    ...
    else
    {
        ...
        TCAP_ERROR(("TCAP_SetQOS_TTC: Could not find DID entry, DID: %d\n",
                     did));
 ...
 ****************************************************************************/
TCAP_INVOKE_ENTRY *
TCAP_FindInvokeContext_TTC(ITS_CTXT did,
                             CPT_INV_ID *invokeId,
                             int dir)
{
    int error = 0, ret = ITS_SUCCESS;
            ...
            if (tmp == NULL && cptList->next == NULL)
            {
                ...
                if (error == ITS_ENOTFOUND)
                {
                    ...
                    if (cptL != NULL)
                    {
                        ...
                        if (ret != ITS_SUCCESS)
                        {
                            TCAP_ERROR(("TCAP_FindInvokeContext_TTC: DSM_Commit failed, error "
                                        "code %d\n", ret));
                        ...
                        {
                            TCAP_DEBUG(("TCAP_FindInvokeContext_TTC: Successfully allocated "
                                        "Cpt List %d for did %d\n", tx->num_CptList - 1, did));
                        }
                    }
                    else
                    {
                        TCAP_CRITICAL(("TCAP_FindInvokeContext_TTC: DSM_Alloc "
                                       "failed, error code: %d\n", error));
 ...
 ****************************************************************************/
TCAP_INVOKE_ENTRY *
TCAP_FindInvokeContextWithTimer_TTC(ITS_CTXT did,
                                      ITS_OCTET invokeId, 
                                      ITS_SERIAL timer)
{
    TCAP_TRANS_ENTRY* tx = NULL;
 ...
 ****************************************************************************/
int
TCAP_AllocInvokeContext_TTC(ITS_CTXT did, TCAP_CPT *cpt, 
 ...
 ****************************************************************************/
void
TCAP_FreeInvokeContext_TTC(TCAP_INVOKE_ENTRY *invoke)
{
    if (invoke == NULL)
    {
