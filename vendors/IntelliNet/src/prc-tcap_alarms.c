 ...
 ****************************************************************************
 *
 *  ID:$Id: prc-tcap_alarms.c,v 1.2 2007/02/01 13:26:40 yranade Exp $
 *
 * LOG: $Log: prc-tcap_alarms.c,v $
 ...
 ****************************************************************************/
void
TCAP_Alarm_PRC(int which, const char *file, const int line,
                 char *format, ...)
{
    ...
    ITS_C_ASSERT(which >= PRC_TCAP_ALARM_START &&
                 which <= PRC_TCAP_ALARM_END);

    fmt = PRC_TCAP_Alarms[which - PRC_TCAP_ALARM_START];

    if (fmt->severity > TCAP_AlarmLevel_PRC)
    {
        return;
    }
