 ...
 ****************************************************************************
 *
 *  ID: $Id: itu-wait_in_rlc_itu_99.c,v 1.2 2006/01/09 06:44:52 omayor Exp $
 *
 *  LOG: $Log: itu-wait_in_rlc_itu_99.c,v $
...
/*
 * Functions for handling messages in the Wait_Incoming_RLC state for ITU_99
 * variant. 
 */
 ...
 *
 *  Note: We allow PRI here because REL can be sent and received in this
 *  state (as per 93). It makes sense to allow receipt of PRI from L3 (REL
 *  collision, but if we are in this state it means we sent REL once 
 *  and we are re-sending... Leave it like this for now.
 *   
 *  Revision History:
 *  ----------------------------------------------------------------------------
...
__ISUP_GetMsgProc_WaitIncomingRlc_ITU_99(ITS_OCTET msgType)
{
    switch(msgType)
    {
    case ISUP_MSG_APM:
