 ...
 ****************************************************************************
 *
 *  ID: $Id: itu-wait_in_acm_itu_99.c,v 1.2 2006/01/09 06:44:52 omayor Exp $
 *
 *  LOG: $Log: itu-wait_in_acm_itu_99.c,v $
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an SDM received in the WAIT_INCOMING_ACM state
 *      is done here. The message is forwarded to  L3 if received from the 
 *      application, ignored if from L3.
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    if (src == ISUP_SRC_L3_ITU)
    {
        ...
        ISUP_ERROR(("RCVD_IN_ACM SDM: Unexpected SDM from MTP3. pcic 0x%08x, "
    ...
    if (src != ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received SDM in RCVD_IN_ACM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
...
__ISUP_GetMsgProc_WaitIncomingAcm_ITU_99(ITS_OCTET msgType)
{
    switch(msgType)
    {
    case ISUP_MSG_APM:
