 ...
 ****************************************************************************
 *
 *  ID: $Id: prc-stm.c,v 1.4 2008/06/17 06:39:08 nvijikumar Exp $
 *
 * LOG: $Log: prc-stm.c,v $
 ...
 * LOG: Add prc files
 * LOG:
 * LOG: Revision 1.1.4.9  2007/11/22 14:08:46  mshanmugam
 * LOG: Fix for BSG Redundancy Enhancement
 * LOG:
 * LOG: Revision 1.1.4.8  2007/07/24 06:33:57  raghavendrabk
 * LOG: Propagated the fix provided for issue 6171
 * LOG:
 * LOG: Revision 1.1.4.7  2007/07/22 13:30:26  raghavendrabk
 * LOG: Propogated the fix given for Issue ID:6172
 * LOG:
 * LOG: Revision 1.1.4.6  2007/06/14 05:54:45  raghavendrabk
 * LOG: Fix for Issue#6013 and Issue#6012
 * LOG:
 * LOG: Revision 1.1.4.5  2007/05/18 10:03:45  raghavendrabk
 * LOG: Changes for Alarm Optimisation
 * LOG:
 * LOG: Revision 1.1.4.4  2007/03/29 11:18:29  sdwivedi
 * LOG: Fix for the 1st CBD sent to the network
 * LOG:
 * LOG: Revision 1.1.4.3  2007/02/06 12:02:14  ssingh
 * LOG: Fixing issue:: Accelero 6.7:: PRC links are not aligning (#5340)
 * LOG:
 * LOG: Revision 1.1.4.2  2007/02/02 14:36:27  raghavendrabk
 ...
 * LOG:
 * LOG: Revision 9.4.4.7  2007/01/18 14:44:32  ssingh
...
#ident "$Id: prc-stm.c,v 1.4 2008/06/17 06:39:08 nvijikumar Exp $"
...
SPRestarting_PRC()
{
    return BEHAVIORS_GetBehavior(&PRC_MTP3_Behaviors, MTP3_B_spRestarting);
}
 ...
 ****************************************************************************/
int
SPRestartingSet_PRC(ITS_BOOLEAN isRestarting)
{
 ...
 ****************************************************************************/
ITS_BOOLEAN
FullRestartNeeded_PRC()
{
    return BEHAVIORS_GetBehavior(&PRC_MTP3_Behaviors, 
 ...
 ****************************************************************************/
int
FullRestartNeededSet_PRC(ITS_BOOLEAN restartNeeded)
{
...
SendingStatus_PRC()
{
    return BEHAVIORS_GetBehavior(&PRC_MTP3_Behaviors, MTP3_B_sendingStatus);
}
 ...
 ****************************************************************************/
int
SendingStatusSet_PRC(ITS_BOOLEAN status) 
{
 ...
 ****************************************************************************/
int 
STM_Main_PRC(MTP3_SubSystems src, MTP3_SubSystems dest,
               ...
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_STM, src, trigger, sif, len,
                          linkSet, linkCode);
    ...
    if (trigger == SRM_TRIGGER_L2_MSG)
    {
        ...
        case SNMM_HC0_CHM:  /* changeover/changeback */
            ...
            case SNMM_HC1_COO:   /* changeover */
                return TLAC_Main_PRC(MTP3_SMH_HMDT,
                                      STM_TRIGGER_CHANGEOVER_ORDER,
                                      sif, len,
                                      linkSet, linkCode);

            case SNMM_HC1_COA:   /* changeover ack */
                return TCOC_Main_PRC(MTP3_SMH_HMDT,
            ...
            case SNMM_HC1_CBD:   /* changeback */
                return TCBC_Main_PRC(MTP3_SMH_HMDT,
        ...
        case SNMM_HC0_TRM:  /* traffic restart */
            ...
            case SNMM_HC1_TRA:   /* traffic restart allowed */
                return TPRC_Main_PRC(MTP3_SMH_HMDT,
            ...
            case SNMM_HC1_TRW:   /* traffer restart waiting */
                ...
                return TSRC_Main_PRC(MTP3_SMH_HMDT,
        ...
        case SNMM_HC0_UFC:  /* user flow control */
            ...
            case SNMM_HC1_UPU:   /* user part unavailable */
                ...
                return TSFC_Main_PRC(MTP3_SMH_HMDT,
                                      STM_TRIGGER_UPU,
                                      sif, len,
    ...
    if (i == ent->entries[whichEntry]->linkSet->numLinks && !gotMatch)
    {
        ...
        if (idx > lastMatch)
        {
            MTP3_DEBUG(("No match found for alternate route  for linkSet "
                        "%d\n", linkSet));
            MTP3_Alarm_PRC(2466, __FILE__, __LINE__,"ls=%x:family=%s",linkSet, "PRC");
        }
    }
...
SetLinkUnavailable(ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ...
    ret = TSRC_Main_PRC(MTP3_STM_TLAC,
    ...
    if (SPRestarting_PRC())
    {
        ret = TPRC_Main_PRC(MTP3_STM_TLAC,
                             STM_TRIGGER_LINK_UNAVAILABLE,
                             NULL, 0,
                             linkSet, linkCode);
...
ForceUninhibitLink(ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ...
    ret = HMRT_Main_PRC(MTP3_STM_TLAC,
    ...
    /* Increament the peg for Link ForceUnInhibit */
     PEG_IncrPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_FORCE_UNINHIBIT);
 ...
 ****************************************************************************/
int
TLAC_Main_PRC(MTP3_SubSystems src, STM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_STM_TLAC, src, trigger, sif, len,
                          linkSet, linkCode);
    ...
    case STM_TRIGGER_INHIBIT_LINK:           /* from MGMT, HMDT */
        ...
        if (src == MTP3_SMH_HMDT)
        {
            ...
            linkSet = MTP3_FindAdjacent_PRC(ni, pc1, linkSet);

            if (linkSet == ITS_SS7_DEFAULT_LINK_SET)
            {

                MTP3_Alarm_PRC(2471, __FILE__, __LINE__, 
                                 "ls=%x:link=%x:family=%s", linkSet, linkCode, "PRC");
            ...
            MTP3_Alarm_PRC(2516, __FILE__, __LINE__,
                            "ls=%x:link=%x:family=%s",linkSet,linkCode, "PRC");
        ...
        else if (src == MTP3_MGMT)
        {
            ...
            MTP3_Alarm_PRC(2517, __FILE__, __LINE__,
                            "ls=%x:link=%x:family=%s",linkSet,linkCode, "PRC");
            ...
            if (ln->availableState == LINK_UNAVAILABLE)
            {
                ...
                else
                {
                    if (ln->stFlags & LINK_LOC_INHIBITED)
                    {
                        ...
                        MTP3_Alarm_PRC(2519, __FILE__, __LINE__, 
                               "ls=%x:link=%x:family=%s", linkSet,ln->linkCode, "PRC");
    ...
    case STM_TRIGGER_UNINHIBIT_LINK:         /* from HMDT, MGMT, TSRC */
        ...
        {                        
            ...
            if (ln->availableState == LINK_AVAILABLE)
            {
                ...
                PEG_IncrPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_UNINHIBIT);
            ...
            else if (ln->availableState == LINK_UNAVAILABLE)
            {
                ...
                ret = MGMT_Main_PRC(MTP3_STM_TLAC, 
               ...
               /* Increament the peg for Link ForceUnInhibit */
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        ret = TCBC_Main_PRC(MTP3_STM_TLAC,
    ...
    case STM_TRIGGER_LOCAL_INHIBIT_ALLOWED:  /* from TSRC */
        ...
        if (ln->availableState == LINK_AVAILABLE)
        {
            ...
            MTP3_Alarm_PRC(2518, __FILE__, __LINE__,
                            "ls=%x:link=%x:family=%s",linkSet,ln->linkCode, "PRC");
    ...
    case STM_TRIGGER_LOCAL_INHIBIT_DENIED:   /* from TSRC */
        ...
        MTP3_Alarm_PRC(2520, __FILE__, __LINE__,"ls=%x:link=%x:family=%s",
                          linkSet,linkCode, "PRC");

        PEG_IncrPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_INHIBIT_DNY);
    ...
    case STM_TRIGGER_REMOTE_INHIBIT_ALLOWED: /* from TSRC */
        ...
        if (ln->availableState == LINK_AVAILABLE)
        {
            ...
            ret = HMRT_Main_PRC(MTP3_STM_TLAC, SMH_TRIGGER_TRAFFIC_MGMT_MSG,
                                 ev.data, ev.len,
                                 linkSet, linkCode);
            ITS_EVENT_TERM(&ev);
            ...
            PEG_IncrPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_INHIBIT);
            ...
            else
            {
                ...
                ret = TCOC_Main_PRC(MTP3_STM_TLAC,
                                     STM_TRIGGER_LINK_UNAVAILABLE,
                                     NULL, 0,
                                     linkSet, linkCode);
                ...
                PEG_IncrPeg(PRC_MTP3_Pegs,PEG_MTP3_SL_UNAVAILABLE);
    ...
    case STM_TRIGGER_REMOTE_INHIBIT_DENIED:  /* from TSRC */
        ...
        MTP3_Alarm_PRC(2521, __FILE__, __LINE__,
                        "ls=%x:link=%x:family=%s",linkSet,linkCode, "PRC");
...
/** FIXME: END **/
    ...
    case STM_TRIGGER_CHANGEOVER_EXECUTED:    /* from TCOC */
        ...
        if (ln->availableState == LINK_UNAVAILABLE)
        {
            ...
            {
                ret = LSAC_Main_PRC(MTP3_STM_TLAC,
    ...
    case STM_TRIGGER_LINK_IN_SERVICE:        /* from SLM */
        MTP3_Alarm_PRC(2526, __FILE__, __LINE__,
               "ls=%x:link=%x:family=%s",linkSet,linkCode, "PRC");
    ...
    case STM_TRIGGER_RP_OUTAGE:              /* from SLM */
        ...
        MTP3_Alarm_PRC(2403, __FILE__, __LINE__, "ls=%x:link=%x:family=%s",
                        linkSet,linkCode, "PRC");
    ...
    case STM_TRIGGER_RP_RECOVERD:            /* from SLM */
        ...
        MTP3_Alarm_PRC(2404, __FILE__, __LINE__, "ls=%x:link=%x:family=%s",
                        linkSet,linkCode, "PRC");
    ...
    case STM_TRIGGER_LP_OUTAGE:              /* from SLM */
        ...
        MTP3_Alarm_PRC(2524, __FILE__, __LINE__, "ls=%x:link=%x:family=%s",
                        linkSet,linkCode, "PRC");
    ...
    case STM_TRIGGER_LP_RECOVERD:            /* from SLM */
        ...
        MTP3_Alarm_PRC(2525, __FILE__, __LINE__, "ls=%x:link=%x:family=%s",
                        linkSet,linkCode, "PRC");
    ...
    case STM_TRIGGER_RESTART_BEGIN:          /* from TPRC */        
        ...
        for (i = 0; i < ITS_SS7_DEFAULT_LINK_SET; i++)
        {
            if ((ls = LINKSET_FindLinkSet(i)) == NULL ||
                 ls->adjacent == 0 || ls->family != FAMILY_CHINA)
            {
                continue;
            }
            ...
            {                 
                ...
                else if (ln->availableState == LINK_UNAVAILABLE)
                {
                    ...
                    * they have.
                    */
                    if ((FullRestartNeeded_PRC() && SPRestarting_PRC()) ||
                        (ln->stFlags & LINK_FAILED) ||
                        (ln->stFlags & LINK_LOC_BLOCKED) ||
                        (ln->stFlags & LINK_REM_BLOCKED))
                    {
    ...
    case STM_TRIGGER_CHANGEOVER_ORDER:  /* from SMH */
        ...
        MTP3_Alarm_PRC(2510, __FILE__, __LINE__, "ls=%x:link=%x:family=%s",
                        linkSet,linkCode, "PRC");
    ...
    case STM_TRIGGER_EMERG_CHANGEOVER_ORDER:       /* from SMH */
        ...
        MTP3_Alarm_PRC(2512, __FILE__, __LINE__, "ls=%x:link=%x:family=%s",
                        linkSet,linkCode, "PRC");
    ...
    case STM_TRIGGER_T20:                    /* from timer control */
        ...
        if (ln->availableState == LINK_UNAVAILABLE)
        {
            if (ln->stFlags & LINK_LOC_INHIBITED)
            {
                ...
                MTP3_Alarm_PRC(2522, __FILE__, __LINE__,"ls=%x:link=%x:family=%s", linkSet,linkCode, "PRC");
    ...
    case STM_TRIGGER_T21:                    /* from timer control */
        ...
        if (ln->availableState == LINK_UNAVAILABLE)
        {
            if (ln->stFlags & LINK_REM_INHIBITED)
            {
                ...
                MTP3_Alarm_PRC(2523, __FILE__, __LINE__,
                                 "ls=%x:link=%x:family=%s", linkSet,linkCode, "PRC");
...
EmergencyState(ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ...
    if (!SPRestarting_PRC() && !RemoteSPAccessible(ls->adjacent, ls->ni))
    {
        return (ITS_TRUE);
    }
               ...
               int firstMatch, int lastMatch)
{
        ...
        ri->priority <= ent->entries[current]->priority)
    {
         ...
         * like PRC.  Don't forget that you don't remove STPs for
 ...
 ****************************************************************************/
int
TSRC_Main_PRC(MTP3_SubSystems src, STM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_STM_TSRC, src, trigger, sif, len,
                          linkSet, linkCode);
    ...
    case STM_TRIGGER_RESTART_END:               /* from TPRC */
        ...
        for (i = 0; i < ITS_SS7_DEFAULT_LINK_SET; i++)
        {
            ...
            if (ls && ls->adjacent != 0 && ls->family == FAMILY_CHINA)
            {
                /* Build SNMM traffic restart allowed */
                MTP3_HDR_SET_SIO(omtp3, MTP3_SIO_SNMM | ls->ni);
                MTP3_RL_SET_DPC_VALUE(omtp3.label, ls->adjacent);
        ...
        SendingStatusSet_PRC(ITS_FALSE);

        break;

    case STM_TRIGGER_RESTART_BEGIN:             /* from TPRC */

        PEG_IncrPeg(PRC_MTP3_Pegs, PEG_MTP3_RESTART);
        if (SPRestarting_PRC())
        {
            /*
    ...
    case STM_TRIGGER_CHANGEOVER_EXECUTED:       /* from TCOC */
        ...
        PEG_IncrPeg(PRC_MTP3_Pegs, PEG_MTP3_CHANGEOVER);
    ...
    case STM_TRIGGER_LINK_UNAVAILABLE:          /* from TLAC */
        ...
        if (!isUp)
            PEG_IncrPeg(PRC_MTP3_Pegs,PEG_MTP3_SLS_UNAVAILABLE);
    ...
    case STM_TRIGGER_LINK_CONGESTION_STATUS:    /* implementation dependent */
        ...
        if (ls != NULL)
        {
            ...
            if (sif[0] == L2_L3_CONGESTION_ONSET)
            {
                ...
                MTP2_SendData_PRC(linkSet, linkCode,
...
/* ITU doesn't do this */
    ...
    case STM_TRIGGER_TRAFFIC_RESTART_WAITING:   /* from HMDT */
        ...
        linkSet = MTP3_FindAdjacent_PRC(ni, pc, linkSet);
 ...
 ****************************************************************************/
int
TRCC_Main_PRC(MTP3_SubSystems src, STM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    MTP3_HEADER *mtp3 = (MTP3_HEADER *)&sif[1], omtp3;
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_STM_TRCC, src, trigger, sif, len,
                          linkSet, linkCode);
    ...
    case STM_TRIGGER_DEST_CONGESTION_STATUS:    /* from RTCC */
                                                 ...
                                                 ITS_SS7_DEFAULT_SLS)) != ITS_SUCCESS)
        {
            ...
            MTP3_Alarm_PRC(2477, __FILE__, __LINE__,"type=%s:dpc=%x:ni=%x:family=%s","remote",pc,ni,"PRC");
 ...
 ****************************************************************************/
int
TCOC_Main_PRC(MTP3_SubSystems src, STM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    MTP3_TimerContext tc;
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_STM_TCOC, src, trigger, sif, len,
                          linkSet, linkCode);
    ...
    case STM_TRIGGER_LINK_AVAILABLE:            /* from TLAC */
        if (ln->changeoverState == CHANGEOVER_WAIT_1)
        {
            ...
            {
                evRec = ItsEventToMTP3EventRec_PRC(*cocbEv);

                fetched = &evRec.event;

                MTP2_SendData_PRC(L2_LINK_SET(fetched),
    ...
    case STM_TRIGGER_EMERG_CHANGEOVER_ORDER:    /* from TLAC */
        ...
        PEG_IncrPeg(PRC_MTP3_Pegs, PEG_MTP3_EMER_CHANGEOVER);
        ...
        else if (ln->changeoverState == CHANGEOVER_WAIT_FOR_ACK)
        {
            ...
            {                              
                                                  ...
                                                  ITS_SS7_DEFAULT_SLS) != ITS_SUCCESS)
                {
                    ...
                    MTP3_Alarm_PRC(2482, __FILE__, __LINE__, "pc=%x:family=%s",ls->adjacent, "PRC");
    ...
    case STM_TRIGGER_BSNT:                      /* from L2 */
        ...
        if (ln->changeoverState == CHANGEOVER_WAIT_2)
        {
            ...
            MTP3_Alarm_PRC(2513, __FILE__, __LINE__,
    ...
    case STM_TRIGGER_BSNT_NOT_RETRIEVABLE:      /* from L2 */
        if (ln->changeoverState == CHANGEOVER_WAIT_2)
        {
            ...
            MTP3_Alarm_PRC(2515, __FILE__, __LINE__,
                            "ls=%x:link=%x:family=%s",
                             tls,tlc, "PRC");
...
{ 
    ...
    if (prev == NULL)
    {
        here  = ItsEventToMTP3EventRec_PRC(item->data);
        there = ItsEventToMTP3EventRec_PRC(next->data);
        /*
    ...
    here = ItsEventToMTP3EventRec_PRC(item->data);
    there = ItsEventToMTP3EventRec_PRC(next->data);
    before = ItsEventToMTP3EventRec_PRC(prev->data);
...
DsmQueueEntryToMTP3EventRec_PRC(DSMQUEUE_Entry entry)
{
    MTP3_EventRec evRec;
    ...
    for (i = 0; i < ls->numLinks; i++)
    {
        ...
        if (ls->links[i]->linkCode != linkCode)
        {
            ...
            for (i = 0, ent = &ln->cocbBuf->entries[ln->cocbBuf->firstEntry];
                 ...
                 i++)
            {
                evRec = DsmQueueEntryToMTP3EventRec_PRC(*ent);
 ...
 ****************************************************************************/
int
TCBC_Main_PRC(MTP3_SubSystems src, STM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{    
    MTP3_HEADER *mtp3 = (MTP3_HEADER *)&sif[1];
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_STM_TCBC, src, trigger, sif, len,
                          linkSet, linkCode);
    ...
    case STM_TRIGGER_CHANGEBACK_DECL:           /* from HMDT */
        ...
        MTP3_Alarm_PRC(2511, __FILE__, __LINE__, "ls=%x:link=%x:family=%s",
                        linkSet,linkCode, "PRC");
        ...
        PEG_IncrPeg(PRC_MTP3_Pegs, PEG_MTP3_CHANGEBACK);
...
*/
                    ...
                    MTP3_Alarm_PRC(2514, __FILE__, __LINE__,
                                    "ls=%x:link=%x:family=%s",linkSet,ln->linkCode, "PRC");
        ...
        else
        {
            ...
            MTP3_Alarm_PRC(2487, __FILE__, __LINE__, 
                             "ls=%x:link=%x:family=%s",tls, tlc, "PRC");
        }
        break;
 ...
 ****************************************************************************/
int
TFRC_Main_PRC(MTP3_SubSystems src, STM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_STM_TFRC, src, trigger, sif, len,
                          linkSet, linkCode);
    ...
    case STM_TRIGGER_ROUTE_UNAVAILABLE:         /* from TSRC */
                                                 ...
                                                 ITS_SS7_DEFAULT_SLS)) != ITS_SUCCESS)
        {
            ...
            MTP3_Alarm_PRC(2489, __FILE__, __LINE__, "pc=%x:family=%s",pc, "PRC");
    ...
    case STM_TRIGGER_ALTERNATE_ROUTE_DATA:      /* from TSRC */
        ...
        if (ri->forcedState == FORCE_REROUTING_WAIT_1)
        {
            ...
            RTAC_Main_PRC(MTP3_STM_TFRC,
                           SRM_TRIGGER_FORCED_REROUTE_END,
                           ev.data, ev.len,
                           oldLS, oldLC);

            RTRC_Main_PRC(MTP3_STM_TFRC,
                           SRM_TRIGGER_FORCED_REROUTE_END,
                           ev.data, ev.len,
                           oldLS, oldLC);
 ...
 ****************************************************************************/
int
TCRC_Main_PRC(MTP3_SubSystems src, STM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    MTP3_HEADER *mtp3 = (MTP3_HEADER *)&sif[1];
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_STM_TCRC, src, trigger, sif, len,
                          linkSet, linkCode);
    ...
    case STM_TRIGGER_T6:                    /* from timer control */
        ...
        if (ri->controlledState == CONT_REROUTING_WAIT_2)
        {
            ...
            RTAC_Main_PRC(MTP3_STM_TCRC,
                           SRM_TRIGGER_CONTROLLED_REROUTE_END,
                           sif, len,
                           linkSet, linkCode);

            RTRC_Main_PRC(MTP3_STM_TCRC,
                           SRM_TRIGGER_CONTROLLED_REROUTE_END,
                           sif, len,
                           linkSet, linkCode);
 ...
 ****************************************************************************/
int
TSFC_Main_PRC(MTP3_SubSystems src, STM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_STM_TSFC, src, trigger, sif, len,
                          linkSet, linkCode);
    ...
    case STM_TRIGGER_DEST_CONGESTION_STATUS:    /* from TRCC, RTCC, TSRC */
        ...
        PEG_IncrPeg(PRC_MTP3_Pegs,PEG_MTP3_CONGESTION);
    ...
    case STM_TRIGGER_DEST_INACCESS:             /* from TSRC */
        ...
        PEG_IncrPeg(PRC_MTP3_Pegs, PEG_MTP3_ADJECENT_SP_UNACCESIBLE);

        /* Increament the peg for Route Set Unavailable */
        PEG_IncrPeg(PRC_MTP3_Pegs, PEG_MTP3_ROUTESET_UNAVAILABLE);

        /* This alarm is moved from ttc-mtp3.c module, since MTP3_Pause callback
           in ttc-mtp3.c is not invoked */

        MTP3_Alarm_PRC(2401, __FILE__, __LINE__,"pc=%x:family=%s", apc, "PRC");
    ...
    case STM_TRIGGER_DEST_ACCESS:               /* from TSRC */
        ...
        MTP3_Alarm_PRC(2402, __FILE__, __LINE__, "pc=%x:family=%s", apc, "PRC");
 ...
 ****************************************************************************/
int
TPRC_Main_PRC(MTP3_SubSystems src, STM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    MTP3_HEADER *mtp3 = (MTP3_HEADER *)&sif[1];
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_STM_TPRC, src, trigger, sif, len,
                          linkSet, linkCode);
    ...
    case STM_TRIGGER_T27: /* from timer control */
        ...
        LLSC_Main_PRC(MTP3_STM_TPRC,
                       SLM_TRIGGER_RESTART_BEGINS,
                       NULL, 0,
                       ITS_SS7_DEFAULT_LINK_SET,
                       ITS_SS7_DEFAULT_LINK_CODE);

        TSRC_Main_PRC(MTP3_STM_TPRC,
                       STM_TRIGGER_RESTART_BEGIN,
                       NULL, 0,
                       ITS_SS7_DEFAULT_LINK_SET,
                       ITS_SS7_DEFAULT_LINK_CODE);

        TSFC_Main_PRC(MTP3_STM_TPRC,
                       STM_TRIGGER_RESTART_BEGIN,
                       NULL, 0,
                       ITS_SS7_DEFAULT_LINK_SET,
                       ITS_SS7_DEFAULT_LINK_CODE);

        TLAC_Main_PRC(MTP3_STM_TPRC,
                       STM_TRIGGER_RESTART_BEGIN,
                       NULL, 0,
                       ITS_SS7_DEFAULT_LINK_SET,
                       ITS_SS7_DEFAULT_LINK_CODE);

        RSRT_Main_PRC(MTP3_STM_TPRC,
