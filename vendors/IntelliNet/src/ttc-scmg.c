 ...
 ****************************************************************************
 *
 *  ID: $Id: ttc-scmg.c,v 9.7 2008/07/18 05:45:33 ssingh Exp $
 *
 * LOG: $Log: ttc-scmg.c,v $
 * LOG: Revision 9.7  2008/07/18 05:45:33  ssingh
 * LOG: SDS ID:: ID :: D0240 Warning removal for Solaris Compilation
 * LOG:
 * LOG: Revision 9.6  2008/06/24 11:31:43  ssingh
 * LOG: fix for unit testing issue #1009
 * LOG:
 ...
 * LOG:
 * LOG: Revision 9.2.2.1  2007/05/18 06:47:41  pspl
 ...
 * LOG:
 * LOG: Revision 1.1.2.4  2007/02/13 04:02:03  raghavendrabk
 * LOG: Changed alarm format to put family at beginning
 * LOG:
 * LOG: Revision 1.1.2.3  2007/02/02 14:18:41  raghavendrabk
...
#ident "$Id: ttc-scmg.c,v 9.7 2008/07/18 05:45:33 ssingh Exp $"
 ...
 ****************************************************************************/
int
SendSCMG_TTC(MTP3_HEADER* mtp3, SCCP_SCMG* scmg, ITS_UINT toSSN)
{
    ITS_SCCP_IE lies[32];
    ITS_OCTET addrInd;

    SCCP_DEBUG(("SendSCMG_TTC: type %02x ssn %02x pc %08x\n",
    ...
    return SCLCHandleSCMGMsg_TTC(mtp3, SCCP_MSG_UDT, lies, 4);
}
...
SSTCStartSST_TTC(MTP3_HEADER* mtp3, ITS_UINT pc, ITS_OCTET ssn)
{
     ...
     * lock the table
     */
    if (DSM_LockTable(DSM_SccpFtGroupId, DSM_TABLE_TTC_SCMG) != ITS_SUCCESS)
    {
                              ...
                              &error)) != NULL)
    {
        ...
        DSM_UnlockTable(DSM_SccpFtGroupId, DSM_TABLE_TTC_SCMG);
    ...
    skey = DSM_MGMT_REC_TTC_SCCP_SST_SERIAL;
 ...
 ****************************************************************************/
void
SSTCStopSST_TTC(MTP3_HEADER* mtp3, ITS_UINT pc, ITS_OCTET ssn)
{
    SST_Key key;
    ROUTE_Info* tmp;
    ITS_UINT size, skey;
    ...
    size = 0;
    DSM_TableGetSize(DSM_SccpFtGroupId, DSM_TABLE_TTC_SCMG, &size);
 ...
 ****************************************************************************/
ITS_BOOLEAN
SCMGIsSSTPending_TTC(ROUTE_Info *rinfo)
{
    ITS_INT error;
    SST_Key key;
    ROUTE_Info* tmp;
...
BCSTSendSSA(MTP3_HEADER* mtp3, SCCP_SCMG* oscmg, ITS_BOOLEAN fromCPC)
{
    ...
    if (!fromCPC)
    {
        ...
        for (tinfo1 = rinfo; rinfo != NULL; rinfo = rinfo->next)
        {
            ...
            {
                SendSCMG_TTC(&hdr, &scmg, rinfo->basic.criteria.ssn);
            }
            else if (rinfo->basic.style == ROUTE_DPC_SIO_SSN_RANGE ||
                     rinfo->basic.style == ROUTE_DPC_OPC_SIO_SSN_RANGE)
            {
                SendSCMG_TTC(&hdr, &scmg, rinfo->basic.criteria.range.begin);
            }
        }
    ...
    else
    {
        ...
        for (tinfo1 = linfo; linfo != NULL; linfo = linfo->next)
        {
            ...
            for (tinfo2 = rinfo; rinfo != NULL; rinfo = rinfo->next)
            {
                ...
                SendSCMG_TTC(&hdr, &scmg, localSSN);
            }
        }
...
BCSTSendSSC(MTP3_HEADER* mtp3, SCCP_SCMG* oscmg, ITS_BOOLEAN fromCPC)
{
    ...
    if (DSM_LockTable(DSM_SccpFtGroupId, DSM_TABLE_TTC_SCCP_CPC) != ITS_SUCCESS)
    {
        SCCP_ERROR(("Can't lock table\n"));
                        ...
                        ITS_OCTET ni, ITS_UINT pc, ITS_OCTET ssn)
{
    ...
    MTP3_RL_SET_OPC_VALUE(hdr.label, 
                          MTP3_PC_GET_VALUE_TTC(uos->pointCode));
    ...
    skey = DSM_MGMT_REC_TTC_SCCP_SOR_SERIAL;
    if ((serial = DSM_FindNoLock(DSM_SccpFtGroupId,
                                 DSM_TABLE_MGMT_DATA,
                                 (ITS_OCTET *)&skey,
                                 sizeof(ITS_UINT),
...
SCMG_SendSOGToDuplicate(MTP3_HEADER *mtp3, SCCP_SCMG *sor)
{
    ...
    SendSCMG_TTC(&resp, &scmg, sor->ssn);
 ...
 *
 *  Notes:
 *      There's a comment in SCRCHandleSCLCMsg_TTC that explains why
...
SendSST(ITS_POINTER data, void *in, void *out)
{
    ...
    {
        SendSCMG_TTC(&tmp->sourceHdr, &scmg,
...
SCMGHandleSSTTimeout_TTC()
{
    ...
    /* Generate the SSTs. */
    DSM_TableIterate(
        DSM_SccpFtGroupId, DSM_TABLE_TTC_SCMG, NULL, NULL, SendSST);
 ...
 ****************************************************************************/
int
SCMGSSPCHandleMsgForProhibitedSSN_TTC(MTP3_HEADER* mtp3, ITS_OCTET mType,
                                        ...
                                        ITS_OCTET reason)
{
    ...
    if (SCCP_IsSSNEquipped(ni, pc, ssn))
    {
        ...
        return SendSCMG_TTC(&resp, &scmg, toSSN);
    }
 ...
 ****************************************************************************/
int
SCMGHandleSCLCMsg_TTC(MTP3_HEADER* mtp3, ITS_OCTET mType,
                        ITS_SCCP_IE* ies, int ieCount)
{
    int i, error;
    SCCP_SCMG scmg;
    ...
    if (data == NULL || clp == NULL)
    {
        ...
        SCCP_Alarm_TTC(450, __FILE__, __LINE__, "family=%s", "TTC");
    ...
    case SCCP_SCMG_SS_ALLOWED:
        ...
        PEG_IncrPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_SSA_RCVD);

        SCCP_Alarm_TTC(443, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                       "TTC", MTP3_RL_GET_OPC_VALUE(mtp3->label), scmg.ssn);
        ...
        SSTCStopSST_TTC(mtp3, pc, scmg.ssn);

        if (isAllowed)
        {
            /* Already in the allowed state SSN=%x. */
            SCCP_Alarm_TTC(446, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                           "TTC", pc, scmg.ssn);
        ...
        SCCP_Alarm_TTC(455, __FILE__, __LINE__,"family=%s:pc=%x:ssn=%x",
                       "TTC", MTP3_RL_GET_OPC_VALUE(mtp3->label), scmg.ssn);
        break;
    ...
    case SCCP_SCMG_SS_PROHIBIT:
        ...
        PEG_IncrPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_SSP_RCVD);

        /* Q752 Implementation */
            SsnOmImage[omBuf.activeOm].SCCPSSNPegs[ssn].ssn = ssn;
            SsnOmImage[omBuf.activeOm].SCCPSSNPegs[ssn].messageSSPRcvd++;

        SCCP_Alarm_TTC(444, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                       "TTC", MTP3_RL_GET_OPC_VALUE(mtp3->label), scmg.ssn);
        ...
        if (!isAllowed)
        {
            ...
            SCCP_Alarm_TTC(447, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                           "TTC", pc, scmg.ssn);
            return (ITS_SUCCESS);
        }
        ...
        SSTCStartSST_TTC(mtp3, pc, scmg.ssn);
        ...
        /* State change to Prohibted State SSN=%x */
         SCCP_Alarm_TTC(456, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                        "TTC", pc, scmg.ssn);
    ...
    case SCCP_SCMG_SS_STATUS_TEST:
        ...
        skey = DSM_MGMT_REC_TTC_SCCP_SST_IGNORE_SERIAL;
        if ((serial = DSM_FindNoLock(DSM_SccpFtGroupId,
                                     DSM_TABLE_MGMT_DATA,
                                     (ITS_OCTET *)&skey,
                                     sizeof(ITS_UINT),
    ...
    case SCCP_SCMG_SS_CONGESTED:
        ...
        if (!isAllowed)
        {
            SCCP_Alarm_TTC(448, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                           "TTC", pc, scmg.ssn);
            return (ITS_SUCCESS);
        }
    ...
    case SCCP_SCMG_SS_UIS:
        ...
        if (!isLocal)
        {
            SCCP_Alarm_TTC(454, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                           "TTC", pc, scmg.ssn);
            /* Discarding Message - Invalid or Corrupted. */
        ...
        if (isAllowed)
        {
            SCCP_Alarm_TTC(451, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                           "TTC", pc, scmg.ssn);
    ...
    case SCCP_SCMG_SS_UOS:
    {
        ...
        if (!isLocal)
        {
            SCCP_Alarm_TTC(453, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                           "TTC", pc,scmg.ssn);
            /* How the heck did we get this */
        ...
        if (!isAllowed)
        {
            ...
            SCCP_Alarm_TTC(452, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                           "TTC", MTP3_RL_GET_OPC_VALUE(mtp3->label), scmg.ssn);
    ...
    default:
        ...
        SCCP_Alarm_TTC(449, __FILE__, __LINE__, "family=%s", "TTC");
        break;
    }
                                        ...
                                        sizeof(ITS_UINT), &error)) != NULL)
            {
                for (tinfo2 = rinfo; rinfo != NULL; rinfo = rinfo->next)
                {
                    if (rinfo->basic.dpc == pc)
                    {
                        if (rinfo->basic.style == ROUTE_DPC_OPC_SIO_SSN)
                        {
                            SSTCStopSST_TTC(mtp3, pc, rinfo->basic.criteria.ssn);
                        ...
                        else if (rinfo->basic.style == ROUTE_DPC_OPC_SIO_SSN_RANGE)
                        {
                            ...
                            for (cnt = rinfo->basic.criteria.range.begin;
                                 ...
                                 cnt++)
                            {
                                SSTCStopSST_TTC(mtp3, pc, (ITS_OCTET)cnt);
                                        ...
                                        sizeof(ITS_UINT), &error)) != NULL)
            {
                for (tinfo2 = rinfo; rinfo != NULL; rinfo = rinfo->next)
                {
                    if (rinfo->basic.dpc == pc)
                    {
                        if (rinfo->basic.style == ROUTE_DPC_OPC_SIO_SSN)
                        {
                            SSTCStartSST_TTC(mtp3, pc, rinfo->basic.criteria.ssn);
                        }
                        else if (rinfo->basic.style == ROUTE_DPC_OPC_SIO_SSN_RANGE)
                        {
                            ITS_USHORT cnt;
                            ...
                            for (cnt = rinfo->basic.criteria.range.begin;
                                 ...
                                 cnt++)
                            {
                                SSTCStartSST_TTC(mtp3, pc, (ITS_OCTET)cnt);
                            }
                        }
                    }
                }
 ...
 ****************************************************************************/
int
SCMGHandleMTP3Msg_TTC(MTP3_HEADER* mtp3, ITS_OCTET mType,
                        ITS_OCTET* data, ITS_USHORT len)
{
    ...
    case MTP3_MSG_PAUSE:
        ...
        SCCP_Alarm_TTC(437, __FILE__, __LINE__, "family=%s:pc=%x",
                       "TTC", MTP3_RL_GET_OPC_VALUE(mtp3->label));

        if (len != sizeof(MTP3_PAUSE_RESUME))
        {
           SCCP_Alarm_TTC(439, __FILE__, __LINE__, "family=%s", "TTC");
           break;
        }
        ...
        PEG_IncrPeg(TTC_SCCP_Pegs, PEG_MTP3_MSG_PAUSE);
    ...
    case MTP3_MSG_RESUME:
        ...
        SCCP_Alarm_TTC(436, __FILE__, __LINE__, "family=%s:pc=%x",
                       "TTC", MTP3_RL_GET_OPC_VALUE(mtp3->label));

        /* Incr Resume Indication Counter for SCCP */
        PEG_IncrPeg(TTC_SCCP_Pegs, PEG_MTP3_MSG_RESUME);
    ...
    case MTP3_MSG_STATUS:
        ...
        SCCP_Alarm_TTC(438, __FILE__, __LINE__, "pc=%x:family=%s",
                       MTP3_RL_GET_OPC_VALUE(mtp3->label), "TTC");
        ...
        PEG_IncrPeg(TTC_SCCP_Pegs, PEG_MTP3_MSG_STATUS);
