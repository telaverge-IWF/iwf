 ...
 ****************************************************************************
 *
 *  ID: $Id: itu-wait_in_anm_itu93.c,v 9.2 2005/04/06 07:17:24 ssingh Exp $
 *
 * LOG: $Log: itu-wait_in_anm_itu93.c,v $
 ...
 * LOG:
 * LOG: Revision 7.3.2.2.6.1.12.2  2005/02/22 11:56:09  ssingh
 * LOG: Fix for #1597 (Ramesh KV)
 * LOG:
 * LOG: Revision 7.3.2.2.6.1.12.1  2004/12/29 14:56:05  csireesh
 * LOG: ISUP OAM Changes
 * LOG:
 * LOG: Revision 7.3.2.2.6.1  2004/01/05 10:10:45  sjaddu
 * LOG: Merged code from tcs isup branch.
 * LOG:
 * LOG: Revision 7.3.2.2.2.2  2003/10/10 09:28:34  akumar
 * LOG: Fied bug #497.
 * LOG:
 * LOG: Revision 7.3.2.2.2.1  2003/09/05 09:44:02  akumar
 * LOG: Modifications for Msg Based redundancy scheme.
 * LOG:
 * LOG: Revision 7.3.2.2  2003/07/25 05:13:08  akumar
 ...
 * LOG:
 * LOG: Revision 8.4  2003/04/04 05:10:50  ssingh
 * LOG: ITS_ISUP_CIC_REL_CFM is removed.
 * LOG:
 * LOG: Revision 8.3  2003/04/03 15:29:29  sjaddu
 * LOG: ISUP DSM reorganization.
 * LOG:
 * LOG: Revision 8.2  2003/02/27 11:04:10  akumar
 * LOG: Warning Removal
 * LOG:
 ...
 * LOG:
 * LOG: Revision 1.6.2.9  2002/04/05 19:37:45  ssharma
 * LOG: Add parameter to timeout handling procedures.
 * LOG:
 * LOG: Revision 1.6.2.8  2002/04/02 23:53:55  ssharma
 ...
 * LOG:
 * LOG: Revision 1.4  2002/02/21 23:50:25  ssharma
...
/*
 * Functions for handling messages in the Wait_Incoming_ANM state for ITU93 
 * variant. This is considered to be the generic ITU variant, and all other 
 * ITU variantsuse these functions as defaults if they do not have different
 * behaviour.
 */
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received ANM in WAIT_IN_ANM from MTP3, pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        /* Check cic as outgoing_busy */ 
        if (ISUP_CIC_GET_CALL(cic_rec->cic_state) != CKTS_OUT_CTK_BUSY)
        {
            ISUP_ERROR(("WAIT_IN_ANM ANM: ANM from L3 in unexp cic state "
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CON received in the WAIT_INCOMING_ANM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    if (src != ISUP_SRC_L3_ITU)
    {
        ...
        /* Msg out of sequence. Drop it and generate alarm. */
        ISUP_ERROR(("WAIT_IN_ANM CON: Unexpected CON from App. pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
    }

    if (src == ISUP_SRC_L3_ITU)
    {
         ISUP_DEBUG(("Received CON in WAIT_IN_ANM from MTP3, pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
...
__ISUP_Received_CPG(ITS_EVENT *msg,  ISUP_CIC_INFO *cic_rec,
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    /* JP: FIXME Is this required here ? */

    if (src != ISUP_SRC_L3_ITU) 
    {
        ISUP_Alarm_CCITT(3485, __FILE__, __LINE__, "pcic %x", cic_rec->pcic);
        /* Msg out of sequence. Drop it and generate alarm. */
        ISUP_ERROR(("WAIT_IN_ANM CPG: Unexpected CPG from App. pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
    }

    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received CPG in WAIT_IN_ANM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an IDR received in the WAIT_INCOMING_ANM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received IDR in WAIT_IN_ANM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
    ...
    if (src != ISUP_SRC_L3_ITU) 
    {
        ISUP_DEBUG(("Received IDR in WAIT_IN_ANM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        ...
        if (ret != ITS_SUCCESS)
        {
            ISUP_ERROR(("WAIT_IN_ANM IDR: Failed to start T39. pcic 0x%08x, vcic %d.\n",
                        cic_rec->pcic, cic_rec->vcic));
        }
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received INF in WAIT_IN_ANM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Stop timer T33 */
    ...
    if (src != ISUP_SRC_L3_ITU) 
    {
        ...
        /* Msg out of sequence. Drop it and generate alarm. */
        ISUP_ERROR(("WAIT_IN_ANM INF: Unexpected INF from APP. pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
    }
}
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ...
        /* Msg out of sequence. Drop it and generate alarm. */
        ISUP_ERROR(("WAIT_IN_ANM INR: Unexpected INR from MTP3. pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
    }

    if (src != ISUP_SRC_L3_ITU) 
    {
        ISUP_DEBUG(("Received INR in WAIT_IN_ANM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        ...
        if (ret != ITS_SUCCESS)
        {
            ISUP_ERROR(("WAIT_IN_ANM INR: Failed to start T33. pcic 0x%08x, vcic %d.\n",
                        cic_rec->pcic, cic_rec->vcic));
        }
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an IRS received in the WAIT_INCOMING_ANM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received IRS in WAIT_IN_ANM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
    ...
    if (src != ISUP_SRC_L3_ITU) 
    {
        ISUP_DEBUG(("Received IRS in WAIT_IN_ANM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an NRM received in the WAIT_INCOMING_ANM state is 
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received NRM in WAIT_IN_ANM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to app */
        SPRC_SendIsupToApp_CCITT(cic_rec->vcic, msg);
    ...
    {
        ISUP_DEBUG(("Received NRM in WAIT_IN_ANM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to L3 */
        SPRC_SendIsupToMTP3_CCITT(msg, cic_rec->vcic);
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an OLM received in the WAIT_INCOMING_ANM state is 
 *      done here. The message is forwarded to the application if received  
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received OLM in WAIT_IN_ANM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Start Timer T3 */
        ...
        if (ret != ITS_SUCCESS)
        {
            ISUP_ERROR(("WAIT_IN_ANM OLM: Failed to start T3. pcic 0x%08x, vcic %d.\n",
                        cic_rec->pcic, cic_rec->vcic));
        }
        else
        {
    ...
    {
        ISUP_DEBUG(("Received OLM in WAIT_IN_ANM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to L3 */
        SPRC_SendIsupToMTP3_CCITT(msg, cic_rec->vcic);
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received REL in WAIT_IN_ANM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received RLC in WAIT_IN_ANM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an USR received in the WAIT_INCOMING_ANM state is 
 *      done here. The message is forwarded to the application if received  
 *      from the network.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received USR in WAIT_IN_ANM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to app */
        SPRC_SendIsupToApp_CCITT(cic_rec->vcic, msg);
    ...
    {
        ISUP_DEBUG(("Received USR in WAIT_IN_ANM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to L3 */
        SPRC_SendIsupToMTP3_CCITT(msg, cic_rec->vcic);
                          ...
                          ISUP_OPC_DPC_INFO *config_info)
{
    ...
    ISUP_DEBUG(("TimeoutT3 event received in WAIT_IN_ANM state. "
    ...
    {
        /* log error */
        ISUP_ERROR(("TimeoutT3 in WAIT_IN_ANM state: Unable to remove T3 "
                          ...
                          ISUP_OPC_DPC_INFO *config_info)
{
    ...
    ISUP_Alarm_CCITT(3419, __FILE__, __LINE__,NULL);
                           ...
                           ISUP_OPC_DPC_INFO *config_info)
{
    ...
    ISUP_DEBUG(("TimeoutT33 event received in WAIT_IN_ANM state. "
                "Info from CIC rec: pcic 0x%08x, vcic %d, cic %d.\n",
                 cic_rec->pcic, cic_rec->vcic, ISUP_PCIC_TO_CIC(cic_rec->pcic)));
    ...
    {
        /* log error */
        ISUP_ERROR(("TimeoutT33 in WAIT_IN_ANM state: Unable to remove T33 "
                    "info from CIC rec. Error %s. pcic 0x%08x, vcic %d,"
                    "cic %d.\n", ITS_GetErrorText(ret), cic_rec->pcic, 
                     cic_rec->vcic, ISUP_PCIC_TO_CIC(cic_rec->pcic)));
    }
                           ...
                           ISUP_OPC_DPC_INFO *config_info)
{
    ...
    ISUP_DEBUG(("TimeoutT39 event received in WAIT_IN_ANM state. "
                "Info from CIC rec: pcic 0x%08x, vcic %d, cic %d.\n",
                 cic_rec->pcic, cic_rec->vcic, ISUP_PCIC_TO_CIC(cic_rec->pcic)));
    ...
    {
        /* log error */
        ISUP_ERROR(("TimeoutT39 in WAIT_IN_ANM state: Unable to remove T39 "
                    "info from CIC rec. Error %s. pcic 0x%08x, vcic %d,"
                    "cic %d.\n", ITS_GetErrorText(ret), cic_rec->pcic, 
                     cic_rec->vcic, ISUP_PCIC_TO_CIC(cic_rec->pcic)));
    }
