 ...
 ****************************************************************************
 *
 *  ID: $Id: itu-wait_out_acm_itu93.c,v 9.2 2005/04/06 07:17:24 ssingh Exp $
 *
 * LOG: $Log: itu-wait_out_acm_itu93.c,v $
 ...
 * LOG:
 * LOG: Revision 7.3.2.3.6.1.12.2  2005/02/08 12:30:26  ssingh
 * LOG: Peg increament after receiving Unexpected Msg in given FSM state.
 * LOG: Bug# 1448. (Ramesh KV, Abhijit Dutta)
 * LOG:
 * LOG: Revision 7.3.2.3.6.1.12.1  2004/12/29 14:56:05  csireesh
 * LOG: ISUP OAM Changes
 * LOG:
 * LOG: Revision 7.3.2.3.6.1  2004/01/05 10:10:45  sjaddu
 * LOG: Merged code from tcs isup branch.
 * LOG:
 * LOG: Revision 7.3.2.3.2.1  2003/09/05 09:46:52  akumar
 * LOG: Modifications for msg based redundancy scheme.
 * LOG:
 * LOG: Revision 7.3.2.3  2003/07/25 05:13:08  akumar
 * LOG: ITU National Matter is implemented.
 * LOG:
 * LOG: Revision 7.3.2.2  2003/05/07 11:03:13  ssingh
 ...
 * LOG:
 * LOG: Revision 8.4  2003/05/02 13:51:16  akumar
 * LOG: Fixed Bug #242.
 * LOG:
 * LOG: Revision 8.3  2003/04/30 05:22:45  akumar
 * LOG: Aligned with SDL.
 * LOG:
...
static void __ISUP_Received_UnexpectedMsg(ITS_EVENT *msg, 
                                          ISUP_CIC_INFO *cic_rec,
                                          ISUP_OPC_DPC_INFO *config_info, 
                                          ITS_OCTET src);
/*
 * Functions for handling messages in the Wait_Outgoing_ACM state for ITU93 variant.
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an ACM received in the WAIT_OUTGOING_ACM state 
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    /* Check cic as incoming_busy and forward ACM to MTP3 */

    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_Alarm_CCITT(3492, __FILE__, __LINE__, "pcic %x", cic_rec->pcic);

        /* Msg out of sequence. Drop it and generate alarm. */
        ISUP_ERROR(("WAIT_OUT_ACM ACM: Unexpected ACM from MTP3. pcic 0x%08x, "
    ...
    if (src != ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received ACM in WAIT_OUT_ACM from APP, pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
        if (ISUP_CIC_GET_CALL(cic_rec->cic_state) != CKTS_IN_CTK_BUSY)
        {
            ISUP_ERROR(("WAIT_OUT_ACM ACM: ACM from App in unexp cic state "
        ...
        {
            __ISUP_SegmentOverlenMsg_ITU93(msg, cic_rec, ISUP_MSG_ACM,
                                                          config_info);
        }
        else
        {
        ...
        cic_rec->fsm_state = WAIT_OUTGOING_ANM;
    }
}
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an BLA received in the WAIT_OUTGOING_ACM state 
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    __ISUP_Received_UnexpectedMsg(msg, cic_rec, config_info, src);
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an BLO received in the WAIT_OUTGOING_ACM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CPG received in the WAIT_OUTGOING_ACM state 
 *      is done here. The message is forwarded to the network if received 
 *      from the application.
 *
 *  Input Parameters:
...
{
    ITS_USHORT dataLen = 0;
    /* Check cic as outgoing_busy and forward CPG to app */
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ...
        /* Msg out of sequence. Drop it and generate alarm. */
        ISUP_ERROR(("WAIT_OUT_ACM CPG: Unexpected CPG from MTP3. pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
    }

    if (src != ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received CPG in WAIT_OUT_ACM from App, pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
        if (ISUP_CIC_GET_CALL(cic_rec->cic_state) != CKTS_IN_CTK_BUSY)
        {
            ISUP_ERROR(("WAIT_OUT_ACM CPG: CPG from App in unexp cic state "
                        "0x%08x. pcic 0x%08x, vcic %d.\n", cic_rec->cic_state,
                         cic_rec->pcic, cic_rec->vcic));
        }
        /* Calculate the Encoded Msg Length */
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGB received in the WAIT_OUTGOING_ACM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGBA received in the WAIT_OUTGOING_ACM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGU received in the WAIT_OUTGOING_ACM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGUA received in the WAIT_OUTGOING_ACM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CON received in the WAIT_OUTGOING_ACM state 
 *      is done here. The message is forwarded to the network if received 
 *      from the application.
 *
 *  Input Parameters:
...
{
    ITS_USHORT dataLen = 0;
    /* Check cic as outgoing_busy and forward CON to app */
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_Alarm_CCITT(3495, __FILE__, __LINE__, "pcic %x", cic_rec->pcic);

        /* Msg out of sequence. Drop it and generate alarm. */
        ISUP_ERROR(("WAIT_OUT_ACM CON: Unexpected CON from MTP3. pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

    }
    else 
    {
        ISUP_DEBUG(("Received CON in WAIT_OUT_ACM from APP, pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CQM received in the WAIT_OUTGOING_ACM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CQR received in the WAIT_OUTGOING_ACM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an GRA received in the WAIT_OUTGOING_ACM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an GRS received in the WAIT_OUTGOING_ACM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an IDR received in the WAIT_OUTGOING_ACM state is 
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received IDR in WAIT_OUT_ACM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
    ...
    if (src != ISUP_SRC_L3_ITU) 
    {
        ISUP_DEBUG(("Received IDR in WAIT_OUT_ACM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        ...
        if (ret != ITS_SUCCESS)
        {
            ISUP_ERROR(("WAIT_OUT_ACM IDR: Failed to start T39. pcic 0x%08x, "
                        "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
        }
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an INF received in the WAIT_OUTGOING_ACM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    if (src == ISUP_SRC_L3_ITU)
    {
        ...
        /* Msg out of sequence. Drop it and generate alarm. */
        ISUP_ERROR(("WAIT_OUT_ACM INF: Unexpected INF from MTP3. pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
        /* State remains same */
    }
    if (src != ISUP_SRC_L3_ITU) 
    {
        ISUP_DEBUG(("Received INF in WAIT_OUT_ACM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Forward INF to MTP3 */
        SPRC_SendIsupToMTP3_CCITT(msg, cic_rec->vcic);
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an INR received in the WAIT_OUTGOING_ACM state 
 *      is done here. The message is forwarded to the network if received 
 *      from the application.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    if (src != ISUP_SRC_L3_ITU) 
    {
        ...
        /* Msg out of sequence. Drop it and generate alarm. */
        ISUP_ERROR(("WAIT_OUT_ACM INR: Unexpected INR from APP. pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
    }
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ...
        {
            ISUP_DEBUG(("Received INR in WAIT_OUT_ACM from MTP3, pcic 0x%08x, "
                        "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
            /* Forward INR to App */
            SPRC_SendIsupToApp_CCITT(cic_rec->vcic, msg);
        }
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an IRS received in the WAIT_OUTGOING_ACM state is 
 *      done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received IRS in WAIT_OUT_ACM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
    ...
    if (src != ISUP_SRC_L3_ITU) 
    {
        ISUP_DEBUG(("Received IRS in WAIT_OUT_ACM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an NRM received in the WAIT_OUTGOING_ACM state is 
 *      done here. The message is forwarded to the application if received  
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received NRM in WAIT_OUT_ACM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to app */
        SPRC_SendIsupToApp_CCITT(cic_rec->vcic, msg);
    ...
    {
        ISUP_DEBUG(("Received NRM in WAIT_OUT_ACM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to L3 */
        SPRC_SendIsupToMTP3_CCITT(msg, cic_rec->vcic);
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an OLM received in the WAIT_OUTGOING_ACM state is 
 *      done here. The message is forwarded to the application if received  
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received OLM in WAIT_OUT_ACM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Start Timer T3 */
        ...
        if (ret != ITS_SUCCESS)
        {
            ISUP_ERROR(("WAIT_OUT_ACM OLM: Failed to start T3. pcic 0x%08x, vcic %d.\n",
                        cic_rec->pcic, cic_rec->vcic));
        }
        else
        {
    ...
    {
        ISUP_DEBUG(("Received OLM in WAIT_OUT_ACM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to L3 */
        SPRC_SendIsupToMTP3_CCITT(msg, cic_rec->vcic);
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an PAM received in the WAIT_OUTGOING_ACM state is 
 *      done here. The message is forwarded to the application if received  
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received PAM in WAIT_OUT_ACM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to app */
        SPRC_SendIsupToApp_CCITT(cic_rec->vcic, msg);

        /* Change state to WAIT_OUTGOING_ACM */
    }
    else 
    {
        ISUP_DEBUG(("Received PAM in WAIT_OUT_ACM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to L3 */
        SPRC_SendIsupToMTP3_CCITT(msg, cic_rec->vcic);
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an REL received in the WAIT_OUTGOING_ACM state is 
 *      done here. The message is forwarded to the application if received  
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received REL in WAIT_OUT_ACM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
    ...
    {
        ISUP_DEBUG(("Received REL in WAIT_OUT_ACM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an RLC received in the WAIT_OUTGOING_ACM state is 
 *      done here. The message is forwarded to the application if received  
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received RLC in WAIT_OUT_ACM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
    ...
    {
        ISUP_DEBUG(("Received RLC in WAIT_OUT_ACM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to L3 */
        SPRC_SendIsupToMTP3_CCITT(msg, cic_rec->vcic);
    }
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an RSC received in the WAIT_OUTGOING_ACM state is 
 *      done here.
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an SAM received in the WAIT_OUTGOING_ACM state is 
 *      done here. The message is forwarded to the application if received  
 *      from the network.
 *
 *  Input Parameters:
...
__ISUP_Received_SAM(ITS_EVENT *msg,  ISUP_CIC_INFO *cic_rec,
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    /* Forward SAM appropriately */
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received SAM in WAIT_OUT_ACM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to app */
        SPRC_SendIsupToApp_CCITT(cic_rec->vcic, msg);

        /* Change state to WAIT_OUTGOING_ACM, i.e. no change. */
    }
    else 
    {
        /* Msg out of sequence, drop it. */
        ISUP_ERROR(("WAIT_OUT_ACM SAM: Unexpected SAM from APP. pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        ISUP_Alarm_CCITT(3497, __FILE__, __LINE__, "pcic %x", cic_rec->pcic);

        /* Change state to WAIT_OUTGOING_ACM i.e. same state */
    }
}
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UBA received in the WAIT_OUTGOING_ACM state is 
 *      done here. The message is forwarded to the application if received  
 *      from the network.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UBL received in the WAIT_OUTGOING_ACM state is 
 *      done here. The message is forwarded to the application if received  
 *      from the network.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an USR received in the WAIT_OUTGOING_ACM state is 
 *      done here. The message is forwarded to the application if received  
 *      from the network.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received USR in WAIT_OUT_ACM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to app */
        SPRC_SendIsupToApp_CCITT(cic_rec->vcic, msg);
    ...
    {
        ISUP_DEBUG(("Received USR in WAIT_OUT_ACM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to L3 */
        SPRC_SendIsupToMTP3_CCITT(msg, cic_rec->vcic);
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UCIC received in the WAIT_OUT_ACM state is done
 *      here.
 *
 *  Input Parameters:
 *      msg - the ISUP msg received by the stack as an ITS_EVENT.
 ...
 *      the WAIT_OUTGOING_ACM state. It also serves as a default handler for 
                              ...
                              ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ...
        ISUP_ERROR(("Rcvd Unexpected msg 0x%02x from MTP3 in WAIT_OUT_ACM. "
    ...
    {
        ISUP_ERROR(("Rcvd Unexpected msg 0x%02x from App in WAIT_OUT_ACM. "
                    "cic state 0x%02x, pcic 0x%08x, vcic %d.\n", 
                    msg->data[sizeof(MTP3_HEADER_CCITT) + sizeof (ITS_USHORT)],
                    cic_rec->cic_state, cic_rec->pcic, cic_rec->vcic));
        ...
        SPRC_SendErrorToApp_CCITT(msg->data[sizeof(MTP3_HEADER_CCITT) +
                                 sizeof (ITS_USHORT) + sizeof(ITS_OCTET)],
                                 ITS_ISUP_UNEXP_APPMSG_WAITOUT_ACM_ERR,
                                 cic_rec);
    }
}
                           ...
                           ISUP_OPC_DPC_INFO *config_info)
{
    ...
    ISUP_DEBUG(("TimeoutT3 event received in WAIT_OUT_ACM state. "
                "Info from CIC rec: pcic 0x%08x, vcic %d, cic %d.\n",
                 cic_rec->pcic, cic_rec->vcic, ISUP_PCIC_TO_CIC(cic_rec->pcic)));

    /* Remove info about timer from cic_rec. It is not restarted. */
    ret = ISUP_DeletePndgTmrInfo(cic_rec, ITS_ISUP_TIMER_T3);
    if (ret != ITS_SUCCESS)
    {
        /* log error */
        ISUP_ERROR(("TimeoutT3 in WAIT_OUT_ACM state: Unable to remove T3 "
                           ...
                           ISUP_OPC_DPC_INFO *config_info)
{
    ...
    ISUP_DEBUG(("TimeoutT28 event received in WAIT_OUT_ACM state. "
                "Info from CIC rec: pcic 0x%08x, vcic %d, cic %d.\n",
                 cic_rec->pcic, cic_rec->vcic, ISUP_PCIC_TO_CIC(cic_rec->pcic)));
    ...
    {
        /* log error */
        ISUP_ERROR(("TimeoutT28 in WAIT_OUT_ACM state: Unable to remove T28 info "
                           ...
                           ISUP_OPC_DPC_INFO *config_info)
{
    ...
    ISUP_DEBUG(("TimeoutT39 event received in WAIT_OUT_ACM state. "
                "Info from CIC rec: pcic 0x%08x, vcic %d, cic %d.\n",
                 cic_rec->pcic, cic_rec->vcic, ISUP_PCIC_TO_CIC(cic_rec->pcic)));
    ...
    {
        /* log error */
        ISUP_ERROR(("TimeoutT39 in WAIT_OUT_ACM state: Unable to remove T39 info "
                    "from CIC rec. Error %s. pcic 0x%08x, vcic %d, cic %d.\n",
                    ITS_GetErrorText(ret), cic_rec->pcic, cic_rec->vcic,
                    ISUP_PCIC_TO_CIC(cic_rec->pcic)));
    }
                           ...
                           ISUP_OPC_DPC_INFO *config_info)
{
    ...
    ISUP_ERROR(("Rcvd Unexpected Timeout T%d for CIC %d from MTP3 in "
                "WAIT_OUT_ACM. cic state 0x%02x, pcic 0x%08x, vcic %d.\n", 
 ...
 *      the WAIT_OUTGOING_ACM state, based on the ID of the timer that has 
 *      expired.
 *
 *  Input Parameters:
