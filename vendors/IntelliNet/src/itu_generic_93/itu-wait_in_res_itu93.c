 ...
 ****************************************************************************
 *
 *  ID: $Id: itu-wait_in_res_itu93.c,v 9.2 2005/04/06 07:17:24 ssingh Exp $
 *
 * LOG: $Log: itu-wait_in_res_itu93.c,v $
 ...
 * LOG: Begin PR6.5
 * LOG:
 * LOG: Revision 8.7  2005/03/21 13:54:09  cvsadmin
 ...
 * LOG:
 * LOG: Revision 7.2.4.2  2003/07/25 05:13:08  akumar
 ...
 * LOG: Begin PR6.2
 * LOG:
 * LOG: Revision 6.5  2002/08/01 15:57:49  ssharma
...
/*
 * Functions for handling messages in the Wait_Incoming_RES state for ITU93 
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an NRM received in the WAIT_INCOMING_RES state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        ISUP_DEBUG(("Received NRM in WAIT_IN_RES from MTP3, pcic 0x%08x, vcic %d.\n",
    ...
    {
        ISUP_DEBUG(("Received NRM in WAIT_IN_RES from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to L3 */
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received REL in WAIT_IN_RES from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Stop Timer T2, if User initiated SUS */
        __ISUP_StopTimer(cic_rec, ITS_ISUP_TIMER_T2);
        /* Stop Timer T6, if Network initiated SUS */
    ...
    {
        ISUP_Alarm_CCITT(3490, __FILE__, __LINE__, "pcic %x", cic_rec->pcic);

        /* Msg out of sequence. Drop it and generate alarm. */
        ISUP_ERROR(("WAIT_IN_RES REL: Unexpected REL from APP. pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
    }
}
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received RES in WAIT_IN_RES from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Stop T2 if User initiated SUS, or T6 if Network initiated SUS. */
        __ISUP_StopTimer(cic_rec, ITS_ISUP_TIMER_T2);
        __ISUP_StopTimer(cic_rec, ITS_ISUP_TIMER_T6);
    ...
    {
        ISUP_Alarm_CCITT(3491, __FILE__, __LINE__, "pcic %x", cic_rec->pcic);

        /* Msg out of sequence. Drop it and generate alarm. */
        ISUP_ERROR(("WAIT_IN_RES REL: Unexpected RES from APP. pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received RLC in WAIT_IN_RES from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Stop Timer T2, if User initiated SUS */
        __ISUP_StopTimer(cic_rec, ITS_ISUP_TIMER_T2);
        /* Stop Timer T6, if Network initiated SUS */
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UCIC received in the WAIT_IN_RES state is done
 *      here.
 *
 *  Input Parameters:
 *      msg - the ISUP msg received by the stack as an ITS_EVENT.
 ...
 ****************************************************************************
 *  Purpose:
 *      This function handles the processing at expiry of T2 timer in the 
 ...
 ****************************************************************************/
static void
__ISUP_Received_TimeoutT2(ISUP_TIMER_ENTRY* t_entry,  ISUP_CIC_INFO *cic_rec,
                           ISUP_OPC_DPC_INFO *config_info) 
{
    int ret = ITS_SUCCESS;

    ISUP_DEBUG(("TimeoutT2 event received in WAIT_IN_RES state. "
    ...
    ret = ISUP_DeletePndgTmrInfo(cic_rec, ITS_ISUP_TIMER_T2);
    if (ret != ITS_SUCCESS)
    {
        /* log error */
        ISUP_ERROR(("TimeoutT2 in WAIT_IN_RES state: Unable to remove T2 "
                    "info from CIC rec. Error %s. pcic 0x%08x, vcic %d,"
                    "cic %d.\n", ITS_GetErrorText(ret), cic_rec->pcic, 
                     cic_rec->vcic, ISUP_PCIC_TO_CIC(cic_rec->pcic)));
    }

    if (config_info->tmr_hndl_info[ITS_ISUP_TIMER_T2].tmr_hndl_layer 
        == ISUP_TMR_HNDL_STACK)
    {
        /* Do the thing. Phase II. */
        /* Send REL to L3 */
        SPRC_SendRELToMTP3_CCITT(CI_CAUSE_RECOVERY_ON_TIMER_EXPIRY,
                                 cic_rec,
                                 config_info);
    ...
    ISUP_Alarm_CCITT(3473, __FILE__, __LINE__, NULL);
                           ...
                           ISUP_OPC_DPC_INFO *config_info)  
{
    ...
    ISUP_Alarm_CCITT(3470, __FILE__, __LINE__, NULL);
...
__ISUP_GetTimeoutProc_WaitIncomingRes_ITU93(ITS_UINT timerID)
{
    ...
    case ITS_ISUP_TIMER_T2: /*  2    Suspend timer          */
        return (__ISUP_TimeoutHandlerProc)__ISUP_Received_TimeoutT2;
