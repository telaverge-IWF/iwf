 ...
 ****************************************************************************
 *
 *  ID: $Id: itu-wait_in_rlc_itu93.c,v 9.2 2005/04/06 07:17:24 ssingh Exp $
 *
 * LOG: $Log: itu-wait_in_rlc_itu93.c,v $
 ...
 * LOG: Begin PR6.5
 * LOG:
 * LOG: Revision 8.8  2005/03/21 13:54:09  cvsadmin
 * LOG: PR6.4.2 Source Propagated to Current
 * LOG:
 * LOG: Revision 7.3.2.4.6.1.12.2  2005/02/08 12:30:26  ssingh
 * LOG: Peg increament after receiving Unexpected Msg in given FSM state.
 * LOG: Bug# 1448. (Ramesh KV, Abhijit Dutta)
 * LOG:
 * LOG: Revision 7.3.2.4.6.1.12.1  2004/12/29 14:56:05  csireesh
 * LOG: ISUP OAM Changes
 * LOG:
 * LOG: Revision 7.3.2.4.6.1  2004/01/05 10:10:45  sjaddu
 * LOG: Merged code from tcs isup branch.
 * LOG:
 * LOG: Revision 7.3.2.4.2.2  2003/11/24 06:40:44  akumar
 * LOG: bug fix cycle.
 * LOG:
 * LOG: Revision 7.3.2.4.2.1  2003/09/05 09:46:52  akumar
 * LOG: Modifications for msg based redundancy scheme.
 * LOG:
 * LOG: Revision 7.3.2.4  2003/07/25 05:13:08  akumar
 * LOG: ITU National Matter is implemented.
 * LOG:
 * LOG: Revision 7.3.2.3  2003/06/13 03:32:35  akumar
 * LOG: Fixed Bug #373.
 * LOG:
 ...
 * LOG:
 * LOG: Revision 8.3  2003/04/30 05:20:30  akumar
 * LOG: Misc. Change.
 * LOG:
 ...
 * LOG: Begin PR6.4
 * LOG:
 * LOG: Revision 7.3  2003/01/22 08:44:24  akumar
 ...
 * LOG: Merge from PR6.
 * LOG:
 * LOG: Revision 1.6.2.11  2002/05/14 18:12:17  sjaddu
 * LOG: Added MTP3 Indications.
 * LOG:
 * LOG: Revision 1.6.2.10  2002/05/09 18:38:10  labuser
 * LOG: Merge from ISUP_APR_19_2002 branch to PR6.
 * LOG:
 * LOG: Revision 1.6.2.9.2.2  2002/05/02 22:30:27  labuser
 * LOG: Rel collision taken care of. Bug 44
 * LOG:
 * LOG: Revision 1.6.2.9.2.1  2002/04/27 17:53:43  labuser
 * LOG: crs state idled for incoming rlc
 * LOG:
 * LOG: Revision 1.6.2.9  2002/04/05 19:39:40  ssharma
 ...
 * LOG:
 * LOG: Revision 1.6.2.7  2002/04/01 21:08:03  ssharma
 ...
 * LOG:
 * LOG: Revision 1.6.2.5  2002/03/22 16:29:00  ssharma
...
/*
 * Functions for handling messages in the Wait_Incoming_RLC state for ITU93 
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an NRM received in the WAIT_INCOMING_RLC state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        ISUP_DEBUG(("Received NRM in WAIT_IN_RLC from MTP3, pcic 0x%08x, vcic %d.\n",
    ...
    {
        ISUP_DEBUG(("Received NRM in WAIT_IN_RLC from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to L3 */
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received REL in WAIT_IN_RLC from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));

       /*
        * Q.764 2.3.1.e Release collision
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received RLC in WAIT_IN_RLC from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));

        /* Stop Timer T1 and T5 if running. */
        __ISUP_StopRELTimers(cic_rec, config_info);

        /* Stop Timer T16 and T17, as this could be a response to RSC. */
    ...
    else 
    {
         ...
         */
        ISUP_DEBUG(("Received RLC in WAIT_IN_RLC from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
    ...
    /* if CRS in CRS_WAIT_FOR_RELEASE, IDLE it. */
    if(cic_rec->crs_state == CRS_WAIT_FOR_RELEASE)
    {
        cic_rec->crs_state = CRS_IDLE;
        ...
        ISUP_Alarm_CCITT(3415, __FILE__, __LINE__, "pCIC : %x", cic_rec->pcic);
    }
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an USR received in the WAIT_INCOMING_RLC state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received USR in WAIT_IN_RLC from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to app */
        SPRC_SendIsupToApp_CCITT(cic_rec->vcic, msg);
    ...
    {
        ISUP_DEBUG(("Received USR in WAIT_IN_RLC from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to L3 */
        SPRC_SendIsupToMTP3_CCITT(msg, cic_rec->vcic);
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UCIC received in the WAIT_IN_RLC state is done
 *      here.
 *
 *  Input Parameters:
 *      msg - the ISUP msg received by the stack as an ITS_EVENT.
                           ...
                           ISUP_OPC_DPC_INFO *config_info) 
{
        ...
        == ISUP_TMR_HNDL_STACK)
    {
        ...
        /* Resend REL to L3 */
        SPRC_ResendRELToMTP3_CCITT(cic_rec, config_info);
    ...
    ISUP_Alarm_CCITT(3474, __FILE__, __LINE__, NULL);
                           ...
                           ISUP_OPC_DPC_INFO *config_info)  
{
        ...
        == ISUP_TMR_HNDL_STACK)
    {
        ...
        /* Change the state to WAIT for RLC */
    ...
    ISUP_Alarm_CCITT(3471, __FILE__, __LINE__, NULL);
