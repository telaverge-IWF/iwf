 ...
 ****************************************************************************
 *
 *  ID: $Id: itu-sent_out_anm_itu93.c,v 9.2 2005/04/06 07:17:24 ssingh Exp $
 *
 * LOG: $Log: itu-sent_out_anm_itu93.c,v $
 ...
 * LOG: Begin PR6.5
 * LOG:
 * LOG: Revision 8.7  2005/03/21 13:54:07  cvsadmin
...
/*
 * Functions for handling messages in the Sent_Outgoing_ANM state for ITU93 
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received FOT in SENT_OUT_ANM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
    ...
    else 
    {
        ...
        SPRC_SendErrorToApp_CCITT(ISUP_MSG_FOT,
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an IDR received in the SENT_OUTGOING_ANM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received IDR in SENT_OUT_ANM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
    ...
    if (src != ISUP_SRC_L3_ITU) 
    {
        ISUP_DEBUG(("Received IDR in SENT_OUT_ANM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        ...
        if (ret != ITS_SUCCESS)
        {
            ISUP_ERROR(("SENT_OUT_ANM IDR: Failed to start T39. pcic 0x%08x, vcic %d.\n",
                        cic_rec->pcic, cic_rec->vcic));
        }
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    if (src == ISUP_SRC_L3_ITU)
    {
        ...
        ISUP_ERROR(("SENT_OUT_ANM INR: Unexpected INF from MTP3. pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
    ...
    if (src != ISUP_SRC_L3_ITU) 
    {
        ISUP_DEBUG(("Received INF in SENT_OUT_ANM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        /* Msg out of sequence. Drop it and generate alarm. */
        ISUP_ERROR(("SENT_OUT_ANM INR: Unexpected INR from APP. pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
    }
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ...
        {
            ISUP_DEBUG(("Received INR in SENT_OUT_ANM from MTP3,"
                        " pcic 0x%08x, vcic %d.\n",
                          cic_rec->pcic, cic_rec->vcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an IRS received in the SENT_OUTGOING_ANM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received IRS in SENT_OUT_ANM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
    ...
    if (src != ISUP_SRC_L3_ITU) 
    {
        ISUP_DEBUG(("Received IRS in SENT_OUT_ANM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an NRM received in the SENT_OUTGOING_ANM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received NRM in SENT_OUT_ANM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to app */
        SPRC_SendIsupToApp_CCITT(cic_rec->vcic, msg);
    ...
    {
        ISUP_DEBUG(("Received NRM in SENT_OUT_ANM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to L3 */
        SPRC_SendIsupToMTP3_CCITT(msg, cic_rec->vcic);
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an OLM received in the SENT_OUTGOING_ANM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received OLM in SENT_OUT_ANM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Start Timer T3 */
        ...
        if (ret != ITS_SUCCESS)
        {
            ISUP_ERROR(("SENT_OUT_ANM OLM: Failed to start T3. pcic 0x%08x, vcic %d.\n",
                        cic_rec->pcic, cic_rec->vcic));
        }
        else
        {
    ...
    {
        /* Msg out of sequence, drop it and generate alarm. */
        ISUP_ERROR(("SENT_OUT_ANM OLM: Unexpected OLM from APP. pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received REL in SENT_OUT_ANM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
    ...
    {
        ISUP_DEBUG(("Received REL in SENT_OUT_ANM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        ISUP_DEBUG(("Received RLC in SENT_OUT_ANM from APP, pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
        /* Send to L3 */
        SPRC_SendIsupToMTP3_CCITT(msg, cic_rec->vcic);
    }
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received SUS in SENT_OUT_ANM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
    ...
    {
        ISUP_DEBUG(("Received SUS in SENT_OUT_ANM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to L3 */
        SPRC_SendIsupToMTP3_CCITT(msg, cic_rec->vcic);
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an USR received in the SENT_OUTGOING_ANM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received USR in SENT_OUT_ANM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to app */
        SPRC_SendIsupToApp_CCITT(cic_rec->vcic, msg);
    ...
    {
        ISUP_DEBUG(("Received USR in SENT_OUT_ANM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to L3 */
        SPRC_SendIsupToMTP3_CCITT(msg, cic_rec->vcic);
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UCIC received in the SENT_OUT_ANM state is done
 *      here.
 *
 *  Input Parameters:
 *      msg - the ISUP msg received by the stack as an ITS_EVENT.
                           ...
                           ISUP_OPC_DPC_INFO *config_info) 
{
    ...
    ISUP_DEBUG(("TimeoutT3 event received in SENT_OUT_ANM state. "
                "Info from CIC rec: pcic 0x%08x, vcic %d, cic %d.\n",
                 cic_rec->pcic, cic_rec->vcic, ISUP_PCIC_TO_CIC(cic_rec->pcic)));
    
    /* Remove info about timer from cic_rec. Will not be restarted. */
    ret = ISUP_DeletePndgTmrInfo(cic_rec, ITS_ISUP_TIMER_T3);
    if (ret != ITS_SUCCESS)
    {
        /* log error */
        ISUP_ERROR(("TimeoutT3 in SENT_OUT_ANM state: Unable to remove T3 "
                           ...
                           ISUP_OPC_DPC_INFO *config_info)  
{
    ...
    ISUP_DEBUG(("TimeoutT39 event received in SENT_OUT_ANM state. "
                "Info from CIC rec: pcic 0x%08x, vcic %d, cic %d.\n",
                 cic_rec->pcic, cic_rec->vcic, ISUP_PCIC_TO_CIC(cic_rec->pcic)));
    ...
    {
        /* log error */
        ISUP_ERROR(("TimeoutT39 in SENT_OUT_ANM state: Unable to remove T39 info "
                    "from CIC rec. Error %s. pcic 0x%08x, vcic %d, cic %d.\n",
                    ITS_GetErrorText(ret), cic_rec->pcic, cic_rec->vcic,
                    ISUP_PCIC_TO_CIC(cic_rec->pcic)));
    }
