 ...
 ****************************************************************************
 *
 *  ID: $Id: itu-wait_out_rlc_itu93.c,v 9.2 2005/04/06 07:17:23 ssingh Exp $
 *
 * LOG: $Log: itu-wait_out_rlc_itu93.c,v $
 ...
 * LOG: Merge from PR6.
 * LOG:
 * LOG: Revision 1.5.2.11  2002/05/14 18:12:17  sjaddu
 * LOG: Added MTP3 Indications.
 * LOG:
 * LOG: Revision 1.5.2.10  2002/05/09 18:38:10  labuser
 * LOG: Merge from ISUP_APR_19_2002 branch to PR6.
 * LOG:
 * LOG: Revision 1.5.2.9.2.1  2002/04/27 01:03:34  labuser
 * LOG: change of crr_state state to idle for outgoing RLC
 * LOG:
...
/*
 * Functions for handling messages in the Wait_Outgoing_RLC state for ITU93.
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an NRM received in the WAIT_OUTGOING_RLC state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        ISUP_DEBUG(("Received NRM in WAIT_OUT_RLC from MTP3, pcic 0x%08x, vcic %d.\n",
    ...
    {
        ISUP_DEBUG(("Received NRM in WAIT_OUT_RLC from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to L3 */
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received REL in WAIT_OUT_RLC from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to app */
        SPRC_SendIsupToApp_CCITT(cic_rec->vcic, msg);
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
         ...
         */
        ISUP_DEBUG(("Received RLC in WAIT_OUT_RLC from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
    ...
    {
        ISUP_DEBUG(("Received RLC in WAIT_OUT_RLC from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to L3 */
        SPRC_SendIsupToMTP3_CCITT(msg, cic_rec->vcic);
    }
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an USR received in the WAIT_OUTGOING_RLC state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ITU)
    {
        ISUP_DEBUG(("Received USR in WAIT_OUT_RLC from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to app */
        SPRC_SendIsupToApp_CCITT(cic_rec->vcic, msg);
    ...
    {
        ISUP_DEBUG(("Received USR in WAIT_OUT_RLC from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
        /* Send to L3 */
        SPRC_SendIsupToMTP3_CCITT(msg, cic_rec->vcic);
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UCIC received in the WAIT_OUT_RLC state is done
 *      here.
 *
 *  Input Parameters:
 *      msg - the ISUP msg received by the stack as an ITS_EVENT.
