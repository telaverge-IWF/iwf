 ...
 ****************************************************************************
 *
 *  ID: $Id: ansi.c,v 9.1 2005/03/23 12:54:37 cvsadmin Exp $
 *
 * LOG: $Log: ansi.c,v $
 * LOG: Revision 9.1  2005/03/23 12:54:37  cvsadmin
 * LOG: Begin PR6.5
 * LOG:
 * LOG: Revision 8.5  2005/03/21 13:53:41  cvsadmin
 * LOG: PR6.4.2 Source Propagated to Current
 * LOG:
 * LOG: Revision 7.3.2.1.6.1.6.1  2005/01/18 13:05:24  mmanikandan
 * LOG: NULL is added for TTC send.
 * LOG:
 * LOG: Revision 7.3.2.1.6.1  2004/09/15 05:46:43  mmanikandan
 * LOG: XML Persistency propagation.
 * LOG:
 * LOG: Revision 7.3.2.1  2003/05/07 09:33:33  ssingh
 * LOG: UnReserveCIC related changes done.
 * LOG:
 * LOG: Revision 7.3  2003/02/05 17:06:40  sjaddu
 ...
 * LOG:
 * LOG: Revision 6.5  2002/07/17 19:20:27  mmiers
 * LOG: Add missing NULLs
 * LOG:
 * LOG: Revision 6.4  2002/07/01 21:51:10  mmiers
 * LOG: Vframe changes to support ISUP.  Warning removal
 * LOG:
 * LOG: Revision 6.3.2.1  2002/06/28 23:39:10  hbalimid
 ...
 * LOG: Begin PR7.
 * LOG:
 * LOG: Revision 5.5  2002/02/13 21:19:15  mmiers
 * LOG: Finish up the ISUP integration.
 * LOG:
 * LOG: Revision 5.4  2002/02/01 21:57:12  mmiers
...
#ident "$Id: ansi.c,v 9.1 2005/03/23 12:54:37 cvsadmin Exp $"
 ...
 * layer functions
 */
extern int IMTP3_Initialize_ANSI(ITS_SS7_HANDLE);
extern int IISUP_Initialize_ANSI(ITS_SS7_HANDLE);
extern int ISCCP_Initialize_ANSI(ITS_SS7_HANDLE);
extern int ITCAP_Initialize_ANSI(ITS_SS7_HANDLE);
extern int ITCAP_Initialize_CCITT(ITS_SS7_HANDLE);
extern int IMTP3_Terminate_ANSI(ITS_SS7_HANDLE);
extern int IISUP_Terminate_ANSI(ITS_SS7_HANDLE);
extern int ISCCP_Terminate_ANSI(ITS_SS7_HANDLE);
extern int ITCAP_Terminate_ANSI(ITS_SS7_HANDLE);
extern int ITCAP_Terminate_CCITT(ITS_SS7_HANDLE);

extern int INTELLINET_SendMTP3Event_ANSI(ITS_SS7_HANDLE handl,
                                         ITS_OCTET type,
                                         MTP3_HEADER_ANSI *mtp3,
                                         ITS_OCTET *buf, ITS_USHORT len);
extern int INTELLINET_ReserveCIC_ANSI(ITS_SS7_HANDLE handle,
                                       ITS_OCTET sio,
                                       ITS_UINT opc, ITS_UINT dpc,
                                       ITS_USHORT cic,
                                       ITS_OCTET *status, ITS_CTXT *vcic);

extern int INTELLINET_UnReserveCIC_ANSI(ITS_SS7_HANDLE handle,
                                      ITS_OCTET sio,
                                      ITS_UINT opc, ITS_UINT dpc,
                                      ITS_USHORT cic);

extern int INTELLINET_SendISUPEvent_ANSI(ITS_HANDLE handle, ITS_OCTET type,
                                         ITS_HDR *hdr,
                                         ITS_ISUP_IE *ies, int ieCount,
                                         ISUP_MSG_DESC *desc);
extern int INTELLINET_ReceiveISUPEvent_ANSI(ITS_HANDLE handle, ITS_EVENT *ev,
                                            ITS_HDR *hdr,
                                            ITS_ISUP_IE *ies, int *ieCount,
                                            ISUP_MSG_DESC *desc);
extern int INTELLINET_SendSCCPEvent_ANSI(ITS_HANDLE handle, ITS_OCTET type,
                                         ITS_HDR *hdr,
...
extern int INTELLINET_SendUIS_ANSI(ITS_HANDLE handle, ITS_OCTET ni,
                                   ITS_UINT pc, ITS_OCTET ssn);
extern int INTELLINET_SetCongestion_ANSI(ITS_HANDLE handle, ITS_OCTET ni,
                                         ITS_UINT pc, ITS_OCTET ssn,
                                         ITS_OCTET cong);
 ...
 * Commit methods
 */
extern int INTELLINET_MTP3_Commit_ANSI(FILE* fp);
extern int INTELLINET_SCCP_Commit_ANSI(FILE* fp);
extern int INTELLINET_TCAP_Commit_ANSI(FILE* fp);
extern int INTELLINET_TCAP_Commit_CCITT(FILE* fp);
extern int INTELLINET_ISUP_Commit_ANSI(FILE* fp, int pos,
...
ClassInit(ITS_Class objClass)
{
     ...
     */
    ITS_CLASS_SUPERCLASS(itsINTELLISS7_ClassANSI) = itsSS7STACK_ClassANSI;

    /*
     * create the ANSI stack instance
     */
    ...
    SS7TRAN_SUBORDINATE(ISS7_ANSI_Stack) =
        (SS7STACK_ClassRec *)itsINTELLISS7_ClassANSI;
    ...
    SS7TRAN_HANDLE(ISS7_ANSI_Stack) =
        ITS_ConstructObject(itsINTELLISS7_ClassANSI, ISS7_ANSI_Stack);
...
/*
 * the ANSI stack class record
 */
ITSSS7DLLAPI VENDOR_ClassRec itsINTELLISS7_ClassRecANSI =
{
    /* core part */
    {
        ...
        0,                              /* initial ref count */
        INTELLISS7_CLASS_NAME "ANSI",   /* class name */
    ...
    /* vendor part */
    {
        ...
        /* MTP3 */
        {
            IMTP3_Initialize_ANSI,          /* initialize */
            IMTP3_Terminate_ANSI,           /* terminate */
            NULL,                           /* no retrieve */
            INTELLINET_SendMTP3Event_ANSI,  /* send ANSI */
            NULL,                           /* no send CCITT */
            NULL,                           /* no send PRC */
            NULL,
            INTELLINET_MTP3_Commit_ANSI     /* commit */
        },
        /* ISUP */
        {
            IISUP_Initialize_ANSI,          /* initialize */
            IISUP_Terminate_ANSI,           /* terminate */
            INTELLINET_ReserveCIC_ANSI,    /* getCIC */
            INTELLINET_UnReserveCIC_ANSI,    /* unreserveCIC */
            NULL,                           /* no retrieve */
            INTELLINET_ReceiveISUPEvent_ANSI,/* receive ANSI */
            NULL,                           /* no CCITT receive */
            INTELLINET_SendISUPEvent_ANSI,  /* send event */
            NULL,                           /* no CCITT send */
            INTELLINET_ISUP_Commit_ANSI     /* commit */
        },
        /* SCCP */
        {
            ISCCP_Initialize_ANSI,          /* initialize */
            ISCCP_Terminate_ANSI,           /* terminate */
            INTELLINET_GetSCCPNextRef_ANSI, /* getRef */
            NULL,                           /* no retrieve */
            INTELLINET_SendSCCPEvent_ANSI,  /* send */
            INTELLINET_SendUOS_ANSI,        /* UOS */
            INTELLINET_SendUIS_ANSI,        /* UIS */
            INTELLINET_SetCongestion_ANSI,  /* setCong */
            INTELLINET_SCCP_Commit_ANSI     /* commit */
        },
        /* TCAP */
        {
...
ITSSS7DLLAPI ITS_Class itsINTELLISS7_ClassANSI =
    (ITS_Class)&itsINTELLISS7_ClassRecANSI;
