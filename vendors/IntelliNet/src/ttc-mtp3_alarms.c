 ...
 ****************************************************************************
 *
 *  ID:$Id: ttc-mtp3_alarms.c,v 9.1 2007/01/11 12:43:40 yranade Exp $
 *
 * LOG: $Log: ttc-mtp3_alarms.c,v $
 ...
 ****************************************************************************/
void
MTP3_Alarm_TTC(int which, const char *file, const int line,
                char *format, ...)
{
    va_list alist;
    ALARM_Format *fmt;

    ITS_C_ASSERT(which >= TTC_MTP3_ALARM_START &&
                 which <= TTC_MTP3_ALARM_END);

    fmt = TTC_MTP3_Alarms[which - TTC_MTP3_ALARM_START];

    if (fmt->severity > MTP3_AlarmLevel_TTC)
    {
        return;
    }
