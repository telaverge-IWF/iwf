 ...
 ****************************************************************************
 *
 *  ID: $Id: ansi-wait_in_cot_ansi95.c,v 9.2 2005/04/06 06:39:10 ssingh Exp $
 *
 * LOG: $Log: ansi-wait_in_cot_ansi95.c,v $
 ...
 * LOG:
 * LOG: Revision 1.1.2.1.6.1.12.2  2005/02/08 12:19:16  ssingh
 * LOG: Peg increament after receiving Unexpected Msg in given FSM state.
 * LOG: Bug# 1448.
 * LOG: (Ramesh KV, Abhijit Dutta)
 * LOG:
 * LOG: Revision 1.1.2.1.6.1.12.1  2004/12/29 14:41:07  csireesh
 * LOG: ISUP OAM Changes
 * LOG:
 * LOG: Revision 1.1.2.1.6.1  2004/01/05 10:10:45  sjaddu
 * LOG: Merged code from tcs isup branch.
 * LOG:
 * LOG: Revision 1.1.2.1.2.1  2003/09/13 06:36:51  akumar
 * LOG: Msg based redundancy scheme works for ANSI.
 * LOG:
 * LOG: Revision 1.1.2.1  2003/05/07 10:49:27  ssingh
 ...
 * LOG:
 * LOG: Revision 8.2  2003/04/16 06:53:21  akumar
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an FAC received in the WAIT_IN_COT state
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        ISUP_DEBUG(("Received FAC in WAIT_IN_COT from MTP3,pcic 0x%08x,"
    ...
    {
        ISUP_DEBUG(("Received FAC in WAIT_IN_COT from APP, pcic 0x%08x,"
                     "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
 ...
 *      the WAIT_IN_COT state. It also serves as a default handler for
                              ...
                              ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_ERROR(("Rcvd Unexpected msg 0x%02x from MTP3 in WAIT_IN_COT. "
        ...
        ISUP_Alarm_ANSI(3172, __FILE__, __LINE__, "dpc: %d opc: %d "
                        "cic: %x  Message: %x",
    ...
    {
        ISUP_ERROR(("Rcvd Unexpected msg 0x%02x from App in WAIT_IN_COT. "
                    "cic state 0x%02x, pcic 0x%08x, vcic %d.\n",
                    msg->data[sizeof(MTP3_HEADER_ANSI) + sizeof (ITS_USHORT) +
                    sizeof(ITS_OCTET)],
                    cic_rec->cic_state, cic_rec->pcic, cic_rec->vcic));

        ISUP_Alarm_ANSI(3180, __FILE__, __LINE__, "Message:%x cic : %x",
                        msg->data[sizeof(MTP3_HEADER_ANSI) + 
                        sizeof(ITS_USHORT) + sizeof(ITS_OCTET)],
                        ISUP_PCIC_TO_CIC(cic_rec->pcic));
...
__ISUP_GetMsgProc_WaitIncomingCot_ANSI95(ITS_OCTET msgType)
{
    switch(msgType)
    {
