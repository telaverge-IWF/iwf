 ...
 ****************************************************************************
 *
 *  ID: $Id: ansi-wait_out_anm_ansi95.c,v 9.2 2005/04/06 06:39:10 ssingh Exp $
 *
 * LOG: $Log: ansi-wait_out_anm_ansi95.c,v $
 ...
 * LOG: Begin PR6.5
 * LOG:
 * LOG: Revision 8.5  2005/03/21 13:54:03  cvsadmin
 * LOG: PR6.4.2 Source Propagated to Current
 * LOG:
 * LOG: Revision 7.2.4.1.6.1.12.1  2005/02/08 12:19:16  ssingh
 * LOG: Peg increament after receiving Unexpected Msg in given FSM state.
 * LOG: Bug# 1448.
 * LOG: (Ramesh KV, Abhijit Dutta)
 * LOG:
 * LOG: Revision 7.2.4.1.6.1  2004/01/05 10:10:45  sjaddu
 * LOG: Merged code from tcs isup branch.
 * LOG:
 * LOG: Revision 7.2.4.1.2.1  2003/09/13 06:38:36  akumar
 * LOG: Msg based redundancy works for ANSI.
 * LOG:
 * LOG: Revision 7.2.4.1  2003/05/07 10:49:27  ssingh
 * LOG: Latest changes propped from Current to 6.3
 * LOG:
 * LOG: Revision 8.3  2003/04/28 15:09:07  sjaddu
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an FAC received in the WAIT_OUT_ANM state
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        ISUP_DEBUG(("Received FAC in WAIT_OUT_ANM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));

        /* Send to app */
        SPRC_SendIsupToApp_ANSI(cic_rec->vcic, msg);

        /* Change state to WAIT_OUT_ANM */
    }
    else
    {
        ISUP_DEBUG(("Received FAC in WAIT_OUT_ANM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
 ...
 *      the WAIT_OUT_ANM state. It also serves as a default handler for
 ...
 *      for the WAIT_OUT_ANM state, based on the msgType received.
 *
 *  Input Parameters:
...
__ISUP_GetMsgProc_WaitOutgoingAnm_ANSI95(ITS_OCTET msgType)
{
    switch(msgType)
    {
    case ISUP_MSG_ACM:
