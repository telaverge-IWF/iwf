 ...
 ****************************************************************************
 *
 *  ID: $Id: ansi-sent_out_anm_ansi95.c,v 9.2 2005/04/06 06:39:10 ssingh Exp $
 *
 * LOG: $Log: ansi-sent_out_anm_ansi95.c,v $
 ...
 * LOG:
 * LOG: Revision 7.5.2.1.2.1  2003/09/13 06:35:40  akumar
 ...
 * LOG: Begin PR6.4
 * LOG:
 * LOG: Revision 7.5  2002/12/06 05:44:46  akumar
 * LOG: Removed unwanted TABS
 * LOG:
 * LOG: Revision 7.4  2002/11/20 12:57:03  akumar
 ...
 * LOG:
 * LOG: Revision 1.1.2.1  2002/07/03 23:00:57  hbalimid
 * LOG: ANSI95 specific.
 * LOG:
 *****************************************************************************/
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an FAC received in the SEND_OUT_ANM state
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        ISUP_DEBUG(("Received FAC in SEND_OUT_ANM from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));

        /* Send to app */
        SPRC_SendIsupToApp_ANSI(cic_rec->vcic, msg);

        /* Change state to SEND_OUT_ANM */
    }
    else
    {
        ISUP_DEBUG(("Received FAC in SEND_OUT_ANM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
 ...
 *      the SEND_OUT_ANM state. It also serves as a default handler for
                              ...
                              ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_ERROR(("Rcvd Unexpected msg 0x%02x from MTP3 in SEND_OUT_ANM. "
        ...
        ISUP_Alarm_ANSI(3143, __FILE__, __LINE__, "dpc: %d opc: %d "
                        "cic: %x  Message: %x",
    ...
    {
        ISUP_ERROR(("Rcvd Unexpected msg 0x%02x from App in SEND_OUT_ANM. "
                    "cic state 0x%02x, pcic 0x%08x, vcic %d.\n",
                    msg->data[sizeof(MTP3_HEADER_ANSI) + sizeof (ITS_USHORT) +
                    sizeof(ITS_OCTET)],
                    cic_rec->cic_state, cic_rec->pcic, cic_rec->vcic));

        ISUP_Alarm_ANSI(3144, __FILE__, __LINE__, "Message: %x cic : %x",
                         msg->data[sizeof(MTP3_HEADER_ANSI) + 
                         sizeof(ITS_USHORT) + sizeof(ITS_OCTET)] ,
                         ISUP_PCIC_TO_CIC(cic_rec->pcic));
 ...
 *      for the SEND_OUT_ANM state, based on the msgType received.
 *
 *  Input Parameters:
...
__ISUP_GetMsgProc_SentOutgoingAnm_ANSI95(ITS_OCTET msgType)
{
    switch(msgType)
    {
    case ISUP_MSG_ACM:
