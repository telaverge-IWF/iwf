 ...
 *                                                                          *
 ****************************************************************************
 *  ID: $Id: ansi-wait_in_cra_ansi95.c,v 9.2 2005/04/06 06:39:10 ssingh Exp $
 * LOG: $Log: ansi-wait_in_cra_ansi95.c,v $
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an FAC received in the WAIT_INCOMING_CRA state
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        ISUP_DEBUG(("Received FAC in WAIT_INCOMING_CRA from MTP3,"
                     "pcic 0x%08x, vcic %d .\n",
    ...
    {
        ISUP_DEBUG(("Received FAC in WAIT_INCOMING_CRA from APP,"
                     "pcic 0x%08x, vcic %d. \n",
                     cic_rec->pcic, cic_rec->vcic));
                              ...
                              ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_ERROR(("Rcvd Unexpected msg 0x%x from MTP3 in WAIT_INCOMING_CRA."
    ...
    {
        ISUP_ERROR(("Rcvd Unexpected msg 0x%02x from App in WAIT_INCOMING_CRA."
                    "cic state 0x%02x, pcic 0x%08x, vcic %d.\n",
                    msg->data[sizeof(MTP3_HEADER_ANSI) + sizeof (ITS_USHORT) +
                    sizeof(ITS_OCTET)],
                    cic_rec->cic_state, cic_rec->pcic, cic_rec->vcic));
...
__ISUP_GetMsgProc_WaitIncomingCra_ANSI95(ITS_OCTET msgType)
{
    switch(msgType)
    {
