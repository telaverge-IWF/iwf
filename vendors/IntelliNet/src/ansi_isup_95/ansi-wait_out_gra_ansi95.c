 ...
 ****************************************************************************
 *
 *  ID: $Id: ansi-wait_out_gra_ansi95.c,v 9.2 2005/04/06 06:39:10 ssingh Exp $
 *
 * LOG: $Log: ansi-wait_out_gra_ansi95.c,v $
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an FAC received in the WAIT_OUT_GRA state
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        ISUP_DEBUG(("Received FAC in WAIT_OUT_GRA from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));

        /* Send to app */
        SPRC_SendIsupToApp_ANSI(cic_rec->vcic, msg);

        /* Change state to WAIT_OUT_GRA */
    }
    else
    {
        ISUP_DEBUG(("Received FAC in WAIT_OUT_GRA from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
 ...
 *      the WAIT_OUT_GRA state. It also serves as a default handler for
                              ...
                              ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ...
        ISUP_Alarm_ANSI(3157, __FILE__, __LINE__, "dpc: %d opc: %d "
                        "cic: %x Message: %x",
    ...
    else
    {
        ...
        ISUP_Alarm_ANSI(3158, __FILE__, __LINE__, "Message %x cic : %x",
                         msg->data[sizeof(MTP3_HEADER_ANSI) + 
                         sizeof(ITS_USHORT) + sizeof(ITS_OCTET)],
                         ISUP_PCIC_TO_CIC(cic_rec->pcic));
 ...
 *      for the WAIT_OUT_GRA state, based on the msgType received.
 *
 *  Input Parameters:
...
__ISUP_GetMsgProc_WaitOutgoingGra_ANSI95(ITS_OCTET msgType)
{
    switch(msgType)
    {
    case ISUP_MSG_ACM:
