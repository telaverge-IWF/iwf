 ...
 ****************************************************************************
 *
 *  ID: $Id: ansi-wait_cot_report_ansi95.c,v 9.2 2005/04/06 06:39:10 ssingh Exp $
 *
 * LOG: $Log: ansi-wait_cot_report_ansi95.c,v $
 ...
 * LOG: Begin PR6.5
 * LOG:
 * LOG: Revision 8.7  2005/03/21 13:54:03  cvsadmin
 * LOG: PR6.4.2 Source Propagated to Current
 * LOG:
 * LOG: Revision 1.2.2.1.6.1.12.2  2005/02/08 12:19:16  ssingh
 * LOG: Peg increament after receiving Unexpected Msg in given FSM state.
 * LOG: Bug# 1448.
 * LOG: (Ramesh KV, Abhijit Dutta)
 * LOG:
 * LOG: Revision 1.2.2.1.6.1.12.1  2004/12/29 14:41:07  csireesh
 * LOG: ISUP OAM Changes
 * LOG:
 * LOG: Revision 1.2.2.1.6.1  2004/01/05 10:10:45  sjaddu
 * LOG: Merged code from tcs isup branch.
 * LOG:
 * LOG: Revision 1.2.2.1.2.1  2003/09/13 06:35:40  akumar
 * LOG: Msg based redundancy works for ANSI.
 * LOG:
 * LOG: Revision 1.2.2.1  2003/05/07 10:49:27  ssingh
 ...
 * LOG:
 * LOG: Revision 8.2  2003/04/16 06:51:58  akumar
 * LOG: ITS_EVENT_TERM Added, to avoid mem leak, ISUP_ERROR() modified for print.
 * LOG:
 * LOG: Revision 8.1  2003/02/13 17:44:28  yranade
 * LOG: Begin PR6.4
 * LOG:
 * LOG: Revision 1.2  2002/12/05 12:04:32  sjaddu
 * LOG: Fixed funny bug gcc couldn't cath it.
 * LOG:
 * LOG: Revision 1.1  2002/11/28 06:15:42  sjaddu
 * LOG: Handling COT states in 95.
 * LOG:
 *****************************************************************************/
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an FAC received in the WAIT_IN_COT_REPORT state
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        ISUP_DEBUG(("Received FAC in WAIT_IN_COT_REPORT from MTP3,"
                     "pcic 0x%08x, vcic %d .\n",
    ...
    {
        ISUP_DEBUG(("Received FAC in WAIT_IN_COT_REPORT from APP,"
                     "pcic 0x%08x, vcic %d. \n",
                     cic_rec->pcic, cic_rec->vcic));
 ...
 *      the WAIT_IN_COT_REPORT state. It also serves as a default handler for
                              ...
                              ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_ERROR(("Rcvd Unexpected msg 0x%x from MTP3 in WAIT_IN_COT_REPORT."
        ...
        ISUP_Alarm_ANSI(3155, __FILE__, __LINE__, "dpc: %d opc: %d "
                        "cic: %x  Message: %x",
    ...
    {
        ISUP_ERROR(("Rcvd Unexpected msg 0x%02x from App in WAIT_IN_COT_REPORT. "
                    "cic state 0x%02x, pcic 0x%08x, vcic %d.\n",
                    msg->data[sizeof(MTP3_HEADER_ANSI) + sizeof (ITS_USHORT) +
                    sizeof(ITS_OCTET)],
                    cic_rec->cic_state, cic_rec->pcic, cic_rec->vcic));
        ...
        ISUP_Alarm_ANSI(3156, __FILE__, __LINE__, "Message: %x cic : %x",
                         msg->data[sizeof(MTP3_HEADER_ANSI) + 
                         sizeof(ITS_USHORT) + sizeof(ITS_OCTET)],
                         ISUP_PCIC_TO_CIC(cic_rec->pcic));
 ...
 *      for the WAIT_INCOMING_COT_REPORT state, based on the msgType received.
 *
 *  Input Parameters:
...
__ISUP_GetMsgProc_WaitCotReport_ANSI95(ITS_OCTET msgType)
{
    switch(msgType)
    {
