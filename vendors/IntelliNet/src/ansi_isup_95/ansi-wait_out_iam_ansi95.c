 ...
 *                                                                          *
 ****************************************************************************
 *  ID: $Id: ansi-wait_out_iam_ansi95.c,v 9.2 2005/04/06 06:39:10 ssingh Exp $
 * LOG: $Log: ansi-wait_out_iam_ansi95.c,v $
 ...
 * LOG:
 * LOG: Revision 1.5.2.1.2.1  2003/09/13 06:38:36  akumar
 ...
 * LOG:
 * LOG: Revision 1.3  2003/04/16 06:54:10  akumar
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an FAC received in the WAIT_OUTGOING_IAM state
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        ISUP_DEBUG(("Received FAC in WAIT_OUTGOING_IAM from MTP3,"
                     "pcic 0x%08x, vcic %d .\n",
    ...
    {
        ISUP_DEBUG(("Received FAC in WAIT_OUTGOING_IAM from APP,"
                     "pcic 0x%08x, vcic %d. \n",
                     cic_rec->pcic, cic_rec->vcic));
 ...
 *      the WAIT_OUTGOING_IAM state. It also serves as a default handler for
                              ...
                              ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_ERROR(("Rcvd Unexpected msg 0x%x from MTP3 in WAIT_OUTGOING_IAM."
    ...
    {
        ISUP_ERROR(("Rcvd Unexpected msg 0x%02x from App in WAIT_OUTGOING_IAM."
                    "cic state 0x%02x, pcic 0x%08x, vcic %d.\n",
                    msg->data[sizeof(MTP3_HEADER_ANSI) + sizeof (ITS_USHORT) +
                    sizeof(ITS_OCTET)],
                    cic_rec->cic_state, cic_rec->pcic, cic_rec->vcic));
 ...
 *      for the WAIT_OUTGOING_IAM state, based on the msgType received.
 *
 *  Input Parameters:
...
__ISUP_GetMsgProc_WaitOutgoingIam_ANSI95(ITS_OCTET msgType)
{
    switch(msgType)
    {
