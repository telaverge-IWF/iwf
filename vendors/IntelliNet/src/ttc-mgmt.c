 ...
 ****************************************************************************
 *
 *  ID: $Id: ttc-mgmt.c,v 9.7 2008/06/17 06:38:02 nvijikumar Exp $
 *
 * LOG: $Log: ttc-mgmt.c,v $
 ...
 * LOG:
 * LOG: Revision 9.4  2008/05/01 13:42:58  kramesh
 ...
 * LOG:
 * LOG: Revision 9.1.10.15  2008/04/10 12:20:33  ssodhi
 * LOG: Fixed warnings
 * LOG:
 * LOG: Revision 9.1.10.14  2008/01/14 10:20:25  ssodhi
 * LOG: Code changes for Accelero Performance Enhancement - ALU BSG Project
 * LOG:
 * LOG: Revision 9.1.10.13.6.2  2008/01/04 07:52:41  ssodhi
 * LOG: Revision 9.1.10.13.6.1  2007/12/26 15:30:37  ssodhi
 * LOG: Revision 9.1.10.13  2007/10/09 16:19:35  mshanmugam
 * LOG: Fix for Issue id 127, mt_get_all_lpc and mt_get_all_rpc
 * LOG:
 * LOG: Revision 9.1.10.12  2007/10/05 11:14:46  mshanmugam
 * LOG: CPOT Stack support as per design Id D0060
 * LOG:
 * LOG: Revision 9.1.10.11  2007/05/09 12:26:26  mshanmugam
 * LOG: Fix for 5731 - skatta
 * LOG:
 * LOG: Revision 9.1.10.10  2007/04/26 08:59:10  raghavendrabk
 * LOG: Resolved issue with Linkset Peg collection
 * LOG:
 * LOG: Revision 9.1.10.9  2007/04/11 07:49:15  mshanmugam
 * LOG: Family check is added for link pegs
 * LOG:
 * LOG: Revision 9.1.10.8  2007/03/21 12:15:24  sdwivedi
 * LOG: Fix for Parallel Stack Issue #5230
 * LOG:
 * LOG: Revision 9.1.10.7  2007/02/26 10:20:09  raghavendrabk
 * LOG: Family structure filled in OM API
 * LOG:
 * LOG: Revision 9.1.10.6  2007/02/07 12:00:32  gharipriya
 * LOG: Fix for Parallel Stack Issues 5223,5231, 5295,5296 raised on MMLs
 * LOG:
 * LOG: Revision 9.1.10.5  2007/02/02 14:17:02  raghavendrabk
 * LOG: Parallel Stack changes for Alarms and Statistics
 * LOG:
 * LOG: Revision 9.1.10.4  2007/01/23 13:35:10  ssingh
 * LOG: #5197 fix; separate mml for ttc
 * LOG:
 * LOG: Revision 9.1.10.3  2006/12/23 10:05:36  randresol
 * LOG: Modifications to allow TTC and ITU stacks to run in parallel
 * LOG:
 * LOG: Revision 9.1.10.2  2006/11/08 15:36:12  mshanmugam
 * LOG: Fix for Bug id:4401  SRT,SRA message pegs implementation (skatta)
 * LOG:
 * LOG: Revision 9.1.10.1  2006/11/07 13:46:41  gharipriya
 * LOG: Fix for Bug Id 4385. Link Inhibition and Uninhibition procedures deleted
 * LOG: for TTC.
 * LOG:
 * LOG: Revision 9.1  2005/03/23 12:54:48  cvsadmin
 * LOG: Begin PR6.5
 * LOG:
 * LOG: Revision 8.1  2005/03/23 09:41:22  cvsadmin
 * LOG: PR6.4.2 sources propagated to Current.
 * LOG:
 * LOG: Revision 1.1.2.1  2005/02/18 11:26:51  dsatish
...
#include <dbc_serv.h>
#ident "$Id: ttc-mgmt.c,v 9.7 2008/06/17 06:38:02 nvijikumar Exp $"
...
void MTP3_ResetOm_TTC();
void MTP3_GetOm_TTC(mtp3Om *);
#if defined(__cplusplus)
}
#endif /* defined(__cplusplus) */

void MTP3_ResetOm_MML_TTC();
void MTP3_GetOm_MML_TTC(DBC_Server *);
...
static ITS_CHAR *mtp3LPegStr[] =
{
      ...
      "PEG_MTP3_LINK_SRT_TX",
      "PEG_MTP3_LINK_SRT_RX",
      "PEG_MTP3_LINK_SRA_TX",
      "PEG_MTP3_LINK_SRA_RX",
       NULL
};
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
MGMT_Main_TTC(MTP3_SubSystems src, MGMT_Triggers trigger,
                ITS_OCTET *sif, ITS_USHORT len,
                ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    int ret;

    MTP3_TRACE_ENTRY_TTC(MTP3_MGMT, src, trigger, sif, len,
                          linkSet, linkCode);
 ...
 *                                                mt_shutdown  MML command
 ****************************************************************************/
void
MTP3_Console_TTC(DBC_Server *dbc, const char *cmdLine)
{    
    char buf[ITS_PATH_MAX];
    ...
    if (DBC_Cmd(dbc, "mt_get_gen_cfg", "MTP3 Get General Config", "", ""))
    {
        MTP3GeneralCfg *cfg = 0;
        cfg =  MTP3_GetGeneralCfg_TTC();
        sprintf(buf, "\n*********** MTP3 General Config *************\n");
        DBC_AppendText(dbc, buf);
    ...
    else if (DBC_Cmd(dbc, "mt_set_gen_cfg", "MTP3 Set General Config",
                     "<alarm level> <Trace Type> <traceOutput> <On/Off>",""))
    {
        ...
        if (ret != 5)
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n"
                         "Usage: mt_set_gen_cfg <alarmlevel> <tracetype>" 
                         " <trOutput> <On/Off>\n"
        ...
        ret =  MTP3_SetGeneralCfg_TTC(&cfg);
        if (ret != ITS_SUCCESS)
        {
            return;
        }
    ...
    else if (DBC_Cmd(dbc, "mt_deact_ls", "Deactivate Link Set", "<linkset>", ""))
    {
        ret = sscanf(cmdLine, "%s %d", buf, &linkSet);
        if (ret != 2)
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n" 
                         "Usage: mt_deact_ls <linkset> \n");
            DBC_AppendText(dbc, buf);
            return;
        }
    ...
    else if (DBC_Cmd(dbc, "mt_act_ls", "Activate Link Set", "<linkset>", ""))
    {
        ret = sscanf(cmdLine, "%s %d", buf, &linkSet);
        if (ret != 2)
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n" 
                         "Usage: mt_act_ls <linkset> \n");
            DBC_AppendText(dbc, buf);
            return;
        }
        ...
        /* While activating linkSet give default linkCode as done in deactivation of linkset */
        ret= MGMT_Main_TTC(MTP3_USER,
    ...
    else if (DBC_Cmd(dbc, "mt_deact_link", "Deactivate Link", 
             "<linkset> <linkcode>", ""))
    {
        ...
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n" 
                         "Usage: mt_deact_link <linkset> <linkcode> \n");
            DBC_AppendText(dbc, buf);
            return;
        }
    ...
    else if (DBC_Cmd(dbc, "mt_act_link", "Activate Link", 
             "<linkset> <linkcode>", ""))
    {
        ret = sscanf(cmdLine, "%s %d %d", buf, &linkSet, &linkCode);
        if (ret != 3)
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n" 
                         "Usage: mt_act_link <linkset> <linkcode> \n");
            DBC_AppendText(dbc, buf);
            return;
        }
    ...
    else if (DBC_Cmd(dbc, "mt_start", "Start MTP3", "", ""))
    {
        sprintf(buf, "\n*********** Starting MTP3 **************\n");
        DBC_AppendText(dbc, buf);
    ...
    else if (DBC_Cmd(dbc, "mt_get_ls", "Dump Link Sets", "", ""))
    {
        char *dump = NULL;
        SS7_Family family = FAMILY_JAPAN;
    ...
    else if (DBC_Cmd(dbc, "mt_get_ls_s", "Dump Link Sets (simple)", "", ""))
    {
        char *dump = NULL;
        SS7_Family family = FAMILY_JAPAN;
        ...
        LINKSET_DumpLinkSetHashtable_simple(&dump, family, ITS_FALSE);
...
#if 0
    else if (DBC_Cmd(dbc, "mt_get_rt", "Dump Routes Info", "<>", "<>"))
    {
        char *dump = NULL;
...
#endif
    else if (DBC_Cmd(dbc, "mt_get_link", "get Link Status", 
             "<linkset> <linkcode>", ""))
    {
        char *dump = NULL;
        ...
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n"
                         "Usage: mt_get_link <linkset> <linkcode>\n");
            DBC_AppendText(dbc, buf);
            return;
        }
    ...
    else if (DBC_Cmd(dbc, "mt_get_gen_peg", "Get MTP3 General Pegs", 
             "<Peg Id>", ""))
    {
        ...
        {
             sprintf(buf,  "\n**** Invalid Command Arguments *******\n"
                           "Usage: mt_get_gen_peg [<Peg Id> or <All/all>]\n"); 
             DBC_AppendText(dbc, buf);
             return;
        }
        ...
        {
            /* Invoke function MTP3_GetOm_MML_TTC() */ 
            MTP3_GetOm_MML_TTC(dbc);
            DumpGlobalPegs(dbc);
            return;
        }
        else 
        ...
        if (pgNum >= PEG_MTP3_NUM_PEGS_TTC)
        {
            sprintf(buf, "\n   **** Invalid Peg Id *******\n");
            DBC_AppendText(dbc, buf);
            return;
        ...
        value = PEG_GetPeg(TTC_MTP3_Pegs, pgNum);

        sprintf(buf, "\n   value is %d\n", value);
        DBC_AppendText(dbc, buf);
    }
    else if (DBC_Cmd(dbc, "mt_get_link_peg", "Get Link Level Pegs", 
             "<linkset> <linkcode> <Peg Id>", ""))
    {
        ...
        {
            sprintf(buf,  "\n**** Invalid Command Arguments *******\n"
                          "Usage: mt_get_link_peg <linkset> <linkcode> [<Peg Id> or<All/all>]\n");
            DBC_AppendText(dbc, buf);
            return;
        }
...
#if 0
    else if (DBC_Cmd(dbc, "mt_get_gen_pegid", "Show all general peg IDs", "", ""))
    {
        DumpGlobalPegId(dbc);
    }
    else if (DBC_Cmd(dbc, "mt_get_link_pegid", "Show all link level peg IDs", "", ""))
    {
        DumpLinkPegId(dbc);
    }
#endif

    else if (DBC_Cmd(dbc, "mt_clr_gen_peg", "MTP3 Reset a particular gen Peg",
                          "[<Pegid> (or) <All/all>]",""))
    {
        ...
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n"
                         "Usage: mt_clr_gen_peg [<Pegid> (or) <All/all>]\n");
            DBC_AppendText(dbc, buf);
            return;
        }
        ...
        {
            /* Invoke function MTP3_ResetOm_MML_TTC() */
            MTP3_ResetOm_MML_TTC();

            for (i = 0; i < PEG_MTP3_NUM_PEGS_TTC; i++)
            {
                PEG_ClearPeg(TTC_MTP3_Pegs, i);
            }
        ...
        if (pgNum > PEG_MTP3_NUM_PEGS_TTC - 1)
        {
        ...
        PEG_ClearPeg(TTC_MTP3_Pegs, pgNum);

        sprintf(buf, "\n** Cleared MTP3 general peg id %d **\n", pgNum);
        DBC_AppendText(dbc, buf);
    }
    else if (DBC_Cmd(dbc, "mt_clr_link_peg", "MTP3 Clear particular Link Peg",
                          "<linkset> <linkcode> [<Pegid> (or) <All/all>]",""))
    {
        ...
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n"
            "Usage: mt_clr_link_peg <linkset> <linkcode> [<Pegid> (or) <All/all>]\n");
            DBC_AppendText(dbc, buf);
            return;
        }
        ...
        if (pgNum >= PEG_MTP3_NUM_LINK_PEGS)
        {
         ...
         sprintf(buf, "\n** Cleared peg id %d for linkset %d linkcode %d **\n", pgNum, linkSet, linkCode); 
                                                
         DBC_AppendText(dbc, buf);
    }
#if 0
    else if (DBC_Cmd(dbc, "mt_debug_trace", "Debug Trace", "<on, off>", "<>"))
    {
        char onOff[10];
    ...
    else if (DBC_Cmd(dbc, "mt_warning_trace", "Warning Trace", "<on, off>", "<>"
))
    {
        char onOff[10];
    ...
    else if (DBC_Cmd(dbc, "mt_error_trace", "Error Trace", "<on, off>", "<>"))
    {
        char onOff[10];
...
#endif
    else if (DBC_Cmd(dbc, "mt_shutdown", "shutdown mtp3", "", ""))
    {
    ...
    else if (DBC_Cmd(dbc, "mt_set_rpc", "set a remote point code",
               "<rpc><ni><ls><f>", "<remote pc><ni><link set><family>"))
    {
       ...
       if (ret != 5)
       {
           sprintf(buf,"*** Error: Invalid parameters, try help mt_set_rpc \n");
           DBC_AppendText(dbc, buf);
           return;
       }
       /*** process family ***/
...
;
    ...
    else if (DBC_Cmd(dbc, "mt_rem_rpc","Remove a remote point code",
         "<rpc><ni><LinkSet><family>", "<remote pc><ni><linkset><family>"))
    {
        ...
        if (ret != 5)
        {
           sprintf(buf, "*** Error: Invalid parameters, try help mt_rem_rpc \n"
);
           DBC_AppendText(dbc, buf);
           return;
        }
...
;
    ...
    else if (DBC_Cmd(dbc, "mt_get_all_rpc", "get all remote PC", "<>", "<>"))
    {
        char *dump = NULL;
        char** pStrDumpa;
        char* strDumpEntries = NULL;
    ...
    else if (DBC_Cmd(dbc, "mt_get_all_lpc", "get all local PC", "<>", "<>"))
    {
        char *dump = NULL;
        char* strDumpEntries = NULL;
...
{    
    ...
    if (DBC_Cmd(dbc, "mt_dls", "Deactivate Link Set", "<ls>", "<link set>"))
    {
        ret = sscanf(cmdLine, "%s %d", buf, &linkSet);
        if (ret != 2)
        {
    ...
    else if (DBC_Cmd(dbc, "mt_als", "Activate Link Set", "<ls>", "<link set>"))
    {
        ret = sscanf(cmdLine, "%s %d", buf, &linkSet);
        if (ret != 2)
        {
    ...
    else if (DBC_Cmd(dbc, "mt_dln", "Deactivate Link", "<ls> <lc>", "<link set> <link code>"))
    {
        ret = sscanf(cmdLine, "%s %d %d", buf, &linkSet, &linkCode);
        if (ret != 3)
        {
    ...
    else if (DBC_Cmd(dbc, "mt_aln", "Activate Link", "<ls> <lc>", "<link set> <link code>"))
    {
        ret = sscanf(cmdLine, "%s %d %d", buf, &linkSet, &linkCode);
        if (ret != 3)
        {
    ...
    else if (DBC_Cmd(dbc, "mt_dumpls", "Dump Link Sets", "<>", "<>"))
    {
        char *dump = NULL;
        SS7_Family family = FAMILY_JAPAN;
    ...
    else if (DBC_Cmd(dbc, "mt_dumprt", "Dump Routes Info", "<>", "<>"))
    {
        char *dump = NULL;
    ...
    else if (DBC_Cmd(dbc, "mt_link_status", "get Link Status", "<ls>", "<lc>"))
    {
        char *dump = NULL;
    ...
    else if (DBC_Cmd(dbc, "mt_get_peg", "Get Pegs", "<pg> ", "<Peg#> <>"))
    {
        ...
        if (pg >= PEG_MTP3_NUM_PEGS_TTC || pg < 0)
        {
            sprintf(buf, "range for peg is %d - %d", 0,PEG_MTP3_NUM_PEGS_TTC-1);
            DBC_AppendText(dbc, buf);
            return ;
        }
        ...
        value = PEG_GetPeg(TTC_MTP3_Pegs, pg);

        sprintf(buf, "value is %d\n", value);
        DBC_AppendText(dbc, buf);
    }
    else if (DBC_Cmd(dbc, "mt_get_link_peg", "Get Link Level Pegs", "<ls> <lc> <p g>", "<link set> <link code><Peg#>"))
    {
    ...
    else if (DBC_Cmd(dbc, "mt_show_peg_ids", "Dump Peg Ids", "<>", "<>"))
    {
        DumpGlobalPegId(dbc);
        DumpLinkPegId(dbc);

    }
    else if (DBC_Cmd(dbc, "mt_clr_gen_pegs", "MTP3 Reset a particular gen Peg",
                          "[<Pegid> (or) <All/all>]",""))
    {
        char pegType[ITS_PATH_MAX];
        ...
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n"
                         "Usage: mt_clr_gen_pegs [<Pegid> (or) <All/all>]\n");
            DBC_AppendText(dbc, buf);
            return;
        }
        ...
        if ( atoi(pegType) > PEG_MTP3_NUM_PEGS_TTC - 1)
        {
             sprintf(buf, "\n** MTP3 general peg %d does not exist **\n",
                                                       atoi(pegType));
             DBC_AppendText(dbc, buf);
    ...
    else if (DBC_Cmd(dbc, "mt_clr_link_pegs", "MTP3 Clear particular Link Peg",
                          "<linkset> <linkcode> [<Pegid> (or) <All/all>]",""))
    {
        char pegType[ITS_PATH_MAX];
        ret = sscanf(cmdLine, "%s %d %d %s", buf, &linkSet, &linkCode,pegType);
        ...
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n"
            "Usage: mt_clr_link_pegs <linkset> <linkcode> [<Pegid> (or) <All/all>]\n");
            DBC_AppendText(dbc, buf);
            return;
        }
...
DumpRemotePointCodeData(ITS_POINTER data, void *in, void *out)
{
    ...
    for (i = 0; i < ss7Context->numEntries; i++)
    {
       if ( ss7Context->entries[i]->basic.type == REMOTE_ROUTE &&
             ss7Context->entries[i]->basic.family == FAMILY_JAPAN )
       {
           strcat(*pStrDump, "------------------------------\n");
...
DumpGlobalPegs(DBC_Server *dbc)
{
     ...
     sprintf(buf, "\n%12s:              value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_MSG_PAUSE),
              PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_MSG_PAUSE));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_MSG_RESUME),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_MSG_RESUME));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_MSG_STATUS),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_MSG_STATUS));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_INITIALIZE),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_INITIALIZE));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:              value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_TERMINATE),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_TERMINATE));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:   value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_MSG_RECEIVED_INERROR),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_MSG_RECEIVED_INERROR));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_CHANGEOVER),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_CHANGEOVER));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_CHANGEBACK),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_CHANGEBACK));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:         value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_SL_UNAVAILABLE),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_SL_UNAVAILABLE));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:           value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_LINK_INHIBIT),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_LINK_INHIBIT));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:         value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_LINK_UNINHIBIT),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_LINK_UNINHIBIT));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:   value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_LINK_FORCE_UNINHIBIT),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_LINK_FORCE_UNINHIBIT));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_CONGESTION),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_CONGESTION));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:        value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_SLS_UNAVAILABLE),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_SLS_UNAVAILABLE));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:           value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_TFC_RECEIVED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_TFC_RECEIVED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:           value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_TFA_RECEIVED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_TFA_RECEIVED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:   value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_ROUTESET_UNAVAILABLE),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_ROUTESET_UNAVAILABLE));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_ADJECENT_SP_UNACCESIBLE),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_ADJECENT_SP_UNACCESIBLE));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:         value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_USERPART_ENABLE),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_USERPART_ENABLE));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:        value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_USERPART_DISABLE),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_USERPART_DISABLE));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:              value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T1_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T1_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:              value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T2_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T2_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:              value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T3_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T3_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:              value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T4_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T4_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:              value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T5_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T5_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:              value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T6_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T6_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:              value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T7_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T7_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:              value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T8_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T8_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:              value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T9_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T9_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T10_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T10_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T11_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T11_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T12_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T12_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T13_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T13_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T14_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T14_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T15_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T15_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T16_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T16_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T17_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T17_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T18_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T18_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T19_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T19_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T20_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T20_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T21_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T21_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T22_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T22_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T23_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T23_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T24_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T24_EXPIRED));
     DBC_AppendText(dbc, buf);

#if 0
     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T25_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T25_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T26_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T26_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T27_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T27_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T28_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T28_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T29_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T29_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T30_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T30_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T31_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T31_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T32_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T32_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T33_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T33_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T34_EXPIRED),
             PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T34_EXPIRED));
     DBC_AppendText(dbc, buf);
#endif
#endif
     sprintf(buf, "\n*******************************************\n");
...
DumpGlobalPegId(DBC_Server *dbc)
{
     ...
     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_MSG_PAUSE),
             PEG_MTP3_MSG_PAUSE, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_MSG_PAUSE));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_MSG_RESUME),
             PEG_MTP3_MSG_RESUME, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_MSG_RESUME));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_MSG_STATUS),
             PEG_MTP3_MSG_STATUS, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_MSG_STATUS));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_INITIALIZE),
             PEG_MTP3_INITIALIZE, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_INITIALIZE));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_TERMINATE),
             PEG_MTP3_TERMINATE, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_TERMINATE));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_MSG_RECEIVED_INERROR),
             PEG_MTP3_MSG_RECEIVED_INERROR, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_MSG_RECEIVED_INERROR));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_CHANGEOVER),
             PEG_MTP3_CHANGEOVER, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_CHANGEOVER));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_CHANGEBACK),
             PEG_MTP3_CHANGEBACK, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_CHANGEBACK));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_SL_UNAVAILABLE),
             PEG_MTP3_SL_UNAVAILABLE, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_SL_UNAVAILABLE));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_LINK_INHIBIT),
             PEG_MTP3_LINK_INHIBIT, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_LINK_INHIBIT));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_LINK_UNINHIBIT),
             PEG_MTP3_LINK_UNINHIBIT, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_LINK_UNINHIBIT));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_LINK_FORCE_UNINHIBIT),
             PEG_MTP3_LINK_FORCE_UNINHIBIT, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_LINK_FORCE_UNINHIBIT));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_CONGESTION),
             PEG_MTP3_CONGESTION, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_CONGESTION));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_SLS_UNAVAILABLE),
             PEG_MTP3_SLS_UNAVAILABLE, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_SLS_UNAVAILABLE));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_TFC_RECEIVED),
             PEG_MTP3_TFC_RECEIVED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_TFC_RECEIVED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_TFA_RECEIVED),
             PEG_MTP3_TFA_RECEIVED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_TFA_RECEIVED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_ROUTESET_UNAVAILABLE),
             PEG_MTP3_ROUTESET_UNAVAILABLE, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_ROUTESET_UNAVAILABLE));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_ADJECENT_SP_UNACCESIBLE),
             PEG_MTP3_ADJECENT_SP_UNACCESIBLE, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_ADJECENT_SP_UNACCESIBLE));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_USERPART_ENABLE),
             PEG_MTP3_USERPART_ENABLE, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_USERPART_ENABLE));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_USERPART_DISABLE),
             PEG_MTP3_USERPART_DISABLE, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_USERPART_DISABLE));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T1_EXPIRED),
             PEG_MTP3_T1_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T1_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T2_EXPIRED),
             PEG_MTP3_T2_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T2_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T3_EXPIRED),
             PEG_MTP3_T3_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T3_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T4_EXPIRED),
             PEG_MTP3_T4_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T4_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T5_EXPIRED),
             PEG_MTP3_T5_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T5_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T6_EXPIRED),
             PEG_MTP3_T6_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T6_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T7_EXPIRED),
             PEG_MTP3_T7_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T7_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T8_EXPIRED),
             PEG_MTP3_T8_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T8_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T9_EXPIRED),
             PEG_MTP3_T9_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T9_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T10_EXPIRED),
             PEG_MTP3_T10_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T10_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T11_EXPIRED),
             PEG_MTP3_T11_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T11_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T12_EXPIRED),
             PEG_MTP3_T12_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T12_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T13_EXPIRED),
             PEG_MTP3_T13_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T13_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T14_EXPIRED),
             PEG_MTP3_T14_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T14_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T15_EXPIRED),
             PEG_MTP3_T15_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T15_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T16_EXPIRED),
             PEG_MTP3_T16_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T16_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T17_EXPIRED),
             PEG_MTP3_T17_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T17_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T18_EXPIRED),
             PEG_MTP3_T18_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T18_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T19_EXPIRED),
             PEG_MTP3_T19_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T19_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T20_EXPIRED),
             PEG_MTP3_T20_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T20_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T21_EXPIRED),
             PEG_MTP3_T21_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T21_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T22_EXPIRED),
             PEG_MTP3_T22_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T22_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T23_EXPIRED),
             PEG_MTP3_T23_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T23_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T24_EXPIRED),
             PEG_MTP3_T24_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T24_EXPIRED));
     strcat(buf1, buf);

#if 0
     /* Commented as per spec */
     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T25_EXPIRED),
             PEG_MTP3_T25_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T25_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T26_EXPIRED),
             PEG_MTP3_T26_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T26_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T27_EXPIRED),
             PEG_MTP3_T27_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T27_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T28_EXPIRED),
             PEG_MTP3_T28_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T28_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T29_EXPIRED),
             PEG_MTP3_T29_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T29_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T30_EXPIRED),
             PEG_MTP3_T30_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T30_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T31_EXPIRED),
             PEG_MTP3_T31_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T31_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T32_EXPIRED),
             PEG_MTP3_T32_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T32_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T33_EXPIRED),
             PEG_MTP3_T33_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T33_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T34_EXPIRED),
             PEG_MTP3_T34_EXPIRED, PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_T34_EXPIRED));
     strcat(buf1, buf);
#endif
#endif
...
/****************************************************************************
 *  Purpose:
 *      To print the values of Link pegs 
 *
 *  Input Parameters:
 *      None
 *
...
DumpLinkPegs(SS7_LinkPtr ln, DBC_Server *dbc)
{
 ...
 /* Print the pegs and their assciated Id's */
     sprintf(buf, "\n%12s:              value %d\n", LINK_PEGID_TO_TEXT(PEG_MTP3_LINK_MSU_TX),
             PEG_GetPeg(&ln->MTP3_LINK_PEGS, PEG_MTP3_LINK_MSU_TX));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "\n%12s:             value %d\n", LINK_PEGID_TO_TEXT( PEG_MTP3_LINK_MSU_RX),
             PEG_GetPeg(&ln->MTP3_LINK_PEGS,  PEG_MTP3_LINK_MSU_RX ));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "\n%12s:             value %d\n", LINK_PEGID_TO_TEXT(PEG_MTP3_LINK_OCTETS_TX),
             PEG_GetPeg(&ln->MTP3_LINK_PEGS, PEG_MTP3_LINK_OCTETS_TX));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "\n%12s:             value %d\n", LINK_PEGID_TO_TEXT(PEG_MTP3_LINK_OCTETS_RX),
             PEG_GetPeg(&ln->MTP3_LINK_PEGS, PEG_MTP3_LINK_OCTETS_RX));
     DBC_AppendText(dbc, buf);
     ...
     sprintf(buf, "\n%12s:             value %d\n", LINK_PEGID_TO_TEXT(PEG_MTP3_LINK_SRT_TX),
             PEG_GetPeg(&ln->MTP3_LINK_PEGS, PEG_MTP3_LINK_SRT_TX));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "\n%12s:             value %d\n", LINK_PEGID_TO_TEXT(PEG_MTP3_LINK_SRT_RX),
             PEG_GetPeg(&ln->MTP3_LINK_PEGS, PEG_MTP3_LINK_SRT_RX));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "\n%12s:             value %d\n", LINK_PEGID_TO_TEXT(PEG_MTP3_LINK_SRA_TX),
             PEG_GetPeg(&ln->MTP3_LINK_PEGS, PEG_MTP3_LINK_SRA_TX));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "\n%12s:             value %d\n", LINK_PEGID_TO_TEXT(PEG_MTP3_LINK_SRA_RX),
             PEG_GetPeg(&ln->MTP3_LINK_PEGS, PEG_MTP3_LINK_SRA_RX));
     DBC_AppendText(dbc, buf);
...
DumpLinkPegId(DBC_Server *dbc)
{
     ...
     sprintf(buf, "PEG_MTP3_LINK_SRT_TX                   %3d\n",PEG_MTP3_LINK_SRT_TX);
     strcat(buf1, buf);

     sprintf(buf, "PEG_MTP3_LINK_SRT_RX                   %3d\n",PEG_MTP3_LINK_SRT_RX);
     strcat(buf1, buf);

     sprintf(buf, "PEG_MTP3_LINK_SRA_TX                   %3d\n",PEG_MTP3_LINK_SRA_TX);
     strcat(buf1, buf);

     sprintf(buf, "PEG_MTP3_LINK_SRA_RX                   %3d\n",PEG_MTP3_LINK_SRA_RX);
     strcat(buf1, buf);
...
MTP3_OSS_GetGeneralCfg_TTC(MTP3_GeneralCfg* genCfg)
{
    if (genCfg == NULL)
        return ITS_ENOMEM;

    genCfg->alarmLevel = (MGMT_AlarmLevel)MTP3_AlarmLevel_TTC;

    genCfg->debugTrace = TRACE_IsLevelOn(MTP3_TTC_TraceData, MTP3_TRACE_DEBUG); 
    if (genCfg->debugTrace)
    {
        genCfg->debugTraceOutput = 0;
        if (TRACE_IsOutputOn(MTP3_TTC_TraceData, MTP3_TRACE_DEBUG, 0))
        {
            genCfg->debugTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(MTP3_TTC_TraceData, MTP3_TRACE_DEBUG, 1))
        {
            genCfg->debugTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(MTP3_TTC_TraceData, MTP3_TRACE_DEBUG, 2))
        {
    ...
    genCfg->warningTrace = TRACE_IsLevelOn(MTP3_TTC_TraceData, MTP3_TRACE_WARNING); 
    if (genCfg->warningTrace)
    {
        genCfg->warningTraceOutput = 0;
        if (TRACE_IsOutputOn(MTP3_TTC_TraceData, MTP3_TRACE_WARNING, 0))
        {
            genCfg->warningTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(MTP3_TTC_TraceData, MTP3_TRACE_WARNING, 1))
        {
            genCfg->warningTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(MTP3_TTC_TraceData, MTP3_TRACE_WARNING, 2))
        {
    ...
    genCfg->errorTrace = TRACE_IsLevelOn(MTP3_TTC_TraceData, MTP3_TRACE_ERROR); 
    if (genCfg->errorTrace)
    {
        genCfg->errorTraceOutput = 0;
        if (TRACE_IsOutputOn(MTP3_TTC_TraceData, MTP3_TRACE_ERROR, 0))
        {
            genCfg->errorTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(MTP3_TTC_TraceData, MTP3_TRACE_ERROR, 1))
        {
            genCfg->errorTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(MTP3_TTC_TraceData, MTP3_TRACE_ERROR, 2))
        {
    ...
    genCfg->criticalTrace = TRACE_IsLevelOn(MTP3_TTC_TraceData, MTP3_TRACE_ABORT); 
    if (genCfg->criticalTrace)
    {
        genCfg->criticalTraceOutput = 0;
        if (TRACE_IsOutputOn(MTP3_TTC_TraceData, MTP3_TRACE_ABORT, 0))
        {
            genCfg->criticalTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(MTP3_TTC_TraceData, MTP3_TRACE_ABORT, 1))
        {
            genCfg->criticalTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(MTP3_TTC_TraceData, MTP3_TRACE_ABORT, 2))
        {
    ...
    genCfg->eventTrace = TRACE_IsLevelOn(MTP3_TTC_TraceData, MTP3_TRACE_EVENT); 
    if (genCfg->eventTrace)
    {
        genCfg->eventTraceOutput = 0;
        if (TRACE_IsOutputOn(MTP3_TTC_TraceData, MTP3_TRACE_EVENT, 0))
        {
            genCfg->eventTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(MTP3_TTC_TraceData, MTP3_TRACE_EVENT, 1))
        {
            genCfg->eventTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(MTP3_TTC_TraceData, MTP3_TRACE_EVENT, 2))
        {
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_SetGeneralCfg_TTC(MTP3_GeneralCfg* genCfg)
{
    if (genCfg == NULL)
        return ITS_ENOMEM;

    MTP3_AlarmLevel_TTC = genCfg->alarmLevel;

    if (genCfg->debugTrace)
    {
        TRACE_SetLevelOnOffByName(MTP3_TTC_TraceData, MTP3_DEBUG_STRING,  ITS_TRUE); 
        ...
        if (genCfg->debugTraceOutput & TRACE_TO_NONE)
        {
            ...
            TRACE_SetLevelOnOffByName(MTP3_TTC_TraceData, MTP3_DEBUG_STRING,  ITS_FALSE); 
        }
    }

    if (genCfg->warningTrace)
    {
        TRACE_SetLevelOnOffByName(MTP3_TTC_TraceData, MTP3_WARNING_STRING,  ITS_TRUE); 
        ...
        if (genCfg->warningTraceOutput & TRACE_TO_NONE)
        {
            ...
            // Set trace level to false
            TRACE_SetLevelOnOffByName(MTP3_TTC_TraceData, MTP3_WARNING_STRING,  ITS_FALSE); 
        }
    }

    if (genCfg->errorTrace)
    {
        TRACE_SetLevelOnOffByName(MTP3_TTC_TraceData, MTP3_ERROR_STRING,  ITS_TRUE); 
        ...
        if (genCfg->errorTraceOutput & TRACE_TO_NONE)
        {
            ...
            // Set trace level to false
            TRACE_SetLevelOnOffByName(MTP3_TTC_TraceData, MTP3_ERROR_STRING,  ITS_FALSE); 
        }
    }

    if (genCfg->criticalTrace)
    {
        TRACE_SetLevelOnOffByName(MTP3_TTC_TraceData, MTP3_CRITICAL_STRING,  ITS_TRUE); 
        ...
        if (genCfg->criticalTraceOutput & TRACE_TO_NONE)
        {
            ...
            // Set trace level to false
            TRACE_SetLevelOnOffByName(MTP3_TTC_TraceData, MTP3_CRITICAL_STRING,  ITS_FALSE); 
        }
    }

    if (genCfg->eventTrace)
    {
        TRACE_SetLevelOnOffByName(MTP3_TTC_TraceData, MTP3_EVENT_STRING,  ITS_TRUE); 
        ...
        if (genCfg->eventTraceOutput & TRACE_TO_NONE)
        {
            ...
            // Set trace level to false
            TRACE_SetLevelOnOffByName(MTP3_TTC_TraceData, MTP3_EVENT_STRING,  ITS_FALSE); 
        }
    }
 ...
 ****************************************************************************/
ITSSS7DLLAPI void
MTP3_OSS_SetBehavior_TTC(ITS_BOOLEAN isStp, ITS_BOOLEAN handleSpareNic)
{
    BEHAVIORS_SetBehavior(&TTC_MTP3_Behaviors, MTP3_B_stpFunction, isStp);
    MTP3_TTC_HandleSpareNIC = handleSpareNic;    
}
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_GetBehavior_TTC(ITS_BOOLEAN* isStp, ITS_BOOLEAN* handleSpareNic)
{
    if (isStp == NULL || handleSpareNic == NULL)
        return ITS_ENOMEM;

    *isStp = BEHAVIORS_GetBehavior(&TTC_MTP3_Behaviors, MTP3_B_stpFunction);
    *handleSpareNic = MTP3_TTC_HandleSpareNIC; 
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_InhibitLink_TTC(ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    SS7_LinkPtr ln;
    ITS_INT     ret = ITS_SUCCESS; 
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_UninhibitLink_TTC(ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    SS7_LinkPtr ln;
    ITS_INT     ret = ITS_SUCCESS; 
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_DeactivateLink_TTC(ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    SS7_LinkPtr ln;
    ITS_INT     ret = ITS_SUCCESS; 
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_ActivateLink_TTC(ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    SS7_LinkPtr ln;
    ITS_INT     ret = ITS_SUCCESS; 
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_DeactivateLinkset_TTC (ITS_OCTET linkSet)
{
    SS7_LinkSetPtr  ls;
    ITS_INT         ret = ITS_SUCCESS; 
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_ActivateLinkset_TTC (ITS_OCTET linkSet)
{
    SS7_LinkSetPtr  ls;
    ITS_INT         ret = ITS_SUCCESS; 
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_GetLinkset_TTC(ITS_OCTET linkSet, SS7_LinkSetInfo* linkSetInfoPtr, ITS_OCTET* numLinksets)
{
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_GetLink_TTC(ITS_OCTET linkSet, ITS_OCTET linkCode, SS7_LinkInfo* linkInfoPtr)
{
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT
MTP3_OSS_GetGeneralPegs_TTC(MTP3_GenPegs* globalPegs)
{
    if (globalPegs == NULL)
        return ITS_ENOMEM;

    globalPegs->msgPause             = PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_MSG_PAUSE);
    globalPegs->msgResume            = PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_MSG_RESUME);
    globalPegs->msgStatus            = PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_MSG_STATUS);
    globalPegs->initialize           = PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_INITIALIZE);
    globalPegs->terminate            = PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_TERMINATE);
    globalPegs->msgReceivedInError   = PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_MSG_RECEIVED_INERROR);
    globalPegs->changeover           = PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_CHANGEOVER);
    globalPegs->changeback           = PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_CHANGEBACK);
    globalPegs->slUnavailable        = PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_SL_UNAVAILABLE);
    globalPegs->congestion           = PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_CONGESTION);
    globalPegs->slsUnavailable       = PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_SLS_UNAVAILABLE);
    globalPegs->tfcReceived          = PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_TFC_RECEIVED);
    globalPegs->tfaReceived          = PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_TFA_RECEIVED);
    globalPegs->routesetUnavailable  = PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_ROUTESET_UNAVAILABLE);
    globalPegs->adjacentSpUnaccesible= PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_ADJECENT_SP_UNACCESIBLE);
    globalPegs->userpartEnable       = PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_USERPART_ENABLE);
    globalPegs->userpartDisable      = PEG_GetPeg(TTC_MTP3_Pegs, PEG_MTP3_USERPART_DISABLE);
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_GetLinkPegs_TTC(ITS_OCTET linkSet, ITS_OCTET linkCode, MTP3_LinkPegs* linkPegs)
{
    if (linkPegs == NULL)
        return ITS_ENOMEM;
    ...
    linkPegs->sltmTx    = PEG_GetPeg(&ln->MTP3_LINK_PEGS, PEG_MTP3_LINK_SRT_TX);
    linkPegs->sltmRx    = PEG_GetPeg(&ln->MTP3_LINK_PEGS, PEG_MTP3_LINK_SRT_RX);
    linkPegs->sltaTx    = PEG_GetPeg(&ln->MTP3_LINK_PEGS, PEG_MTP3_LINK_SRA_TX);
    linkPegs->sltaRx    = PEG_GetPeg(&ln->MTP3_LINK_PEGS, PEG_MTP3_LINK_SRA_RX);
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_ClearGeneralPeg_TTC(ITS_UINT pegId)
{
    if (pegId == MTP3_ALL_PEGS)
    {
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_INITIALIZE);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_TERMINATE);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_RESTART);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_MSG_PAUSE); 
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_MSG_RESUME);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_MSG_STATUS);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_LINK_OUT_OF_SERVICE);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_LINK_IN_SERVICE);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_CHANGEOVER);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_EMER_CHANGEOVER);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_CHANGEBACK);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_LINK_BLOCK);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_LINK_UNBLOCK);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_LINK_ACTIVATION);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_LINK_DEACTIVATION);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_CONGESTION_DETECT);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_MSU_TX);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_MSU_RX);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_SRT_TX);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_SRT_RX);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_SRA_TX);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_SRA_RX);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_SLT_PASSED);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_SLT_FAILED);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_TFP_TX);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_TFP_RX);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_TFA_TX);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_TFA_RX);
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_REMOTE_ROUTE_FAILURE );
        PEG_ClearPeg(TTC_MTP3_Pegs, PEG_MTP3_USERPART_ROUTE_FAILURE );
    }
    else
    {
        if (pegId > PEG_MTP3_NUM_PEGS_TTC)
        {
        ...
        {
            PEG_ClearPeg(TTC_MTP3_Pegs, pegId);
        }
    }
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_ClearLinkPeg_TTC(ITS_OCTET linkSet, ITS_OCTET linkCode, ITS_UINT pegId)
{
    SS7_LinkPtr ln = NULL;
    ...
    if (pegId == MTP3_ALL_PEGS)
    {
        ...
        PEG_ClearPeg(&ln->MTP3_LINK_PEGS, PEG_MTP3_LINK_SRT_TX);
        PEG_ClearPeg(&ln->MTP3_LINK_PEGS, PEG_MTP3_LINK_SRT_RX);
        PEG_ClearPeg(&ln->MTP3_LINK_PEGS, PEG_MTP3_LINK_SRA_TX);
        PEG_ClearPeg(&ln->MTP3_LINK_PEGS, PEG_MTP3_LINK_SRA_RX);
    }
    else
    {
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT
MTP3_OSS_AddLinkset_TTC(ITS_OCTET linkSet, SS7_Family family, ITS_UINT lpc, ITS_UINT apc, ITS_UINT ni)
{
    ITS_INT         ret;
    SS7_LinkSetPtr  ls;
    SS7_LinkSet      linkset;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_RemoveLinkset_TTC(ITS_OCTET linkSet)
{
    ITS_INT        ret;
    SS7_LinkSetPtr  ls;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_AddLink_TTC(ITS_OCTET linkSet, ITS_OCTET linkCode, ITS_UINT transId)
{
    ITS_INT             ret;
    SS7_LinkPtr         lp;
    SS7_LinkSetPtr      ls;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_RemoveLink_TTC(ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ITS_INT         ret;
    SS7_LinkPtr     lp;
    SS7_LinkSetPtr  ls;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_AddRemotePC_TTC(ITS_UINT rpc, ITS_OCTET ni, ITS_OCTET linkSet, SS7_Family family)
{
    ITS_INT         ret;
    SS7_LinkSetPtr  ls;
    SS7_LinkPtr     lp;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_RemoveRemotePC_TTC(ITS_UINT rpc, ITS_OCTET ni, ITS_OCTET linkSet, SS7_Family family)
{
    ITS_INT             ret;
    SS7_LinkSetPtr      ls;
    SS7_Destination     *rc;
    ...
    rc = ROUTE_FindDestination(rpc, ni, family);
    if (rc == NULL)
    {
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_GetRemotePC_TTC(SS7_DestInfo *remPc, ITS_OCTET* numDests)
{
    ITS_INT     ret;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_GetLocalPC_TTC(SS7_DestInfo *localPc, ITS_OCTET* numDests)
{
    ITS_INT     ret;
 ...
 *
 ****************************************************************************/
void MTP3_GetOm_TTC(mtp3Om *OmMtp3Data)
{
  ...
  for(i = 1; i <= 250; i++)
  {
    ...
    for (j = 0; j < ls->numLinks; j++)
    {
	  ...
	  //cshekhar CHECK: RPO, Local and Remote Inhibitions not present in TTC; so commenting below lines
...
/****************************************************************************
 *  Purpose:
 *      This API is invoked by the mt_clr_gen_peg() MML command. This will
 *      clear the ACTIVE MTP3 OM Buffer.
 *
...
/****************************************************************************
 *  Purpose:
 *      This API is invoked by the mt_get_gen_peg() MML command.  This will
 *      clear the INACTIVE MTP3 OM Buffer, toggle the ACTIVE/INACTIVE
 ...
 *                           D0380 - D0460
 *
 * mshanmugam  11-04-2007                        To display only TTC link pegs,
 *                                               family check is added.    
 *                                               To allow linkset no upto 255
 *                                               in  the for loop.
 ****************************************************************************/

void MTP3_GetOm_MML_TTC(DBC_Server *dbc)
{
  char buf[32000];
  char buf1[32000];
  ...
  for(i = 1; i <= 250; i++)
  {
    ...
    for (j = 0; j < ls->numLinks; j++)
    {
      ...
      LinkOmImage[omBuf.inactiveOm].MTP3LinkStat[linkset][linkcode].slInsDuration *= DURATION_OM_TICK;
      LinkOmImage[omBuf.inactiveOm].MTP3LinkStat[linkset][linkcode].slUnavailDuration *= DURATION_OM_TICK;
	  //cshekhar CHECK commenting RPO, Local and Remote Inhibition pegs as not present in TTC 
      //LinkOmImage[omBuf.inactiveOm].MTP3LinkStat[linkset][linkcode].slRPODuration *= DURATION_OM_TICK;
      //LinkOmImage[omBuf.inactiveOm].MTP3LinkStat[linkset][linkcode].slLocalInhibitDuration *= DURATION_OM_TICK;
      //LinkOmImage[omBuf.inactiveOm].MTP3LinkStat[linkset][linkcode].slRemoteInhibitDuration *= DURATION_OM_TICK;
      LinkOmImage[omBuf.inactiveOm].MTP3LinkStat[linkset][linkcode].slCumulativeCongDuration *= DURATION_OM_TICK;
...
#if 0
// cshekhar CHECK Following pegs will not be present in TTC 
	  ...
	  sprintf(buf, "\n SRTs Transmitted             : %d", LinkOmImage[omBuf.inactiveOm].MTP3LinkStat[linkset][linkcode].srtTx);
      strcat(buf1, buf);

	  sprintf(buf, "\n SRTs Received                : %d", LinkOmImage[omBuf.inactiveOm].MTP3LinkStat[linkset][linkcode].srtRx);
      strcat(buf1, buf);

	  sprintf(buf, "\n SRAs Transmitted             : %d", LinkOmImage[omBuf.inactiveOm].MTP3LinkStat[linkset][linkcode].sraTx);
      strcat(buf1, buf);
	
	  sprintf(buf, "\n SRAs Received                : %d", LinkOmImage[omBuf.inactiveOm].MTP3LinkStat[linkset][linkcode].sraRx);
      strcat(buf1, buf);
