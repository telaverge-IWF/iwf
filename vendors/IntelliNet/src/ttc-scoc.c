 ...
 ****************************************************************************
 *
 *  ID: $Id: ttc-scoc.c,v 9.4 2008/07/18 05:47:53 ssingh Exp $
 *
 * LOG: $Log: ttc-scoc.c,v $
 * LOG: Revision 9.4  2008/07/18 05:47:53  ssingh
 ...
 * LOG:
 * LOG: Revision 1.1.2.7  2008/02/27 07:35:46  ssodhi
 * LOG: 1. Implementation of DBG command st_get_conns to get the SCCP connections
 * LOG: 2. Canceling the connect and release timer in HandleAppRLSD function
 * LOG:
 * LOG: Revision 1.1.2.6  2008/02/19 08:41:52  ssodhi
 * LOG: Printing more debugging info in existing trace prints.
 * LOG:
 * LOG: Revision 1.1.2.5  2008/02/08 06:22:26  ssodhi
 * LOG: Displaying connection reference also.
 * LOG:
 * LOG: Revision 1.1.2.4  2007/10/04 05:32:16  mshanmugam
 * LOG: Fix for issue 105
 * LOG:
 * LOG: Revision 1.1.2.3  2007/02/13 04:02:28  raghavendrabk
 * LOG: Changed alarm format to put family at beginning
 * LOG:
 * LOG: Revision 1.1.2.2  2007/02/02 14:19:10  raghavendrabk
...
#include <its_pegs.h>
#ident "$Id: ttc-scoc.c,v 9.4 2008/07/18 05:47:53 ssingh Exp $"
               ...
               ITS_EVENT* ev)
{
    ...
    SCCP_DEBUG(("SendToAPP: send connection initiator. conref %u\n",hdr->context.conref));
    ...
    {
        SCCP_WARNING(("SendToAPP: no route to dest. conref %u\n",hdr->context.conref));
    ...
    else
    {
        ...
        SCCP_DEBUG(("SendToAPP: PutEvent returned value %d\n", ret));
    }
    ...
    if (ret != ITS_SUCCESS)
    {
        ...
        return SCOCHandleRoutingFailure_TTC(mtp3,type,ies, ieCount,
         ...
         ITS_BOOLEAN toApp)
{
    ...
    if (toApp)
    {
        SCOCHandleSCRCMsg_TTC(&ies[0].param_data.routingLabel,
           ...
           ITS_SCCP_IE* ies,int errormsgID,ITS_OCTET cause)
{
    ...
    SCCP_DeleteLocalReference_TTC(conref);
    return(ITS_SUCCESS);
}
            ...
            ITS_SCCP_IE* ies, int ieCount)
{
    ...
    if (error != ITS_SUCCESS)
    {
        switch(error)
        {
            case (ITS_EBADMUTEX):
                ...
                break;
            case (ITS_EINUSE):
                SCCP_CRITICAL(("HandleAppCR: DSM Row Locked.conref %u\n",hdr->context.conref));
    ...
    if (cdp == NULL)
    {
        ...
        SCCP_DeleteLocalReference_TTC(hdr->context.conref);
    ...
    switch ((pc->pclass & SCCP_PCLASS_MASK))
    {
        ...
        default:
            ...
            SCCP_Alarm_TTC(686, __FILE__, __LINE__, "family=%s:pclass=%x",
                           "TTC", pc->pclass);
            ...
            if (error != ITS_SUCCESS)
            {
                SCCP_ERROR(("HandleAppCR: DSM unable to Commit Reference in"
                            " order to Release Lock. conref: %u\n",hdr->context.conref));
    ...
    *mtp3 = *rl;
    ...
    return SCRCHandleSCOCMsg_TTC(mtp3, SCCP_MSG_CR, ies, ieCount, cdp);
}
...
DumpContextEntry(ITS_POINTER data, char** pStrDump)
{
    ...
    **pStrDump = 0;
	...
	GetMTP3HeaderString(&c->inLabel, mtp3HdrStr_in);
        ...
        sprintf(printBuf, "%8u %12s %8u %8u %8u %8u %8u %s %s\n",
	        c->connRef,SCCP_CONN_STATE_STR(c->state), c->slref, c->dlref,
...
SCOC_DumpSCCPContexts(char** pStrDump, ITS_BOOLEAN onlyNumConns)
{
    ...
    *pStrDump = NULL;
    ...
    if (DSM_LockTable(DSM_SccpFtGroupId, DSM_TABLE_TTC_SCCP_CTXT) != ITS_SUCCESS)
    {   
    ...
    DSM_TableGetSize(DSM_SccpFtGroupId, DSM_TABLE_TTC_SCCP_CTXT, &size);
             ...
             ITS_UINT dpc, ITS_OCTET ssn, ITS_BOOLEAN needCouple)
{
    ...
    PEG_IncrPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_CR_RCVD);

    /* Q752 Implementation */
    /* message CR recieved from remote */    
    SccpGenOmImage_TTC[omBuf.activeOm].SCCPGenPegs.messageCRRcvd++;
    ...
    if (sl == NULL)
    {
        ...
        SccpGenOmImage_TTC[omBuf.activeOm].SCCPGenPegs.syntaxValueErrDetected++;
        return (ITS_EMISSMAND);
    }
    ...
    switch ((pc->pclass & SCCP_PCLASS_MASK))
    {
        ...
        default:
            SCCP_WARNING(("HandleDownCR: Invalid protocol class for"
                          " connection\n"));
            /* Protocol Class of Message does not match Connection.*/
            SCCP_Alarm_TTC(685, __FILE__, __LINE__, "family=%s:pclass=%x",
                           "TTC", pc->pclass);
    ...
    if (hc && pc && pc->pclass == SCCP_PCLASS_3)
    {
        ...
        if (hc->count == 0 || hc->count == 255)
        {
            ...
            SCCP_ERROR(("HandleDownCR: Hop count exceeded. count is %d\n",hc->count));
    ...
    if (needCouple)
    {
        ...
        if (error != ITS_SUCCESS)
        {
            ...
            SCCP_DeleteLocalReference_TTC(chain.context.conref);
    ...
    *mtp3 = foundCtxt->outLabel;
    ...
    switch ((pc->pclass & SCCP_PCLASS_MASK))
    {
        ...
        case SCCP_PCLASS_3:
            ...
            {
                SCCP_DEBUG(("HandleAppCC:Invalid Credit value %u sent in CC from"
    ...
    return SCRCHandleSCOCMsg_TTC(mtp3, SCCP_MSG_CC, ies, ieCount, cdp);
}
/*
 * send released
 */
              ...
              ITS_SCCP_IE* ies, int ieCount)
{
    ...
    //PEG_IncrPeg(TTC_SCCP_Pegs,PEG_SCCP_RLSD_TO_REMOTE_SENT);
   ...
   {
       SCCP_ERROR(("HandleAppRLSD:RLSD received in Connection Pending state %x"
  ...
  */
   ...
   {
        SCCP_ERROR(("RLSD is received in Outgoing Connection pending state %x"
   ...
   {
       /* SCCP_Alarm_TTC(658, __FILE__, __LINE__, "family=%s", "TTC"); */
   }
   else
   {
    ...
    {
        SCCP_DEBUG(("HandleAppRLSD:Connect timer %u is killed \n",foundCtxt->connectTimer));
        TIMERS_CancelTimer(foundCtxt->connectTimer);
    }
    if (foundCtxt->releaseTimer)
    {
        SCCP_DEBUG(("HandleAppRLSD:Release timer %u is killed \n",foundCtxt->releaseTimer));
    ...
    *mtp3 = foundCtxt->outLabel;
    ...
    if (foundCtxt->state==SCCP_CONN_OUT_PEND)
    {
        SCCP_DEBUG(("HandleAppRLSD:RLSD is not propogated to the other end"
                      "as waiting for CC or CREF.conref:%u\n",hdr->context.conref));
    ...
    return SCRCHandleSCOCMsg_TTC(mtp3, SCCP_MSG_RLSD, ies, ieCount, cdp);
}
             ...
             ITS_SCCP_IE* ies, int ieCount)
{
    ...
    if (foundCtxt->pclass != SCCP_PCLASS_3)
    {
        SCCP_ERROR(("HandleAppRSR: RSR is invalid for %u protocol class."
		    "\n",foundCtxt->pclass ));
        /* Outgoing Message - Discarding Message - Message Type not valid 
           for Protocol Class of Connection. */

        SCCP_Alarm_TTC(671, __FILE__, __LINE__, "family=%s:pclass=%x",
                       "TTC", foundCtxt->pclass);
    ...
    *mtp3 = foundCtxt->outLabel;
    ...
    PEG_IncrPeg(TTC_SCCP_Pegs, PEG_SCCP_RSR_TO_REMOTE_SENT);
    ...
    return SCRCHandleSCOCMsg_TTC(mtp3, SCCP_MSG_RSR, ies, ieCount, cdp);
}
             ...
             ITS_SCCP_IE* ies, int ieCount, ITS_BOOLEAN* sendToApp)
{
        ...
        || foundCtxt->state == SCCP_CONN_IN_PEND)
    {
        ...
        /* Unknown message in this state*/
        SCCP_Alarm_TTC(656, __FILE__, __LINE__, "family=%s", "TTC");
    ...
    switch ((pc->pclass & SCCP_PCLASS_MASK))
    {
        ...
        case SCCP_PCLASS_3:
           ...
           if (foundCtxt->windowSize < creditvalue)
           {
               ...
               SCCP_DEBUG(("HandleDownCC:Invalid Credit value %u sent in CC from"
                           "the application,message is discarded\n",creditvalue));
    ...
    if(foundCtxt->releaseTimer)
    {
        SCCP_DEBUG(("HandleDownCC:ReleaseTimer %u is killed as waiting CC"
                    "  received.\n",foundCtxt->releaseTimer));       
              ...
              ITS_SCCP_IE* ies, int ieCount)
{
    ...
    *mtp3 = foundCtxt->outLabel;
    ...
    PEG_IncrPeg(TTC_SCCP_Pegs, PEG_SCCP_CREF_TO_REMOTE_SENT);

    /*Q752 Implementation Message CREF Recieved */
    SccpGenOmImage_TTC[omBuf.activeOm].SCCPGenPegs.messageCREFSent++;
    ...
    return SCRCHandleSCOCMsg_TTC(mtp3, SCCP_MSG_CREF, ies, ieCount, cdp);
}
               ...
               ITS_SCCP_IE* ies, int ieCount, ITS_BOOLEAN* sendToApp)
{
    ...
    PEG_IncrPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_CREF_RCVD);

    /*Q752 Implementation Message CREF Recieved */ 
    SccpGenOmImage_TTC[omBuf.activeOm].SCCPGenPegs.messageCREFRcvd++;
    ...
    if (error != ITS_SUCCESS)
    {
        switch(error)
        {
            ...
            case (ITS_ENOTFOUND):
                ...
                SCCP_ERROR(("HandleDownCREF:CREF received with unassigned DLR %u"
       ...
       || foundCtxt->state == SCCP_CONN_IN_PEND)
    {
        ...
        /* Unknown message in this state*/
        SCCP_Alarm_TTC(657, __FILE__, __LINE__, "family=%s", "TTC");
    ...
    if (foundCtxt->state == SCCP_IN_RELEASE_PEND)
    {
        ...
        if (foundCtxt->releaseTimer)
        {
            SCCP_DEBUG(("HandleDownCREF:Release timer %u is killed"
			"\n",foundCtxt->releaseTimer));
             ...
             ITS_SCCP_IE* ies, int ieCount)
{
    ...
    if (RN_GET_REF_NUM(*sl) != hdr->context.conref)
    {
        SCCP_ERROR(("HandleAppRLC: Released (%u)and context (conref: %u) mismatch\n"
		    ,RN_GET_REF_NUM(*sl), hdr->context.conref));
    ...
    *mtp3 = foundCtxt->outLabel;
    ...
    return SCRCHandleSCOCMsg_TTC(mtp3, SCCP_MSG_RLC, ies, ieCount, cdp);
}
               ...
               ITS_SCCP_IE* ies, int ieCount, ITS_BOOLEAN* sendToApp)
{
    ...
    /* Q752 Implementation */
        SccpGenOmImage_TTC[omBuf.activeOm].SCCPGenPegs.providerInitiatedReleaseConn++;   
    ...
    {
        SCCP_ERROR(("HandleDownRLSD:Orignating Point Code %u received not equal\
                     to the Pc %u stored locally\n", MTP3_RL_GET_OPC_VALUE(mtp3->label),
    ...
    if (foundCtxt->dlref != RN_GET_REF_NUM(*sl))
    {
        SCCP_DEBUG(("HandleDownRLSD:Connection context dlref mismatch with"
                     " the received slref %u",foundCtxt->dlref));
         /* Release the lock on row */
        error = DSM_Commit(DSM_SccpFtGroupId,
                           DSM_TABLE_TTC_SCCP_CTXT,
                           (ITS_OCTET *)&hdr->context.conref,
              ...
              ITS_SCCP_IE* ies, int ieCount, ITS_BOOLEAN* sendToApp)
{
    ...
    if (MTP3_RL_GET_DPC_VALUE(foundCtxt->outLabel.label)!=
        MTP3_RL_GET_OPC_VALUE(mtp3->label))
    {
        SCCP_ERROR(("HandleDownRLC:Orignating Point Code %u received not equal to\
                     the Pc %u stored locally hence the msg is discarded\n",
		     MTP3_RL_GET_OPC_VALUE(mtp3->label),
		     MTP3_RL_GET_DPC_VALUE(foundCtxt->outLabel.label)));
    ...
    if (foundCtxt->state == SCCP_CONN_OUT_PEND)
    {
        SCCP_DEBUG(("HandleDownRLC:RLC received in outgoing connection"
                    " pending state\n"));
        SCCP_Alarm_TTC(659, __FILE__, __LINE__, "family=%s", "TTC");
    ...
    SCCP_DeleteLocalReference_TTC((ITS_CTXT)dlref);
             ...
             ITS_SCCP_IE* ies, int ieCount)
{
    ...
    if (foundCtxt->pclass != SCCP_PCLASS_2)
    {
        ...
        SCCP_Alarm_TTC(672, __FILE__, __LINE__, "family=%s:pclass=%x",
                       "TTC", foundCtxt->pclass);
    ...
    if (foundCtxt->state != SCCP_DATA_TRANSFER)
    {
        SCCP_ERROR(("HandleAppDT1: DT1 is invalid for connection state %u.\n",
		    foundCtxt->state));
        SCCP_Alarm_TTC(660, __FILE__, __LINE__, "family=%s", "TTC");
    ...
    *mtp3 = foundCtxt->outLabel;
    ...
    if (dataIndex < 0)
    {
        SCCP_ERROR(("HandleAppDT1: Missing Data IE. dataIndex:%d\n",dataIndex));
    ...
    for (i = 0;i < totalSegment; i++)
    {
        if (segIndex < 0)
        {
            SCCP_ERROR(("HandleAppDT1: Missing  segment reassem ie segIndex: %d\n",segIndex));
...
/*        if (dataLen < (SCCP_MAX_DATA + (i * SCCP_MAX_DATA)))
        ...
        ret = SCRCHandleSCOCMsg_TTC(mtp3, SCCP_MSG_DT1, ies, 
                                      ieCount, cdp);
              ...
              ITS_SCCP_IE* ies, int ieCount, ITS_BOOLEAN* sendToApp)
{
    ...
    PEG_IncrPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_DT1_RCVD);
    ...
    if (dataIndex < 0)
    {
        SCCP_ERROR(("HandleAppDT1: Missing data. dataIndex: %d\n",dataIndex));
    ...
    if (foundCtxt->pclass != SCCP_PCLASS_2)
    {
        SCCP_ERROR(("HandleDownDT1: DT1 is invalid for protocol class."
		   "Protocol class %x\n",foundCtxt->pclass));
        SCCP_Alarm_TTC(673, __FILE__, __LINE__, "family=%s:pclass=%x",
                       "TTC", foundCtxt->pclass);
    ...
    if(foundCtxt->state == SCCP_CONN_OUT_PEND)
    {
        SCCP_DEBUG(("HandleDownDT1:DT1 received in outgoing connection"
                    " pending state\n"));    
        SCCP_Alarm_TTC(661, __FILE__, __LINE__, "family=%s", "TTC");
    ...
    if (tempIsIntermediate)
    {
        ...
        *mtp3 = foundCtxt2->outLabel;
        ...
        if (error != ITS_SUCCESS)
        {
            SCCP_ERROR(("HandleDownDT1: DSM unable to free Reference.%u\n",tempAssocRef));
    ...
    else
    {
        if (segIndex < 0)
        {
            ...
            PEG_IncrPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_REASSEMBLE_ERR);
             ...
             ITS_SCCP_IE* ies, int ieCount)
{
    ...
    if (dl == NULL)
    {
        ...
        SccpGenOmImage_TTC[omBuf.activeOm].SCCPGenPegs.syntaxConstructionErrDetected++;
    ...
    if (foundCtxt->pclass != SCCP_PCLASS_3)
    {
        SCCP_ERROR(("HandleAppDT2: DT2 is invalid for protocol class %u\n",
		    foundCtxt->pclass));

        /* Discarding Outgoing Message - Message Type not valid for Protocol Class of Connection. */
         SCCP_Alarm_TTC(674, __FILE__, __LINE__, "family=%s:pclass=%x",
                        "TTC", foundCtxt->pclass);
    ...
    if (foundCtxt->state != SCCP_DATA_TRANSFER)
    {
        ...
        SCCP_Alarm_TTC(662, __FILE__, __LINE__, "family=%s", "TTC");
    ...
    *mtp3 = foundCtxt->outLabel;
    ...
    return SCRCHandleSCOCMsg_TTC(mtp3, SCCP_MSG_DT2, ies, ieCount, cdp);
}
              ...
              ITS_SCCP_IE* ies, int ieCount, ITS_BOOLEAN* sendToApp)
{
    ...
    PEG_IncrPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_DT2_RCVD);
    ...
    if (MTP3_RL_GET_DPC_VALUE(foundCtxt->outLabel.label)!=
        MTP3_RL_GET_OPC_VALUE(mtp3->label))
    {
        SCCP_ERROR(("HandleDownDT2:Orignating Point Code %u received not equal to"
                    " the Pc %u stored locally hence the msg is discarded\n",
		    MTP3_RL_GET_OPC_VALUE(mtp3->label),
		    MTP3_RL_GET_DPC_VALUE(foundCtxt->outLabel.label)));
    ...
    if (foundCtxt->pclass != SCCP_PCLASS_3)
    {
        ...
        SCCP_Alarm_TTC(675, __FILE__, __LINE__, "family=%s:pclass=%x",
                       "TTC", foundCtxt->pclass);
    ...
    if  (foundCtxt->state == SCCP_CONN_OUT_PEND)
    {
        SCCP_DEBUG(("HandleDownDT2:DT2 received in Invalid state"
		    "(%x)\n",foundCtxt->state));
        SCCP_Alarm_TTC(663, __FILE__, __LINE__, "family=%s", "TTC");
        ...
        if (error != ITS_SUCCESS)
        {
            SCCP_ERROR(("HandleDownDT2: DSM unable to free Reference %u",
			hdr->context.conref));
    ...
    if (foundCtxt->state != SCCP_DATA_TRANSFER)
    {
        SCCP_ERROR(("HandleDownDT2: DT2 is invalid for connection state(%u)",
		    foundCtxt->state));
        /* Incoming Message - Message Type not valid for State of Connection. */
        SCCP_Alarm_TTC(663, __FILE__, __LINE__, "family=%s", "TTC");
            ...
            ITS_SCCP_IE* ies, int ieCount)
{
    ...
    if (foundCtxt->state != SCCP_DATA_TRANSFER)
    {
        SCCP_ERROR(("HandleAppAK: AK is invalid for connection state"
		    "=%x\n",foundCtxt->state));
        SCCP_Alarm_TTC(664, __FILE__, __LINE__, "family=%s", "TTC");
    ...
    *mtp3 = foundCtxt->outLabel;
    ...
    return SCRCHandleSCOCMsg_TTC(mtp3, SCCP_MSG_AK, ies, ieCount, cdp);
}
             ...
             ITS_SCCP_IE* ies, int ieCount, ITS_BOOLEAN* sendToApp)
{
    ...
    if (foundCtxt->pclass != SCCP_PCLASS_3)
    {
        ...
        SCCP_Alarm_TTC(677, __FILE__, __LINE__, "family=%s:pclass=%x",
                       "TTC", foundCtxt->pclass);
    ...
    if(foundCtxt->state == SCCP_CONN_OUT_PEND)
    {
        SCCP_DEBUG(("HandleDownAK:AK received in outgoing connection"
                    " pending state\n"));
        SCCP_Alarm_TTC(665, __FILE__, __LINE__, "family=%s", "TTC");
            ...
            ITS_SCCP_IE* ies, int ieCount)
{
    ...
    if (foundCtxt->pclass != SCCP_PCLASS_3)
    {
        ...
        /* Outgoing Message - Discarding Message - Message Type not valid for 
           Protocol Class of Connection. */
        SCCP_Alarm_TTC(678, __FILE__, __LINE__, "family=%s:pclass=%x",
                       "TTC", foundCtxt->pclass);
    ...
    if (foundCtxt->state != SCCP_DATA_TRANSFER)
    {
        SCCP_ERROR(("HandleAppED: ED is invalid for connection state"
		    "=%x\n",foundCtxt->state));
        SCCP_Alarm_TTC(666, __FILE__, __LINE__, "family=%s", "TTC");
    ...
    *mtp3 = foundCtxt->outLabel;
    ...
    return SCRCHandleSCOCMsg_TTC(mtp3, SCCP_MSG_ED, ies, ieCount, cdp);
}
            ...
            ITS_SCCP_IE* ies, int ieCount)
{
    ...
    if (foundCtxt->pclass != SCCP_PCLASS_3)
    {
        ...
        /* Outgoing Message - Discarding Message - Message Type not valid for 
           State of Connection. */
        SCCP_Alarm_TTC(679, __FILE__, __LINE__, "family=%s:pclass=%x",
                       "TTC", foundCtxt->pclass);
    ...
    if (foundCtxt->state != SCCP_DATA_TRANSFER)
    {
        SCCP_ERROR(("HandleAppEA: EA is invalid for connection state."
		    "conref:%x\n",foundCtxt->state));
        SCCP_Alarm_TTC(668, __FILE__, __LINE__, "family=%s", "TTC");
    ...
    *mtp3 = foundCtxt->outLabel;
    ...
    return SCRCHandleSCOCMsg_TTC(mtp3, SCCP_MSG_EA, ies, ieCount, cdp);
}
             ...
             ITS_SCCP_IE* ies, int ieCount, ITS_BOOLEAN* sendToApp)
{
    ...
    PEG_IncrPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_ED_RCVD);
    ...
    if (MTP3_RL_GET_DPC_VALUE(foundCtxt->outLabel.label)!=
        MTP3_RL_GET_OPC_VALUE(mtp3->label))
    {
        SCCP_ERROR(("HandleED:Orignating Point Code %u received not equal to"
                    "  the Pc %u stored locally hence the msg is discarded"
		    "\n",MTP3_RL_GET_OPC_VALUE(mtp3->label),
		    MTP3_RL_GET_DPC_VALUE(foundCtxt->outLabel.label)));
    ...
    if (foundCtxt->state == SCCP_IN_RELEASE_PEND)
    {
        SCCP_DEBUG(("HandleDownED:ED received in incoming"
                        " Release Pending state\n" ));
        SCCP_Alarm_TTC(667, __FILE__, __LINE__, "family=%s", "TTC");
    ...
    if (foundCtxt->pclass != SCCP_PCLASS_3)
    {
        SCCP_ERROR(("HandleDownED: ED is invalid for protocol class "
		    "%x\n",foundCtxt->pclass));
        SCCP_Alarm_TTC(680, __FILE__, __LINE__, "family=%s:pclass=%x",
                       "TTC", foundCtxt->pclass);
             ...
             ITS_SCCP_IE* ies, int ieCount, ITS_BOOLEAN* sendToApp)
{
    ...
    if (MTP3_RL_GET_DPC_VALUE(foundCtxt->outLabel.label)!=
        MTP3_RL_GET_OPC_VALUE(mtp3->label))
    {
        SCCP_ERROR(("HandleDownEA:Orignating Point Code %u received not equal to"
                     " the Pc %u stored locally hence the msg is discarded\n",
		     MTP3_RL_GET_OPC_VALUE(mtp3->label),
		     MTP3_RL_GET_DPC_VALUE(foundCtxt->outLabel.label)));
    ...
    if (foundCtxt->pclass != SCCP_PCLASS_3)
    {
        ...
        /* Incoming Message - Discarding Message - Message Type not valid for 
           Protocol Class of Connection. */
        SCCP_Alarm_TTC(681, __FILE__, __LINE__, "family=%s:pclass=%x",
                       "TTC", foundCtxt->pclass);
    ...
    if(foundCtxt->state == SCCP_CONN_OUT_PEND)
    {
        SCCP_DEBUG(("HandleDownEA:EA received in outgoing connection"
                    " pending state\n"));
        SCCP_Alarm_TTC(669, __FILE__, __LINE__, "family=%s", "TTC");
              ...
              ITS_SCCP_IE* ies, int ieCount, ITS_BOOLEAN* sendToApp)
{
    ...
    PEG_IncrPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_RSR_RCVD);

    /*Q752 Implementation Message RSR Recieved */
    SccpGenOmImage_TTC[omBuf.activeOm].SCCPGenPegs.providerInitiatedResetConn++;

    SccpGenOmImage_TTC[omBuf.activeOm].SCCPGenPegs.messageRSRRcvd++;
    ...
    if (MTP3_RL_GET_DPC_VALUE(foundCtxt->outLabel.label)!=
        MTP3_RL_GET_OPC_VALUE(mtp3->label))
    {
        SCCP_ERROR(("HandleDownRSR:Orignating Point Code %u received not equal to"
                    "the Pc %u stored locally\n",MTP3_RL_GET_OPC_VALUE(mtp3->label),
		    MTP3_RL_GET_DPC_VALUE(foundCtxt->outLabel.label)));
    ...
    if (foundCtxt->pclass != SCCP_PCLASS_3)
    {
        ...
        /* Incoming Message - Discarding Message - Message Type not valid for 
           Protocol Class of Connection. */
        SCCP_Alarm_TTC(682, __FILE__, __LINE__, "family=%s:pclass=%x",
                       "TTC", foundCtxt->pclass);
             ...
             ITS_SCCP_IE* ies, int ieCount)
{
    ...
    if (foundCtxt->pclass != SCCP_PCLASS_3)
    {
        SCCP_ERROR(("HandleAppRSC: RSC is invalid for protocol class"
		    " %x\n",foundCtxt->pclass));
        SCCP_Alarm_TTC(683, __FILE__, __LINE__, "family=%s:pclass=%x",
                       "TTC", foundCtxt->pclass);
    ...
    *mtp3 = foundCtxt->outLabel;
    ...
    return SCRCHandleSCOCMsg_TTC(mtp3, SCCP_MSG_RSC, ies, ieCount, cdp);
}
              ...
              ITS_SCCP_IE* ies, int ieCount, ITS_BOOLEAN* sendToApp)
{
    ...
    if (MTP3_RL_GET_DPC_VALUE(foundCtxt->outLabel.label)!=
        MTP3_RL_GET_OPC_VALUE(mtp3->label))
    {
        SCCP_ERROR(("HandleDownRSC:Orignating Point Code %u received not equal to"
                    "the Pc %u stored locally\n",MTP3_RL_GET_OPC_VALUE(mtp3->label),
		    MTP3_RL_GET_DPC_VALUE(foundCtxt->outLabel.label)));
    ...
    if (foundCtxt->pclass != SCCP_PCLASS_3)
    {
        ...
        /* Incoming Message - Discarding Message - Message Type not valid for 
           Protocol Class of Connection. */
        SCCP_Alarm_TTC(684, __FILE__, __LINE__, "family=%s:pclass=%x",
                       "TTC", foundCtxt->pclass);
             ...
             ITS_SCCP_IE* ies, int ieCount)
{
    ...
    *mtp3 = foundCtxt->outLabel;
    ...
    PEG_IncrPeg(TTC_SCCP_Pegs, PEG_SCCP_ERR_TO_REMOTE_SENT);

    /*Q752 Implementation Message Error Sent */
    SccpGenOmImage_TTC[omBuf.activeOm].SCCPGenPegs.messageErrSent++;
    ...
    return SCRCHandleSCOCMsg_TTC(mtp3, SCCP_MSG_ERR, ies, ieCount, cdp);
}
              ...
              ITS_SCCP_IE* ies, int ieCount, ITS_BOOLEAN* sendToApp)
{
    ...
    PEG_IncrPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_ERR_RCVD);

    /*Q752 Implementation Message Error Recieved */   
    SccpGenOmImage_TTC[omBuf.activeOm].SCCPGenPegs.messageErrRcvd++;
    ...
    if (MTP3_RL_GET_DPC_VALUE(foundCtxt->outLabel.label)!=
        MTP3_RL_GET_OPC_VALUE(mtp3->label))
    {
        SCCP_ERROR(("HandleDownERR:Orignating Point Code %u received not equal to"
                    " the Pc %u stored locally hence the msg is discarded\n",
		    MTP3_RL_GET_OPC_VALUE(mtp3->label),
		    MTP3_RL_GET_DPC_VALUE(foundCtxt->outLabel.label)));
            ...
            ITS_SCCP_IE* ies, int ieCount)
{
    ...
    *mtp3 = foundCtxt->outLabel;
    ...
    return SCRCHandleSCOCMsg_TTC(mtp3, SCCP_MSG_IT, ies, ieCount, cdp);
}
             ...
             ITS_SCCP_IE* ies, int ieCount, ITS_BOOLEAN* sendToApp)
{
    ...
    *sendToApp = ITS_FALSE;
    ...
    if (MTP3_RL_GET_DPC_VALUE(foundCtxt->outLabel.label)!=
        MTP3_RL_GET_OPC_VALUE(mtp3->label))
    {
        SCCP_ERROR(("HandleDownIT:Orignating Point Code %u received not equal to"
                    "the Pc %u stored locally hence the msg is discarded\n",
		     MTP3_RL_GET_OPC_VALUE(mtp3->label),
		     MTP3_RL_GET_DPC_VALUE(foundCtxt->outLabel.label)));
 ...
 ****************************************************************************/
int
SCOCHandleRoutingFailure_TTC(MTP3_HEADER* mtp3, ITS_OCTET mType,
                               ITS_SCCP_IE* ies, int ieCount,
                               ITS_OCTET reason)
{
    ITS_INT opc, dpc;
    ...
    ITS_OCTET ni = MTP3_HDR_GET_SIO_TTC(*mtp3) & MTP3_NIC_MASK;
    SCCP_ADDR *cgp = NULL;
 ...
 ****************************************************************************/
int
SCOCHandleAPPMsg_TTC(ITS_EVENT *event)
{
    ...
    switch (SCCP_MSG_TYPE(event))
    {
        case SCCP_MSG_CR:
            ...
            PEG_IncrPeg(TTC_SCCP_Pegs,PEG_SCCP_USER_CR_SENT);
            
            /*Q752 Implementation Number of CR messages Sent */
            SccpGenOmImage_TTC[omBuf.activeOm].SCCPGenPegs.messageCRSent++;
        ...
        case SCCP_MSG_RLSD:
            ...
            PEG_IncrPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_INITIATED_RLS);
   
            /*Q752 Implementation User Initiated Release */
            SccpGenOmImage_TTC[omBuf.activeOm].SCCPGenPegs.userInitiatedReleaseConn++;
        ...
        case SCCP_MSG_DT1:
            ...
            PEG_IncrPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_DT1_SENT);
        ...
        case SCCP_MSG_DT2:
            ...
            PEG_IncrPeg(TTC_SCCP_Pegs,PEG_SCCP_USER_DT2_SENT);
        ...
        case SCCP_MSG_ED:
            ...
            PEG_IncrPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_ED_SENT);
        ...
        case SCCP_MSG_RSR:
            ...
            SccpGenOmImage_TTC[omBuf.activeOm].SCCPGenPegs.messageRSRSent++;
  
            PEG_IncrPeg(TTC_SCCP_Pegs,PEG_SCCP_USER_INITITATED_RESET);
            
            /*Q752 Implementation User Initiated Reset */ 
            SccpGenOmImage_TTC[omBuf.activeOm].SCCPGenPegs.userInitiatedResetConn++; 
        ...
        default:
            SCCP_ERROR(("SCOCHandleAPPMsg_TTC: Invalid msg type."
			"conref:%u\n",hdr.context.conref));
            return (ITS_ENOCONN);
    }
}
 ...
 ****************************************************************************/
int
SCOCHandleSCRCMsg_TTC(MTP3_HEADER* mtp3, ITS_OCTET mType,
                        ITS_SCCP_IE* ies, int ieCount,
                        ITS_UINT pc, ITS_OCTET ssn, ITS_BOOLEAN needCouple)
{
    ...
    SCCP_DEBUG(("SCOCHandleSCRCMsg_TTC\n"));
    ...
    switch (mType)
    {
        ...
        default:
            SCCP_ERROR(("SCOCHandleSCRCMsg_TTC: Discard unknown type"
			"conref:%u\n",hdr.context.conref));
            break;
    }
 ...
 ****************************************************************************/
int
SCOCHandleTimer_TTC(ITS_TimerData* td)
{
    ...
    if (error != ITS_SUCCESS)
    {
        ...
        {
            case (ITS_EBADMUTEX):
                SCCP_ERROR(("SCOCHandleTimer_TTC: Couldn't acquire mutex."
                            "conref:%u\n",tempConnRef));

                break;
            case (ITS_EINUSE):
                SCCP_ERROR(("SCOCHandleTimer_TTC: DSM Row Locked."
			    "conref:%u\n",tempConnRef));

                break;
            case (ITS_ENOTFOUND):
                SCCP_WARNING(("SCOCHandleTimer_TTC: Can't find Connection"
                               "Reference in table.conref:%u\n",tempConnRef));
    ...
    if (error != ITS_SUCCESS)
    {
        SCCP_ERROR(("SCOCHandleTimer_TTC: DSM unable to free Reference."
		    "conref:%u\n",tempConnRef));
    ...
    if (isSCCPBackup)
    {
       SCCP_DeleteLocalReference_TTC(copyCtxt.connRef);
    ...
    switch (copyCtxt.state)
    {
        ...
        case SCCP_CONN_IN_PEND:
            SCCP_ERROR(("SCOCHandleTimer_TTC: Connection segment expired: ref %04x\n",copyCtxt.connRef));
        ...
        case SCCP_IN_RESET_PEND:
            SCCP_ERROR(("SCOCHandleTimer_TTC: Reset expired: ref %04x\n",
                        copyCtxt.connRef));
        ...
        case SCCP_IN_RELEASE_PEND:
            SCCP_ERROR(("SCOCHandleTimer_TTC: HandleTIMER: Release expired:"
                        "ref %04x\n",copyCtxt.connRef));
            ...
            else if (copyCtxt.state == SCCP_OUT_RELEASE_PEND)
            {
                ...
                SCCP_DEBUG(("SCOCHandleTimer_TTC:Released send to MTP3 After"
                            "Timer Expiry\n"));
                break;
        ...
        default:
            ...
            {
                SCCP_DEBUG(("SCOCHandleTimer_TTC: SendInactivity: ref %04x\n",
                             copyCtxt.connRef));
            ...
            {
                SCCP_DEBUG(("SCOCHandLeTimer_TTC: RecieveInactivity: ref %04x" 
                            "\n",copyCtxt.connRef));

                /* receive inactivity */    
                /* should release connection on this. */
                PEG_IncrPeg(TTC_SCCP_Pegs, PEG_SCCP_TIMER_TIAR_EXPIRY);
       
                /*Q752 Implementation Timer Tair Expiry */
                SccpGenOmImage_TTC[omBuf.activeOm].SCCPGenPegs.timerTiarExpiry++;
 
                SendRLSD(&copyCtxt, SCCP_RLC_INACT_EXPIRY, ITS_FALSE);
            }
            else if (td->serial == copyCtxt.releaseTimer)
            {
                SCCP_DEBUG(("SCOCHandleTimer_TTC: Release Timer Expired : ref %04x\n",
                          copyCtxt.connRef));
            ...
            {
                SCCP_DEBUG(("SCOCHandleTimer_TTC: Repeat Release Timer Expired : ref %04x\n",
                          copyCtxt.connRef));
            ...
            {
                SCCP_DEBUG(("SCOCHandleTimer_TTC: Interval Timer Expired : ref %04x\n",
                          copyCtxt.connRef));
            ...
            {
                SCCP_DEBUG(("SCOCHandleTimer_TTC: Unknown timer: ref %04x"
                  ...
                  ITS_OCTET len)
{
    ...
    if(*startKey == 0)
    {
        *startKey = segRefNum | 0x80000000;
        ...
        {
            SCCP_CRITICAL(("PutDataInFragList: Couldn't alloc segment.key=%u\n",key));
            ...
            if (error != ITS_SUCCESS)
            {
                switch(error)
                {
                    case (ITS_EBADMUTEX):
                        ...
                        break;
                    case (ITS_ENOMEM):
                        SCCP_CRITICAL(("GetNextLocalRef: No Memory.key=%u\n",key));
                        break;
                }
        ...
        if (error != ITS_SUCCESS)
        {
            SCCP_ERROR(("PutDataInFragList: Couldn't add context.key=%u\n",key));
...
SCOC_GetDataFromFragList(ITS_OCTET *data, ITS_UINT *startKey)
{
    ...
    if (error == ITS_ENOTFOUND)
    {
        SCCP_CRITICAL(("GetDataInFragList: Couldn't alloc segment.key=%u\n",key));
    ...
    *startKey = 0;
    if (error != ITS_SUCCESS)
    {
        SCCP_ERROR(("GetDataFromFragList: DSM unable to free Segment.key=%u\n",key));
