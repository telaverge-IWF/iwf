 ...
 ****************************************************************************
 *
 *  ID: $Id: prc-mgmt.c,v 1.5 2008/06/17 06:39:07 nvijikumar Exp $
 *
 * LOG: $Log: prc-mgmt.c,v $
 ...
 * LOG:
 * LOG: Revision 1.3  2008/05/01 13:42:58  kramesh
 * LOG: Propagated WSC Issue 1030. rt_dump mml display issue.
 * LOG:
 * LOG: Revision 1.2  2007/02/01 13:26:38  yranade
 * LOG: Add prc files
 * LOG: 
 * LOG: Revision 1.1.4.9  2007/05/09 12:26:35  mshanmugam
 * LOG: Fix for 5731 - skatta
 * LOG:
 * LOG: Revision 1.1.4.8  2007/04/26 08:58:35  raghavendrabk
 * LOG: Resolved issue with Linkset Peg collection
 * LOG:
 * LOG: Revision 1.1.4.7  2007/04/11 07:49:27  mshanmugam
 * LOG: Family check is added for link pegs
 * LOG:
 * LOG: Revision 1.1.4.6  2007/03/31 10:28:54  mshanmugam
 * LOG: Fix for MMLs mp_get_all_lpc and mp_get_all_rpc
 * LOG:
 * LOG: Revision 1.1.4.5  2007/03/21 12:12:48  sdwivedi
 * LOG: Fix for Parallel Stack Issue #5230
 * LOG:
 * LOG: Revision 1.1.4.4  2007/02/26 10:20:33  raghavendrabk
 * LOG: Family structure filled in OM API
 * LOG:
 * LOG: Revision 1.1.4.3  2007/02/09 10:25:00  gharipriya
 * LOG: Added MMLs as a part of Parallel Stack Implementation
 * LOG:
 * LOG: Revision 1.1.4.2  2007/02/06 12:02:13  ssingh
 * LOG: Fixing issue:: Accelero 6.7:: PRC links are not aligning (#5340)
 * LOG:
 * LOG: Revision 1.1.4.1  2007/01/25 10:59:33  randresol
...
#ident "$Id: prc-mgmt.c,v 1.5 2008/06/17 06:39:07 nvijikumar Exp $"
...
void MTP3_ResetOm_PRC();
void MTP3_GetOm_PRC(mtp3Om *);
#if defined(__cplusplus)
}
#endif /* defined(__cplusplus) */

void MTP3_ResetOm_MML_PRC();
void MTP3_GetOm_MML_PRC(DBC_Server *);
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
MGMT_Main_PRC(MTP3_SubSystems src, MGMT_Triggers trigger,
                ITS_OCTET *sif, ITS_USHORT len,
                ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    int ret;

    MTP3_TRACE_ENTRY_PRC(MTP3_MGMT, src, trigger, sif, len,
                          linkSet, linkCode);
...
ShutDownNode(DBC_Server *dbc)
{
          ...
          tr != NULL; tr = TRANSPORT_HMI_NEXT(tr))
    {
        ...
        if ((mask & ITS_MTP2_PRC) == ITS_MTP2_PRC)
        {
            SS7_LinkPtr lp;
...
{    
    ...
    if (DBC_Cmd(dbc, "mp_get_gen_cfg", "MTP3 Get General Config", "", ""))
    {
        MTP3GeneralCfg *cfg = 0;
        cfg =  MTP3_GetGeneralCfg_PRC();
        sprintf(buf, "\n*********** MTP3 General Config *************\n");
        DBC_AppendText(dbc, buf);
        ...
        if ( cfg->traceOn)
        {
           if (TRACE_IsLevelOn(MTP3_PRC_TraceData,
                                    MTP3_TRACE_DEBUG))
           {
               strcpy(buf, "        Debug     = ");
    ...
    else if (DBC_Cmd(dbc, "mp_set_gen_cfg", "MTP3 Set General Config",
                     "<alarm level> <Trace Type> <traceOutput> <On/Off>",""))
    {
        ...
        if (ret != 5)
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n"
                         "Usage: mp_set_gen_cfg <alarmlevel> <tracetype>" 
                         " <trOutput> <On/Off>\n"
        ...
        ret =  MTP3_SetGeneralCfg_PRC(&cfg);
        if (ret != ITS_SUCCESS)
        {
            return;
        }
    ...
    else if (DBC_Cmd(dbc, "mp_in_link", "Inhibit Link", "<linkset> <linkcode>",  ""))
    {
        ret = sscanf(cmdLine, "%s %d %d", buf, &linkSet, &linkCode);
        if (ret != 3)
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n"
                         "Usage: mp_in_link <linkset> <linkcode>\n");
            DBC_AppendText(dbc, buf);
            return;
        }
    ...
    else if (DBC_Cmd(dbc, "mp_unin_link", "Uninhibit Link", 
             "<linkset> <linkcode>", ""))
    {
        ret = sscanf(cmdLine, "%s %d %d", buf, &linkSet, &linkCode);
        if (ret != 3)
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n" 
                         "Usage: mp_unin_link <linkset> <linkcode>\n");
            DBC_AppendText(dbc, buf);
            return;
        }
    ...
    else if (DBC_Cmd(dbc, "mp_deact_ls", "Deactivate Link Set", "<linkset>", ""))
    {
        ret = sscanf(cmdLine, "%s %d", buf, &linkSet);
        if (ret != 2)
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n" 
                         "Usage: mp_deact_ls <linkset> \n");
            DBC_AppendText(dbc, buf);
            return;
        }
    ...
    else if (DBC_Cmd(dbc, "mp_act_ls", "Activate Link Set", "<linkset>", ""))
    {
        ret = sscanf(cmdLine, "%s %d", buf, &linkSet);
        if (ret != 2)
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n" 
                         "Usage: mp_act_ls <linkset> \n");
            DBC_AppendText(dbc, buf);
            return;
        }
    ...
    else if (DBC_Cmd(dbc, "mp_deact_link", "Deactivate Link", 
             "<linkset> <linkcode>", ""))
    {
        ret = sscanf(cmdLine, "%s %d %d", buf, &linkSet, &linkCode);
        if (ret != 3)
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n" 
                         "Usage: mp_deact_link <linkset> <linkcode> \n");
            DBC_AppendText(dbc, buf);
            return;
        }
    ...
    else if (DBC_Cmd(dbc, "mp_act_link", "Activate Link", 
             "<linkset> <linkcode>", ""))
    {
        ret = sscanf(cmdLine, "%s %d %d", buf, &linkSet, &linkCode);
        if (ret != 3)
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n" 
                         "Usage: mp_act_link <linkset> <linkcode> \n");
            DBC_AppendText(dbc, buf);
            return;
        }
    ...
    else if (DBC_Cmd(dbc, "mp_start", "Start MTP3", "", ""))
    {
        sprintf(buf, "\n*********** Starting MTP3 **************\n");
        DBC_AppendText(dbc, buf);
    ...
    else if (DBC_Cmd(dbc, "mp_get_ls", "Dump Link Sets", "", ""))
    {
        char *dump = NULL;
        SS7_Family family = FAMILY_CHINA; 
...
#if 0
    else if (DBC_Cmd(dbc, "mp_get_rt", "Dump Routes Info", "<>", "<>"))
    {
        char *dump = NULL;
...
#endif
    else if (DBC_Cmd(dbc, "mp_get_link", "get Link Status", 
             "<linkset> <linkcode>", ""))
    {
        char *dump = NULL;
        ...
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n"
                         "Usage: mp_get_link <linkset> <linkcode>\n");
            DBC_AppendText(dbc, buf);
            return;
        }
    ...
    else if (DBC_Cmd(dbc, "mp_get_gen_peg", "Get MTP3 General Pegs", 
             "<Peg Id>", ""))
    {
        ...
        {
             sprintf(buf,  "\n**** Invalid Command Arguments *******\n"
                           "Usage: mp_get_gen_peg [<Peg Id> or <ALL/all>]\n"); 
             DBC_AppendText(dbc, buf);
             return;
        }
        ...
        {
            /* Invoke function MTP3_GetOm_MML_PRC() */
            MTP3_GetOm_MML_PRC(dbc);
            DumpGlobalPegs(dbc);
            return;
        }
        else
        ...
        if (pgNum >= PEG_MTP3_NUM_PEGS_PRC)
        {
            sprintf(buf, "\n   **** Invalid Peg Id *******\n");
            DBC_AppendText(dbc, buf);
            return;
        ...
        value = PEG_GetPeg(PRC_MTP3_Pegs, pgNum);

        sprintf(buf, "\n   value is %d\n", value);
        DBC_AppendText(dbc, buf);
    }
    else if (DBC_Cmd(dbc, "mp_get_link_peg", "Get Link Level Pegs", 
             "<linkset> <linkcode> <Peg Id>", ""))
    {
        ...
        {
            sprintf(buf,  "\n**** Invalid Command Arguments *******\n"
                          "Usage: mp_get_link_peg <linkset> <linkcode> [<Peg Id> or <ALL/all>]\n");
            DBC_AppendText(dbc, buf);
            return;
        }
    ...
    else if (DBC_Cmd(dbc, "mp_get_route_peg", "Get Route Level Pegs",
             "<dpc> <ni> <Peg Id>", ""))
    {
        char pgName[ITS_PATH_MAX];
        ...
        {
            sprintf(buf,  "\n**** Invalid Command Arguments *******\n" \
                          "Usage: mp_get_dpc_peg <dpc> <ni> \
                                           [<Peg Id> or <ALL/all>]\n");
            DBC_AppendText(dbc, buf);
            return;
        }

        if ((dest = ROUTE_FindDestination(dpc, ni, FAMILY_CHINA)) == NULL)
        {
...
#if 0
    else if (DBC_Cmd(dbc, "mp_get_gen_pegid", "Show all general peg IDs", "", ""))
    {
        DumpGlobalPegId(dbc);
    }
    else if (DBC_Cmd(dbc, "mp_get_link_pegid", "Show all link level peg IDs", "", ""))
    {
        DumpLinkPegId(dbc);
    }
#endif

    else if (DBC_Cmd(dbc, "mp_clr_gen_peg", "MTP3 Reset a particular gen Peg",
                          "[<Pegid> (or) <ALL/all>]",""))
    {
        ...
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n"
                         "Usage: mp_clr_gen_peg [<Pegid> (or) <ALL/all>]\n");
            DBC_AppendText(dbc, buf);
            return;
        }
        ...
        {
            /* Invoke function MTP3_ResetOm_MML_PRC() */
            MTP3_ResetOm_MML_PRC();

            for (i = 0; i < PEG_MTP3_NUM_PEGS_PRC; i++)
            {
                PEG_ClearPeg(PRC_MTP3_Pegs, i);
            }
        ...
        if (pgNum > PEG_MTP3_NUM_PEGS_PRC - 1)
        {
            sprintf(buf, "\n   **** Invalid Peg Id *******\n");
            DBC_AppendText(dbc, buf);
            return;
        ...
        PEG_ClearPeg(PRC_MTP3_Pegs, pgNum);

        sprintf(buf, "Clearing the Peg for %s Now\n", mtp3GPegStr[pgNum]);
        DBC_AppendText(dbc, buf);
    }
    else if (DBC_Cmd(dbc, "mp_clr_link_peg", "MTP3 Clear particular Route Peg",
                          "<linkSet> <linkCode> [<Pegid> (or) <ALL/all>]",""))
    {
        ...
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n"
            "Usage: mp_clr_link_peg <linkset> <linkcode> [<Pegid> (or) <ALL/all>]\n");
            DBC_AppendText(dbc, buf);
            return;
        }
    ...
    else if (DBC_Cmd(dbc, "mp_clr_route_peg", "Clear route Level Pegs",
             "<dpc> <ni> <Peg Id>", ""))
    {
        char pgName[ITS_PATH_MAX];
        SS7_Destination* dest;
...
#if 0
    else if (DBC_Cmd(dbc, "mp_debug_trace", "Debug Trace", "<on, off>", "<>"))
    {
        char onOff[10];
        ...
        if (strstr(onOff, "on") != NULL)
        {
            ...
            TRACE_SetOutputOnOffByName(MTP3_PRC_TraceData,
                                       MTP3_DEBUG_STRING,
                                       MTP3_STDOUT_STRING, ITS_TRUE);


            TRACE_SetLevelOnOffByName(MTP3_PRC_TraceData,
    ...
    else if (DBC_Cmd(dbc, "mp_warning_trace", "Warning Trace", "<on, off>", "<>"
))
    {
        char onOff[10];
    ...
    else if (DBC_Cmd(dbc, "mp_error_trace", "Error Trace", "<on, off>", "<>"))
    {
        char onOff[10];
...
#endif
    else if (DBC_Cmd(dbc, "mp_shutdown", "shutdown mtp3", "", ""))
    {
        ShutDownNode(dbc);
    ...
    else if (DBC_Cmd(dbc, "mp_set_rpc", "set a remote point code",
               "<rpc><ni><ls><f>", "<remote pc><ni><link set><family>"))
    {
       ...
       if (ret != 5)
       {
           sprintf(buf,"*** Error: Invalid parameters, try help mp_set_rpc \n");
           DBC_AppendText(dbc, buf);
           return;
       }
       /*** process family ***/
...
;
    ...
    else if (DBC_Cmd(dbc, "mp_rem_rpc","Remove a remote point code",
         "<rpc><ni><LinkSet><family>", "<remote pc><ni><linkset><family>"))
    {
        ...
        if (ret != 5)
        {
           sprintf(buf, "*** Error: Invalid parameters, try help mp_rem_rpc \n"
);
           DBC_AppendText(dbc, buf);
           return;
        }
...
;
    ...
    else if (DBC_Cmd(dbc, "mp_get_all_rpc", "get all remote PC", "<>", "<>"))
    {
        char *dump = NULL;
    ...
    else if (DBC_Cmd(dbc, "mp_get_all_lpc", "get all local PC", "<>", "<>"))
    {
        char *dump = NULL;
        char* strDumpEntries = NULL;
    ...
    else if (DBC_Cmd(dbc, "mp_lpo", "Local Processor Outage", "<ls> <lc>", "<linkset> <linkcode>"))
    {
        ret = sscanf(cmdLine, "%s %d %d", buf, &linkSet, &linkCode);
        if (ret != 3)
        {
            sprintf(buf, "*** Error: Invalid parameters, try help mp_lpo \n");
            DBC_AppendText(dbc, buf);
            return;
        }
    ...
    else if (DBC_Cmd(dbc, "mp_lpr", "Local Processor Recovery", "<ls> <lc>", "<linkset> <linkcode>"))
    {
        ret = sscanf(cmdLine, "%s %d %d", buf, &linkSet, &linkCode);
        if (ret != 3)
        {
            sprintf(buf, "*** Error: Invalid parameters, try help mp_lpr \n");
            DBC_AppendText(dbc, buf);
            return;
        }
...
{    
    ...
    if (DBC_Cmd(dbc, "mp_il", "Inhibit Link", "<ls> <lc>", "<link set> <link code>"))
    {
        ret = sscanf(cmdLine, "%s %d %d", buf, &linkSet, &linkCode);
        if (ret != 3)
        {
    ...
    else if (DBC_Cmd(dbc, "mp_ul", "Uninhibit Link", "<ls> <lc>", "<link set> <link code>"))
    {
        ret = sscanf(cmdLine, "%s %d %d", buf, &linkSet, &linkCode);
        if (ret != 3)
        {
    ...
    else if (DBC_Cmd(dbc, "mp_dls", "Deactivate Link Set", "<ls>", "<link set>"))
    {
        ret = sscanf(cmdLine, "%s %d", buf, &linkSet);
        if (ret != 2)
        {
    ...
    else if (DBC_Cmd(dbc, "mp_als", "Activate Link Set", "<ls>", "<link set>"))
    {
        ret = sscanf(cmdLine, "%s %d", buf, &linkSet);
        if (ret != 2)
        {
    ...
    else if (DBC_Cmd(dbc, "mp_dln", "Deactivate Link", "<ls> <lc>", "<link set> <link code>"))
    {
        ret = sscanf(cmdLine, "%s %d %d", buf, &linkSet, &linkCode);
        if (ret != 3)
        {
    ...
    else if (DBC_Cmd(dbc, "mp_aln", "Activate Link", "<ls> <lc>", "<link set> <link code>"))
    {
        ret = sscanf(cmdLine, "%s %d %d", buf, &linkSet, &linkCode);
        if (ret != 3)
        {
    ...
    else if (DBC_Cmd(dbc, "mp_dumpls", "Dump Link Sets", "<>", "<>"))
    {
        char *dump = NULL;
        SS7_Family family = FAMILY_CHINA;
    ...
    else if (DBC_Cmd(dbc, "mp_dumprt", "Dump Routes Info", "<>", "<>"))
    {
        char *dump = NULL;
    ...
    else if (DBC_Cmd(dbc, "mp_link_status", "get Link Status", "<ls>", "<lc>"))
    {
        char *dump = NULL;
    ...
    else if (DBC_Cmd(dbc, "mp_get_peg", "Get Pegs", "<pg> ", "<Peg#> <>"))
    {
        ...
        if (pg >= PEG_MTP3_NUM_PEGS_PRC || pg < 0)
        {
            sprintf(buf, "range for peg is %d - %d", 0,PEG_MTP3_NUM_PEGS_PRC-1);
            DBC_AppendText(dbc, buf);
            return ;
        }
        ...
        value = PEG_GetPeg(PRC_MTP3_Pegs, pg);

        sprintf(buf, "value is %d\n", value);
        DBC_AppendText(dbc, buf);
    }
    else if (DBC_Cmd(dbc, "mp_get_link_peg", "Get Link Level Pegs", "<ls> <lc> <p g>", "<link set> <link code><Peg#>"))
    {
    ...
    else if (DBC_Cmd(dbc, "mp_show_peg_ids", "Dump Peg Ids", "<>", "<>"))
    {
        DumpGlobalPegId(dbc);
        DumpLinkPegId(dbc);

    }
    else if (DBC_Cmd(dbc, "mp_clr_gen_pegs", "MTP3 Reset a particular gen Peg",
                          "[<Pegid> (or) <All/all>]",""))
    {
        char pegType[ITS_PATH_MAX];
        ...
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n"
                         "Usage: mp_clr_gen_pegs [<Pegid> (or) <All/all>]\n");
            DBC_AppendText(dbc, buf);
            return;
        }
        ...
        if ( atoi(pegType) > PEG_MTP3_NUM_PEGS_PRC - 1)
        {
    ...
    else if (DBC_Cmd(dbc, "mp_clr_link_pegs", "MTP3 Clear particular Link Peg",
                          "<linkset> <linkcode> [<Pegid> (or) <All/all>]",""))
    {
        char pegType[ITS_PATH_MAX];
        ret = sscanf(cmdLine, "%s %d %d %s", buf, &linkSet, &linkCode,pegType);
        ...
        {
            sprintf(buf, "\n**** Invalid Command Arguments *******\n"
            "Usage: mp_clr_link_pegs <linkset> <linkcode> [<Pegid> (or) <All/all>]\n");
            DBC_AppendText(dbc, buf);
            return;
        }
...
DumpRemotePointCodeData(ITS_POINTER data, void *in, void *out)
{
    ...
    for (i = 0; i < ss7Context->numEntries; i++)
    {
       if ( ss7Context->entries[i]->basic.type == REMOTE_ROUTE &&
              ss7Context->entries[i]->basic.family == FAMILY_CHINA )
       {
           strcat(*pStrDump, "------------------------------\n");
...
DumpGlobalPegs(DBC_Server *dbc)
{
     ...
     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_MSG_PAUSE),
              PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_MSG_PAUSE));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_MSG_RESUME),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_MSG_RESUME));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_MSG_STATUS),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_MSG_STATUS));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_MSG_RECEIVED_INERROR),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_MSG_RECEIVED_INERROR));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_CHANGEOVER),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_CHANGEOVER));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_CHANGEBACK),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_CHANGEBACK));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_SL_UNAVAILABLE),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_SL_UNAVAILABLE));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_LINK_INHIBIT),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_INHIBIT));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_LINK_UNINHIBIT),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_UNINHIBIT));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_LINK_FORCE_UNINHIBIT),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_FORCE_UNINHIBIT));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_CONGESTION),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_CONGESTION));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_SLS_UNAVAILABLE),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_SLS_UNAVAILABLE));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_TFC_RECEIVED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_TFC_RECEIVED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_TFA_RECEIVED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_TFA_RECEIVED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_ROUTESET_UNAVAILABLE),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_ROUTESET_UNAVAILABLE));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_ADJECENT_SP_UNACCESIBLE),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_ADJECENT_SP_UNACCESIBLE));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_USERPART_ENABLE),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_USERPART_ENABLE));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T1_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T1_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T2_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T2_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T3_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T3_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T4_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T4_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T5_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T5_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T6_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T6_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T7_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T7_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T8_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T8_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T9_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T9_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T10_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T10_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T11_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T11_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T12_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T12_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T13_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T13_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T14_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T14_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T15_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T15_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T16_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T16_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T17_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T17_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T18_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T18_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T19_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T19_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T20_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T20_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T21_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T21_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T22_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T22_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T23_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T23_EXPIRED));

     DBC_AppendText(dbc, buf);

     sprintf(buf,"%-35s  :  %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T24_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T24_EXPIRED));

     DBC_AppendText(dbc, buf);

#if 0
     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T25_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T25_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T26_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T26_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T27_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T27_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T28_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T28_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T29_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T29_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T30_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T30_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T31_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T31_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T32_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T32_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T33_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T33_EXPIRED));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T34_EXPIRED),
             PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T34_EXPIRED));
     DBC_AppendText(dbc, buf);
#endif
#endif
    return ITS_SUCCESS;
...
DumpGlobalPegId(DBC_Server *dbc)
{
     ...
     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_MSG_PAUSE),
             PEG_MTP3_MSG_PAUSE, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_MSG_PAUSE));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_MSG_RESUME),
             PEG_MTP3_MSG_RESUME, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_MSG_RESUME));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_MSG_STATUS),
             PEG_MTP3_MSG_STATUS, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_MSG_STATUS));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_INITIALIZE),
             PEG_MTP3_INITIALIZE, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_INITIALIZE));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_TERMINATE),
             PEG_MTP3_TERMINATE, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_TERMINATE));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_MSG_RECEIVED_INERROR),
             PEG_MTP3_MSG_RECEIVED_INERROR, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_MSG_RECEIVED_INERROR));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_CHANGEOVER),
             PEG_MTP3_CHANGEOVER, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_CHANGEOVER));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_CHANGEBACK),
             PEG_MTP3_CHANGEBACK, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_CHANGEBACK));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_SL_UNAVAILABLE),
             PEG_MTP3_SL_UNAVAILABLE, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_SL_UNAVAILABLE));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_LINK_INHIBIT),
             PEG_MTP3_LINK_INHIBIT, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_INHIBIT));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_LINK_UNINHIBIT),
             PEG_MTP3_LINK_UNINHIBIT, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_UNINHIBIT));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_LINK_FORCE_UNINHIBIT),
             PEG_MTP3_LINK_FORCE_UNINHIBIT, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_FORCE_UNINHIBIT));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_CONGESTION),
             PEG_MTP3_CONGESTION, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_CONGESTION));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_SLS_UNAVAILABLE),
             PEG_MTP3_SLS_UNAVAILABLE, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_SLS_UNAVAILABLE));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_TFC_RECEIVED),
             PEG_MTP3_TFC_RECEIVED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_TFC_RECEIVED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_TFA_RECEIVED),
             PEG_MTP3_TFA_RECEIVED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_TFA_RECEIVED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_ROUTESET_UNAVAILABLE),
             PEG_MTP3_ROUTESET_UNAVAILABLE, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_ROUTESET_UNAVAILABLE));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_ADJECENT_SP_UNACCESIBLE),
             PEG_MTP3_ADJECENT_SP_UNACCESIBLE, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_ADJECENT_SP_UNACCESIBLE));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_USERPART_ENABLE),
             PEG_MTP3_USERPART_ENABLE, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_USERPART_ENABLE));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_USERPART_DISABLE),
             PEG_MTP3_USERPART_DISABLE, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_USERPART_DISABLE));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T1_EXPIRED),
             PEG_MTP3_T1_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T1_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T2_EXPIRED),
             PEG_MTP3_T2_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T2_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T3_EXPIRED),
             PEG_MTP3_T3_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T3_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T4_EXPIRED),
             PEG_MTP3_T4_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T4_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T5_EXPIRED),
             PEG_MTP3_T5_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T5_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T6_EXPIRED),
             PEG_MTP3_T6_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T6_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T7_EXPIRED),
             PEG_MTP3_T7_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T7_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T8_EXPIRED),
             PEG_MTP3_T8_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T8_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T9_EXPIRED),
             PEG_MTP3_T9_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T9_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T10_EXPIRED),
             PEG_MTP3_T10_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T10_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T11_EXPIRED),
             PEG_MTP3_T11_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T11_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T12_EXPIRED),
             PEG_MTP3_T12_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T12_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T13_EXPIRED),
             PEG_MTP3_T13_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T13_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T14_EXPIRED),
             PEG_MTP3_T14_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T14_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T15_EXPIRED),
             PEG_MTP3_T15_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T15_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T16_EXPIRED),
             PEG_MTP3_T16_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T16_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T17_EXPIRED),
             PEG_MTP3_T17_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T17_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T18_EXPIRED),
             PEG_MTP3_T18_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T18_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T19_EXPIRED),
             PEG_MTP3_T19_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T19_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T20_EXPIRED),
             PEG_MTP3_T20_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T20_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T21_EXPIRED),
             PEG_MTP3_T21_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T21_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T22_EXPIRED),
             PEG_MTP3_T22_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T22_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T23_EXPIRED),
             PEG_MTP3_T23_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T23_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T24_EXPIRED),
             PEG_MTP3_T24_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T24_EXPIRED));
     strcat(buf1, buf);

#if 0
     /* Commented as per spec */
     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T25_EXPIRED),
             PEG_MTP3_T25_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T25_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T26_EXPIRED),
             PEG_MTP3_T26_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T26_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T27_EXPIRED),
             PEG_MTP3_T27_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T27_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T28_EXPIRED),
             PEG_MTP3_T28_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T28_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T29_EXPIRED),
             PEG_MTP3_T29_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T29_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T30_EXPIRED),
             PEG_MTP3_T30_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T30_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T31_EXPIRED),
             PEG_MTP3_T31_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T31_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T32_EXPIRED),
             PEG_MTP3_T32_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T32_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T33_EXPIRED),
             PEG_MTP3_T33_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T33_EXPIRED));
     strcat(buf1, buf);

     sprintf(buf, "%12s %5d: value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_T34_EXPIRED),
             PEG_MTP3_T34_EXPIRED, PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_T34_EXPIRED));
     strcat(buf1, buf);
#endif
#endif
...
MTP3_OSS_GetGeneralCfg_PRC(MTP3_GeneralCfg* genCfg)
{
    if (genCfg == NULL)
        return ITS_ENOMEM;

    genCfg->alarmLevel = (MGMT_AlarmLevel)MTP3_AlarmLevel_PRC;

    genCfg->debugTrace = TRACE_IsLevelOn(MTP3_PRC_TraceData, MTP3_TRACE_DEBUG); 
    if (genCfg->debugTrace)
    {
        genCfg->debugTraceOutput = 0;
        if (TRACE_IsOutputOn(MTP3_PRC_TraceData, MTP3_TRACE_DEBUG, 0))
        {
            genCfg->debugTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(MTP3_PRC_TraceData, MTP3_TRACE_DEBUG, 1))
        {
            genCfg->debugTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(MTP3_PRC_TraceData, MTP3_TRACE_DEBUG, 2))
        {
    ...
    genCfg->warningTrace = TRACE_IsLevelOn(MTP3_PRC_TraceData, MTP3_TRACE_WARNING); 
    if (genCfg->warningTrace)
    {
        genCfg->warningTraceOutput = 0;
        if (TRACE_IsOutputOn(MTP3_PRC_TraceData, MTP3_TRACE_WARNING, 0))
        {
            genCfg->warningTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(MTP3_PRC_TraceData, MTP3_TRACE_WARNING, 1))
        {
            genCfg->warningTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(MTP3_PRC_TraceData, MTP3_TRACE_WARNING, 2))
        {
    ...
    genCfg->errorTrace = TRACE_IsLevelOn(MTP3_PRC_TraceData, MTP3_TRACE_ERROR); 
    if (genCfg->errorTrace)
    {
        genCfg->errorTraceOutput = 0;
        if (TRACE_IsOutputOn(MTP3_PRC_TraceData, MTP3_TRACE_ERROR, 0))
        {
            genCfg->errorTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(MTP3_PRC_TraceData, MTP3_TRACE_ERROR, 1))
        {
            genCfg->errorTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(MTP3_PRC_TraceData, MTP3_TRACE_ERROR, 2))
        {
    ...
    genCfg->criticalTrace = TRACE_IsLevelOn(MTP3_PRC_TraceData, MTP3_TRACE_ABORT); 
    if (genCfg->criticalTrace)
    {
        genCfg->criticalTraceOutput = 0;
        if (TRACE_IsOutputOn(MTP3_PRC_TraceData, MTP3_TRACE_ABORT, 0))
        {
            genCfg->criticalTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(MTP3_PRC_TraceData, MTP3_TRACE_ABORT, 1))
        {
            genCfg->criticalTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(MTP3_PRC_TraceData, MTP3_TRACE_ABORT, 2))
        {
    ...
    genCfg->eventTrace = TRACE_IsLevelOn(MTP3_PRC_TraceData, MTP3_TRACE_EVENT); 
    if (genCfg->eventTrace)
    {
        genCfg->eventTraceOutput = 0;
        if (TRACE_IsOutputOn(MTP3_PRC_TraceData, MTP3_TRACE_EVENT, 0))
        {
            genCfg->eventTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(MTP3_PRC_TraceData, MTP3_TRACE_EVENT, 1))
        {
            genCfg->eventTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(MTP3_PRC_TraceData, MTP3_TRACE_EVENT, 2))
        {
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_SetGeneralCfg_PRC(MTP3_GeneralCfg* genCfg)
{
    if (genCfg == NULL)
        return ITS_ENOMEM;

    MTP3_AlarmLevel_PRC = genCfg->alarmLevel;

    if (genCfg->debugTrace)
    {
        TRACE_SetLevelOnOffByName(MTP3_PRC_TraceData, MTP3_DEBUG_STRING,  ITS_TRUE); 
        ...
        if (genCfg->debugTraceOutput & TRACE_TO_NONE)
        {
            ...
            TRACE_SetLevelOnOffByName(MTP3_PRC_TraceData, MTP3_DEBUG_STRING,  ITS_FALSE); 
        }
    }

    if (genCfg->warningTrace)
    {
        TRACE_SetLevelOnOffByName(MTP3_PRC_TraceData, MTP3_WARNING_STRING,  ITS_TRUE); 
        ...
        if (genCfg->warningTraceOutput & TRACE_TO_NONE)
        {
            ...
            // Set trace level to false
            TRACE_SetLevelOnOffByName(MTP3_PRC_TraceData, MTP3_WARNING_STRING,  ITS_FALSE); 
        }
    }

    if (genCfg->errorTrace)
    {
        TRACE_SetLevelOnOffByName(MTP3_PRC_TraceData, MTP3_ERROR_STRING,  ITS_TRUE); 
        ...
        if (genCfg->errorTraceOutput & TRACE_TO_NONE)
        {
            ...
            // Set trace level to false
            TRACE_SetLevelOnOffByName(MTP3_PRC_TraceData, MTP3_ERROR_STRING,  ITS_FALSE); 
        }
    }

    if (genCfg->criticalTrace)
    {
        TRACE_SetLevelOnOffByName(MTP3_PRC_TraceData, MTP3_CRITICAL_STRING,  ITS_TRUE); 
        ...
        if (genCfg->criticalTraceOutput & TRACE_TO_NONE)
        {
            ...
            // Set trace level to false
            TRACE_SetLevelOnOffByName(MTP3_PRC_TraceData, MTP3_CRITICAL_STRING,  ITS_FALSE); 
        }
    }

    if (genCfg->eventTrace)
    {
        TRACE_SetLevelOnOffByName(MTP3_PRC_TraceData, MTP3_EVENT_STRING,  ITS_TRUE); 
        ...
        if (genCfg->eventTraceOutput & TRACE_TO_NONE)
        {
            ...
            // Set trace level to false
            TRACE_SetLevelOnOffByName(MTP3_PRC_TraceData, MTP3_EVENT_STRING,  ITS_FALSE); 
        }
    }
 ...
 ****************************************************************************/
ITSSS7DLLAPI void
MTP3_OSS_SetBehavior_PRC(ITS_BOOLEAN isStp, ITS_BOOLEAN handleSpareNic)
{
    BEHAVIORS_SetBehavior(&PRC_MTP3_Behaviors, MTP3_B_stpFunction, isStp);
    MTP3_PRC_HandleSpareNIC = handleSpareNic;    
}
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_GetBehavior_PRC(ITS_BOOLEAN* isStp, ITS_BOOLEAN* handleSpareNic)
{
    if (isStp == NULL || handleSpareNic == NULL)
        return ITS_ENOMEM;

    *isStp = BEHAVIORS_GetBehavior(&PRC_MTP3_Behaviors, MTP3_B_stpFunction);
    *handleSpareNic = MTP3_PRC_HandleSpareNIC; 
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_InhibitLink_PRC(ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    SS7_LinkPtr ln;
    ITS_INT     ret = ITS_SUCCESS; 
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_UninhibitLink_PRC(ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    SS7_LinkPtr ln;
    ITS_INT     ret = ITS_SUCCESS; 
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_DeactivateLink_PRC(ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    SS7_LinkPtr ln;
    ITS_INT     ret = ITS_SUCCESS; 
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_ActivateLink_PRC(ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    SS7_LinkPtr ln;
    ITS_INT     ret = ITS_SUCCESS; 
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_DeactivateLinkset_PRC (ITS_OCTET linkSet)
{
    SS7_LinkSetPtr  ls;
    ITS_INT         ret = ITS_SUCCESS; 
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_ActivateLinkset_PRC (ITS_OCTET linkSet)
{
    SS7_LinkSetPtr  ls;
    ITS_INT         ret = ITS_SUCCESS; 
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_GetLinkset_PRC(ITS_OCTET linkSet, SS7_LinkSetInfo* linkSetInfoPtr, ITS_OCTET* numLinksets)
{
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_GetLink_PRC(ITS_OCTET linkSet, ITS_OCTET linkCode, SS7_LinkInfo* linkInfoPtr)
{
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT
MTP3_OSS_GetGeneralPegs_PRC(MTP3_GenPegs* globalPegs)
{
    if (globalPegs == NULL)
        return ITS_ENOMEM;

    globalPegs->msgPause             = PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_MSG_PAUSE);
    globalPegs->msgResume            = PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_MSG_RESUME);
    globalPegs->msgStatus            = PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_MSG_STATUS);
    globalPegs->initialize           = PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_INITIALIZE);
    globalPegs->terminate            = PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_TERMINATE);
    globalPegs->msgReceivedInError   = PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_MSG_RECEIVED_INERROR);
    globalPegs->changeover           = PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_CHANGEOVER);
    globalPegs->changeback           = PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_CHANGEBACK);
    globalPegs->slUnavailable        = PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_SL_UNAVAILABLE);
    globalPegs->linkInhibit          = PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_INHIBIT);
    globalPegs->linkUnInhibit        = PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_UNINHIBIT);
    globalPegs->linkforceUninhibit   = PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_FORCE_UNINHIBIT);
    globalPegs->congestion           = PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_CONGESTION);
    globalPegs->slsUnavailable       = PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_SLS_UNAVAILABLE);
    globalPegs->tfcReceived          = PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_TFC_RECEIVED);
    globalPegs->tfaReceived          = PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_TFA_RECEIVED);
    globalPegs->routesetUnavailable  = PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_ROUTESET_UNAVAILABLE);
    globalPegs->adjacentSpUnaccesible= PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_ADJECENT_SP_UNACCESIBLE);
    globalPegs->userpartEnable       = PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_USERPART_ENABLE);
    globalPegs->userpartDisable      = PEG_GetPeg(PRC_MTP3_Pegs, PEG_MTP3_USERPART_DISABLE);
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_GetLinkPegs_PRC(ITS_OCTET linkSet, ITS_OCTET linkCode, MTP3_LinkPegs* linkPegs)
{
    SS7_LinkPtr ln = NULL;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT
MTP3_OSS_GetDpcPegs_PRC(ITS_UINT pc, ITS_UINT ni, MTP3_RoutePegs* routePegs)
{
    SS7_Destination* dest;


    if ((dest = ROUTE_FindDestination(pc, ni, FAMILY_CHINA)) == NULL)
    {
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_ClearGeneralPeg_PRC(ITS_UINT pegId)
{
    if (pegId == MTP3_ALL_PEGS)
    {
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_INITIALIZE);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_TERMINATE);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_RESTART);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_MSG_PAUSE); 
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_MSG_RESUME);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_MSG_STATUS);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_OUT_OF_SERVICE);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_IN_SERVICE);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_CHANGEOVER);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_EMER_CHANGEOVER);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_CHANGEBACK);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_INHIBIT);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_INHIBIT_DNY);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_UNINHIBIT);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_FORCE_UNINHIBIT);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_BLOCK);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_UNBLOCK);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_ACTIVATION);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_DEACTIVATION);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_CONGESTION_DETECT);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_MSU_TX);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_MSU_RX);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_SLTM_TX);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_SLTM_RX);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_SLTA_TX);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_SLTA_RX);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_SLT_PASSED);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_SLT_FAILED);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_TFP_TX);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_TFP_RX);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_TFA_TX);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_TFA_RX);
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_REMOTE_ROUTE_FAILURE );
        PEG_ClearPeg(PRC_MTP3_Pegs, PEG_MTP3_USERPART_ROUTE_FAILURE );
    }
    else
    {
        if (pegId > PEG_MTP3_NUM_PEGS_PRC)
        {
        ...
        {
            PEG_ClearPeg(PRC_MTP3_Pegs, pegId);
        }
    }
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_ClearLinkPeg_PRC(ITS_OCTET linkSet, ITS_OCTET linkCode, ITS_UINT pegId)
{
    SS7_LinkPtr ln = NULL;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT
MTP3_OSS_ClearDpcPeg_PRC(ITS_UINT pc, ITS_UINT ni, ITS_UINT pegId)
{
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT
MTP3_OSS_AddLinkset_PRC(ITS_OCTET linkSet, SS7_Family family, ITS_UINT lpc, ITS_UINT apc, ITS_UINT ni)
{
    ITS_INT         ret;
    SS7_LinkSetPtr  ls;
    SS7_LinkSet      linkset;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_RemoveLinkset_PRC(ITS_OCTET linkSet)
{
    ITS_INT        ret;
    SS7_LinkSetPtr  ls;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_AddLink_PRC(ITS_OCTET linkSet, ITS_OCTET linkCode, ITS_UINT transId)
{
    ITS_INT             ret;
    SS7_LinkPtr         lp;
    SS7_LinkSetPtr      ls;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_RemoveLink_PRC(ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ITS_INT         ret;
    SS7_LinkPtr     lp;
    SS7_LinkSetPtr  ls;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_AddRemotePC_PRC(ITS_UINT rpc, ITS_OCTET ni, ITS_OCTET linkSet, SS7_Family family)
{
    ITS_INT         ret;
    SS7_LinkSetPtr  ls;
    SS7_LinkPtr     lp;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_RemoveRemotePC_PRC(ITS_UINT rpc, ITS_OCTET ni, ITS_OCTET linkSet, SS7_Family family)
{
    ITS_INT             ret;
    SS7_LinkSetPtr      ls;
    SS7_Destination     *rc;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_GetRemotePC_PRC(SS7_DestInfo *remPc, ITS_OCTET* numDests)
{
    ITS_INT     ret;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
MTP3_OSS_GetLocalPC_PRC(SS7_DestInfo *localPc, ITS_OCTET* numDests)
{
    ITS_INT     ret;
 ...
 *
 ****************************************************************************/
void MTP3_GetOm_PRC(mtp3Om *OmMtp3Data)
{
...
/****************************************************************************
 *  Purpose:
 *      This API is invoked by the mp_clr_gen_peg() MML command. This will
 *      clear the ACTIVE MTP3 OM Buffer.
 *
...
/****************************************************************************
 *  Purpose:
 *      This API is invoked by the mp_get_gen_peg() MML command.  This will
 *      clear the INACTIVE MTP3 OM Buffer, toggle the ACTIVE/INACTIVE
 ...
 *                           D0380 - D0460
 *
 * mshanmugam  11-04-2007                        To display only PRC link pegs,
 *                                               family check is added.    
 *                                               To allow linkset no upto 255
 *                                               in  the for loop.
 ****************************************************************************/

void MTP3_GetOm_MML_PRC(DBC_Server *dbc)
{
  char buf[32000];
  char buf1[32000];
