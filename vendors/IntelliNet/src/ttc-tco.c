 ...
 ****************************************************************************
 *
 *  ID: $Id: ttc-tco.c,v 9.2 2008/06/04 10:38:48 ssingh Exp $
 *
 * LOG: $Log: ttc-tco.c,v $
 ...
 * LOG:
 * LOG: Revision 1.1.2.3  2007/02/08 14:40:25  raghavendrabk
 * LOG: Changed alarm format to put family at beginning
 * LOG:
 * LOG: Revision 1.1.2.2  2007/02/02 14:22:07  raghavendrabk
...
#ident "$Id: ttc-tco.c,v 9.2 2008/06/04 10:38:48 ssingh Exp $"
 ...
 ****************************************************************************/
int
TCOHandleCSLMsg_TTC(ITS_CTXT did, ITS_SCCP_IE *ies, int ieCount)
{
    return TSMHandleTCOCSLMsg_TTC(did, ies, ieCount);
}
 ...
 ****************************************************************************/
int
TCOHandleSCCPMsg_TTC(ITS_EVENT *event)
{
    ...
    default:
        ...
        TCAP_Alarm_TTC(1446, __FILE__, __LINE__, "family=%s", "TTC");
       
        PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_INVALID_SCCP_RCVD);

        return (ITS_SUCCESS);
    }

    if (FindPTypeAndTID_TTC(ies, ieCount,
                              ...
                              &cdp, &clp) == ITS_NO)
    {
        ...
        if (!pType)
        {
            ...
            TCAP_Alarm_TTC(1448, __FILE__, __LINE__,
                           "family=%s:PType=%x:Tx-id=%x", "TTC", pType, tid2);
        ...
        else
        {
            ...
            TCAP_Alarm_TTC(1453, __FILE__, __LINE__, "family=%s:PType=%x", "TTC", 
                           pType);
    ...
    {
        sharedTx = TCAP_FindTransactionContext_TTC(tid1,
                                                     tidKey, &tidKeyLen);
        if (sharedTx != NULL)
        {
        ...
        if (localTx != NULL)
        {
            DHAHandleTCOMsg_TTC(ies, ieCount, localTx);
        }
    ...
    case TCPPT_TC_UNI:
        ...
        if (DHAHandleTCOMsg_TTC(ies, ieCount, NULL) != ITS_SUCCESS)
        {
    ...
    case TCPPT_TC_BEGIN:
        ...
        if (localTx == NULL)
        {
            ...
            tcapOmImage[omBuf.activeOm].errorsDetectedInTP++;
            TCAP_Alarm_TTC(1447, __FILE__, __LINE__,
                           "family=%s:Transaction-id=%x:PType=%x", "TTC", tid2, pType); 
 
            return (ITS_SUCCESS);
        }

        AllocateLTIDinTx_TTC(localTx);

        ret = TCAP_PopulateLocalTx_TTC(localTx, did, tid1, tid1Len, &cdp, &clp);
        ...
        ret = TSMHandleTCOSCCPMsg_TTC(pType, ies, ieCount, &localTx);
 ...
 */
        ...
        if (ret == ITS_SUCCESS)
        {
            sharedTx = TCAP_AllocTransactionContext_TTC(localTx->ltid,
                                                          tidKey, &tidKeyLen);
    ...
    case TCPPT_TC_CONTINUE:
        ...
        sharedTx = TCAP_FindTransactionContext_TTC(tid2, tidKey, &tidKeyLen);
        ...
        if (sharedTx)
        {
            if (sharedTx->rtidLen)
            {
                ...
                if (tid1 != sharedTx->rtid)
                {
                    ...
                    TCAP_Alarm_TTC(1453, __FILE__, __LINE__,
                                   "family=%s:PType=%x", "TTC", pType); 
            ...
            */  
            ...
            if (ret != ITS_SUCCESS)
            {
                TCAP_ERROR(("TCOHandleSCCPMsg_TTC: DSM_Commit failed "
                            "returned code: %d\n", ret));
                tcapOmImage[omBuf.activeOm].errorsDetectedInTP++;
        ...
        else
        {
            ...
            /* Incoming Dialogue - Discarding Dialogue -
               Rejection Sent by stack. */
           TCAP_Alarm_TTC(1453, __FILE__, __LINE__, "family=%s:PType=%x", "TTC", pType);
           TCAP_Alarm_TTC(1447, __FILE__, __LINE__,
                          "family=%s:Transaction-id=%x:PType=%x", "TTC", tid2, pType);
    ...
    default: 
        ...
        TCAP_Alarm_TTC(1448, __FILE__, __LINE__, "family=%s:PType=%x:Tx-id=%x", "TTC", 
                       pType, tid2);  
    ...
    if (localTx != NULL)  /* only if it was not freed before */
    {
       ...
       {
           TCAP_ERROR(("TCOHandleSCCPMsg_TTC: SharedTx is NULL for did %d ",
                        localTx->did));
            tcapOmImage[omBuf.activeOm].errorsDetectedInTP++;
           TCAP_FreeLocalTx_TTC(localTx->did);
       }
    }
    else
    {
           TCAP_DEBUG(("TCOHandleSCCPMsg_TTC: LocalTx has been freed for "
                       "did %d ",did));
    }
#endif
