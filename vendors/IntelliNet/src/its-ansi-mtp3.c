 ...
 ****************************************************************************
 *
 *  ID: $Id: its-ansi-mtp3.c,v 9.2 2008/06/04 11:02:19 ssingh Exp $
 *
 *  LOG: $Log: its-ansi-mtp3.c,v $
 *  LOG: Revision 9.2  2008/06/04 11:02:19  ssingh
 *  LOG: ALU porting, B-0100LX-LUSG-00 (june 04,2008 base)
 *  LOG:
 *  LOG: Revision 9.1.8.4  2007/05/18 11:35:39  raghavendrabk
 *  LOG: Changes for Alarm Optimisation
 *  LOG:
 *  LOG: Revision 9.1.8.3  2007/03/21 12:02:24  sdwivedi
 *  LOG: Fix for Parallel Stack Issue #5230
 *  LOG:
 *  LOG: Revision 9.1.8.2  2007/02/05 06:04:33  clin
 *  LOG: for MTP3_Resume, need to check each entry in ROUTE_Info, only care about
 *  LOG: REMOTE_ROUTE entry
 *  LOG:
 *  LOG: Revision 9.1.8.1  2007/02/02 14:10:39  raghavendrabk
 ...
 *  LOG:
 *  LOG: Revision 8.19  2003/11/24 09:12:10  kannanp
 *  LOG:  Fix Bug 490 trace problem.
 *  LOG:
 *  LOG: Revision 8.18  2003/08/04 13:26:44  ttipatre
 *  LOG: Bug for 401,404 and other pegs not getting increamented.
 *  LOG:
 *  LOG: Revision 8.17  2003/07/22 15:43:46  mmiers
 *  LOG: Warning removal.  Fix Windoze build.
 *  LOG:
 *  LOG: Revision 8.16  2003/07/17 08:48:53  ttipatre
 *  LOG: MML Modification.
 *  LOG:
 *  LOG: Revision 8.15  2003/06/16 13:49:29  mmanikandan
 *  LOG: Xml Persistency is added.
 *  LOG:
 *  LOG: Revision 8.14  2003/06/13 07:58:28  ttipatre
 *  LOG: Pegs Output Justified.
 *  LOG:
 *  LOG: Revision 8.13  2003/06/05 11:57:16  ttipatre
 *  LOG: adding/updating MML commands
 *  LOG:
 *  LOG: Revision 8.12  2003/05/19 13:25:48  ssanyal
 *  LOG: Alarms output causing core dump: temporarily commented out
 *  LOG:
 *  LOG: Revision 8.11  2003/05/19 06:32:21  ttipatre
 *  LOG: Alarms updated.
 *  LOG:
 *  LOG: Revision 8.10  2003/04/07 21:07:20  mmiers
 *  LOG: Warning removal.
 *  LOG:
 *  LOG: Revision 8.9  2003/04/01 17:05:16  randresol
 *  LOG: Change naming convention for Debug Console commands
 *  LOG:
 *  LOG: Revision 8.8  2003/03/06 19:11:42  mmiers
 *  LOG: Convert to new tracing, simply by removing file names from
 *  LOG: existing trace subsystems.
 *  LOG:
 *  LOG: Revision 8.7  2003/02/26 21:02:22  mmiers
 *  LOG: Warning removal.
 *  LOG:
 *  LOG: Revision 8.6  2003/02/26 17:01:16  mmiers
 *  LOG: Warning removal, extra library generation.
 *  LOG:
 *  LOG: Revision 8.5  2003/02/26 02:00:45  randresol
 *  LOG: log mtp3 alarms to file
 *  LOG:
 *  LOG: Revision 8.4  2003/02/25 22:58:07  randresol
 ...
 *  LOG:
 *  LOG: Revision 8.2  2003/02/17 05:32:04  ttipatre
 *  LOG: Typo Rectified.
 *  LOG:
...
#ident "$Id: its-ansi-mtp3.c,v 9.2 2008/06/04 11:02:19 ssingh Exp $"
...
static void MTP3_Console_ANSI(DBC_Server *dbc, const char *cmdLine);
                     ...
                     count++, tmp = DSMQUEUE_NEXT(link->linkSet->routes, tmp) )
                {
                    ...
                    if (allDown)
                    {
                        ...
                        MTP3_Alarm_ANSI(2001, __FILE__, __LINE__, 
                                        "pc=%x:family=%s", rinfo->basic.dpc, "ANSI");
 ...
 ****************************************************************************/
int
INTELLINET_MTP3_Initialize_ANSI(ITS_SS7_HANDLE handle)
{
    int i, j, ret;
    ITS_Class dbClass;
    RESFILE_Manager *res;
    ...
    res = TRANSPORT_RES(__MTP3_ANSI_RcvQueue);
    name = TRANSPORT_NAME(__MTP3_ANSI_RcvQueue);

    ret = MTP3_Init_ANSI(res, name);
    if (ret != ITS_SUCCESS)
    {
 ...
 ****************************************************************************/
void
INTELLINET_MTP3_Terminate_ANSI(ITS_SS7_HANDLE handle)
{
    int i;
    ITS_EVENT ev;
    void *status;
        ...
        default:
            ...
            else
            {
                                          ...
                                          buf, &len) != ITS_SUCCESS)
                {
                    ...
                    /*MTP3_Alarm_ANSI(2005, __FILE__, __LINE__, "family=%s", "ANSI");*/
                }
                else
                {
...
DumpGlobalPegs(DBC_Server *dbc)
{
     ...
     sprintf(buf, "%12s:             value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_INITIALIZE),
             PEG_GetPeg(ANSI_MTP3_Pegs, PEG_MTP3_INITIALIZE));
     DBC_AppendText(dbc, buf);

     sprintf(buf, "%12s:              value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_TERMINATE),
             PEG_GetPeg(ANSI_MTP3_Pegs, PEG_MTP3_TERMINATE));
     DBC_AppendText(dbc, buf);
     ...
     sprintf(buf, "%12s:        value %d\n", GLOBAL_PEGID_TO_TEXT(PEG_MTP3_USERPART_DISABLE),
             PEG_GetPeg(ANSI_MTP3_Pegs, PEG_MTP3_USERPART_DISABLE));
     DBC_AppendText(dbc, buf);
...
{    
    ...
    if (DBC_Cmd(dbc, "ma_get_gen_cfg", "MTP3 Get General Config", "", ""))
    {
        ...
        if ( cfg->traceOn)
        {
                                    ...
                                    MTP3_TRACE_DEBUG))
           {
               ...
               else if (!TRACE_IsOutputOn(MTP3_ANSI_TraceData,
                                      MTP3_TRACE_DEBUG,
                                      0))
               {
    ...
    else if (DBC_Cmd(dbc, "ma_get_gen_pegs", "Get MTP3 General Peg", 
             "<Peg>", ""))
    {
        ...
        {
             sprintf(buf,  "\n**** Invalid Command Arguments *******\n"
                           "Usage: ma_get_gen_pegs [<Peg> or <All/all>]\n");
             DBC_AppendText(dbc, buf);
             return;
        }
    ...
    else if (DBC_Cmd(dbc, "ma_get_link_pegs", "Get Link Level Pegs", 
            "<linkset> <linkcode> <Peg>", ""))
    {
        ...
        {
            sprintf(buf,  "\n**** Invalid Command Arguments *******\n" 
                          "Usage: ma_get_link_pegs <linkset> <linkcode> <Peg>\n");
            DBC_AppendText(dbc, buf);
            return;
        }
