 ...
 ****************************************************************************
 *
 *  ID: $Id: ttc-tsm.c,v 9.2 2008/06/04 10:38:48 ssingh Exp $
 *
 * LOG: $Log: ttc-tsm.c,v $
 ...
 * LOG:
 * LOG: Revision 1.1.2.3  2007/02/08 14:40:50  raghavendrabk
 * LOG: Changed alarm format to put family at beginning
 * LOG:
 * LOG: Revision 1.1.2.2  2007/02/02 14:22:25  raghavendrabk
...
#ident "$Id: ttc-tsm.c,v 9.2 2008/06/04 10:38:48 ssingh Exp $"
 ...
 ****************************************************************************/
int
TSMHandleTCOCSLMsg_TTC(ITS_CTXT did, ITS_SCCP_IE *ies, int ieCount)
{
    ...
    trCtxt = TCAP_FindLocalTx_TTC(did);
    if (trCtxt == NULL)
    {
    ...
    else if (trCtxt->state == ITS_TCAP_IDLE)
    {
        ...
        case TCPPT_TC_UNI:
            ...
            PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_UNI_SENT);
        ...
        case TCPPT_TC_BEGIN:
            ...
            sharedTx = TCAP_AllocTransactionContext_TTC(trCtxt->ltid,
                                                          tidKey, &tidKeyLen);
            if (sharedTx == NULL)
            {
            ...
            PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_BEGIN_SENT);
            PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_MSG_SENT);
            tcapOmImage[omBuf.activeOm].appMsgSent++;
        ...
        default:
            ...
            /* Outgoing Message - Discarding Message - Invalid State. */
            TCAP_Alarm_TTC(1443, __FILE__, __LINE__,
                           "family=%s:PType=%x:Dialog-Id=%x", "TTC", pType, did);
    ...
    else if (trCtxt->state == ITS_TCAP_BEGIN_RCVD)
    {
        ...
        case TCPPT_TC_CONTINUE:
            ...
            sharedTx = TCAP_FindTransactionContext_TTC(trCtxt->ltid, tidKey,
                                                         &tidKeyLen);
            if (sharedTx == NULL)
            {
           ...
           PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_CONT_SENT);
           PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_MSG_SENT);
           tcapOmImage[omBuf.activeOm].appMsgSent++;
         ...
         */
            ...
            sharedTx = TCAP_FindTransactionContext_TTC(trCtxt->ltid, 
                                                         tidKey, &tidKeyLen);
            if (sharedTx == NULL)
            {
                TCAP_DEBUG(("TCOHandleCSLMsg: Cannot Find DSM Tx for did"
            ...
            PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_MSG_SENT);
            tcapOmImage[omBuf.activeOm].appMsgSent++;
            PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_END_SENT);
        ...
        default:
            ...
            /* Outgoing Message - Discarding Message - Invalid State. */
            TCAP_Alarm_TTC(1444, __FILE__, __LINE__,
                           "family=%s:PType=%x:Dialog-Id=%x", "TTC", pType, did); 
    ...
    else if (trCtxt->state == ITS_TCAP_BEGIN_SENT)
    {
        ...
        default:
            ...
            /* Outgoing Message - Discarding Message - Invalid State. */
            TCAP_Alarm_TTC(1445, __FILE__, __LINE__,
                           "family=%s:PType=%x:Dialog-Id=%x", "TTC", pType, did); 
    ...
    else if (trCtxt->state == ITS_TCAP_ACTIVE)
    {
        ...
        default:
            ...
            /* Outgoing Message - Discarding Message - Invalid State. */
            TCAP_Alarm_TTC(1446, __FILE__, __LINE__,
                           "family=%s:PType=%x:Dialog-Id=%x", "TTC", pType, did);
        ...
        pType == TCPPT_TC_BEGIN)
    {
        ...
        if (ret != ITS_SUCCESS)
        {
            TCAP_ERROR(("TSMHandleTCOCSLMsg_TTC: DSM_Commit "
                        "failed, error code: %d\n", ret));
            PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_MSG_DISCARDED);
            tcapOmImage[omBuf.activeOm].appMsgDiscarded++;
            tcapOmImage[omBuf.activeOm].errorsDetectedInTP++;
        ...
        {
            TCAP_DEBUG(("TSMHandleTCOCSLMsg_TTC: DSM-Commit the DSM Tx for "
                        "did %d, pType = %d\n", did, pType));
        }
 ...
 ****************************************************************************/
int
TSMHandleTCOSCCPMsg_TTC(ITS_OCTET pType, ITS_SCCP_IE *ies, int ieCount,
                          TCAP_TRANS_ENTRY **trCtxt)
{
    ...
    case ITS_TCAP_IDLE:
        /* send to CSL */
        if (DHAHandleTCOMsg_TTC(ies, ieCount, *trCtxt) == ITS_SUCCESS)
        {
    ...
    case ITS_TCAP_BEGIN_SENT:
        ...
        case TCPPT_TC_P_ABORT:
            ...
            TCAP_FreeLocalTx_TTC((*trCtxt)->did);
        ...
        default:
            ...
            TCAP_FreeLocalTx_TTC((*trCtxt)->did);
            TCAP_Alarm_TTC(1451, __FILE__, __LINE__,
                           "family=%s:PType=%x:Dialog-Id=%x", "TTC", pType,
                           (*trCtxt)->did);
    ...
    case ITS_TCAP_BEGIN_RCVD:
        ...
        default:
            ...
            /* Incomming Dialogue - Discarding Dialogue -
               Rejection Sent by stack. */
            TCAP_Alarm_TTC(1450, __FILE__, __LINE__,
                           "family=%s:PType=%x:Dialog-Id=%x", "TTC", pType,
                           (*trCtxt)->did);
            PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_MSG_DISCARDED);
            tcapOmImage[omBuf.activeOm].remMsgDiscarded++;
    ...
    case ITS_TCAP_ACTIVE:
        ...
        default:
            ...
            TCAP_Alarm_TTC(1452, __FILE__, __LINE__,
                           "family=%s:PType=%x:Dialog-Id=%x", "TTC", pType,
                           (*trCtxt)->did);
            PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_MSG_DISCARDED);
            tcapOmImage[omBuf.activeOm].remMsgDiscarded++;
