 ...
 ****************************************************************************
 *
 *  ID: $Id: prc-scrc.c,v 1.7 2008/07/18 05:44:02 ssingh Exp $
 *
 * LOG: $Log: prc-scrc.c,v $
 * LOG: Revision 1.7  2008/07/18 05:44:02  ssingh
 * LOG: SDS ID:: ID :: D0240 Warning removal for Solaris Compilation
 * LOG:
 * LOG: Revision 1.6  2008/06/20 09:11:14  ssingh
 * LOG: updated with code review comments, ipDestination check
 * LOG: removed as not working in ASP configuration.
 * LOG:
 * LOG: Revision 1.5  2008/06/17 06:39:08  nvijikumar
 ...
 * LOG:
 * LOG: Revision 1.2.2.2  2007/08/16 12:40:58  pspl
 ...
 * LOG:
 * LOG: Revision 1.2.2.1  2007/07/31 11:26:39  pspl
 ...
 * LOG: Add prc files
 * LOG: 
 * LOG: Revision 1.1.2.6  2007/05/18 09:59:21  raghavendrabk
 * LOG: ROUTE_GetRoutingKeyRouteInfo API called to get RoutingKey Route info
 * LOG:
 * LOG: Revision 1.1.2.5  2007/04/11 07:04:34  mshanmugam
 * LOG: Modifications to allow PRC and ITU stacks to run in parallel
 * LOG:
 * LOG: Revision 1.1.2.4  2007/02/13 04:06:00  raghavendrabk
 ...
 * LOG:
 * LOG: Revision 1.1.2.2  2007/02/02 14:33:07  raghavendrabk
 ...
 * LOG:
 * LOG: Revision 9.6.4.5  2007/01/15 15:31:41  ssingh
 * LOG: taking back r9.6.4.4 changes
 * LOG:
...
#ident "$Id: prc-scrc.c,v 1.7 2008/07/18 05:44:02 ssingh Exp $"
...
sendConnectionRefuse(MTP3_HEADER* mtp3, ITS_USHORT conref, SCCP_ADDR *cgp)
{
    ...
    if (cgp)
    {
        ...
        info_Count++;
        if (SCCP_DecodeAddr_PRC(cgp, &addrInd, &cgppc, &ssn,
                         ...
                         ITS_OCTET reason)
{
    switch (mType)
    {
         ...
         case SCCP_MSG_LUDT:
         case SCCP_MSG_LUDTS:
             return SCLCHandleRoutingFailure_PRC(mtp3, mType, ies, ieCount, reason);

         default:
             return SCOCHandleRoutingFailure_PRC(mtp3, mType, ies, ieCount, reason);
    }
}
                   ...
                   ITS_OCTET addrInd, ITS_UINT pc, ITS_OCTET ssn)
{
    ...
    if (!SCCP_IsSSNEquipped(ni, pc, ssn))
    {
        ...
        if (SCCP_IsSSNAllowed(ni, pc, ssn))
        {
            ...
            switch (mType)
            {
                 ...
                 case SCCP_MSG_LUDT:
                 case SCCP_MSG_LUDTS:
                      return SCRCHandleSCLCMsg_PRC(mtp3, mType, ies, ieCount);

                 default:
                      /* EEK! WE'RE AN INTERMEDIATE!! */
                      return SCOCHandleSCRCMsg_PRC(mtp3, mType, ies, ieCount,
                                               pc, ssn, ITS_TRUE);
            }
        }
        else
        {
            ...
            SCCP_Alarm_PRC(643, __FILE__, __LINE__,
                           "family=%s:type=%s:pc=%x:ni=%x:ssn=%x", "PRC", "local", pc,
                           ni, ssn);
    ...
    else if (!SCCP_IsSSNAllowed(ni, pc, ssn))
    {
        ...
        SCCP_Alarm_PRC(643, __FILE__, __LINE__,
                       "family=%s:type=%s:pc=%x:ni=%x:ssn=%x", "PRC", "local", pc, ni,
                       ssn);

        return SCMGSSPCHandleMsgForProhibitedSSN_PRC(mtp3, mType, ies, ieCount,
                                                       pc, ssn,
    ...
    else
    {
        ...
        switch (mType)
        {
            ...
            return SCLCHandleSCRCMsg_PRC(mtp3, mType, ies, ieCount, pc, ssn);
...
SCRCDeliverLocal(MTP3_HEADER* mtp3, ITS_OCTET mType,
                 ...
                 ITS_OCTET addrInd, ITS_UINT pc, ITS_OCTET ssn)
{
    PEG_IncrPeg(PRC_SCCP_Pegs,PEG_SCCP_REMOTE_MSG_FOR_LOCAL_SS_RCVD);

    /* Q752 Implementation */
    SccpGenOmImage_PRC[omBuf.activeOm].SCCPGenPegs.messageForLocalSS++;
    ...
    else
    {
        switch (mType)
        {
            ...
            SCCP_Alarm_PRC(644, __FILE__, __LINE__, "family=%s:mType=%x", "PRC", mType);
                  ...
                  ITS_OCTET addrInd, ITS_UINT pc, ITS_OCTET ssn)
{
    ...
    if (addrInd & SCCP_CPA_HAS_SSN)
    {
        ...
        {
            /* SSN is not allowed */
             SCCP_Alarm_PRC(648, __FILE__, __LINE__,
                            "family=%s:type=%s:pc=%x:ni=%x:ssn=%x", "PRC", "remote", pc, ni,
                            ssn);
                 ...
                 ITS_OCTET *gttInfo, ITS_USHORT gttLen)
{
    ...
    */
    ...
    if (SCCP_GTTTranslate_PRC(mtp3, &dpc, ai, pc, ssn, gttInfo, gttLen,
                                &transAddr, ies, ieCount) != ITS_SUCCESS)
    {
        ...
        SCCP_Alarm_PRC(645, __FILE__, __LINE__, "family=%s", "PRC");

        return SCRCHandleRoutingFailure(mtp3, mType, ies, ieCount,
                                        SCCP_RET_NO_TRANS_THIS_ADDR);
    }

    ni = MTP3_HDR_GET_SIO_PRC(*mtp3) & MTP3_NIC_MASK;
                        ...
                        NULL, NULL) != ITS_SUCCESS)
    {
        ...
        SCCP_Alarm_PRC(646, __FILE__, __LINE__, "family=%s", "PRC");
 ...
 ****************************************************************************/
int
SCRCHandleMTP3Msg_PRC(ITS_EVENT* ev)
{
    ITS_UINT pc;
    ...
    {
        return SCMGHandleMTP3Msg_PRC(&hdr3, mType, data, len);
    }
    ...
    switch (mType)
    {
        ...
        return SCOCHandleSCRCMsg_PRC(&hdr3, mType, ies, ieCount,
                                       0, ssn, ITS_FALSE);
     ...
     */
        ...
        for (i = 0; i < ieCount; i++)
        {
            if (ies[i].param_id == SCCP_PRM_PROTOCOL_CLASS)
            {
                ...
                {
                     SCCP_DEBUG(("SCRCHandleMTP3Msg_PRC: \
                ...
                else
                {
                    ...
                    SCCP_Alarm_PRC(670, __FILE__, __LINE__,
                                   "family=%s:pclass=%x", "PRC", lpclass);
        ...
        SCCP_Alarm_PRC(636, __FILE__, __LINE__, "family=%s:mType=%x", "PRC", mType);
        return (ITS_EINVALIDARGS);
    }
    ...
    if (i == ieCount)
    {
        ...
        SCCP_Alarm_PRC(647, __FILE__, __LINE__, "family=%s:pc=%x",
                       "PRC", MTP3_RL_GET_OPC_VALUE(hdr3.label)); 
    ...
    if (addrInd & SCCP_CPA_ROUTE_SSN)
    {
        ...
        if (SCCP_UserIntervention_PRC(&hdr3, addrInd, pc, ssn,
        ...
        *cdp = transAddr;
        ...
        else
        {
            ...
            SCCP_Alarm_PRC(644, __FILE__, __LINE__, "family=%s:mType=%x",
                           "PRC", mType);
...
****************************************************************************/
int
SCRCHandleSCLCMsg_PRC(MTP3_HEADER* mtp3, ITS_OCTET mType,
                        ITS_SCCP_IE* ies, int ieCount)
{
    ITS_UINT pc;
 ...
 ****************************************************************************/
int
SCRCHandleSCOCMsg_PRC(MTP3_HEADER* mtp3, ITS_OCTET mType,
                        ITS_SCCP_IE* ies, int ieCount,
                        SCCP_ADDR* cdp)
{
    ITS_UINT pc;
    ...
    /*printf("Result in CO =%x",MTP3_HDR_GET_SIO_PRC(*mtp3));*/
