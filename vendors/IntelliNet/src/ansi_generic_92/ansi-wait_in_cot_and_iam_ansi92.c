 ...
 *                                                                          *
 ****************************************************************************
 *  ID: $Id: ansi-wait_in_cot_and_iam_ansi92.c,v 9.2 2005/04/06 07:30:01 ssingh Exp $
 * LOG: $Log: ansi-wait_in_cot_and_iam_ansi92.c,v $
 ...
 * LOG: Begin PR6.5
 * LOG:
 * LOG: Revision 1.10  2005/03/21 13:53:58  cvsadmin
 * LOG: PR6.4.2 Source Propagated to Current
 * LOG:
 * LOG: Revision 1.7.2.1.6.1.12.3  2005/02/03 12:44:40  ssingh
 ...
 * LOG:
 * LOG: Revision 1.7  2003/05/06 13:55:52  sswami
...
/*
 * Functions for handling messages in the WAIT_INCOMING_COT_AND_IAM state for 
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an INF received in the WAIT_INCOMING_COT_AND_IAM state 
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an INR received in the WAIT_INCOMING_COT_AND_IAM state
 *      is done here. The message is forwarded to the application if received
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an REL received in the WAIT_INCOMING_COT_AND_IAM state is 
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        ISUP_DEBUG(("WAIT_INCOMING_COT_AND_IAM: Received REL from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        __ISUP_StopTimer(cic_rec,ITS_ISUP_TIMER_TCRA);

        ret = __ISUP_StopCPCToCCI(cic_rec,config_info);
        ...
        {
            ret = __ISUP_StopCPCToCRI(cic_rec, config_info);

            if(ret == ITS_FALSE)
            {
                ISUP_ERROR(("Invalid CRI state : IDLE pcic 0x%08x, vcic %d.\n",
                                   cic_rec->pcic, cic_rec->vcic));
                return;
            }
        }
    ...
    {
        ISUP_DEBUG(("WAIT_INCOMING_COT_AND_IAM: Received REL from APP, "
                     "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an RLC received in the WAIT_INCOMING_COT_AND_IAM state is 
 *      done here. The message is forwarded to the application if received  
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_DEBUG(("WAIT_INCOMING_COT_AND_IAM: Received RLC from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
    ...
    {
        ISUP_DEBUG(("WAIT_INCOMING_COT_AND_IAM: Received RLC from APP, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
 ...
 *      the WAIT_INCOMING_COT_AND_IAM state. 
 *
 *  Input Parameters:
 *      msg - the ISUP msg received by the stack as an ITS_EVENT.
 *      cic_rec - pointer to the CIC record relevant for this msg.
                              ...
                              ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_DEBUG(("WAIT_INCOMING_COT_AND_IAM: Received COT from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
        ...
        case CONTINUITY_CHECK_SUCCESS:
            ...
            ISUP_DEBUG(("COT (Success) pcic 0x%08x, vcic %d.\n",
             ...
             if (ret != ITS_TRUE)
             {
                ISUP_CRITICAL((" COT (Success) Received in invalid CRI state"));
                break;
             }
        ...
        case CONTINUITY_CHECK_FAILED:
            ...
            ISUP_DEBUG(("COT (Failure) pcic 0x%08x, vcic %d.\n",
             ...
             __ISUP_StartCPCToCRI(cic_rec, config_info);
    ...
    {
        ISUP_ERROR(("WAIT_INCOMING_COT_AND_IAM: Unexpected COT from App",
                    " pcic 0x%08x,vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        /* Alert Maintenance; Alarm */
        ISUP_Alarm_ANSI(3278, __FILE__, __LINE__, "Message: %x cic : %x", 
        ...
        SPRC_SendErrorToApp_ANSI(ISUP_MSG_COT,
                                 ITS_ISUP_UNEXP_APPMSG_WAITINC_COT_AND_IAM_ERR,
                                 cic_rec);
    }
}
/*.implementation:static
 ****************************************************************************
 *  Purpose:
 *      The processing for an BLA received in the WAIT_INCOMING_COT_AND_IAM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an BLO received in the WAIT_INCOMING_COT_AND_IAM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CCR received in the WAIT_INCOMING_COT_AND_IAM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGB received in the WAIT_INCOMING_COT_AND_IAM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGBA received in the WAIT_INCOMING_COT_AND_IAM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGU received in the WAIT_INCOMING_COT_AND_IAM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGUA received in the WAIT_INCOMING_COT_AND_IAM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CQM received in the WAIT_INCOMING_COT_AND_IAM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CQR received in the WAIT_INCOMING_COT_AND_IAM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an GRA received in the WAIT_INCOMING_COT_AND_IAM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an GRS received in the WAIT_INCOMING_COT_AND_IAM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an RSC received in the WAIT_INCOMING_COT_AND_IAM state is 
 *      done here.
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UBA received in the WAIT_INCOMING_COT_AND_IAM state is 
 *      done here. The message is forwarded to the application if received  
 *      from the network.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UBL received in the WAIT_INCOMING_COT_AND_IAM state is 
 *      done here. The message is forwarded to the application if received  
 *      from the network.
 *
 *  Input Parameters:
 ...
 *      the WAIT_INCOMING_COT_AND_IAM state. It also serves as a default handler for 
                              ...
                              ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_ERROR(("Rcvd Unexpected msg 0x%02x from MTP3 in WAIT_INCOMING_COT_AND_IAM."
        ...
        /* Alert Maintenance; Alarm */
        ISUP_Alarm_ANSI(3279, __FILE__, __LINE__,"dpc: %d opc: %d "
                        "Message: %x cic: %x",
    ...
    {
        ISUP_ERROR(("Rcvd Unexpected msg 0x%02x from App in WAIT_INCOMING_COT_AND_IAM. "
                    "cic state 0x%02x, pcic 0x%08x, vcic %d.\n", 
                    msg->data[sizeof(MTP3_HEADER_ANSI) + sizeof (ITS_USHORT) +
                    sizeof(ITS_OCTET)],
                    cic_rec->cic_state, cic_rec->pcic, cic_rec->vcic));
        ...
        /* Alert Maintenance; Alarm */
        ISUP_Alarm_ANSI(3280, __FILE__, __LINE__, "Message: %x cic: %x",
                        msg->data[sizeof(MTP3_HEADER_ANSI) +
                        sizeof (ITS_USHORT) + sizeof(ITS_OCTET)],
                        ISUP_PCIC_TO_CIC(cic_rec->pcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CPG received in the WAIT_INCOMING_COT_AND_IAM
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ...
        ISUP_ERROR(("WAIT_INCOMING_COT_AND_IAM: Unexpexted CPG from MTP3"
                    " pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        /* Alert Maintenance; Alarm */
        ISUP_Alarm_ANSI(3281, __FILE__, __LINE__, "dpc: opc: %d %d cic : %x",
                        MTP3_RL_GET_DPC_VALUE_ANSI(config_info->out_hdr.label),
                        MTP3_RL_GET_OPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                        ISUP_PCIC_TO_CIC(cic_rec->pcic));
    ...
    {
        ISUP_ERROR(("WAIT_IN_COT_AND_IAM: Received CPG from App. "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        /*Forward CPG to MTP3, this is Error any way app wants it*/
        SPRC_SendIsupToMTP3_ANSI(msg, cic_rec->vcic);

        /* Change state to WAIT_INCOMING_COT_AND_IAM, i.e. no change */
    }
}
 ...
 *      for the WAIT_INCOMING_COT_AND_IAM state, based on the msgType received.
 *
 *  Input Parameters:
...
__ISUP_GetMsgProc_WaitIncomingCotAndIam_ANSI92(ITS_OCTET msgType)
{
    switch(msgType)
    {
    case ISUP_MSG_REL:
 ...
 *      WAIT_INCOMING_COT_AND_IAM state. 
 *
 *  Input Parameters:
...
{
    /* Alert Maintenance; Alarm */
    ISUP_Alarm_ANSI(3282, __FILE__, __LINE__, "dpc: %d opc: %d cic: %x", 
                           ...
                           ISUP_OPC_DPC_INFO *config_info)
{
    ...
    ISUP_DEBUG(("WAIT_INCOMING_COT_AND_IAM: TimeoutT28 received"
    ...
    /* Alert Maintenance; Alarm */
    ISUP_Alarm_ANSI(3283, __FILE__, __LINE__, "dpc: %d opc: %d cic: %x", 
                    MTP3_RL_GET_DPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                    MTP3_RL_GET_OPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                    ISUP_PCIC_TO_CIC(cic_rec->pcic));
...
{
    int ret = ITS_SUCCESS;
    ITS_BOOLEAN ret_bool;

    ISUP_DEBUG(("WAIT_INCOMING_COT_AND_IAM: Timeout TCRA received"
                " pcic 0x%08x, vcic %d, cic %d.\n",
                 cic_rec->pcic, cic_rec->vcic, ISUP_PCIC_TO_CIC(cic_rec->pcic)));

    /* Alert Maintenance; Alarm */
    ISUP_Alarm_ANSI(3284, __FILE__, __LINE__, "dpc: %d opc: %d cic: %x", 
                   MTP3_RL_GET_DPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                   MTP3_RL_GET_OPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                   ISUP_PCIC_TO_CIC(cic_rec->pcic));

    ret_bool = __ISUP_StopCPCToCCI(cic_rec, config_info);

    if(ret_bool == ITS_TRUE)
    {
       __ISUP_StopTimer(cic_rec,ITS_ISUP_TIMER_T8);
    }
    else
...
{
    /* Log error, update stats and drop msg */
    ISUP_ERROR(("WAIT_INCOMING_COT_AND_IAM: Unexpected Timeout T%d received "
    ...
    /* Alert Maintenance; Alarm */
    ISUP_Alarm_ANSI(3285, __FILE__, __LINE__, "State: %x, Timeout T%d",
 ...
 *      the WAIT_INCOMING_COT_AND_IAM state, based on the ID of the timer that has 
 *      expired.
 *
 *  Input Parameters:
...
ITSSS7DLLAPI __ISUP_TimeoutHandlerProc
        __ISUP_GetTimeoutProc_WaitIncomingCotAndIam_ANSI92(ITS_UINT timerID)
{
    switch(timerID)
    {
