 ...
 ****************************************************************************
 *
 *  ID: $Id: ansi-wait_out_acm_ansi92.c,v 9.2 2005/04/06 07:30:01 ssingh Exp $
 *
 * LOG: $Log: ansi-wait_out_acm_ansi92.c,v $
 ...
 * LOG: Begin PR6.5
 * LOG:
 * LOG: Revision 8.10  2005/03/21 13:53:59  cvsadmin
 * LOG: PR6.4.2 Source Propagated to Current
 * LOG:
 * LOG: Revision 7.8.2.3.2.1.12.2  2004/12/29 14:35:55  csireesh
 * LOG: ISUP OAM Changes
 * LOG:
 * LOG: Revision 7.8.2.3.2.1.12.1  2004/12/06 13:34:50  mmanikandan
 * LOG: Windows Compilation Support
 * LOG:
 * LOG: Revision 7.8.2.3.2.1  2004/01/05 10:10:44  sjaddu
 * LOG: Merged code from tcs isup branch.
 * LOG:
 * LOG: Revision 7.8.2.2.2.1  2003/09/13 06:32:42  akumar
 * LOG: Msg based redundancy works for ANSI.
 * LOG:
 * LOG: Revision 7.8.2.2  2003/05/07 10:38:00  ssingh
 * LOG: Latest Changes propped from Current to 6.3.
 * LOG:
 * LOG: Revision 8.7  2003/05/06 13:55:52  sswami
 ...
 * LOG:
 * LOG: Revision 8.4  2003/04/16 06:45:13  akumar
 ...
 * LOG:
 * LOG: Revision 8.2  2003/03/14 08:40:51  ssingh
 * LOG: Fixes for Bug# 242/229 propped from PR6.3
 * LOG:
 ...
 * LOG:
 * LOG: Revision 7.7  2002/12/06 05:34:12  akumar
 ...
 * LOG:
 * LOG: Revision 1.1.2.14  2002/07/23 18:24:13  ssingh
 * LOG: Added/Changed Code to fix BUG noticed during testing.
 * LOG:
 * LOG: Revision 1.1.2.13  2002/07/14 19:01:41  hbalimid
 ...
 * LOG:
 * LOG: Revision 1.1.2.11  2002/07/12 01:02:00  hbalimid
 ...
 * LOG:
 * LOG: Revision 1.1.2.7  2002/07/09 00:00:16  ssingh
 * LOG: CQR implemented. Bug Fixed.
 * LOG:
 * LOG: Revision 1.1.2.6  2002/07/08 14:17:11  ssingh
 ...
 * LOG:
 * LOG: Revision 1.1.2.4  2002/07/04 23:30:07  hbalimid
...
/*
 * Functions for handling messages in the Wait_Outgoing_ACM state for ANSI92 variant.
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ...
        ISUP_ERROR(("WAIT_OUT_ACM: Unexpected ACM from MTP3. "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));


        ISUP_Alarm_ANSI(3098, __FILE__, __LINE__, "dpc: %d opc: %d "
                        "cic: %x Message:%x",
    ...
    if (src != ISUP_SRC_L3_ANSI)
    {
        ISUP_DEBUG(("Received ACM in WAIT_OUT_ACM from APP, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        /* OGC Complete */
        /* check for ACM Control */

        /* Send to L3 */
        SPRC_SendIsupToMTP3_ANSI(msg, cic_rec->vcic);

        /* Change state to WAIT_OUTGOING_ANM - Wait for Answer */
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an ANM received in the WAIT_OUTGOING_ACM state 
 *      is done here. The message is forwarded to the network if received 
 *      from the application.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        /* Msg out of sequence. Drop it and generate alarm. */
        ISUP_ERROR(("WAIT_OUT_ACM: Unexpected ANM from MTP3. "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));


        ISUP_Alarm_ANSI(3099, __FILE__, __LINE__, "dpc: %d opc: %d "
                        "cic: %x Message:%x",
                        MTP3_RL_GET_DPC_VALUE_ANSI(config_info->out_hdr.label),
                        MTP3_RL_GET_OPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                        ISUP_PCIC_TO_CIC(cic_rec->pcic),
    ...
    if (src != ISUP_SRC_L3_ANSI)
    {
        ...
        /* Check for the charging office */

        /* Send to L3 */
        SPRC_SendIsupToMTP3_ANSI(msg, cic_rec->vcic);

        /* Change state to SENT_OUTGOING_ANM - ICC Answered */
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CCR received in the WAIT_OUTGOING_ACM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an LPA received in the WAIT_OUTGOING_ACM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CVT received in the WAIT_OUTGOING_ACM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CVR received in the WAIT_OUTGOING_ACM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UCIC received in the WAIT_OUTGOING_ACM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ...
        ISUP_Alarm_ANSI(3100, __FILE__, __LINE__, "dpc: %d opc: %d "
                        "cic: %x  Message:%x",
                        MTP3_RL_GET_DPC_VALUE_ANSI(config_info->out_hdr.label),
                        MTP3_RL_GET_OPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                        ISUP_PCIC_TO_CIC(cic_rec->pcic),
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_DEBUG(("Received REL in WAIT_OUT_ACM from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        /* Send to app */
        SPRC_SendIsupToApp_ANSI(cic_rec->vcic, msg);

        /* Check for PRE ACM Connect
         * will be done by the application
         * In that case App will request for RLC
         * So change state to Wait for Out
         */
    ...
    {
        ISUP_DEBUG(("Received REL in WAIT_OUT_ACM from APP, "
                     "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
        ...
        /* Start the RLC Timer T5, REL Timer T1 */
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_DEBUG(("Received RLC in WAIT_OUT_ACM from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
    ...
    {
        ISUP_DEBUG(("Received RLC in WAIT_OUT_ACM from APP, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
                              ...
                              ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ...
        ISUP_Alarm_ANSI(3102, __FILE__, __LINE__, "dpc: %d opc: %d "
                        "cic: %x  Message:%x",
                        MTP3_RL_GET_DPC_VALUE_ANSI(config_info->out_hdr.label),
                        MTP3_RL_GET_OPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                        ISUP_PCIC_TO_CIC(cic_rec->pcic),
    ...
    else
    {
        ...
        ISUP_Alarm_ANSI(3103, __FILE__, __LINE__, "Message %x cic : %x",
                         msg->data[sizeof(MTP3_HEADER_ANSI) + 
                         sizeof(ITS_USHORT) + sizeof(ITS_OCTET)],
                         ISUP_PCIC_TO_CIC(cic_rec->pcic));
...
__ISUP_GetMsgProc_WaitOutgoingAcm_ANSI92(ITS_OCTET msgType)
{
    switch(msgType)
    {
                           ...
                           ISUP_OPC_DPC_INFO *config_info)
{
    ...
    ISUP_Alarm_ANSI(3104, __FILE__, __LINE__, "dpc: %d opc: %d cic: %x",
                     MTP3_RL_GET_DPC_VALUE_ANSI(config_info->out_hdr.label),
                     MTP3_RL_GET_OPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                     ISUP_PCIC_TO_CIC(cic_rec->pcic),t_entry->timerId);
                           ...
                           ISUP_OPC_DPC_INFO *config_info)
{
    ...
    ISUP_Alarm_ANSI(3105, __FILE__, __LINE__, "State:%x Timer id: T%d",
...
__ISUP_GetTimeoutProc_WaitOutgoingAcm_ANSI92(ITS_UINT timerID)
{
    switch(timerID)
    {
