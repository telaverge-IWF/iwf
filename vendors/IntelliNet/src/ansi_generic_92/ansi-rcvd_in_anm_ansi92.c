 ...
 ****************************************************************************
 *
 *  ID: $Id: ansi-rcvd_in_anm_ansi92.c,v 9.2 2005/04/06 07:30:01 ssingh Exp $
 *
 * LOG: $Log: ansi-rcvd_in_anm_ansi92.c,v $
 ...
 * LOG: Begin PR6.5
 * LOG:
 * LOG: Revision 8.10  2005/03/21 13:53:56  cvsadmin
 * LOG: PR6.4.2 Source Propagated to Current
 * LOG:
 * LOG: Revision 7.7.2.1.6.1.12.3  2005/02/03 12:44:40  ssingh
 * LOG: Bug #1448; unexpected Msg from Network and peg/stats handling.
 * LOG: (Ramesh KV, Abhijit D)
 * LOG:
 * LOG: Revision 7.7.2.1.6.1.12.2  2004/12/29 14:35:54  csireesh
 * LOG: ISUP OAM Changes
 * LOG:
 * LOG: Revision 7.7.2.1.6.1.12.1  2004/12/06 13:34:50  mmanikandan
 * LOG: Windows Compilation Support
 * LOG:
 * LOG: Revision 7.7.2.1.6.1  2004/01/05 10:10:44  sjaddu
 * LOG: Merged code from tcs isup branch.
 * LOG:
 * LOG: Revision 7.7.2.1.2.1  2003/09/13 06:27:00  akumar
 * LOG: Msg Based redundancy works for ANSI.
 * LOG:
 * LOG: Revision 7.7.2.1  2003/05/07 10:38:00  ssingh
 ...
 * LOG: Begin PR6.4
 * LOG:
 * LOG: Revision 7.7  2002/12/11 14:23:05  ssingh
 * LOG: Changed code for INF/INR/CPG and T33 expiry handling.
 * LOG:
 * LOG: Revision 7.6  2002/11/20 12:47:34  akumar
 * LOG: Worked on ALARMS indentation
 * LOG:
 * LOG: Revision 7.5  2002/11/12 05:37:27  akumar
 * LOG: Added ANSI ISUP Alarms, replaced ISUP_SRC_L3_ITU with ANSI
 * LOG: and added new arg to exchange control function call
 * LOG:
 ...
 * LOG:
 * LOG: Revision 7.2  2002/10/25 13:43:57  ssingh
 ...
 * LOG:
 * LOG: Revision 1.1.2.15  2002/07/13 20:25:57  hbalimid
 ...
 * LOG:
 * LOG: Revision 1.1.2.7  2002/07/08 14:13:04  ssingh
...
/*
 * Functions for handling messages in the Rcvd_Incoming_ANM state for ANSI92 
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CCR received in the RCVD_INCOMING_ANM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an LPA received in the RCVD_INCOMING_ANM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CVT received in the RCVD_INCOMING_ANM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CVR received in the RCVD_INCOMING_ANM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UCIC received in the RCVD_INCOMING_ANM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        ISUP_DEBUG(("Received CRG in RCVD_IN_ANM from MTP3, "
    ...
    {
        ISUP_DEBUG(("Received CRG in RCVD_IN_ANM from APP, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
 ...
 *
 *  Notes:
 *      Procedure implemented from T1.113-4
 *
 ****************************************************************************/
static void
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ...
        ISUP_Alarm_ANSI(3061, __FILE__, __LINE__, "dpc: %d opc: %d "
                        "cic: %x Message: %x",
    ...
    else 
    {
        ...
        ISUP_DEBUG(("Received FOT in RCVD_IN_ANM from APP, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        /* Forward FOT to L3 */
        SPRC_SendIsupToMTP3_ANSI(msg, cic_rec->vcic);

        /* No Change of state - OGC Answered */
    }
}
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_DEBUG(("Received REL in RCVD_IN_ANM from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        /* Send to app */
        SPRC_SendIsupToApp_ANSI(cic_rec->vcic, msg);

        /* expecting RLC to be sent out to network from app */
    ...
    else 
    {
        ...
        ISUP_DEBUG(("Received REL in RCVD_IN_ANM from APP, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_DEBUG(("Received RLC in RCVD_IN_ANM from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        /* Outgoing Release Request/Ind to App */
    ...
    {
        ISUP_DEBUG(("Received RLC in RCVD_IN_ANM from APP, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
 ...
 *
 *  Notes:
 *      Procedure is implemented according to T1.113-1992 Fig25 (35 of 41)
 *
 ****************************************************************************/
static void
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_DEBUG(("Received SUS in RCVD_IN_ANM from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        /* FIXME: Check for the controlling exchange
         * If so, start the TRD Timer T6
         * Give Network Suspend Ind to App
         */

         /* Note: only Network Initaited suspend is
          * is supported by ANSI and it is the default.
          */
        ...
        /* Network suspend Ind to app */

        /* Send to app */
        SPRC_SendIsupToApp_ANSI(cic_rec->vcic, msg);

        /* Change state to WAIT_INCOMING_RES - OGC Suspended */
        cic_rec->fsm_state = WAIT_INCOMING_RES;

        /* pre-suspended state stored below in the common section */
    }
    else 
    {
        ISUP_DEBUG(("Received SUS in RCVD_IN_ANM from APP, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
        /* Send to L3 */
        SPRC_SendIsupToMTP3_ANSI(msg, cic_rec->vcic);
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CPG received in the RCVD_INCOMING_ANM state
 *      is done here. The message is forwarded to the application if received
 *      from the network, or vice versa.
 *
 *  Input Parameters:
...
__ISUP_Received_CPG(ITS_EVENT *msg,  ISUP_CIC_INFO *cic_rec,
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    /* Forward CPG appropriately */
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_DEBUG(("Received CPG in RCVD_INCOMING_ANM from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
    ...
    {
        ISUP_DEBUG(("Received CPG in RCVD_INCOMING_ANM from APP, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
                              ...
                              ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ...
        ISUP_Alarm_ANSI(3064, __FILE__, __LINE__, "dpc: %d opc: %d "
                       "cic: %x Message: %x", 
                       MTP3_RL_GET_DPC_VALUE_ANSI(config_info->out_hdr.label),
                       MTP3_RL_GET_OPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                       ISUP_PCIC_TO_CIC(cic_rec->pcic),
    ...
    else
    {
        ...
        ISUP_Alarm_ANSI(3065, __FILE__, __LINE__, "Message %x cic : %x",
                        msg->data[sizeof(MTP3_HEADER_ANSI) +
                        sizeof(ITS_USHORT) + sizeof(ITS_OCTET)],
                        ISUP_PCIC_TO_CIC(cic_rec->pcic));
...
__ISUP_GetMsgProc_RcvdIncomingAnm_ANSI92(ITS_OCTET msgType)
{
    switch(msgType)
    {
                           ...
                           ISUP_OPC_DPC_INFO *config_info)
{
   ...
   ISUP_Alarm_ANSI(3081, __FILE__, __LINE__, "dpc: %d opc: %d cic: %x",
                    MTP3_RL_GET_DPC_VALUE_ANSI(config_info->out_hdr.label),
                    MTP3_RL_GET_OPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                                             ISUP_PCIC_TO_CIC(cic_rec->pcic));
                                  ...
                                  ISUP_OPC_DPC_INFO *config_info)
{
    ...
    ISUP_Alarm_ANSI(3066, __FILE__, __LINE__, "State: %x Timer id: T%d",
...
__ISUP_GetTimeoutProc_RcvdIncomingAnm_ANSI92(ITS_UINT timerID)
{
    switch(timerID)
    {
