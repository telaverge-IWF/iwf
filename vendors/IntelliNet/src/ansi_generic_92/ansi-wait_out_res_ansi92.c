 ...
 ****************************************************************************
 *
 *  ID: $Id: ansi-wait_out_res_ansi92.c,v 9.2 2005/04/06 07:30:01 ssingh Exp $
 *
 * LOG: $Log: ansi-wait_out_res_ansi92.c,v $
 ...
 * LOG: Begin PR6.5
 * LOG:
 * LOG: Revision 8.9  2005/03/21 13:54:01  cvsadmin
 ...
 * LOG:
 * LOG: Revision 7.8.2.1.6.1  2004/01/05 10:10:45  sjaddu
 ...
 * LOG:
 * LOG: Revision 1.1.2.10  2002/07/30 13:54:55  sjaddu
 ...
 * LOG:
 * LOG: Revision 1.1.2.8  2002/07/10 21:36:58  hbalimid
 * LOG: OGC Answered complete.
 * LOG:
 * LOG: Revision 1.1.2.7  2002/07/10 19:32:12  ssharma
 * LOG: Changes for redundancy.
 * LOG:
 * LOG: Revision 1.1.2.6  2002/07/10 05:36:01  hbalimid
...
/*
 * Functions for handling messages in the Wait_Outgoing_RES state for ANSI92 
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an BLA received in the WAIT_OUTGOING_RES state 
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an BLO received in the WAIT_OUTGOING_RES state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGB received in the WAIT_OUTGOING_RES state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGU received in the WAIT_OUTGOING_RES state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGUA received in the WAIT_OUTGOING_RES state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CQM received in the WAIT_OUTGOING_RES state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CQR received in the WAIT_OUTGOING_RES state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an GRA received in the WAIT_OUTGOING_RES state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an GRS received in the WAIT_OUTGOING_RES state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an REL received in the WAIT_OUTGOING_RES state 
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        ISUP_DEBUG(("Received REL in WAIT_OUT_RES from MTP3, pcic 0x%08x, vcic %d.\n",
    ...
    {
        ISUP_DEBUG(("Received REL in WAIT_OUT_RES from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an RES received in the WAIT_OUTGOING_RES state 
 *      is done here. The message is forwarded to the network if received 
 *      from the application.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ...
        ISUP_ERROR(("WAIT_OUT_RES RES: Unexpected RES from MTP3. pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));


        ISUP_Alarm_ANSI(3124, __FILE__, __LINE__, "dpc: %d opc: %d "
                       "cic: %x  Message: %x",
    ...
    {
        ISUP_DEBUG(("Received RES in WAIT_OUT_RES from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));

        /* Send to L3 */
        SPRC_SendIsupToMTP3_ANSI(msg, cic_rec->vcic);

        /* Change state to pre-suspend state - ICC Answered */
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an RLC received in the WAIT_OUTGOING_RES state 
 *      is done here. The message is forwarded to the network if received 
 *      from the application.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_DEBUG(("Received RLC in WAIT_OUT_RES from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));

        /* Incoming Release Request to App */
         SPRC_SendIndToApp_ANSI(ITS_ISUP_RLC_RELEASE_IND, cic_rec); 

        /* Controlling exchange ?
         * (y) then Charging Office ? (Y) Stop Charging (n) Out
         * (n) Out
         */
    ...
    {
        ISUP_DEBUG(("Received RLC in WAIT_OUT_RES from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an RSC received in the WAIT_OUTGOING_RES state 
 *      is done here. The message is forwarded to the network if received 
 *      from the application.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UBA received in the WAIT_OUTGOING_RES state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UBL received in the WAIT_OUTGOING_RES state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      This function handles the processing of FOT message in 
 *      the WAIT_OUTGOING_RES state.
 *
 *  Input Parameters:
 *      msg - the ISUP msg received by the stack as an ITS_EVENT.
 *      cic_rec - pointer to the CIC record relevant for this msg.
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_DEBUG(("Received FOT in IDLE from MTP3, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
    ...
    {
        ISUP_DEBUG(("Received FOT in IDLE from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an INR received in the WAIT_OUTGOING_RES state
 *      is done here. The message is forwarded to the application if received
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 *      the WAIT_OUTGOING_RES state. It also serves as a default handler for 
                              ...
                              ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_ERROR(("Rcvd Unexpected msg 0x%02x from MTP3 in WAIT_OUT_RES. "
        ...
        ISUP_Alarm_ANSI(3125, __FILE__, __LINE__, "dpc: %d opc: %d "
                       "cic: %x  Message: %x",
                       MTP3_RL_GET_DPC_VALUE_ANSI(config_info->out_hdr.label),
                       MTP3_RL_GET_OPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                       ISUP_PCIC_TO_CIC(cic_rec->pcic),
    ...
    {
        ISUP_ERROR(("Rcvd Unexpected msg 0x%02x from App in WAIT_OUT_RES. "
                    "cic state 0x%02x, pcic 0x%08x, vcic %d.\n", 
                    msg->data[sizeof(MTP3_HEADER_ANSI) + sizeof (ITS_USHORT) +
                    sizeof(ITS_OCTET)],
                    cic_rec->cic_state, cic_rec->pcic, cic_rec->vcic));
        ...
        ISUP_Alarm_ANSI(3126, __FILE__, __LINE__, "Message %x cic : %x",
                         msg->data[sizeof(MTP3_HEADER_ANSI) + 
                         sizeof(ITS_USHORT) + sizeof(ITS_OCTET)],
                         ISUP_PCIC_TO_CIC(cic_rec->pcic));
        ...
        SPRC_SendErrorToApp_ANSI(msg->data[sizeof(MTP3_HEADER_ANSI) +
                                 sizeof (ITS_USHORT) + sizeof(ITS_OCTET)],
                                 ITS_ISUP_UNEXP_APPMSG_WAITOUT_RES_ERR,
                                 cic_rec);
    }
}
 ...
 *      for the WAIT_OUTGOING_RES state, based on the msgType received.
 *
 *  Input Parameters:
...
__ISUP_GetMsgProc_WaitOutgoingRes_ANSI92(ITS_OCTET msgType)
{
    switch(msgType)
    {
    case ISUP_MSG_ACM:
                           ...
                           ISUP_OPC_DPC_INFO *config_info)  
{
    ...
    ISUP_DEBUG(("TimeoutT28 event received in WAIT_OUT_RES state. "
    ...
    ISUP_Alarm_ANSI(3127, __FILE__, __LINE__, "dpc: %d opc: %d cic: %x",
                    MTP3_RL_GET_DPC_VALUE_ANSI(config_info->out_hdr.label),
                    MTP3_RL_GET_OPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                    ISUP_PCIC_TO_CIC(cic_rec->pcic));
    ...
    {
        /* log error */
        ISUP_ERROR(("TimeoutT28 in WAIT_OUT_RES state: Unable to remove T28 info "
                                  ...
                                  ISUP_OPC_DPC_INFO *config_info)
{
    ...
    ISUP_ERROR(("Rcvd Unexpected Timeout T%d for CIC %d from MTP3 in "
                "WAIT_OUT_RES. cic state 0x%02x, pcic 0x%08x, vcic %d.\n", 
                t_entry->cic, t_entry->timerId,
                cic_rec->cic_state, cic_rec->pcic, cic_rec->vcic));
    /* Could print out mtp3 header or entire ITS_EVENT */ 


    ISUP_Alarm_ANSI(3128, __FILE__, __LINE__, " State: %x Timer id: T%d",
 ...
 *      the WAIT_OUTGOING_RES state, based on the ID of the timer that has 
 *      expired.
 *
 *  Input Parameters:
...
__ISUP_GetTimeoutProc_WaitOutgoingRes_ANSI92(ITS_UINT timerID)
{
    switch(timerID)
    {
