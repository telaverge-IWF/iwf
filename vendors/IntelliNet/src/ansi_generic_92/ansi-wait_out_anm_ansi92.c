 ...
 ****************************************************************************
 *
 *  ID: $Id: ansi-wait_out_anm_ansi92.c,v 9.2 2005/04/06 07:30:01 ssingh Exp $
 *
 * LOG: $Log: ansi-wait_out_anm_ansi92.c,v $
 ...
 * LOG: Begin PR6.5
 * LOG:
 * LOG: Revision 8.10  2005/03/21 13:54:00  cvsadmin
 * LOG: PR6.4.2 Source Propagated to Current
 * LOG:
 * LOG: Revision 7.8.2.3.6.1.12.2  2004/12/29 14:35:55  csireesh
 * LOG: ISUP OAM Changes
 * LOG:
 * LOG: Revision 7.8.2.3.6.1.12.1  2004/12/06 13:34:50  mmanikandan
 * LOG: Windows Compilation Support
 * LOG:
 * LOG: Revision 7.8.2.3.6.1  2004/01/05 10:10:44  sjaddu
 * LOG: Merged code from tcs isup branch.
 * LOG:
 * LOG: Revision 7.8.2.3.2.1  2003/09/13 06:32:42  akumar
 * LOG: Msg based redundancy works for ANSI.
 * LOG:
 * LOG: Revision 7.8.2.3  2003/05/07 10:38:00  ssingh
...
/*
 * Functions for handling messages in the Wait_Outgoing_ANM state for ANSI92 
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        /* Msg out of sequence. Drop it, reset Ckt and generate alarm. */
        ISUP_ERROR(("WAIT_OUT_ANM ANM: Unexpected ANM from MTP3. pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        ISUP_Alarm_ANSI(3106, __FILE__, __LINE__, "dpc: %d opc: %d "
                        "cic: %x  Message: %x",
    ...
    if (src != ISUP_SRC_L3_ANSI)
    {
        ISUP_DEBUG(("Received ANM in WAIT_OUT_ANM from APP, pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
        ...
        /* charging will be initiated in the app */
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CCR received in the WAIT_OUTGOING_ANM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an LPA received in the WAIT_OUTGOING_ANM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CVT received in the WAIT_OUTGOING_ANM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CVR received in the WAIT_OUTGOING_ANM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UCIC received in the WAIT_OUTGOING_ANM state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_DEBUG(("Received CPG in WAIT_OUT_ANM from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
    ...
    else
    {
        ...
        /* No change in call state - Wait for Answer */
    }
}
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_DEBUG(("Received REL in WAIT_OUT_ANM from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        /* Send to app */
        SPRC_SendIsupToApp_ANSI(cic_rec->vcic, msg);

        /* Wait for the app to send RLC request */
    ...
    {
        ISUP_DEBUG(("Received REL in WAIT_OUT_ANM from APP, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_DEBUG(("Received RLC in WAIT_OUT_ANM from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
    ...
    {
        ISUP_DEBUG(("Received RLC in WAIT_OUT_ANM from APP, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
                              ...
                              ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_DEBUG(("Received FOT in WAIT_OUTGOING_ANM from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
        ...
        /* No change of state - Wait for answer */
    }
    else
    {
        ISUP_DEBUG(("Received FOT in WAIT_OUTGOING_ANM from APP, pcic 0x%08x, vcic %d.\n",
                     cic_rec->pcic, cic_rec->vcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an ACM received in the WAIT_OUTGOING_ANM state
 *      is done here. The message is forwarded to the network if received
 *      from the application.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    if (src == ISUP_SRC_L3_ANSI)
    {
        /* Msg out of sequence. Discard it */
        ISUP_ERROR(("WAIT_OUT_ANM : Unexpected ACM from MTP3. pcic 0x%08x, "
                    "vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
    ...
    else
    {
        ...
        ISUP_Alarm_ANSI(3109, __FILE__, __LINE__, "Message: %x cic : %x",
                        msg->data[sizeof(MTP3_HEADER_ANSI) + 
                        sizeof (ITS_USHORT) + sizeof(ITS_OCTET)],
                        ISUP_PCIC_TO_CIC(cic_rec->pcic));
                              ...
                              ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ...
        ISUP_Alarm_ANSI(3110, __FILE__, __LINE__, "dpc: %d opc: %d "
                       "cic: %x  Message: %x",
                       MTP3_RL_GET_DPC_VALUE_ANSI(config_info->out_hdr.label),
                       MTP3_RL_GET_OPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                       ISUP_PCIC_TO_CIC(cic_rec->pcic),
    ...
    else
    {
        ...
        ISUP_Alarm_ANSI(3111, __FILE__, __LINE__, "Message %x cic : %x",
                         msg->data[sizeof(MTP3_HEADER_ANSI) + 
                         sizeof (ITS_USHORT) + sizeof(ITS_OCTET)],
                         ISUP_PCIC_TO_CIC(cic_rec->pcic));
...
__ISUP_GetMsgProc_WaitOutgoingAnm_ANSI92(ITS_OCTET msgType)
{
    switch(msgType)
    {
                           ...
                           ISUP_OPC_DPC_INFO *config_info)
{
    ...
    ISUP_Alarm_ANSI(3112, __FILE__, __LINE__, "dpc: %d opc: %d cic: %x",
                    MTP3_RL_GET_DPC_VALUE_ANSI(config_info->out_hdr.label),
                    MTP3_RL_GET_OPC_VALUE_ANSI(config_info->opc_dpc_ni.label), 
                    ISUP_PCIC_TO_CIC(cic_rec->pcic));
                           ...
                           ISUP_OPC_DPC_INFO *config_info)
{
    ...
    ISUP_Alarm_ANSI(3113, __FILE__, __LINE__, "State: %x Timer id: T%d",
...
__ISUP_GetTimeoutProc_WaitOutgoingAnm_ANSI92(ITS_UINT timerID)
{
    switch(timerID)
    {
