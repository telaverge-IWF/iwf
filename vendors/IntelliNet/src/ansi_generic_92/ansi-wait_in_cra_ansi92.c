 ...
 *                                                                          *
 ****************************************************************************
 *  ID: $Id: ansi-wait_in_cra_ansi92.c,v 9.2 2005/04/06 07:30:01 ssingh Exp $
 * LOG: $Log: ansi-wait_in_cra_ansi92.c,v $
 ...
 * LOG: Begin PR6.5
 * LOG:
 * LOG: Revision 1.9  2005/03/21 13:53:59  cvsadmin
 ...
 * LOG:
 * LOG: Revision 1.6.2.1.2.1  2003/09/13 06:30:15  akumar
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an IAM received in the WAIT_INCOMING_CRA state
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        ISUP_DEBUG(("WAIT_INCOMING_CRA: Received IAM from MTP3, "
        ...
        if (!__ISUP_IsExchangeControlling(msg, cic_rec, config_info))
        {
            ...
            /*Send a CRM Reattempt indication to the application */
    ...
    {
        ISUP_DEBUG(("WAIT_INCOMING_CRA: Unexpected IAM from APP; Discarding"
                    " pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        /* Alert maintenance: Alarm */
        ISUP_Alarm_ANSI(3286, __FILE__, __LINE__, "Message: %x cic: %x",
        ...
        SPRC_SendErrorToApp_ANSI(ISUP_MSG_IAM,
                                 ITS_ISUP_UNEXP_APPMSG_WAITINC_CRA_ERR,
                                 cic_rec);
    }
    return;
}
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CRA received in the WAIT_INCOMING_CRA state 
 *      is done here. The message is forwarded to the application if received 
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        ISUP_DEBUG(("WAIT_INCOMING_CRA: Received CRA from MTP3,"
                    " pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
    ...
    else
    {
        ...
        ISUP_ERROR(("WAIT_INCOMING_CRA: Unexpected CRA from App."
                    " pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        /* Alert Maintenance, Alarm */
        ISUP_Alarm_ANSI(3287, __FILE__, __LINE__, "Message: %x cic: %x", 
                        msg->data[sizeof(MTP3_HEADER_ANSI) + 
                        sizeof (ITS_USHORT) + sizeof(ITS_OCTET)],
                        ISUP_PCIC_TO_CIC(cic_rec->pcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an INF received in the WAIT_INCOMING_CRA state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an INR received in the WAIT_INCOMING_CRA state 
 ...
 ****************************************************************************
 *  Purpose:
 *     The processing for an REL received in WAIT_INCOMING_CRA state is 
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_DEBUG(("WAIT_INCOMING_CRA: Received REL from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
    ...
    {
        ISUP_DEBUG(("WAIT_INCOMING_CRA: Received REL from APP, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an RLC received in  WAIT_INCOMING_CRA state is 
 *      done here. The message is forwarded to the application if received  
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_DEBUG(("WAIT_INCOMING_CRA: Received RLC from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
    ...
    {
        ISUP_DEBUG(("WAIT_INCOMING_CRA: Received RLC from APP, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an BLA received in the WAIT_INCOMING_CRA state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an BLO received in the WAIT_INCOMING_CRA state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CPG received in the WAIT_INCOMING_CRA
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        /* Msg out of sequence. Drop it and generate alarm. */
        ISUP_ERROR(("WAIT_INCOMING_CRA: Unexpected CPG from App."
                    " pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
     
        /* Alert Maintenance, Alarm */
        ISUP_Alarm_ANSI(3288, __FILE__, __LINE__, "Message: %x  cic: %x", 
                        msg->data[sizeof(MTP3_HEADER_ANSI) + 
                        sizeof (ITS_USHORT) + sizeof(ITS_OCTET)],
                        ISUP_PCIC_TO_CIC(cic_rec->pcic));
    ...
    {
        ISUP_DEBUG(("WAIT_INCOMING_CRA: Received CPG from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        /* Forward CPG to app ,App will reset ciruit*/
        SPRC_SendIsupToApp_ANSI(cic_rec->vcic, msg);

        /* Change state to WAIT_FOR_COT_REPORTS, i.e. no change */
    }
}
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGB received in the WAIT_INCOMING_CRA state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGBA received in the WAIT_INCOMING_CRA state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGU received in the WAIT_INCOMING_CRA state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGUA received in the WAIT_INCOMING_CRA state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an GRA received in the WAIT_INCOMING_CRA state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an GRS received in the WAIT_INCOMING_CRA state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an RSC received in  WAIT_INCOMING_CRA state is 
 *      done here.
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UBA received in  WAIT_INCOMING_CRA state is 
 *      done here. The message is forwarded to the application if received  
 *      from the network.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UBL received in  WAIT_INCOMING_CRA state is 
 *      done here. The message is forwarded to the application if received  
 *      from the network.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UCIC received in the WAIT_INCOMING_CRA state
 *      is done here. The message is forwarded to the application if received
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CQM received in WAIT_INCOMING_CRA.
 *      The message is forwarded to the application if received from the
 *      network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CQR received in WAIT_INCOMING_CRA.
 *      The message is forwarded to the application if received from the
 *      network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CVT received in WAIT_INCOMING_CRA
 *      is done here. The message is forwarded to the application if received
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CVR received in WAIT_INCOMING_CRA
 *      is done here. The message is forwarded to the application if received
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 *      the WAIT_INCOMING_CRA state. It also serves as a default handler for 
                              ...
                              ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ...
        /* Alert maintenance; Alarm */
        ISUP_Alarm_ANSI(3289, __FILE__, __LINE__,"dpc: %d opc: %d "
                        "Message: %x cic: %x", 
        ...
        SPRC_SendIndToApp_ANSI(ITS_ISUP_RSC_REATTEMPT_RQIND, cic_rec);
    ...
    else
    {
        ...
        /* Alert maintenance; Alarm */
        ISUP_Alarm_ANSI(3290, __FILE__, __LINE__, "Message: %x cic: %x", 
                        msg->data[sizeof(MTP3_HEADER_ANSI) +
                        sizeof (ITS_USHORT) + sizeof(ITS_OCTET)],
                        ISUP_PCIC_TO_CIC(cic_rec->pcic));
 ...
 *      for the WAIT_INCOMING_CRA state, based on the msgType received.
 *
 *  Input Parameters:
...
__ISUP_GetMsgProc_WaitIncomingCra_ANSI92(ITS_OCTET msgType)
{
    switch(msgType)
    {
                          ...
                          ISUP_OPC_DPC_INFO *config_info)
{
    ...
    ISUP_DEBUG(("WAIT_INCOMING_CRA: TimeoutTCRM received "
                "pcic 0x%08x, vcic %d, cic %d.\n",
                 cic_rec->pcic, cic_rec->vcic, ISUP_PCIC_TO_CIC(cic_rec->pcic)));

    /* Alert Maintenance; Alarm */
    ISUP_Alarm_ANSI(3291, __FILE__, __LINE__, "dpc: %d opc: %d cic: %x", 
    ...
    {
        /* log error */
        ISUP_ERROR(("Unable to remove TCRM info "
                           ...
                           ISUP_OPC_DPC_INFO *config_info)
{
    ...
    ISUP_DEBUG(("WAIT_INCOMING_CRA: TimeoutT28 received"
                " pcic 0x%08x, vcic %d, cic %d.\n",
                 cic_rec->pcic, cic_rec->vcic, ISUP_PCIC_TO_CIC(cic_rec->pcic)));
    ...
    /* Alert Maintenance; Alarm */
    ISUP_Alarm_ANSI(3292, __FILE__, __LINE__, "dpc: %d opc: %d cic: %x", 
                    MTP3_RL_GET_DPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                    MTP3_RL_GET_OPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                                          ISUP_PCIC_TO_CIC(cic_rec->pcic));
...
{
    /* Log error, update stats and drop msg */
    ISUP_ERROR(("WAIT_INCOMING_CRA: Unexpected Timeout T%d for CIC %d in "
                "cic state 0x%02x, pcic 0x%08x, vcic %d.\n", 
                t_entry->timerId, t_entry->cic,
                cic_rec->cic_state, cic_rec->pcic, cic_rec->vcic));

    /* Alert Maintenance, Alarm */
    ISUP_Alarm_ANSI(3293, __FILE__, __LINE__, "State: %x, Timeout T%d",
                                 t_entry->timerId,t_entry->timerId);
 ...
 *      the WAIT_INCOMING_CRA state, based on the ID of the timer that has 
 *      expired.
 *
 *  Input Parameters:
...
__ISUP_GetTimeoutProc_WaitIncomingCra_ANSI92(ITS_UINT timerID)
{
    switch(timerID)
    {
