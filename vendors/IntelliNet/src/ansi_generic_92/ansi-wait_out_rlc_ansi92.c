 ...
 ****************************************************************************
 *
 *  ID: $Id: ansi-wait_out_rlc_ansi92.c,v 9.2 2005/04/06 07:30:01 ssingh Exp $
 *
 * LOG: $Log: ansi-wait_out_rlc_ansi92.c,v $
 ...
 * LOG: Begin PR6.5
 * LOG:
 * LOG: Revision 8.10  2005/03/21 13:54:01  cvsadmin
 ...
 * LOG:
 * LOG: Revision 8.7  2003/05/06 13:55:53  sswami
 ...
 * LOG: Begin PR6.4
 * LOG:
 * LOG: Revision 7.8  2003/01/22 14:55:02  vnitin
 ...
 * LOG:
 * LOG: Revision 7.6  2002/12/06 05:39:03  akumar
 * LOG: Removed unwanted TABS
 * LOG:
 * LOG: Revision 7.5  2002/11/20 12:55:09  akumar
 * LOG: Worked on ALARMS indentation
 * LOG:
 * LOG: Revision 7.4  2002/11/12 06:08:20  akumar
 * LOG: Added ANSI ISUP Alarms
 * LOG:
 * LOG: Revision 7.3  2002/11/08 08:08:43  ssingh
 ...
 * LOG: Begin PR6.2
 * LOG:
 * LOG: Revision 1.3  2002/08/13 20:48:27  ssingh
 * LOG: Changed the debug statement text.
 * LOG:
 ...
 * LOG:
 * LOG: Revision 1.1.2.12  2002/07/14 19:01:41  hbalimid
 ...
 * LOG:
 * LOG: Revision 1.1.2.4  2002/07/04 23:30:08  hbalimid
...
/*
 * Functions for handling messages in the Wait_Outgoing_RLC state for ANSI92.
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an BLA received in the WAIT_OUTGOING_RLC state 
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an BLO received in the WAIT_OUTGOING_RLC state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGB received in the WAIT_OUTGOING_RLC state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGBA received in the WAIT_OUT_RLC state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGU received in the WAIT_OUTGOING_RLC state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGUA received in the WAIT_OUTGOING_RLC state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CQM received in the WAIT_OUTGOING_RLC state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an REL received in the WAIT_OUTGOING_RLC state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        ISUP_DEBUG(("Received REL in WAIT_OUT_RLC from MTP3, "
    ...
    else 
    {
        ...
        ISUP_ERROR(("Received REL in WAIT_OUT_RLC from APP instead of RLC, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
        /* Change state to WAIT_OUTGOING_RLC, i.e., no change. */

        ISUP_Alarm_ANSI(3129, __FILE__, __LINE__, "Message %x cic : %x",
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an RLC received in the WAIT_OUTGOING_RLC state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    if (src == ISUP_SRC_L3_ANSI) /* CPC incoming */
    {
         ...
         */
        ISUP_DEBUG(("Received RLC in WAIT_OUT_RLC from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        /* Release Request Indication to App */
    ...
    {
        ISUP_DEBUG(("Received RLC in WAIT_OUT_RLC from APP, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
        ...
        /* Idle ckt and fsm state to IDLE - clear cic_rec */
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an RSC received in the WAIT_OUTGOING_RLC state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UBA received in the WAIT_OUTGOING_RLC state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UBL received in the WAIT_OUTGOING_RLC state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 *      the WAIT_OUTGOING_RLC state. It also serves as a default handler for 
                              ...
                              ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ...
        ISUP_Alarm_ANSI(3130, __FILE__, __LINE__, "Message %x cic : %x",
                         msg->data[sizeof(MTP3_HEADER_ANSI) + 
                         sizeof(ITS_USHORT) + sizeof(ITS_OCTET)],
                         ISUP_PCIC_TO_CIC(cic_rec->pcic));
    }
    else
    {
        ...
        ISUP_Alarm_ANSI(3131, __FILE__, __LINE__, "Message: %x cic: %x",
                        msg->data[sizeof(MTP3_HEADER_ANSI) + 
                        sizeof (ITS_USHORT) + sizeof(ITS_OCTET)],
                        ISUP_PCIC_TO_CIC(cic_rec->pcic));
...
__ISUP_GetMsgProc_WaitOutgoingRlc_ANSI92(ITS_OCTET msgType)
{
    switch(msgType)
    {
    case ISUP_MSG_ACM:
                           ...
                           ISUP_OPC_DPC_INFO *config_info)  
{
    ...
    ISUP_DEBUG(("TimeoutT28 event received in WAIT_OUT_RLC state. "
    ...
    ISUP_Alarm_ANSI(3132, __FILE__, __LINE__, "dpc: %d opc: %d cic: %x",
    ...
    {
        /* log error */
        ISUP_ERROR(("TimeoutT28 in WAIT_OUT_RLC state: Unable to remove T28 info "
                                  ...
                                  ISUP_OPC_DPC_INFO *config_info)
{
    ...
    ISUP_ERROR(("Rcvd Unexpected Timeout T%d for CIC %d from MTP3 in "
                "WAIT_OUT_RLC. cic state 0x%02x, pcic 0x%08x, vcic %d.\n", 
                 t_entry->timerId, t_entry->cic,
                cic_rec->cic_state, cic_rec->pcic, cic_rec->vcic));
    /* Could print out mtp3 header or entire ITS_EVENT */ 


    ISUP_Alarm_ANSI(3133, __FILE__, __LINE__, "State: %x Timer id: T%d",
 ...
 *      the WAIT_OUTGOING_RLC state, based on the ID of the timer that has 
 *      expired.
 *
 *  Input Parameters:
...
__ISUP_GetTimeoutProc_WaitOutgoingRlc_ANSI92(ITS_UINT timerID)
{
    switch(timerID)
    {
