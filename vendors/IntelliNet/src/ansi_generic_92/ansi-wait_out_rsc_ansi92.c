 ...
 ****************************************************************************
 *
 *  ID: $Id: ansi-wait_out_rsc_ansi92.c,v 9.2 2005/04/06 07:30:01 ssingh Exp $
 *
 * LOG: $Log: ansi-wait_out_rsc_ansi92.c,v $
 * LOG: Revision 9.2  2005/04/06 07:30:01  ssingh
 * LOG: ISUP Bug Fix propagation. BugId #389 (kramesh)
 * LOG:
 * LOG: Revision 9.1  2005/03/23 12:54:55  cvsadmin
 * LOG: Begin PR6.5
 * LOG:
 ...
 * LOG:
 * LOG: Revision 7.8.2.1.2.1  2003/09/13 06:34:20  akumar
 ...
 * LOG:
 * LOG: Revision 8.5  2003/05/06 13:55:53  sswami
 ...
 * LOG:
 * LOG: Revision 7.6  2002/11/20 12:55:09  akumar
 ...
 * LOG:
 * LOG: Revision 1.1.2.8  2002/07/30 13:54:55  sjaddu
 * LOG: Added MTP indications and CFN procedures.
 * LOG:
 * LOG: Revision 1.1.2.7  2002/07/12 15:25:08  ssingh
 * LOG: Added/Changed CQM/CQR related methods.
 * LOG:
 * LOG: Revision 1.1.2.6  2002/07/10 21:36:58  hbalimid
 * LOG: OGC Answered complete.
 * LOG:
 * LOG: Revision 1.1.2.5  2002/07/10 19:32:12  ssharma
 * LOG: Changes for redundancy.
 * LOG:
 * LOG: Revision 1.1.2.4  2002/07/09 00:00:16  ssingh
...
/*
 * Functions for handling messages in the Wait_Outgoing_RSC state for ANSI92.
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    else 
    {
        ...
        ISUP_Alarm_ANSI(3134, __FILE__, __LINE__, "dpc: %d opc: %d "
                        "cic: %x  Message: %x",
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ...
        SPRC_SendIndToApp_ANSI(ITS_ISUP_RLC_RESET_IND, cic_rec);
                              ...
                              ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ...
        ISUP_Alarm_ANSI(3135, __FILE__, __LINE__, "dpc: %d opc: %d "
                        "cic: %x  Message: %x",
                        MTP3_RL_GET_DPC_VALUE_ANSI(config_info->out_hdr.label),
                        MTP3_RL_GET_OPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                        ISUP_PCIC_TO_CIC(cic_rec->pcic),
    ...
    else
    {
        ...
        ISUP_Alarm_ANSI(3136, __FILE__, __LINE__, "Message %x cic : %x",
                         msg->data[sizeof(MTP3_HEADER_ANSI) + 
                         sizeof(ITS_USHORT) + sizeof(ITS_OCTET)],
                         ISUP_PCIC_TO_CIC(cic_rec->pcic));
...
__ISUP_GetMsgProc_WaitOutgoingRsc_ANSI92(ITS_OCTET msgType)
{
    switch(msgType)
    {
    case ISUP_MSG_ACM:
                           ...
                           ISUP_OPC_DPC_INFO *config_info)  
{
    ...
    ISUP_Alarm_ANSI(3137, __FILE__, __LINE__, "dpc: %d opc: %d cic: %x",
                    MTP3_RL_GET_DPC_VALUE_ANSI(config_info->out_hdr.label),
                    MTP3_RL_GET_OPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                    ISUP_PCIC_TO_CIC(cic_rec->pcic));
                                  ...
                                  ISUP_OPC_DPC_INFO *config_info)
{
    ...
    ISUP_Alarm_ANSI(3138, __FILE__, __LINE__, "State: %x,Timer id: T%d",
...
__ISUP_GetTimeoutProc_WaitOutgoingRsc_ANSI92(ITS_UINT timerID)
{
    switch(timerID)
    {
