 ...
 *                                                                          *
 ****************************************************************************
 *  ID: $Id: ansi-wait_cotreports_and_cra_ansi92.c,v 9.2 2005/04/06 07:30:01 ssingh Exp $
 * LOG: $Log: ansi-wait_cotreports_and_cra_ansi92.c,v $
 ...
 * LOG: Begin PR6.5
 * LOG:
 * LOG: Revision 1.9  2005/03/21 13:53:57  cvsadmin
 * LOG: PR6.4.2 Source Propagated to Current
 * LOG:
 * LOG: Revision 1.6.2.2.6.1.12.3  2005/02/03 12:44:40  ssingh
 * LOG: Bug #1448; unexpected Msg from Network and peg/stats handling.
 * LOG: (Ramesh KV, Abhijit D)
 * LOG:
 * LOG: Revision 1.6.2.2.6.1.12.2  2004/12/29 14:35:54  csireesh
 * LOG: ISUP OAM Changes
 * LOG:
 * LOG: Revision 1.6.2.2.6.1.12.1  2004/12/06 13:34:50  mmanikandan
 * LOG: Windows Compilation Support
 * LOG:
 * LOG: Revision 1.6.2.2.6.1  2004/01/05 10:10:44  sjaddu
 * LOG: Merged code from tcs isup branch.
 * LOG:
 * LOG: Revision 1.6.2.2.2.1  2003/09/13 06:30:15  akumar
 * LOG: Msg Based redundancy works for ANSI.
 * LOG:
 * LOG: Revision 1.6.2.2  2003/06/13 13:20:07  ssingh
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        ISUP_DEBUG(("WAIT_COT_REPORTS_AND_CRA: Received IAM from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        /* Check node for Controlled/Controlling exchange */
        if (!__ISUP_IsExchangeControlling(msg, cic_rec, config_info))
        {
            /* Not-Controlling Exchange for this circuit; respect Incoming */

            /* Stop CRA timer i.e. TCRM */
        ...
        {
            /* Controlling exhange for this circuit;
             *  Disregard this IAM message
             */

            ISUP_DEBUG(("Received IAM Ignored! pcic 0x%08x, vcic %d.\n", 
                         cic_rec->pcic, cic_rec->vcic));
        }
    }
    else
    {
        ISUP_ERROR(("WAIT_COT_REPORTS_AND_CRA: Unexpected IAM from APP,"
                    " pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        /* Alert maintenance: Alarm */
        ISUP_Alarm_ANSI(3261, __FILE__, __LINE__, "Message:%x cic : %x",
        ...
        SPRC_SendErrorToApp_ANSI(ISUP_MSG_IAM,
                                 ITS_ISUP_UNEXP_APPMSG_WAITFOR_COTREPORTS_AND_CRA_ERR,
                                 cic_rec);
    }
    return;
}
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CRA received in the WAIT_COT_REPORTS_AND_CRA state 
 *      is done here. The message is forwarded to the application if received 
 ...
 ****************************************************************************/
static void
__ISUP_Received_CRA(ITS_EVENT *msg,  ISUP_CIC_INFO *cic_rec,
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    if (src == ISUP_SRC_L3_ANSI)    /* CPC outgoing */
    {
        ISUP_DEBUG(("WAIT_COT_REPORTS_AND_CRA: Received CRA from MTP3,"
                    " pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        __ISUP_StopTimer(cic_rec, ITS_ISUP_TIMER_TCRM);

        /* Already marked as outgoing busy while sending CRM */

        SPRC_SendIsupToApp_ANSI(cic_rec->vcic, msg);

        cic_rec->fsm_state = WAIT_COT_REPORTS_HAVE_CRA;
    }
    else
    {
        /* Msg out of sequence. Drop it and generate alarm. */
        ISUP_ERROR(("WAIT_COT_REPORTS_AND_CRA CRA: Unexpected CRA from App."
                    " pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));


        /* Alert maintenance: Alarm */
        ISUP_Alarm_ANSI(3262, __FILE__, __LINE__, "Message: %x cic : %x",
                        msg->data[sizeof(MTP3_HEADER_ANSI) + 
                        sizeof (ITS_USHORT) + sizeof(ITS_OCTET)],
                        ISUP_PCIC_TO_CIC(cic_rec->pcic));

        /* Send error indication to application. */
        SPRC_SendErrorToApp_ANSI(ISUP_MSG_CRA,
                            ITS_ISUP_UNEXP_APPMSG_WAITFOR_COTREPORTS_AND_CRA_ERR,
                            cic_rec);
    }
}
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an COT received in the WAIT_COT_REPORTS_AND_CRA state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or it is dropped.
 *
 *  Input Parameters:
                              ...
                              ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI) /* CPC Outgoing */
    {
        /* Msg out of sequence. Send reset and generate alarm. */
        ISUP_ERROR(("WAIT_COT_REPORTS_AND_CRA: Unexpected COT from MTP3",
                    " pcic 0x%08x,vcic %d.\n", cic_rec->pcic, cic_rec->vcic));

        /* Alert maintenance: Alarm */
        ISUP_Alarm_ANSI(3263, __FILE__, __LINE__, "dpc: opc: %d %d cic : %x",
        ...
        SPRC_SendIndToApp_ANSI(ITS_ISUP_RSC_CRMRETRY_RQIND, cic_rec);
    ...
    {
        ISUP_DEBUG(("WAIT_COT_REPORTS_AND_CRA: Received COT from App."
                    " pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an INF received in the WAIT_COT_REPORTS_AND_CRA state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an INR received in the WAIT_COT_REPORTS_AND_CRA state 
 ...
 ****************************************************************************
 *  Purpose:
 *     The processing for an REL received in WAIT_COT_REPORTS_AND_CRA state is 
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_DEBUG(("WAIT_COT_REPORTS_AND_CRA: Received REL from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
    ...
    {
        ISUP_DEBUG(("WAIT_COT_REPORTS_AND_CRA: Received REL from APP, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an RLC received in  WAIT_COT_REPORTS_AND_CRA state is 
 *      done here. The message is forwarded to the application if received  
 *      from the network, or vice versa.
 *
 *  Input Parameters:
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ISUP_DEBUG(("WAIT_COT_REPORTS_AND_CRA: Received RLC from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
    ...
    {
        ISUP_DEBUG(("WAIT_COT_REPORTS_AND_CRA: Received RLC from APP, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an LPA received in the WAIT_INCOMING_COT_AND_IAM state
 *      is done here. The message is forwarded to the application if received
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an BLA received in the WAIT_COT_REPORTS_AND_CRA state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an BLO received in the WAIT_COT_REPORTS_AND_CRA state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CPG received in the WAIT_COT_REPORTS_AND_CRA
                    ...
                    ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    {
        /* Msg out of sequence. Drop it and generate alarm. */
        ISUP_ERROR(("WAIT_COT_REPORTS_AND_CRA: Unexpextec CPG from App."
                    "  pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
     
        /* Alert Maintenance; Alarm */
        ISUP_Alarm_ANSI(3264, __FILE__, __LINE__, "Message: %d cic: %x", 
                        msg->data[sizeof(MTP3_HEADER_ANSI) + 
                        sizeof (ITS_USHORT) + sizeof(ITS_OCTET)],
                        ISUP_PCIC_TO_CIC(cic_rec->pcic));
    ...
    {
        ISUP_DEBUG(("WAIT_COT_REPORTS_AND_CRA: Received CPG from MTP3, "
                    "pcic 0x%08x, vcic %d.\n", cic_rec->pcic, cic_rec->vcic));
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGB received in the WAIT_COT_REPORTS_AND_CRA state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGBA received in the WAIT_COT_REPORTS_AND_CRA state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGU received in the WAIT_COT_REPORTS_AND_CRA state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CGUA received in the WAIT_COT_REPORTS_AND_CRA state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an GRA received in the WAIT_COT_REPORTS_AND_CRA state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an GRS received in the WAIT_COT_REPORTS_AND_CRA state 
 *      is done here. The message is forwarded to the application if received 
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an RSC received in  WAIT_COT_REPORTS_AND_CRA state is 
 *      done here.
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UBA received in  WAIT_COT_REPORTS_AND_CRA state is 
 *      done here. The message is forwarded to the application if received  
 *      from the network.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UBL received in  WAIT_COT_REPORTS_AND_CRA state is 
 *      done here. The message is forwarded to the application if received  
 *      from the network.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an UCIC received in the WAIT_COT_REPORTS_AND_CRA state
 *      is done here. The message is forwarded to the application if received
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CQM received in WAIT_COT_REPORTS_AND_CRA.
 *      The message is forwarded to the application if received from the
 *      network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CQR received in WAIT_COT_REPORTS_AND_CRA.
 *      The message is forwarded to the application if received from the
 *      network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CVT received in WAIT_COT_REPORTS_AND_CRA
 *      is done here. The message is forwarded to the application if received
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 ****************************************************************************
 *  Purpose:
 *      The processing for an CVR received in WAIT_COT_REPORTS_AND_CRA
 *      is done here. The message is forwarded to the application if received
 *      from the network, or vice versa.
 *
 *  Input Parameters:
 ...
 *      the WAIT_COT_REPORTS_AND_CRA state. It also serves as a default handler for 
                              ...
                              ISUP_OPC_DPC_INFO *config_info, ITS_OCTET src)
{
    ...
    if (src == ISUP_SRC_L3_ANSI)
    {
        ...
        /* Alert Maintenance; Alarm */
        ISUP_Alarm_ANSI(3265, __FILE__, __LINE__, "dpc: %d opc: %d "
                       "Message: %x cic: %x",
                       MTP3_RL_GET_DPC_VALUE_ANSI(config_info->out_hdr.label),
                       MTP3_RL_GET_OPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                       msg->data[sizeof(MTP3_HEADER_ANSI) +
    ...
    else
    {
        ...
        /* Alert maintenance; Alarm */
        ISUP_Alarm_ANSI(3266, __FILE__, __LINE__, "Message: %x cic: %x", 
                        msg->data[sizeof(MTP3_HEADER_ANSI) +
                        sizeof(ITS_USHORT) + sizeof(ITS_OCTET)],
                        ISUP_PCIC_TO_CIC(cic_rec->pcic));
 ...
 *      for the WAIT_COT_REPORTS_AND_CRA state, based on the msgType received.
 *
 *  Input Parameters:
...
__ISUP_GetMsgProc_WaitCotReportAndCra_ANSI92(ITS_OCTET msgType)
{
    switch(msgType)
    {
    ...
    case ISUP_MSG_CRA:
        return (__ISUP_MsgHandlerProc)__ISUP_Received_CRA;
    case ISUP_MSG_IAM:
 ...
 *      WAIT_COT_REPORTS_AND_CRA state. 
 *
 *  Input Parameters:
 ...
 ****************************************************************************/
static void
__ISUP_Received_TimeoutTCRM(ISUP_TIMER_ENTRY* t_entry, ISUP_CIC_INFO *cic_rec,
                          ISUP_OPC_DPC_INFO *config_info)
{
    int ret = ITS_SUCCESS;

    ISUP_DEBUG(("WAIT_COT_REPORTS_AND_CRA: Timeout TCRM received. "
                " pcic 0x%08x, vcic %d, cic %d.\n",
                 cic_rec->pcic, cic_rec->vcic, ISUP_PCIC_TO_CIC(cic_rec->pcic)));

    /* Alert Maintenance: Alarm */
    ISUP_Alarm_ANSI(3267, __FILE__, __LINE__, "dpc: %d opc: %d cic: %x", 
                    MTP3_RL_GET_DPC_VALUE_ANSI(config_info->opc_dpc_ni.label), 
                    MTP3_RL_GET_OPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                    ISUP_PCIC_TO_CIC(cic_rec->pcic));

    /* Remove info about timer from cic_rec */
    ret = ISUP_DeletePndgTmrInfo(cic_rec, ITS_ISUP_TIMER_TCRM);
    if (ret != ITS_SUCCESS)
    {
        /* log error */
        ISUP_ERROR(("Timeout TCRM: Unable to remove TCRM info "
    ...
    if (config_info->tmr_hndl_info[ITS_ISUP_TIMER_TCRM].tmr_hndl_layer 
        == ISUP_TMR_HNDL_STACK)
    {
        /* Send REL to L3 */
        SPRC_SendRELToMTP3_ANSI(CI_CAUSE_NORMAL_UNSPECIFIED,
...
__ISUP_Received_TimeoutT24(ISUP_TIMER_ENTRY* t_entry, ISUP_CIC_INFO *cic_rec,
                           ISUP_OPC_DPC_INFO *config_info)
{
    ISUP_DEBUG(("WAIT_COT_REPORTS_AND_CRA: TimeoutT24 received. "
                " pcic 0x%08x, vcic %d, cic %d.\n",
                 cic_rec->pcic, cic_rec->vcic, ISUP_PCIC_TO_CIC(cic_rec->pcic)));
    ...
    /* Alert Maintenance: Alarm */
    ISUP_Alarm_ANSI(3268, __FILE__, __LINE__, "dpc: %d opc: %d cic: %x", 
                    MTP3_RL_GET_DPC_VALUE_ANSI(config_info->opc_dpc_ni.label), 
                    MTP3_RL_GET_OPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                    ISUP_PCIC_TO_CIC(cic_rec->pcic));

    /*Send Re-attempt indication */
    SPRC_SendIndToApp_ANSI(ITS_ISUP_FAILURE_CRMRETRY_RQIND, cic_rec);
                           ...
                           ISUP_OPC_DPC_INFO *config_info)
{
    ...
    ISUP_DEBUG(("WAIT_COT_REPORTS_AND_CRA: TimeoutT28 received. "
                " pcic 0x%08x, vcic %d, cic %d.\n",
                 cic_rec->pcic, cic_rec->vcic, ISUP_PCIC_TO_CIC(cic_rec->pcic)));
    ...
    /* Alert Maintenance: Alarm */
    ISUP_Alarm_ANSI(3269, __FILE__, __LINE__, "dpc: %d opc: %d cic: %x", 
                    MTP3_RL_GET_DPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                    MTP3_RL_GET_OPC_VALUE_ANSI(config_info->opc_dpc_ni.label),
                    ISUP_PCIC_TO_CIC(cic_rec->pcic));
...
{
    /* Log error, update stats and drop msg */
    ISUP_ERROR(("WAIT_COT_REPORTS_AND_CRA: Unexpected Timeout T%d received "
                "for CIC %d cic state 0x%02x, pcic 0x%08x, vcic %d.\n", 
                t_entry->timerId, t_entry->cic,
                cic_rec->cic_state, cic_rec->pcic, cic_rec->vcic));

    /* Alert Maintenance, Alarm */
    ISUP_Alarm_ANSI(3270, __FILE__, __LINE__, "state: %x, Timeout T%d", 
 ...
 *      the WAIT_COT_REPORTS_AND_CRA state, based on the ID of the timer that has 
 *      expired.
 *
 *  Input Parameters:
...
__ISUP_GetTimeoutProc_WaitCotReportAndCra_ANSI92(ITS_UINT timerID)
{
    switch(timerID)
    {
    ...
    case ITS_ISUP_TIMER_T25: /* 25   CCR timing timer       */
        return (__ISUP_TimeoutHandlerProc)__ISUP_CSC_Received_TimeoutT25;
    case ITS_ISUP_TIMER_T26: /* 26   CCR response timer     */
        return (__ISUP_TimeoutHandlerProc)__ISUP_CSC_Received_TimeoutT26;
    ...
    case ITS_ISUP_TIMER_TCCR: /* CCR          */
        return (__ISUP_TimeoutHandlerProc)__ISUP_CSC_Received_TimeoutTCCR;
    case ITS_ISUP_TIMER_TCRM: /* CRM        */
        return (__ISUP_TimeoutHandlerProc)__ISUP_Received_TimeoutTCRM;
    default:
