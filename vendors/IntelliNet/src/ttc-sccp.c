 ...
 ****************************************************************************
 *
 *  ID: $Id: ttc-sccp.c,v 9.6.6.1 2009/12/21 03:46:43 rajeshak Exp $
 *
 * LOG: $Log: ttc-sccp.c,v $
 * LOG: Revision 9.6.6.1  2009/12/21 03:46:43  rajeshak
 * LOG: Removing CTF_TraceEvent function call in vendors
 * LOG:
 * LOG: Revision 9.6  2008/07/18 05:44:02  ssingh
 ...
 * LOG:
 * LOG: Revision 9.3  2008/04/15 05:03:18  kramesh
 * LOG: Merged Persistent Branch Changes to current.
 * LOG:
 * LOG: Revision 9.2.2.1  2007/05/18 06:42:06  pspl
 ...
 * LOG:
 * LOG: Revision 1.1.2.12  2008/02/27 07:30:51  ssodhi
 ...
 * LOG: 3. -c option in st_get_conns command to print only number of Connections
 * LOG:
 * LOG: Revision 1.1.2.11  2008/02/13 08:02:41  ssodhi
 ...
 * LOG:
 * LOG: Revision 1.1.2.10  2008/02/08 06:34:30  ssodhi
 * LOG: Displaying connection reference also.
 * LOG:
 * LOG: Revision 1.1.2.9  2008/01/14 10:16:53  ssodhi
 * LOG: Code changes for Accelero Performance Enhancement - ALU BSG Project
 * LOG:
 * LOG: Revision 1.1.2.8  2007/10/09 11:41:15  mshanmugam
 * LOG: Validation check is done for CREF number
 * LOG:
 * LOG: Revision 1.1.2.7  2007/04/10 15:32:44  raghavendrabk
 * LOG: Check Validation for SCCP_TTC_TraceData
 * LOG:
 * LOG: Revision 1.1.2.6  2007/02/26 10:23:14  raghavendrabk
 * LOG: Family filled in OAM API
 * LOG:
 * LOG: Revision 1.1.2.5  2007/02/13 04:00:20  raghavendrabk
 * LOG: Changed alarm format to put family at beginning
 * LOG:
 * LOG: Revision 1.1.2.4  2007/02/05 06:17:48  clin
 ...
 * LOG:
 * LOG: Revision 1.1.2.3  2007/02/02 14:17:41  raghavendrabk
 * LOG: Parallel Stack changes for Alarms and Statistics
 * LOG:
 * LOG: Revision 1.1.2.2  2007/01/19 07:57:31  ssingh
...
#ident "$Id: ttc-sccp.c,v 9.6.6.1 2009/12/21 03:46:43 rajeshak Exp $"
 ...
 * global data
 */
TQUEUETRAN_Manager* __SCCP_TTC_RcvQueue = NULL;

/*
 * local data
 */
THREAD_RET_TYPE
TTC_SCCPThread(void *arg);
...
static ITS_CHAR* SCCP_TTC_Timers[]=
{
...
TRACE_Data* SCCP_TTC_TraceData = NULL;

ITS_BOOLEAN gcsMessageBasedRedundancyIsActive = ITS_FALSE;

static ITS_UINT pegs[PEG_SCCP_NUM_PEGS];
static PEGS_DECL_SUBSYS(TTC_SCCP, pegs);
PEG_Manager *TTC_SCCP_Pegs = &TTC_SCCP_PEGS;

static ITS_TIME timers[SCCP_NUM_TIMERS];
TIMERS_DECL_SUBSYS(TTC_SCCP, timers);

static ITS_BOOLEAN behaviors[SCCP_NUM_BEHAVIORS];
BEHAVIORS_DECL_SUBSYS(TTC_SCCP, behaviors);
...
void SCCP_ResetOm_TTC();
void SCCP_GetOm_TTC(sccpOm*);
#if defined(__cplusplus)
}
#endif /* defined(__cplusplus) */
void SCCP_ResetOm_MML_TTC();
void SCCP_GetOm_MML_TTC(DBC_Server *);
void SCCPPegsOmDataInitialize(sccp_gen_stat_table *);
void SCCPSSNPegsOmDataInitialize(sccp_ssn_stat_table*);

extern int
SendSCMG_TTC(MTP3_HEADER* mtp3, SCCP_SCMG_TTC* scmg, ITS_UINT toSSN);

extern int 
SSTCStartSST_TTC(MTP3_HEADER* mtp3, ITS_UINT pc, ITS_OCTET ssn);

/*
 * management class record
 */
static SCCP_MgmtClassRec SCCP_MgmtClassRec_TTC =
{
    /* core part */
    {
    ...
    /* management part */
    {
        ...
        /* function part */
        SCCP_GetGeneralCfg_TTC,
        SCCP_SetGeneralCfg_TTC,
        SCCP_GetSSEntry_TTC,
        SCCP_GetAllSSEntries_TTC,
        SCCP_SetSSEntry_TTC,
        SCCP_RemSSEntry_TTC,
        SCCP_GetPointCodeEntry_TTC,
        SCCP_GetAllPointCodeEntries_TTC,
        SCCP_SetPointCodeEntry_TTC,
        SCCP_RemPointCodeEntry_TTC,
        SCCP_GetConcernedPC_TTC,
        SCCP_GetAllCPCEntries_TTC,
        SCCP_SetConcernedPC_TTC,
        SCCP_RemConcernedPC_TTC,
        SCCP_GetConcernedSS_TTC,
        SCCP_GetAllCSSEntries_TTC,
        SCCP_SetConcernedSS_TTC,
        SCCP_RemConcernedSS_TTC,
        SCCP_SendSCMG_TTC,
        SCCP_SendSST_TTC,
...
ITSSS7DLLAPI ITS_Class SCCP_MgmtClass_TTC =
    (ITS_Class)&SCCP_MgmtClassRec_TTC;

/* implementation:
 ***************************************************************************
 *  Purpose:
 *	This function cancel all the connection timers
 *  
 *  Input Parameters:
 *	None.
 * 
...
int SCCP_CancelTimers(SCCP_CONN_CTXT *foundCtxt)
{
     ...
     SCCP_DEBUG(("SCCP_CancelTimers: Cancelling all Timers for conRef %04x(hex) %u(dec)\n",
		 foundCtxt->connRef,foundCtxt->connRef));
 ...
 ****************************************************************************/
void
SCCP_DeleteLocalReference_TTC(ITS_CTXT cref)
{
    ...
    DSM_LockTableLocal(DSM_SccpFtGroupId, DSM_TABLE_TTC_SCCP_CTXT);
    ...
    if (error != ITS_SUCCESS)
    {
        ...
        case (ITS_EBADMUTEX):
            ...
            break;
        case (ITS_ENOTFOUND):
            SCCP_WARNING(("DeleteLocalRef: Could not find context (cref: %u).\n",cref));
            break;
        case (ITS_EINUSE):
        ...
        DSM_UnlockTableLocal(DSM_SccpFtGroupId, DSM_TABLE_TTC_SCCP_CTXT);
    ...
    if (error != ITS_SUCCESS)
    {
        SCCP_ERROR(("DeleteLocalRef: DSM unable to free Reference %u.\n",cref));
    }
 ...
 ****************************************************************************/
int
INTELLINET_GetSCCPNextRef_TTC(ITS_SS7_HANDLE handle,
                                ITS_CTXT *cref)
{
    ...
    if (DSM_LockTableLocal(
            DSM_SccpFtGroupId, 
            DSM_TABLE_TTC_SCCP_CTXT) != ITS_SUCCESS)
    {
 ...
 ****************************************************************************/
int
SCCP_GetSharedLocalRef_TTC(void *ctxtData,
                             int len)
{
    ITS_EVENT ev;
 ...
 ****************************************************************************/
ITS_BOOLEAN
SCCP_IsSSNEquipped_TTC(ITS_OCTET ni, ITS_UINT pc, ITS_OCTET ssn)
{
    ITS_BOOLEAN isLocal = ITS_FALSE;

    SCCP_DEBUG(("SCCP_IsSSNEquipped_TTC:: pc %x, ssn %x, ni %x\n", pc, ssn, ni));
    ...
    SCCP_DEBUG(("SCCP_IsSSNEquipped_TTC::Route context status lookup success\n"));
 ...
 ****************************************************************************/
ITS_BOOLEAN
SCCP_IsSSNAllowed_TTC(ITS_OCTET ni, ITS_UINT pc, ITS_OCTET ssn)
{
    ...
    if (ROUTE_GetRouteContextStatus(pc, &isAllowed,
                                    ...
                                    ITS_SS7_DEFAULT_SLS) != ITS_SUCCESS)
    {
        SCCP_DEBUG(("SCCP_IsSSNAllowed_TTC::Route context status lookup failed\n"));
 ...
 ****************************************************************************/
ITS_OCTET
SCCP_IsSSNCongested_TTC(ITS_OCTET ni, ITS_UINT pc, ITS_OCTET ssn)
{
    ITS_OCTET congLevel;
...
static ITS_BOOLEAN
CheckForCSS_TTC(ROUTE_Info *linfo)
{
    ...
    {
        if (DSM_LockTable(DSM_SccpFtGroupId, DSM_TABLE_TTC_SCCP_LPC)
                                                                != ITS_SUCCESS)
        {
        ...
        rinfo = (ROUTE_Info *)DSM_FindNoLock(DSM_SccpFtGroupId,
                                                 DSM_TABLE_TTC_SCCP_LPC,
                                                 (ITS_OCTET *)&ctxt,
                                                 sizeof(ctxt), &error);
        if (error != ITS_SUCCESS)
        {
           DSM_UnlockTable(DSM_SccpFtGroupId, DSM_TABLE_TTC_SCCP_LPC);
    ...
    else
    {
        ...
        if (DSM_LockTable(DSM_SccpFtGroupId, DSM_TABLE_TTC_SCCP_CPC)
                                                                != ITS_SUCCESS)
        {
            SCCP_ERROR(("Can't lock table\n"));
        ...
        rinfo = (ROUTE_Info *)DSM_FindNoLock(DSM_SccpFtGroupId,
                                                 DSM_TABLE_TTC_SCCP_CPC,
                                                 (ITS_OCTET *)&ctxt,
                                                 sizeof(ctxt), &error);
        if (error != ITS_SUCCESS)
        {
           DSM_UnlockTable(DSM_SccpFtGroupId, DSM_TABLE_TTC_SCCP_CPC);
...
SCCP_AddConcernedSubSystem(ROUTE_Info *linfo, ROUTE_Info *cinfo)
{
    ...
    if (DSM_LockTable(
            DSM_SccpFtGroupId, 
            DSM_TABLE_TTC_SCCP_LPC) != ITS_SUCCESS)
    {
         /* complain */
    ...
    if (DSM_LockTable(
            DSM_SccpFtGroupId,
            DSM_TABLE_TTC_SCCP_CPC) != ITS_SUCCESS)
    {
        /* complain */
    ...
    if ((current = (ROUTE_Info *)DSM_FindNoLock(DSM_SccpFtGroupId,
                                                ...
                                                &error)) != NULL)
    {
        ret = AddNode(DSM_TABLE_TTC_SCCP_LPC, ctxt, current, cinfo);
    ...
    if ((current = (ROUTE_Info *)DSM_FindNoLock(DSM_SccpFtGroupId,
                                                ...
                                                &error)) != NULL)
    {
        ret = AddNode(DSM_TABLE_TTC_SCCP_CPC, ctxt, current, linfo);
...
SCCP_RemConcernedSubSystem(ROUTE_Info *linfo, ROUTE_Info *cinfo)
{
    ...
    if ((current = (ROUTE_Info *)DSM_FindNoLock(DSM_SccpFtGroupId,
                                                ...
                                                &error)) != NULL)
    {
        ret = RemNode(DSM_TABLE_TTC_SCCP_LPC, current, cinfo);
    ...
    if ((current = (ROUTE_Info *)DSM_FindNoLock(DSM_SccpFtGroupId,
                                                ...
                                                &error)) != NULL)
    {
        ret = RemNode(DSM_TABLE_TTC_SCCP_CPC, current, linfo);
...
SCCP_AddDuplicatedSubSystem(ROUTE_Info *rinfo)
{
    ...
    if (DSM_LockTable(
            DSM_SccpFtGroupId, 
            DSM_TABLE_TTC_SCCP_DUPS) != ITS_SUCCESS)
    {
        /* complain */
                                ...
                                &error)) != NULL)
    {
         ...
         */
        DSM_UnlockTable(DSM_SccpFtGroupId, DSM_TABLE_TTC_SCCP_DUPS);
...
SCCP_GetGeneralCfg_TTC()
{
    ...
    ret->traceOn = (TRACE_IsLevelOn(SCCP_TTC_TraceData,
                                    SCCP_TRACE_DEBUG) &&
                    (TRACE_IsOutputOn(SCCP_TTC_TraceData,
                                      SCCP_TRACE_DEBUG,
                                      0) ||
                     TRACE_IsOutputOn(SCCP_TTC_TraceData,
                                      SCCP_TRACE_DEBUG,
                     ...
                     TRACE_IsOutputOn(SCCP_TTC_TraceData,
                                      SCCP_TRACE_DEBUG,
                                      2))) ||
                   (TRACE_IsLevelOn(SCCP_TTC_TraceData,
                                    SCCP_TRACE_WARNING) &&
                    (TRACE_IsOutputOn(SCCP_TTC_TraceData,
                                      SCCP_TRACE_WARNING,
                                      0) ||
                   ...
                   (TRACE_IsLevelOn(SCCP_TTC_TraceData,
                    ...
                    (TRACE_IsOutputOn(SCCP_TTC_TraceData,
                     ...
                     TRACE_IsOutputOn(SCCP_TTC_TraceData,
                                      SCCP_TRACE_EVENT,
                                      2)));
    ret->alarmLevel = SCCP_AlarmLevel_TTC;
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_SetGeneralCfg_TTC(SCCPGeneralCfg *data)
{
    if(SCCP_TTC_TraceData == NULL)
    {
      return !ITS_SUCCESS;
    }

    SCCP_AlarmLevel_TTC = data->alarmLevel;

    if (strstr(data->traceType, "all") != NULL)
    { 
        if (data->traceOn == ITS_TRUE)
        {
            TRACE_SetOutputOnOffByName(SCCP_TTC_TraceData,
                                       SCCP_CRITICAL_STRING,
                                       data->traceOutput, ITS_TRUE);
            TRACE_SetLevelOnOffByName(SCCP_TTC_TraceData,
...
SCCP_GetSSEntry_TTC(ITS_UINT sspc, ITS_OCTET ni, ITS_OCTET ssn)
{
    ...
    ret->testOn = SCMGIsSSTPending_TTC(NULL);
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_SendSST_TTC(SCCPConcernedSSInfo *data)
{
    ...
    error = SSTCStartSST_TTC(&hdr, cpc, cssn);
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_SendSCMG_TTC(SCCPConcernedSSInfo *data)
{
    ...
    if (!SCCP_IsSSNEquipped_TTC(ni, apc, data->ssn))
    {
     ...
     * Lock the tables we need
     */
    if (DSM_LockTable(DSM_SccpFtGroupId, DSM_TABLE_TTC_SCCP_LPC) != ITS_SUCCESS)
    {
        SCCP_ERROR(("Can't lock table\n"));
    ...
    if (!data->inservice)
    {
        ...
        PEG_IncrPeg(TTC_SCCP_Pegs, PEG_SCCP_LOCAL_SS_PROHIBITED);
    }
    else
    {
        scmg.type = SCCP_SCMG_SS_ALLOWED;
        PEG_IncrPeg(TTC_SCCP_Pegs, PEG_SCCP_LOCAL_SS_ALLOWED);
    }
    scmg.ssn = data->ssn;
    ...
    for (tinfo = rinfo; tinfo != NULL; tinfo = tinfo->next)
    {
                           ...
                           (tinfo->basic.dpc == data->cpc))
        {
            ...
            error = SendSCMG_TTC(&hdr, &scmg, tinfo->basic.criteria.ssn);
...
GetAllSSNRoutes(ITS_POINTER entry, void *in, void *out)
{
    ...
    if (rinfo->basic.family != FAMILY_JAPAN)
    {
        return (ITS_SUCCESS);
    }
...

    {
        ...
        if (*iter->entries != NULL)
        {
            ...
            ent->hasConcernPC = CheckForCSS_TTC(rinfo);
            ...
            ent->testOn = SCMGIsSSTPending_TTC(rinfo);
 ...
 ****************************************************************************/
ITSSS7DLLAPI void
SCCP_GetAllSSEntries_TTC(ITS_UINT* numEntries,
                           SCCPSubSystemInfo** entries)
{
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_SetSSEntry_TTC(SCCPSubSystemInfo *data)
{
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_RemSSEntry_TTC(SCCPSubSystemInfo *data)
{
    ROUTE_MatchInfo rinfo;
    int ret;
...
SCCP_GetPointCodeEntry_TTC(ITS_OCTET ni, ITS_UINT pc)
{
    ...
    dest = ROUTE_FindDestination(pc, ni, FAMILY_JAPAN);
    if (dest == NULL)
    {
        ROUTE_UnlockTable();
 ...
 ****************************************************************************/
ITSSS7DLLAPI void
SCCP_GetAllPointCodeEntries_TTC(ITS_UINT* numEntries,
                                  SCCPPointCodeInfo** entries)
{
    PCIterStruct iter;
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_SetPointCodeEntry_TTC(SCCPPointCodeInfo *data)
{
    ROUTE_MatchInfo rinfo;
    int ret;
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_RemPointCodeEntry_TTC(SCCPPointCodeInfo *data)
{
    ROUTE_MatchInfo rinfo;
    int ret;
...
SCCP_GetConcernedPC_TTC(ITS_UINT sspc, ITS_OCTET ni,
 ...
 ****************************************************************************/
ITSSS7DLLAPI void
SCCP_GetAllCPCEntries_TTC(ITS_UINT* numEntries,
                            SCCPConcernedPCInfo** entries)
{
    CPCIterStruct iter;
    ...
    *numEntries = 0;
    ...
    DSM_TableIterate(DSM_SccpFtGroupId, DSM_TABLE_TTC_SCCP_LPC,
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_SetConcernedPC_TTC(SCCPConcernedPCInfo* data)
{
    ...
    cinfo.basic.family = FAMILY_JAPAN;
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_RemConcernedPC_TTC(SCCPConcernedPCInfo* data)
{
...
SCCP_GetConcernedSS_TTC(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn, 
...
GetAllConcernedSS(ITS_POINTER entry, void *in, void *out)
{
    ...
    while (rinfo != NULL)
    {
        ...
        if (*iter->entries != NULL)
        {
            ...
            inService = SCCP_IsSSNAllowed_TTC(ent->ni,ent->cpc,ent->cssn);
 ...
 ****************************************************************************/
ITSSS7DLLAPI void
SCCP_GetAllCSSEntries_TTC(ITS_UINT* numEntries,
                            SCCPConcernedSSInfo** entries)
{
    CSSIterStruct iter;
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_SetConcernedSS_TTC(SCCPConcernedSSInfo *data)
{
    ROUTE_Info* rinfo, cinfo, *tlList;
    SCCPConcernedSSInfo* tmp = NULL;
    ITS_BOOLEAN found = ITS_FALSE;
    ...
    tmp = SCCP_GetConcernedSS_TTC(data->pc,
                                   data->ni,
                                   data->ssn,
                                   data->cpc,
                                   data->cssn);
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_RemConcernedSS_TTC(SCCPConcernedSSInfo *data)
{
   ...
   lpcList = NULL;
    for (rinfo = tlList; rinfo != NULL; rinfo = rinfo->next)
    {
        ...
        for (cinfo = tcList; cinfo != NULL; cinfo = cinfo->next)
        {
            ...
            if (IsConcerned(rinfo, cinfo))
            {
                tmp = SCCP_GetConcernedSS_TTC(rinfo->basic.dpc,
...
CheckTimeout(ITS_TimerData *td)
{
    ...
    {
	    /* Q752 Implementation */
	    /* Reassembly error- Timer_Treass_expiry */
	    SccpGenOmImage[omBuf.activeOm].SCCPGenPegs.timerTreassExpiry++;

	    SCLCHandleReasmTimer_TTC(td);
	    return;
    }
    ...
    key = DSM_MGMT_REC_TTC_SCCP_SST_SERIAL;
        ...
        error == ITS_SUCCESS)
    {
        if (td->serial == *serial)
        {
            ...
            SCMGHandleSSTTimeout_TTC();

            return;
        }
    }

    key = DSM_MGMT_REC_TTC_SCCP_SOR_SERIAL;
    if ((serial = DSM_FindNoLock(DSM_SccpFtGroupId,
                                 DSM_TABLE_MGMT_DATA,
                                 (ITS_OCTET *)&key, sizeof(ITS_UINT),
                                 &error)) != NULL &&
    ...
    key = DSM_MGMT_REC_TTC_SCCP_SST_IGNORE_SERIAL;
    if ((serial = DSM_FindNoLock(DSM_SccpFtGroupId,
                                 DSM_TABLE_MGMT_DATA,
                                 (ITS_OCTET *)&key, sizeof(ITS_UINT),
                                 &error)) != NULL &&
...
InitSettings()
{
     ...
     * timers
     */
    TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_statInfo, 30);
    TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_coordChg, 30);
    TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_ignoreSST, 30);
    TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_rtgStatInfo, 30);
    TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_conn, 180);
    TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_release, 10);
    TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_reset, 20);
    TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_ias, 300);
    TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_iar, 660);
    TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_guard, 240);
    TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_reassembly, 10);
    TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_interval, 30);
    TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_repeatRelease, 10);

    /*
     * behavior control
     */
    BEHAVIORS_SetBehavior(&TTC_SCCP_Behaviors, SCCP_B_propagate_IT,
                          ITS_FALSE);
    BEHAVIORS_SetBehavior(&TTC_SCCP_Behaviors, SCCP_B_propagate_ERR,
                          ITS_FALSE);
    BEHAVIORS_SetBehavior(&TTC_SCCP_Behaviors, SCCP_B_propagate_SCMG,
                          ITS_TRUE);
    BEHAVIORS_SetBehavior(&TTC_SCCP_Behaviors, SCCP_B_use_SCMG,
                          ITS_TRUE);

    /*
     * determine tracing from INI files
     */
    if (TRANSPORT_RES(__SCCP_TTC_RcvQueue) != NULL)
    {
        char value[ITS_PATH_MAX];
        RESFILE_Manager *res = TRANSPORT_RES(__SCCP_TTC_RcvQueue);
        char *name = TRANSPORT_NAME(__SCCP_TTC_RcvQueue);
        ...
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TSTAT_INFO_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_statInfo,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TCOORD_CHG_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_coordChg,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TIGNORE_SST_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_ignoreSST,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TRTG_STAT_INFO_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_rtgStatInfo,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TCONN_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_conn,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TRESET_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_reset,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TRELEASE_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_release,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TINACT_SEND_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_ias,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TINACT_RECV_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_iar,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TGUARD_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_guard,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TREASSEM_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_reassembly,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TINTERVAL_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_interval,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TREPEAT_REL_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_repeatRelease,
                                 atoi(value));
        }
        ...
        if (RESFILE_GetKeyValueOf(res, name, SCCP_PROPAGATE_IT_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            BEHAVIORS_SetBehavior(&TTC_SCCP_Behaviors,
                                  SCCP_B_propagate_IT,
                                  (strcmp(value, "yes") == 0 ||
                                   strcmp(value, "YES") == 0));
        }
 ...
 ****************************************************************************/
void
ISCCP_Terminate_TTC(ITS_SS7_HANDLE handle)
{
    ITS_EVENT ev;
    void *status;
    ITS_UINT key;
    ...
    {
        DBC_RemDoCommand(SCCP_Console_TTC);
    }
    ...
    assert(__SCCP_TTC_RcvQueue != NULL);
    TQUEUETRAN_DeleteTransport(__SCCP_TTC_RcvQueue);

    SCCP_Alarm_TTC(423, __FILE__, __LINE__, "family=%s", "TTC");

    TRACE_Destructor(SCCP_TTC_TraceData);
}
 ...
 ****************************************************************************/
static void
SCCP_StateChangeCallback_TTC(ITS_POINTER object,
 ...
 ****************************************************************************/
int
ISCCP_Initialize_TTC(ITS_SS7_HANDLE handle)
{
    ITS_SERIAL *serial;
    ITS_UINT key;
    int i, j, error;
    ...
    if ((SCCP_TTC_TraceData = TRACE_Constructor("TTC-SCCP",
    ...
    if ((__SCCP_TTC_RcvQueue = TQUEUETRAN_CreateTransport("TTC-SCCP",
                                                            ITS_SCCP_SRC,
                                                            ITS_SCCP)) == NULL)
    {
        ...
        initialized = ITS_FALSE;
        SCCP_Alarm_TTC(416, __FILE__, __LINE__, "family=%s", "TTC");
    ...
    for (i = 0; i < numThreads; i++)
    {
        if (THREAD_CreateThread(&sccpThread[i], 0, TTC_SCCPThread,
    ...
    DSM_LockTable(DSM_SccpFtGroupId, DSM_TABLE_MGMT_DATA);
...

    {
        ...
        *connRefNum = 0;
        ...
        DSM_CommitNoLock(DSM_SccpFtGroupId, DSM_TABLE_MGMT_DATA,
    ...
    SCCP_Alarm_TTC(422, __FILE__, __LINE__, "family=%s", "TTC");

    if ((dbClass = ITS_FindFeature(DBC_CLASS_NAME)) != NULL)
    {
        DBC_AddDoCommand(SCCP_Console_TTC);
    }
    ...
    if (redunClass != NULL)
    {
        ...
        if (REDUNDANCY_CONFIGURED_TYPE(redunClass)  == ITS_ACTIVE_STANDBY)
        {
            ...
            CALLBACK_AddCallback(REDUNDANCY_CALLBACK_MGR(redunClass),
                                 SCCP_StateChangeCallback_TTC, NULL);
 ...
 ****************************************************************************
 *  Purpose:
 *      This function is the entry point for the TTC SCCP thread.
 *
 *  Input Parameters:
 *      None.
 *
...
TTC_SCCPThread(void *arg)
{
    ...
    TRANSPORT_IS_ACTIVE(__SCCP_TTC_RcvQueue) = ITS_TRUE;

    while (cont &&
           TRANSPORT_CLASS_GET_NEXT_EVENT(ITS_OBJ_CLASS(__SCCP_TTC_RcvQueue))
                                                        (__SCCP_TTC_RcvQueue,
                                                         &sccpEvent)
           == ITS_SUCCESS)
    {
        ...
        case ITS_SCCP_LOCAL_REF_RESPONSE:
        {
            ...
            if (INTELLINET_GetSCCPNextRef_TTC(NULL,
        ...
        case ITS_GCS_EVENT_FROM_STACK:
            ...
            if (TRACE_IsLevelOn(SCCP_TTC_TraceData, SCCP_TRACE_EVENT))
            {
                ITS_UINT output = 0;
                if(TRACE_IsOutputOn(SCCP_TTC_TraceData,
                    SCCP_TRACE_EVENT,
                    0))
                {
                    output += 1;
                ...
                //CTF_TraceEvent(sccpEvent.src, ITS_SCCP_TTC_SRC,
                //        output, &sccpEvent);
            }

            sccpEvent.src = ITS_MTP3_SRC;
            SCRCHandleMTP3Msg_TTC(&sccpEvent);
            break;

        case ITS_SCCP_SRC:
            SCCP_DEBUG(("SCCP_Main: handle termination event.\n"));
            
            TRANSPORT_IS_ACTIVE(__SCCP_TTC_RcvQueue) = ITS_FALSE;
        ...
        default:
            ...
            if (TRACE_IsLevelOn(SCCP_TTC_TraceData, SCCP_TRACE_EVENT))
            {
                ...
                else if ((gmask & ITS_TCAP_TTC) == ITS_TCAP_TTC)
                {
                    src = ITS_TCAP_TTC_SRC;
                }
                ...
                //CTF_TraceEvent(src, ITS_SCCP_TTC_SRC,
                //    output, &sccpEvent);
            }
            ...
            case SCCP_MSG_LUDT:
            case SCCP_MSG_LUDTS:  
                SCLCHandleAPPMsg_TTC(&sccpEvent);
                break;
            ...
            case SCCP_MSG_ERR:
            case SCCP_MSG_IT:
                SCOCHandleAPPMsg_TTC(&sccpEvent);
                break;

            default:
                SCCP_ERROR(("SCRCHandleMTP3Msg: invalid message type\n"));
                SCCP_Alarm_TTC(634, __FILE__, __LINE__, "family=%s:mType=%x",
                               "TTC", SCCP_MSG_TYPE(&sccpEvent));
            }
        }

        ROUTE_UnlockRead(routeRWLock, __SCCP_TTC_RcvQueue);
    }
 ...
 ****************************************************************************/
int
SCCP_AddSegmentRef_TTC(ITS_OCTET *dsmKey,
    ...
    if (error == ITS_ENOTFOUND)
    {
        ...
        ctxt->reasmTimer =
            TIMERS_StartSharedTimer(ITS_SCCP_SRC, 
                                    TIMERS_GetTimerValue(&TTC_SCCP_Timers,
                                                         SCCP_T_reassembly),
                                    ctxt, 
 ...
 ****************************************************************************/
void
SCCP_DeleteSegmentRef_TTC(ITS_OCTET *dsmKey, ITS_OCTET keyLen, 
                            SCCP_REASM_CTXT *ctxt)
{
    int   error;
 ...
 ****************************************************************************/
void
SCCP_Console_TTC(DBC_Server *dbc, const char *cmdLine)
{
    ...
    if (DBC_Cmd(dbc, "st_get_gen_cfg", "SCCP Get General Config", "", ""))
    {
        SCCPGeneralCfg *cfg;
        cfg =  SCCP_GetGeneralCfg_TTC();
    ...
    else if (DBC_Cmd(dbc, "st_set_gen_cfg", "SCCP Set General Config", 
...

    {
        ...
        ret =  SCCP_SetGeneralCfg_TTC(&cfg);
        if (ret != ITS_SUCCESS)
        {
            return;
        }
...
#if 0
    else if (DBC_Cmd(dbc, "st_get_ss", "SCCP Get Subsystem Entry", 
                     "<sspc> <ni> <ssn>",
...

    {
        ...
        info = SCCP_GetSSEntry_TTC( (ITS_UINT)pc, (ITS_OCTET)ni, 
                                       (ITS_OCTET)ssn);
    ...
    else if (DBC_Cmd(dbc, "st_get_all_ssn", "SCCP Get All Subsystem Entries", 
                     "", ""))
    {
        SCCPSubSystemInfo *entry;
        ITS_UINT numEntries;

        SCCP_GetAllSSEntries_TTC( &numEntries, &entry);
    ...
    else if (DBC_Cmd(dbc, "st_rem_lssn", "SCCP Remove Local Subsystem Entry",
                     "<pc> <ni> <ssn>",
                     "<pc> <ni> <subsystem>"))
    ...
    else if (DBC_Cmd(dbc, "st_rem_rssn", "SCCP Remove Remote Subsystem Entry", 
                     "<sspc> <ni> <ssn> ",
                     "<point code> <ni> <ssn>"))
    {
        SCCPSubSystemInfo info;
        ...
        destPtr = ROUTE_FindDestination(pc, ni, FAMILY_JAPAN);
        if (destPtr == NULL)
        {
            sprintf(buf, "\n*** Invalid PC/NI ***\n");
            DBC_AppendText(dbc, buf);
        ...
        ROUTE_CommitDestination(destPtr, pc, ni, FAMILY_JAPAN);
        ...
        SCCP_Alarm_TTC(500, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                        "TTC", pc, ssn);

        DBC_AppendText(dbc, buf);
    }
    else if(DBC_Cmd(dbc, "st_set_rssn", "SCCP Set Remote Subsystem Entry", 
                     "<sspc> <ni> <ssn> [<linkset>]",
                     "<point code> <ni> <ssn> [<linkset>]"))
    {
        ...
        SCCP_Alarm_TTC(501, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                       "TTC", pc, ssn);
    }
    else if (DBC_Cmd(dbc, "st_set_lssn", "SCCP Set Local Subsystem Entry",
                     "<pc> <ni> <lssn>",""))
    ...
    else if (DBC_Cmd(dbc, "st_get_pc", "SCCP Get Point Code Entry", 
                     "<sspc> <ni>",
...

    {
        ...
        info = SCCP_GetPointCodeEntry_TTC( (ITS_OCTET)ni, (ITS_UINT)pc);
    ...
    else if (DBC_Cmd(dbc, "st_get_all_pc", "SCCP Get All Point Code Entries", 
                     "", ""))

    {
        ...
        SCCP_GetAllPointCodeEntries_TTC( &numEntries, &entry);
    ...
    else if (DBC_Cmd(dbc, "st_set_pc", "SCCP Set Point Code Entry", 
                     "<sspc> <linkset>",
                   "<point code> <linkset>"))
    {
        ITS_INT linkset;
        ...
        ret = SCCP_SetPointCodeEntry_TTC(&info);
        if (ret != ITS_SUCCESS)
        {
        ...
        SCCP_Alarm_TTC(502, __FILE__, __LINE__, "family=%s:pc=%x", "TTC", pc);
        DBC_AppendText(dbc, buf);

    }
    else if (DBC_Cmd(dbc, "st_rem_pc", "SCCP Remove Point Code Entry", 
                     "<sspc> <linkset>",
                   "<point code> <linkset>"))

    {
        ...
        ret = SCCP_RemPointCodeEntry_TTC(&info);
        if (ret != ITS_SUCCESS)
        {
        ...
        SCCP_Alarm_TTC(503, __FILE__, __LINE__, "family=%s:pc=%x", "TTC", pc);
        DBC_AppendText(dbc, buf);

    }
    else if (DBC_Cmd(dbc, "st_get_cpc", "SCCP Get Concerned Point Code Entry", 
                     "<sspc> <ni> <ssn> <cpc>",
                     "<point code> <network indicator> <subsystem>" 
...

    {
        ...
        info = SCCP_GetConcernedPC_TTC( (ITS_UINT)pc, (ITS_OCTET)ni, 
                                          (ITS_OCTET)ssn, (ITS_UINT)cpc);
    ...
    else if (DBC_Cmd(dbc, "st_get_all_cpc", "SCCP Get All CPC Entries", 
                     "", ""))

    {
        ...
        SCCP_GetAllCPCEntries_TTC( &numEntries, &entry);
    ...
    else if (DBC_Cmd(dbc, "st_set_cpc", "SCCP Set Concerned Point Code Entry", 
                     "<sspc> <ni> <ssn> <cpc>",
                     "<point code> <network indicator> <subsystem>" 
                      " <concerned point code>"))

    {
        ...
        ret = SCCP_SetConcernedPC_TTC(&info);
        ...
        SCCP_Alarm_TTC(504, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                       "TTC", pc, ssn);
        DBC_AppendText(dbc, buf);

    }

    else if (DBC_Cmd(dbc, "st_rem_cpc", "SCCP Remove CPC Entry", 
                     "<sspc> <ni> <ssn> <cpc>",
                     "<point code> <network indicator> <subsystem>" 
                      " <concerned point code>"))

    {
        ...
        ret = SCCP_RemConcernedPC_TTC(&info);
        if (ret != ITS_SUCCESS)
        {
        ...
        SCCP_Alarm_TTC(505, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                       "TTC", pc, ssn);
        DBC_AppendText(dbc, buf);

    }
    else if (DBC_Cmd(dbc, "st_get_cssn", "SCCP Get Concerned SSN Entry", 
...

    {
        ...
        info = SCCP_GetConcernedSS_TTC( (ITS_UINT)pc, (ITS_OCTET)ni, 
                                         (ITS_OCTET)ssn, (ITS_UINT)cpc, 
                                          (ITS_OCTET)cssn);
    ...
    else if (DBC_Cmd(dbc, "st_get_all_cssn", "SCCP Get All Concerned SSN Entry",
                     "", ""))

    {
        ...
        SCCP_GetAllCSSEntries_TTC( &numEntries, &entry);
    ...
    else if (DBC_Cmd(dbc, "st_set_cssn", "SCCP Set Concerned SSN Entry", 
                     "<sspc> <ni> <ssn> <cpc> <cssn>",
                     "<point code> <network indicator> <subsystem>" 
                      " <concerned point code> <concerned ssn"))

    {
        ...
        ret = SCCP_SetConcernedSS_TTC(&info);
        if (ret == ITS_ENOTFOUND)
        {
             sprintf(buf, "No Local Route Found For pc=%d, ni=%d, ssn=%d\n",
                     pc, ni, ssn);
        ...
        SCCP_Alarm_TTC(506, __FILE__, __LINE__, "family:pc=%x:ssn=%x",
                       "TTC", pc, ssn);
        DBC_AppendText(dbc, buf);

    }
    else if (DBC_Cmd(dbc, "st_rem_cssn", "SCCP Remove Concerned SSN Entry", 
                     "<sspc> <ni> <ssn> <cpc> <cssn>",
                     "<point code> <network indicator> <subsystem>" 
                      " <concerned point code> <concerned ssn"))

    {
        ...
        ret = SCCP_RemConcernedSS_TTC(&info);
        if (ret != ITS_SUCCESS)
        {
        ...
        SSTCStopSST_TTC(&hdr, cpc, cssn);

        sprintf(buf, "****** SCCP Concerned Subsystem Entry Removed*****\n");
        SCCP_Alarm_TTC(507, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",
                       "TTC", pc, ssn);
        DBC_AppendText(dbc, buf);

    }
    else if (DBC_Cmd(dbc, "st_get_behavior", "SCCP Get Behaviors", 
                     "", ""))

    {
        if (BEHAVIORS_GetBehavior(&TTC_SCCP_Behaviors, SCCP_B_use_SCMG))
        {
        ...
        if (BEHAVIORS_GetBehavior(&TTC_SCCP_Behaviors, SCCP_B_propagate_SCMG))
        {
        ...
        if (BEHAVIORS_GetBehavior(&TTC_SCCP_Behaviors, SCCP_B_propagate_IT))
        {
        ...
        if (BEHAVIORS_GetBehavior(&TTC_SCCP_Behaviors, SCCP_B_propagate_ERR))
        {
    ...
    else if (DBC_Cmd(dbc, "st_set_behavior", "SCCP Set Behaviors", 
                     "<ps> <us> <pi> <pe>", ""))
    ...
    else if (DBC_Cmd(dbc, "st_get_tmr", "SCCP Get Timers", 
                     "", ""))
    {
        int value = 0;

        value = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_statInfo);
        sprintf(buf, "    SCCP_T_statInfo       <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_coordChg);
        sprintf(buf, "    SCCP_T_coordChg       <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_ignoreSST);
        sprintf(buf, "    SCCP_T_ignoreSST      <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_rtgStatInfo);
        sprintf(buf, "    SCCP_T_rtgStatInfo    <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_conn);
        sprintf(buf, "    SCCP_T_conn           <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_release);
        sprintf(buf, "    SCCP_T_release        <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_reset);
        sprintf(buf, "    SCCP_T_reset          <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_ias);
        sprintf(buf, "    SCCP_T_ias            <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_iar);
        sprintf(buf, "    SCCP_T_iar            <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_guard);
        sprintf(buf, "    SCCP_T_guard          <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_reassembly);
        sprintf(buf, "    SCCP_T_reassembly     <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_interval);
        sprintf(buf, "    SCCP_T_interval       <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_repeatRelease);
        sprintf(buf, "    SCCP_T_repeatRelease  <%d> \n", value);
        DBC_AppendText(dbc, buf);

    }
    else if (DBC_Cmd(dbc, "st_set_tmr", "SCCP Set Timer", 
...

    {
        ...
        TIMERS_SetTimerValue(&TTC_SCCP_Timers, tr, value);

        sprintf(buf, "    Timer # %d Set \n", tr);
        DBC_AppendText(dbc, buf);

    }
    else if (DBC_Cmd(dbc, "st_get_conns","Get SCCP Connection", "", ""))
    {
    ...
    else if (DBC_Cmd(dbc, "st_get_gen_peg", "Get Pegs", "<pg> ", "<Peg#> <>"))
    {
        ...
        {
            /* Q752 Implementation Invoke function SCCP_GetOm_MML_TTC() */
            SCCP_GetOm_MML_TTC(dbc);
        ...
        value = PEG_GetPeg(TTC_SCCP_Pegs, pgNum);
    ...
    else if (DBC_Cmd(dbc, "st_show_all_pegs", "Dump All SCCP Pegs", "<>", "<>"))
    {
        DumpAllSCCPPegs(dbc);
    ...
    else if (DBC_Cmd(dbc, "st_clr_gen_peg", "Clear Peg", "<pg> ", "<Peg#> <>"))
    {
        char pgName[ITS_PATH_MAX];
        ITS_INT pgNum;
        ...
        if((strncmp(pgName, "ALL", 3) == 0)||
           ...
           (atoi(pgName) == ALL_SCCP_PEGS))
        {
            /* Q752 Implementation Invoke function SCCP_ResetOm_MML_TTC() */
            SCCP_ResetOm_MML_TTC();

            PEG_ClearBlock(TTC_SCCP_Pegs);

            sprintf(buf, "Clearing All TTC SCCP Pegs for  Now\n");
            DBC_AppendText(dbc, buf);
            return;
        }
        else
        ...
        PEG_ClearPeg(TTC_SCCP_Pegs, pgNum);

    }

#if 0

    else if (DBC_Cmd(dbc, "st_clr_all_pegs", "Clear All Pegs", "<> ", "<>"))
    {
        PEG_ClearBlock(TTC_SCCP_Pegs);
    ...
    else if (DBC_Cmd(dbc, "st_send_ssp", "SCCP Send Sub System Prohibited",
                     "<sspc> <ni> <ssn> <cpc> <cssn> ", "<>"))
    {
        SCCPConcernedSSInfo info;
    ...
    else if (DBC_Cmd(dbc, "st_send_ssa", "SCCP Send Sub System Allowed",
                     "<sspc> <ni> <ssn> <cpc> <cssn> ", "<>"))
    {
        SCCPConcernedSSInfo info;
    ...
    else if (DBC_Cmd(dbc, "st_send_sst", "SCCP Send Sub System Test",
                     "<sspc> <ni> <ssn> <cpc> <cssn> ", "<>"))
    {
        SCCPConcernedSSInfo info;
...
SCCP_GetAllPegs(SCCP_Pegs *genPegs)
{
    ...
    genPegs->userMsgsSent = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_MSG_SENT);
    genPegs->remoteMsgsRcvd = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_MSG_RECEIVED);
    genPegs->routingFailure = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_ROUTING_FAILURE);
    genPegs->remoteGTTRequest = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_GTT_REQUEST);
    genPegs->remoteGTTSucc = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_GTT_SUCCESS);
    genPegs->remoteGTTFail = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_GTT_FAILURE);

    genPegs->remoteMsgsDecodeErr = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_MSG_DECODE_ERR);
    genPegs->userMsgsEncodeErr = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_MSG_ENCODE_ERR);

    genPegs->remoteReassembleErr = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_REASSEMBLE_ERR);
    genPegs->hopCounterVilotionErr = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_HOP_COUNTER_VIOLATION_ERR);
    genPegs->userSegmentationErr = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_SEGMENTATION_ERR);
    genPegs->timerTiarExpiry = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_TIMER_TIAR_EXPIRY);
    genPegs->userInitiatedRLSD = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_INITIATED_RLS);
    genPegs->userInitiatedReset = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_INITITATED_RESET);
    genPegs->remoteUDTRcvd = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_UDT_RCVD);
    genPegs->remoteXUDTRcvd = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_XUDT_RCVD);
    genPegs->remoteUDTSRcvd = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_UDTS_RCVD);
    genPegs->remoteXUDTSRcvd = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_XUDTS_RCVD);
    genPegs->remoteLUDTRcvd = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_LUDT_RCVD);
    genPegs->remoteLUDTSRcvd = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_LUDTS_RCVD);
    genPegs->userUDTSent = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_UDT_SENT);
    genPegs->userXUDTSent = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_XUDT_SENT);
    genPegs->userUDTSSent = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_UDTS_SENT);
    genPegs->userXUDTSSent= PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_XUDTS_SENT);
    genPegs->userLUDTSent= PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_LUDT_SENT);
    genPegs->userLUDTSSent = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_LUDTS_SENT);
    genPegs->userCRSent = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_CR_SENT);
    genPegs->sentCREFToRemote = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_CREF_TO_REMOTE_SENT);
    genPegs->sentRSRToRemote = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_RSR_TO_REMOTE_SENT);
    genPegs->sentERRToRemote = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_ERR_TO_REMOTE_SENT);
    genPegs->remoteCRRcvd = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_CR_RCVD);
    genPegs->remoteCREFRcvd = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_CREF_RCVD);
    genPegs->remoteRSRRcvd = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_RSR_RCVD);
    genPegs->remoteERRRcvd = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_ERR_RCVD);
    genPegs->remoteMsgsForLocalSSRcvd = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_MSG_FOR_LOCAL_SS_RCVD);
    genPegs->userDT1Sent = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_DT1_SENT);
    genPegs->remoteDT1Rcvd = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_DT1_RCVD);
    genPegs->userDT2Sent = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_DT2_SENT);
    genPegs->remoteDT2Rcvd = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_DT2_RCVD);
    genPegs->userEDSent = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_ED_SENT);
    genPegs->remoteEDRcvd = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_ED_RCVD);
    genPegs->remoteSSPRcvd= PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_SSP_RCVD);
    genPegs->remoteSSARcvd = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_SSA_RCVD);
    genPegs->remoteSSCRcvd = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_SSC_RCVD);
    genPegs->localSSProhibited = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_LOCAL_SS_PROHIBITED);
    genPegs->localSSAllowed = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_LOCAL_SS_ALLOWED);
    genPegs->localSSCongested = PEG_GetPeg(TTC_SCCP_Pegs, PEG_SCCP_LOCAL_SS_CONGESTED);
...
SCCP_ClearPegs()
{
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_MSG_SENT);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_MSG_RECEIVED);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_ROUTING_FAILURE);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_GTT_REQUEST);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_GTT_SUCCESS);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_GTT_FAILURE);

    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_MSG_DECODE_ERR);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_MSG_ENCODE_ERR);

    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_REASSEMBLE_ERR);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_HOP_COUNTER_VIOLATION_ERR);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_SEGMENTATION_ERR);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_TIMER_TIAR_EXPIRY);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_INITIATED_RLS);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_INITITATED_RESET);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_UDT_RCVD);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_XUDT_RCVD);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_UDTS_RCVD);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_XUDTS_RCVD);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_LUDT_RCVD);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_LUDTS_RCVD);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_UDT_SENT);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_XUDT_SENT);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_UDTS_SENT);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_XUDTS_SENT);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_LUDT_SENT);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_LUDTS_SENT);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_CR_SENT);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_CREF_TO_REMOTE_SENT);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_RSR_TO_REMOTE_SENT);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_ERR_TO_REMOTE_SENT);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_CR_RCVD);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_CREF_RCVD);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_RSR_RCVD);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_ERR_RCVD);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_MSG_FOR_LOCAL_SS_RCVD);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_DT1_SENT);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_DT1_RCVD);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_DT2_SENT);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_DT2_RCVD);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_USER_ED_SENT);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_ED_RCVD);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_SSP_RCVD);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_SSA_RCVD);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_REMOTE_SSC_RCVD);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_LOCAL_SS_PROHIBITED);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_LOCAL_SS_ALLOWED);
    PEG_ClearPeg(TTC_SCCP_Pegs, PEG_SCCP_LOCAL_SS_CONGESTED);
...
int
INTELLINET_SCCP_Commit_TTC(FILE* fp)
{
    int count = 0;
    TRACE_Data* td = SCCP_TTC_TraceData;
    ...
    for (count = 0; count < (int)TTC_SCCP_Timers.numTimers; count++)
    {
        fprintf(fp,"\t\t\t\t\t%s = \"%d\"\n",SCCP_TTC_Timers[count],
                                             TTC_SCCP_Timers.timers[count]);
    }

    fprintf(fp,"\t\t\t\t/>\n");
    fprintf(fp,"\t\t\t\t<BehaviorControl\n");
    fprintf(fp,"\t\t\t\t\t%s = \"%s\"\n", SCCP_PROPAGATE_SCMG_STRING,
            BEHAVIORS_GetBehavior(&TTC_SCCP_Behaviors,
                                   SCCP_B_propagate_SCMG)?
            "yes" : "no");
...
SCCP_OSS_GetGeneralCfg_TTC(SCCP_GeneralCfg* genCfg)
{
    if (genCfg == NULL)
        return ITS_ENOMEM;

    genCfg->alarmLevel = (MGMT_AlarmLevel)SCCP_AlarmLevel_TTC;

    genCfg->debugTrace = TRACE_IsLevelOn(SCCP_TTC_TraceData, SCCP_TRACE_DEBUG);
    if (genCfg->debugTrace)
    {
        genCfg->debugTraceOutput = 0;
        if (TRACE_IsOutputOn(SCCP_TTC_TraceData, SCCP_TRACE_DEBUG, 0))
        {
            genCfg->debugTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(SCCP_TTC_TraceData, SCCP_TRACE_DEBUG, 1))
        {
            genCfg->debugTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(SCCP_TTC_TraceData, SCCP_TRACE_DEBUG, 2))
        {
    ...
    genCfg->warningTrace = TRACE_IsLevelOn(SCCP_TTC_TraceData, SCCP_TRACE_WARNING);
    if (genCfg->warningTrace)
    {
        genCfg->warningTraceOutput = 0;
        if (TRACE_IsOutputOn(SCCP_TTC_TraceData, SCCP_TRACE_WARNING, 0))
        {
            genCfg->warningTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(SCCP_TTC_TraceData, SCCP_TRACE_WARNING, 1))
        {
            genCfg->warningTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(SCCP_TTC_TraceData, SCCP_TRACE_WARNING, 2))
        {
    ...
    genCfg->errorTrace = TRACE_IsLevelOn(SCCP_TTC_TraceData, SCCP_TRACE_ERROR);
    if (genCfg->errorTrace)
    {
        genCfg->errorTraceOutput = 0;
        if (TRACE_IsOutputOn(SCCP_TTC_TraceData, SCCP_TRACE_ERROR, 0))
        {
            genCfg->errorTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(SCCP_TTC_TraceData, SCCP_TRACE_ERROR, 1))
        {
            genCfg->errorTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(SCCP_TTC_TraceData, SCCP_TRACE_ERROR, 2))
        {
    ...
    genCfg->criticalTrace = TRACE_IsLevelOn(SCCP_TTC_TraceData, SCCP_TRACE_CRITICAL);
    if (genCfg->criticalTrace)
    {
        genCfg->criticalTraceOutput = 0;
        if (TRACE_IsOutputOn(SCCP_TTC_TraceData, SCCP_TRACE_CRITICAL, 0))
        {
            genCfg->criticalTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(SCCP_TTC_TraceData, SCCP_TRACE_CRITICAL, 1))
        {
            genCfg->criticalTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(SCCP_TTC_TraceData, SCCP_TRACE_CRITICAL, 2))
        {
    ...
    genCfg->eventTrace = TRACE_IsLevelOn(SCCP_TTC_TraceData, SCCP_TRACE_EVENT);
    if (genCfg->eventTrace)
    {
        genCfg->eventTraceOutput = 0;
        if (TRACE_IsOutputOn(SCCP_TTC_TraceData, SCCP_TRACE_EVENT, 0))
        {
            genCfg->eventTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(SCCP_TTC_TraceData, SCCP_TRACE_EVENT, 1))
        {
            genCfg->eventTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(SCCP_TTC_TraceData, SCCP_TRACE_EVENT, 2))
        {
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_SetGeneralCfg_TTC(SCCP_GeneralCfg* genCfg)
{
    if (genCfg == NULL)
        return ITS_ENOMEM;

    SCCP_AlarmLevel_TTC = genCfg->alarmLevel;

    if (genCfg->debugTrace)
    {
        TRACE_SetLevelOnOffByName(SCCP_TTC_TraceData, SCCP_DEBUG_STRING,  ITS_TRUE);
        ...
        if (genCfg->debugTraceOutput & TRACE_TO_NONE)
        {
            ...
            TRACE_SetLevelOnOffByName(SCCP_TTC_TraceData, SCCP_DEBUG_STRING,  ITS_FALSE);
        }
    }

    if (genCfg->warningTrace)
    {
        TRACE_SetLevelOnOffByName(SCCP_TTC_TraceData, SCCP_WARNING_STRING,  ITS_TRUE);
        ...
        if (genCfg->warningTraceOutput & TRACE_TO_NONE)
        {
            ...
            // Set trace level to false
            TRACE_SetLevelOnOffByName(SCCP_TTC_TraceData, SCCP_WARNING_STRING,  ITS_FALSE);
        }
    }

    if (genCfg->errorTrace)
    {
        TRACE_SetLevelOnOffByName(SCCP_TTC_TraceData, SCCP_ERROR_STRING,  ITS_TRUE);
        ...
        if (genCfg->errorTraceOutput & TRACE_TO_NONE)
        {
            ...
            // Set trace level to false
            TRACE_SetLevelOnOffByName(SCCP_TTC_TraceData, SCCP_ERROR_STRING,  ITS_FALSE);
        }
   }

    if (genCfg->criticalTrace)
    {
        TRACE_SetLevelOnOffByName(SCCP_TTC_TraceData, SCCP_CRITICAL_STRING,  ITS_TRUE);
        ...
        if (genCfg->criticalTraceOutput & TRACE_TO_NONE)
        {
            ...
            // Set trace level to false
            TRACE_SetLevelOnOffByName(SCCP_TTC_TraceData, SCCP_CRITICAL_STRING,  ITS_FALSE);
        }
    }

    if (genCfg->eventTrace)
    {
        TRACE_SetLevelOnOffByName(SCCP_TTC_TraceData, SCCP_EVENT_STRING,  ITS_TRUE);
        ...
        if (genCfg->eventTraceOutput & TRACE_TO_NONE)
        {
            ...
            // Set trace level to false
            TRACE_SetLevelOnOffByName(SCCP_TTC_TraceData, SCCP_EVENT_STRING,  ITS_FALSE);
        }
    }
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_GetBehaviors_TTC(SCCP_Behaviors* behaviors)
{
    if (behaviors == NULL)
        return ITS_ENOMEM;

    behaviors->useSCMG = BEHAVIORS_GetBehavior(&TTC_SCCP_Behaviors, SCCP_B_use_SCMG);
    behaviors->propogateSCMG = BEHAVIORS_GetBehavior(&TTC_SCCP_Behaviors, SCCP_B_propagate_SCMG);
    behaviors->propogateIT = BEHAVIORS_GetBehavior(&TTC_SCCP_Behaviors, SCCP_B_propagate_IT);
    behaviors->propogateERR = BEHAVIORS_GetBehavior(&TTC_SCCP_Behaviors, SCCP_B_propagate_ERR);
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_SetBehavior_TTC(ITS_UINT behId, ITS_BOOLEAN behValue, SCCP_Behaviors* behaviors)
{
    if (behId == SCCP_ALL_BEHAVIORS)
    {
        ...
        BEHAVIORS_SetBehavior(&TTC_SCCP_Behaviors, SCCP_B_propagate_IT, behaviors->useSCMG);
          BEHAVIORS_SetBehavior(&TTC_SCCP_Behaviors, SCCP_B_propagate_ERR, behaviors->propogateSCMG);
          BEHAVIORS_SetBehavior(&TTC_SCCP_Behaviors, SCCP_B_propagate_SCMG, behaviors->propogateIT);
          BEHAVIORS_SetBehavior(&TTC_SCCP_Behaviors, SCCP_B_use_SCMG, behaviors->propogateERR);
    }
    else
    {
        ...
        {
            BEHAVIORS_SetBehavior(&TTC_SCCP_Behaviors, behId, behValue);
        }
    }
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_AddLocalSSN_TTC(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn)
{
    ITS_INT             ret;
    SCCPSubSystemInfo   info;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_RemoveLocalSSN_TTC(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn)
{
    ITS_INT             ret;
    SCCPSubSystemInfo   info;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_AddRemoteSSN_TTC(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn)
{
    ITS_INT             ret;
    SCCPSubSystemInfo   info;
    ITS_OCTET           linkset = 0;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_RemoveRemoteSSN_TTC(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn)
{
    ITS_INT             ret;
    SCCPSubSystemInfo   info;
    ITS_OCTET           linkset = 0;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_GetAllSSN_TTC(SCCPSubSystemInfo** entries, ITS_UINT* numEntries)
{
    SCCP_GetAllSSEntries_TTC(numEntries, entries);
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT
SCCP_OSS_AddConcernedSSN_TTC(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn, ITS_UINT cpc, ITS_OCTET cssn)
{
    ITS_INT                ret;
    SCCPConcernedSSInfo    info;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_RemoveConcernedSSN_TTC(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn, ITS_UINT cpc, ITS_OCTET cssn)
{
    ITS_INT             ret;
    SCCPConcernedSSInfo info;
    MTP3_HEADER         hdr;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_GetConcernedSSN_TTC(SCCPConcernedSSInfo** entries, ITS_UINT* numEntries)
{
    SCCP_GetAllCSSEntries_TTC(numEntries, entries);
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_GetPegs_TTC(SCCP_Pegs* sccpPegs)
{
    if (sccpPegs == NULL)
        return ITS_ENOMEM;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_ClearPeg_TTC(ITS_UINT pegId)
{
    ...
    else
    {
        ...
        {
            PEG_ClearPeg(TTC_SCCP_Pegs, pegId);
        }
    }
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_GetTimers_TTC(SCCP_Timers* timers)
{

    if (timers == NULL)
        return ITS_ENOMEM;

    timers->statInfo = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_statInfo);
    timers->coordChg = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_coordChg);
    timers->ignoreSST = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_ignoreSST);
    timers->rtgStatInfo = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_rtgStatInfo);
    timers->conn = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_conn);
    timers->release = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_release);
    timers->reset = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_reset);
    timers->ias = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_ias);
    timers->iar = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_iar);
    timers->guard = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_guard);
    timers->reassembly = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_reassembly);
    timers->interval = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_interval);
    timers->repeatRelease = TIMERS_GetTimerValue(&TTC_SCCP_Timers, SCCP_T_repeatRelease);
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_SetTimer_TTC(ITS_UINT timerId, ITS_UINT timerValue, SCCP_Timers* timers)
{
    if (timerId == SCCP_ALL_TIMERS)
    {
        ...
        TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_statInfo, timers->statInfo);
        TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_coordChg, timers->coordChg);
        TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_ignoreSST, timers->ignoreSST);
        TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_rtgStatInfo, timers->rtgStatInfo);
        TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_conn, timers->conn);
        TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_release, timers->release);
        TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_reset, timers->reset);
        TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_ias, timers->ias);
        TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_iar, timers->iar);
        TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_guard, timers->guard);
        TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_reassembly, timers->reassembly);
        TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_interval, timers->interval);
        TIMERS_SetTimerValue(&TTC_SCCP_Timers, SCCP_T_repeatRelease, timers->repeatRelease);
    }
    else
    {
        ...
        {
            TIMERS_SetTimerValue(&TTC_SCCP_Timers, timerId, timerValue);
        }
    }
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_SendSSA_TTC(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn, ITS_UINT cpc, ITS_OCTET cssn)
{
    ITS_INT                ret;
    SCCPConcernedSSInfo info;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_SendSSP_TTC(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn, ITS_UINT cpc, ITS_OCTET cssn)
{
    ITS_INT                ret;
    SCCPConcernedSSInfo info;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_SendSST_TTC(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn, ITS_UINT cpc, ITS_OCTET cssn)
{
    ITS_INT             ret;
    SCCPConcernedSSInfo info;
...
void SCCP_ResetOm_TTC()
{
    ...
    SCCPPegsOmDataInitialize(&SccpGenOmImage_TTC[omBuf.activeOm]);
 ...
 *
 ****************************************************************************/
void SCCP_GetOm_TTC(sccpOm *OmSccpData)
{
    ITS_OCTET j, ssn;
    SS7_Family famDesc;
    ...
    SCCPPegsOmDataInitialize(&SccpGenOmImage_TTC[omBuf.inactiveOm]);
    ...
    memcpy(&OmSccpData->genStats,
           &SccpGenOmImage_TTC[omBuf.inactiveOm],
           sizeof(sccpGenPegs));
...
/****************************************************************************
 *  Purpose:
 *      This API is invoked by the st_clr_gen_peg() MML command. This will
 *      clear the ACTIVE SCCP OM Buffer.
 *
...
/****************************************************************************
 *  Purpose:
 *      This API is invoked by the st_get_gen_peg() MML command.  This will
 *      clear the INACTIVE SCCP OM Buffer, toggle the ACTIVE/INACTIVE
...
void SCCP_GetOm_MML_TTC(DBC_Server *dbc)
{
    char buf[32000];
    char buf1[32000];
    ...
    sprintf(buf, "\n Routing Failure No Translation Address   : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.routingFailureNoTransAddr);
    strcat(buf1, buf);

    sprintf(buf, "\n Routing Failure No Trans Specific Address: %d",  
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.routingFailureNoTransSpecificAddr);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Syntax Value Error Detected              : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.syntaxValueErrDetected);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Syntax Construction Err Detected         : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.syntaxConstructionErrDetected);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Timer Treass Expiry                      : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.timerTreassExpiry);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Reassembly Error Segmentation Rx OOS     : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.reassemblyErrSegRxOOS);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Reassembly Error No Reassembly Space     : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.reassemblyErrNoReassemblySpace);
    strcat(buf1, buf);
           
    sprintf(buf, "\n HopCounter Violation Error               : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.hopCounterViolationErr);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Timer Tiar Expiry                        : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.timerTiarExpiry);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Provider Initiated Reset Connection      : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.providerInitiatedResetConn);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Provider Initiated Release Connection    : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.providerInitiatedReleaseConn);
    strcat(buf1, buf);
           
    sprintf(buf, "\n User Initiated Reset Connection          : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.userInitiatedResetConn);
    strcat(buf1, buf);
           
    sprintf(buf, "\n User Initiated Release Connection        : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.userInitiatedReleaseConn);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Segmentation Error Segmentation Failed   : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.segmentationErrSegFailed);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Reassembly Error Reassembly Failed       : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.reassemblyErrReassemblyFailed);
    strcat(buf1, buf);
    
    sprintf(buf, "\n Msg UDTS Sent                            : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageUDTSSent);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg UDTS Recd                            : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageUDTSRcvd);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg UDT Sent                             : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageUDTSent);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg UDT Recd                             : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageUDTRcvd);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg CR Sent                              : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageCRSent);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg CR Recd                              : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageCRRcvd);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg CREF Sent                            : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageCREFSent);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg CREF Recd                            : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageCREFRcvd);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg RSR Sent                             : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageRSRSent);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg RSR Recd                             : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageRSRRcvd);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg ERR Sent                             : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageErrSent);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg ERR Recd                             : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageErrRcvd);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg XUDT Sent                            : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageXUDTSent);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg XUDT Recd                            : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageXUDTRcvd);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg XUDTS Sent                           : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageXUDTSSent);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg XUDTS Recd                           : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageXUDTSRcvd);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg LUDT Sent                            : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageLUDTSent);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg LUDT Recd                            : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageLUDTRcvd);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg LUDTS Sent                           : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageLUDTSSent);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg LUDTS Recd                           : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageLUDTSRcvd);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg From Local SubSystem                 : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageFromLocalSS);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg From Remote SubSystem                : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageFromRemoteSS);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg Intended for Local SubSystem         : %d", 
                     SccpGenOmImage_TTC[omBuf.inactiveOm].SCCPGenPegs.messageForLocalSS);
    strcat(buf1, buf);
