 ...
 ****************************************************************************
 *
 *  ID:$Id: prc-mtp3_alarms.c,v 1.2 2007/02/01 13:26:38 yranade Exp $
 *
 * LOG: $Log: prc-mtp3_alarms.c,v $
 ...
 ****************************************************************************/
void
MTP3_Alarm_PRC(int which, const char *file, const int line,
                char *format, ...)
{
    va_list alist;
    ALARM_Format *fmt;

    ITS_C_ASSERT(which >= PRC_MTP3_ALARM_START &&
                 which <= PRC_MTP3_ALARM_END);

    fmt = PRC_MTP3_Alarms[which - PRC_MTP3_ALARM_START];

    if (fmt->severity > MTP3_AlarmLevel_PRC)
    {
        return;
    }
