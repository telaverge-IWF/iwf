 ...
 ****************************************************************************
 *
 *  ID: $Id: prc-tsm.c,v 1.4 2008/06/24 07:53:07 ssingh Exp $
 *
 * LOG: $Log: prc-tsm.c,v $
 ...
 * LOG: Add prc files
 * LOG:
 * LOG: Revision 1.1.2.4  2007/04/11 06:34:25  mshanmugam
 * LOG: Fix for 5538
 * LOG:
 * LOG: Revision 1.1.2.3  2007/02/08 14:38:05  raghavendrabk
 * LOG: Changed alarm format to put family at beginning
 * LOG:
 * LOG: Revision 1.1.2.2  2007/02/02 14:41:12  raghavendrabk
...
#ident "$Id: prc-tsm.c,v 1.4 2008/06/24 07:53:07 ssingh Exp $"
 ...
 ****************************************************************************/
int
TSMHandleTCOCSLMsg_PRC(ITS_CTXT did, ITS_SCCP_IE *ies, int ieCount)
{
    ...
    trCtxt = TCAP_FindLocalTx_PRC(did);
    if (trCtxt == NULL)
    {
    ...
    else if (trCtxt->state == ITS_TCAP_IDLE)
    {
        ...
        case TCPPT_TC_UNI:
            ...
            PEG_IncrPeg(PRC_TCAP_Pegs, PEG_TCAP_APP_UNI_SENT);
        ...
        case TCPPT_TC_BEGIN:
            ...
            sharedTx = TCAP_AllocTransactionContext_PRC(trCtxt->ltid,
                                                          tidKey, &tidKeyLen);
            if (sharedTx == NULL)
            {
            ...
            PEG_IncrPeg(PRC_TCAP_Pegs, PEG_TCAP_APP_BEGIN_SENT);
            PEG_IncrPeg(PRC_TCAP_Pegs, PEG_TCAP_APP_MSG_SENT);
            tcapOmImage[omBuf.activeOm].appMsgSent++;
        ...
        default:
            ...
            /* Outgoing Message - Discarding Message - Invalid State. */
            TCAP_Alarm_PRC(1443, __FILE__, __LINE__,
                           "family=%s:PType=%x:Dialog-Id=%x", "PRC", pType, did );
    ...
    else if (trCtxt->state == ITS_TCAP_BEGIN_RCVD)
    {
        ...
        case TCPPT_TC_CONTINUE:
            ...
            sharedTx = TCAP_FindTransactionContext_PRC(trCtxt->ltid, tidKey,
                                                         &tidKeyLen);
            if (sharedTx == NULL)
            {
           ...
           PEG_IncrPeg(PRC_TCAP_Pegs, PEG_TCAP_APP_CONT_SENT);
           PEG_IncrPeg(PRC_TCAP_Pegs, PEG_TCAP_APP_MSG_SENT);
           tcapOmImage[omBuf.activeOm].appMsgSent++;
         ...
         */
            ...
            sharedTx = TCAP_FindTransactionContext_PRC(trCtxt->ltid, 
                                                         tidKey, &tidKeyLen);
            if (sharedTx == NULL)
            {
                TCAP_DEBUG(("TCOHandleCSLMsg: Cannot Find DSM Tx for did"
            ...
            PEG_IncrPeg(PRC_TCAP_Pegs, PEG_TCAP_APP_MSG_SENT);
            tcapOmImage[omBuf.activeOm].appMsgSent++;
            PEG_IncrPeg(PRC_TCAP_Pegs, PEG_TCAP_APP_END_SENT);
        ...
        default:
            ...
            /* Outgoing Message - Discarding Message - Invalid State. */
            TCAP_Alarm_PRC(1444, __FILE__, __LINE__,
                           "family=%s:PType=%x:Dialog-Id=%x", "PRC", pType, did); 
    ...
    else if (trCtxt->state == ITS_TCAP_BEGIN_SENT)
    {
        ...
        default:
            ...
            /* Outgoing Message - Discarding Message - Invalid State. */
            TCAP_Alarm_PRC(1445, __FILE__, __LINE__,
                           "family=%s:PType=%x:Dialog-Id=%x", "PRC", pType, did); 
    ...
    else if (trCtxt->state == ITS_TCAP_ACTIVE)
    {
        ...
        default:
            ...
            TCAP_Alarm_PRC(1446, __FILE__, __LINE__,
                           "family=%s:PType=%x:family=%s:Dialog-Id=%x", "PRC", pType,
                           did);
        ...
        pType == TCPPT_TC_BEGIN)
    {
        ...
        if (ret != ITS_SUCCESS)
        {
            TCAP_ERROR(("TSMHandleTCOCSLMsg_PRC: DSM_Commit "
                        "failed, error code: %d\n", ret));
            PEG_IncrPeg(PRC_TCAP_Pegs, PEG_TCAP_APP_MSG_DISCARDED);
            tcapOmImage[omBuf.activeOm].appMsgDiscarded++;
            tcapOmImage[omBuf.activeOm].errorsDetectedInTP++;
        ...
        {
            TCAP_DEBUG(("TSMHandleTCOCSLMsg_PRC: DSM-Commit the DSM Tx for "
                        "did %d, pType = %d\n", did, pType));
        }
 ...
 ****************************************************************************/
int
TSMHandleTCOSCCPMsg_PRC(ITS_OCTET pType, ITS_SCCP_IE *ies, int ieCount,
                          TCAP_TRANS_ENTRY **trCtxt)
{
    ...
    case ITS_TCAP_IDLE:
        /* send to CSL */
        if (DHAHandleTCOMsg_PRC(ies, ieCount, *trCtxt) == ITS_SUCCESS)
        {
    ...
    case ITS_TCAP_BEGIN_SENT:
        ...
        case TCPPT_TC_P_ABORT:
            ...
            TCAP_FreeLocalTx_PRC((*trCtxt)->did);
        ...
        default:
            ...
            TCAP_FreeLocalTx_PRC((*trCtxt)->did);
            TCAP_Alarm_PRC(1451, __FILE__, __LINE__,
                           "family=%s:PType=%x:Dialog-Id=%x", "PRC", pType,
                           (*trCtxt)->did);
    ...
    case ITS_TCAP_BEGIN_RCVD:
        ...
        default:
            ...
            /* Incomming Dialogue - Discarding Dialogue -
               Rejection Sent by stack. */
            TCAP_Alarm_PRC(1450, __FILE__, __LINE__,
                           "family=%s:PType=%x:Dialog-Id=%x", "PRC", pType,
                           (*trCtxt)->did);
            PEG_IncrPeg(PRC_TCAP_Pegs, PEG_TCAP_REMOTE_MSG_DISCARDED);
            tcapOmImage[omBuf.activeOm].remMsgDiscarded++;
    ...
    case ITS_TCAP_ACTIVE:
        ...
        default:
            ...
            TCAP_Alarm_PRC(1452, __FILE__, __LINE__,
                           "family=%s:PType=%x:Dialog-Id=%x","PRC", pType,
                           (*trCtxt)->did);
            PEG_IncrPeg(PRC_TCAP_Pegs, PEG_TCAP_REMOTE_MSG_DISCARDED);
            tcapOmImage[omBuf.activeOm].remMsgDiscarded++;
