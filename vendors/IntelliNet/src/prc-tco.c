 ...
 ****************************************************************************
 *
 *  ID: $Id: prc-tco.c,v 1.4 2008/06/24 07:53:07 ssingh Exp $
 *
 * LOG: $Log: prc-tco.c,v $
 * LOG: Revision 1.4  2008/06/24 07:53:07  ssingh
 ...
 * LOG: Add prc files
 * LOG:
 * LOG: Revision 1.1.2.3  2007/02/08 14:37:42  raghavendrabk
 * LOG: Changed alarm format to put family at beginning
 * LOG:
 * LOG: Revision 1.1.2.2  2007/02/02 14:40:40  raghavendrabk
...
#ident "$Id: prc-tco.c,v 1.4 2008/06/24 07:53:07 ssingh Exp $"
 ...
 ****************************************************************************/
int
TCOHandleCSLMsg_PRC(ITS_CTXT did, ITS_SCCP_IE *ies, int ieCount)
{
    return TSMHandleTCOCSLMsg_PRC(did, ies, ieCount);
}
 ...
 ****************************************************************************/
int
TCOHandleSCCPMsg_PRC(ITS_EVENT *event)
{
    ...
    default:
        ...
        TCAP_Alarm_PRC(1446, __FILE__, __LINE__, "family=%s", "PRC");
       
        PEG_IncrPeg(PRC_TCAP_Pegs, PEG_TCAP_INVALID_SCCP_RCVD);

        return (ITS_SUCCESS);
    }

    if (FindPTypeAndTID_PRC(ies, ieCount,
                              ...
                              &cdp, &clp) == ITS_NO)
    {
        ...
        if (!pType)
        {
            ...
            TCAP_Alarm_PRC(1448, __FILE__, __LINE__,
                           "family=%s:PType=%x:Tx-id=%x", "PRC", pType, tid2);
        ...
        else
        {
            ...
            TCAP_Alarm_PRC(1453, __FILE__, __LINE__, "family=%s:PType=%x", "PRC", pType);
    ...
    {
        sharedTx = TCAP_FindTransactionContext_PRC(tid1,
                                                     tidKey, &tidKeyLen);
        if (sharedTx != NULL)
        {
        ...
        if (localTx != NULL)
        {
            DHAHandleTCOMsg_PRC(ies, ieCount, localTx);
        }
    ...
    case TCPPT_TC_UNI:
        ...
        if (DHAHandleTCOMsg_PRC(ies, ieCount, NULL) != ITS_SUCCESS)
        {
    ...
    case TCPPT_TC_BEGIN:
        ...
        if (localTx == NULL)
        {
            ...
            tcapOmImage[omBuf.activeOm].errorsDetectedInTP++;
            TCAP_Alarm_PRC(1447, __FILE__, __LINE__,
                           "family=%s:Transaction-id=%x:PType=%x", "PRC", tid2, pType); 
 
            return (ITS_SUCCESS);
        }

        AllocateLTIDinTx_PRC(localTx);

        ret = TCAP_PopulateLocalTx_PRC(localTx, did, tid1, tid1Len, &cdp, &clp);
        ...
        ret = TSMHandleTCOSCCPMsg_PRC(pType, ies, ieCount, &localTx);
 ...
 */
        ...
        if (ret == ITS_SUCCESS)
        {
            sharedTx = TCAP_AllocTransactionContext_PRC(localTx->ltid,
                                                          tidKey, &tidKeyLen);
    ...
    case TCPPT_TC_CONTINUE:
        ...
        sharedTx = TCAP_FindTransactionContext_PRC(tid2, tidKey, &tidKeyLen);
        ...
        if (sharedTx)
        {
            if (sharedTx->rtidLen)
            {
                ...
                if (tid1 != sharedTx->rtid)
                {
                    ...
                    TCAP_Alarm_PRC(1453, __FILE__, __LINE__,
                                   "family=%s:PType=%x", "PRC", pType); 
            ...
            */  
            ...
            if (ret != ITS_SUCCESS)
            {
                TCAP_ERROR(("TCOHandleSCCPMsg_PRC: DSM_Commit failed "
                            "returned code: %d\n", ret));
                tcapOmImage[omBuf.activeOm].errorsDetectedInTP++;
    ...
    if (localTx != NULL)  /* only if it was not freed before */
    {
       ...
       {
           TCAP_ERROR(("TCOHandleSCCPMsg_PRC: SharedTx is NULL for did %d ",
                        localTx->did));
            tcapOmImage[omBuf.activeOm].errorsDetectedInTP++;
           TCAP_FreeLocalTx_PRC(localTx->did);
       }
    }
    else
    {
           TCAP_DEBUG(("TCOHandleSCCPMsg_PRC: LocalTx has been freed for "
                       "did %d ",did));
    }
#endif
