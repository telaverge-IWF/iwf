 ...
 ****************************************************************************
 *
 *  ID: $Id: prc-slm.c,v 1.4 2008/06/17 06:39:08 nvijikumar Exp $ 
 *
 * LOG: $Log: prc-slm.c,v $
 ...
 * LOG: Add prc files
 * LOG:
 * LOG: Revision 1.1.4.4  2007/05/18 10:01:26  raghavendrabk
 ...
 * LOG:
 * LOG: Revision 1.1.4.2  2007/02/02 14:33:33  raghavendrabk
...
#ident "$Id: prc-slm.c,v 1.4 2008/06/17 06:39:08 nvijikumar Exp $"
 ...
 ****************************************************************************/
int
SLM_Main_PRC(MTP3_SubSystems src, MTP3_SubSystems dest,
              ...
              ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_SLM, src, trigger, sif, len,
                           linkSet, linkCode);
    ...
    if (trigger == SLM_TRIGGER_L2_MSG)
    {
        ...
        case SNMM_HC0_DLM:
            ...
            case SNMM_HC1_DLC:
                ...
                LSDA_Main_PRC(src, SLM_TRIGGER_CONNECTION_ORDER,
                               sif, len,
                               linkSet, linkCode);
                break;
            ...
            case SNMM_HC1_CSS:
                ...
                LSDA_Main_PRC(src, SLM_TRIGGER_CONNECTION_SUCCESSFUL,
                               sif, len,
                               linkSet, linkCode);
                break;
            ...
            case SNMM_HC1_CNS:
                ...
                LSDA_Main_PRC(src, SLM_TRIGGER_CONNECTION_UNSUCCESS,
                               sif, len,
                               linkSet, linkCode);
                break;
            ...
            case SNMM_HC1_CNP:
                ...
                LSDA_Main_PRC(src, SLM_TRIGGER_CONNECTION_NOT_POSS,
                               sif, len,
                               linkSet, linkCode);
                break;
 ...
 ****************************************************************************/
int
LLSC_Main_PRC(MTP3_SubSystems src, SLM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    SS7_LinkSetPtr ls;
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_SLM_LLSC, src, trigger, sif, len,
                           linkSet, linkCode);
    ...
    case SLM_TRIGGER_ACTIVATE_LINK_SET:      /* from MGMT */
        if ((ls = LINKSET_FindLinkSet(linkSet)) == NULL)
        {
            ...
            MTP3_Alarm_PRC(2423, __FILE__, __LINE__, "ls=%x:family=%s", linkSet, "PRC");
    ...
    case SLM_TRIGGER_LINK_ACTIVE:            /* from LSAC */
        if ((ls = LINKSET_FindLinkSet(linkSet)) == NULL)
        {
            ...
            MTP3_Alarm_PRC(2423, __FILE__, __LINE__, "ls=%x:family=%s", linkSet, "TTC");
    ...
    case SLM_TRIGGER_ACTIVATE_ANOTHER_LINK:  /* from LSAC */
        ...
        if (j < ls->numToStart)
        {
            ...
            for (cnt = (int)ls->numLinks - 1; cnt >= 0; cnt--)
            {
                if (!ls->links[cnt]->isUp)
                {
                    return LSAC_Main_PRC(MTP3_SLM_LLSC,
                                          SLM_TRIGGER_ACTIVATE_LINK,
                                          NULL, 0,
                                          linkSet, linkCode);
                }
 ...
 ****************************************************************************/
int
LSAC_Main_PRC(MTP3_SubSystems src, SLM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    SS7_LinkPtr ln;
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_SLM_LSAC, src, trigger, sif, len,
                           linkSet, linkCode);
    ...
    case SLM_TRIGGER_DATA_LINK:              /* from LSDA */
        if (ln->activityState == LINK_ACTIVITY_INACTIVE)
        {
            ...
            PEG_IncrPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_ACTIVATION);
            PEG_IncrPeg(&ln->MTP3_LINK_PEGS, PEG_LINK_ACTIVATION);

            LSLA_Main_PRC(MTP3_SLM_LSAC,
    ...
    case SLM_TRIGGER_L2_IN_SERVICE:          /* from L2 */
        ...
        MTP3_Alarm_PRC(2527, __FILE__, __LINE__,"ls=%x:link=%x:family=%s", linkSet, linkCode, "PRC");
        ...
        if (ln->activityState == LINK_ACTIVITY_ACTIVATING_RESTORING)
        {
            ...
            PEG_IncrPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_IN_SERVICE);
           ...
           MTP3_Alarm_PRC(2528, __FILE__, __LINE__,
                            "ls=%x:link=%x:family=%s", linkSet,linkCode, "PRC");

            /* start the link test */
            SLTC_Main_PRC(MTP3_SLM_LSAC,
    ...
    case SLM_TRIGGER_LP_RECOVERD:                 /* from MGMT */
        if (ln->activityState == LINK_ACTIVITY_ACTIVE)
        {
            ...
            TLAC_Main_PRC(MTP3_SLM_LSAC, STM_TRIGGER_LP_RECOVERD,
                           NULL, 0,
                           linkSet, linkCode);
        }
        break;
    ...
    case SLM_TRIGGER_L2_OUT_OF_SERVICE:      /* from L2 */
        MTP3_Alarm_PRC(2509, __FILE__, __LINE__, "ls=%x:link=%x:family=%s",
        ...
        else if (ln->activityState == LINK_ACTIVITY_ACTIVE)
        {
            ...
            PEG_IncrPeg(PRC_MTP3_Pegs, PEG_MTP3_LINK_OUT_OF_SERVICE);
    ...
    case SLM_TRIGGER_START_LINK:             /* from LSLA: from LSLR */
        if (ln->activityState == LINK_ACTIVITY_ACTIVATING_RESTORING)
        {
            ...
            MTP3_Alarm_PRC(2505, __FILE__, __LINE__,
                           "ls=%x:link=%x:family=%s", linkSet, linkCode, "PRC");
    ...
    case SLM_TRIGGER_T17:
        if (ln->activityState == LINK_ACTIVITY_WAIT_1)
        {
            ...
            MTP3_Alarm_PRC(2506, __FILE__, __LINE__,
                           "ls=%x:link=%x:family=%s", linkSet, linkCode, "PRC");
    ...
    case SLM_TRIGGER_RESUME:                 /* from TLAC */
        if (ln->activityState == LINK_ACTIVITY_ACTIVATING_RESTORING ||
            ln->activityState == LINK_ACTIVITY_ACTIVE)
        {
            MTP2_SendData_PRC(linkSet, linkCode, L3_L2_RESUME,
                               NULL, 0);
        }
        break;
    ...
    case SLM_TRIGGER_CLEAR_BUFFERS:          /* from TLAC */
        if (ln->activityState == LINK_ACTIVITY_ACTIVATING_RESTORING ||
            ln->activityState == LINK_ACTIVITY_ACTIVE)
        {
            MTP2_SendData_PRC(linkSet, linkCode, L3_L2_CLEAR_BUFFERS,
                               NULL, 0);
        }
        break;
...
STM_Ready:
    ...
    case SLM_TRIGGER_L2_RPR:                 /* from L2 */
        if (ln->activityState == LINK_ACTIVITY_ACTIVE)
        {
            TLAC_Main_PRC(MTP3_SLM_LSAC, STM_TRIGGER_RP_RECOVERD,
                           NULL, 0,
                           linkSet, linkCode);
        }
        break;
...
TerminalPredetermined(SS7_LinkPtr ln)
{
    ...
    if (MTP3_PRC_AutomaticTerminalAllocation)
    {
        return (ln->termFixed);
    }
...
DataLinkPredetermined(SS7_LinkPtr ln)
{
    ...
    if (MTP3_PRC_AutomaticDataLinkAllocation)
    {
        return (ln->linkFixed);
    }
 ...
 ****************************************************************************/
int
LSLA_Main_PRC(MTP3_SubSystems src, SLM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    SS7_LinkPtr ln;

    MTP3_TRACE_ENTRY_PRC(MTP3_SLM_LSLA, src, trigger, sif, len,
                           linkSet, linkCode);
    ...
    case SLM_TRIGGER_START_ACTIVATION:       /* from LSAC */
        if (ln->activationState == LINK_ACTIVATION_IDLE)
        {
            ...
            {
                MTP2_SendData_PRC(linkSet, linkCode, L3_L2_CONNECT, NULL, 0);

                LSAC_Main_PRC(MTP3_SLM_LSLA,
 ...
 ****************************************************************************/
int
LSLR_Main_PRC(MTP3_SubSystems src, SLM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    SS7_LinkPtr ln;

    MTP3_TRACE_ENTRY_PRC(MTP3_SLM_LSLR, src, trigger, sif, len,
                           linkSet, linkCode);
 ...
 ****************************************************************************/
int
LSLD_Main_PRC(MTP3_SubSystems src, SLM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    MTP3_TRACE_ENTRY_PRC(MTP3_SLM_LSLD, src, trigger, sif, len,
                           linkSet, linkCode);
    ...
    case SLM_TRIGGER_DEACTIVATE_LINK:      /* from LSAC */
        ...
        MTP2_SendData_PRC(linkSet, linkCode, L3_L2_DISCONNECT, NULL, 0);

        MTP3_Alarm_PRC(2509, __FILE__, __LINE__, "ls=%x:link=%x:family=%s",
                        linkSet, linkCode, "PRC");

        LSDA_Main_PRC(MTP3_SLM_LSLD,
                       SLM_TRIGGER_LINK_AVAILABLE,
                       NULL, 0,
                       linkSet, linkCode);

        LSTA_Main_PRC(MTP3_SLM_LSLD,
 ...
 ****************************************************************************/
int
LSTA_Main_PRC(MTP3_SubSystems src, SLM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    SS7_LinkSetPtr ls;
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_SLM_LSTA, src, trigger, sif, len,
                           linkSet, linkCode);
    ...
    case SLM_TRIGGER_DETERMINE_TERMINAL:     /* from LSLA: from LSLR */
        ...
        {
            MTP3_Alarm_PRC(2434, __FILE__, __LINE__, "ls=%x:link=%x:family=%s",linkSet, linkCode, "PRC");
            break;
        }
 ...
 ****************************************************************************/
int
LSDA_Main_PRC(MTP3_SubSystems src, SLM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ...
    MTP3_TRACE_ENTRY_PRC(MTP3_SLM_LSDA, src, trigger, sif, len,
                           linkSet, linkCode);
    ...
    case SLM_TRIGGER_DETERMINE_DATA_LINK:    /* from LSLA: from LSLR */
        ...
        {
            MTP3_Alarm_PRC(2439, __FILE__, __LINE__, "ls=%x:link=%x:family=%s",linkSet, linkCode, "PRC");
        ...
        TSRC_Main_PRC(MTP3_SLM_LSDA, STM_TRIGGER_ALTERNATE_ROUTE_REQ,
                       ev.data, ev.len,
                       linkSet, linkCode);
    ...
    case SLM_TRIGGER_CONNECTION_ORDER:       /* from HMDT */
        ...
        if (linkSet == ITS_SS7_DEFAULT_LINK_SET)
        {
            ...
            MTP3_Alarm_PRC(2440, __FILE__, __LINE__, "family=%s", "PRC");
        ...
        else if (ln->allocationState == LINK_ALLOCATION_WAIT_1)
        {
            ...
            if (pc1 > pc2)
            {
                ...
                if (ln->slFlags & LINK_IS_AVAILABLE)
                {
                    ...
                    ret = HMRT_Main_PRC(MTP3_SLM_LSDA,
