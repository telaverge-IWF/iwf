 ...
 ****************************************************************************
 *
 *  ID: $Id: ttc-scrc.c,v 9.6 2008/07/18 05:47:53 ssingh Exp $
 *
 * LOG: $Log: ttc-scrc.c,v $
 * LOG: Revision 9.6  2008/07/18 05:47:53  ssingh
 * LOG: SDS ID:: ID :: D0240 Warning removal for Solaris Compilation
 * LOG:
 * LOG: Revision 9.5  2008/06/20 09:11:14  ssingh
 * LOG: updated with code review comments, ipDestination check
 * LOG: removed as not working in ASP configuration.
 * LOG:
 * LOG: Revision 9.4  2008/06/17 06:38:02  nvijikumar
 ...
 * LOG:
 * LOG: Revision 9.2  2008/04/15 05:03:18  kramesh
 * LOG: Merged Persistent Branch Changes to current.
 * LOG:
 * LOG: Revision 9.1.2.2  2007/08/16 12:40:58  pspl
 ...
 * LOG:
 * LOG: Revision 9.1.2.1  2007/07/31 11:27:43  pspl
 ...
 * LOG:
 * LOG: Revision 1.1.2.4  2007/05/18 10:00:55  raghavendrabk
 * LOG: ROUTE_GetRoutingKeyRouteInfo API called to get RoutingKey Route info
 * LOG:
 * LOG: Revision 1.1.2.3  2007/02/13 04:02:55  raghavendrabk
 * LOG: Changed alarm format to put family at beginning
 * LOG:
 * LOG: Revision 1.1.2.2  2007/02/02 14:19:30  raghavendrabk
 ...
 * LOG:
 * LOG: Revision 9.6.4.4  2006/10/07 14:21:05  ssingh
 * LOG: Lucent BSG specfic, removing dummy route check
 * LOG:
 * LOG: Revision 9.6.4.3  2006/09/25 13:51:39  mshanmugam
 * LOG: Removing the Q752 compilation Flag
 * LOG:
 * LOG: Revision 9.6.4.2  2006/08/18 12:40:09  mshanmugam
 * LOG: Added new SCCP Pegs for Q.752 Implementation.
 * LOG:
 * LOG: Revision 9.6.4.1  2006/04/11 09:18:49  ssingh
...
#ident "$Id: ttc-scrc.c,v 9.6 2008/07/18 05:47:53 ssingh Exp $"
...
extern ITS_USR_PART_STATE ITS_usrPartState;
                ...
                ITS_UINT pc, ITS_OCTET ssn)
{
...
#if 0
    ----- no dummy route check (we are using IntelliMTP3)
    ----- Sending directly to MTP3
    ret = ROUTE_SendToRouteContext(pc, &ev,
                                   ROUTE_DPC_SIO_SSN,
                                   FAMILY_JAPAN,
                                   ROUTE_SIO_SCCP | ni,
    ...
    return (ret);
    ----- Comment finishes [ do not remove]
#endif

    if (ITS_usrPartState.state == ITS_STATE_PRIMARY)
    {
#if 0
   ...
   /* If node is not active, we have to release memory allocated for event explicitly */
    ITS_EVENT_TERM(&ev);
...
sendConnectionRefuse(MTP3_HEADER* mtp3, ITS_USHORT conref, SCCP_ADDR *cgp)
{
    ...
    if (cgp)
    {
        ...
        info_Count++;
        if (SCCP_DecodeAddr_TTC(cgp, &addrInd, &cgppc, &ssn,
                         ...
                         ITS_OCTET reason)
{
    switch (mType)
    {
         ...
         case SCCP_MSG_LUDT:
         case SCCP_MSG_LUDTS:
             return SCLCHandleRoutingFailure_TTC(mtp3, mType, ies, ieCount, reason);

         default:
             return SCOCHandleRoutingFailure_TTC(mtp3, mType, ies, ieCount, reason);
    }
}
 ...
 *
 *  ssingh    10-04-2006  BugId: 3505             poriting NI problem 4 SPARE
 *                                                variants from BSG 0.2+
 *
 ****************************************************************************/
static int
                   ...
                   ITS_OCTET addrInd, ITS_UINT pc, ITS_OCTET ssn)
{
    ...
    if (!SCCP_IsSSNEquipped(ni, pc, ssn))
    {
        ...
        if (SCCP_IsSSNAllowed(ni, pc, ssn))
        {
            ...
            switch (mType)
            {
                 ...
                 case SCCP_MSG_LUDT:
                 case SCCP_MSG_LUDTS:
                      return SCRCHandleSCLCMsg_TTC(mtp3, mType, ies, ieCount);

                 default:
                      /* EEK! WE'RE AN INTERMEDIATE!! */
                      return SCOCHandleSCRCMsg_TTC(mtp3, mType, ies, ieCount,
                                               pc, ssn, ITS_TRUE);
            }
        }
        else
        {
            ...
            SCCP_Alarm_TTC(643, __FILE__, __LINE__,
                           "family=%s:type=%s:pc=%x:ni=%x:ssn=%x", "TTC", "local", pc,
                           ni, ssn);
    ...
    else if (!SCCP_IsSSNAllowed(ni, pc, ssn))
    {
        ...
        SCCP_Alarm_TTC(643, __FILE__, __LINE__,
                       "family=%s:type=%s:pc=%x:ni=%x:ssn=%x", "TTC", "local", pc, ni,
                       ssn);

        return SCMGSSPCHandleMsgForProhibitedSSN_TTC(mtp3, mType, ies, ieCount,
                                                       pc, ssn,
    ...
    else
    {
        ...
        switch (mType)
        {
            ...
            return SCLCHandleSCRCMsg_TTC(mtp3, mType, ies, ieCount, pc, ssn);
...
SCRCDeliverLocal(MTP3_HEADER* mtp3, ITS_OCTET mType,
                 ...
                 ITS_OCTET addrInd, ITS_UINT pc, ITS_OCTET ssn)
{
    PEG_IncrPeg(TTC_SCCP_Pegs,PEG_SCCP_REMOTE_MSG_FOR_LOCAL_SS_RCVD);

    /* Q752 Implementation */
    SccpGenOmImage_TTC[omBuf.activeOm].SCCPGenPegs.messageForLocalSS++;
    ...
    else
    {
        switch (mType)
        {
            ...
            SCCP_Alarm_TTC(644, __FILE__, __LINE__, "family=%s:mType=%x",
                           "TTC", mType);
                  ...
                  ITS_OCTET addrInd, ITS_UINT pc, ITS_OCTET ssn)
{
    ...
    if (addrInd & SCCP_CPA_HAS_SSN)
    {
        ...
        {
            /* SSN is not allowed */
            SCCP_Alarm_TTC(648, __FILE__, __LINE__,
                           "family=%s:type=%s:pc=%x:ni=%x:ssn=%x", "TTC", "remote", pc,
                           ni, ssn);
                 ...
                 ITS_OCTET *gttInfo, ITS_USHORT gttLen)
{
    ...
    */
    ...
    if (SCCP_GTTTranslate_TTC(mtp3, &dpc, ai, pc, ssn, gttInfo, gttLen,
                                &transAddr, ies, ieCount) != ITS_SUCCESS)
    {
        ...
        SCCP_Alarm_TTC(645, __FILE__, __LINE__, "family=%s", "TTC");

        return SCRCHandleRoutingFailure(mtp3, mType, ies, ieCount,
                                        SCCP_RET_NO_TRANS_THIS_ADDR);
    }

    ni = MTP3_HDR_GET_SIO_TTC(*mtp3) & MTP3_NIC_MASK;
    ...
    for (i = 0; i < ieCount; i++)
    {
        ...
        {
            if (ies[i].param_data.hopCount.count >= 0)  //chandrabs  changes previours i<=
            {
                        ...
                        NULL, NULL) != ITS_SUCCESS)
    {
        ...
        SCCP_Alarm_TTC(646, __FILE__, __LINE__, "family=%s", "TTC");
 ...
 ****************************************************************************/
int
SCRCHandleMTP3Msg_TTC(ITS_EVENT* ev)
{
    ITS_UINT pc;
    ...
    {
        return SCMGHandleMTP3Msg_TTC(&hdr3, mType, data, len);
    }
    ...
    switch (mType)
    {
        ...
        return SCOCHandleSCRCMsg_TTC(&hdr3, mType, ies, ieCount,
                                       0, ssn, ITS_FALSE);
     ...
     */
        ...
        for (i = 0; i < ieCount; i++)
        {
            if (ies[i].param_id == SCCP_PRM_PROTOCOL_CLASS)
            {
                ...
                {
                     SCCP_DEBUG(("SCRCHandleMTP3Msg_TTC: \
                ...
                else
                {
                    ...
                    SCCP_Alarm_TTC(670, __FILE__, __LINE__,
                                   "family=%s:pclass=%x", "TTC", lpclass);
        ...
        SCCP_Alarm_TTC(636, __FILE__, __LINE__,"family=%s:mType=%x", "TTC", mType);
        return (ITS_EINVALIDARGS);
    }
    ...
    if (i == ieCount)
    {
        ...
        SCCP_Alarm_TTC(647, __FILE__, __LINE__, "family=%s:pc=%x",
                       "TTC", MTP3_RL_GET_OPC_VALUE(hdr3.label)); 
    ...
    if (addrInd & SCCP_CPA_ROUTE_SSN)
    {
        ...
        if (SCCP_UserIntervention_TTC(&hdr3, addrInd, pc, ssn,
...
****************************************************************************/
int
SCRCHandleSCLCMsg_TTC(MTP3_HEADER* mtp3, ITS_OCTET mType,
                        ITS_SCCP_IE* ies, int ieCount)
{
    ITS_UINT pc;
    ...
    if (addrInd & SCCP_CPA_ROUTE_SSN)
    {
        ...
        else
        {
            ...
            SCCP_Alarm_TTC(644, __FILE__, __LINE__, "family=%s:mType=%x", "TTC", mType);
 ...
 ****************************************************************************/
int
SCRCHandleSCOCMsg_TTC(MTP3_HEADER* mtp3, ITS_OCTET mType,
                        ITS_SCCP_IE* ies, int ieCount,
                        SCCP_ADDR* cdp)
{
    ITS_UINT pc;
    ...
    /*printf("Result in CO =%x",MTP3_HDR_GET_SIO_TTC(*mtp3));*/
