 ...
 ****************************************************************************
 *
 *  ID: $Id: ttc-dha.c,v 9.2.8.1 2009/12/21 03:46:43 rajeshak Exp $
 *
 * LOG: $Log: ttc-dha.c,v $
 * LOG: Revision 9.2.8.1  2009/12/21 03:46:43  rajeshak
 * LOG: Removing CTF_TraceEvent function call in vendors
 * LOG:
 * LOG: Revision 9.2  2008/06/04 10:38:48  ssingh
 * LOG: ALU porting, B-0100LX-LUSG-00 (june 04,2008 base)
 * LOG:
 * LOG: Revision 9.1  2007/01/11 12:43:39  yranade
 * LOG: Merge changes from 6.5.3 and Lucent branches
 * LOG:
 * LOG: Revision 1.1.2.3  2007/02/08 14:39:58  raghavendrabk
 * LOG: Changed alarm format to put family at beginning
 * LOG:
 * LOG: Revision 1.1.2.2  2007/02/02 14:16:44  raghavendrabk
 * LOG: Parallel Stack changes for Alarms and Statistics
 * LOG:
 * LOG: Revision 1.1.2.1  2006/12/23 10:05:36  randresol
 ...
 * LOG:         the offsets of the type_ITU__TCAP_ComponentPortion struct, instead of
...
#ident "$Id: ttc-dha.c,v 9.2.8.1 2009/12/21 03:46:43 rajeshak Exp $"
...
MLIST_Manager *__TCAP_TTC_primQueue;
 ...
 ****************************************************************************/
void
__TCAP_SendAppPAbort_TTC(ITS_CTXT did, ITS_EVENT *event,
                           ITS_OCTET reason)
{
    ITS_EVENT ev;
    TCAP_DLG dlg;
    ...
    {
        TCAP_ERROR(("__TCAP_SendAppPAbort_TTC: Sanity check failed.\n"));
    ...
    {
        TCAP_WARNING(("__TCAP_SendAppPAbort_TTC: unable to send PAbort response.\n"));

        /* Incoming as in toward application */
        /* Incoming Dialogue - Discarding Dialogue - No Route to Application. */

         PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_DLG_SEND_APP_FAIL); 
    }
    else
    {
        /* Add to Count "Received" ==> toward Application */
        PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_ABORT_SENT);
...
void
__TCAP_SendTransPAbort_TTC(ITS_SCCP_IE *ies, int ieCount,
                             ITS_OCTET reason)
{
    TCAP_DLG dlg;
    ITS_OCTET pType = 0;
...
#if defined (PEPSY_CODEC)
    int len = 0;
    struct type_TTC__TCAP_MessageType *pdu;
    char *enc = NULL;
    PE pe;
#elif defined (TCAP_CODEC)
     ...
     * event.
     */
    if (FindPTypeAndTID_TTC(ies, ieCount, &pType,
    ...
    if (rtid && ((sharedTx = TCAP_FindTransactionContext_TTC(rtid, tidKey,
                                                            &keyLen)) != NULL))
    {
        TCAP_DEBUG(("TCAP_SendTransPAbort: Freeing DSM Tx for tid"
                    "%u.\n", rtid));

        TCAP_FreeDSMTransactionContext_TTC(sharedTx);
     
        TCAP_FreeLocalTx_TTC(sharedTx->did);
    }
...
#if defined (PEPSY_CODEC)
    pdu = TCAP_CvtDLGToMessage_TTC(&dlg, ltid, rtid, rtidLen, TCAP_Variant);
    if (!pdu)
    {
    ...
    /* perform the encode */
    if (encode_TTC__TCAP_MessageType(&pe, 1, 0, NULL, (void *)pdu)
        == NOTOK)
    {
        TCAP_ERROR(("SendPAbort: Could not encode package.\n"));

        free_TTC__TCAP_MessageType(pdu);
    ...
    print_TTC__TCAP_MessageType(pe, 1, 0, NULL, pdu);
#endif
    ...
    if (TCAP_CvtDLGToData_TTC(&dlg, tp, &tpLen, 
                                NULL, 0, ltid, 
                                rtid, rtidLen, 
    ...
    PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_MSG_SENT);
...
void
__TCAP_SendAppReject_TTC(ITS_CTXT did, TCAP_CPT *comp,
                           ...
                           ITS_BOOLEAN isLast)
{
    ...
    */
    ...
    {
        TCAP_DEBUG(("__TCAP_SendAppReject_TTC: sending INVOKE to app: %04x\n",dest));
...
void
__TCAP_SendRemoteReject_TTC(ITS_CTXT did, TCAP_CPT *comp,
                              ITS_OCTET family, ITS_OCTET reason)
{
    TCAP_CPT cpt;
    ITS_EVENT event;
    ...
    if (comp == NULL)
    {
        ...
        /* Incoming Component - Discarding Component - Invalid or Corrupted. */
        TCAP_Alarm_TTC(1437, __FILE__, __LINE__, "family=%s", "TTC");
        return;
    }
 ...
 ****************************************************************************/
ITS_BOOLEAN
FindPTypeAndTID_TTC(ITS_SCCP_IE *ies, int ieCount,
                      ITS_OCTET *pType,
                      ...
                      SCCP_ADDR *cdp, SCCP_ADDR *clp)
{
    ...
    *tid1 = *tid2 = 0;
    ...
    if (i == ieCount || ies[i].param_length < 1)
    {
        ...
        TCAP_Alarm_TTC(1429, __FILE__, __LINE__, "family=%s", "TTC");
                    ...
                    ITS_SCCP_IE *ies, int ieCount)
{
    ...
    case TCPPT_TC_END:
            ...
            dlg->u.end.user_info.len != 0)
        {
            ...
            {
                __TCAP_SendAppPAbort_TTC(did, event,
                                           TCPABT_REASON_BADLY_STRUCT_TRANS_PORT);

                __TCAP_SendTransPAbort_TTC(ies, ieCount,
                                             TCPABT_REASON_BADLY_STRUCT_TRANS_PORT);

        
                /* Incoming Dialogue - Discarding Dialogue -
                   Rejection Sent by stack. */
                TCAP_Alarm_TTC(1438, __FILE__, __LINE__, "family=%s", "TTC");
        ...
        else
        {
            ...
            else if (tr->dstate == ITS_DLG_ACTIVE)
            {
                if (dlg->ptype == TCPPT_TC_U_ABORT)
                {
                    ...
                    /* Incoming Dialogue - Discarding Dialogue -
                       Rejection Sent by stack. */
                    TCAP_Alarm_TTC(1438, __FILE__, __LINE__, "family=%s",
                                   "TTC");
...
UpdateDlgPeg(TCAP_DLG *dlg)
{
    ...
    {
        case TCPPT_TC_UNI:
            PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_UNI_RCVD);
            break;

        case TCPPT_TC_BEGIN:
            PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_BEGIN_RCVD);
            break;

        case TCPPT_TC_END:
            PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_END_RCVD);

            PEG_DecrPeg(TTC_TCAP_Pegs, PEG_TCAP_ACTIVE_DIALOGUE);
            PEG_DecrPeg(TTC_TCAP_Pegs,PEG_TCAP_ACTIVE_TRANSACTIONS);
            break;

        case TCPPT_TC_CONTINUE:
            PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_CONT_RCVD);
            break;

        case TCPPT_TC_P_ABORT:
        case TCPPT_TC_U_ABORT:
            PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_ABORT_RCVD);
            PEG_DecrPeg(TTC_TCAP_Pegs, PEG_TCAP_ACTIVE_DIALOGUE);
            PEG_DecrPeg(TTC_TCAP_Pegs,PEG_TCAP_ACTIVE_TRANSACTIONS);
...
UpdateCptPeg(TCAP_CPT *cpt)
{
    switch(cpt->ptype)
    {
        case TCPPT_TC_INVOKE:
            PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_INVOKE_RCVD);
            break;

        case TCPPT_TC_RESULT_NL:
        case TCPPT_TC_RESULT_L: 
            PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_RESULT_RCVD);
            break;

        case TCPPT_TC_U_ERROR:
            PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_ERROR_RCVD);
            break;

        case TCPPT_TC_U_REJECT:
        case TCPPT_TC_R_REJECT: 
            PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_REJECT_RCVD);
 ...
 ****************************************************************************/
static int
ParseTRP(struct type_TTC__TCAP_MessageType **pdu, ITS_OCTET *tp, int wlen)
{
    ITS_OCTET *data;
    ITS_UINT tag, len;
    struct type_TTC__TCAP_MessageType *ret = NULL;
    ...
    *pdu = NULL;
    ...
    /* allocate PDU */
    ret = (struct type_TTC__TCAP_MessageType *)
            calloc(1, sizeof(struct type_TTC__TCAP_MessageType));
    ...
    {
    case TCPPT_TC_UNI:
        ret->offset = type_TTC__TCAP_MessageType_unidirectional;

        /* allocate TRP */
        ret->un.unidirectional = (struct type_TTC__TCAP_Unidirectional *)
                                    calloc(1, sizeof(struct type_TTC__TCAP_Unidirectional));
    ...
    case TCPPT_TC_BEGIN:
        ret->offset = type_TTC__TCAP_MessageType_begin;

        /* Allocate TRP */
        ret->un.begin = (struct type_TTC__TCAP_Begin *)
                            calloc(1, sizeof(struct type_TTC__TCAP_Begin));
        ...
        ret->un.begin->otid = (struct type_TTC__TCAP_OrigTransactionID *)
                                calloc(1, sizeof(struct type_TTC__TCAP_OrigTransactionID) + wlen);
    ...
    case TCPPT_TC_CONTINUE:
        ret->offset = type_TTC__TCAP_MessageType_contin;

        /* allocate TRP */
        ret->un.contin = (struct type_TTC__TCAP_Continue *)
                            calloc(1, sizeof(struct type_TTC__TCAP_Continue));
        ...
        ret->un.contin->otid = (struct type_TTC__TCAP_OrigTransactionID *)
                                calloc(1, sizeof(struct type_TTC__TCAP_OrigTransactionID) + wlen);
        ...
        ret->un.contin->dtid = (struct type_TTC__TCAP_DestTransactionID *)
                                calloc(1, sizeof(struct type_TTC__TCAP_DestTransactionID) + wlen);
    ...
    case TCPPT_TC_END:
        ret->offset = type_TTC__TCAP_MessageType_end;

        /* Allocate TRP */
        ret->un.end = (struct type_TTC__TCAP_End *)
                        calloc(1, sizeof(struct type_TTC__TCAP_End));
        ...
        ret->un.end->dtid = (struct type_TTC__TCAP_DestTransactionID *)
                                calloc(1, sizeof(struct type_TTC__TCAP_DestTransactionID) + wlen);
    ...
    case TCPPT_TC_P_ABORT:
        ret->offset = type_TTC__TCAP_MessageType_abort;

        ret->un.abort = (struct type_TTC__TCAP_Abort *)
                           calloc(1, sizeof(struct type_TTC__TCAP_Abort));
        ...
        ret->un.abort->dtid = (struct type_TTC__TCAP_DestTransactionID *)
                                calloc(1, sizeof(struct type_TTC__TCAP_DestTransactionID) + wlen);
 ...
 ****************************************************************************/
static int
ParseDLG(struct type_TTC__TCAP_MessageType *pdu,
         ITS_OCTET *dp, ITS_OCTET *stop)
{
    ...
    {
        if (pdu->offset != type_TTC__TCAP_MessageType_abort)
        {
            return (ITS_EPROTERR);
        }
    }
    ...
    switch (pdu->offset)
    {
    case type_TTC__TCAP_MessageType_unidirectional:
        pdu->un.unidirectional->dialoguePortion = pe;
        break;

    case type_TTC__TCAP_MessageType_begin:
        pdu->un.begin->dialoguePortion = pe;
        break;

    case type_TTC__TCAP_MessageType_contin:
        pdu->un.contin->dialoguePortion = pe;
        break;

    case type_TTC__TCAP_MessageType_end:
        pdu->un.end->dialoguePortion = pe;
        break;

    case type_TTC__TCAP_MessageType_abort:
        pdu->un.abort->reason = (struct choice_TTC__TCAP_2 *)
                                    calloc(1, sizeof(struct choice_TTC__TCAP_2));
        ...
        if (tag == DIALOG_IDENT)
        {
            pdu->un.abort->reason->offset = choice_TTC__TCAP_2_dialoguePortion;
            pdu->un.abort->reason->un.dialoguePortion = pe;
        }
        else if (tag == PABT_IDENT)
        {
            pdu->un.abort->reason->offset = choice_TTC__TCAP_2_p__abort__cause;
            if (decode_TTC__TCAP_P__AbortCause(pe, 1, NULL, NULL, 
 ...
 *  Return Value:
 ****************************************************************************/
static struct type_TTC__TCAP_ComponentPDU *
GenerateReject(ITS_BOOLEAN derivable, ITS_OCTET *inv_id, ITS_OCTET* stop)
{
    ...
    return TCAP_CvtCPTToComponentPDU_TTC(&rej);
}
 ...
 ****************************************************************************/
static int
ParseCMP(struct type_TTC__TCAP_MessageType *pdu,
         ITS_OCTET *cp, ITS_OCTET *stop)
{
    ITS_OCTET *data = cp;
    ITS_UINT tag, len;
    struct type_TTC__TCAP_ComponentPortion *cseq = NULL;
    ...
    {
        if (pdu->offset == type_TTC__TCAP_MessageType_unidirectional)
        {
            return (ITS_EPROTERR);
        }
        else
    ...
    while (data < cp + len && data[0] != 0 && data[1] != 0)
    {
        ...
        PE pe;
        int err;
        struct type_TTC__TCAP_ComponentPortion *c, **list;
        ...
        if (limit == data)
        {
            if (cseq)
            {
                free_TTC__TCAP_ComponentPortion(cseq);
            }
...
*/
        ...
        c = (struct type_TTC__TCAP_ComponentPortion *)
                calloc(1, sizeof(struct type_TTC__TCAP_ComponentPortion));
        ...
        if (decode_TTC__TCAP_ComponentPDU(pe, 1, NULL, NULL,
 ...
 *  Return Value:
 ****************************************************************************/
struct type_TTC__TCAP_MessageType *
TTC_DecodeTCAPData(ITS_OCTET *data, ITS_USHORT len)
{
    PE pe = NULL;
    struct type_TTC__TCAP_MessageType *pdu = NULL;
    int err = PS_ERR_NONE;
    ...
    if (err != PS_ERR_NONE)
    {
        TCAP_ERROR(("TTC_DecodeTCAPData: decode failed.\n"));

        /* Failed to decode TCAP data */
        TCAP_Alarm_TTC(1439, __FILE__, __LINE__, "family=%s", "TTC");  

        return (NULL);
    }

    if (decode_TTC__TCAP_MessageType(pe, 1, NULL, NULL, (char **)&pdu) == OK)
    {
        TCAP_DEBUG(("TTC_DecodeTCAPData: Package decode successful.\n"));
    }
    else
    {
        ...
        TCAP_DEBUG(("TTC_DecodeTCAPData: PE decode failed.  "
                    "Trying partial decode.\n"));

        pc = TCAP_FindParts_TTC(data, len, &tp, &dp, &cp);

        if (pc != PC_PARSE_OK)
        {
            TCAP_ERROR(("TTC_DecodeTCAPData: parser failure.\n"));
        ...
        {
            TCAP_ERROR(("TTC_DecodeTCAPData: TRP parser failure.\n"));
        ...
        {
            TCAP_ERROR(("TTC_DecodeTCAPData: DLG parser failure.\n"));
...
PEPSY_DisassemblePDU(ITS_SCCP_IE *ies, int ieCount, TCAP_TRANS_ENTRY* tr)
{
    ...
    for (i = 0; i < ieCount; i++)
    {
        ...
        else if (ies[i].param_id == SCCP_PRM_RETURN_CAUSE)
        {
            ...
            if (TCAP_VendorDialogueSanityCheck(&dlg) != ITS_SUCCESS)
            {
                ...
                PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_MSG_DISCARDED); 
            ...
            if (dest != ITS_INVALID_SRC && dest != ITS_NO_SRC)
            {
                ...
                PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_MSG_RCVD);
                TRANSPORT_PutEvent(dest, &event);
                TCAP_Alarm_TTC(1430, __FILE__, __LINE__, "family=%s", "TTC");
            }
            else
            {
                /* Notice Discarded */
    ...
    {
        PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_SCCP_CLASS_0_RCVD);
    }
    else
    {
        PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_SCCP_CLASS_1_RCVD);
    }
    ...
    if (i == ieCount || ies[i].param_length < 1)
    {
         ...
         */
        PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_UNKNOWN_MSG_RCVD);
    ...
    pdu = TTC_DecodeTCAPData(data, len);
    if (pdu == NULL)
    {
        PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_MSG_DISCARDED);
        ...
        {
            __TCAP_SendAppPAbort_TTC(tr->did, NULL,
                                       TCPABT_REASON_BADLY_STRUCT_TRANS_PORT);

            /* Incoming Dialogue - Discarding Dialogue -
               Rejection Sent by stack. */
            TCAP_Alarm_TTC(1431, __FILE__, __LINE__, "family=%s", "TTC");
        }
     ...
     */
    if (TCAP_CvtMessageToDLG_TTC(&dlg, pdu, TCAP_Variant) != ITS_SUCCESS)
    {
        PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_MSG_DISCARDED);
    ...
    if (TCAP_VendorDialogueSanityCheck(&dlg) != ITS_SUCCESS)
    {
        ...
        /* Incoming Dialogue - Discarding Dialogue - Invalid or Corrupted. */
        TCAP_Alarm_TTC(1432, __FILE__, __LINE__, "family=%s", "TTC");
    ...
    if (dest != ITS_INVALID_SRC && dest != ITS_NO_SRC)
    {
        ...
        PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_MSG_RCVD);
        PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_DLG_RCVD);
        UpdateDlgPeg(&dlg);
    ...
    if (pdu->offset != type_TTC__TCAP_MessageType_abort)
    {
        struct type_TTC__TCAP_ComponentPortion *comp = NULL;
        ...
        while (comp != NULL)
        {
            ...
            if ((ret = TCAP_CvtComponentPDUToCPT_TTC(&cpt, comp->ComponentPDU))
                != ITS_SUCCESS)
            {
                ...
                __TCAP_SendRemoteReject_TTC(did, NULL,
                                              TCPPROB_GENERAL,
                ...
                {
                    /*
                     * TODO TCAP_CleanInvokeContext_TTC(did);
                     */
                }
            ...
            /* Add To Component Count for Received Component */
             PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_REMOTE_CMP_RCVD);

            /* Send to CMP */
            if ((ret = CCOHandleDHAMsg_TTC(did, &cpt)) != ITS_SUCCESS)
            {
...
TCAP_DisassemblePDU(ITS_SCCP_IE *ies, int ieCount, TCAP_TRANS_ENTRY* tr)
{
    ...
    if (TCAP_CvtDataToDLG_TTC(&dlg, &pType, tp, dp, cpPresent,
    ...
    if (pType != TCPPT_TC_P_ABORT || pType != TCPPT_TC_U_ABORT)
    {
        if (cp != NULL) 
        {
            ...
            while (cpData < (cp + cpLen) && cpData[0] != 0 && cpData[1] != 0)
            {
                ...
                if ((ret = TCAP_CvtDataToCPT_TTC(&cpt, comp, cTag, cLen)) != ITS_SUCCESS)
                {
                    ...
                    __TCAP_SendRemoteReject_TTC(did, &cpt,
                                                  TCPPROB_GENERAL,
                                                  (ret & 0x0F));

                    __TCAP_SendAppReject_TTC(did, &cpt,
                                               TCPPROB_GENERAL,
                                               (ret & 0x0F),
                                               ITS_TRUE);
                ...
                if ((ret = CCOHandleDHAMsg_TTC(did, &cpt)) != ITS_SUCCESS)
                {
                    ...
                    /* Incoming Component - Discarding Component -
                       Invalid or Corrupted. */
                    TCAP_Alarm_TTC(1439, __FILE__, __LINE__, "family=%s",
                                   "TTC");
                  ...
                  TCAP_TRANS_ENTRY *tx)
{
    ...
    if (cdp)
    {
        ...
        {
            if (BEHAVIORS_GetBehavior(&TTC_TCAP_Behaviors,
        ...
        else if (BEHAVIORS_GetBehavior(&TTC_TCAP_Behaviors,
                                       TCAP_B_use_reserved_NI))
        {
...
PEPSY_AssemblePDU(ITS_CTXT did, TCAP_DLG *dlg, ITS_SCCP_IE *ies, int *ieCount)
{
    ...
    ITS_EVENT *ev;
    struct type_TTC__TCAP_ComponentPortion *seq = NULL;
    struct type_TTC__TCAP_MessageType *pdu;
    int ret = ITS_SUCCESS;
    PE pe = NULL;
    int len = 0;
    ...
    *ieCount = 0;
     ...
     */
    MQUEUE_Lock(__TCAP_TTC_primQueue);
    ...
    for (entry = LIST_TAIL(__TCAP_TTC_primQueue);
         entry != NULL;
         entry = prev)
    {
        ITS_CTXT cdid;
        ...
        {
            struct type_TTC__TCAP_ComponentPortion *tmp;

            MQUEUE_RemoveNoLock(__TCAP_TTC_primQueue, entry);

            tmp = seq;
            if ((seq =
                (struct type_TTC__TCAP_ComponentPortion *)malloc
                    (sizeof(struct type_TTC__TCAP_ComponentPortion))) == NULL)
            {
            ...
            {
                TCAP_CPT cpt;
                /* building link list of type_TTC__TCAP_ComponentPortion
                ...
                if (cpt.ptype == TCPPT_TC_INVOKE)
                {
                    ...
                    {
                    case 1:
                        PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_OP_CLASS_1_SENT);
                        break;
                    case 2:
                        PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_OP_CLASS_2_SENT);
                        break;
                    case 3:
                        PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_OP_CLASS_3_SENT);
                        break;
                    case 4:
                        PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_OP_CLASS_4_SENT);
                        break;
                    default:
                        break;
                    }
                ...
                seq->ComponentPDU = TCAP_CvtCPTToComponentPDU_TTC(&cpt);
    ...
    MQUEUE_Unlock(__TCAP_TTC_primQueue);
    ...
    if (ret != ITS_SUCCESS)
    {
        ...
        free_TTC__TCAP_ComponentPortion(seq);

        localTx = TCAP_FindLocalTx_TTC(did);

        if (localTx)
        {
             TCAP_FindAndFreeTransactionContext_TTC(localTx->ltid,
                                                 tidKey, &tidKeyLen);

             TCAP_FreeLocalTx_TTC(did);
        }
    ...
    case TCPPT_TC_BEGIN:
        ...
        ret = TCAP_PopulateLocalTx_TTC(localTx, did, 0, 0, cdp, clp);
        if (ret != ITS_SUCCESS)
        {
    ...
    case TCPPT_TC_END:
        ...
        if (localTx == NULL)
        {
            ...
            TCAP_Alarm_TTC(1440, __FILE__, __LINE__,
                           "family=%s:Dialog-Id=%x:PType=%x", "TTC", did, dlg->ptype); 
    ...
    {
        PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_PREARRANGED_END_SENT);
        ...
        if (localTx)
        {
            sharedTx = TCAP_FindTransactionContext_TTC(localTx->ltid,
                                                         tidKey,
                                                         &tidKeyLen);
            if (sharedTx == NULL)
            {
     ...
     */
    pdu = TCAP_CvtDLGToMessage_TTC(dlg, ltid, rtid,
    ...
    if (!pdu)
    {
        ...
        if (localTx)
        {
            sharedTx = TCAP_FindTransactionContext_TTC(localTx->ltid, tidKey,
                                                         &tidKeyLen);
            if (sharedTx == NULL)
            {
                TCAP_DEBUG(("PEPSY_AssemblePDU: Cannot Find DSM Tx for did"
        ...
        /* Outgoing Dialogue - Discarding Dialogue - Invalid or Corrupted */
        TCAP_Alarm_TTC(1436, __FILE__, __LINE__, "family=%s", "TTC");

        PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_MSG_DISCARDED);
   ...
   case TCPPT_TC_CONTINUE:
    ...
    {
        PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_SCCP_CLASS_1_SENT);
        ies[0].param_data.protocolClass.pclass = SCCP_PCLASS_1;
    }
    else
    {
        PEG_IncrPeg(TTC_TCAP_Pegs, PEG_TCAP_APP_SCCP_CLASS_0_SENT);
    }
    ...
    *ieCount = 0;
         ...
         entry = prev)
    {
        ...
        if (ev->data[0] == ITS_TCAP_CPT && cdid == did)
        {
            ...
            ret = TCAP_CvtCPTToData_TTC(&cpt, cp, &cpLen);
     ...
     */
    if (TCAP_CvtDLGToData_TTC(dlg, tp, &tpLen, cpList, cptsLen,
    ...
    if (TRACE_IsLevelOn(TCAP_TTC_TraceData, TCAP_TRACE_EVENT))
    {
        ITS_EVENT tcapEvent;
        ITS_INT output = 0;
 ...
 ****************************************************************************/
int
DHAHandleCCOMsg_TTC(ITS_EVENT *event)
{
    int ret;

    TCAP_DEBUG(("DHAHandleCCOMsg_TTC:\n"));

    ret = MQUEUE_Enqueue(__TCAP_TTC_primQueue, event, sizeof(ITS_EVENT));
    event->data = NULL;
    return (ret);
}
 ...
 ****************************************************************************/
int
DHAHandleCCOCancelMsg_TTC(ITS_CTXT did, CPT_INV_ID *invId)
{
    QUEUE_Msg *entry, *tmp;
    ITS_EVENT *ev;
    TCAP_CPT cpt;
    ...
    for (entry = LIST_HEAD(__TCAP_TTC_primQueue);
         entry != NULL;
         entry = tmp)
    {
        ITS_CTXT cdid;
 ...
 *      TTC White Book 97 variant only.
 *
 *  Input Parameters:
 ...
 ****************************************************************************/
int
DHAHandleCCOResetTimerMsg_TTC(TCAP_INVOKE_ENTRY *invoke)
{
    if (invoke == NULL)
    {
        return (ITS_ENULLPTR);
 ...
 ****************************************************************************/
int
DHAHandleDIDClean_TTC(ITS_CTXT did)
{
 ...
 ****************************************************************************/
int
DHAHandleAPPMsg_TTC(ITS_EVENT *event)
{
    ITS_CTXT did;
    TCAP_DLG dlg;
    ITS_SCCP_IE ies[8];
    ...
    TCAP_DEBUG(("DHAHandleAPPMsg_TTC:\n"));
    if (event->data[0] != ITS_TCAP_DLG)
    {
        TCAP_ERROR(("DHAHandleAPPMsg_TTC:  Event is not user dialog: %02x\n",
                    event->data[0]));
    ...
    if (AssemblePDU(did, &dlg, ies, &ieCount) == ITS_SUCCESS)
    {
        ...
        {
            return TCOHandleCSLMsg_TTC(did, ies, ieCount);
        }
    ...
    {
        TCAP_ERROR(("DHAHandleAPPMsg_TTC: Failed to Assemble PDU: did %04x\n",
                    did));
 ...
 ****************************************************************************/
int
DHAHandleTCOMsg_TTC(ITS_SCCP_IE *ies, int ieCount, TCAP_TRANS_ENTRY *tr)
{
    /*
