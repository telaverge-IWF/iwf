 ...
 ****************************************************************************
 *
 *  ID: $Id: prc-sccp.c,v 1.7.6.1 2009/12/21 03:46:43 rajeshak Exp $
 *
 * LOG: $Log: prc-sccp.c,v $
 * LOG: Revision 1.7.6.1  2009/12/21 03:46:43  rajeshak
 * LOG: Removing CTF_TraceEvent function call in vendors
 * LOG:
 * LOG: Revision 1.7  2008/07/18 05:38:12  ssingh
 * LOG: SDS ID:: ID :: D0240 Warning removal for Solaris Compilation
 * LOG:
 * LOG: Revision 1.6  2008/06/20 09:22:28  ssingh
 * LOG: corrected family, unit testing issue
 * LOG:
 * LOG: Revision 1.5  2008/06/17 06:39:07  nvijikumar
 * LOG: Updated for ITU over PRC support (split stack)
 * LOG: ACC-CG-SDS-SS7-V1.0.02::D7030,D7070
 * LOG:
 * LOG: Revision 1.4  2008/06/04 10:51:02  ssingh
 * LOG: ALU porting, B-0100LX-LUSG-00 (june 04,2008 base)
 * LOG:
 * LOG: Revision 1.3  2008/04/15 05:03:18  kramesh
 * LOG: Merged Persistent Branch Changes to current.
 * LOG:
 * LOG: Revision 1.2.2.1  2007/05/18 06:42:06  pspl
 ...
 * LOG: Add prc files
 * LOG:
 * LOG: Revision 1.1.2.13  2008/02/13 08:10:47  ssodhi
 ...
 * LOG:
 * LOG: Revision 1.1.2.12  2007/10/09 11:41:35  mshanmugam
 * LOG: Validation check is done for CREF number
 * LOG:
 * LOG: Revision 1.1.2.11  2007/05/15 14:19:33  raghavendrabk
 * LOG: Changed the function names - DeleteDupData & DeleteCPCData
 * LOG:
 * LOG: Revision 1.1.2.10  2007/04/16 12:26:59  raghavendrabk
 * LOG: Changed function names - BuildCPCData and BuildDupData
 * LOG:
 * LOG: Revision 1.1.2.9  2007/04/11 07:01:50  mshanmugam
 * LOG: Modifications to allow PRC and ITU stacks to run in parallel
 * LOG:
 * LOG: Revision 1.1.2.8  2007/04/10 15:30:57  raghavendrabk
 * LOG: Check Validation for SCCP_PRC_TraceData
 * LOG:
 * LOG: Revision 1.1.2.7  2007/02/26 10:23:43  raghavendrabk
 * LOG: Family filled in OAM API
 * LOG:
 * LOG: Revision 1.1.2.6  2007/02/13 04:19:52  raghavendrabk
 * LOG: Changed alarm format to put family at beginning
 * LOG:
 * LOG: Revision 1.1.2.5  2007/02/09 10:25:38  gharipriya
 * LOG: Added MMLs as a part of Parallel Stack Implementation
 * LOG:
 * LOG: Revision 1.1.2.4  2007/02/06 12:02:13  ssingh
 * LOG: Fixing issue:: Accelero 6.7:: PRC links are not aligning (#5340)
 * LOG:
 * LOG: Revision 1.1.2.3  2007/02/05 06:16:43  clin
 ...
 * LOG:
 * LOG: Revision 1.1.2.2  2007/02/02 14:31:33  raghavendrabk
...
#ident "$Id: prc-sccp.c,v 1.7.6.1 2009/12/21 03:46:43 rajeshak Exp $"
 ...
 * global data
 */
TQUEUETRAN_Manager* __SCCP_PRC_RcvQueue = NULL;

/*
 * local data
 */
THREAD_RET_TYPE
PRC_SCCPThread(void *arg);
...
static ITS_CHAR* SCCP_PRC_Timers[]=
{
...
TRACE_Data* SCCP_PRC_TraceData = NULL;

ITS_BOOLEAN gcsMessageBasedRedundancyIsActive = ITS_FALSE;

static ITS_UINT pegs[PEG_SCCP_NUM_PEGS];
static PEGS_DECL_SUBSYS(PRC_SCCP, pegs);
PEG_Manager *PRC_SCCP_Pegs = &PRC_SCCP_PEGS;

static ITS_TIME timers[SCCP_NUM_TIMERS];
TIMERS_DECL_SUBSYS(PRC_SCCP, timers);

static ITS_BOOLEAN behaviors[SCCP_NUM_BEHAVIORS];
BEHAVIORS_DECL_SUBSYS(PRC_SCCP, behaviors);
...
void BuildCPCData_PRC();
void BuildDupData_PRC();
void DeleteCPCData_PRC();
void DeleteDupData_PRC();
void SCCP_ResetOm_PRC();
void SCCP_GetOm_PRC(sccpOm*);
#if defined(__cplusplus)
}
#endif /* defined(__cplusplus) */
void SCCP_ResetOm_MML_PRC();
void SCCP_GetOm_MML_PRC(DBC_Server *);
void SCCPPegsOmDataInitialize(sccp_gen_stat_table *);
void SCCPSSNPegsOmDataInitialize(sccp_ssn_stat_table*);

extern int
SendSCMG_PRC(MTP3_HEADER* mtp3, SCCP_SCMG_PRC* scmg, ITS_UINT toSSN);

extern int 
SSTCStartSST_PRC(MTP3_HEADER* mtp3, ITS_UINT pc, ITS_OCTET ssn);

/*
 * management class record
 */
static SCCP_MgmtClassRec SCCP_MgmtClassRec_PRC =
{
    /* core part */
    {
    ...
    /* management part */
    {
        ...
        /* function part */
        SCCP_GetGeneralCfg_PRC,
        SCCP_SetGeneralCfg_PRC,
        SCCP_GetSSEntry_PRC,
        SCCP_GetAllSSEntries_PRC,
        SCCP_SetSSEntry_PRC,
        SCCP_RemSSEntry_PRC,
        SCCP_GetPointCodeEntry_PRC,
        SCCP_GetAllPointCodeEntries_PRC,
        SCCP_SetPointCodeEntry_PRC,
        SCCP_RemPointCodeEntry_PRC,
        SCCP_GetConcernedPC_PRC,
        SCCP_GetAllCPCEntries_PRC,
        SCCP_SetConcernedPC_PRC,
        SCCP_RemConcernedPC_PRC,
        SCCP_GetConcernedSS_PRC,
        SCCP_GetAllCSSEntries_PRC,
        SCCP_SetConcernedSS_PRC,
        SCCP_RemConcernedSS_PRC,
        SCCP_SendSCMG_PRC,
        SCCP_SendSST_PRC,
...
ITSSS7DLLAPI ITS_Class SCCP_MgmtClass_PRC =
    (ITS_Class)&SCCP_MgmtClassRec_PRC;
 ...
 ****************************************************************************/
void
SCCP_DeleteLocalReference_PRC(ITS_CTXT cref)
{
 ...
 ****************************************************************************/
int
INTELLINET_GetSCCPNextRef_PRC(ITS_SS7_HANDLE handle,
                                ITS_CTXT *cref)
{
 ...
 ****************************************************************************/
int
SCCP_GetSharedLocalRef_PRC(void *ctxtData,
                             int len)
{
    ITS_EVENT ev;
...
static ITS_BOOLEAN
CheckForCSS_PRC(ROUTE_Info *linfo)
{
...
SCCP_GetGeneralCfg_PRC()
{
    ...
    ret->traceOn = (TRACE_IsLevelOn(SCCP_PRC_TraceData,
                                    SCCP_TRACE_DEBUG) &&
                    (TRACE_IsOutputOn(SCCP_PRC_TraceData,
                                      SCCP_TRACE_DEBUG,
                                      0) ||
                     TRACE_IsOutputOn(SCCP_PRC_TraceData,
                                      SCCP_TRACE_DEBUG,
                     ...
                     TRACE_IsOutputOn(SCCP_PRC_TraceData,
                                      SCCP_TRACE_DEBUG,
                                      2))) ||
                   (TRACE_IsLevelOn(SCCP_PRC_TraceData,
                                    SCCP_TRACE_WARNING) &&
                    (TRACE_IsOutputOn(SCCP_PRC_TraceData,
                                      SCCP_TRACE_WARNING,
                                      0) ||
                   ...
                   (TRACE_IsLevelOn(SCCP_PRC_TraceData,
                    ...
                    (TRACE_IsOutputOn(SCCP_PRC_TraceData,
                     ...
                     TRACE_IsOutputOn(SCCP_PRC_TraceData,
                                      SCCP_TRACE_EVENT,
                                      2)));
    ret->alarmLevel = SCCP_AlarmLevel_PRC;
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_SetGeneralCfg_PRC(SCCPGeneralCfg *data)
{
    if(SCCP_PRC_TraceData == NULL)
    {
      return !ITS_SUCCESS;
    }

    SCCP_AlarmLevel_PRC = data->alarmLevel;

    if (strstr(data->traceType, "all") != NULL)
    { 
        if (data->traceOn == ITS_TRUE)
        {
            TRACE_SetOutputOnOffByName(SCCP_PRC_TraceData,
                                       SCCP_CRITICAL_STRING,
                                       data->traceOutput, ITS_TRUE);
            TRACE_SetLevelOnOffByName(SCCP_PRC_TraceData,
...
SCCP_GetSSEntry_PRC(ITS_UINT sspc, ITS_OCTET ni, ITS_OCTET ssn)
{
    ...
    ret->testOn = SCMGIsSSTPending_PRC(NULL);
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_SendSST_PRC(SCCPConcernedSSInfo *data)
{
    ...
    error = SSTCStartSST_PRC(&hdr, cpc, cssn);
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_SendSCMG_PRC(SCCPConcernedSSInfo *data)
{
    ...
    if (!data->inservice)
    {
        ...
        PEG_IncrPeg(PRC_SCCP_Pegs, PEG_SCCP_LOCAL_SS_PROHIBITED);
    }
    else
    {
        scmg.type = SCCP_SCMG_SS_ALLOWED;
        PEG_IncrPeg(PRC_SCCP_Pegs, PEG_SCCP_LOCAL_SS_ALLOWED);
    }
    scmg.ssn = data->ssn;
    ...
    for (tinfo = rinfo; tinfo != NULL; tinfo = tinfo->next)
    {
                           ...
                           (tinfo->basic.dpc == data->cpc))
        {
            ...
            error = SendSCMG_PRC(&hdr, &scmg, tinfo->basic.criteria.ssn);
...
GetAllSSNRoutes(ITS_POINTER entry, void *in, void *out)
{
    ...
    if (rinfo->basic.family != FAMILY_CHINA)
    {
        return (ITS_SUCCESS);
    }
...

    {
        ...
        if (*iter->entries != NULL)
        {
            ...
            ent->hasConcernPC = CheckForCSS_PRC(rinfo);
            ...
            ent->testOn = SCMGIsSSTPending_PRC(rinfo);
 ...
 ****************************************************************************/
ITSSS7DLLAPI void
SCCP_GetAllSSEntries_PRC(ITS_UINT* numEntries,
                           SCCPSubSystemInfo** entries)
{
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_SetSSEntry_PRC(SCCPSubSystemInfo *data)
{
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_RemSSEntry_PRC(SCCPSubSystemInfo *data)
{
    ROUTE_MatchInfo rinfo;
    int ret;
...
SCCP_GetPointCodeEntry_PRC(ITS_OCTET ni, ITS_UINT pc)
{
    ...
    dest = ROUTE_FindDestination(pc, ni, FAMILY_CHINA);
    if (dest == NULL)
    {
        ROUTE_UnlockTable();
 ...
 ****************************************************************************/
ITSSS7DLLAPI void
SCCP_GetAllPointCodeEntries_PRC(ITS_UINT* numEntries,
                                  SCCPPointCodeInfo** entries)
{
    PCIterStruct iter;
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_SetPointCodeEntry_PRC(SCCPPointCodeInfo *data)
{
    ROUTE_MatchInfo rinfo;
    int ret;
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_RemPointCodeEntry_PRC(SCCPPointCodeInfo *data)
{
    ROUTE_MatchInfo rinfo;
    int ret;
...
SCCP_GetConcernedPC_PRC(ITS_UINT sspc, ITS_OCTET ni,
 ...
 ****************************************************************************/
ITSSS7DLLAPI void
SCCP_GetAllCPCEntries_PRC(ITS_UINT* numEntries,
                            SCCPConcernedPCInfo** entries)
{
    CPCIterStruct iter;
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_SetConcernedPC_PRC(SCCPConcernedPCInfo* data)
{
    ...
    cinfo.basic.family = FAMILY_CHINA;
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_RemConcernedPC_PRC(SCCPConcernedPCInfo* data)
{
...
SCCP_GetConcernedSS_PRC(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn, 
 ...
 ****************************************************************************/
ITSSS7DLLAPI void
SCCP_GetAllCSSEntries_PRC(ITS_UINT* numEntries,
                            SCCPConcernedSSInfo** entries)
{
    CSSIterStruct iter;
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_SetConcernedSS_PRC(SCCPConcernedSSInfo *data)
{
    ROUTE_Info* rinfo, cinfo, *tlList;
    SCCPConcernedSSInfo* tmp = NULL;
    ITS_BOOLEAN found = ITS_FALSE;
    ...
    tmp = SCCP_GetConcernedSS_PRC(data->pc,
                                   data->ni,
                                   data->ssn,
                                   data->cpc,
                                   data->cssn);
 ...
 ****************************************************************************/
ITSSS7DLLAPI int
SCCP_RemConcernedSS_PRC(SCCPConcernedSSInfo *data)
{
   ...
   lpcList = NULL;
    for (rinfo = tlList; rinfo != NULL; rinfo = rinfo->next)
    {
        ...
        for (cinfo = tcList; cinfo != NULL; cinfo = cinfo->next)
        {
            ...
            if (IsConcerned(rinfo, cinfo))
            {
                tmp = SCCP_GetConcernedSS_PRC(rinfo->basic.dpc,
...
CheckTimeout(ITS_TimerData *td)
{
    ...
    key = DSM_MGMT_REC_PRC_SCCP_SST_SERIAL;
        ...
        error == ITS_SUCCESS)
    {
        if (td->serial == *serial)
        {
            ...
            SCMGHandleSSTTimeout_PRC();

            return;
        }
    }

    key = DSM_MGMT_REC_PRC_SCCP_SOR_SERIAL;
    if ((serial = DSM_FindNoLock(DSM_SccpFtGroupId,
                                 DSM_TABLE_MGMT_DATA,
                                 (ITS_OCTET *)&key, sizeof(ITS_UINT),
                                 &error)) != NULL &&
    ...
    key = DSM_MGMT_REC_PRC_SCCP_SST_IGNORE_SERIAL;
    if ((serial = DSM_FindNoLock(DSM_SccpFtGroupId,
                                 DSM_TABLE_MGMT_DATA,
                                 (ITS_OCTET *)&key, sizeof(ITS_UINT),
                                 &error)) != NULL &&
    ...
    if (((SCCP_REASM_CTXT *)(td->context))->type == SCLC_TIMER_TYPE)
    {
        ...
        SccpGenOmImage_PRC[omBuf.activeOm].SCCPGenPegs.timerTreassExpiry++;
 
        /* Reassembly error- Segment received out of sequence */
        SccpGenOmImage_PRC[omBuf.activeOm].SCCPGenPegs.reassemblyErrSegRxOOS++;
  
        SCLCHandleReasmTimer_PRC(td);
    }
    else
    {
       SCOCHandleTimer_PRC(td);
    }
}
...
InitSettings()
{
     ...
     * timers
     */
    TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_statInfo, 30);
    TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_coordChg, 30);
    TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_ignoreSST, 30);
    TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_rtgStatInfo, 30);
    TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_conn, 180);
    TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_release, 10);
    TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_reset, 20);
    TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_ias, 300);
    TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_iar, 660);
    TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_guard, 240);
    TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_reassembly, 10);
    TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_interval, 30);
    TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_repeatRelease, 10);

    /*
     * behavior control
     */
    BEHAVIORS_SetBehavior(&PRC_SCCP_Behaviors, SCCP_B_propagate_IT,
                          ITS_FALSE);
    BEHAVIORS_SetBehavior(&PRC_SCCP_Behaviors, SCCP_B_propagate_ERR,
                          ITS_FALSE);
    BEHAVIORS_SetBehavior(&PRC_SCCP_Behaviors, SCCP_B_propagate_SCMG,
                          ITS_TRUE);
    BEHAVIORS_SetBehavior(&PRC_SCCP_Behaviors, SCCP_B_use_SCMG,
                          ITS_TRUE);

    /*
     * determine tracing from INI files
     */
    if (TRANSPORT_RES(__SCCP_PRC_RcvQueue) != NULL)
    {
        char value[ITS_PATH_MAX];
        RESFILE_Manager *res = TRANSPORT_RES(__SCCP_PRC_RcvQueue);
        char *name = TRANSPORT_NAME(__SCCP_PRC_RcvQueue);
        ...
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TSTAT_INFO_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_statInfo,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TCOORD_CHG_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_coordChg,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TIGNORE_SST_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_ignoreSST,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TRTG_STAT_INFO_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_rtgStatInfo,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TCONN_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_conn,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TRESET_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_reset,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TRELEASE_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_release,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TINACT_SEND_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_ias,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TINACT_RECV_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_iar,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TGUARD_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_guard,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TREASSEM_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_reassembly,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TINTERVAL_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_interval,
                                 atoi(value));
        }
        if (RESFILE_GetKeyValueOf(res, name, SCCP_TREPEAT_REL_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_repeatRelease,
                                 atoi(value));
        }
        ...
        if (RESFILE_GetKeyValueOf(res, name, SCCP_PROPAGATE_IT_STRING,
                                  value, ITS_PATH_MAX) == ITS_SUCCESS)
        {
            BEHAVIORS_SetBehavior(&PRC_SCCP_Behaviors,
                                  SCCP_B_propagate_IT,
                                  (strcmp(value, "yes") == 0 ||
                                   strcmp(value, "YES") == 0));
        }
 ...
 ****************************************************************************/
void
ISCCP_Terminate_PRC(ITS_SS7_HANDLE handle)
{
    ITS_EVENT ev;
    void *status;
    ITS_UINT key;
    ...
    {
        DBC_RemDoCommand(SCCP_Console_PRC);
    }
    ...
    assert(__SCCP_PRC_RcvQueue != NULL);
    TQUEUETRAN_DeleteTransport(__SCCP_PRC_RcvQueue);

    SCCP_Alarm_PRC(423, __FILE__, __LINE__, "family=%s", "PRC");

    TRACE_Destructor(SCCP_PRC_TraceData);
}
 ...
 ****************************************************************************/
static void
SCCP_StateChangeCallback_PRC(ITS_POINTER object,
 ...
 ****************************************************************************/
int
ISCCP_Initialize_PRC(ITS_SS7_HANDLE handle)
{
    ITS_SERIAL *serial;
    ITS_UINT key;
    int i, j, error;
    ...
    if ((SCCP_PRC_TraceData = TRACE_Constructor("PRC-SCCP",
    ...
    if ((__SCCP_PRC_RcvQueue = TQUEUETRAN_CreateTransport("PRC-SCCP",
                                                            ITS_SCCP_SRC,
                                                            ITS_SCCP)) == NULL)
    {
        ...
        initialized = ITS_FALSE;
        SCCP_Alarm_PRC(416, __FILE__, __LINE__, "family=%s", "PRC");
    ...
    for (i = 0; i < numThreads; i++)
    {
        if (THREAD_CreateThread(&sccpThread[i], 0, PRC_SCCPThread,
    ...
    SCCP_Alarm_PRC(422, __FILE__, __LINE__, "family=%s", "PRC");

    if ((dbClass = ITS_FindFeature(DBC_CLASS_NAME)) != NULL)
    {
        DBC_AddDoCommand(SCCP_Console_PRC);
    }
    ...
    if (redunClass != NULL)
    {
        ...
        if (REDUNDANCY_CONFIGURED_TYPE(redunClass)  == ITS_ACTIVE_STANDBY)
        {
            ...
            CALLBACK_AddCallback(REDUNDANCY_CALLBACK_MGR(redunClass),
                                 SCCP_StateChangeCallback_PRC, NULL);
 ...
 ****************************************************************************
 *  Purpose:
 *      This function is the entry point for the PRC SCCP thread.
 *
 *  Input Parameters:
 *      None.
 *
...
PRC_SCCPThread(void *arg)
{
    ...
    TRANSPORT_IS_ACTIVE(__SCCP_PRC_RcvQueue) = ITS_TRUE;

    while (cont &&
           TRANSPORT_CLASS_GET_NEXT_EVENT(ITS_OBJ_CLASS(__SCCP_PRC_RcvQueue))
                                                        (__SCCP_PRC_RcvQueue,
                                                         &sccpEvent)
           == ITS_SUCCESS)
    {
        ...
        case ITS_GCS_EVENT_FROM_STACK:
            ...
            if (TRACE_IsLevelOn(SCCP_PRC_TraceData,
                                    SCCP_TRACE_DEBUG))
            {
            ...
            if (TRACE_IsLevelOn(SCCP_PRC_TraceData, SCCP_TRACE_EVENT))
            {
                ITS_UINT output = 0;
                if(TRACE_IsOutputOn(SCCP_PRC_TraceData,
                    SCCP_TRACE_EVENT,
                    0))
                {
                    output += 1;
                ...
                //CTF_TraceEvent(sccpEvent.src, ITS_SCCP_PRC_SRC,
                //        output, &sccpEvent);
            }

            sccpEvent.src = ITS_MTP3_SRC;
            SCRCHandleMTP3Msg_PRC(&sccpEvent);
            break;

        case ITS_SCCP_SRC:
            SCCP_DEBUG(("SCCP_Main: handle termination event.\n"));
            
            TRANSPORT_IS_ACTIVE(__SCCP_PRC_RcvQueue) = ITS_FALSE;
        ...
        default:
            ...
            if (TRACE_IsLevelOn(SCCP_PRC_TraceData, SCCP_TRACE_EVENT))
            {
                ...
                //CTF_TraceEvent(src, ITS_SCCP_PRC_SRC,
                //    output, &sccpEvent);
            }
            ...
            case SCCP_MSG_LUDT:
            case SCCP_MSG_LUDTS:  
                SCLCHandleAPPMsg_PRC(&sccpEvent);
                break;
            ...
            case SCCP_MSG_ERR:
            case SCCP_MSG_IT:
                SCOCHandleAPPMsg_PRC(&sccpEvent);
                break;

            default:
                SCCP_ERROR(("SCRCHandleMTP3Msg: invalid message type\n"));
                SCCP_Alarm_PRC(634, __FILE__, __LINE__, "family=%s:mType=%x",
                               "PRC", SCCP_MSG_TYPE(&sccpEvent));
            }
        }

        ROUTE_UnlockRead(routeRWLock, __SCCP_PRC_RcvQueue);
    }
 ...
 ****************************************************************************/
int
SCCP_AddSegmentRef_PRC(ITS_OCTET *dsmKey,
                         ...
                         ITS_OCTET *data, ITS_INT segRefNum)
{
    ...
    if (error == ITS_ENOTFOUND)
    {
                             ...
                             data, dataLen) != ITS_SUCCESS)
        {
            ...
            SCCP_DeleteSegmentRef_PRC(dsmKey, keyLen, ctxt);
 ...
 ****************************************************************************/
void
SCCP_DeleteSegmentRef_PRC(ITS_OCTET *dsmKey, ITS_OCTET keyLen, 
                            SCCP_REASM_CTXT *ctxt)
{
    int   error;
 ...
 ****************************************************************************/
void
SCCP_Console_PRC(DBC_Server *dbc, const char *cmdLine)
{
    ...
    if (DBC_Cmd(dbc, "sp_get_gen_cfg", "SCCP Get General Config", "", ""))
    {
        SCCPGeneralCfg *cfg;
        cfg =  SCCP_GetGeneralCfg_PRC();
    ...
    else if (DBC_Cmd(dbc, "sp_set_gen_cfg", "SCCP Set General Config", 
...

    {
        ...
        ret =  SCCP_SetGeneralCfg_PRC(&cfg);
        if (ret != ITS_SUCCESS)
        {
            return;
        }
...
#if 0
    else if (DBC_Cmd(dbc, "sp_get_ss", "SCCP Get Subsystem Entry", 
                     "<sspc> <ni> <ssn>",
...

    {
        ...
        info = SCCP_GetSSEntry_PRC( (ITS_UINT)pc, (ITS_OCTET)ni, 
                                       (ITS_OCTET)ssn);
    ...
    else if (DBC_Cmd(dbc, "sp_get_all_ssn", "SCCP Get All Subsystem Entries", 
                     "", ""))
    {
        SCCPSubSystemInfo *entry;
        ITS_UINT numEntries;

        SCCP_GetAllSSEntries_PRC( &numEntries, &entry);
    ...
    else if (DBC_Cmd(dbc, "sp_rem_lssn", "SCCP Remove Local Subsystem Entry",
                     "<pc> <ni> <ssn>",
                     "<pc> <ni> <subsystem>"))
    ...
    else if (DBC_Cmd(dbc, "sp_rem_rssn", "SCCP Remove Remote Subsystem Entry", 
                     "<sspc> <ni> <ssn> ",
                     "<point code> <ni> <ssn>"))
    {
        SCCPSubSystemInfo info;
        ...
        destPtr = ROUTE_FindDestination(pc, ni, FAMILY_CHINA);
        if (destPtr == NULL)
        {
            sprintf(buf, "\n*** Invalid PC/NI ***\n");
            DBC_AppendText(dbc, buf);
        ...
        ROUTE_CommitDestination(destPtr, pc, ni, FAMILY_CHINA);
        ...
        SCCP_Alarm_PRC(500, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x", pc,
                       "PRC", ssn);

        DBC_AppendText(dbc, buf);
    }
    else if(DBC_Cmd(dbc, "sp_set_rssn", "SCCP Set Remote Subsystem Entry", 
                     "<sspc> <ni> <ssn> [<linkset>]",
                     "<point code> <ni> <ssn> [<linkset>]"))
    {
        ...
        SCCP_Alarm_PRC(501, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x", "PRC", pc,
                       ssn);
    }
    else if (DBC_Cmd(dbc, "sp_set_lssn", "SCCP Set Local Subsystem Entry",
                     "<pc> <ni> <lssn>",""))
    ...
    else if (DBC_Cmd(dbc, "sp_get_pc", "SCCP Get Point Code Entry", 
                     "<sspc> <ni>",
...

    {
        ...
        info = SCCP_GetPointCodeEntry_PRC( (ITS_OCTET)ni, (ITS_UINT)pc);
    ...
    else if (DBC_Cmd(dbc, "sp_get_all_pc", "SCCP Get All Point Code Entries", 
                     "", ""))

    {
        ...
        SCCP_GetAllPointCodeEntries_PRC( &numEntries, &entry);
    ...
    else if (DBC_Cmd(dbc, "sp_set_pc", "SCCP Set Point Code Entry", 
                     "<sspc> <linkset>",
                   "<point code> <linkset>"))
    {
        ITS_INT linkset;
        ...
        ret = SCCP_SetPointCodeEntry_PRC(&info);
        if (ret != ITS_SUCCESS)
        {
        ...
        SCCP_Alarm_PRC(502, __FILE__, __LINE__, "family=%s:pc=%x", "PRC", pc);
        DBC_AppendText(dbc, buf);

    }
    else if (DBC_Cmd(dbc, "sp_rem_pc", "SCCP Remove Point Code Entry", 
                     "<sspc> <linkset>",
                   "<point code> <linkset>"))

    {
        ...
        ret = SCCP_RemPointCodeEntry_PRC(&info);
        if (ret != ITS_SUCCESS)
        {
        ...
        SCCP_Alarm_PRC(503, __FILE__, __LINE__, "family=%s:pc=%x", "PRC", pc);
        DBC_AppendText(dbc, buf);

    }
    else if (DBC_Cmd(dbc, "sp_get_cpc", "SCCP Get Concerned Point Code Entry", 
                     "<sspc> <ni> <ssn> <cpc>",
                     "<point code> <network indicator> <subsystem>" 
...

    {
        ...
        info = SCCP_GetConcernedPC_PRC( (ITS_UINT)pc, (ITS_OCTET)ni, 
                                          (ITS_OCTET)ssn, (ITS_UINT)cpc);
    ...
    else if (DBC_Cmd(dbc, "sp_get_all_cpc", "SCCP Get All CPC Entries", 
                     "", ""))

    {
        ...
        SCCP_GetAllCPCEntries_PRC( &numEntries, &entry);
    ...
    else if (DBC_Cmd(dbc, "sp_set_cpc", "SCCP Set Concerned Point Code Entry", 
                     "<sspc> <ni> <ssn> <cpc>",
                     "<point code> <network indicator> <subsystem>" 
                      " <concerned point code>"))

    {
        ...
        ret = SCCP_SetConcernedPC_PRC(&info);
        ...
        SCCP_Alarm_PRC(504, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x", "PRC", pc,
                       ssn);
        DBC_AppendText(dbc, buf);

    }

    else if (DBC_Cmd(dbc, "sp_rem_cpc", "SCCP Remove CPC Entry", 
                     "<sspc> <ni> <ssn> <cpc>",
                     "<point code> <network indicator> <subsystem>" 
                      " <concerned point code>"))

    {
        ...
        ret = SCCP_RemConcernedPC_PRC(&info);
        if (ret != ITS_SUCCESS)
        {
        ...
        SCCP_Alarm_PRC(505, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x", "PRC", pc,
                       ssn);
        DBC_AppendText(dbc, buf);

    }
    else if (DBC_Cmd(dbc, "sp_get_cssn", "SCCP Get Concerned SSN Entry", 
...

    {
        ...
        info = SCCP_GetConcernedSS_PRC( (ITS_UINT)pc, (ITS_OCTET)ni, 
                                         (ITS_OCTET)ssn, (ITS_UINT)cpc, 
                                          (ITS_OCTET)cssn);
    ...
    else if (DBC_Cmd(dbc, "sp_get_all_cssn", "SCCP Get All Concerned SSN Entry",
                     "", ""))

    {
        ...
        SCCP_GetAllCSSEntries_PRC( &numEntries, &entry);
    ...
    else if (DBC_Cmd(dbc, "sp_set_cssn", "SCCP Set Concerned SSN Entry", 
                     "<sspc> <ni> <ssn> <cpc> <cssn>",
                     "<point code> <network indicator> <subsystem>" 
                      " <concerned point code> <concerned ssn"))

    {
        ...
        ret = SCCP_SetConcernedSS_PRC(&info);
        if (ret == ITS_ENOTFOUND)
        {
             sprintf(buf, "No Local Route Found For pc=%d, ni=%d, ssn=%d\n",
                     pc, ni, ssn);
        ...
        SCCP_Alarm_PRC(506, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x", "PRC", pc,
                       ssn);
        DBC_AppendText(dbc, buf);

    }
    else if (DBC_Cmd(dbc, "sp_rem_cssn", "SCCP Remove Concerned SSN Entry", 
                     "<sspc> <ni> <ssn> <cpc> <cssn>",
                     "<point code> <network indicator> <subsystem>" 
                      " <concerned point code> <concerned ssn"))

    {
        ...
        ret = SCCP_RemConcernedSS_PRC(&info);
        if (ret != ITS_SUCCESS)
        {
        ...
        SSTCStopSST_PRC(&hdr, cpc, cssn);

        sprintf(buf, "****** SCCP Concerned Subsystem Entry Removed*****\n");
        SCCP_Alarm_PRC(507, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x",  "PRC", pc,
                       ssn);
        DBC_AppendText(dbc, buf);

    }
    else if (DBC_Cmd(dbc, "sp_get_behavior", "SCCP Get Behaviors", 
                     "", ""))

    {
        if (BEHAVIORS_GetBehavior(&PRC_SCCP_Behaviors, SCCP_B_use_SCMG))
        {
        ...
        if (BEHAVIORS_GetBehavior(&PRC_SCCP_Behaviors, SCCP_B_propagate_SCMG))
        {
        ...
        if (BEHAVIORS_GetBehavior(&PRC_SCCP_Behaviors, SCCP_B_propagate_IT))
        {
        ...
        if (BEHAVIORS_GetBehavior(&PRC_SCCP_Behaviors, SCCP_B_propagate_ERR))
        {
    ...
    else if (DBC_Cmd(dbc, "sp_set_behavior", "SCCP Set Behaviors", 
                     "<ps> <us> <pi> <pe>", ""))
    ...
    else if (DBC_Cmd(dbc, "sp_get_tmr", "SCCP Get Timers", 
                     "", ""))
    {
        int value = 0;

        value = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_statInfo);
        sprintf(buf, "    SCCP_T_statInfo       <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_coordChg);
        sprintf(buf, "    SCCP_T_coordChg       <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_ignoreSST);
        sprintf(buf, "    SCCP_T_ignoreSST      <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_rtgStatInfo);
        sprintf(buf, "    SCCP_T_rtgStatInfo    <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_conn);
        sprintf(buf, "    SCCP_T_conn           <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_release);
        sprintf(buf, "    SCCP_T_release        <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_reset);
        sprintf(buf, "    SCCP_T_reset          <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_ias);
        sprintf(buf, "    SCCP_T_ias            <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_iar);
        sprintf(buf, "    SCCP_T_iar            <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_guard);
        sprintf(buf, "    SCCP_T_guard          <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_reassembly);
        sprintf(buf, "    SCCP_T_reassembly     <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_interval);
        sprintf(buf, "    SCCP_T_interval       <%d> \n", value);
        DBC_AppendText(dbc, buf);

        value = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_repeatRelease);
        sprintf(buf, "    SCCP_T_repeatRelease  <%d> \n", value);
        DBC_AppendText(dbc, buf);

    }
    else if (DBC_Cmd(dbc, "sp_set_tmr", "SCCP Set Timer", 
...

    {
        ...
        TIMERS_SetTimerValue(&PRC_SCCP_Timers, tr, value);

        sprintf(buf, "    Timer # %d Set \n", tr);
        DBC_AppendText(dbc, buf);

    }
    else if (DBC_Cmd(dbc, "sp_get_gen_peg", "Get Pegs", "<pg> ", "<Peg#> <>"))
    {
        ...
        {
            /* Q752 Implementation Invoke function SCCP_GetOm_MML_PRC() */
            SCCP_GetOm_MML_PRC(dbc);
        ...
        value = PEG_GetPeg(PRC_SCCP_Pegs, pgNum);
    ...
    else if (DBC_Cmd(dbc, "sp_show_all_pegs", "Dump All SCCP Pegs", "<>", "<>"))
    {
        DumpAllSCCPPegs(dbc);
    ...
    else if (DBC_Cmd(dbc, "sp_clr_gen_peg", "Clear Peg", "<pg> ", "<Peg#> <>"))
    {
        char pgName[ITS_PATH_MAX];
        ITS_INT pgNum;
        ...
        if((strncmp(pgName, "ALL", 3) == 0)||
           ...
           (atoi(pgName) == ALL_SCCP_PEGS))
        {
            /* Q752 Implementation Invoke function SCCP_ResetOm_MML_PRC() */
            SCCP_ResetOm_MML_PRC();

            PEG_ClearBlock(PRC_SCCP_Pegs);
        ...
        PEG_ClearPeg(PRC_SCCP_Pegs, pgNum);

    }

#if 0

    else if (DBC_Cmd(dbc, "sp_clr_all_pegs", "Clear All Pegs", "<> ", "<>"))
    {
        PEG_ClearBlock(PRC_SCCP_Pegs);
    ...
    else if (DBC_Cmd(dbc, "sp_send_ssp", "SCCP Send Sub System Prohibited",
                     "<sspc> <ni> <ssn> <cpc> <cssn> ", "<>"))
    {
        SCCPConcernedSSInfo info;
    ...
    else if (DBC_Cmd(dbc, "sp_send_ssa", "SCCP Send Sub System Allowed",
                     "<sspc> <ni> <ssn> <cpc> <cssn> ", "<>"))
    {
        SCCPConcernedSSInfo info;
    ...
    else if (DBC_Cmd(dbc, "sp_send_sst", "SCCP Send Sub System Test",
                     "<sspc> <ni> <ssn> <cpc> <cssn> ", "<>"))
    {
        SCCPConcernedSSInfo info;
...
SCCP_GetAllPegs(SCCP_Pegs *genPegs)
{
    ...
    genPegs->userMsgsSent = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_MSG_SENT);
    genPegs->remoteMsgsRcvd = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_MSG_RECEIVED);
    genPegs->routingFailure = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_ROUTING_FAILURE);
    genPegs->remoteGTTRequest = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_GTT_REQUEST);
    genPegs->remoteGTTSucc = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_GTT_SUCCESS);
    genPegs->remoteGTTFail = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_GTT_FAILURE);

    genPegs->remoteMsgsDecodeErr = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_MSG_DECODE_ERR);
    genPegs->userMsgsEncodeErr = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_MSG_ENCODE_ERR);

    genPegs->remoteReassembleErr = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_REASSEMBLE_ERR);
    genPegs->hopCounterVilotionErr = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_HOP_COUNTER_VIOLATION_ERR);
    genPegs->userSegmentationErr = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_SEGMENTATION_ERR);
    genPegs->timerTiarExpiry = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_TIMER_TIAR_EXPIRY);
    genPegs->userInitiatedRLSD = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_INITIATED_RLS);
    genPegs->userInitiatedReset = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_INITITATED_RESET);
    genPegs->remoteUDTRcvd = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_UDT_RCVD);
    genPegs->remoteXUDTRcvd = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_XUDT_RCVD);
    genPegs->remoteUDTSRcvd = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_UDTS_RCVD);
    genPegs->remoteXUDTSRcvd = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_XUDTS_RCVD);
    genPegs->remoteLUDTRcvd = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_LUDT_RCVD);
    genPegs->remoteLUDTSRcvd = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_LUDTS_RCVD);
    genPegs->userUDTSent = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_UDT_SENT);
    genPegs->userXUDTSent = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_XUDT_SENT);
    genPegs->userUDTSSent = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_UDTS_SENT);
    genPegs->userXUDTSSent= PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_XUDTS_SENT);
    genPegs->userLUDTSent= PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_LUDT_SENT);
    genPegs->userLUDTSSent = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_LUDTS_SENT);
    genPegs->userCRSent = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_CR_SENT);
    genPegs->sentCREFToRemote = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_CREF_TO_REMOTE_SENT);
    genPegs->sentRSRToRemote = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_RSR_TO_REMOTE_SENT);
    genPegs->sentERRToRemote = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_ERR_TO_REMOTE_SENT);
    genPegs->remoteCRRcvd = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_CR_RCVD);
    genPegs->remoteCREFRcvd = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_CREF_RCVD);
    genPegs->remoteRSRRcvd = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_RSR_RCVD);
    genPegs->remoteERRRcvd = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_ERR_RCVD);
    genPegs->remoteMsgsForLocalSSRcvd = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_MSG_FOR_LOCAL_SS_RCVD);
    genPegs->userDT1Sent = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_DT1_SENT);
    genPegs->remoteDT1Rcvd = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_DT1_RCVD);
    genPegs->userDT2Sent = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_DT2_SENT);
    genPegs->remoteDT2Rcvd = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_DT2_RCVD);
    genPegs->userEDSent = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_ED_SENT);
    genPegs->remoteEDRcvd = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_ED_RCVD);
    genPegs->remoteSSPRcvd= PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_SSP_RCVD);
    genPegs->remoteSSARcvd = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_SSA_RCVD);
    genPegs->remoteSSCRcvd = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_SSC_RCVD);
    genPegs->localSSProhibited = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_LOCAL_SS_PROHIBITED);
    genPegs->localSSAllowed = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_LOCAL_SS_ALLOWED);
    genPegs->localSSCongested = PEG_GetPeg(PRC_SCCP_Pegs, PEG_SCCP_LOCAL_SS_CONGESTED);
...
SCCP_ClearPegs()
{
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_MSG_SENT);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_MSG_RECEIVED);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_ROUTING_FAILURE);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_GTT_REQUEST);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_GTT_SUCCESS);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_GTT_FAILURE);

    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_MSG_DECODE_ERR);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_MSG_ENCODE_ERR);

    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_REASSEMBLE_ERR);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_HOP_COUNTER_VIOLATION_ERR);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_SEGMENTATION_ERR);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_TIMER_TIAR_EXPIRY);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_INITIATED_RLS);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_INITITATED_RESET);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_UDT_RCVD);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_XUDT_RCVD);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_UDTS_RCVD);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_XUDTS_RCVD);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_LUDT_RCVD);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_LUDTS_RCVD);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_UDT_SENT);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_XUDT_SENT);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_UDTS_SENT);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_XUDTS_SENT);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_LUDT_SENT);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_LUDTS_SENT);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_CR_SENT);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_CREF_TO_REMOTE_SENT);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_RSR_TO_REMOTE_SENT);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_ERR_TO_REMOTE_SENT);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_CR_RCVD);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_CREF_RCVD);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_RSR_RCVD);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_ERR_RCVD);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_MSG_FOR_LOCAL_SS_RCVD);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_DT1_SENT);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_DT1_RCVD);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_DT2_SENT);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_DT2_RCVD);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_USER_ED_SENT);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_ED_RCVD);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_SSP_RCVD);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_SSA_RCVD);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_REMOTE_SSC_RCVD);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_LOCAL_SS_PROHIBITED);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_LOCAL_SS_ALLOWED);
    PEG_ClearPeg(PRC_SCCP_Pegs, PEG_SCCP_LOCAL_SS_CONGESTED);
...
int
INTELLINET_SCCP_Commit_PRC(FILE* fp)
{
    int count = 0;
    TRACE_Data* td = SCCP_PRC_TraceData;
    ...
    for (count = 0; count < (int)PRC_SCCP_Timers.numTimers; count++)
    {
        fprintf(fp,"\t\t\t\t\t%s = \"%d\"\n",SCCP_PRC_Timers[count],
                                             PRC_SCCP_Timers.timers[count]);
    }

    fprintf(fp,"\t\t\t\t/>\n");
    fprintf(fp,"\t\t\t\t<BehaviorControl\n");
    fprintf(fp,"\t\t\t\t\t%s = \"%s\"\n", SCCP_PROPAGATE_SCMG_STRING,
            BEHAVIORS_GetBehavior(&PRC_SCCP_Behaviors,
                                   SCCP_B_propagate_SCMG)?
            "yes" : "no");
...
SCCP_OSS_GetGeneralCfg_PRC(SCCP_GeneralCfg* genCfg)
{
    if (genCfg == NULL)
        return ITS_ENOMEM;

    genCfg->alarmLevel = (MGMT_AlarmLevel)SCCP_AlarmLevel_PRC;

    genCfg->debugTrace = TRACE_IsLevelOn(SCCP_PRC_TraceData, SCCP_TRACE_DEBUG);
    if (genCfg->debugTrace)
    {
        genCfg->debugTraceOutput = 0;
        if (TRACE_IsOutputOn(SCCP_PRC_TraceData, SCCP_TRACE_DEBUG, 0))
        {
            genCfg->debugTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(SCCP_PRC_TraceData, SCCP_TRACE_DEBUG, 1))
        {
            genCfg->debugTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(SCCP_PRC_TraceData, SCCP_TRACE_DEBUG, 2))
        {
    ...
    genCfg->warningTrace = TRACE_IsLevelOn(SCCP_PRC_TraceData, SCCP_TRACE_WARNING);
    if (genCfg->warningTrace)
    {
        genCfg->warningTraceOutput = 0;
        if (TRACE_IsOutputOn(SCCP_PRC_TraceData, SCCP_TRACE_WARNING, 0))
        {
            genCfg->warningTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(SCCP_PRC_TraceData, SCCP_TRACE_WARNING, 1))
        {
            genCfg->warningTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(SCCP_PRC_TraceData, SCCP_TRACE_WARNING, 2))
        {
    ...
    genCfg->errorTrace = TRACE_IsLevelOn(SCCP_PRC_TraceData, SCCP_TRACE_ERROR);
    if (genCfg->errorTrace)
    {
        genCfg->errorTraceOutput = 0;
        if (TRACE_IsOutputOn(SCCP_PRC_TraceData, SCCP_TRACE_ERROR, 0))
        {
            genCfg->errorTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(SCCP_PRC_TraceData, SCCP_TRACE_ERROR, 1))
        {
            genCfg->errorTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(SCCP_PRC_TraceData, SCCP_TRACE_ERROR, 2))
        {
    ...
    genCfg->criticalTrace = TRACE_IsLevelOn(SCCP_PRC_TraceData, SCCP_TRACE_CRITICAL);
    if (genCfg->criticalTrace)
    {
        genCfg->criticalTraceOutput = 0;
        if (TRACE_IsOutputOn(SCCP_PRC_TraceData, SCCP_TRACE_CRITICAL, 0))
        {
            genCfg->criticalTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(SCCP_PRC_TraceData, SCCP_TRACE_CRITICAL, 1))
        {
            genCfg->criticalTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(SCCP_PRC_TraceData, SCCP_TRACE_CRITICAL, 2))
        {
    ...
    genCfg->eventTrace = TRACE_IsLevelOn(SCCP_PRC_TraceData, SCCP_TRACE_EVENT);
    if (genCfg->eventTrace)
    {
        genCfg->eventTraceOutput = 0;
        if (TRACE_IsOutputOn(SCCP_PRC_TraceData, SCCP_TRACE_EVENT, 0))
        {
            genCfg->eventTraceOutput |= TRACE_TO_DISPLAY;
        }
        if (TRACE_IsOutputOn(SCCP_PRC_TraceData, SCCP_TRACE_EVENT, 1))
        {
            genCfg->eventTraceOutput |= TRACE_TO_FILE;
        }
        if (TRACE_IsOutputOn(SCCP_PRC_TraceData, SCCP_TRACE_EVENT, 2))
        {
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_SetGeneralCfg_PRC(SCCP_GeneralCfg* genCfg)
{
    if (genCfg == NULL)
        return ITS_ENOMEM;

    SCCP_AlarmLevel_PRC = genCfg->alarmLevel;

    if (genCfg->debugTrace)
    {
        TRACE_SetLevelOnOffByName(SCCP_PRC_TraceData, SCCP_DEBUG_STRING,  ITS_TRUE);
        ...
        if (genCfg->debugTraceOutput & TRACE_TO_NONE)
        {
            ...
            TRACE_SetLevelOnOffByName(SCCP_PRC_TraceData, SCCP_DEBUG_STRING,  ITS_FALSE);
        }
    }

    if (genCfg->warningTrace)
    {
        TRACE_SetLevelOnOffByName(SCCP_PRC_TraceData, SCCP_WARNING_STRING,  ITS_TRUE);
        ...
        if (genCfg->warningTraceOutput & TRACE_TO_NONE)
        {
            ...
            // Set trace level to false
            TRACE_SetLevelOnOffByName(SCCP_PRC_TraceData, SCCP_WARNING_STRING,  ITS_FALSE);
        }
    }

    if (genCfg->errorTrace)
    {
        TRACE_SetLevelOnOffByName(SCCP_PRC_TraceData, SCCP_ERROR_STRING,  ITS_TRUE);
        ...
        if (genCfg->errorTraceOutput & TRACE_TO_NONE)
        {
            ...
            // Set trace level to false
            TRACE_SetLevelOnOffByName(SCCP_PRC_TraceData, SCCP_ERROR_STRING,  ITS_FALSE);
        }
   }

    if (genCfg->criticalTrace)
    {
        TRACE_SetLevelOnOffByName(SCCP_PRC_TraceData, SCCP_CRITICAL_STRING,  ITS_TRUE);
        ...
        if (genCfg->criticalTraceOutput & TRACE_TO_NONE)
        {
            ...
            // Set trace level to false
            TRACE_SetLevelOnOffByName(SCCP_PRC_TraceData, SCCP_CRITICAL_STRING,  ITS_FALSE);
        }
    }

    if (genCfg->eventTrace)
    {
        TRACE_SetLevelOnOffByName(SCCP_PRC_TraceData, SCCP_EVENT_STRING,  ITS_TRUE);
        ...
        if (genCfg->eventTraceOutput & TRACE_TO_NONE)
        {
            ...
            // Set trace level to false
            TRACE_SetLevelOnOffByName(SCCP_PRC_TraceData, SCCP_EVENT_STRING,  ITS_FALSE);
        }
    }
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_GetBehaviors_PRC(SCCP_Behaviors* behaviors)
{
    if (behaviors == NULL)
        return ITS_ENOMEM;

    behaviors->useSCMG = BEHAVIORS_GetBehavior(&PRC_SCCP_Behaviors, SCCP_B_use_SCMG);
    behaviors->propogateSCMG = BEHAVIORS_GetBehavior(&PRC_SCCP_Behaviors, SCCP_B_propagate_SCMG);
    behaviors->propogateIT = BEHAVIORS_GetBehavior(&PRC_SCCP_Behaviors, SCCP_B_propagate_IT);
    behaviors->propogateERR = BEHAVIORS_GetBehavior(&PRC_SCCP_Behaviors, SCCP_B_propagate_ERR);
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_SetBehavior_PRC(ITS_UINT behId, ITS_BOOLEAN behValue, SCCP_Behaviors* behaviors)
{
    if (behId == SCCP_ALL_BEHAVIORS)
    {
        ...
        BEHAVIORS_SetBehavior(&PRC_SCCP_Behaviors, SCCP_B_propagate_IT, behaviors->useSCMG);
          BEHAVIORS_SetBehavior(&PRC_SCCP_Behaviors, SCCP_B_propagate_ERR, behaviors->propogateSCMG);
          BEHAVIORS_SetBehavior(&PRC_SCCP_Behaviors, SCCP_B_propagate_SCMG, behaviors->propogateIT);
          BEHAVIORS_SetBehavior(&PRC_SCCP_Behaviors, SCCP_B_use_SCMG, behaviors->propogateERR);
    }
    else
    {
        ...
        {
            BEHAVIORS_SetBehavior(&PRC_SCCP_Behaviors, behId, behValue);
        }
    }
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_AddLocalSSN_PRC(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn)
{
    ITS_INT             ret;
    SCCPSubSystemInfo   info;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_RemoveLocalSSN_PRC(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn)
{
    ITS_INT             ret;
    SCCPSubSystemInfo   info;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_AddRemoteSSN_PRC(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn)
{
    ITS_INT             ret;
    SCCPSubSystemInfo   info;
    ITS_OCTET           linkset = 0;
    ...
    SCCP_Alarm_PRC(501, __FILE__, __LINE__, "family=%s:pc=%x:ssn=%x", "PRC", pc, ssn);
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_RemoveRemoteSSN_PRC(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn)
{
    ITS_INT             ret;
    SCCPSubSystemInfo   info;
    ITS_OCTET           linkset = 0;
    ...
    SCCP_Alarm_PRC(500, __FILE__, __LINE__, "famiy=%s:pc=%x:ssn=%x", "PRC", pc, ssn);
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_GetAllSSN_PRC(SCCPSubSystemInfo** entries, ITS_UINT* numEntries)
{
    SCCP_GetAllSSEntries_PRC(numEntries, entries);
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT
SCCP_OSS_AddConcernedSSN_PRC(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn, ITS_UINT cpc, ITS_OCTET cssn)
{
    ITS_INT                ret;
    SCCPConcernedSSInfo    info;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_RemoveConcernedSSN_PRC(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn, ITS_UINT cpc, ITS_OCTET cssn)
{
    ITS_INT             ret;
    SCCPConcernedSSInfo info;
    MTP3_HEADER         hdr;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_GetConcernedSSN_PRC(SCCPConcernedSSInfo** entries, ITS_UINT* numEntries)
{
    SCCP_GetAllCSSEntries_PRC(numEntries, entries);
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_GetPegs_PRC(SCCP_Pegs* sccpPegs)
{
    if (sccpPegs == NULL)
        return ITS_ENOMEM;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_ClearPeg_PRC(ITS_UINT pegId)
{
    ...
    else
    {
        ...
        {
            PEG_ClearPeg(PRC_SCCP_Pegs, pegId);
        }
    }
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_GetTimers_PRC(SCCP_Timers* timers)
{

    if (timers == NULL)
        return ITS_ENOMEM;

    timers->statInfo = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_statInfo);
    timers->coordChg = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_coordChg);
    timers->ignoreSST = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_ignoreSST);
    timers->rtgStatInfo = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_rtgStatInfo);
    timers->conn = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_conn);
    timers->release = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_release);
    timers->reset = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_reset);
    timers->ias = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_ias);
    timers->iar = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_iar);
    timers->guard = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_guard);
    timers->reassembly = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_reassembly);
    timers->interval = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_interval);
    timers->repeatRelease = TIMERS_GetTimerValue(&PRC_SCCP_Timers, SCCP_T_repeatRelease);
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_SetTimer_PRC(ITS_UINT timerId, ITS_UINT timerValue, SCCP_Timers* timers)
{
    if (timerId == SCCP_ALL_TIMERS)
    {
        ...
        TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_statInfo, timers->statInfo);
        TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_coordChg, timers->coordChg);
        TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_ignoreSST, timers->ignoreSST);
        TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_rtgStatInfo, timers->rtgStatInfo);
        TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_conn, timers->conn);
        TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_release, timers->release);
        TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_reset, timers->reset);
        TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_ias, timers->ias);
        TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_iar, timers->iar);
        TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_guard, timers->guard);
        TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_reassembly, timers->reassembly);
        TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_interval, timers->interval);
        TIMERS_SetTimerValue(&PRC_SCCP_Timers, SCCP_T_repeatRelease, timers->repeatRelease);
    }
    else
    {
        ...
        {
            TIMERS_SetTimerValue(&PRC_SCCP_Timers, timerId, timerValue);
        }
    }
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_SendSSA_PRC(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn, ITS_UINT cpc, ITS_OCTET cssn)
{
    ITS_INT                ret;
    SCCPConcernedSSInfo info;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_SendSSP_PRC(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn, ITS_UINT cpc, ITS_OCTET cssn)
{
    ITS_INT                ret;
    SCCPConcernedSSInfo info;
 ...
 ****************************************************************************/
ITSSS7DLLAPI ITS_INT 
SCCP_OSS_SendSST_PRC(ITS_UINT pc, ITS_OCTET ni, ITS_OCTET ssn, ITS_UINT cpc, ITS_OCTET cssn)
{
    ITS_INT             ret;
    SCCPConcernedSSInfo info;
...
void SCCP_ResetOm_PRC()
{
    ...
    SCCPPegsOmDataInitialize(&SccpGenOmImage_PRC[omBuf.activeOm]);
 ...
 *
 ****************************************************************************/
void SCCP_GetOm_PRC(sccpOm *OmSccpData)
{
    ITS_OCTET j, ssn;
    SS7_Family famDesc;
    ...
    SCCPPegsOmDataInitialize(&SccpGenOmImage_PRC[omBuf.inactiveOm]);
    ...
    memcpy(&OmSccpData->genStats,
           &SccpGenOmImage_PRC[omBuf.inactiveOm],
           sizeof(sccpGenPegs));
...
/****************************************************************************
 *  Purpose:
 *      This API is invoked by the sp_clr_gen_peg() MML command. This will
 *      clear the ACTIVE SCCP OM Buffer.
 *
...
/****************************************************************************
 *  Purpose:
 *      This API is invoked by the sp_get_gen_peg() MML command.  This will
 *      clear the INACTIVE SCCP OM Buffer, toggle the ACTIVE/INACTIVE
...
void SCCP_GetOm_MML_PRC(DBC_Server *dbc)
{
    char buf[32000];
    char buf1[32000];
    ...
    sprintf(buf, "\n Routing Failure No Translation Address   : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.routingFailureNoTransAddr);
    strcat(buf1, buf);

    sprintf(buf, "\n Routing Failure No Trans Specific Address: %d",  
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.routingFailureNoTransSpecificAddr);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Syntax Value Error Detected              : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.syntaxValueErrDetected);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Syntax Construction Err Detected         : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.syntaxConstructionErrDetected);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Timer Treass Expiry                      : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.timerTreassExpiry);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Reassembly Error Segmentation Rx OOS     : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.reassemblyErrSegRxOOS);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Reassembly Error No Reassembly Space     : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.reassemblyErrNoReassemblySpace);
    strcat(buf1, buf);
           
    sprintf(buf, "\n HopCounter Violation Error               : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.hopCounterViolationErr);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Timer Tiar Expiry                        : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.timerTiarExpiry);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Provider Initiated Reset Connection      : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.providerInitiatedResetConn);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Provider Initiated Release Connection    : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.providerInitiatedReleaseConn);
    strcat(buf1, buf);
           
    sprintf(buf, "\n User Initiated Reset Connection          : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.userInitiatedResetConn);
    strcat(buf1, buf);
           
    sprintf(buf, "\n User Initiated Release Connection        : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.userInitiatedReleaseConn);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Segmentation Error Segmentation Failed   : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.segmentationErrSegFailed);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Reassembly Error Reassembly Failed       : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.reassemblyErrReassemblyFailed);
    strcat(buf1, buf);
    
    sprintf(buf, "\n Msg UDTS Sent                            : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageUDTSSent);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg UDTS Recd                            : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageUDTSRcvd);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg UDT Sent                             : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageUDTSent);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg UDT Recd                             : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageUDTRcvd);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg CR Sent                              : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageCRSent);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg CR Recd                              : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageCRRcvd);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg CREF Sent                            : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageCREFSent);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg CREF Recd                            : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageCREFRcvd);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg RSR Sent                             : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageRSRSent);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg RSR Recd                             : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageRSRRcvd);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg ERR Sent                             : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageErrSent);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg ERR Recd                             : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageErrRcvd);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg XUDT Sent                            : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageXUDTSent);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg XUDT Recd                            : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageXUDTRcvd);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg XUDTS Sent                           : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageXUDTSSent);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg XUDTS Recd                           : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageXUDTSRcvd);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg LUDT Sent                            : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageLUDTSent);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg LUDT Recd                            : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageLUDTRcvd);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg LUDTS Sent                           : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageLUDTSSent);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg LUDTS Recd                           : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageLUDTSRcvd);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg From Local SubSystem                 : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageFromLocalSS);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg From Remote SubSystem                : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageFromRemoteSS);
    strcat(buf1, buf);
           
    sprintf(buf, "\n Msg Intended for Local SubSystem         : %d", 
                     SccpGenOmImage_PRC[omBuf.inactiveOm].SCCPGenPegs.messageForLocalSS);
    strcat(buf1, buf);
