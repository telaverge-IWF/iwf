 ...
 ****************************************************************************
 *
 *  ID: $Id: ansi-srm.c,v 9.8.8.1 2010/06/09 20:20:57 omayor Exp $
 *
 * LOG: $Log: ansi-srm.c,v $
 * LOG: Revision 9.8.8.1  2010/06/09 20:20:57  omayor
 ...
 * LOG:
 * LOG: Revision 9.7.4.2  2007/05/18 09:51:49  raghavendrabk
 * LOG: Changes for Alarm Optimisation
 * LOG:
 * LOG: Revision 9.7.4.1  2007/02/02 14:09:46  raghavendrabk
 * LOG: Parallel Stack changes for Alarms and Statistics
 * LOG:
 * LOG: Revision 9.7  2005/05/17 06:02:54  adutta
 * LOG: Fixed issue# 1839
 * LOG:
 * LOG: Revision 9.6  2005/05/12 11:59:20  adutta
 * LOG: Fixed issue #1833
 * LOG:
 * LOG: Revision 9.5  2005/05/10 14:03:52  adutta
 * LOG: Route peg clean ups
 * LOG:
 * LOG: Revision 9.4  2005/05/10 06:36:55  adutta
 * LOG: Null checks for route pegs
 * LOG:
 * LOG: Revision 9.3  2005/05/09 11:43:31  adutta
 * LOG: Typeo
 * LOG:
 * LOG: Revision 9.2  2005/05/06 06:09:58  adutta
 * LOG: Fix for Bug#1808
 * LOG: MTP Route peg declaration
 * LOG:
 * LOG: Revision 9.1  2005/03/23 12:54:33  cvsadmin
 * LOG: Begin PR6.5
 * LOG:
 * LOG: Revision 8.4  2005/03/21 13:53:37  cvsadmin
 * LOG: PR6.4.2 Source Propagated to Current
 * LOG:
 * LOG: Revision 7.4.2.2.2.1.8.2  2005/01/05 12:51:40  sbabu
 * LOG:  Removed unwanted alarms
 * LOG:
 * LOG: Revision 7.4.2.2.2.1.8.1  2004/12/31 06:54:48  sbabu
 * LOG: MTP3 OAM Changes
 * LOG:
 * LOG: Revision 7.4.2.2.2.1  2004/08/10 12:14:45  kannanp
 * LOG: Changes for MTP3 pegs, alarms & MMLs - propagated from TCS.
 * LOG:
 * LOG: Revision 7.4.2.2  2003/12/12 11:47:05  kannanp
 * LOG: Fix bug 586 RSP message is not sent for every T10 expiry.
 * LOG:
 * LOG: Revision 7.4.2.1  2003/11/28 07:50:11  kannanp
 * LOG: Peg Fix patch from Current.
 * LOG:
 * LOG: Revision 7.4  2003/02/13 10:25:54  ttipatre
 * LOG: Pegs are modified.
 * LOG:
 * LOG: Revision 7.3  2002/12/24 07:41:44  ttipatre
 * LOG: Link Level Pegs.
 * LOG:
 * LOG: Revision 7.2  2002/11/20 05:11:58  ttipatre
 * LOG: Alarms and Pegs code submission.
 * LOG:
 * LOG: Revision 7.1  2002/08/26 22:10:28  mmiers
 * LOG: Begin PR6.2
 * LOG:
 * LOG: Revision 6.4  2002/07/25 14:21:51  randresol
 * LOG: Fix bugs found during functional testing.
 * LOG:
 * LOG: Revision 6.3  2002/06/21 22:49:38  mmiers
 ...
 * LOG: Begin PR7.
 * LOG:
 * LOG: Revision 5.47  2001/12/11 23:49:52  mmiers
 * LOG: Get rid of FIXMEs.
 * LOG:
 * LOG: Revision 5.46  2001/12/06 22:39:04  mmiers
 * LOG: First cut of ITU MTP3.
 * LOG:
 * LOG: Revision 5.45  2001/12/05 23:41:58  mmiers
 * LOG: Continuing with STP testing.
 * LOG:
 * LOG: Revision 5.44  2001/12/04 23:59:50  mmiers
 * LOG: Route testing.
 * LOG:
 * LOG: Revision 5.43  2001/12/04 00:15:55  mmiers
 * LOG: STP testing.
 * LOG:
 * LOG: Revision 5.42  2001/11/30 23:43:14  mmiers
 * LOG: Start testing config-d (route management).
 * LOG:
 * LOG: Revision 5.41  2001/11/14 23:17:13  mmiers
 * LOG: Many changes from conformance testing.
 * LOG:
 * LOG: Revision 5.40  2001/10/22 21:18:21  rsonak
...
#ident "$Id: ansi-srm.c,v 9.8.8.1 2010/06/09 20:20:57 omayor Exp $"
...
/*
 * for testing.  INET doesn't consider clusters.
 */
#define MTP3_SUPPORTS_CLUSTERS  0
 ...
 ****************************************************************************/
int
SRM_Main_ANSI(MTP3_SubSystems src, MTP3_SubSystems dest,
              ...
              ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ...
    MTP3_TRACE_ENTRY_ANSI(MTP3_SRM, src, trigger, sif, len,
                          linkSet, linkCode);
    ...
    if (trigger == SRM_TRIGGER_L2_MSG)
    {
        ...
        case SNMM_HC0_FCM_ANSI: /* transfer controlled/congestion */
            ...
            {
            case SNMM_HC1_RCT_ANSI: /* route congestion test */
                return RCAT_Main_ANSI(src, SRM_TRIGGER_CONG_TEST,
                                      sif, len,
                                      linkSet, linkCode);

            case SNMM_HC1_TFC_ANSI: /* transfer controlled */
                return RTCC_Main_ANSI(src, SRM_TRIGGER_TRANSFER_CONTROL,
                                      sif, len,
                                      linkSet, linkCode);
            default:
        ...
        case SNMM_HC0_TFM_ANSI: /* transfer prohibit/allow/restrict */
            switch (SNMM_HC1_GET_ANSI(*snmm))
            {
            case SNMM_HC1_TFP_ANSI: /* transfer prohibited */
                return RTPC_Main_ANSI(src,
                                      SRM_TRIGGER_TRANSFER_PROHIBIT,
                                      sif, len,
                                      linkSet, linkCode);

            case SNMM_HC1_TCP_ANSI: /* cluster transfer prohibited */
                return RTPC_Main_ANSI(src,
                                      SRM_TRIGGER_TCP,
                                      sif, len,
                                      linkSet, linkCode);

            case SNMM_HC1_TFR_ANSI: /* transfer restricted */
                return RTRC_Main_ANSI(src,
                                      SRM_TRIGGER_TRANSFER_RESTRICT,
                                      sif, len,
                                      linkSet, linkCode);

            case SNMM_HC1_TCR_ANSI: /* cluster transfer restricted */
                return RTRC_Main_ANSI(src,
                                      SRM_TRIGGER_TCR,
                                      sif, len,
                                      linkSet, linkCode);

            case SNMM_HC1_TFA_ANSI: /* transfer allowed */
                return RTAC_Main_ANSI(src,
                                      SRM_TRIGGER_TRANSFER_ALLOW,
                                      sif, len,
                                      linkSet, linkCode);

            case SNMM_HC1_TCA_ANSI: /* cluster transfer allowed */
                return RTAC_Main_ANSI(src,
                                      SRM_TRIGGER_TRANSFER_ALLOW,
                                      sif, len,
                                      linkSet, linkCode);
        ...
        case SNMM_HC0_RSM_ANSI: /* route set test */
            switch (SNMM_HC1_GET_ANSI(*snmm))
            {
            case SNMM_HC1_RSP_ANSI: /* route set test prohibited */
            case SNMM_HC1_RSR_ANSI: /* route set test restricted */
                return RSRT_Main_ANSI(src,
                                      SRM_TRIGGER_ROUTE_SET_TEST,
                                      sif, len,
                                      linkSet, linkCode);

            case SNMM_HC1_RCP_ANSI: /* cluster route set test prohibited */
            case SNMM_HC1_RCR_ANSI: /* cluster route set test restricted */
                return RSRT_Main_ANSI(src,
                                      SRM_TRIGGER_CLUSTER_ROUTE_SET_TEST,
                                      sif, len,
                                      linkSet, linkCode);
            default:
 ...
 ****************************************************************************/
ITS_OCTET
MTP3_FindAdjacent_ANSI(ITS_OCTET ni, ITS_UINT pc,
                       ITS_OCTET linkSet)
{
 ...
 ****************************************************************************/
int
RTPC_Main_ANSI(MTP3_SubSystems src, SRM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    ...
    MTP3_TRACE_ENTRY_ANSI(MTP3_SRM_RTPC, src, trigger, sif, len,
                          linkSet, linkCode);
    ...
    case SRM_TRIGGER_TRANSFER_PROHIBIT:      /* from HMDT */
        isnmm = (SNMM_MESSAGE *)&sif[sizeof(MTP3_HEADER_ANSI) +
                                     sizeof(ITS_OCTET)];
        pc1 = SNMM_TRANSFER_STATE_GET_DEST_ANSI(isnmm->data.transferState);
        ...
        linkSet = MTP3_FindAdjacent_ANSI(ni, pc2, linkSet);

        rc = ROUTE_FindDestination(pc2, ni, FAMILY_ANSI);
        if (rc)
        {
            PEG_IncrPeg(&rc->MTP3_DPC_PEGS, PEG_MTP3_DPC_TFP_RX);
        }
        ...
        if (linkSet == ITS_SS7_DEFAULT_LINK_SET)
        {
            ...
            MTP3_Alarm_ANSI(2048, __FILE__, __LINE__, "pc=%x:family=%s",pc2, "ANSI");
    ...
    case SRM_TRIGGER_MSG_FOR_INACCESS_SP:    /* from HMRT */
                                      ...
                                      ITS_FALSE)) == NULL)
        {
            ...
            SNMM_HC0_SET_ANSI(snmm, SNMM_HC0_TFM_ANSI);
            SNMM_HC1_SET_ANSI(snmm, SNMM_HC1_TFP_ANSI);
            SNMM_TRANSFER_STATE_SET_DEST_ANSI(snmm.data.transferState, pc1);

            ITS_EVENT_INIT(&ev, ITS_MTP3_ANSI_SRC, 0);

            SNMM_Encode_ANSI(&ev, &omtp3, &snmm);

            rc = ROUTE_FindDestination(ls->adjacent, ni, FAMILY_ANSI);
            if (rc)
            {
             ...
             * available route.
             */
            ret = HMRT_Main_ANSI(MTP3_SRM_RTPC,
        ...
        if (rc->entries[whichEntry]->t8 == TIMER_BAD_SERIAL)
        {
            ...
            rc = ROUTE_FindDestination(pc1, ni, FAMILY_ANSI);
            if (rc)
            {
                PEG_IncrPeg(&rc->MTP3_DPC_PEGS, PEG_MTP3_DPC_TFP_TX);
            }
    ...
    case SRM_TRIGGER_NEW_STP_DEST:           /* from TSRC */
        ...
        if (rc->entries[whichEntry]->t8 == TIMER_BAD_SERIAL)
        {
            ...
            rc = ROUTE_FindDestination(pc1, ls->ni, FAMILY_ANSI);
        //    if (rc)
            {
                PEG_IncrPeg(&rc->MTP3_DPC_PEGS, PEG_MTP3_DPC_TFP_TX);
            }
    ...
    case SRM_TRIGGER_CLUSTER_INACCESS:       /* from TSRC */
        /* only STPs perform this action */
        if (!STPFunction_ANSI())
        {
            return (ITS_SUCCESS);
        ...
        for (pc2 = 0; pc2 < ITS_SS7_DEFAULT_LINK_SET; pc2++)
        {
            ...
            /* USE any route to the destination */
            ret = HMRT_Main_ANSI(MTP3_SRM_RTPC,
                                 SMH_TRIGGER_ROUTE_MGMT_MSG,
                                 ev.data, ev.len,
                                 ITS_SS7_DEFAULT_LINK_SET,
    ...
    case SRM_TRIGGER_MSG_FOR_INACCESS_CLUST: /* from HMRT */
        /* only STPs perform this action */
        if (!STPFunction_ANSI())
        {
            return (ITS_SUCCESS);
                                      ...
                                      ITS_FALSE)) == NULL)
        {
            ...
            SNMM_HC0_SET_ANSI(snmm, SNMM_HC0_TFM_ANSI);
#if MTP3_SUPPORTS_CLUSTERS
            SNMM_HC1_SET_ANSI(snmm, SNMM_HC1_TCP_ANSI);
#else
            SNMM_HC1_SET_ANSI(snmm, SNMM_HC1_TFP_ANSI);
#endif
            SNMM_TRANSFER_STATE_SET_DEST_ANSI(snmm.data.transferState, pc1);
    ...
    case SRM_TRIGGER_SEND_TCP:               /* from RSRT */
    case SRM_TRIGGER_NEW_STP_FOR_CLUST:      /* from TSRC */
        /* only STPs perform this action */
        if (!STPFunction_ANSI())
        {
            return (ITS_SUCCESS);
    ...
    case SRM_TRIGGER_TCP:                    /* from HMDT */
        isnmm = (SNMM_MESSAGE *)&sif[sizeof(MTP3_HEADER_ANSI) +
                                     sizeof(ITS_OCTET)];
        pc1 = SNMM_TRANSFER_STATE_GET_DEST_ANSI(isnmm->data.transferState);
        ni = MTP3_HDR_GET_SIO(*mtp3) & MTP3_NIC_MASK;
 ...
 ****************************************************************************/
int
RTAC_Main_ANSI(MTP3_SubSystems src, SRM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    MTP3_HEADER_ANSI *mtp3 = (MTP3_HEADER_ANSI *)&sif[1];
    ...
    MTP3_TRACE_ENTRY_ANSI(MTP3_SRM_RTAC, src, trigger, sif, len,
                          linkSet, linkCode);
    ...
    case SRM_TRIGGER_TRANSFER_ALLOW:         /* from HMRT */
        ...
        PEG_IncrPeg(ANSI_MTP3_Pegs, PEG_MTP3_TFA_RECEIVED);

        rc = ROUTE_FindDestination(pc2, ni, FAMILY_ANSI);
        if (rc)
        {
            PEG_IncrPeg(&rc->MTP3_DPC_PEGS, PEG_MTP3_DPC_TFA_RX);
        }
        ...
        if (linkSet == ITS_SS7_DEFAULT_LINK_SET)
        {
            ...
            MTP3_Alarm_ANSI(2051, __FILE__, __LINE__, "pc=%x:family=%s",pc2, "ANSI");
            return (ITS_EINVALIDARGS);
        }
    ...
    case SRM_TRIGGER_DEST_ACCESS:            /* from TSRC */
        ...
        for (pc2 = 0; pc2 < ITS_SS7_DEFAULT_LINK_SET; pc2++)
        {
            ...
            SNMM_HC0_SET_ANSI(snmm, SNMM_HC0_TFM_ANSI);
            SNMM_HC1_SET_ANSI(snmm, SNMM_HC1_TFA_ANSI);
            SNMM_TRANSFER_STATE_SET_DEST_ANSI(snmm.data.transferState, pc1);
            ...
            ret = HMRT_Main_ANSI(MTP3_SRM_RTAC,
                                 SMH_TRIGGER_ROUTE_MGMT_MSG,
                                 ev.data, ev.len,
                                 ITS_SS7_DEFAULT_LINK_SET,
                                 ITS_SS7_DEFAULT_LINK_CODE);
    ...
    case SRM_TRIGGER_CLUSTER_ACCESS:         /* from TSRC */
        /* only STPs perform this action */
        if (!STPFunction_ANSI())
        {
            return (ITS_SUCCESS);
        ...
        for (pc2 = 0; pc2 < ITS_SS7_DEFAULT_LINK_SET; pc2++)
        {
            ...
            SNMM_HC0_SET_ANSI(snmm, SNMM_HC0_TFM_ANSI);
#if MTP3_SUPPORTS_CLUSTERS
            SNMM_HC1_SET_ANSI(snmm, SNMM_HC1_TCA_ANSI);
#else
            SNMM_HC1_SET_ANSI(snmm, SNMM_HC1_TFA_ANSI);
#endif
            SNMM_TRANSFER_STATE_SET_DEST_ANSI(snmm.data.transferState, pc1);
    ...
    case SRM_TRIGGER_REM_STP_FOR_CLUST:      /* from TSRC */
    case SRM_TRIGGER_SEND_TCA:               /* from RSRT */
        /* only STPs perform this action */
        if (!STPFunction_ANSI())
        {
            return (ITS_SUCCESS);
         ...
         * NOTE: pc1 is dest to allow.  linkSet says which
         * adjacent to send to.
         * DON'T CARE about linkSet.  TSRC and RSRT have to get
         * this right.
         */
        pc1 = MTP3_RL_GET_DPC_VALUE(mtp3->label);
    ...
    case SRM_TRIGGER_TCA:                    /* from HMDT */
        isnmm = (SNMM_MESSAGE *)&sif[sizeof(MTP3_HEADER_ANSI) +
                                     sizeof(ITS_OCTET)];
        pc1 = SNMM_TRANSFER_STATE_GET_DEST_ANSI(isnmm->data.transferState);
        ni = MTP3_HDR_GET_SIO(*mtp3) & MTP3_NIC_MASK;
        ...
        /* get the routing info for this destination */
        /* USE the link set we just found */
        if (ROUTE_ParseRouteMatchInfoArgs(&rinfo, pc1,
                                          REMOTE_ROUTE, ROUTE_DPC_NI,
                                          FAMILY_ANSI,
                                          ni, linkSet,
                                      ...
                                      ITS_FALSE)) != NULL)
        {
            ...
            TSRC_Main_ANSI(MTP3_SRM_RTAC, STM_TRIGGER_TCA,
                           ev.data, ev.len,
                           linkSet,
                           ITS_SS7_DEFAULT_LINK_CODE);
            ...
            RSRT_Main_ANSI(MTP3_SRM_RTAC, SRM_TRIGGER_TCA_RCVD,
                           ev.data, ev.len,
                           linkSet,
                           ITS_SS7_DEFAULT_LINK_CODE);
        ...
        else
        {
            ...
            for (i = 0; i < 255; i++)
            {
                ...
                else
                {
                    ...
                    /* notify TCRC signalling route available */
                    /* TELL TCRC which path (linkSet) */
                    TCRC_Main_ANSI(MTP3_SRM_RTAC,
                                   STM_TRIGGER_ROUTE_AVAILABLE,
                                   ev.data, ev.len,
                                   linkSet,
                                   ITS_SS7_DEFAULT_LINK_CODE);

                    /* notify RTPC of controlled rerouting */
                    /* TELL RTPC which path (linkSet) */
                    RTPC_Main_ANSI(MTP3_SRM_RTAC,
 ...
 ****************************************************************************/
int
RSRT_Main_ANSI(MTP3_SubSystems src, SRM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    MTP3_HEADER_ANSI *mtp3 = (MTP3_HEADER_ANSI *)&sif[1];
    ...
    MTP3_TRACE_ENTRY_ANSI(MTP3_SRM_RSRT, src, trigger, sif, len,
                          linkSet, linkCode);
    ...
    case SRM_TRIGGER_START_ROUTE_SET_TEST:   /* from RTRC: RTPC: TSRC */
        ...
        if (rc->entries[whichEntry]->testState == ROUTE_TEST_IDLE)
        {
            if (src == MTP3_STM_TSRC)
            {
                ...
                SNMM_HC0_SET_ANSI(snmm, SNMM_HC0_RSM_ANSI);
                if (rc->status == DESTINATION_UNAVAILABLE)
                {
                    if (MTP3_PC_IS_CLUSTER_ANSI(tpc))
                    {
#if MTP3_SUPPORTS_CLUSTERS
                        SNMM_HC1_SET_ANSI(snmm, SNMM_HC1_RCP_ANSI);
#else
                        SNMM_HC1_SET_ANSI(snmm, SNMM_HC1_RSP_ANSI);
#endif
                    }
                    else
                    {
                ...
                else if (rc->status == DESTINATION_RESTRICTED)
                {
                    ...
                    {
#if MTP3_SUPPORTS_CLUSTERS
                        SNMM_HC1_SET_ANSI(snmm, SNMM_HC1_RCR_ANSI);
#else
                        SNMM_HC1_SET_ANSI(snmm, SNMM_HC1_RSR_ANSI);
#endif
                    }
                    else
                    {
                ...
                SNMM_ROUTE_SET_TEST_SET_DEST_ANSI(snmm.data.routeSetTest, pc1);
                ...
                if (rc)
                {
                    PEG_IncrPeg(&rc->MTP3_DPC_PEGS, PEG_MTP3_DPC_RST_TX);
                }

                /* USE any available route */
                ret = HMRT_Main_ANSI(MTP3_SRM_RSRT,
                                     SMH_TRIGGER_ROUTE_MGMT_MSG,
                                     ev.data, ev.len,
                                     ITS_SS7_DEFAULT_LINK_SET,
                                     ITS_SS7_DEFAULT_LINK_CODE);
    ...
    case SRM_TRIGGER_T10:                    /* from timer control */
        ...
        rc1 = ROUTE_FindDestination(MTP3_RL_GET_DPC_VALUE(omtp3.label),
                                                            ni, FAMILY_ANSI);
        if (rc1)
        {
            PEG_IncrPeg(&rc1->MTP3_DPC_PEGS, PEG_MTP3_DPC_RST_TX);
        }
    ...
    case SRM_TRIGGER_ROUTE_SET_TEST:         /* from HMDT */
        ...
        if (rc)
        {
            PEG_IncrPeg(&rc->MTP3_DPC_PEGS, PEG_MTP3_DPC_RST_RX);
        }
        ...
        if (STPFunction_ANSI())
        {
            if (!SPRestarting_ANSI())
            {
                ...
                pc1 = SNMM_ROUTE_SET_TEST_GET_DEST_ANSI(isnmm->data.routeSetTest);
                ni = MTP3_HDR_GET_SIO(*mtp3) & MTP3_NIC_MASK;
                                              ...
                                              ITS_FALSE)) == NULL)
                {
		            ...
		            MTP3_Alarm_ANSI(2056, __FILE__, __LINE__, "pc=%x:family=%s",pc2, "ANSI");
    ...
    case SRM_TRIGGER_CLUSTER_ROUTE_SET_TEST:/* from HMDT */
        /* I gather these are treated the same? */
        if (!STPFunction_ANSI())
        {
            if (SPRestarting_ANSI())
            {
            ...
            else
            {
                ...
                SNMM_Encode_ANSI(&ev, &omtp3, isnmm);
                
                rc->entries[whichEntry]->testState = ROUTE_TEST_WAIT_5;
    ...
    case SRM_TRIGGER_ACCESSIBILITY_DATA:     /* from TSRC */
        ...
        if (rc->entries[whichEntry]->testState == ROUTE_TEST_WAIT_2)
        {
             ...
             * passed in the accessibility request.  The message body
            ...
            mtp3 = (MTP3_HEADER_ANSI *)&sif[2 * sizeof(ITS_OCTET) +
                                            sizeof(MTP3_HEADER)];
            pc2 = MTP3_RL_GET_OPC_VALUE(mtp3->label);
                      ...
                      onlyThroughY))
            {
                ...
                ret = RTPC_Main_ANSI(MTP3_SRM_RSRT,
                     ...
                     ri && ri->priority != 0)
            {
                ...
                ret = RTRC_Main_ANSI(MTP3_SRM_RSRT,
            ...
            else
            {
                ...
                ret = RTAC_Main_ANSI(MTP3_SRM_RSRT,
        ...
        else if (rc->entries[whichEntry]->testState == ROUTE_TEST_WAIT_5)
        {
            ...
            if ((rc->status == DESTINATION_UNAVAILABLE &&
                 hc1 == SNMM_HC1_RCP_ANSI) ||
                (rc->status == DESTINATION_RESTRICTED &&
                 hc1 == SNMM_HC1_RCR_ANSI))
            {
                return (ITS_SUCCESS);
            }
 ...
 * -----------------------------------------------------------------------------
 * manish   05-05-2005   Bug# 1808 (propagation from WSC)               
 *
 *
 *
 ****************************************************************************/
int
RTRC_Main_ANSI(MTP3_SubSystems src, SRM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    MTP3_HEADER_ANSI *mtp3 = (MTP3_HEADER_ANSI *)&sif[1];
    ...
    MTP3_TRACE_ENTRY_ANSI(MTP3_SRM_RTRC, src, trigger, sif, len,
                          linkSet, linkCode);
    ...
    case SRM_TRIGGER_TRANSFER_RESTRICT:      /* from HMDT */
        ...
        if (rc)
        {
            PEG_IncrPeg(&rc->MTP3_DPC_PEGS, PEG_MTP3_DPC_TFR_RX);
        }
        ...
        if (linkSet == ITS_SS7_DEFAULT_LINK_SET)
        {
            ...
            MTP3_Alarm_ANSI(2059, __FILE__, __LINE__, "pc=%x:family=%s",pc2, "ANSI");
        ...
        {
            /* Return error, No route is confogured on this linkset
             * Fix propagated from WSC #1808 */
            if (whichEntry == -1)
            {
                MTP3_ERROR((" TFR received no route found PC = %d, \
                             LinkSet = %d\n", rinfo.basic.dpc, rinfo.linkSet));
    ...
    case SRM_TRIGGER_MSG_FOR_RESTRICTED_CLUST:
        /* only STPs perform this action */
        if (!STPFunction_ANSI())
        {
            return (ITS_SUCCESS);
        ...
        pc1 = MTP3_RL_GET_DPC_VALUE_ANSI(mtp3->label);
        ni = MTP3_HDR_GET_SIO(*mtp3) & MTP3_NIC_MASK;
        ...
        if (!(rc->entries[whichEntry]->flags & ROUTE_SENT_TFR_TWICE))
        {
            ...
            else
            {
                ...
                SNMM_HC0_SET_ANSI(snmm, SNMM_HC0_TFM_ANSI);
                if (trigger == SRM_TRIGGER_MSG_FOR_RESTRICTED_DEST)
                {
                    SNMM_HC1_SET_ANSI(snmm, SNMM_HC1_TFR_ANSI);
                }
                else
                {
#if MTP3_SUPPORTS_CLUSTERS
                    SNMM_HC1_SET_ANSI(snmm, SNMM_HC1_TCR_ANSI);
#else
                    SNMM_HC1_SET_ANSI(snmm, SNMM_HC1_TFR_ANSI);
#endif
                }
                ...
                ret = HMRT_Main_ANSI(MTP3_SRM_RTRC,
                                     SMH_TRIGGER_ROUTE_MGMT_MSG,
                                     ev.data, ev.len,
                                     ITS_SS7_DEFAULT_LINK_SET,
                                     ITS_SS7_DEFAULT_LINK_CODE);
    ...
    case SRM_TRIGGER_SEND_TRANSFER_RESTRICT: /* from RSRT: from TSRC */
        ...
        if ((ls = LINKSET_FindLinkSet(linkSet)) == NULL)
        {
            ...
            MTP3_Alarm_ANSI(2082, __FILE__, __LINE__, "ls=%x:family=%s", linkSet, "ANSI");
    ...
    case SRM_TRIGGER_CLUSTER_RESTRICT:       /* from TSRC */
    case SRM_TRIGGER_SEND_TCR:               /* from RSRT */
        /* only STPs perform this action */
        if (!STPFunction_ANSI())
        {
            return (ITS_SUCCESS);
        ...
        /*
         * get the routing info for this destination.
         * DONT USE the linkSet.  Use the normal route (the destination
         * is restricted).
         */
        if (ROUTE_ParseRouteMatchInfoArgs(&rinfo, pc1,
                                          REMOTE_ROUTE, ROUTE_DPC_NI,
    ...
    case SRM_TRIGGER_REM_STP_FOR_CLUST:     /* from TSRC */
        ...
        /*
         * DON'T CARE what the linkSet is.  TSRC has to get this right
         */
        pc1 = MTP3_RL_GET_DPC_VALUE(mtp3->label);
        ni = MTP3_HDR_GET_SIO(*mtp3) & MTP3_NIC_MASK;
 ...
 ****************************************************************************/
int
RTCC_Main_ANSI(MTP3_SubSystems src, SRM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    MTP3_HEADER_ANSI *mtp3 = (MTP3_HEADER_ANSI *)&sif[1];
    ...
    MTP3_TRACE_ENTRY_ANSI(MTP3_SRM_RTCC, src, trigger, sif, len,
                          linkSet, linkCode);
    ...
    case SRM_TRIGGER_TRANSFER_CONTROL:       /* from HMDT */
        ...
        PEG_IncrPeg(ANSI_MTP3_Pegs, PEG_MTP3_TFC_RECEIVED);
        ...
        if ((rc = ROUTE_FindBestMatch(&rinfo, &whichEntry,
                                      ...
                                      ITS_FALSE)) != NULL)
        {
            ret = TRCC_Main_ANSI(MTP3_SRM_RTCC,
        ...
        else
        {
            ...
            MTP3_Alarm_ANSI(2077, __FILE__, __LINE__, "pc=%x:ni=%x:family=%s",pc1,ni, "ANSI");
        }
        break;

    case SRM_TRIGGER_USER_PART_UNAVAIL:      /* from HMDT */
        return TSFC_Main_ANSI(MTP3_SRM_RTCC,
                              STM_TRIGGER_UPU,
                              sif, len,
                              linkSet, linkCode);
    ...
    case SRM_TRIGGER_SEND_TRANSFER_CONTROL:  /* from HMCG */
        ...
        if (isLocal)
        {
            ...
            ret = TSFC_Main_ANSI(MTP3_SRM_RTCC,
                                 STM_TRIGGER_DEST_CONGESTION_STATUS,
                                 ev.data, ev.len,
                                 ITS_SS7_DEFAULT_LINK_SET,
                                 ITS_SS7_DEFAULT_LINK_CODE);
        ...
        else
        {
                                          ...
                                          ITS_FALSE)) != NULL)
            {
                ...
                SNMM_HC0_SET_ANSI(snmm, SNMM_HC0_FCM_ANSI);
                SNMM_HC1_SET_ANSI(snmm, SNMM_HC1_TFC_ANSI);
                SNMM_TRANSFER_CONTROL_SET_DEST_ANSI(snmm.data.transferControl,
                                                    pc2);
                SNMM_TRANSFER_CONTROL_SET_STATUS_ANSI(snmm.data.transferControl,
                                                      congLevel);
                ...
                if (rc)
                {
                    PEG_IncrPeg(&rc->MTP3_DPC_PEGS, PEG_MTP3_DPC_TFC_TX);
                }
 ...
 ****************************************************************************/
int
RCAT_Main_ANSI(MTP3_SubSystems src, SRM_Triggers trigger,
               ITS_OCTET *sif, ITS_USHORT len,
               ITS_OCTET linkSet, ITS_OCTET linkCode)
{
    MTP3_HEADER_ANSI *mtp3 = (MTP3_HEADER_ANSI *)&sif[1];
    ...
    MTP3_TRACE_ENTRY_ANSI(MTP3_SRM_RCAT, src, trigger, sif, len,
                          linkSet, linkCode);
    ...
    case SRM_TRIGGER_START_CONG_TEST:        /* from TRCC */
                                      ...
                                      ITS_FALSE)) != NULL)
        {
            ...
            SNMM_HC0_SET_ANSI(snmm, SNMM_HC0_FCM_ANSI);
            SNMM_HC1_SET_ANSI(snmm, SNMM_HC1_RCT_ANSI);

            ITS_EVENT_INIT(&ev, ITS_MTP3_ANSI_SRC, 0);
            SNMM_Encode_ANSI(&ev, &omtp3, &snmm);

            /* send via any route */
            ret = HMRT_Main_ANSI(MTP3_SRM_RCAT,
                                 SMH_TRIGGER_ROUTE_MGMT_MSG,
                                 ev.data, ev.len,
                                 ITS_SS7_DEFAULT_LINK_SET,
                                 ITS_SS7_DEFAULT_LINK_CODE);
    ...
    case SRM_TRIGGER_T16:                    /* from timer control */
        ...
        {
            /* 
             * Signalling information is passed accoss subsystem is more than one
             * way. 1) the original SIF 2) ITS_EVENT here with just the DPC of  the
             * MTP3 HDR containing the convcerned point code. The issue is later on 
             * the call stack TSFC and TRCC expect the original SIF. So the SNMM info
             * is lost. We are adding it back here and other places.
             */
            SNMM_MESSAGE snmm;
            SNMM_TRANSFER_CONTROL_SET_DEST_ANSI(snmm.data.transferControl, pc);
            SNMM_TRANSFER_CONTROL_SET_STATUS_ANSI(snmm.data.transferControl, 
                                           rc->entries[whichEntry]->congLevel);
            ...
            ret = TRCC_Main_ANSI(MTP3_SRM_RCAT,
