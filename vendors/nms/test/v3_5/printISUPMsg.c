 ...
 ****************************************************************************
 *
 *  ID: $Id: printISUPMsg.c,v 9.1 2005/03/23 12:56:18 cvsadmin Exp $
 *
...
int
ReceiveISUPMsg(ITS_HANDLE handle, ITS_EVENT *event)
{
    int          ieCount = 0;
    int          trunkGrp = 0;
    int          circuitNum = 0;
    ...
    {
        printf("test: event not MTP3_MSG_USER_DATA. Error!!\n");
     ...
     *     1   sizeof(^)      2        len
     */
    offset = sizeof(ITS_OCTET) + sizeof(MTP3_HEADER) - sizeof(ITS_OCTET);
    CIC = (event->data[offset] >> 4) & 0x000F;
    offset++;
    CIC |= ((event->data[offset] << 4) & 0x0FF0);
    offset++;
    CIC |= ((event->data[offset] << 12) & 0xF000);

    printf("test: CIC = 0x%04x\n", CIC);
    ...
    if (ISUP_DecodeMTP3(ies, &ieCount, event, &desc) != ITS_SUCCESS)
    {
        printf("test: ISUP_DecodeMTP3 failed. Exit\n");
    }
...
#endif
    trunkGrp = CIC / circuitsPerTrunkGrp;
    circuitNum = CIC % circuitsPerTrunkGrp;
