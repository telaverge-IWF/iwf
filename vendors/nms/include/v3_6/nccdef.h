...
/*-----------------------------------------------------------------------------
  NCC Value Codes.
    ...
    - NCC protocol implementers can define their own values, but only within 
      a 8-bit range supplied by NMS. See the NCC Protocol Implementer's Guide
      for details.
...
#define NCC_BILLINGSET_DEFAULT                    (0xffff)
                              /* call is free */
#define NCC_BILLINGSET_FREE                       (0x0)
...
#define NCC_DIS_CONGESTION                        (0x0043)
                              /* Dial had a failure */
...
#define NCC_DIS_SIGNAL_UNASSIGNED_NUMBER          (0x0053)
...
/* ---------------------------------------- */ 
/* NCCEVN_CALL_STATUS_UPDATE values */ 
/* ---------------------------------------- */ 
                              /* Calling address updated (caller ID info.) */
#define NCC_CALL_STATUS_CALLINGADDR               (0x0080)
...
/* ---------------------------------------- */ 
/* NCCEVN_EXTENDED_CALL_STATUS_UPDATE values */ 
/* ---------------------------------------- */ 
                              /* Automatic tranfer failed. */
#define NCC_EXTENDED_CALL_STATUS_CATEGORY         (0x00E0)
...
#define NCC_CALLSTATE_QTY                  (NCC_CALLSTATE_PROCEEDING+1)
...
typedef struct
{
    ...
    char           callingname [NCC_MAX_CALLING_NAME];
    DWORD          pendingcmd;
    DWORD          held;
    DWORD          direction;
    CTAHD          linehd;
