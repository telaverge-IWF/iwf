...
#define MF_SIZE_EXTTKN  255        /* extended token string size */

#define NUM_EXT_ELMTS    8        /* Number of extended tokens supported */
#define MAX_RAW_LEN     272       /* maximum length of a raw message */
...
typedef struct _tknStr             /* token string */
{
   ...
   U8   len;                      /* length */
   U16  spare1;                   /* for alignment */
   U8   val[(MF_SIZE_TKNSTR + 4) & 0xffc]; /* string value - 4 byte aligned */
} TknStr;

/* Does this really need to be here? */
typedef struct tknExt             /* token extended */
{
   U8   pres;                     /* present flag */
   U8   type;                     /* type */
   U8   len;                      /* length */
   U8   spare1;                   /* spare */
   U8   val[(MF_SIZE_EXTTKN + 4) & 0xffc];      /* string value - 4 byte aligned */
} TknExt;
...
typedef struct _siCnStEvnt       /* Connect Status Event */
{
   ...
   SiCarrierInfoTrans   carrierInfoTrans;   /* carrier information transfer */
   SiOutgTrkGrpNum      outgTrkGrpNum;      /* outgoing trunk group number  */
   SiElementExt         elementExt[NUM_EXT_ELMTS];  /* extended elements */
} SiCnStEvnt;
...
typedef struct _siRawEvnt           /* Raw event */
{
   U16              length;         /* Amount of data in the event */
   U16              spare1;
   U8               data[(MAX_RAW_LEN + 4) & 0xffc]; /* The raw data - 4 byte aligned */
} SiRawEvnt;
