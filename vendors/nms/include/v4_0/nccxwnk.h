...
{                                    
    ...
    WORD    anididtone;              /*15 For DTMF, specifies tone between    */
                                     /*   DID and ANI digits. (0x2a = '*')    */
                                     /*   For MF, '#' pauses between spills,  */
                                     /*   '*' waits for wink between spills,  */
                                     /*   '@' waits for answer between spills.*/
    WORD    endofdigitstone;         /*16 tone that specifies the end of      */
                                     /*   digits. set to 0x23 for '#'         */
    WORD    winktime;                /*17 wink duration (inbound call)        */
    ...
    WORD    winkwaittime;            /*21 max wait for wink (outbound call)   */
                                     /*   0 = no wink, immediate start        */
                                     /*   1 = no wink, but wait for dial tone */
    WORD    minwinktime;             /*22 min wink duration (outbound call)   */
    WORD    maxwinktime;             /*23 max wink duration (outbound call)   */
    ...
    WORD    MLPP;                    /*38 set to 0                            */
    WORD    metering_enabled;        /*39 1 = metering pulses enabled         */
}    NCC_ADI_WNK_PARMS;              
