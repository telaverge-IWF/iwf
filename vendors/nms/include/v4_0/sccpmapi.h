...
#define SCCP_MAXALTOPC  8       /* Max Alt OPC Point Codes per Dpc            */
#define SCCP_LENADDR    32      /* SCCP Address Name/Mask Length              */
#define SCCP_MAXSSNS    8       /* Max Subsystems allowed per Dpc             */
#define SCCP_MAXMASKS   4       /* Max number of Address Masks                */
...
typedef struct  {
    ...
    U16 dummy;                  /* dummy                                      */
    U32 usapOpc;                /* OPCs specified for the usap                */
...
typedef struct {
    ...
    U16 numAltOpc;              /* number of alternate point codes            */
    UsapOpcInfo usapAltOpc[SCCP_MAXALTOPC];  /* multiple OPC information      */
    U32 bkupPc;                 /* backup point code                          */
    SccpAddrMask addrMsk[SCCP_MAXMASKS];  /* address mask for outgoing pkts on this SAP */
...
typedef struct {
    ...
    SccpAddrMask addrMsk[SCCP_MAXMASKS];       /* address mask for incoming pkts on this SAP */
    MemoryId mem;               /* memory region & pool id                    */
...
typedef struct {
    ...
    U8  status;                 /* status                                     */
    U8  bkupPcInd;              /* backup point code indicator (0=none)       */
    U8  upOnResume;             /* subsystem is up on an MTP Resume           */
    U16 numConPc;               /* number of concerned point codes            */
    U32 bkupPc;                 /* backup point code                          */
 ...
 * -------------------------------------------------------------------------- */
typedef struct {
    U32 rteOpc;                 /* OPCs for this route                       */
    U8  status;                 /* status of the route for this OPC,
                                 * adjacent/translator flags                 */
    U8  align1;
    U16 align2;
}RouteOpcInfo;
...
typedef struct {
    ...
    U16 numAltOpc;              /* number of alternate point codes            */
    U16 spare4;
    RouteOpcInfo rteAltOpc[SCCP_MAXALTOPC];
    U32 bkupPc;                 /* backup point code                          */
...
typedef struct {
    ...
    U16 numAltOpc;               /* number of OPCs specified */
