...
*-----------------------------------------------------------------------------
* $Log: tcapdef.h,v $
* Revision 1.1.2.1  2010/04/15 05:12:32  ssingh
* added NMS 4K support includes for vendor abstraction layer.
* $NoKeywords: $
*
...
* Changed TCAP_MSG_SIZE from 512 to 674 . This is the minimum buffer size
...
/****************************************************************************/
/* TRP - add for use by applications */
#define TCAP_SVCID 0x25
...
#define TCAPEVN_CONGEST ((TCAP_SVCID << 16) | 0x2000 | 0x02)     /* 0x252002   */
...
#define TCAP_REASON_SCCP_SPID_RANGE ((TCAP_SVCID << 16) | 0x1000 | 0x03)      /* 0x251003 SCCP SpId out of range */
#define TCAP_REASON_SCCP_SPID_UNDEF ((TCAP_SVCID << 16) | 0x1000 | 0x04)      /* 0x251004 SCCP SpId not configured */
#define TCAP_REASON_SCCP_NO_NSAP    ((TCAP_SVCID << 16) | 0x1000 | 0x05)      /* 0x251005 SCCP NSAP not configured or not bound */
 ...
 */
/* TRP modified MSG_SIZE from 512 to 674  */
/* TRP modified MSG_SIZE from 674 to 1074  */
#define TCAP_MSG_SIZE      1074
...
{
    U8 dlgType;                 /* type of message this dialog is for...     ITU9x */
...
#define TCAP_DLG_RSD_NCDLG  2   /* result source diagnostic: provider:
    ...
    U8 abrtSrc;                 /* source of abort                           ITU9x */
#define TCAP_DLG_USR_ABRT   0   /* Service user                                    */
#define TCAP_DLG_PRV_ABRT   1   /* Service provider                                */
    U8 fill;                    /* fill for alignment                              */
    U8 fill2;                   /* fill for alignment                              */
    TcapOctetStr apConName;     /* application context name         ITU9x & ANSI96 */

    TcapOctetStr secConInfo;    /* security context information             ANSI96 */
    TcapOctetStr confAlgId;     /* confidentiality algoritm ID              ANSI96 */
    TcapOctetStr confValue;     /* confidentiality value                    ANSI96 */

    U8 apConNameType;           /* security context information type        ANSI96 */
    U8 secConInfoType;          /* confidentiality algoritm ID type         ANSI96 */
    U8 confAlgIdType;           /* confidentiality value type               ANSI96 */
#define TCAP_DLGP_INTID   0x01  /* integer ID type                                 */
#define TCAP_DLGP_OBJID   0x02  /* object  ID type                                 */
    U8 confValueId;             /* confidentiality value identifier         ANSI96 */
 ...
 *
 * Note: the status field may take on one of the following values
 * which are defined in <sccpdef.h>:
 *  SP_ACC           0x00   Signalling Point Accessible
 *  SP_INACC         0x01   Signalling Point Inaccessible (all connections dropped)
 *  SP_INACC_NODROP  0x06   Signalling Point Inaccessible, Connections not dropped
 *  SP_CONG_OFF      0x07   Congestion ended on affected SP
 *  SP_CONG1         0x10   Signalling Point Congestion Level 1
 *  SP_CONG2         0x11   Signalling Point Congestion Level 2
 *  SP_CONG3         0x12   Signalling Point Congestion Level 3
 */
typedef struct Tcap_PCState_Event
{
    ...
    U8          fill;           /* fill for alignment */
    U8          fill2;          /* fill for alignment */
    U32         opc;            /* originating point code, added for Mult. OPC */
} TcapPCStateEvent;
...
#define TCAP_CONGESTED     5    /* request invalid for current congestion level */
#define TCAP_RES_FAIL      6    /* Insufficient resources to complete request */
...
/* Note: 0xB8 and 0xB9 are reserved for internal use */
...
typedef struct
{
    U32 qCount;             /* number of API messages currently queued to TCAP layer */
    U32 qPeak;              /* max number of API messages ever queued to TCAP layer */
    U32 txPending;          /* current number of outstanding transmit rqsts to TCAP layer */
    U32 txPendPeak;         /* max number of transmit rqsts ever outstanding to TCAP layer */
    ...
    U8  tcapCongLvl;        /* current TCAP layer congestion level [0..3] */
    U8  tcapCongSrc;        /* reason for TCAP layer congestion */
    U8  spare1;             /* spare for alignment */
} TCAPAPISTATS;

/* TCAP layer congestion source values */
#define TCAP_CONG_LOWMEM    1           /* low memory in TCAP layer */
#define TCAP_CONG_QTHRESH   2           /* TCAP outbound queue threshold exceeded */
...
DWORD NMSAPI TCAPGetApiStats    (CTAHD ctahd, TCAPAPISTATS *pStats, U8 reset);
