...
#define MAX_WAIT_OBJECTS 16       /* max # of wait objects for ctaRegisterWaitObject */
...
#define CTAEVN_TRACE_MESSAGE               0x10002104
...
typedef struct _CTA_SERVICE_NAME_BUF
{
    char svcname[CTA_MAXSVCNAME_LEN + 1];      /* service name (eg. "ADI", "SWI" ...)  */
    char svcmgrname[CTA_MAXSVCNAME_LEN + 4];   /* service manager providing service    */
...
#define CTA_FMT_BYTE        6
#define CTA_FMT_INT8        7
...
#define CTA_UNITS_SECONDS  11
#define CTA_UNITS_CHAR     12
...
#define CTA_TRACETAG_API_COMMAND       0x0000400c /* not for application use */
    /*
...
/*
   This fuctions will be used for multi-box CSCTA.
 */

/*****************************************************************************
 *                          Extended CTA API for Client/Server mode
 *****************************************************************************/
DWORD NMSAPI
ctaGetTextEx(
             /* Formats CT Access codes                */
...
ctaFormatEventEx(
                 /* Formats CT Access events               */
                 CTAHD      ctahd,            /* CTA Server address                     */
                 char      *lineprefix,       /* string placed at start of each line    */
                 CTA_EVENT *event,            /* event to format                        */
                 char      *buffer,           /* returned formatted text string         */
                 unsigned   size);            /* size of buffer                         */
...
ctaGetServiceVersionEx(
                       /* Get version, build date,compat level
                          of an initialized service.            */
                       CTAHD             ctahd,      /* CTA Server address                     */
                       CTA_SERVICE_NAME *svcname,    /* Name of service to retrieve version inf*/
                       CTA_REV_INFO     *info,       /* Reference to store service version info*/
                       DWORD             size );     /* Size of reference buffer.              */
...
typedef  struct
{
    ...
    char       contextname[CTA_CONTEXT_NAME_LEN];
                                         /* context name used in tracing    */
    char      *svcnames;                 /* returned service names divided
                                            by 0, extra 0 at the end        */
    unsigned   size;                     /* sizeof svcnames buffer          */
} CTA_CONTEXT_INFOEX;
...
ctaGetContextInfoEx(
                    ...
                    CTAHD               ctahd,/*   CT Access context handle             */
                    CTA_CONTEXT_INFOEX *info);/*    pointer to context information
                                                    structure                           */
...
#define CTA_CONTEXT_DECLARED_ACCESS     0x1
/*  declared access mode                  */
/*  otherwise - common access mode        */

#define CTA_CONTEXT_PERSISTENT          0x2
...
/*****************************************************************************
 * Detaches from an existing service object
 *****************************************************************************/
DWORD NMSAPI
ctaDetachObject(unsigned  objhd);             /* object handle                          */
...
/*****************************************************************************
 * Set's server trace mask
 *****************************************************************************/
DWORD NMSAPI
ctaSetGlobalTraceMask(
                      CTAHD             hd,           /* CTA handle: context handle */
                      unsigned          tracemask);   /* new tracing mask           */

/*****************************************************************************
 * Shutdowns server or server's host
 *****************************************************************************/

/* ctaShutdown flags */
#define CTA_SERVER_SHUTDOWN     0x01
#define CTA_SYSTEM_SHUTDOWN     0x02
#define CTA_SYSTEM_REBOOT       0x03
#define CTA_SERVER_RESTART      0x04
...
DWORD NMSAPI ctaQueryServerContexts (
                    CTAHD               ctahd,
