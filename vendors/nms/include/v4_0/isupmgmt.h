 ...
 * preprocessor constant "_TX_ISUP_SOURCE_"; this is only for use
...
/* API return values */
#define ISUP_MGMT_SUCCESS       0           /* API call was successful          */
#define ISUP_MGMT_OVERRUN       0x1001      /* data length too large to send    */
#define ISUP_MGMT_UNDERRUN      0x1002      /* received message too large       */
#define ISUP_MGMT_UNBOUND       0x1003      /* not bound to specified board     */
#define ISUP_MGMT_BOARD         0x1004      /* board number out of range        */
#define ISUP_MGMT_DRIVER        0x1005      /* CPI driver returned error        */
#define ISUP_MGMT_NOMSG         0x1006      /* No event messages waiting        */
#define ISUP_MGMT_NOTBOUND      0x1007      /* User failed to call              */
#define ISUP_MGMT_FAILED        0x1008      /* request failed                   */
...
#define ISUP_CIR_INCOMING   0
#define ISUP_CIR_OUTGOING   1
#define ISUP_CIR_BOTHWAY    2

/* Circuit Types */

#define ISUP_CTL_NONE       0
#define ISUP_CTL_ALL        1
#define ISUP_CTL_ODDEVEN    2

/* Bearer Profiles */

#define ISUP_PROF_VOICE      0
...
/* Switch types - defines protocol behavior */

#define ISUP_SW_ITU         1               /* ITU-T - White Book */
#define ISUP_SW_ITUWHITE    1               /* ITU-T - White Book */
#define ISUP_SW_ANS88       2               /* ANSI-1988 operation */
#define ISUP_SW_ANS92       3               /* ANSI-1992 operation */
#define ISUP_SW_ITUBLUE     4               /* ITU-T - Blue Book */
#define ISUP_SW_ANS95       5               /* ANSI-1995 operation */
#define ISUP_SW_Q767        6               /* ITU-T - Q.767 */
#define ISUP_SW_JNTT        7               /* NTT */
#define ISUP_SW_ITU97       8               /* switch - ITU 1997 */
#define ISUP_SW_ETSIV2      9               /* switch - ETSI V2 */
#define ISUP_SW_ETSIV3     10               /* switch - ETSI V3 */
#define ISUP_SW_JTTC       11               /* switch - Japan TTC */

/* Subservice Types */

#define ISUP_SSF_INTL   0
#define ISUP_SSF_SPARE  1
#define ISUP_SSF_NAT 	2
#define ISUP_SSF_RES    3
...
#define ISUP_ADDRIND_NAT   0x00        /* National address indicator       */
#define ISUP_ADDRIND_INT   0x01        /* International address indicator  */

/* encoding scheme values */
#define ES_UNKNOWN         0x00        /* Encoding unknown                 */
#define ES_BCD_ODD         0x01        /* BCD, odd number of digits        */
#define ES_BCD_EVN         0x02        /* BCD, even number of digits       */
...
#define NP_EXTN             0x0F        /* reserved for extension           */
...
typedef struct _IsupAddr        /* ISUP address (global title digits) */
{
   ...
   U8 fill1;                    /* alignment */
   U16 fill2;                   /* alignment */
   U8 string[ISUPADDRLEN];      /* address value - ASCII hex digits - 4 byte aligned */
} IsupAddr;
...
typedef struct _IsupGenParms
{
  ...
  TimerCfg  tGrpReset;      /* group reset timer */
	PDesc		stkMgr;
  U32     traceFlags;
  U32     txmonTimeout;   /* this value is sent to TXMON during PAUSE, RESUME and other large operations to ask for more time to report */
  U8      igPassAlng;     /* Don't use Pass Along messaging with end to end connection */
  U8      extElmts;       /* Send up unrecognized I.E.s as extended I.E.s */
  U8      rawMsgs;        /* Send up unrecognized messages as raw message indications */
  U8      oneGrpMsg;      /* if true, should react to first group message */
  U8      rmtUsrUnavl;    /* if true, should start appropriate User Part Test procedure */
  U8      grpResetEvent;  /* Send up one group reset event instead of many separate circuit reset events */
  U8      slsFromCICs;    /* set the ANSI SLS value to the bottom bits of the CIC (default TRUE) */
  U8      spare1;         /* spare */
  U8      dsblRmtUsrUnavl;/* if true, should disable appropriate User Part Test procedure - might be SSURN specific */
  U8      restartT7;      /* restart T7 when an ibound INR is received - might be SSURN specific */
...
typedef struct _IsupCircCfg
{
   ...
   U32    altOrgPointCode;        /* alternative origination point code altOPC */
   ...
   U8     ssf;                    /* SSF, if this is non - 0xff, then use it instead */
   U8     fill2;                  /* alignment */
   ...
   TimerCfg t37;                    /* t15 timer - initial unblocking sent */
...
} IsupSapStats;         this structure is not used */
...
#define IDLE              0      /* idle */
#define MGMT_UNEQUIP      1      /* unequipped */
...
/* trace bit values */
#define EVENT_TRACE       1<<0
#define TRACE_DATA        1<<8
#define ENCODE_ERROR      1<<9
#define ENUM_WARNING      1<<10
#define INIT_ELMT_DBG     1<<11
#define INIT_TKN_DBG      1<<12
#define TMR_TRACE         1<<13
...
S16  TXISUPMGMTFUNC isupTraceControl(U8 board, Bool onOff, U32 flags);
...
S16  TXISUPMGMTFUNC isupNSapStatsEx(U8 board, S16 sapId, IsupNSapStats *stats, U8 reset);
S16  TXISUPMGMTFUNC isupCircuitStatsEx(U8 board, U32 circId, IsupCircStats *stats, U8 reset);
...
S16  TXISUPMGMTFUNC isupQuietReset(U8 board);
#ifdef __cplusplus
}
#endif
#endif
