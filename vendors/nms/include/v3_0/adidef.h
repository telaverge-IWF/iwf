...
#define ADI_CC_ERR_FALSE_SEIZURE    1 /* DID:Seized, then gone before digits*/
                                      /* LPS:Ring stopped b4 inc-call event */
#define ADI_CC_ERR_EXTRA_DIGITS     2 /* DID:More digits rec'd than expected*/
#define ADI_CC_ERR_PREMATURE_ANSWER 3 /* OGT:Answer before all digits given */
#define ADI_CC_ERR_DIGIT_TIMEOUT    4 /* timeout in digit reception         */
#define ADI_CC_ERR_INVALID_DIGIT    5 /* invalid compelled digit received   */
#define ADI_CC_ERR_BAD_DIALSTRING   6 /* invalid formatting of dial string  */
#define ADI_CC_ERR_NO_CS_RESOURCE   7 /* call setup resource unavailable    */
#define ADI_CC_ERR_NOT_SUPPORTED    8 /* generic: last command unsupported  */
...
#define        ADI_STA_PARMID  (ADI_BASEID + 19)
#define        ADI_LPS_PARMID  (ADI_BASEID + 20)
#define        ADI_WNK_PARMID  (ADI_BASEID + 21)
#define        ADI_OGT_PARMID  (ADI_BASEID + 22)
#define        ADI_DID_PARMID  (ADI_BASEID + 23)
#define        ADI_FDI_PARMID  (ADI_BASEID + 26)
#define        ADI_GST_PARMID  (ADI_BASEID + 27)
...
/* The following parameters are specific to their respective protocol.
 * In addition, every TCP receives parameters from ADI_START_PARMS and
 * ADI_PLACECALL_PARMS.
 */

/*------ WNK PARMS --(2-way wink start)-------------------------------------*/

typedef struct                    /* passed with adiStartProtocol("WNK#")   */
{                                 /*  for all wink-start family of protocols*/
    DWORD size;                   /*  Size of this structure.               */
    ...
    WORD releaseguardtime;        /*   (ms) duration before next call       */
                                  /*  inbound only:                         */
    WORD dtmfmask;                /*   Mask for behavior of # and * tones   */
                                  /*     0x1 = on #, send up incoming call  */
                                  /*     0x2 = * splits CID info            */
                                  /*     0x4 = ANI precedes * (def = DNIS)  */
    WORD maxresourcetime;         /*   (ms) max time to wait for rsrcs      */
    WORD noresourcemode;          /*   Behavior if no resource is available */
                                  /*     0 = send error, do nothing         */
                                  /*     1 = generate wink, send error      */
    WORD xfersupport;             /*   indicates if PBX xfer supported      */
    BYTE xferstring[6];           /*   default xfer string, = "!;"          */
    BYTE connstring[6];           /*   default reconnect string, = "!"      */
} ADI_WNK_PARMS;


/*------ FDI PARMS ---(Feature Group D Inbound)-----------------------------*/

typedef struct                    /* passed with adiStartProtocol("FDI#")   */
    {                             /*  for all wink-start family of protocols*/
    DWORD size;                   /*  Size of this structure.               */
                                  /*  inbound and outbound:                 */
    WORD qualaddron;              /*   (ms) qual dur. ON during addressing  */
    ...
    WORD waitfordigitstime;       /*   (ms) waiting until N-th digit comes  */
                                  /*                                        */
    WORD winkwaittime;            /*   not used                             */
    WORD minwinktime;             /*   not used                             */
    WORD maxwinktime;             /*   not used                             */
    WORD predialtime;             /*   not used                             */
    WORD mfkpstontime;            /*   not used                             */
    WORD mfkpstofftime;           /*   not used                             */
    WORD mfkpstampl;              /*   not used                             */
    WORD releaseguardtime;        /*   not used                             */
    WORD preanswertime;           /*   (ms) duration before answering call  */
    WORD maxresourcetime;         /*   (ms) max time to wait for rsrcs      */
    WORD noresourcemode;          /*   Behavior if no resource is available */
                                  /*     0 = send error, do nothing         */
                                  /*     1 = generate wink, send error      */
...
/*------ OGT PARMS ------(Outgoing Trunk Wink Start)------------------------*/

typedef struct                    /* passed with adiStartProtocol("OGT#")   */
{                                 /*  for all wink-start family of protocols*/
    DWORD size;                   /*  Size of this structure.               */
                                  /*  inbound and outbound:                 */
    WORD qualaddron;              /*   (ms) qual dur. ON during addressing  */
...
/*------ DID PARMS ------(Incoming Wink Start)------------------------------*/

typedef struct                    /* passed with adiStartProtocol("DID#")   */
{                                 /*  for all wink-start family of protocols*/
    DWORD size;                   /*  Size of this structure.               */
                                  /*  inbound and outbound:                 */
    WORD qualaddron;              /*   (ms) qual dur. ON during addressing  */
...
/*------ LPS PARMS -----(Loop Start) ---------------------------------------*/

typedef struct                    /* passed with adiStartProtocol("LPS#")   */
{
    DWORD size;                   /*  Size of this structure.               */
    WORD cleardownflag;           /* 0 1 = cleardown enabled, 0 = disabled. */
    WORD qualloopoff;             /* 1 (ms) Qualification time of loop-off. */
    WORD ringdetectmode;          /* 2 0=Level sens 1=Freq sens             */
    WORD qualringon;              /* 3 (ms) qualification time of ring begin*/
    WORD qualringoff;             /* 4 (ms) qualification time of ring end  */
    WORD freqringlow;             /* 5 low  freq cut-off in Hz (1 to 250)   */
    WORD freqringhigh;            /* 6 high freq cut-off in Hz (1 to 250)   */
    WORD freqringperiods;         /* 7 candidate periods avg'd to qual.(1-8)*/
    WORD freqdeglitchtime;        /* 8 periods to deglitch(8=1ms);must be>=2*/
    WORD freqtolerance;           /* 9 percent tolerance (1-100;<50 is good */
    ...
    WORD numrings;                /*13 when to alert user of incoming call: */
                                  /*     0=1st ring begin; 1=1st ring end   */
                                  /*     n= after n rings                   */
...
#define ADILPS_REVERSAL_BOTH    3 /*    reversal indicates answer & hangup  */
    WORD releaseguardtime;        /*20 minimum time between hangup & offhook*/
    WORD CIDtype;                 /*21 the CID type:                        */
#define ADI_CID_BELL 0    /* US (BellCore)    */
#define ADI_CID_NTT  1    /* Japan            */
#define ADI_CID_ETSI 2    /* France           */
    WORD CIDmaxwaittime;          /*22 max time to wait for CID to arrive   */
                                  /*   before concluding the caller hangup  */
    BYTE xferstring[6];           /*23 default xfer string, = "!;"          */
    BYTE connstring[6];           /*26 default reconnect string, = "!"      */
    WORD CIDalertmaxtime;         /*29 max duration of an alert signal in   */
                                  /*   CID protocol. (if CID is enabled)    */
    WORD qualreversal;            /*30 qual time for loop current reversal  */
    WORD polaranswertime;         /*31 time after answering a call, before  */
                                  /*   starting to detect loop current      */
                                  /*   reversal on far-end disconnect       */
    WORD polardialtime;           /*32 time after getting dial tone, before */
                                  /*   reading the loop current polarity    */
    WORD relaytype;               /*33 type of the board's special relay    */
                                  /*   Note: reserved for CT Access 2.1     */
    WORD CIDminmarktime;          /*34 minimum mark time for caller id      */
                                  /*   signal                               */
#define ADILPS_RING_DET_RELAY  0
#define ADILPS_TRANS_LIM_RELAY 1
#define ADILPS_DIALSHUNT_RELAY 2
} ADI_LPS_PARMS;

/*------ GST PARMS -----(Ground Start)--------------------------------------*/

typedef struct                    /* passed with adiStartProtocol("GST#")   */
    {
    DWORD size;                   /*  Size of this structure.               */
    WORD reserved1;               /*   reserved                             */
    WORD qualloopoff;             /*   (ms) Qualification time of loop-off. */
    WORD reserved2;               /*   reserved                             */
    WORD qualringon;              /*   (ms) qualification time of ring begin*/
    WORD qualringoff;             /*   (ms) qualification time of ring end  */
    WORD reserved3;               /*   reserved                             */
    WORD reserved4;               /*   reserved                             */
    WORD reserved5;               /*   reserved                             */
    WORD reserved6;               /*   reserved                             */
    WORD reserved7;               /*   reserved                             */
    WORD ringsigmintime;          /*   inbound minimum ring time (+qual)    */
    WORD ringsigmaxtime;          /*   inbound maximum ring time (+qual)    */
    WORD interringmaxtime;        /*   inbound max between ring-begins      */
    WORD numrings;                /*   when to alert user of incoming call: */
                                  /*     0=1st ring begin; 1=1st ring end   */
                                  /*     n= after n rings                   */
    WORD dialtonewaittime;        /*   maximum wait for dial-tone           */
    WORD dialtonemintime;         /*   minimum dial-tone duration required  */
    WORD nodialtoneaction;        /*   what to do if no dialtone:           */
#define ADILPS_NO_DT_DISCONNECT 0 /*    disconnect (hangup)                 */
#define ADILPS_NO_DT_PROCEED    1 /*    proceed anyways (continue dialing)  */
#define ADILPS_NO_DT_OFFERCALL  2 /*    offer as an incoming call           */
    WORD xfersupport;             /*   indicates if PBX xfer supported      */
    WORD cidsupport;              /*   0 CID disabled ; 1 CID enabled       */
    WORD reserved8;               /*   reserved                             */
    WORD releaseguardtime;        /*   minimum time between hangup & offhook*/
    WORD CIDtype;                 /*   the CID type                         */
    WORD CIDreserved;             /*   timing for Japan protocol            */
    } ADI_GST_PARMS;


/*------ STA PARMS ------(Station)------------------------------*/

typedef struct                    /* passed with adiStartProtocol("STA#")   */
{                                 /*                                        */
    DWORD size;                   /*  Size of this structure.               */
    ...
    WORD defaulttone;             /*   default tone if PC does not respond  */
                                  ...
                                  /*     1 = ringing                        */
                                  /*     2 = busy                           */
    WORD playdialtone;            /*   Behaviour for playing dt on inbound  */
                                  /*     0 = don't play dialtone            */
                                  /*     1 = play dialtone                  */
    WORD releasecalloption;       /*   behavior upon release from Connected */
                                  /*     0 = do nothing                     */
                                  /*     1 = deny battery for loopdroptime  */
                                  /*     2 = play dialtone                  */
                                  /*     3 = play busy                      */
                                  /*     4 = play reorder                   */
    WORD optionflags;             /*   a mask of options:                   */
                                  /*     0x0 = Play reorder when blocking   */
                                  /*     0x1 = Deny battery when blocking   */
    WORD reserved;                /*   reserved                             */
    WORD maxringseconds;          /*   the maximum time to wait for the     */
                                  /*     remote to answer, in seconds       */
                                  /*  network-regulated:                    */
    WORD loopdroptime;            /*   (ms) drop loop current after release */
    WORD cidsupport;              /*   Caller ID support type; disabled     */
    WORD qualaddron;              /*   (ms) qual dur. ON during addressing  */
    WORD qualaddroff;             /*   (ms) qual dur. OFF during addressing */
    WORD qualdisconnect;          /*   (ms) qual dur. of disconnect         */
    WORD qualpermsignal;          /*   (ms) qual dur. of permanent signal   */
                                  /*  Parameters for outbound ring voltage: */
    WORD outring1ontime;          /*   (ms) ontime for the 1st ring cycle   */
    WORD outring1offtime;         /*   (ms) offtime for the 1st ring cycle  */
    WORD outring2ontime;          /*   (ms) ontime for the 2nd ring cycle   */
    WORD outring2offtime;         /*   (ms) offtime for the 2nd ring cycle  */
                                  /*  Parameters for inbound ring tone      */
    WORD ringamp;                 /*   (Internal) Amplitude of ring tone    */
    WORD ringfreql;               /*   (Hz) low frequency of ring tone      */
    WORD ringfreqh;               /*   (Hz) high frequency of ring tone     */
    WORD ringontime;              /*   (ms) ring tone ON time               */
    WORD ringofftime;             /*   (ms) ring tone OFF time              */
                                  /*  Parameters for generated busy tone    */
    WORD busyamp;                 /*   (Internal) Amplitude of busy tone    */
    WORD busyfreql;               /*   (Hz) low frequency of busy tone      */
    WORD busyfreqh;               /*   (Hz) high frequency of busy tone     */
    WORD busyontime;              /*   (ms) busy tone ON time               */
    WORD busyofftime;             /*   (ms) busy tone OFF time              */
                                  /*  Parameters for generated reorder tone */
    WORD reorderamp;              /*   (Internal) Amplitude of reorder tone */
    WORD reorderfreql;            /*   (Hz) low frequency of reorder tone   */
    WORD reorderfreqh;             /*   (Hz) high frequency of reorder tone  */
    WORD reorderontime;           /*   (ms) reorder tone ON                 */
    WORD reorderofftime;          /*   (ms) reorder tone OFF                */
                                  /*  Parameters for generated dialtone     */
    WORD dialtoneamp;             /*   (Internal) Amplitude of dialtone     */
    WORD dialtonefreql;           /*   (Hz) low frequency of dialtone       */
    WORD dialtonefreqh;           /*   (Hz) high frequency of dialtone      */
    WORD dialtoneontime;          /*   (ms) dialtone ON time                */
    WORD dialtoneofftime;         /*   (ms) dialtone OFF time               */
} ADI_STA_PARMS;
