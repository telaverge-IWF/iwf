...
*
*
*    Copyright (c) 1991-1998  Natural MicroSystems Inc.
*****************************************************************************/
...
/*
 * On the OS/2 side, there are two macros to control 16- vs. 32-bit issues.
 * The first is SWPATH32.  This macro should be defined when compiling a
 * switching driver to 32-bit code, when compiling the SwitchPath library
 * to 32-bit code, or when compiling an application that calls a 32-bit
 * switching driver.  Do NOT define SWPATH32 when using a 32-bit compiler
 * to compile an application that calls the 16-bit version of SwitchPath
 * or a 16-bit switching driver.
 *
 * The second macro is DRVINT32.  When this macro is defined, the data
 * structures used in a switching driver use 32-bit integers.  If DRVINT32
 * is not defined, the structures use 16-bit integers.
 *
 * Possible combinations:
 *
 *   SWPATH32 defined and DRVINT32 defined:  a switching driver compiled
 *   to 32-bit code using 32-bit integers in the switching data structures
 *
 *   SWPATH32 undefined and DRVINT32 undefined:  a switching driver compiled
 *   to 16-bit code using 16-bit integers in the switching data structures
 *   (the original "classic" case)
 *
 *   SWPATH32 undefined and DRVINT32 defined:  a switching driver compiled
 *   to 16-bit code using 32-bit integers in the switching data structures
 *
 *   SWPATH32 defined and DRVINT32 undefined:  a switching driver compiled
 *   to 32-bit code using 16-bit integers in the switching data structures.
 *   (It seems unclear why this would be used, since code that currently
 *   loads and calls a 16-bit switching DLL would have to be recompiled
 *   anyway to use a 32-bit switching driver [eliminating the 32-bit to
 *   16-bit thunking].  Such code could then just as easily use the 32-bit
 *   data structures.)
 */
...
/*
 * NOTE:  The following assumes that SWPATH32 implies use of 32-bit data
 *        structures and forces DRVINT32 to be defined.
 */
...
#define DRVINT int              /* On UNIX, this is a 32-bit integer. */
#ifdef SUNDDI
#define DRVENTRY
#endif
  ...
  #pragma pack (4)
    ...
    #ifdef SWPATH32
      #define DRVENTRY _Pascal
    #else
      #define DRVENTRY _Far16 _Pascal
    #endif
    ...
    #ifdef FAR      /* Because OS2DEF.H defines FAR as blank */
      #undef FAR
    #endif
  ...
  #elif defined (DOSMODE)          /* dos-mode */

    #ifndef FAR
      #define FAR  far
    #endif
    #define DRVENTRY _pascal _loadds
    #ifdef DRVINT32
      #define DRVINT long
    #else
      #define DRVINT int
    ...
    #ifdef SWPATH32
      #error Can not compile to 32-bit code with a 16-bit compiler
    #else
      #define DRVENTRY _export far _pascal _loadds
    #endif
...
#if !defined(unix) && !defined (WIN32) && !defined(__QNX__)
