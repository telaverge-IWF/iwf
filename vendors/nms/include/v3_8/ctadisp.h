...
/*
*  Macros to handle a group of IDs owned by a service 
*/

#define CTA_SVCGRP_MASK  0xF000
#define CTA_SVC_GRPID( svcid )    (((svcid) & CTA_SVCGRP_MASK) >> 12 )

/* In future, can add additional service groups to definition. */
#define CTA_IS_SVCGRP( svcid )    (((svcid) & CTA_SVCGRP_MASK) == CTA_OAM_SVCID )
#define CTA_IS_SVCGRP_ID( svcid ) (svcid == CTA_OAM_SVCID)

#define CTA_MAX_GRPS( svcid )     (CTA_SVC_GRPID( svcid ) + 1) 
...
typedef struct _DISP_COMMAND
{
    ...
    UINTPTR  objHd;             /* service object handle (formerly "reserved")*/
...
typedef struct _DISP_MESSAGE
{
    DWORD    id;                /* Event code (and source service id)         */
    CTAHD    ctahd;             /* CT Access context handle                   */
    DWORD    specific[ 3];       /* Fields specific to event or command        */
    void    *specific1[3];       /* Fields specific to event or command        */
    UINTPTR  reserved;          /* must be 0                                  */
    MESSAGE_ADDR addr;          /* Source/Destination service id              */
} DISP_MESSAGE;
...
/* service manager binding functions 
   which can be retrieved with xxxGetFunctionPointer function					*/
#define		XXX_EXIT_MANAGER	0		

/* prototype for XXX_EXIT_MANAGER function */
typedef DWORD (NMSSTDCALLPTR XXXEXITMANAGER)();
...
typedef struct
{
    ...
    UINTPTR   sessionId;    /* session created context                */
...
typedef struct
{
    ...
    /* client information */
    DWORD               dwClntNum;
    UINTPTR		pdwClntSessId[64];
    UINTPTR             pdwClntCtaHd[64];
} CTASRV_SVC_INFO; 
...
DWORD NMSAPI ctaDestroySvrSession( 
                                  char*             addr,        /* CTA Server address                     */
                                  UINTPTR       ctaSvrSessionID);/* CTA Server sesion ID               */
