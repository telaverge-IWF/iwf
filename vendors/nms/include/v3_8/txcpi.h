...
*
*       This file defines the interface to the NMS Communications CPI library.
...
*
* 32    3/13/:1 3:18p Jrego
* Merging MR485 to rel_5_0/Mainline.
*
* 34    3/12/:1 9:06p Tmckeown
*
* 33    3/12/:1 7:29p Tmckeown
*
* 31    1/16/:1 12:00p Tmckeown
* Merging PR382 to rel_5_0/Mainline.
*
* 33    1/02/:1 12:06p Jrego
* Checking in code from STG (diffs not yet complete)
*
...
#define     cpi_wait_obj(x)         ((CPI_WAIT_TYPE)(get_descriptor(x)))
...
extern          "C"
{
    ...
    TXXPORT TX_HANDLE TXCPIFUNC cpi_open(unsigned short port, short mode,
                                         short (*receiver) (short handle,
                                                            short len));
    TXXPORT TX_HANDLE TXCPIFUNC cpia_open(void *chkey, unsigned short board,
    ...
    TXXPORT short TXCPIFUNC cpia_send(TX_HANDLE handle, CPIPKT *buffer,
                                      void *user_tx_key);
    TXXPORT short TXCPIFUNC cpi_get_data(TX_HANDLE handle, CPIPKT *buffer,
                                         short *len);
    TXXPORT short TXCPIFUNC cpia_get_data(TX_HANDLE handle, CPIPKT *buffer,
                                          short *len);
    TXXPORT CPI_ERR_TYPE TXCPIFUNC cpia_intr(TX_HANDLE handle);
    TXXPORT short TXCPIFUNC cpi_wait_msg(TX_HANDLE handle, CPIPKT *buffer,
                                         short *len, long millisecs);
    ...
    TXXPORT short TXCPIFUNC cpi_write_dpr(TX_HANDLE handle, char *buffer,
                                          unsigned long off, short len);
    TXXPORT short TXCPIFUNC cpi_pre_boot(TX_HANDLE handle, char *buffer,
                                         short len, unsigned short off,
                                         short xqt);
    TXXPORT short TXCPIFUNC cpi_load_krnl(TX_HANDLE handle, char *buffer,
                                          short len, short *seqnum,
                                          unsigned long destaddr);
    TXXPORT short TXCPIFUNC cpi_exec_krnl(TX_HANDLE handle,
...
#define cpi_htocp_l(x) NMSTX_HTOCP_L(x)
    TXXPORT short TXCPIFUNC cpi_get_board(TX_HANDLE handle,
                                          unsigned char *board,
                                          unsigned char *chan);

    TXXPORT short TXCPIFUNC cpi_read_control(TX_HANDLE handle,
                                             unsigned short basereg,
                                             unsigned short numreg,
                                             unsigned long *regarray,
                                             unsigned short *actcnt);
    TXXPORT short TXCPIFUNC cpi_write_control(TX_HANDLE handle,
                                              unsigned short basereg,
                                              unsigned short numreg,
                                              unsigned long *regarray,
                                              unsigned short *actcnt);
    ...
    short TXXPORT TXCPIFUNC cpi_set_cpid(short type, unsigned long param1,
