 ...
 * LOG:
 ****************************************************************************/
#ident "$Id: ReAuthTest.cpp,v 1.1 2011/05/03 04:54:00 vkumara Exp $"
...
int SendSWmReAuthRequest()
{
    ...
    string sessIdString;
    cin>>sessIdString;
    staReq.setSessionId(SessionId(sessIdString));
 
    staReq.setSessionIndex(sIdx);
    ...
    staReq.setOriginHost(originHost);
    ...
    staReq.setOriginRealm(originRealm);
    ...
    staReq.setDestinationRealm(destinationRealm);
    ...
    staReq.setDestinationHost(destinationHost);
    ...
    AuthApplicationId authApplicationId(SWM_INTERFACE_APP_ID);
    staReq.setAuthApplicationId(authApplicationId);

    ReAuthRequestType reAuthRequestType(ReAuthRequestType::AUTHORIZE_ONLY);
    staReq.setReAuthRequestType(reAuthRequestType);
    ...
    UserName userName("user@sta-network.com");
    staReq.setUserName(userName);

    cout<<staReq;

    SendDiaAppMsg(staReq, sIdx);
    SWmStats::GetSWmStats()->UpdateSendStats(staReq.getCommandCode(), ITS_TRUE);
 ...
 *  See Also:
 ****************************************************************************/
int SendSWmReAuthAnswer(const CommandImpl * impl1)
{
    ...
    // Optional param error - whether this command carries an error indicator
    // Optional param retrans - whether this is a retransmission
    diameter::swm::ReAuthAnswer staAns;

    // Create an instance of PushProfileRequest class
    ReAuthRequest stareq(*impl1);

    // Get some AVP values from Request
    const SessionId &sid1 = stareq.getSessionId();
    const OriginHost &oh1 = stareq.getOriginHost();
    const OriginRealm &or1 = stareq.getOriginRealm();

    const UserName &uname = stareq.getUserName();
    ...
    // The AVP Code of Session-Id is 263
    // The data type of Session-Id is UTF8String.
    staAns.setSessionId(sid1);

    staAns.setSessionIndex(stareq.getSessionIndex());
    ...
    staAns.setResultCode(resultCode);
    ...
    // The data type of Origin-Host AVP is DiameterIdentity.
    OriginHost originHost(GetOriginHost());
    staAns.setOriginHost(originHost);
    ...
    // The data type of Origin-Host AVP is DiameterIdentity.
    OriginRealm originRealm(GetOriginRealm());
    staAns.setOriginRealm(originRealm);

    cout<<staAns;

    SendDiaAppMsg(staAns, sIdx);
    SWmStats::GetSWmStats()->UpdateSendStats(staAns.getCommandCode(), ITS_FALSE);
