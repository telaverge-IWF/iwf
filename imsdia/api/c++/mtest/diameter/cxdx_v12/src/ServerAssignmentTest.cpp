 ...
 * LOG: Added cxdx_v10
 * LOG:
 * LOG: Revision 1.1.2.1.2.1  2013/07/01 09:42:24  nazad
 ...
 * LOG:
 *
 * ID: $Id: ServerAssignmentTest.cpp,v 1.1.2.1 2015/05/18 05:15:10 kpnithin Exp $
 ****************************************************************************/
#ident "$Id: ServerAssignmentTest.cpp,v 1.1.2.1 2015/05/18 05:15:10 kpnithin Exp $"
...
int SendCxdxServerAssignmentRequest()
{
    ...
    SessionPriority sessionPriority(SessionPriority::PRIORITY0);
    sar.setSessionPriority(sessionPriority);
    ...
    sar.setWildcardedPublicIdentity(WildcardedPublicIdentity("WcPI"));

    SCSCFRestorationInfo ScSCFRestorationInfo;
    
    RestorationInfo restorationInfo;
    restorationInfo.setPath(Path("PATH"));
    restorationInfo.setContact(Contact("CONTACT"));
    SubscriptionInfo subscriptionInfo;
    subscriptionInfo.setCallIDSIPHeader(CallIDSIPHeader("Call-ID-SIP-Header"));
    subscriptionInfo.setFromSIPHeader(FromSIPHeader("From-SIP-Header"));
    subscriptionInfo.setToSIPHeader(ToSIPHeader("To-SIP-Header"));
    subscriptionInfo.setRecordRoute(RecordRoute("Record-Route"));
    subscriptionInfo.setContact(Contact("CONTACT"));
    restorationInfo.setSubscriptionInfo(subscriptionInfo);
    
    ScSCFRestorationInfo.setUserName(userName);
    ScSCFRestorationInfo.addRestorationInfo(restorationInfo);
    ScSCFRestorationInfo.setSIPAuthenticationScheme(SIPAuthenticationScheme("SIP-Authentication-Scheme"));

    sar.setSCSCFRestorationInfo(ScSCFRestorationInfo);

    sar.setMultipleRegistrationIndication(MultipleRegistrationIndication(MultipleRegistrationIndication :: NOT_MULTIPLE_REGISTRATION));
...
int SendCxdxServerAssignmentAnswer(const CommandImpl * impl1)
{
    ...
    Json::Value* cfg = &(*m_jsonRoot1)["ServerAssignmentAnswer"];
    ...
    associatedIdentities.addUserName(UserName("User-Name"));
    saa.setAssociatedIdentities(associatedIdentities);

    saa.setLooseRouteIndication(LooseRouteIndication(LooseRouteIndication :: LOOSE_ROUTE_NOT_REQUIRED));
    ...
    ScSCFRestorationInfo.addRestorationInfo(restorationInfo);
    ScSCFRestorationInfo.setSIPAuthenticationScheme(SIPAuthenticationScheme("SIP-Authentication-Scheme"));
    saa.addSCSCFRestorationInfo(ScSCFRestorationInfo);

    AssociatedRegisteredIdentities associatedRegisteredIdentities;
    associatedRegisteredIdentities.addUserName(UserName("User-Name"));

    saa.setAssociatedRegisteredIdentities(associatedRegisteredIdentities);

    saa.setServerName(ServerName("Server-Name"));    
   
    saa.setWildcardedPublicIdentity(WildcardedPublicIdentity("WcPI"));
    
    isAVPEnabled = (*cfg)["PriviledgedSenderIndication"]["Enabled"].asBool();
    isNull = (*cfg)["PriviledgedSenderIndication"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
   
        psi=((*cfg)["PriviledgedSenderIndication"]["Value"].asInt());
        
        //CCSessionFailover Failover((CCSessionFailover::Value)failover);
        //cca.setCCSessionFailover(Failover);
   
        //saa.setPriviledgedSenderIndication(PriviledgedSenderIndication(PriviledgedSenderIndication:: NOT_PRIVILEDGED_SENDER));      
        PriviledgedSenderIndication  priviledgedSenderIndication((PriviledgedSenderIndication ::Value)psi);      
        saa.setPriviledgedSenderIndication(priviledgedSenderIndication); 
    }
