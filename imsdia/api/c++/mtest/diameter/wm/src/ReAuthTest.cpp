 ...
 * LOG:
 *
 * ID: $Id: ReAuthTest.cpp,v 3.1 2008/03/31 10:33:14 nvijikumar Exp $
 ****************************************************************************/
#ident "$Id: ReAuthTest.cpp,v 3.1 2008/03/31 10:33:14 nvijikumar Exp $"
...
int SendWmReAuthRequest()
{
    ...
    raReq.setUserName(userName);

    cout<<raReq;

    SendDiaAppMsg(raReq, sIdx);
    WmStats::GetWmStats()->UpdateSendStats(raReq.getCommandCode(), ITS_TRUE);
 ...
 *  See Also:
 ****************************************************************************/
int SendWmReAuthAnswer(const CommandImpl * impl1)
{
    ...
    // Optional param error - whether this command carries an error indicator
    // Optional param retrans - whether this is a retransmission
    diameter::wm::ReAuthAnswer raAns;

    // Create an instance of PushProfileRequest class
    ReAuthRequest rareq(*impl1);
    ...
    const UserName &uname = rareq.getUserName();
    ...
    // The AVP Code of User-Name AVP is 1.
    // The data type of User-Name AVP is UTF8String.
    raAns.setUserName(uname);

    cout<<raAns;

    SendDiaAppMsg(raAns, sIdx);
    WmStats::GetWmStats()->UpdateSendStats(raAns.getCommandCode(), ITS_FALSE);
