 ...
 *  See Also:
 ****************************************************************************/
int SendWmSessionTerminationRequest()
{
    ...
    diameter::wm::SessionTerminationRequest stReq(false,false);
    ...
    UserName userName("user@wm-network.com");
    ...
    WmStats::GetWmStats()->UpdateSendStats(stReq.getCommandCode(),
                                                                ITS_TRUE);
    return ITS_SUCCESS;
}
...
int SendWmSessionTerminationAnswer(const CommandImpl * impl1)
{
    ...
    // Optional param error - whether this command carries an error indicator.
    // Optional param retrans - whether this is a retransmission.
    diameter::wm::SessionTerminationAnswer sta(false,false);
    ...
    // The AVP Code of User-Name AVP is 1.
    // The data type of User-Name AVP is UTF8String.
    UserName uname("user@wm-network.com");
    sta.setUserName(uname);

    cout << sta;

    SendDiaAppMsg(sta, sIdx);
    WmStats::GetWmStats()->UpdateSendStats(sta.getCommandCode(), ITS_FALSE);
    return (ITS_SUCCESS);
}
