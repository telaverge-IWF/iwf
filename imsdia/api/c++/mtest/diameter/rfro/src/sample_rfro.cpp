 ...
 * LOG:
 *
 * ID: $Id: sample_rfro.cpp,v 3.2 2010/06/24 08:35:58 nvijikumar Exp $
 ****************************************************************************/
#ident "$Id: sample_rfro.cpp,v 3.2 2010/06/24 08:35:58 nvijikumar Exp $"
...
int HandleDiaAppMsg(const diameter::CommandImpl *impl, unsigned int &sIdxRfro)
{
    try
    {
        RfRoStats::GetRfRoStats()->UpdateRecvStats(impl->getCommandCode(),
                                                   impl->isRequest());
            ...
            switch (impl->getCommandCode())
            {
                case RFRO_CC_MSG_CMD_CODE:
                    if(impl->isRequest())
                    {
                        ...
                        SendRfRoCreditControlAnswer(impl);
                    }
                    else
                    {
    ...
    while(k)
    {
        ...
        { 
            switch (ret)
            {
                ...
                case 3:
                case 4:
                    SendRfRoCreditControlRequest(ret); 
                    break;
                case 5:
...
int GenerateLoad()
{
    ...
    while (true)
    {
        if (RfRoStats::sendTraffic)
        {
             ...
             */
            for (int i = 0; i < RfRoStats::burstSize; i++)
            {
             ...
             */
            ftime(&RfRoStats::current);

            TIMERS_USleep(RfRoStats::slpTime * 1000);

            /*
             * If the traffic duration (seconds) expires, update the stats
             */
            if ((RfRoStats::current.time - RfRoStats::starter.time) >=
                 RfRoStats::duration)
            {
                if (RfRoStats::firstTime)
                {
                    ftime(&RfRoStats::sendComplete);
                    RfRoStats::sendTraffic = ITS_FALSE;
                    RfRoStats::firstTime = false;
                    time(&tx);
...
{
    AppInitialize(RFRO_INTERFACE_APP_ID);
    AppMain(argc,argv);
