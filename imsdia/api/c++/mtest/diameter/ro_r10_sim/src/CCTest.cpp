 ...
 * LOG: Revision 1.1.4.2  2014/02/12 07:59:40  millayaraja
 * LOG: testapp for IWF performance
 * LOG:
 * LOG: Revision 1.1.2.3  2013/09/06 12:52:14  jkchaitanya
 * LOG: changes for the bug 2185
 * LOG:
 * LOG: Revision 1.1.2.2  2013/08/28 11:34:01  jkchaitanya
 ...
 * LOG:
 *
 * ID: $Id: CCTest.cpp,v 1.1.4.2 2014/02/12 07:59:40 millayaraja Exp $
 ****************************************************************************/
...
int SendRoCreditControlAnswer(const diameter::CommandImpl* impl ) 
{
    ...
    if((opfromserv == 11 && (ccRequestType.value() == CCRequestType::UPDATE_REQUEST)) ||
            (opfromserv == 12 && (ccRequestType.value() == CCRequestType::INITIAL_REQUEST)))
    {
    ...
    if((opfromserv == 5 && (ccRequestType.value() == CCRequestType::INITIAL_REQUEST)) ||
            (opfromserv == 6 && (ccRequestType.value() == CCRequestType::UPDATE_REQUEST)))
    {
        ResultCode resultCode(4012);
        cca.setResultCode(resultCode);
    }
    else if(opfromserv == 9 && (ccRequestType.value() == CCRequestType::UPDATE_REQUEST))
    {
        ResultCode resultCode(4010);
        cca.setResultCode(resultCode);
    }
    else if(opfromserv == 10 && (ccRequestType.value() == CCRequestType::INITIAL_REQUEST))
    {
        ResultCode resultCode(4014);
        cca.setResultCode(resultCode);
    }
    else if((ccRequestType.value() == CCRequestType::INITIAL_REQUEST) && 
            (opfromserv == 13 || opfromserv == 14 || opfromserv == 15))
    {
        cout<< "Setting SMS CallFlow Result Code" << endl;
        int res = 0;
        Json::Value* cfgres = &(*m_jsonRoot1)["CreditControlAnswer"]["SMS_CALL_FLOWS"]["CCA_I"];
        if(opfromserv == 13)
            res = (*cfgres)["CallFlow_13"]["ResultCode"]["Value"].asInt();
        else if(opfromserv == 14)
            res = (*cfgres)["CallFlow_14"]["ResultCode"]["Value"].asInt();
        else if(opfromserv == 15)
            res = (*cfgres)["CallFlow_15"]["ResultCode"]["Value"].asInt();

        ResultCode resultCode(res);
        cca.setResultCode(resultCode);
    }
    else
    {
    ...
    if (appMode == UNIT_TEST)
    {
        ...
        if(opfromserv == 4 && (ccRequestType.value() == CCRequestType::UPDATE_REQUEST))
        {
            //NOTE: For CallFlow 4 In CCA-U message LowBalanceIndication should be set to YES
...
void PopulateFinalUnitIndication(FinalUnitIndication &finalUnitInd)
{
    ...
    if(opfromserv == 7 && CCRequestTypeReceived == 2)
    {
        //NOTE: For CallFlow 7 FinalUnitAction is set to terminate for CCA-U
