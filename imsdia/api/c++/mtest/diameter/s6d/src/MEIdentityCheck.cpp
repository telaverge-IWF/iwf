 ...
 * CONTRACT: INTERNAL                                                       *
 ****************************************************************************
 * LOG: $Log: MEIdentityCheck.cpp,v $
 * LOG: Revision 1.1  2011/05/04 10:46:01  vkumara
 * LOG: Baseline for S6d interface.
 * LOG:
 * LOG: 
 * ID: $Id: MEIdentityCheck.cpp,v 1.1 2011/05/04 10:46:01 vkumara Exp $ 								    	
 ****************************************************************************/
#ident "$Id: MEIdentityCheck.cpp,v 1.1 2011/05/04 10:46:01 vkumara Exp $"
 ...
 ****************************************************************************
 *  Purpose: This function sends the  MEIdentityCheck to the
 *      diameter stack. Create the  MEIdentityCheck object,
 ...
 *  See Also:
 ****************************************************************************/
int SendS6DMEIdCheckRequest()
{
    // [1]. Create an instance of MEIdentityCheckRequest.
    // Optional param error - whether this command carries an error indicator.
    // Optional param retrans - whether this is a retransmission.
    diameter::s6d::MEIdentityCheckRequest MEReq;
    ...
    AllocateSession(S6D_ECR_ECA_INTERFACE_APP_ID, sIdx, sessIdString);
    MEReq.setSessionId(diameter::base::SessionId(sessIdString));

    MEReq.setSessionIndex(sIdx);
    ...
    MEReq.setAuthSessionState(authSessionState);
    ...
    MEReq.setOriginHost(originHost);
    ...
    MEReq.setOriginRealm(originRealm);
    ...
    MEReq.setDestinationHost(DestinationHost);
    ...
    MEReq.setDestinationRealm(destinationRealm);
    ...
    */
    ...
    IMEI imei("VKUMARA@S6D-INTERFACE");
    ti.setIMEI(imei);
    MEReq.setTerminalInformation(ti);
    ...
    SendDiaAppMsg(MEReq, sIdx);

    S6DStats::GetS6DStats()->UpdateSendStats(MEReq.getCommandCode(), ITS_TRUE);
    return ITS_SUCCESS;
}
 ...
 ****************************************************************************
 *  Purpose:
 *      This function sends the Answer to the MEIdentityCheck request
 *      made by the diameter stack. Create the MEIdentityCheckAnswer
 ...
 *  See Also:
 ****************************************************************************/
int SendS6DMEIdCheckAnswer(const CommandImpl * impl)
{
    // 1. Create an instance of MEIdCheckAnswer class
    ...
    // Optional param error - whether this command carries an error indicator.
    // Optional param retrans - whether this is a retransmission.
    diameter::s6d::MEIdentityCheckAnswer MEAns;

    // Create an instance of MEIdCheckRequest class to get host and
    // session details
    MEIdentityCheckRequest MEreq(*impl);

    // Get AVP values from requested message
    const SessionId &sid1 = MEreq.getSessionId();
    const OriginHost &oh1 = MEreq.getOriginHost();
    const OriginRealm &or1 = MEreq.getOriginRealm();
    const AuthSessionState &authSessState = MEreq.getAuthSessionState();

    // 2. Create the instances of the classes describing the various
    // AVPs' present in an MEIdCheckAnswer command and
    // pass them by value to the corresponding data members set
    // methods of the MEIdCheckAnswer object created
    // in step 1.
    ...
    // The AVP Code of Session-Id is 263
    // The data type of Session-Id is UTF8String.
    MEAns.setSessionId(sid1);
    MEAns.setSessionIndex(MEreq.getSessionIndex());    
 
    // [2.1.2.x] Required AVPs
    MEAns.setAuthSessionState(authSessState);
    ...
    MEAns.setResultCode(resultCode);
    ...
    MEAns.setExperimentalResult(experimentalResult);
    ...
    // The data type of Origin-Host AVP is DiameterIdentity.
    OriginHost originHost(GetOriginHost());
    MEAns.setOriginHost(originHost);
    ...
    // This AVP should not be set by Application.
    // MEAns.setRouteRecord(diameter::base::RouteRecord(...));
    ...
    // Send Object to stack.
    SendDiaAppMsg(MEAns, sIdx);

    // Update stats.
    S6DStats::GetS6DStats()->UpdateSendStats(MEAns.getCommandCode(), ITS_FALSE);
    return ITS_SUCCESS;
}
