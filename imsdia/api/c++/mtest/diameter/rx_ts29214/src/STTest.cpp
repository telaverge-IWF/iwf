 ...
 * LOG:
 *
 * ID: $Id: STTest.cpp,v 1.2 2011/07/22 11:49:46 rajeshak Exp $
 ****************************************************************************/
#include <app.h>
...
int SendRxSTRequest() 
{
    ...
    diameter::rx_ts29214::STRequest stReq;
    ...
    RxStats::GetRxStats()->UpdateSendStats(stReq.getCommandCode(), ITS_TRUE);
    return ITS_SUCCESS;
...
//	stans.addProxyInfo(proxyInfo);
    ...
    for(int i=0; i<pinfo_count; i++)
    {
        ...
        stans.addProxyInfo(pinfo);
    }


    cout << "STA" << stans;
    SendDiaAppMsg(stans, sIdx);
    RxStats::GetRxStats()->UpdateSendStats(stans.getCommandCode(), ITS_FALSE);
