 ...
 * LOG: Revision 3.2  2011/04/28 05:58:04  nvijikumar
 * LOG: Proxy support
 * LOG:
 ...
 * LOG:
 *
 * ID: $Id: SessionTerminationTest.cpp,v 3.2 2011/04/28 05:58:04 nvijikumar Exp $
 ****************************************************************************/
#ident "$Id: SessionTerminationTest.cpp,v 3.2 2011/04/28 05:58:04 nvijikumar Exp $"
 ...
 *  See Also:
 ****************************************************************************/
int SendNasreqSessionTerminationRequest()
{
    ...
    diameter::nasreq::STRequest stReq(false,false);
    ...
    NasreqStats::GetNasreqStats()->UpdateSendStats(stReq.getCommandCode(),
                                                                ITS_TRUE);
    return ITS_SUCCESS;
}
...
int SendNasreqSessionTerminationAnswer(const CommandImpl * impl1)
{
    ...
    // Optional param error - whether this command carries an error indicator.
    // Optional param retrans - whether this is a retransmission.
    diameter::nasreq::STAnswer sta(false,false);
//    STAnswer sta(false, false);
   ...
   // Create an instance of RegistrationTerminationrequest class
    ...
    if (streq.containsUserName())
    {
        const UserName &uname = streq.getUserName();
    }
    ...
    // The AVP Code of User-Name AVP is 1.
    // The data type of User-Name AVP is UTF8String.
    UserName uname("user@nasreq-network.com");
    sta.setUserName(uname);

    int pinfo_count = streq.countProxyInfo();
    for(int i=0; i<pinfo_count; i++)
    {
        ProxyInfo pinfo;
        ProxyHost phost(streq.getProxyInfo(i).getProxyHost());
        pinfo.setProxyHost(phost);

        ProxyState pstate(streq.getProxyInfo(i).getProxyState());
    ...
    NasreqStats::GetNasreqStats()->UpdateSendStats(sta.getCommandCode(), ITS_FALSE);
    return (ITS_SUCCESS);
}
