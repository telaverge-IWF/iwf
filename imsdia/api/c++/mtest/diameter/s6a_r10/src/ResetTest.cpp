 ...
 * LOG:
 * ID: $Id: ResetTest.cpp,v 1.1.6.2 2014/02/12 08:06:13 millayaraja Exp $  
 ****************************************************************************/
#ident "$Id: ResetTest.cpp,v 1.1.6.2 2014/02/12 08:06:13 millayaraja Exp $"
 ...
 ****************************************************************************
 *  Purpose: This function sends the  Reset to the
 *      diameter stack. Create the  Reset object,
...
int SendS6AResetRequest()
{
    ...
    RSReq.setSessionId(diameter::base::SessionId(sessIdString));

    RSReq.setSessionIndex(sIdx);
    ...
    RSReq.setAuthSessionState(authSessionState);
    ...
    RSReq.setOriginHost(originHost);
    ...
    RSReq.setOriginRealm(originRealm);
    ...
    RSReq.setDestinationHost(DestinationHost);
    ...
    // The data type of Destination-Realm AVP is DiameterIdentity.
     ...
     RSReq.setDestinationRealm(destinationRealm);

    // [2.1.2.7] Set the User-Id AVP.
    // Set the User-Id AVP.
    // The data type of User-Name is UTF8String.
    static long long user_id_number=singlton::arg[0];
        char temp[100]={'\0'},*end;
        cout<<"IMSI BASED ROUTING"<<endl;
        sprintf(temp,"%lld",user_id_number);
        UserId userId((const char *)temp);
        RSReq.addUserId(userId);
        if(user_id_number!=singlton::arg[1])
                user_id_number++;
        else
                user_id_number=singlton::arg[0];

    // Application should not set this AVP.
    // RSReq.addProxyInfo(diameter::base::ProxyInfo(...));
    // RSReq.addRouteRecord(diameter::base::RouteRecord(...));
    ...
    SendDiaAppMsg(RSReq, sIdx);

    S6AStats::GetS6AStats()->UpdateSendStats(RSReq.getCommandCode(), ITS_TRUE);
    return ITS_SUCCESS;
}
 ...
 *  See Also:
 ****************************************************************************/
int SendS6AResetAnswer(const CommandImpl * impl)
{
    // 1. Create an instance of ResetAnswer class
    // The ResetAnswer command is sent by a server
    // in response to the ResetRequest command with
    // Command-Code = xxx and 'R' bit reset
    // Optional param error - whether this command carries an error indicator.
    // Optional param retrans - whether this is a retransmission.
    diameter::s6a_r10::ResetAnswer RSAns;
    ...
    const SessionId &sid1 = RSReq.getSessionId();
    const OriginHost &oh1 = RSReq.getOriginHost();
    const OriginRealm &or1 = RSReq.getOriginRealm();
    const AuthSessionState &authSessState = RSReq.getAuthSessionState();

    // 2. Create the instances of the classes describing the various
    // AVPs' present in an ResetAnswer command and
    // pass them by value to the corresponding data members set
    // methods of the ResetAnswer object created
    // in step 1.
    ...
    // The AVP Code of Session-Id is 263
    // The data type of Session-Id is UTF8String.
    RSAns.setSessionId(sid1);
    RSAns.setSessionIndex(RSReq.getSessionIndex());    
 
    // [2.1.2.x] Required AVPs
    RSAns.setAuthSessionState(authSessState);
    ...
    RSAns.setResultCode(resultCode);
    ...
    RSAns.setExperimentalResult(experimentalResult);
    ...
    // The data type of Origin-Host AVP is DiameterIdentity.
    OriginHost originHost(GetOriginHost());
    RSAns.setOriginHost(originHost);
    ...
    // This AVP should not be set by Application.
    // RSAns.setRouteRecord(diameter::base::RouteRecord(...));
    // add more AVP as ANY values
    ...
    // Send Object to stack.
    SendDiaAppMsg(RSAns, sIdx);

    // Update stats.
    S6AStats::GetS6AStats()->UpdateSendStats(RSAns.getCommandCode(), ITS_FALSE);
    return ITS_SUCCESS;
}
