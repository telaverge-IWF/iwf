 ...
 * LOG: Revision 1.1.2.3.2.3.2.1.2.1  2015/04/06 04:10:55  jkchaitanya
 * LOG: changes to set CCSessionFailover as FAILOVER_SUPPORTED
 * LOG:
 * LOG: Revision 1.1.2.3.2.3.2.1  2014/09/17 06:36:37  jkchaitanya
 * LOG: changes for bugs 4965 and 4978
 * LOG:
 * LOG: Revision 1.1.2.3.2.3  2014/08/21 10:51:58  jkchaitanya
 * LOG: changes for executing both sms and local voice call at a time for same imsi
 * LOG:
 * LOG: Revision 1.1.2.3.2.2  2014/07/31 12:14:08  jkchaitanya
 * LOG: changes for transparent followOn calls in load
 * LOG:
 * LOG: Revision 1.1.2.3.2.1  2014/07/30 10:37:08  jkchaitanya
 * LOG: changes for imsi based call flow selection at OCS for load
 * LOG:
 * LOG: Revision 1.1.2.3  2014/04/07 04:50:30  jkchaitanya
 * LOG: changes for ro-tsl-perf to support local/remote/sms calls for load
 * LOG:
 * LOG: Revision 1.1.2.2  2014/03/24 11:36:26  jsarvesh
 ...
 * LOG:
 *
 * ID: $Id: CCTest.cpp,v 1.1.2.3.2.3.2.1.2.1 2015/04/06 04:10:55 jkchaitanya Exp $
 ****************************************************************************/
...
bool DisableUnwantedAVPsForTransatel = true;
int callFlowSelected = 0;
int configuredCCTime = 0;
int scpAction = 0;
int capControlMode = 0;
string sessIdString;
ITS_UINT sIdx;
extern string ImsiForLocalConnectCall;
extern string ImsiForLocalContinueCall;
extern string ImsiForRemoteConnectCall;
extern string ImsiForRemoteContinueCall;
extern string ImsiForSMSCall;
extern int CCTimeForRemoteConnectCall;
extern int CCTimeForRemoteContinueCall;
extern int CCTimeForLocalConnectCall;
extern int CCTimeForLocalContinueCall;
extern string ImsiForTransparentLocalCall;
extern string ImsiForTransparentRemoteCall;
extern int CCTimeForTransparentLocalCall;
extern int CCTimeForTransparentRemoteCall;
extern string ImsiForFollowOnContinueLocalAndLocalConnect;
extern string ImsiForFollowOnContinueLocalAndRemoteConnect;
extern int CCTimeForFollowOnContinueLocalAndLocalConnect;
extern int CCTimeForFollowOnContinueLocalAndRemoteConnect;
extern string ImsiForFollowOnConnectRemoteAndLocalContinue;
extern string ImsiForFollowOnConnectRemoteAndRemoteContinue;
extern int CCTimeForFollowOnConnectRemoteAndLocalContinue;
extern int CCTimeForFollowOnConnectRemoteAndRemoteContinue;
...
std::unordered_map <string, CallContext> gCallFlowState;
    ...
    if (appMode == UNIT_TEST)
    {
        ...
        if(ccr.getCCRequestType().value() != CCRequestType::TERMINATION_REQUEST)
        {
            CCSessionFailover ccSessionFailover(
                    CCSessionFailover::FAILOVER_SUPPORTED);
        ...
        // The AVP Code of Multiple-Services-Credit-Control is 456.
        MultipleServicesCreditControl multipleServicesCC1;
        if(ccr.containsServiceInformation())
        {
            ServiceInformation srvcInf = ccr.getServiceInformation();
            if(srvcInf.containsCSInformation())
            {
                CSInformation csinfo = srvcInf.getCSInformation();
                if(csinfo.containsCAPInformation())
                { 
                    CAPInformation capInfoVal = csinfo.getCAPInformation();
                    if(capInfoVal.containsCAPIMSI())
                    {
                        CAPIMSI capimsi = capInfoVal.getCAPIMSI();
                        SetCallFlowBasedonIncommingImsi(capimsi.value(), ccr.getCCRequestType());
                    }
                }
            }
        }
        ...
        CreditControlFailureHandling Failure((CreditControlFailureHandling::Value)2);
        cca.setCreditControlFailureHandling(Failure);
        
        cca.setOriginStateId((OriginStateId)3);
...
void SetCallFlowBasedonIncommingImsi(string imsiRcvd, CCRequestType ccreqType)
{
    string sub_imsi = imsiRcvd.substr(0,ImsiForLocalConnectCall.length());
    if(!strcmp(sub_imsi.c_str(),ImsiForLocalConnectCall.c_str()))
    {
        callFlowSelected = LOCAL_CONNECT_CALL;
        configuredCCTime = CCTimeForLocalConnectCall;
        scpAction = CONNECT;
        capControlMode = LOCAL_CONTROL;
    }
    else 
    {
        sub_imsi = imsiRcvd.substr(0,ImsiForLocalContinueCall.length());
        if(!strcmp(sub_imsi.c_str(),ImsiForLocalContinueCall.c_str()))
        {
            callFlowSelected = LOCAL_CONTINUE_CALL;
            configuredCCTime = CCTimeForLocalContinueCall;
            scpAction = CONTINUE;
            capControlMode = LOCAL_CONTROL;
        }
        else 
        {
            sub_imsi = imsiRcvd.substr(0,ImsiForRemoteConnectCall.length());
            if(!strcmp(sub_imsi.c_str(),ImsiForRemoteConnectCall.c_str()))
            {
                callFlowSelected = REMOTE_CONNECT_CALL;
                configuredCCTime = CCTimeForRemoteConnectCall;
                scpAction = CONNECT;
                capControlMode = REMOTE_CONTROL;
            }
            else 
            { 
                sub_imsi = imsiRcvd.substr(0,ImsiForRemoteContinueCall.length());
                if(!strcmp(sub_imsi.c_str(),ImsiForRemoteContinueCall.c_str()))
                {
                    callFlowSelected = REMOTE_CONTINUE_CALL;
                    configuredCCTime = CCTimeForRemoteContinueCall;
                    scpAction = CONTINUE;
                    capControlMode = REMOTE_CONTROL;
                }
                else 
                { 
                    sub_imsi = imsiRcvd.substr(0,ImsiForTransparentLocalCall.length());
                    if(!strcmp(sub_imsi.c_str(),ImsiForTransparentLocalCall.c_str()))
                    {
                        callFlowSelected = TRANSPARENT_FOLLOW_ON_LOCAL_CALL;
                        configuredCCTime = CCTimeForTransparentLocalCall;
                        scpAction = TRANSPARENT_FOLLOW_ON;
                        capControlMode = LOCAL_CONTROL;
                    }
                    else 
                    {
                        sub_imsi = imsiRcvd.substr(0,ImsiForTransparentRemoteCall.length());
                        if(!strcmp(sub_imsi.c_str(),ImsiForTransparentRemoteCall.c_str()))
                        {
                            callFlowSelected = TRANSPARENT_FOLLOW_ON_REMOTE_CALL;
                            configuredCCTime = CCTimeForTransparentRemoteCall;
                            scpAction = TRANSPARENT_FOLLOW_ON;
                            capControlMode = REMOTE_CONTROL;
                        }
                        else 
                        {
                            sub_imsi = imsiRcvd.substr(0,ImsiForSMSCall.length());
                            if(!strcmp(sub_imsi.c_str(),ImsiForSMSCall.c_str()))
                            {
                                callFlowSelected = SMS_CALL;
                                scpAction = CONTINUE;
                                capControlMode = LOCAL_CONTROL;
                            }
                            else
                            {
                                sub_imsi = imsiRcvd.substr(0,ImsiForFollowOnContinueLocalAndLocalConnect.length());
                                if(!strcmp(sub_imsi.c_str(),ImsiForFollowOnContinueLocalAndLocalConnect.c_str()))
                                {
                                    callFlowSelected = FOLLOWON_CONTINUE_LOCAL_AND_CONNECT_LOCAL;
                                    configuredCCTime = CCTimeForFollowOnContinueLocalAndLocalConnect;
                                    //NOTE: Check the map 
                                    //1) if entry already there skip insert and update the status
                                    //2) if entry is not there insert into map and set the status
                                    std::unordered_map<string, CallContext>::iterator itr = gCallFlowState.find(imsiRcvd);
                                    if (itr != gCallFlowState.end())
                                    {
                                        scpAction = CONNECT;
                                        capControlMode = LOCAL_CONTROL;
                                        if(ccreqType.value() == CCRequestType::TERMINATION_REQUEST)
                                        {
                                            itr->second.numOfCCRTRcvd++;
                                        }
                                        if(itr->second.numOfCCRTRcvd == FOLLOWON_SECOND_CALL_CCTR_RCVD)
                                        {
                                    ...
                                    else
                                    {
                                        ...
                                        call_info.callFlowId = FOLLOWON_CONTINUE_LOCAL_AND_CONNECT_LOCAL;
                                        call_info.numOfCCRTRcvd = 0;
                                        scpAction = FOLLOW_ON_CONTINUE;
                                        capControlMode = LOCAL_CONTROL;
                                        std::pair<string, CallContext> imsipair(imsiRcvd, call_info);
                                        gCallFlowState.insert(imsipair);
                                    }
                                }
                                else
                                {
                                    sub_imsi = imsiRcvd.substr(0,ImsiForFollowOnContinueLocalAndRemoteConnect.length());
                                    if(!strcmp(sub_imsi.c_str(),ImsiForFollowOnContinueLocalAndRemoteConnect.c_str()))
                                    {
                                        callFlowSelected = FOLLOWON_CONTINUE_LOCAL_AND_CONNECT_REMOTE;
                                        configuredCCTime = CCTimeForFollowOnContinueLocalAndRemoteConnect;
                                        //NOTE: Check the map 
                                        //1) if entry already there skip insert and update the status
                                        //2) if entry is not there insert into map and set the status
                                        std::unordered_map<string, CallContext>::iterator itr = gCallFlowState.find(imsiRcvd);
                                        ...
                                        {
                                            struct CallContext call_info;
                                            call_info.callFlowId = FOLLOWON_CONTINUE_LOCAL_AND_CONNECT_REMOTE;
                                            scpAction = FOLLOW_ON_CONTINUE;
                                            capControlMode = LOCAL_CONTROL;
                                            std::pair<string, CallContext> imsipair(imsiRcvd, call_info);
                                            gCallFlowState.insert(imsipair);
                                    ...
                                    {
                                        sub_imsi = imsiRcvd.substr(0,ImsiForFollowOnConnectRemoteAndLocalContinue.length());
                                        if(!strcmp(sub_imsi.c_str(),ImsiForFollowOnConnectRemoteAndLocalContinue.c_str()))
                                        {
                                            callFlowSelected = FOLLOWON_CONNECT_REMOTE_AND_LOCAL_CONTINUE;
                                            configuredCCTime = CCTimeForFollowOnConnectRemoteAndLocalContinue;
                                            //NOTE: Check the map 
                                            //1) if entry already there skip insert and update the status
                                            //2) if entry is not there insert into map and set the status
                                            std::unordered_map<string, CallContext>::iterator itr = gCallFlowState.find(imsiRcvd);
                                            ...
                                            {
                                                struct CallContext call_info;
                                                call_info.callFlowId = FOLLOWON_CONNECT_REMOTE_AND_LOCAL_CONTINUE;
                                                scpAction = FOLLOW_ON_CONNECT;
                                                capControlMode = REMOTE_CONTROL;
                                                std::pair<string, CallContext> imsipair(imsiRcvd, call_info);
                                                gCallFlowState.insert(imsipair);
                                            }
                                        ...
                                        {
                                            sub_imsi = imsiRcvd.substr(0,ImsiForFollowOnConnectRemoteAndRemoteContinue.length());
                                            if(!strcmp(sub_imsi.c_str(),ImsiForFollowOnConnectRemoteAndRemoteContinue.c_str()))
                                            {
                                                callFlowSelected = FOLLOWON_CONNECT_REMOTE_AND_REMOTE_CONTINUE;
                                                configuredCCTime = CCTimeForFollowOnConnectRemoteAndRemoteContinue;
                                                //NOTE: Check the map 
                                                //1) if entry already there skip insert and update the status
                                                //2) if entry is not there insert into map and set the status
                                                std::unordered_map<string, CallContext>::iterator itr = gCallFlowState.find(imsiRcvd);
                                                ...
                                                {
                                                    struct CallContext call_info;
                                                    call_info.callFlowId = FOLLOWON_CONNECT_REMOTE_AND_REMOTE_CONTINUE;
                                                    scpAction = FOLLOW_ON_CONNECT;
                                                    capControlMode = REMOTE_CONTROL;
                                                    std::pair<string, CallContext> imsipair(imsiRcvd, call_info);
                                                    gCallFlowState.insert(imsipair);
...
void PopulateServiceControl(ServiceControl &serviceCtrl, bool isRequest=true)
{
...
#if 0
    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPACHInformation"]["CAPRRBNoAnswerTimeOut"]["Enabled"].asBool();
    ...
    if(!isNull && isAVPEnabled)
    {
        ...
        CAPRRBInformation rrbInfo;
        CAPRRBNoAnswerTimeout rrbtimeout(timeout);
        rrbInfo.setCAPRRBNoAnswerTimeout(rrbtimeout);
        capInfo.setCAPRRBInformation(rrbInfo);
    }
...
void PopulateCAPConnectInformation(CAPConnectInformation &CAPConnectInfo, bool isRequest=true)
{
...
#endif
    std::string str = "1234";
    CAPConnectInfo.setCAPConnectDestinationNumber((CAPConnectDestinationNumber)str);

    CAPConnectInfo.setCAPConnectDestinationNumberNature((CAPConnectDestinationNumberNature)23);

    CAPConnectInfo.setCAPConnectDestinationNumberNPI((CAPConnectDestinationNumberNPI)21);

    CAPConnectInfo.setCAPConnectGenericNumber((CAPConnectGenericNumber)str);

    CAPConnectInfo.setCAPConnectGenericNumberNOA((CAPConnectGenericNumberNOA)10);

    CAPConnectInfo.setCAPConnectGenericNumberNPI((CAPConnectGenericNumberNPI)9);

    CAPConnectInfo.setCAPConnectCallingCategory((CAPConnectCallingCategory)8);

    CAPConnectInfo.setCAPConnectRedirectingNumber((CAPConnectRedirectingNumber)"5214");

    CAPConnectInfo.setCAPConnectRedirectingIndicator((CAPConnectRedirectingIndicator)12);

    CAPConnectInfo.setCAPConnectOriginalRedirectingReason((CAPConnectOriginalRedirectingReason)15);

    CAPConnectInfo.setCAPConnectRedirectingCounter((CAPConnectRedirectingCounter)10);

    CAPConnectInfo.setCAPConnectRedirectingReason((CAPConnectRedirectingReason)13);

    CAPConnectInfo.setCAPConnectoCSIApplicable((CAPConnectoCSIApplicable)11);

    CAPConnectInfo.setCAPConnectSuppressionOfAnnouncement((CAPConnectSuppressionOfAnnouncement)12);
        
    CAPConnectInfo.setCAPAnnouncementAccessNumber((CAPAnnouncementAccessNumber)"1234");
#if 0
    ...
    if(!isNull && isAVPEnabled)
    {
        ...
        CAPConnectInfo.setCAPConnectOriginalRedirectionReason((CAPConnectOriginalRedirectionReason)redirect_reason);
    }
    ...
    if(!isNull && isAVPEnabled)
    {
        ...
        CAPConnectInfo.setCAPConnectRedirectionCounter((CAPConnectRedirectionCounter)counter);
    }
...
void PopulateGrantedServiceUnit(GrantedServiceUnit &grantedServiceUnit)
{
    ...
    CCTime ccTime(configuredCCTime);
    grantedServiceUnit.setCCTime(ccTime);

//    TariffTimeChange TTtime(10);
//    grantedServiceUnit.setTariffTimeChange(TTtime);

    CCServiceSpecificUnits ccServiceSpecificUnits(1);
