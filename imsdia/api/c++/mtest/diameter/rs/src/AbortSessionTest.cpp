 ...
 * LOG:
 * LOG:
 * ID: $Id: AbortSessionTest.cpp,v 1.1 2011/07/14 11:53:20 vkumara Exp $
 ****************************************************************************/
#ident "$Id: AbortSessionTest.cpp,v 1.1 2011/07/14 11:53:20 vkumara Exp $"
 ...
 ****************************************************************************
 *  Purpose: This function sends the RsASRequest to the
 *      diameter stack. Create the WdASRequest object,
...
int SendRsASRequest()
{
    // 1. Create an instance of  RsASRequest class
    // The ASRequest command is sent from a Diameter
    ...
    // The AVP Code of Event-Timestamp is 55.
     ...
     asReq.setEventTimestamp(EventTimestamp(999));
    ...
    RsStats::GetRsStats()->UpdateSendStats(asReq.getCommandCode(), ITS_TRUE);
...
/*.implementation:public
 ****************************************************************************
 *  Purpose: This function sends the Answer to the RsAS request
 *      made by the diameter stack. Create the RsAS Answer
 ...
 *  See Also:
 ****************************************************************************/
int SendRsASAnswer(const CommandImpl * impl1)
{
    // 1. Create an instance of RsASAnswer class
    // The ASAnswer command is sent by a server
    // in response to the Location-Info-Request command with
    // Command-Code = 274 and 'R' bit reset
    // Optional param error - whether this command carries an error indicator.
    // Optional param retrans - whether this is a retransmission.
    diameter::rs::ASAnswer asAns;
    ...
    RsStats::GetRsStats()->UpdateSendStats(asAns.getCommandCode(), ITS_FALSE);
    return ITS_SUCCESS;
}
