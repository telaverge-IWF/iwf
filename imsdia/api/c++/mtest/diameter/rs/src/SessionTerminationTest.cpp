 ...
 * LOG:
 * LOG:
 * ID: $Id: SessionTerminationTest.cpp,v 1.1 2011/07/14 11:53:20 vkumara Exp $
 ****************************************************************************/
#ident "$Id: SessionTerminationTest.cpp,v 1.1 2011/07/14 11:53:20 vkumara Exp $"
 ...
 ****************************************************************************
 *  Purpose: This function sends the STRequest the
...
int SendRsSTRequest()
{
    // 1. Create an instance of RsSTRequest class
    // The RsSTRequest  command is sent from a Diameter
    // multimedia client to a Diameter multimedia server in order to
    // saReq it to store the name of the server that is currently
    // serving the user.
    // Command-Code = 275 and 'R' bit set
    // Optional param error - whether this command carries an error indicator.
    // Optional param retrans - whether this is a retransmission.
    diameter::rs::STRequest stReq(false,false);
    ...
    cout<<endl<<" STR : "<<stReq<<endl;

    SendDiaAppMsg(stReq, sIdx);
    RsStats::GetRsStats()->UpdateSendStats(stReq.getCommandCode(),
                                                                ITS_TRUE);
    return ITS_SUCCESS;
}
...
/*.implementation:public
 ****************************************************************************
 *  Purpose: This function sends the Answer to the ST
 *      request made by the diameter stack. Create the
 *      STAnswer object, populate the AVPs and send the
 ...
 *  See Also:
 ****************************************************************************/
int SendRsSTAnswer(const CommandImpl * impl1)
{
    // 1. Create an instance of RsSTAnswer class
    // The RsSTAnswer command is sent by a server
    // in response to the Session-Termination-Request command with
    // Command-Code = 275 and 'R' bit reset
    // Optional param error - whether this command carries an error indicator.
    // Optional param retrans - whether this is a retransmission.
    diameter::rs::STAnswer sta(false,false);

    // Create an instance of SessionTerminationrequest class
    ...
    // 2. Create the instances of the classes describing the various
    // AVPs' present in an SessionTerminationAnswer command and
    // pass them by value to the corresponding data members set
    // methods of the SessionTerminationAnswer object created
    // in step 1.
    // [2.1.1] Set the Session-Id AVP.
    // The Session-Id AVP is used to identity a specific session
    // and must appear immediately following the Diameter header.
    ...
    cout <<endl<<" STA : " << sta <<endl;

    SendDiaAppMsg(sta, sIdx);
    RsStats::GetRsStats()->UpdateSendStats(sta.getCommandCode(), ITS_FALSE);
    return (ITS_SUCCESS);
}
