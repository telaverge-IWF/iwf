 ...
 * LOG:
 *
 * ID: $Id: wd_stats.cpp,v 3.1 2008/03/31 10:33:14 nvijikumar Exp $
 ****************************************************************************/
#ident "$Id: wd_stats.cpp,v 3.1 2008/03/31 10:33:14 nvijikumar Exp $"
...
its::ITS_Mutex WdStats::guard(0);
ITS_BOOLEAN WdStats::sendTraffic = ITS_FALSE;
ITS_BOOLEAN WdStats::firstTime = ITS_FALSE;
time_t WdStats::startTime;
time_t WdStats::stopTime;
struct timeb WdStats::sendComplete;
struct timeb WdStats::recvComplete;
struct timeb WdStats::starter;
struct timeb WdStats::current;
int WdStats::duration = 10;
int WdStats::burstSize = 0;
int WdStats::slpTime = 1000;

static WdStats *wdStats = NULL;
...
WdStats::GetWdStats()
{
    ...
    */
    ITS_ASSERT(WdStats::guard.Acquire() == ITS_SUCCESS);
 ...
 ****************************************************************************/
void
WdStats::CalcTPS(std::ostrstream& oss)
{
    Print(oss);
    ...
    if (numRaMsgsRecv == numRqMsgsSent)
    {
         /* Calculate tps
         */
         if ((WdStats::stopTime-WdStats::startTime) > 0 )
         {
             tps = (numRaMsgsRecv)/(WdStats::stopTime-WdStats::startTime);
         }
         else
         {
             //tps = (numRaMsgsRecv);
             tps = (1000/WdStats::slpTime) * WdStats::burstSize;
         }
         ...
         */
          ...
          */
         int st = WdStats::recvComplete.time - WdStats::sendComplete.time;
         int mt = WdStats::recvComplete.millitm - WdStats::sendComplete.millitm;
 ...
 *************************************************************************/
void
WdStats::UpdateSendStats(ITS_UINT commandCode, ITS_BOOLEAN isReq)
{
    ...
    */
	...
	case WD_AA_MSG_CMD_CODE:  /* AAR */
	    numAARSent++;
	    break;
	case WD_RA_MSG_CMD_CODE:  /* RAR */
	    numRARSent++;
            break;
        case WD_ST_MSG_CMD_CODE:  /* STR */
            numSTRSent++;
            break;
        case WD_AS_MSG_CMD_CODE:  /* ASR */
            numASRSent++;
            break;
        case WD_DE_MSG_CMD_CODE:  /* DER */
            numDERSent++;
            break;
        default:
            break;
    ...
    */
    else
    {
        switch(commandCode)
        {
	case WD_AA_MSG_CMD_CODE:  /* AAA */
            numAAASent++;
            break;
        case WD_RA_MSG_CMD_CODE:  /*RAA */
            numRAASent++;
            break;
	case WD_ST_MSG_CMD_CODE:  /* STA */
            numSTASent++;
            break;
        case WD_AS_MSG_CMD_CODE:  /*ASAns */
            numASASent++;
            break;
        case WD_DE_MSG_CMD_CODE:  /*DEAns */
            numDEASent++;
            break;
 ...
 ****************************************************************************/
void
WdStats::UpdateRecvIndications(ITS_UINT indic)
{
    switch (indic)
    {
 ...
 ****************************************************************************/
void
WdStats::UpdateRecvStats(ITS_UINT commandCode, ITS_BOOLEAN isReq)
{
    time_t tx;
    struct tm tbuf, *temp;
    char buff[80];
    ...
    */
	...
	case WD_ST_MSG_CMD_CODE:  /* STR */
            numSTRRecv++;
            break;
        case WD_AS_MSG_CMD_CODE:  /*ASReq */
            numASRRecv++;
            break;
        case WD_DE_MSG_CMD_CODE:  /*DEReq */
            numDERRecv++;
            break;
        default:
            break;
    ...
    */
	...
	case WD_AA_MSG_CMD_CODE:  /* AAA */
        ...
        */
        if (numRaMsgsRecv == 1)
        {
            time(&WdStats::startTime);
        }

        /*Update the stop time details in stats
        */
        if (!WdStats::sendTraffic &&
            (WdStats::numRqMsgsSent == WdStats::numRaMsgsRecv))
        {
            time(&WdStats::stopTime);
            ftime(&WdStats::recvComplete);
            time(&tx);
 ...
 ****************************************************************************/
void
WdStats::PrintIndications(std::ostream& os) const
{
 ...
 ****************************************************************************/
void
WdStats::Print(std::ostream& os) const
{
 ...
 ****************************************************************************/
ostream&
operator<<(ostream& os, const WdStats& wdStats)
{
    wdStats.Print(os);
    return os;
}
...
void StratTraffic(std::ostream& oss , int duration, int burstSize, int slpTime)
{
    ...
    WdStats::GetWdStats()->ResetAll();

    /*
     * Set the duration, burst size and the sleep time
     */
    WdStats::duration = duration;
    WdStats::burstSize = burstSize;
    WdStats::slpTime = slpTime;

    /*
     * Store the start time
     */
    ftime(&WdStats::starter);
    time(&WdStats::startTime);

    WdStats::sendTraffic = ITS_TRUE;
    WdStats::firstTime = ITS_TRUE;
    time(&tx);
    temp =  localtime_r(&tx, &tbuf);
    strftime(buff, 80, "[ %b%d %Y %H:%M:%S  ]" , &tbuf);
     ...
     */
    TrafficGeneratorFunc = SendWdAARequest;

}

void StopTraffic()
{
    WdStats::sendTraffic = ITS_FALSE;
    ftime(&WdStats::sendComplete);
}

void PrintStats(std::ostream& oss)
{
    WdStats::GetWdStats()->Print(oss);
}
...
{
    WdStats::GetWdStats()->UpdateRecvIndications(indType);
}

void CalculateTPS(std::ostrstream& oss)
{
    WdStats::GetWdStats()->CalcTPS(oss);
}
