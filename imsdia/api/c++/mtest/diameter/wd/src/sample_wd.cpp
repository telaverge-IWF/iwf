 ...
 * LOG:
 *
 * ID: $Id: sample_wd.cpp,v 3.1 2008/03/31 10:33:14 nvijikumar Exp $
 ****************************************************************************/
#ident "$Id: sample_wd.cpp,v 3.1 2008/03/31 10:33:14 nvijikumar Exp $"

#include <dia_defines.h>
#include <diameter/Exceptions.h>
#include <diameter/wd/AA.h>
#include <diameter/wd/ReAuth.h>
#include <diameter/wd/SessionTermination.h>
 ...
 *  See Also:
 ****************************************************************************/
int HandleDiaAppMsg(const diameter::CommandImpl *impl, unsigned int &sIdxWd)
{
    try
    {
        // Update the receive statistics
        WdStats::GetWdStats()->UpdateRecvStats(impl->getCommandCode(),
                                               impl->isRequest());
 ...
 ****************************************************************************
 *  Purpose:
 *      This function displays the menu options for the WD interface
 *
 *  Input Parameters:
 *      None.
 *
...
int DisplayAppMenu()
{
    ...
    while(k)
    {
        do
        {
            cout << endl << "Select WD Command" <<endl << endl;
        ...
        try
        {
            switch (ret)
            {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    SendWdSessionTerminationRequest();
                    break;
                case 4:
                    SendWdAbortSessionRequest();
                    break;
                case 5:
                    SendWdDiameterEAPRequest();
                    break;
                case 99:
                    k=0;
                    break;
...
int GenerateLoad()
{
    ...
    while (true)
    {
        if (WdStats::sendTraffic)
        {
             ...
             */
            for (int i = 0; i < WdStats::burstSize; i++)
            {
             ...
             */
            ftime(&WdStats::current);
            TIMERS_USleep(WdStats::slpTime * 1000);

            /*
             * If the traffic duration (seconds) expires, update the stats
             */
            if ((WdStats::current.time - WdStats::starter.time) >=
                 WdStats::duration)
            {
                if (WdStats::firstTime)
                {
                    ftime(&WdStats::sendComplete);
                    WdStats::sendTraffic = ITS_FALSE;
                    WdStats::firstTime = false;
                    time(&tx);
...
{
    AppInitialize(WD_INTERFACE_APP_ID);
    AppMain(argc,argv);
