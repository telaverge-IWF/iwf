 ...
 * LOG:
 * LOG:
 * ID: $Id: DiameterEAPTest.cpp,v 1.1 2011/05/09 11:12:55 vkumara Exp $
 ****************************************************************************/
#ident "$Id: DiameterEAPTest.cpp,v 1.1 2011/05/09 11:12:55 vkumara Exp $"
 ...
 ****************************************************************************
 *  Purpose: This function sends the  WgDiameterEAPRequest to the
 *      diameter stack. Create the  WgDiameterEAPRequest object,
...
int SendWgDiameterEAPRequest()
{
    ...
    AllocateSession(WG_INTERFACE_APP_ID, sIdx, sessIdString);
    wgDerReq.setSessionId(diameter::base::SessionId(sessIdString));

    wgDerReq.setSessionIndex(sIdx);
    ...
    wgDerReq.setAuthApplicationId(authApplicationId);
    ...
    wgDerReq.setOriginHost(originHost);
    ...
    wgDerReq.setOriginRealm(originRealm);
    ...
    wgDerReq.setDestinationRealm(destinationRealm);
   ...
   */
    ...
    wgDerReq.addLocationInformation(lI);

    if (appMode == UNIT_TEST)
    cout<<wgDerReq;

    // Send object to stack.
    SendDiaAppMsg(wgDerReq, sIdx);

    WgStats::GetWgStats()->UpdateSendStats(wgDerReq.getCommandCode(), ITS_TRUE);
    return ITS_SUCCESS;
}
/*.implementation:public
 ****************************************************************************
 *  Purpose:
 *      This function sends the Answer to the WgDiameterEAPRequest 
 *      made by the diameter stack. Create the WgDiameterEAPAnswer
 ...
 *  See Also:
 ****************************************************************************/
int SendWgDiameterEAPAnswer(const CommandImpl * impl)
{
    ...
    // Optional param error - whether this command carries an error indicator.
    // Optional param retrans - whether this is a retransmission.
    diameter::wg::DiameterEAPAnswer wgDerAns;

    // Create an instance of MultimediaAuthRequest class to get host and
    // session details
    DiameterEAPRequest wgDerreq(*impl);

    // Get AVP values from requested message
    const SessionId &sid1 = wgDerreq.getSessionId();
    const AuthApplicationId &aaid = wgDerreq.getAuthApplicationId();
    const AuthRequestType &authRequestType = wgDerreq.getAuthRequestType();

    const OriginHost &oh1 = wgDerreq.getOriginHost();
    const OriginRealm &or1 = wgDerreq.getOriginRealm();
    if (wgDerreq.containsDestinationHost())
        const DestinationHost &dh1 = wgDerreq.getDestinationHost();

    const DestinationRealm &dr1 = wgDerreq.getDestinationRealm();
    ...
    // The AVP Code of Session-Id is 263
    // The data type of Session-Id is UTF8String.
    wgDerAns.setSessionId(sid1);

    wgDerAns.setSessionIndex(wgDerreq.getSessionIndex());


    wgDerAns.setAuthApplicationId(aaid);

    wgDerAns.setAuthRequestType(authRequestType);
    ...
    // The data type of Origin-Host AVP is DiameterIdentity.
    OriginHost originHost(GetOriginHost());
    wgDerAns.setOriginHost(originHost);
    ...
    // The data type of Origin-Host AVP is DiameterIdentity.
    OriginRealm originRealm(GetOriginRealm());
    wgDerAns.setOriginRealm(originRealm);
    ...
    wgDerAns.setResultCode(resultCode);
    ...
    wgDerAns.addSubscriptionId(subscriptionId);

   //QoS-Resources Grouped AVP
    QoSResources qos; 
    FilterRule fr;
    Classifier cl;
    ClassifierID cid("VKUMARA@WG-INTERFACE");
    cl.setClassifierID(cid);
    fr.setClassifier(cl);
    qos.addFilterRule(fr);
    wgDerAns.setQoSResources(qos);

    if (appMode == UNIT_TEST)
    cout<<wgDerAns;

    // Send Object to stack.
    SendDiaAppMsg(wgDerAns, sIdx);

    // Update stats.
    WgStats::GetWgStats()->UpdateSendStats(wgDerAns.getCommandCode(), ITS_FALSE);
    return ITS_SUCCESS;
}
