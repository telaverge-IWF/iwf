 ...
 * LOG:
 * LOG:
 * ID: $Id: SessionTerminationTest.cpp,v 1.1 2011/05/09 11:12:55 vkumara Exp $
 ****************************************************************************/
#ident "$Id: SessionTerminationTest.cpp,v 1.1 2011/05/09 11:12:55 vkumara Exp $"
 ...
 *  See Also:
 ****************************************************************************/
int SendWgSessionTerminationRequest()
{
    ...
    diameter::wg::SessionTerminationRequest stReq(false,false);
    ...
    // This AVP  is used to indicate the reason why a session wgs terminated on
    // the access device.
    ...
    WgStats::GetWgStats()->UpdateSendStats(stReq.getCommandCode(),
                                                                ITS_TRUE);
    return ITS_SUCCESS;
}
 ...
 *  See Also:
 ****************************************************************************/
int SendWgSessionTerminationAnswer(const CommandImpl * impl1)
{
    ...
    // Optional param error - whether this command carries an error indicator.
    // Optional param retrans - whether this is a retransmission.
    diameter::wg::SessionTerminationAnswer sta(false,false);

    // Create an instance of RegistrationTerminationrequest class
    SessionTerminationRequest streq(*impl1);
    ...
    // The AVP Code of User-Name AVP is 1.
    // The data type of User-Name AVP is UTF8String.
    UserName uname("user@wg-network.com");
    sta.setUserName(uname);

    cout << sta;

    SendDiaAppMsg(sta, sIdx);
    WgStats::GetWgStats()->UpdateSendStats(sta.getCommandCode(), ITS_FALSE);
    return (ITS_SUCCESS);
}
