 ...
 *                                                                          *
 ****************************************************************************
 * LOG: $Log: gi_stats.cpp,v $
 * LOG: Revision 1.1  2011/05/26 08:57:24  vkumara
 * LOG: Baseline for Gi interface.
 * LOG:
 * LOG:
 *
 * ID: $Id: gi_stats.cpp,v 1.1 2011/05/26 08:57:24 vkumara Exp $
 ****************************************************************************/
#ident "$Id: gi_stats.cpp,v 1.1 2011/05/26 08:57:24 vkumara Exp $"
...
its::ITS_Mutex GiStats::guard(0);
ITS_BOOLEAN GiStats::sendTraffic = ITS_FALSE;
ITS_BOOLEAN GiStats::firstTime = ITS_FALSE;
time_t GiStats::startTime;
time_t GiStats::stopTime;
struct timeb GiStats::sendComplete;
struct timeb GiStats::recvComplete;
struct timeb GiStats::starter;
struct timeb GiStats::current;
int GiStats::duration = 10;
int GiStats::burstSize = 0;
int GiStats::slpTime = 1000;

static GiStats *giStats = NULL;
...
GiStats::GetGiStats()
{
    ...
    */
    ITS_ASSERT(GiStats::guard.Acquire() == ITS_SUCCESS);
 ...
 ****************************************************************************/
void
GiStats::CalcTPS(std::ostrstream& oss)
{
    Print(oss);
    ...
    if (numRaMsgsRecv == numRqMsgsSent)
    {
         /* Calculate tps
         */
         if ((GiStats::stopTime-GiStats::startTime) > 0 )
         {
             tps = (numRaMsgsRecv)/(GiStats::stopTime-GiStats::startTime);
         }
         else
         {
             //tps = (numRaMsgsRecv);
             tps = (1000/GiStats::slpTime) * GiStats::burstSize;
         }
         ...
         */
          ...
          */
         int st = GiStats::recvComplete.time - GiStats::sendComplete.time;
         int mt = GiStats::recvComplete.millitm - GiStats::sendComplete.millitm;
 ...
 *************************************************************************/
void
GiStats::UpdateSendStats(ITS_UINT commandCode, ITS_BOOLEAN isReq)
{
    ...
    */
	...
	case GI_AA_MSG_CMD_CODE:  /* AAR */
	    numAARSent++;
	    break;
        case GI_AC_MSG_CMD_CODE:  /* ACR */
            numACRSent++;
            break;
        
        case GI_ST_MSG_CMD_CODE:  /* STR */
            numSTRSent++;
            break;
        case GI_AS_MSG_CMD_CODE:  /* ASR */
            numASRSent++;
            break;
        default:
            break;
    ...
    */
    else
    {
        switch(commandCode)
        {
	case GI_AA_MSG_CMD_CODE:  /* AAA */
            numAAASent++;
            break;
        case GI_AC_MSG_CMD_CODE:  /* ACR */
            numACASent++;
            break;
	case GI_ST_MSG_CMD_CODE:  /* STA */
            numSTASent++;
            break;
        case GI_AS_MSG_CMD_CODE:  /*ASAns */
            numASASent++;
            break;
 ...
 ****************************************************************************/
void
GiStats::UpdateRecvIndications(ITS_UINT indic)
{
    switch (indic)
    {
 ...
 ****************************************************************************/
void
GiStats::UpdateRecvStats(ITS_UINT commandCode, ITS_BOOLEAN isReq)
{
    time_t tx;
    struct tm tbuf, *temp;
    char buff[80];
    ...
    */
	...
	case GI_AA_MSG_CMD_CODE:  /* AAA */
        ...
        */
        if (numRaMsgsRecv == 1)
        {
            time(&GiStats::startTime);
        }

        /*Update the stop time details in stats
        */
        if (!GiStats::sendTraffic &&
            (GiStats::numRqMsgsSent == GiStats::numRaMsgsRecv))
        {
            time(&GiStats::stopTime);
            ftime(&GiStats::recvComplete);
            time(&tx);
 ...
 ****************************************************************************/
void
GiStats::PrintIndications(std::ostream& os) const
{
 ...
 ****************************************************************************/
void
GiStats::Print(std::ostream& os) const
{
 ...
 ****************************************************************************/
ostream&
operator<<(ostream& os, const GiStats& giStats)
{
    giStats.Print(os);
    return os;
}
...
void StratTraffic(std::ostream& oss , int duration, int burstSize, int slpTime)
{
    ...
    GiStats::GetGiStats()->ResetAll();

    /*
     * Set the duration, burst size and the sleep time
     */
    GiStats::duration = duration;
    GiStats::burstSize = burstSize;
    GiStats::slpTime = slpTime;

    /*
     * Store the start time
     */
    ftime(&GiStats::starter);
    time(&GiStats::startTime);

    GiStats::sendTraffic = ITS_TRUE;
    GiStats::firstTime = ITS_TRUE;
    time(&tx);
    temp =  localtime_r(&tx, &tbuf);
    strftime(buff, 80, "[ %b%d %Y %H:%M:%S  ]" , &tbuf);
     ...
     */
    TrafficGeneratorFunc = SendGiAARequest;
...
{
    GiStats::GetGiStats()->UpdateRecvIndications(indType);
}

void CalculateTPS(std::ostrstream& oss)
{
    GiStats::GetGiStats()->CalcTPS(oss);
}
