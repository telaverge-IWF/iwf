 ...
 * LOG:
 * LOG:
 * ID: $Id: app_common.cpp,v 3.1 2008/03/31 10:33:06 nvijikumar Exp $
 ***************************************************************************/
#ident "$Id: app_common.cpp,v 3.1 2008/03/31 10:33:06 nvijikumar Exp $"
...
extern IDSCLApi smplApi;

extern unsigned int sendAppId;
extern unsigned int receivedAppId;
extern unsigned int regAppId;

ITS_OCTET msg[1024] = "\0";
...
int
SendDiaAppMsg(diameter::Command& impl, ITS_UINT sesIdx)
{
    return smplApi.Send(impl, sesIdx, sendAppId); 
}

int 
AllocateSession(ITS_UINT appId, ITS_UINT &sessIdx, std::string &sessString)
{
    return smplApi.AllocateSessionIndex(appId,sessIdx,sessString);
}

void 
ReleaseSession(std::string &sessString, ITS_UINT sessIdx)
{
    smplApi.ReleaseSession(sessString, sessIdx);
}

bool 
IsDccaEnabled()
{
    return smplApi.IsDccaEnabled();
}
