 ...
 * LOG:
 *
 * ID: $Id: SubscribeNotificationsTest.cpp,v 3.1 2008/03/31 10:33:13 nvijikumar Exp $
 ****************************************************************************/
#ident "$Id: SubscribeNotificationsTest.cpp,v 3.1 2008/03/31 10:33:13 nvijikumar Exp $"
...
{
    // 1. Create an instance of SubscribeNotificationsRequest class
    // The Subscribe-Notifications-Request command is sent from a Diameter
    ...
    SubscribeNotificationsRequest snReq(false,false);
    ...
    snReq.setSessionId(diameter::base::SessionId(sessIdString));

    snReq.setSessionIndex(sIdx);
    ...
    snReq.setVendorSpecificApplicationId(vendorSpecificApplicationId1);
    ...
    snReq.setAuthSessionState(authSessionState);
    ...
    snReq.setOriginHost(originHost);
    ...
    snReq.setOriginRealm(originRealm);
    ...
    snReq.setDestinationHost(destinationHost);
    ...
    snReq.setDestinationRealm(destinationRealm);


    UserIdentity userIdentity;
    PublicIdentity publicIdentity("sip:user@3gppnetwork.com");
    userIdentity.setPublicIdentity(publicIdentity);
    snReq.setUserIdentity(userIdentity);


    SubsReqType subsReqType(SubsReqType::SUBSCRIBE);
    snReq.setSubsReqType(subsReqType);


    DataReference dataReference(DataReference::IMSPUBLICIDENTITY);
    snReq.setDataReference(dataReference);

    cout<<snReq;

    SendDiaAppMsg(snReq, sIdx);
    ShStats::GetShStats()->UpdateSendStats(snReq.getCommandCode(), ITS_TRUE);
    return ITS_SUCCESS;
}
...
int SendShSubscribeNotificationsAnswer(const CommandImpl * impl1)
{
    // 1. Create an instance of SubscribeNotificationsAnswer class
    // The Subscribe-Notifications-Answer command is sent by a server
    // in response to the Registration-Termination-Request command with
    // Command-Code = 304 and 'R' bit reset
    // Optional param error - whether this command carries an error indicator.
    // Optional param retrans - whether this is a retransmission.
    SubscribeNotificationsAnswer snAns(false, false);

   // Create an instance of RegistrationTerminationrequest class
    SubscribeNotificationsRequest snReq(*impl1);

    // Get some AVP values from requested Message and set them to Answer
    const SessionId &sid1 = snReq.getSessionId();
    const OriginHost &oh1 = snReq.getOriginHost();
    const OriginRealm &or1 = snReq.getOriginRealm();
    ...
    // The AVP Code of Session-Id is 263
    // The data type of Session-Id is UTF8String.
    snAns.setSessionId(sid1);

    snAns.setSessionIndex(snReq.getSessionIndex());
    ...
    snAns.setVendorSpecificApplicationId(vendorSpecificApplicationId1);
    ...
    AuthSessionState authSessionState(AuthSessionState::STATE_MAINTAINED);
    snAns.setAuthSessionState(authSessionState);
    ...
    snAns.setOriginHost(originHost);
    ...
    snAns.setOriginRealm(originRealm);
    ...
    snAns.setResultCode(resultCode);
    ...
    // snAns.setExperimentalResult(experimentalResult);
    ...
    // snAns.addProxyInfo(proxyInfo1);
    // snAns.addProxyInfo(proxyInfo2);
    ...
    // Set Route Record
    // snAns.addRouteRecord(routeRecord1);
    // snAns.addRouteRecord(routeRecord2);
    ...
    SendDiaAppMsg(snAns, sIdx);
    ShStats::GetShStats()->UpdateSendStats(snAns.getCommandCode(), ITS_FALSE);
    return (ITS_SUCCESS);
}
