 ...
 * LOG:
 *
 * ID: $Id: ProfileUpdateTest.cpp,v 3.1 2008/03/31 10:33:13 nvijikumar Exp $
 ****************************************************************************/
#ident "$Id: ProfileUpdateTest.cpp,v 3.1 2008/03/31 10:33:13 nvijikumar Exp $"
...
{
    // [1]. create an instance of ProfileUpdateRequest.
    // Optional param error - whether this command carries an error indicator.
    // Optional param retrans - whether this is a retransmission.
    diameter::sh_3gpp2::ProfileUpdateRequest puReq;
   ...
   // Create a Vendor-Specific-Application-Id AVP object
    ...
    puReq.setVendorSpecificApplicationId(vendorSpecificApplicationId1);
    ...
    puReq.setUserIdentity(userIdentity);



    DataReference dataReference(DataReference::IMSPUBLICIDENTITY);
    puReq.setDataReference(dataReference);

    ShUserData shUserData("UDR_SNR_PUR");
    puReq.setShUserData(shUserData);


    // puReq.setWLANUserData(WLANUserData(...));
    ...
    ShStats::GetShStats()->UpdateSendStats(puReq.getCommandCode(), ITS_TRUE);
 ...
 *  See Also:
 ****************************************************************************/
int SendShProfileUpdateAnswer(const CommandImpl * impl1)
{

    // 1. Create an instance of ProfileUpdateAnswer class
    // The Profile-Update-Answer command is sent by a server
    // in response to the Profile-Update-Request command with
    // Command-Code = 305 and 'R' bit reset
    // Optional param error - whether this command carries an error indicator
    // Optional param retrans - whether this is a retransmission
    diameter::sh_3gpp2::ProfileUpdateAnswer puAns;

    // Create an instance of PushProfileRequest class
    ProfileUpdateRequest pureq(*impl1);
    ...
    //[2.1] Set the  Vendor-Specific-Application-Id AVP.
    puAns.setVendorSpecificApplicationId(vsAppId);
    ...
    // [2.1] Set the Auth-Session-State AVP.
    AuthSessionState authSessionState(AuthSessionState::STATE_MAINTAINED);
    puAns.setAuthSessionState(authSessionState);
    ...
    // puAns.addProxyInfo(diameter::base::ProxyInfo(...));
    // puAns.addRouteRecord(diameter::base::RouteRecord(...));
    // add more AVP as ANY values

    cout<<puAns;

    SendDiaAppMsg(puAns, sIdx);
    ShStats::GetShStats()->UpdateSendStats(puAns.getCommandCode(), ITS_FALSE);
