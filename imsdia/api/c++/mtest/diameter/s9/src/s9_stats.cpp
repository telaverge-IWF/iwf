/**************************-*-Dia-*-*************************************
 *                                                                          *
 *             Copyright 2010 IntelliNet Technologies, Inc.                 *
 *                            All Rights Reserved.                          *
 ...
 ****************************************************************************
 *
 *  ID: $Id: s9_stats.cpp,v 1.1 2010/04/22 11:12:53 nvijikumar Exp $
 *
 *  LOG: $Log: s9_stats.cpp,v $
 ...
 *
 ****************************************************************************/
#ident "$Id: s9_stats.cpp,v 1.1 2010/04/22 11:12:53 nvijikumar Exp $"
...
its::ITS_Mutex S9Stats::guard(0);
ITS_BOOLEAN S9Stats::sendTraffic = ITS_FALSE;
ITS_BOOLEAN S9Stats::firstTime = ITS_FALSE;
time_t S9Stats::startTime;
time_t S9Stats::stopTime;
struct timeb S9Stats::sendComplete;
struct timeb S9Stats::recvComplete;
struct timeb S9Stats::starter;
struct timeb S9Stats::current;
int S9Stats::duration = 10;
int S9Stats::burstSize = 0;
int S9Stats::slpTime = 1000;

static S9Stats *s9Stats = NULL;
...
S9Stats::GetS9Stats()
{
    ...
    */
    ITS_ASSERT(S9Stats::guard.Acquire() == ITS_SUCCESS);
 ...
 ****************************************************************************/
void
S9Stats::CalcTPS(std::ostrstream& oss)
{
    Print(oss);
    ...
    if (numRaMsgsRecv == numRqMsgsSent)
    {
         /* Calculate tps
         */
         if ((S9Stats::stopTime-S9Stats::startTime) > 0 )
         {
             tps = (numRaMsgsRecv)/(S9Stats::stopTime-S9Stats::startTime);
         }
         else
         {
             //tps = (numRaMsgsRecv);
             tps = (1000/S9Stats::slpTime) * S9Stats::burstSize;
         }
         ...
         */
          ...
          */
         int st = S9Stats::recvComplete.time - S9Stats::sendComplete.time;
         int mt = S9Stats::recvComplete.millitm - S9Stats::sendComplete.millitm;
 ...
 *************************************************************************/
void
S9Stats::UpdateSendStats(ITS_UINT commandCode, ITS_BOOLEAN isReq)
{
    ...
    */
    if (isReq)
    {
        switch(commandCode)
        {
        case S9_RA_MSG_CMD_CODE:  /*ReAuth */
            numRARSent++;
            break;
        case S9_CC_MSG_CMD_CODE:  /* CC */
            numCCRSent++;
            break;
        default:
            break;
 ...
 ****************************************************************************/
void
S9Stats::UpdateRecvIndications(ITS_UINT indic)
{
    switch (indic)
    {
 ...
 ****************************************************************************/
void
S9Stats::UpdateRecvStats(ITS_UINT commandCode, ITS_BOOLEAN isReq)
{
    time_t tx;
    struct tm tbuf, *temp;
    char buff[80];
    ...
    */
    else
    {
        ...
        */
        if (numRaMsgsRecv == 1)
        {
            time(&S9Stats::startTime);
        }

        /*Update the stop time details in stats
        */
        if (!S9Stats::sendTraffic && 
            (S9Stats::numRqMsgsSent == S9Stats::numRaMsgsRecv))
        {
            time(&S9Stats::stopTime);
            ftime(&S9Stats::recvComplete);
            time(&tx);
 ...
 ****************************************************************************/
void
S9Stats::PrintIndications(std::ostream& os) const
{
 ...
 ****************************************************************************/
void
S9Stats::Print(std::ostream& os) const
{
 ...
 ****************************************************************************/
ostream&
operator<<(ostream& os, const S9Stats& s9Stats)
{
    s9Stats.Print(os);
    return os;
}
...
void StratTraffic(std::ostream& oss , int duration, int burstSize, int slpTime)
{
    ...
    S9Stats::GetS9Stats()->ResetAll();

    /*
     * Set the duration, burst size and the sleep time
     */
    S9Stats::duration = duration;
    S9Stats::burstSize = burstSize;
    S9Stats::slpTime = slpTime;

    /*
     * Store the start time
     */
    ftime(&S9Stats::starter);
    time(&S9Stats::startTime);

    S9Stats::sendTraffic = ITS_TRUE;
    S9Stats::firstTime = ITS_TRUE;
    time(&tx);
    temp =  localtime_r(&tx, &tbuf);
    strftime(buff, 80, "[ %b%d %Y %H:%M:%S  ]" , &tbuf);
     ...
     */
    TrafficGeneratorFunc = SendS9CCRequest;
...
{
    S9Stats::GetS9Stats()->UpdateRecvIndications(indType);
}

void CalculateTPS(std::ostrstream& oss)
{
    S9Stats::GetS9Stats()->CalcTPS(oss);
}
