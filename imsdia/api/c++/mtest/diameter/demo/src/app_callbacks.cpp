 ...
 * LOG:
 *
 * ID: $Id: app_callbacks.cpp,v 1.1.2.1 2013/03/04 04:44:05 pramana Exp $
 ****************************************************************************/
#ident "$Id: app_callbacks.cpp,v 1.1.2.1 2013/03/04 04:44:05 pramana Exp $"
...
//    RegisterApplicationWithStack(regAppId);
    ...
    if (workerPool->GetFirstAvailThread(ent) == ITS_SUCCESS)
    {
        ...
        if (work)
        {
            ret = workerPool->DispatchOnThread(ent,  (ITS_ThreadCallFunc)DisplayCursesThreadFunc, NULL);
