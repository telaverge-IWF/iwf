 ...
 * LOG: Revision 1.1.2.14  2014/05/06 13:09:12  jkchaitanya
 * LOG: changes for ccfh feature
 * LOG:
 * LOG: Revision 1.1.2.13  2014/04/22 10:09:21  jsarvesh
 * LOG: Changes done for Encoding Connect AVP mapping
 * LOG:
 * LOG: Revision 1.1.2.12  2014/04/18 15:46:57  rsandhya
 * LOG: Removed reference of CAP-Connect-Original-Called-Party-ID AVP
 * LOG:
 * LOG: Revision 1.1.2.11  2014/04/18 12:50:08  jsarvesh
 * LOG: Changes done for Bug 3655
 * LOG:
 * LOG: Revision 1.1.2.10  2014/03/25 07:27:03  jkchaitanya
 * LOG: changes for supporting followon calls
 * LOG:
 * LOG: Revision 1.1.2.9  2014/03/24 11:34:05  jsarvesh
 * LOG: Changed the names of AVPs as per new Protocol file
 * LOG:
 * LOG: Revision 1.1.2.8  2014/03/20 05:58:39  jsarvesh
 * LOG: Added one AVP to Encode
 * LOG:
 * LOG: Revision 1.1.2.7  2014/03/11 06:23:16  jsarvesh
 * LOG: Modified the values for CAP-Connect-Info
 * LOG:
 * LOG: Revision 1.1.2.6  2014/03/09 12:16:12  jsarvesh
 * LOG: Chnage to Encode RRB-Info
 * LOG:
 * LOG: Revision 1.1.2.5  2014/02/25 12:07:21  jsarvesh
 * LOG: Added CAPReleaseCause AVP to json
 * LOG:
 * LOG: Revision 1.1.2.4  2014/02/20 08:42:04  jkchaitanya
 * LOG: changes for bug 3093
 * LOG:
 * LOG: Revision 1.1.2.3  2014/02/13 14:05:14  jkchaitanya
 * LOG: changes to comment hardcoded for sms avps
 * LOG:
 * LOG: Revision 1.1.2.2  2014/02/13 13:01:34  jkchaitanya
 * LOG: changes for removing hardcoding of ana avp
 * LOG:
 * LOG: Revision 1.1.2.1  2014/02/12 07:44:05  millayaraja
 * LOG: added new interface for IWF ro_r10_transatel
 * LOG:
 * LOG: Revision 1.1.2.8  2013/09/06 12:52:14  jkchaitanya
 * LOG: changes for the bug 2185
 * LOG:
 * LOG: Revision 1.1.2.7  2013/08/13 11:27:27  millayaraja
 * LOG: removed the MBMSServiceArea from CCA
 * LOG:
 * LOG: Revision 1.1.2.6  2013/08/06 13:03:57  jkchaitanya
 * LOG: changes to support sleep time before sending CCA messages for bug 2018
 * LOG:
 * LOG: Revision 1.1.2.5  2013/07/25 14:14:52  jkchaitanya
 * LOG: changes for the bug 1901
 * LOG:
 * LOG: Revision 1.1.2.4  2013/07/25 09:59:04  jkchaitanya
 * LOG: changes for the bug 1938, change in the name in json for CCA messages
 * LOG:
 * LOG: Revision 1.1.2.3  2013/07/22 09:43:17  jkchaitanya
 * LOG: changes for the bug 1901
 * LOG:
 ...
 * LOG:
 * LOG: Revision 1.1.2.1.2.3.28.2  2013/06/13 05:14:59  nazad
 * LOG: Changes for getting the session-id
 * LOG:
 * LOG: Revision 1.1.2.1.2.3.28.1  2013/06/06 05:51:50  vsarath
 * LOG: Added for the new call flow for load
 * LOG:
 * LOG: Revision 1.1.2.1.2.3  2012/09/28 07:51:29  pramana
 * LOG: Backing out GB demo changes
 * LOG:
 * LOG: Revision 1.1.2.1.2.2  2012/09/28 07:23:56  pramana
 * LOG: Checkins for the changes done for GenBand demo
 * LOG:
 * LOG: Revision 1.1.2.1.2.1  2012/09/04 02:51:35  pramana
 * LOG: Merging the test application fixes and TLS support for test tools.
 * LOG:
 * LOG: Revision 1.1.2.2  2012/08/08 08:34:41  ncshivakumar
 * LOG: Code changes for Bug: 110
 * LOG:
 ...
 * LOG:
 *
 * ID: $Id: CCTest.cpp,v 1.1.2.14 2014/05/06 13:09:12 jkchaitanya Exp $
 ****************************************************************************/
...
using namespace diameter::transatel_custom_avps;
using namespace diameter::ro_r10_transatel;

//Setting by default CCRequest Type as INITIAL_REQUEST to differentiate parsing for json
int CCRequestTypeReceived = 1;
bool DisableUnwantedAVPsForTransatel = false;
int SCPActioninCCAIForFollowOn = 0;
int ControlModeinCCAIForFollowOn = 0;
bool FirstCCAISentForFollowOn = false;

string sessIdString;
ITS_UINT sIdx;

int
SendCreditControlRequestInitial()
{
    return SendRoCreditControlRequest(INITIAL);
}
 ...
 *  See Also:
 ****************************************************************************/
int SendRoCreditControlRequest(int requestType,
                               ...
                               std::string aSessIdString)
{
    ...
    {
        num =0;
        AllocateSession(RO_INTERFACE_APP_ID,sIdx,sessIdString);
	...
	{
        	ccr.setSessionId(SessionId(aSessIdString));
	}
    ...
    AuthApplicationId authApplicationId(RO_INTERFACE_APP_ID);
	...
	{
		CreditControlAnswer cca(*impl);
		DestinationHost destinationHost(cca.getOriginHost().value().c_str());
		ccr.setDestinationHost(destinationHost);
	}
    }
    ...
    RoStats::GetRoStats()->UpdateSendStats(ccr.getCommandCode(), ITS_TRUE);
    return ITS_SUCCESS;
 ...
 *  See Also:
 ****************************************************************************/
int SendRoCreditControlAnswer(const diameter::CommandImpl* impl ) 
{
    bool isNull;
    bool isAVPEnabled;
    int resultCode = ITS_DIA_SUCCESS;
    bool isError = false;
    ...
    {
        CCRequestTypeReceived = 1;
        cfg = &(*m_jsonRoot1)["CreditControlAnswer"]["CCA_I"];
    }
    else if(ccRequestType.value() == CCRequestType::UPDATE_REQUEST)
    {
        CCRequestTypeReceived = 2;
        cfg = &(*m_jsonRoot1)["CreditControlAnswer"]["CCA_U"];
    }
    else if(ccRequestType.value() == CCRequestType::TERMINATION_REQUEST)
    {
        CCRequestTypeReceived = 3;
        cfg = &(*m_jsonRoot1)["CreditControlAnswer"]["CCA_T"];
    }

    isNull = (*cfg)["ResultCode"]["Value"].isNull();
    if(!isNull)
    {
        resultCode = ((*cfg)["ResultCode"]["Value"].asInt());
        if(resultCode > ITS_DIA_SUCCESS)
            isError = true;
    }

    CreditControlAnswer cca(isError,false);
    
    ResultCode rCode(resultCode);
    cca.setResultCode(rCode);
...
#if 0
    cfg = &(*m_jsonRoot1)["DisableUnwantedAVPsForTransatel"];
    isAVPEnabled = (*cfg)["Enabled"].asBool();
    isNull = (*cfg)["Enabled"].isNull();
    if(!isNull && isAVPEnabled)
    {
        DisableUnwantedAVPsForTransatel = isAVPEnabled;
        cout<< " DisableUnwantedAVPsForTransatel " << DisableUnwantedAVPsForTransatel <<endl;
    }
#endif    

    //NOTE: setting sleep time to simulate timeout scenarios
    isAVPEnabled = (*cfg)["SleepTime"]["Enabled"].asBool();
    isNull = (*cfg)["SleepTime"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int val = (*cfg)["SleepTime"]["Value"].asInt();
        sleep(val);
    }
    ...
    isAVPEnabled = (*cfg)["CCSessionFailover"]["Enabled"].asBool();
    isNull = (*cfg)["CCSessionFailover"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int failover = (*cfg)["CCSessionFailover"]["Value"].asInt();
        CCSessionFailover Failover((CCSessionFailover::Value)failover);
        cca.setCCSessionFailover(Failover);
    }
    ...
    isAVPEnabled = (*cfg)["CreditControlFailureHandling"]["Enabled"].asBool();
    isNull = (*cfg)["CreditControlFailureHandling"]["Value"].empty();
    if(!isNull && isAVPEnabled)
    {
        int failure = (*cfg)["CreditControlFailureHandling"]["Value"].asInt();
        CreditControlFailureHandling Failure((CreditControlFailureHandling::Value)failure);
        cca.setCreditControlFailureHandling(Failure);
    }

    isAVPEnabled = (*cfg)["OriginStateId"]["Enabled"].asBool();
    isNull = (*cfg)["OriginStateId"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        OriginStateId Id((*cfg)["OriginStateId"]["Value"].asInt());
        cca.setOriginStateId(Id);
    }
    ...
    if (appMode == UNIT_TEST)
    {
...
#if 0
        if(ccr.getCCRequestType().value() != CCRequestType::TERMINATION_REQUEST)
        {
        ...
        // service specific information elements
        // The AVP Code of Service-Information is 873.
        if(!DisableUnwantedAVPsForTransatel)
        {   
            ServiceInformation serviceInfo;
        ...
        //Set LowBalanceIndication
        isAVPEnabled = (*cfg)["LowBalanceIndication"]["Enabled"].asBool();
        isNull = (*cfg)["LowBalanceIndication"]["Value"].isNull();
        if(!isNull && isAVPEnabled)
        {
            int val = (*cfg)["LowBalanceIndication"]["Value"].asInt();
            if(val == 0)
            {
                LowBalanceIndication lbi(LowBalanceIndication::NOT_APPLICABLE);
                cca.setLowBalanceIndication(lbi);
            }
            else if(val == 1)
            {   
                LowBalanceIndication lbi(LowBalanceIndication::YES);
                cca.setLowBalanceIndication(lbi);
            }
        }

        ServiceControl serviceCtrl;
        // Populate ServiceControl AVP grouped AVPs
        PopulateServiceControl(serviceCtrl,false);
        cca.setServiceControl(serviceCtrl);

        cout << cca << endl;
    }

    SendDiaAppMsg(cca, sIdx);
    RoStats::GetRoStats()->UpdateSendStats(cca.getCommandCode(),
                                                            ITS_FALSE);
    ...
    FirstCCAISentForFollowOn = true;
    return ITS_SUCCESS;

}

void PopulateServiceControl(ServiceControl &serviceCtrl, bool isRequest=true)
{
    bool isNull;
    bool isAVPEnabled;
    Json::Value l_jsonRoot = parseS6aConfig();
    ...
    Json::Value* m_jsonRoot = &l_jsonRoot;
    Json::Value* m_jsonRoot1 = &(*m_jsonRoot)["ro_r10"];
    Json::Value* cfgAns = NULL;
    if(CCRequestTypeReceived == 1 && !isRequest)
        cfgAns = &(*m_jsonRoot1)["CreditControlAnswer"]["CCA_I"];
    else if(CCRequestTypeReceived == 2 && !isRequest)
        cfgAns = &(*m_jsonRoot1)["CreditControlAnswer"]["CCA_U"];
    else if(CCRequestTypeReceived == 3 && !isRequest)
        cfgAns = &(*m_jsonRoot1)["CreditControlAnswer"]["CCA_T"];
    
    if((CCRequestTypeReceived == 1))
    {
        isAVPEnabled = (*cfgAns)["ServiceControl"]["FollowOnCallFlow"]["Enabled"].asBool();
        isNull = (*cfgAns)["ServiceControl"]["FollowOnCallFlow"]["Enabled"].isNull();
        if(!isNull && isAVPEnabled)
        {
            isAVPEnabled = (*cfgAns)["ServiceControl"]["FollowOnCallFlow"]["SCPAction"]["Enabled"].asBool();
            isNull = (*cfgAns)["ServiceControl"]["FollowOnCallFlow"]["SCPAction"]["Value"].isNull();
            if(!isNull && isAVPEnabled)
            {
                 SCPActioninCCAIForFollowOn = (*cfgAns)["ServiceControl"]["FollowOnCallFlow"]["SCPAction"]["Value"].asInt();
            }

            isAVPEnabled = (*cfgAns)["ServiceControl"]["FollowOnCallFlow"]["CAPControlMode"]["Enabled"].asBool();
            isNull = (*cfgAns)["ServiceControl"]["FollowOnCallFlow"]["CAPControlMode"]["Value"].isNull();
            if(!isNull && isAVPEnabled)
            {
                 ControlModeinCCAIForFollowOn = (*cfgAns)["ServiceControl"]["FollowOnCallFlow"]["CAPControlMode"]["Value"].asInt();
            }
        }
    }

    if(FirstCCAISentForFollowOn && (CCRequestTypeReceived == 1))
    {
        SCPAction action((SCPAction::Value)SCPActioninCCAIForFollowOn);
        serviceCtrl.setSCPAction(action);
    }
    else
    {
        isAVPEnabled = (*cfgAns)["ServiceControl"]["SCPAction"]["Enabled"].asBool();
        isNull = (*cfgAns)["ServiceControl"]["SCPAction"]["Value"].isNull();
        if(!isNull && isAVPEnabled)
        {
            int val = (*cfgAns)["ServiceControl"]["SCPAction"]["Value"].asInt();
            SCPAction action((SCPAction::Value)val);
            serviceCtrl.setSCPAction(action);
        }
    }
    CAPInformation capInfo;
    
    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPRRBInformation"]["CAPRRBNoAnswerTimeOut"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPRRBInformation"]["CAPRRBNoAnswerTimeOut"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int timeout = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPRRBInformation"]["CAPRRBNoAnswerTimeOut"]["Value"].asInt();
        CAPRRBSInformation rrbInfo;
        CAPRRBSNoAnswerTimeout rrbtimeout(timeout);
        rrbInfo.setCAPRRBSNoAnswerTimeout(rrbtimeout);
        capInfo.setCAPRRBSInformation(rrbInfo);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPReleaseCause"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPReleaseCause"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int releaseCause = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPReleaseCause"]["Value"].asInt();

        CAPReleaseCause capReleaseCause(releaseCause);
        capInfo.setCAPReleaseCause(capReleaseCause);
    }

   
    if(FirstCCAISentForFollowOn && (CCRequestTypeReceived == 1))
    {
        CAPControlMode ctrlMode((CAPControlMode::Value)ControlModeinCCAIForFollowOn);
        capInfo.setCAPControlMode(ctrlMode);
        FirstCCAISentForFollowOn = false;
    }
    else
    {
        isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPControlMode"]["Enabled"].asBool();
        isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPControlMode"]["Value"].isNull();
        if(!isNull && isAVPEnabled)
        {
            int controlmode = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPControlMode"]["Value"].asInt();
            CAPControlMode ctrlMode((CAPControlMode::Value)controlmode);
            capInfo.setCAPControlMode(ctrlMode);
        }
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPACHInformation"]["CAPACHBeepTone"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPACHInformation"]["CAPACHBeepTone"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        CAPACHInformation achInfo;
        int beeptone = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPACHInformation"]["CAPACHBeepTone"]["Value"].asInt();
        CAPACHBeepTone tone((CAPACHBeepTone::Value)beeptone);
        achInfo.setCAPACHBeepTone(tone);
        capInfo.setCAPACHInformation(achInfo);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["Enabled"].isNull();
    if(!isNull && isAVPEnabled)
    {
        CAPConnectInformation CAPConnectInfo;
        PopulateCAPConnectInformation(CAPConnectInfo, isRequest);
        capInfo.setCAPConnectInformation(CAPConnectInfo); 
    }

    serviceCtrl.setCAPInformation(capInfo);
}

void PopulateCAPConnectInformation(CAPConnectInformation &CAPConnectInfo, bool isRequest=true)
{
    bool isNull;
    bool isAVPEnabled;
    Json::Value l_jsonRoot = parseS6aConfig();
    ...
    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectDestinationNumber"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectDestinationNumber"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        std::string dest_num =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectDestinationNumber"]["Value"].asString();
        CAPConnectInfo.setCAPConnectDestinationNumber((CAPConnectDestinationNumber)dest_num);
    }
    
    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectDestinationNumberNature"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectDestinationNumberNature"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int dest_num_nature =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectDestinationNumberNature"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectDestinationNumberNature((CAPConnectDestinationNumberNature)dest_num_nature);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectDestinationNumberNPI"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectDestinationNumberNPI"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int number_npi =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectDestinationNumberNPI"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectDestinationNumberNPI((CAPConnectDestinationNumberNPI)number_npi);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumber"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumber"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        std::string connect_gen_num =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumber"]["Value"].asString();
        CAPConnectInfo.setCAPConnectGenericNumber((CAPConnectGenericNumber)connect_gen_num);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumberNOA"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumberNOA"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int number_noa =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumberNOA"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectGenericNumberNOA((CAPConnectGenericNumberNOA)number_noa);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumberNPI"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumberNPI"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int number_npi =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumberNPI"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectGenericNumberNPI((CAPConnectGenericNumberNPI)number_npi);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumberNII"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumberNII"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int number_nii =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumberNII"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectGenericNumberNII((CAPConnectGenericNumberNII)number_nii);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumberNQI"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumberNQI"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int number_nqi =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumberNQI"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectGenericNumberNQI((CAPConnectGenericNumberNQI)number_nqi);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumberARI"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumberARI"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int number_ari =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumberARI"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectGenericNumberARI((CAPConnectGenericNumberARI)number_ari);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumberSCI"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumberSCI"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int number_sci =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectGenericNumberSCI"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectGenericNumberSCI((CAPConnectGenericNumberSCI)number_sci);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectCallingCategory"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectCallingCategory"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int call_category =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectCallingCategory"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectCallingCategory((CAPConnectCallingCategory)call_category);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectingNumber"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectingNumber"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        std::string redirect_number =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectingNumber"]["Value"].asString();
        CAPConnectInfo.setCAPConnectRedirectingNumber((CAPConnectRedirectingNumber)redirect_number);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectingNumberNOA"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectingNumberNOA"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int number_noa =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectingNumberNOA"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectRedirectingNumberNOA((CAPConnectRedirectingNumberNOA)number_noa);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectingNumberNPI"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectingNumberNPI"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int number_noa =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectingNumberNPI"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectRedirectingNumberNPI((CAPConnectRedirectingNumberNPI)number_noa);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectingNumberARI"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectingNumberARI"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int number_noa =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectingNumberARI"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectRedirectingNumberARI((CAPConnectRedirectingNumberARI)number_noa);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectingIndicator"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectingIndicator"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int redirect_indi =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectingIndicator"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectRedirectingIndicator((CAPConnectRedirectingIndicator)redirect_indi);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectOriginalRedirectionReason"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectOriginalRedirectionReason"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int redirect_reason =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectOriginalRedirectionReason"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectOriginalRedirectingReason((CAPConnectOriginalRedirectingReason)redirect_reason);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectionCounter"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectionCounter"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int counter =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectionCounter"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectRedirectingCounter((CAPConnectRedirectingCounter)counter);
    }
    
    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectingReason"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectingReason"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int reason =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectRedirectingReason"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectRedirectingReason((CAPConnectRedirectingReason)reason);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectoCSIApplicable"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectoCSIApplicable"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int csi_applicable =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectoCSIApplicable"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectoCSIApplicable((CAPConnectoCSIApplicable)csi_applicable);
    }
    
    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectSuppressionOfAnnouncement"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectSuppressionOfAnnouncement"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int supp =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectSuppressionOfAnnouncement"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectSuppressionOfAnnouncement((CAPConnectSuppressionOfAnnouncement)supp);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPAnnouncementAccessNumber"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPAnnouncementAccessNumber"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        std::string num =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPAnnouncementAccessNumber"]["Value"].asString();
        CAPConnectInfo.setCAPAnnouncementAccessNumber((CAPAnnouncementAccessNumber)num);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPAnnouncementAccessNumberNOA"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPAnnouncementAccessNumberNOA"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int num =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPAnnouncementAccessNumberNOA"]["Value"].asInt();
        CAPConnectInfo.setCAPAnnouncementAccessNumberNOA((CAPAnnouncementAccessNumberNOA)num);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectOriginalCalledNumber"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectOriginalCalledNumber"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        std::string num =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectOriginalCalledNumber"]["Value"].asString();
        CAPConnectInfo.setCAPConnectOriginalCalledNumber((CAPConnectOriginalCalledNumber)num);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectOriginalCalledNumberNOA"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectOriginalCalledNumberNOA"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int number_noa =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectOriginalCalledNumberNOA"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectOriginalCalledNumberNOA((CAPConnectOriginalCalledNumberNOA)number_noa);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectOriginalCalledNumberNPI"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectOriginalCalledNumberNPI"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int number_noa =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectOriginalCalledNumberNPI"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectOriginalCalledNumberNPI((CAPConnectOriginalCalledNumberNPI)number_noa);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectOriginalCalledNumberARI"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectOriginalCalledNumberARI"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int number_noa =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectOriginalCalledNumberARI"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectOriginalCalledNumberARI((CAPConnectOriginalCalledNumberARI)number_noa);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectDestinationNumberINN"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectDestinationNumberINN"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
       int num =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectDestinationNumberINN"]["Value"].asInt();
       CAPConnectInfo.setCAPConnectDestinationNumberINN((CAPConnectDestinationNumberINN)num);
    }

    isAVPEnabled = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectMaxDuration"]["Enabled"].asBool();
    isNull = (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectMaxDuration"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        int num =  (*cfgAns)["ServiceControl"]["CAPInformation"]["CAPConnectInformation"]["CAPConnectMaxDuration"]["Value"].asInt();
        CAPConnectInfo.setCAPConnectMaxDuration((CAPConnectMaxDuration)num);
    }
        ...
        &multipleServicesCC , bool isRequest=true)
{
    ...
    {
        //Setting Result-code
        isNull = (*cfgAns)["MultipleServicesCreditControl"]["ResultCode"]["Value"].isNull();

        if(!isNull)
        {
            ResultCode resultCode((*cfgAns)["MultipleServicesCreditControl"]["ResultCode"]["Value"].asInt()); 
            multipleServicesCC.setResultCode(resultCode);
        }
          ...
          (PSFurnishChargingInformation &psFurnishChrgInfo)
{
    ...
    Ro3GPPChargingId ps3gppChargingId(1500u);
    psFurnishChrgInfo.setRo3GPPChargingId(ps3gppChargingId);
...
/*  implementation: public
 ****************************************************************************
 *  Purpose: This function populates the UserCSGInformation  
 *           grouped AVP.
 *      This populate the grouped AVPs and sets the values in the
 *      UserCSGInformation object passed as a reference to this 
 *      function.
 *
 *  Input Parameters:
 *      UserCSGInformation &UserInfo: Reference to the
 *      UserCSGInformation object, grouped AVPs of which needs 
 *      to be set.
 *
 *  Input/Output Parameters:
 *      None.
 ...
 *  See Also:
 ****************************************************************************/
void PopulateUserCSGInformation  (UserCSGInformation &UserInfo)
{
    CSGId csg_id(1234);
    UserInfo.setCSGId(csg_id);

    CSGAccessMode csg_acc_mode(CSGAccessMode :: Hybrid_Mode);
    UserInfo.setCSGAccessMode(csg_acc_mode);

    CSGMembershipIndication csg_mem_ind (CSGMembershipIndication :: CSG_Member);
    UserInfo.setCSGMembershipIndication(csg_mem_ind);
...
void PopulateGrantedServiceUnit(GrantedServiceUnit &grantedServiceUnit)
{
    ...
    if(CCRequestTypeReceived == 1)
        cfg = &(*m_jsonRoot1)["CreditControlAnswer"]["CCA_I"]["MultipleServicesCreditControl"]["GrantedServiceUnit"];
    else if(CCRequestTypeReceived == 2)
        cfg = &(*m_jsonRoot1)["CreditControlAnswer"]["CCA_U"]["MultipleServicesCreditControl"]["GrantedServiceUnit"];
    else if(CCRequestTypeReceived == 3)
        cfg = &(*m_jsonRoot1)["CreditControlAnswer"]["CCA_T"]["MultipleServicesCreditControl"]["GrantedServiceUnit"];
    ...
    // This indicates the length of requested,granted or used time in seconds.
    // The AVP Code of CC-Time is 420.
    isAVPEnabled = (*cfg)["CCTime"]["Enabled"].asBool();
    isNull = (*cfg)["CCTime"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        CCTime ccTime((*cfg)["CCTime"]["Value"].asInt());
        grantedServiceUnit.setCCTime(ccTime);
    }
    // Set TariffTimeChange AVP,
    isAVPEnabled = (*cfg)["TariffTimeChange"]["Enabled"].asBool();
    isNull = (*cfg)["TariffTimeChange"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        TariffTimeChange TTtime((*cfg)["TariffTimeChange"]["Value"].asInt());
        grantedServiceUnit.setTariffTimeChange(TTtime);
    }
    isAVPEnabled = (*cfg)["CCServiceSpecificUnits"]["Enabled"].asBool();
    isNull = (*cfg)["CCServiceSpecificUnits"]["Value"].isNull();
    if(!isNull && isAVPEnabled)
    {
        CCServiceSpecificUnits ccServiceSpecificUnits((*cfg)["CCServiceSpecificUnits"]["Value"].asInt());
...
void PopulateFinalUnitIndication(FinalUnitIndication &finalUnitInd)
{
    ...
    Json::Value* cfg = NULL;
    if(CCRequestTypeReceived == 1)
        cfg = &(*m_jsonRoot1)["CreditControlAnswer"]["CCA_I"]["MultipleServicesCreditControl"]["FinalUnitIndication"];
    else if(CCRequestTypeReceived == 2)
        cfg = &(*m_jsonRoot1)["CreditControlAnswer"]["CCA_U"]["MultipleServicesCreditControl"]["FinalUnitIndication"];
    else if(CCRequestTypeReceived == 3)
        cfg = &(*m_jsonRoot1)["CreditControlAnswer"]["CCA_T"]["MultipleServicesCreditControl"]["FinalUnitIndication"];
    ...
    isNull = (*cfg)["FinalUnitAction"]["Value"].isNull();
    if(!isNull)
    {
        int val = (*cfg)["FinalUnitAction"]["Value"].asInt();
        //NOTE: Default Value for FinalUnitAction is TERMINATE 
        if(val == 1)
        {
        ...
        {
            FinalUnitAction finalUnitAction(FinalUnitAction::RESTRICT_ACCESS);
            finalUnitInd.setFinalUnitAction(finalUnitAction);
        }
        else
        {
            FinalUnitAction finalUnitAction(FinalUnitAction::TERMINATE);
            finalUnitInd.setFinalUnitAction(finalUnitAction);
        }
    }
...
void PopulatePSInformation(PSInformation &psInfo) 
{
    ...
    // GGSN-Address constitutes a unique identifier for the PDP context).
    // The AVP Code for 3GPP-Charging-Id is 2
    Ro3GPPChargingId ims3gppChargingId(1500u); 
    psInfo.setRo3GPPChargingId(ims3gppChargingId);
    ...
    Ro3GPPPDPType ims3gppPdpType(1);
    psInfo.setRo3GPPPDPType(ims3gppPdpType);
    ...
    // sETTING THE sgw ADDRESS 
    Address addr5(Address::IPV4, "192.168.4.1");
    SGWAddress sgwAddr(addr5);
    psInfo.setSGWAddress(sgwAddr);
    ...
    Ro3GPPIMSIMCCMNC imsiMccMnc("091080");
    psInfo.setRo3GPPIMSIMCCMNC(imsiMccMnc);
    ...
    Ro3GPPGGSNMCCMNC ggsnMccMnc("091040");
    psInfo.setRo3GPPGGSNMCCMNC(ggsnMccMnc);
    ...
    Ro3GPPNSAPI imsNSAPI("34"); // not clear
    psInfo.setRo3GPPNSAPI(imsNSAPI);
    ...
    Ro3GPPSessionStopIndicator sessStopInd("FF"); // not clear
    psInfo.setRo3GPPSessionStopIndicator(sessStopInd);
    ...
    Ro3GPPSelectionMode selectMode("FC"); // not clear
    psInfo.setRo3GPPSelectionMode(selectMode);
    ...
    Ro3GPPChargingCharacteristics chargingCharacter("F40F"); // not clear
    psInfo.setRo3GPPChargingCharacteristics(chargingCharacter);
    ...
    Ro3GPPSGSNMCCMNC sgsnMccMnc("091044");
    psInfo.setRo3GPPSGSNMCCMNC(sgsnMccMnc);
    ...
    Ro3GPPMSTimeZone msTimeZone("10");
    psInfo.setRo3GPPMSTimeZone(msTimeZone);
    ...
    Ro3GPPUserLocationInfo userLocation("CGI");
    psInfo.setRo3GPPUserLocationInfo(userLocation);
    ...
    Ro3GPPRATType ratType("WLAN"); 
    psInfo.setRo3GPPRATType(ratType);
    ...
    // Set Low Priority Indicator AVP.
    // The data type of Low-Priority-Indicator is enumerated.
    // The AVP Code  is 2602.
    LowPriorityIndicator PrioIndi(LowPriorityIndicator::YES); 
    psInfo.setLowPriorityIndicator(PrioIndi);

    UserCSGInformation UsgInfo;
    PopulateUserCSGInformation(UsgInfo);
    psInfo.setUserCSGInformation(UsgInfo);
}
...
void PopulateSDPMediaComponent(SDPMediaComponent &sdpMediaCompo)
{
    ...
    // The AVP Code for 3GPP-Charging-Id is 2.
    Ro3GPPChargingId ims3gppChargingId(1500u); 
    sdpMediaCompo.setRo3GPPChargingId(ims3gppChargingId);
...
void PopulateOriginatorAddress(OriginatorAddress &origAddr) 
{
    ...
    // The AVP Code for 3GPP-IMSI-MCC-MNC is 8.
    Ro3GPPIMSIMCCMNC imsiMccMnc("091080"); 
    addrDomain.setRo3GPPIMSIMCCMNC(imsiMccMnc);
...
//                 reqMBMSBearerCapb("3,1 kHz audio ex PLMN"); // -- not clear
    ...
    /*ro_r10::MBMSServiceArea mbmsServiceArea1("987654321");  //-- not clear
