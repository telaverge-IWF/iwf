 ...
 ****************************************************************************
 *
 * LOG: $Log: TariffRequestResponseTest.cpp,v $
 * LOG: Revision 1.1  2011/05/20 04:15:48  vkumara
 * LOG: Basline for Re interface.
 * LOG:
 * LOG:
 *
 * ID: $Id: TariffRequestResponseTest.cpp,v 1.1 2011/05/20 04:15:48 vkumara Exp $
 ****************************************************************************/
#ident "$Id: TariffRequestResponseTest.cpp,v 1.1 2011/05/20 04:15:48 vkumara Exp $"
 ...
 ****************************************************************************
 *  Purpose: This function sends the Tariff Request to the
 *      diameter stack. Creates the Tariff Request object,
 ...
 *  See Also:
 ****************************************************************************/
int SendReTariffRequest() 
{
    // [1]. Create an instance of TariffRequest class
    // The Tariff-Request command is sent by a Diameter
    // client to a Diameter Server in order to tariffRequest deregistration
    ...
    diameter::re::TariffRequest  tariffReq;
    ...
    tariffReq.setSessionId(SessionId(sessIdString));

    tariffReq.setSessionIndex(sIdx);
    ...
    tariffReq.setOriginRealm(origr);
    ...
    tariffReq.setDestinationRealm(desr);
   ...
   SubscriptionIdType subscriptionIdType
    ...
    tariffReq.setSubscriptionId(subscriptionId1);
    ...
    */
    ...
    tariffReq.setDestinationHost(destinationHost);
    ...
    tariffReq.setVendorSpecificApplicationId(vendorSpecificApplicationId1);
    ...
    tariffReq.setUserName(userName);

    if (appMode == UNIT_TEST)
    cout<<tariffReq;

    // Send encoded message to stack.
    SendDiaAppMsg(tariffReq, sIdx);
    ReStats::GetReStats()->UpdateSendStats(tariffReq.getCommandCode(), ITS_TRUE);
...
/*.implementation:public
 ****************************************************************************
 *  Purpose: This function sends the Tariff Response to the
 *      diameter stack. Creates the Tariff Response object,
 *      populate the AVPs and send the message to the stack.
 *
 *  Input Parameters:
 ...
 *  See Also:
 ****************************************************************************/
int SendReTariffResponse(const CommandImpl *impl) 
{
    //[ 1] Create an instance of TariffResponse class
    ...
    diameter::re::TariffResponse tariffResponse;

    // Create an instance of TariffRequest class 
    TariffRequest trreq(*impl);

    // Get the some AVP values and set them to Answer Message
    const SessionId& sid = trreq.getSessionId();
    const OriginHost& oh = trreq.getOriginHost();
    const OriginRealm& or1 = trreq.getOriginRealm();
    const DestinationRealm& dr = trreq.getDestinationRealm();

    // Fixed AVP

    // [2] Insert data in the created TariffResponse object.
    ...
    // The AVP Code of Session-Id is 263
    // The data type of Session-Id is UTF8String.
    tariffResponse.setSessionId(sid);

    tariffResponse.setSessionIndex(trreq.getSessionIndex());
    ...
    // The data type of Origin-Host AVP is DiameterIdentity.
    OriginHost origh(GetOriginHost());
    tariffResponse.setOriginHost(origh);
    ...
    tariffResponse.setOriginRealm(origrealm);
    ...
    */
    ...
    tariffResponse.addServiceRating(sR);
    ...
    //[2.1.2.1] Set the  Vendor-Specific-Application-Id AVP.
    tariffResponse.setVendorSpecificApplicationId(vendorSpecificApplicationId1);

    // The AVP Code of User-Name AVP is 1.
    // The data type of User-Name AVP is UTF8String.
    UserName userName("sip:vkumara@3gppnetwork.com");
    tariffResponse.setUserName(userName);
    ...
    // Send Encoded message to stack.
    SendDiaAppMsg(tariffResponse, sIdx);
    ReStats::GetReStats()->UpdateSendStats(tariffResponse.getCommandCode(), ITS_FALSE);
    return ITS_SUCCESS;
