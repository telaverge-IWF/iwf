 ...
 * LOG:
 *
 * ID: $Id: LocationInfoTest.cpp,v 3.1 2008/03/31 10:33:07 nvijikumar Exp $
 ****************************************************************************/
#ident "$Id: LocationInfoTest.cpp,v 3.1 2008/03/31 10:33:07 nvijikumar Exp $"
 ...
 ****************************************************************************
 *  Purpose: This function sends the LocationInfoRequest the
 ...
 *  See Also:
 ****************************************************************************/
int SendCxLocationInfoRequest()
{
    // 1. Create an instance of LocationInfoRequest class
    // The Location-Info-Request command is sent from a Diameter
    ...
    LocationInfoRequest liReq(false,false);
    ...
    liReq.setSessionId(diameter::base::SessionId(sessIdString));

    liReq.setSessionIndex(sIdx);
    ...
    liReq.setVendorSpecificApplicationId(vendorSpecificApplicationId1);
    ...
    liReq.setAuthSessionState(authSessionState);
    ...
    liReq.setOriginHost(originHost);
    ...
    liReq.setOriginRealm(originRealm);
    ...
    liReq.setDestinationHost(destinationHost);
    ...
    liReq.setDestinationRealm(destinationRealm);
    ...
    liReq.setPublicIdentity(publicIdentity);
    ...
    liReq.addSupportedFeatures(supportedFeatures);

    cout<<liReq;

    SendDiaAppMsg(liReq, sIdx);
    CxStats::GetCxStats()->UpdateSendStats(liReq.getCommandCode(), ITS_TRUE);
    return ITS_SUCCESS;
}
...
int SendCxLocationInfoAnswer(const CommandImpl * impl1)
{
    ...
    // Optional param error - whether this command carries an error indicator.
    // Optional param retrans - whether this is a retransmission.
    LocationInfoAnswer liAns(false, false);

   // Create an instance of LocationInfoRequest class
    ...
    const SupportedFeatures &supportedFeatures = rreq.getSupportedFeatures();
    // 2. Create the instances of the classes describing the various
    ...
    // The AVP Code of Session-Id is 263
    // The data type of Session-Id is UTF8String.
    liAns.setSessionId(sid1);

    liAns.setSessionIndex(rreq.getSessionIndex());
    ...
    liAns.setVendorSpecificApplicationId(vendorSpecificApplicationId1);
    ...
    AuthSessionState authSessionState(AuthSessionState::STATE_MAINTAINED);
    liAns.setAuthSessionState(authSessionState);
    ...
    liAns.setOriginHost(originHost);
    ...
    liAns.setOriginRealm(originRealm);
    ...
    liAns.setResultCode(resultCode);
    ...
    // liAns.setExperimentalResult(experimentalResult);
    ...
    // liAns.addProxyInfo(proxyInfo1);
    // liAns.addProxyInfo(proxyInfo2);
    ...
    // Set Route Record
    // liAns.addRouteRecord(routeRecord1);
    // liAns.addRouteRecord(routeRecord2);
    ...
    liAns.setServerName(servername);

    liAns.addSupportedFeatures(supportedFeatures);

    cout << liAns;

    SendDiaAppMsg(liAns, sIdx);
    CxStats::GetCxStats()->UpdateSendStats(liAns.getCommandCode(), ITS_FALSE);
    return (ITS_SUCCESS);
}
