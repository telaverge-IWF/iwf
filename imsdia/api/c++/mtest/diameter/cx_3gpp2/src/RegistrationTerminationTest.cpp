 ...
 * LOG:
 *
 * ID: $Id: RegistrationTerminationTest.cpp,v 3.1 2008/03/31 10:33:08 nvijikumar Exp $
 ****************************************************************************/
#ident "$Id: RegistrationTerminationTest.cpp,v 3.1 2008/03/31 10:33:08 nvijikumar Exp $"
 ...
 *  See Also:
 ****************************************************************************/
int SendCxRegistrationTerminationRequest()
{
    ...
    rtReq.setSessionId(diameter::base::SessionId(sessIdString));

    rtReq.setSessionIndex(sIdx);
    ...
    DeregistrationReason deregistrationReason;      // to change
    ReasonCode reasonCode(ReasonCode::PERMANENT_TERMINATION);
    deregistrationReason.setReasonCode(reasonCode);

    // [6.3.18] Set the Reason-Info AVP
    // contains textual information to inform the user about the
    // reason for a de-registration.
    // The AVP Code of Reason-Info AVP is 617
    // The Reason-Info AVP is of type UTF8String
    ReasonInfo reasonInfo("reason_for_deregistration");
    ...
    rtReq.addPublicIdentity(publicIdentity);
    ...
    rtReq.addSupportedFeatures(supportedFeatures);

    cout<<rtReq;

    SendDiaAppMsg(rtReq, sIdx);
    CxStats::GetCxStats()->UpdateSendStats(rtReq.getCommandCode(), ITS_TRUE);
    return ITS_SUCCESS;
}
...
int SendCxRegistrationTerminationAnswer(const CommandImpl * impl1)
{
   ...
   // Create an instance of RegistrationTerminationrequest class
    ...
    CxStats::GetCxStats()->UpdateSendStats(rta.getCommandCode(), ITS_FALSE);
    return (ITS_SUCCESS);
}
