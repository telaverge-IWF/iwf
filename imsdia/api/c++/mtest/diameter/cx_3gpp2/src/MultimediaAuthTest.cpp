 ...
 * LOG:
 *
 * ID: $Id: MultimediaAuthTest.cpp,v 3.1 2008/03/31 10:33:07 nvijikumar Exp $
 ****************************************************************************/
#ident "$Id: MultimediaAuthTest.cpp,v 3.1 2008/03/31 10:33:07 nvijikumar Exp $"
...
int SendCxMultimediaAuthRequest()
{
    ...
    maReq.setSessionId(diameter::base::SessionId(sessIdString));

    maReq.setSessionIndex(sIdx);
   ...
   // [2.1.2.x] Required AVPs
    ...
    maReq.setPublicIdentity(publicIdentity);
    ...
    maReq.setServerName(servername);
    ...
    // it contains a bit mask indicating the supported features of an application.
    ...
    maReq.addSupportedFeatures(supportedFeatures);
    ...
    SendDiaAppMsg(maReq, sIdx);

    CxStats::GetCxStats()->UpdateSendStats(maReq.getCommandCode(), ITS_TRUE);
    return ITS_SUCCESS;
}
/*.implementation:public
 ****************************************************************************
 ...
 *  See Also:
 ****************************************************************************/
int SendCxMultimediaAuthAnswer(const CommandImpl * impl)
{
    ...
    // Optional param error - whether this command carries an error indicator.
    // Optional param retrans - whether this is a retransmission.
    diameter::cx_3gpp2::MultimediaAuthAnswer maAns;
 ...
 //  const SIPAuthDataItem &sipAuthData = mareq.getSIPAuthDataItem();
    ...
    maAns.setSessionIndex(mareq.getSessionIndex());
    ...
    // The SIP-Number-Auth-Items AVP indicates the number of
    // authentication vectors the S-CSCF is requesting.
    // The AVP Code of SIP-Number-Auth-Items AVP is 607
    // The Data Type of SIP-Number-Auth-Items AVP is Unsigned32
    maAns.setSIPNumberAuthItems(sipNumAuth);

    maAns.setPublicIdentity(pubidentity);

    maAns.addSupportedFeatures(supportedFeatures);
 ...
 //   maAns.setSIPAuthDataItem(sipAuthData);
    ...
    // Send Object to stack.
    SendDiaAppMsg(maAns, sIdx);

    // Update stats.
    CxStats::GetCxStats()->UpdateSendStats(maAns.getCommandCode(), ITS_FALSE);
    return ITS_SUCCESS;
}
