 ...
 *
 ****************************************************************************/
#ident "$Id: SessionTerminationTest.cpp,v 1.1 2011/04/18 09:44:11 vkumara Exp $"
 ...
 *  See Also:
 ****************************************************************************/
int SendWaSessionTerminationRequest()
{
    ...
    diameter::wa::SessionTerminationRequest stReq(false,false);
    ...
    UserName userName("user@wa-network.com");
    ...
    WaStats::GetWaStats()->UpdateSendStats(stReq.getCommandCode(),
                                                                ITS_TRUE);
    return ITS_SUCCESS;
}
...
int SendWaSessionTerminationAnswer(const CommandImpl * impl1)
{
    ...
    // Optional param error - whether this command carries an error indicator.
    // Optional param retrans - whether this is a retransmission.
    diameter::wa::SessionTerminationAnswer sta(false,false);
    ...
    // The AVP Code of User-Name AVP is 1.
    // The data type of User-Name AVP is UTF8String.
    UserName uname("user@wa-network.com");
    sta.setUserName(uname);

    cout << sta;

    SendDiaAppMsg(sta, sIdx);
    WaStats::GetWaStats()->UpdateSendStats(sta.getCommandCode(), ITS_FALSE);
    return (ITS_SUCCESS);
}
