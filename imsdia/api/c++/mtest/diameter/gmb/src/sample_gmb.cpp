 ...
 *                                                                          *
 ****************************************************************************
 * LOG: $Log: sample_gmb.cpp,v $
 * LOG: Revision 1.1  2011/06/14 05:25:38  vkumara
 * LOG: Baseline for Gmb interface.
 * LOG:
 * LOG:
 * 
 * ID: $Id: sample_gmb.cpp,v 1.1 2011/06/14 05:25:38 vkumara Exp $
 ****************************************************************************/
#ident "$Id: sample_gmb.cpp,v 1.1 2011/06/14 05:25:38 vkumara Exp $"
...
#include <diameter/gmb/ASA.h>
#include <diameter/gmb/ASR.h>
 ...
 *  See Also:
 ****************************************************************************/
int HandleDiaAppMsg(const diameter::CommandImpl *impl, unsigned int &sIdxGmb)
{
    try
    {
        // Update the receive statistics
        GmbStats::GetGmbStats()->UpdateRecvStats(impl->getCommandCode(),
                                               impl->isRequest());
        ...
        else
        {
            switch (impl->getCommandCode())
            {
                case GMB_AA_MSG_CMD_CODE:
                    ...
                    break;
                case GMB_RA_MSG_CMD_CODE:
                    if (impl->isRequest())
                    {
                        RAR raReq(*impl);
                        if (appMode == UNIT_TEST)
                        cout << "Re-Auth-Request" << raReq<< endl;
                        SendGmbReAuthAnswer(impl);
                    }
                    else
                    {
                        RAA  raAns(*impl);
                ...
                case GMB_ST_MSG_CMD_CODE:
                    ...
                    break;
                case GMB_AS_MSG_CMD_CODE:
                    if (impl->isRequest())
                    {
                        ASR asReq(*impl);
                        if (appMode == UNIT_TEST)
                        cout <<"Abort-Session-Request"<<asReq << endl;
                        SendGmbAbortSessionAnswer(impl);
                    }
                    else
                    {
                        ASA asAns(*impl);
 ...
 ****************************************************************************
 *  Purpose:
 *      This function displays the menu options for the GMB interface
 *
 *  Input Parameters:
 *      None.
 *
...
int DisplayAppMenu()
{
    ...
    while(k)
    {
        do
        {
            cout << endl << "Select GMB Command" <<endl << endl;
        ...
        try
        {
            switch (ret)
            {
                case 1:
                    ...
                    break;
                case 2:
                    SendGmbReAuthRequest();
                    break;
                case 3:
                    SendGmbSessionTerminationRequest();
                    break;
                case 4:
                    SendGmbAbortSessionRequest();
                    break;
                case 99:
                    k=0;
                    break;
...
int GenerateLoad()
{
    ...
    while (true)
    {
        if (GmbStats::sendTraffic)
        {
             ...
             */
            for (int i = 0; i < GmbStats::burstSize; i++)
            {
             ...
             */
            ftime(&GmbStats::current);
            TIMERS_USleep(GmbStats::slpTime * 1000);

            /*
             * If the traffic duration (seconds) expires, update the stats
             */
            if ((GmbStats::current.time - GmbStats::starter.time) >=
                 GmbStats::duration)
            {
                if (GmbStats::firstTime)
                {
                    ftime(&GmbStats::sendComplete);
                    GmbStats::sendTraffic = ITS_FALSE;
                    GmbStats::firstTime = false;
                    time(&tx);
...
{
    AppInitialize(GMB_INTERFACE_APP_ID);
    AppMain(argc,argv);
