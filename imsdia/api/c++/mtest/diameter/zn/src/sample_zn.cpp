 ...
 ****************************************************************************
 *
 * LOG: $Log: sample_zn.cpp,v $
 * LOG: Revision 1.1  2011/06/02 04:26:21  vkumara
 * LOG: Baseline for Zn interface.
 * LOG:
 * LOG:
 * ID: $Id: sample_zn.cpp,v 1.1 2011/06/02 04:26:21 vkumara Exp $
 ****************************************************************************/
#ident "$Id: sample_zn.cpp,v 1.1 2011/06/02 04:26:21 vkumara Exp $"
 ...
 *  See Also:
 ****************************************************************************/
int HandleDiaAppMsg(const diameter::CommandImpl *impl, unsigned int &sIdxZn)
{
    try
    {
        // Update the receive statistics
        ZnStats::GetZnStats()->UpdateRecvStats(impl->getCommandCode(),
                                               impl->isRequest());
        ...
        else
        {
            ...
            {
                case ZN_BS_MSG_CMD_CODE:
                    if (impl->isRequest())
                    {
                        BootstrappingInfoRequest  bsReq(*impl);
                        if (appMode == UNIT_TEST)
                        cout << "BootstrappingInfoRequest" << bsReq<< endl ;
                        SendZnBootstrappingInfoAnswer( impl);
                    }
                    else
                    {
                        BootstrappingInfoAnswer  bsAns(*impl);
                        if (appMode == UNIT_TEST)
                        cout << " BootstrappingInfoAnswer" << bsAns<< endl;
                    }
                    break;
                ...
                case BASE_AS_MSG_CMD_CODE:
                    HandleBaseMessages(impl,sIdxZn);
                    break;
 ...
 ****************************************************************************
 *  Purpose:
 *      This function displays the menu options for the Zn interface
 *
 *  Input Parameters:
 *      None.
 *
...
int DisplayAppMenu()
{
    ...
    while(k)
    {
        do
        {
            cout << endl << "Select Zn Command" <<endl << endl;
            cout << "1: BSR - Bootstrapping-Info-Request " << endl;
        ...
        try
        {
            ...
            {
                case 1:
                    SendZnBootstrappingInfoRequest ();
                    break;
                case 2:
...
int GenerateLoad()
{
    ...
    while (true)
    {
        if (ZnStats::sendTraffic)
        {
             ...
             */
            for (int i = 0; i < ZnStats::burstSize; i++)
            {
             ...
             */
            ftime(&ZnStats::current);
            TIMERS_USleep(ZnStats::slpTime * 1000);

            /*
             * If the traffic duration (seconds) expires, update the stats
             */
            if ((ZnStats::current.time - ZnStats::starter.time) >=
                 ZnStats::duration)
            {
                if (ZnStats::firstTime)
                {
                    ftime(&ZnStats::sendComplete);
                    ZnStats::sendTraffic = ITS_FALSE;
                    ZnStats::firstTime = false;
                    time(&tx);
...
{
    AppInitialize(ZN_INTERFACE_APP_ID);
    AppMain(argc,argv);
