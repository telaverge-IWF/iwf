 ...
 ****************************************************************************
 *
 * LOG: $Log: BootstrappingInfoTest.cpp,v $
 * LOG: Revision 1.1  2011/06/02 04:26:21  vkumara
 * LOG: Baseline for Zn interface.
 * LOG:
 * LOG:
 * ID: $Id: BootstrappingInfoTest.cpp,v 1.1 2011/06/02 04:26:21 vkumara Exp $
 ****************************************************************************/
#ident "$Id: BootstrappingInfoTest.cpp,v 1.1 2011/06/02 04:26:21 vkumara Exp $"

#include <app.h>
#include <zn_defines.h>
#include <zn_stats.h>
#include <diameter/zn/BootstrappingInfo.h>

using namespace std;
using namespace its;
using namespace diameter;
using namespace diameter::base;
using namespace diameter::zn;
 ...
 ****************************************************************************
 *  Purpose: This function sends the  BootstrappingInfoRequest to the
 *      diameter stack. Create the  BootstrappingInfoRequest object,
 ...
 *  See Also:
 ****************************************************************************/
int SendZnBootstrappingInfoRequest()
{
    // [1]. Create an instance of BootstrappingInfoRequest.
    // Optional param error - whether this command carries an error indicator.
    // Optional param retrans - whether this is a retransmission.
    diameter::zn::BootstrappingInfoRequest  bsReq;
  ...
  AllocateSession(ZN_INTERFACE_APP_ID, sIdx, sessIdString);
  bsReq.setSessionId(diameter::base::SessionId(sessIdString));

    bsReq.setSessionIndex(sIdx);
   ...
   // [2.1.2.x] Required AVPs
    ...
    AuthApplicationId authApplicationId(ZN_INTERFACE_APP_ID);
    ...
    bsReq.setVendorSpecificApplicationId(vendorSpecificApplicationId1);
    ...
    bsReq.setOriginHost(originHost);
    ...
    bsReq.setOriginRealm(originRealm);
    ...
    bsReq.setDestinationRealm(destinationRealm);
    ...
    bsReq.setDestinationHost(destinationHost);

   TransactionIdentifier  transactionIdentifier("1234567");
   bsReq.setTransactionIdentifier(transactionIdentifier);

   NAFId  nAFId("1234567");
   bsReq.setNAFId(nAFId);
    ...
    SendDiaAppMsg(bsReq, sIdx);

    ZnStats::GetZnStats()->UpdateSendStats(bsReq.getCommandCode(), ITS_TRUE);
    return ITS_SUCCESS;
}
/*.implementation:public
 ****************************************************************************
 *  Purpose:
 *      This function sends the Answer to the  BootstrappingInfo Request
 *      made by the diameter stack. Create the BootstrappingInfoAnswer
 ...
 *  See Also:
 ****************************************************************************/
int SendZnBootstrappingInfoAnswer(const CommandImpl * impl)
{
    // 1. Create an instance of BootstrappingInfoAnswer class
    ...
    // Optional param error - whether this command carries an error indicator.
    // Optional param retrans - whether this is a retransmission.
    diameter::zn::BootstrappingInfoAnswer  bsAns;

    // Create an instance of MultimediaAuthRequest class to get host and
    // session details
    BootstrappingInfoRequest bsreq(*impl);

    // Get AVP values from requested message
    const SessionId &sid1 = bsreq.getSessionId();
    const OriginHost &oh1 = bsreq.getOriginHost();
    const OriginRealm &or1 = bsreq.getOriginRealm();
    const DestinationHost &dh1 = bsreq.getDestinationHost();
    const DestinationRealm &dr1 = bsreq.getDestinationRealm();
    const VendorSpecificApplicationId &vsAppId = 
                       bsreq.getVendorSpecificApplicationId();

    // 2. Create the instances of the classes describing the various
    // AVPs' present in an BootstrappingInfoAnswer command and
    // pass them by value to the corresponding data members set
    // methods of the BootstrappingInfoAnswer object created
    // in step 1.
    ...
    // The AVP Code of Session-Id is 263
    // The data type of Session-Id is UTF8String.
    bsAns.setSessionId(sid1);

    bsAns.setSessionIndex(bsreq.getSessionIndex());
    ...
    //[2.1.2.1] Set the  Vendor-Specific-Application-Id AVP.
    bsAns.setVendorSpecificApplicationId(vsAppId);

    // The AVP Code of Origin-Host AVP is 264
    // The data type of Origin-Host AVP is DiameterIdentity.
    OriginHost originHost(GetOriginHost());
    bsAns.setOriginHost(originHost);
    ...
    // The data type of Origin-Host AVP is DiameterIdentity.
    OriginRealm originRealm(GetOriginRealm());
    bsAns.setOriginRealm(originRealm);
    ...
    bsAns.setResultCode(resultCode);
    ...
    // Send Object to stack.
   SendDiaAppMsg(bsAns, sIdx);

    // Update stats.
    ZnStats::GetZnStats()->UpdateSendStats(bsAns.getCommandCode(), ITS_FALSE);
    return ITS_SUCCESS;
}
