...
#include <diameter/s6a_r11/InsertSubscriberData.h>

void testInsertSubscriberDataRequest() {
    diameter::s6a_r11::InsertSubscriberDataRequest request;
...
void testInsertSubscriberDataAnswer() {
    diameter::s6a_r11::InsertSubscriberDataAnswer answer;
    ...
    answer.setIMSVoiceOverPSSessionsSupported(IMSVoiceOverPSSessionsSupported(...));
    answer.setLastUEActivityTime(LastUEActivityTime(...));
    answer.setRATType(RATType(...));
    answer.setIDAFlags(IDAFlags(...));
    answer.setEPSUserState(EPSUserState(...));
    answer.setEPSLocationInformation(EPSLocationInformation(...));
    answer.setLocalTimeZone(LocalTimeZone(...));
