...
#include <diameter/s6a_r11/UpdateLocation.h>

void testUpdateLocationRequest() {
    diameter::s6a_r11::UpdateLocationRequest request;
    ...
    request.setEquivalentPLMNList(EquivalentPLMNList(...));
    request.setMMENumberforMTSMS(MMENumberforMTSMS(...));
    request.setSMSReqisterRequest(SMSReqisterRequest(...));
...
void testUpdateLocationAnswer() {
    diameter::s6a_r11::UpdateLocationAnswer answer;
