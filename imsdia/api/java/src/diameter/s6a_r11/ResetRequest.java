...
*/
public class ResetRequest extends com.intellinet.diameter.Command {
    /**
     * Constructs a new ResetRequest command.
     *
     * @param error - whether this command carries an error indicator
     * @param retrans - whether this is a retransmission
     */
    public ResetRequest(boolean error, boolean retrans) {
        super(true, true, error, retrans, 322, 16777251);
    }

    /**
     * Constructs a ResetRequest command by decoding a byte array.
     *
     * @param buffer - the buffer that contains the encoded bytes
     */
    public ResetRequest(byte[] buffer) throws WrongCommandTypeException, CodecException {
        super(buffer);
        validate();
    }

    /**
     * Constructs a ResetRequest command from a GenericCommand.
     *
     * @param generic - the GenericCommand
     */
    public ResetRequest(GenericCommand generic) throws WrongCommandTypeException {
        super(generic.transfer());
        validate();
    }

    ResetRequest(com.intellinet.diameter.CommandData data) throws WrongCommandTypeException {
        super(data);
        validate();
    }
    ...
    /**
     * Returns number of UserId instances.
     */
    public int countUserId() {
        return _data.getContainer().count(S6a_r11AttributeName.UserId);
    }
     ...
     * @param index - the index of the attribute to return
     */
    public UserId getUserId(int index) throws NoSuchAttributeException {
        return (UserId)_data.getContainer().get(S6a_r11AttributeName.UserId, index);
    }
     ...
     * @param avp - the attribute value
     */
    public void addUserId(UserId avp) {
        _data.getContainer().add(S6a_r11AttributeName.UserId, avp.getData(), false);
    }
    ...
    private void validate() throws WrongCommandTypeException {
        if (getCommandCode() != 322 || !isRequest()) {
