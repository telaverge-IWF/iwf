 ...
 */
public class STAnswer extends com.intellinet.diameter.Command {
    /**
     * Constructs a new STAnswer command.
     *
     * @param error - whether this command carries an error indicator
     * @param retrans - whether this is a retransmission
     */
    public STAnswer(boolean error, boolean retrans) {
        super(false, true, error, retrans, 275, 1);
    }

    /**
     * Constructs a STAnswer command by decoding a byte array.
     *
     * @param buffer - the buffer that contains the encoded bytes
     */
    public STAnswer(byte[] buffer) throws WrongCommandTypeException, CodecException {
        super(buffer);
        validate();
    }

    /**
     * Constructs a STAnswer command from a GenericCommand.
     *
     * @param generic - the GenericCommand
     */
    public STAnswer(GenericCommand generic) throws WrongCommandTypeException {
        super(generic.transfer());
        validate();
    }

    STAnswer(com.intellinet.diameter.CommandData data) throws WrongCommandTypeException {
        super(data);
        validate();
    }
