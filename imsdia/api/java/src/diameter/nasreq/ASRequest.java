 ...
 */
public class ASRequest extends com.intellinet.diameter.Command {
    /**
     * Constructs a new ASRequest command.
     *
     * @param error - whether this command carries an error indicator
     * @param retrans - whether this is a retransmission
     */
    public ASRequest(boolean error, boolean retrans) {
        super(true, true, error, retrans, 274, 1);
    }

    /**
     * Constructs a ASRequest command by decoding a byte array.
     *
     * @param buffer - the buffer that contains the encoded bytes
     */
    public ASRequest(byte[] buffer) throws WrongCommandTypeException, CodecException {
        super(buffer);
        validate();
    }

    /**
     * Constructs a ASRequest command from a GenericCommand.
     *
     * @param generic - the GenericCommand
     */
    public ASRequest(GenericCommand generic) throws WrongCommandTypeException {
        super(generic.transfer());
        validate();
    }

    ASRequest(com.intellinet.diameter.CommandData data) throws WrongCommandTypeException {
        super(data);
        validate();
    }
    ...
    private void validate() throws WrongCommandTypeException {
        if (getCommandCode() != 274 || !isRequest()) {
