...
*/
public class MEIdentityCheckAnswer extends com.intellinet.diameter.Command {
    /**
     * Constructs a new MEIdentityCheckAnswer command.
     *
     * @param error - whether this command carries an error indicator
     * @param retrans - whether this is a retransmission
     */
    public MEIdentityCheckAnswer(boolean error, boolean retrans) {
        super(false, true, error, retrans, 324, 16777252);
    }

    /**
     * Constructs a MEIdentityCheckAnswer command by decoding a byte array.
     *
     * @param buffer - the buffer that contains the encoded bytes
     */
    public MEIdentityCheckAnswer(byte[] buffer) throws WrongCommandTypeException, CodecException {
        super(buffer);
        validate();
    }

    /**
     * Constructs a MEIdentityCheckAnswer command from a GenericCommand.
     *
     * @param generic - the GenericCommand
     */
    public MEIdentityCheckAnswer(GenericCommand generic) throws WrongCommandTypeException {
        super(generic.transfer());
        validate();
    }

    MEIdentityCheckAnswer(com.intellinet.diameter.CommandData data) throws WrongCommandTypeException {
        super(data);
        validate();
    }
    ...
    /**
     * Tests whether there is any EquipmentStatus.
     */
    public boolean containsEquipmentStatus() {
        return _data.getContainer().contains(S13_r10AttributeName.EquipmentStatus);
    }

    /**
     * Gets the EquipmentStatus.
     */
    public EquipmentStatus getEquipmentStatus() throws NoSuchAttributeException {
        return (EquipmentStatus)_data.getContainer().get(S13_r10AttributeName.EquipmentStatus, 0);
    }

    /**
     * Sets the EquipmentStatus.
     *
     * @param avp - the attribute value
     */
    public void setEquipmentStatus(EquipmentStatus avp) {
        _data.getContainer().add(S13_r10AttributeName.EquipmentStatus, avp.getData(), true);
    }
    ...
    public String toString() {
        ...
        if (containsEquipmentStatus()) {
            sb.append("    EquipmentStatus:");
            try {
                sb.append(getEquipmentStatus()).append('\n');
            } catch (NoSuchAttributeException x) {
                sb.append("[said present but missing]\n");
            }
        }
