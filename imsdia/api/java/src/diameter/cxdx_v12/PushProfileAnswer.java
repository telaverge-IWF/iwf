 ...
 * the Command Flags field, is sent by a client in response to the Push-Profile-Request command. The 
 * Experimental-Result AVP may contain one of the values defined in section 6.2 in addition to the values defined in IETF
 * RFC 3588 [6].
 */
public class PushProfileAnswer extends com.intellinet.diameter.Command {
    /**
     * Constructs a new PushProfileAnswer command.
     *
     * @param error - whether this command carries an error indicator
     * @param retrans - whether this is a retransmission
     */
    public PushProfileAnswer(boolean error, boolean retrans) {
        super(false, true, error, retrans, 305, 16777216);
    }

    /**
     * Constructs a PushProfileAnswer command by decoding a byte array.
     *
     * @param buffer - the buffer that contains the encoded bytes
     */
    public PushProfileAnswer(byte[] buffer) throws WrongCommandTypeException, CodecException {
        super(buffer);
        validate();
    }

    /**
     * Constructs a PushProfileAnswer command from a GenericCommand.
     *
     * @param generic - the GenericCommand
     */
    public PushProfileAnswer(GenericCommand generic) throws WrongCommandTypeException {
        super(generic.transfer());
        validate();
    }

    PushProfileAnswer(com.intellinet.diameter.CommandData data) throws WrongCommandTypeException {
        super(data);
        validate();
    }
