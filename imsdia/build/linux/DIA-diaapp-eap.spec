...
%define BACKUPDIR /opt/IntelliNet/IMS-EAP_BKP
%define RELDIR /tmp/RPM_eap
%define VERSION 2.1
...
OLDVERSION=%OLDVERSION
BACKUPDIR=%BACKUPDIR
echo "EAP Pre Removal..."
if [ -d $BASEDIR/config  ]
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/EAP-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/EAP-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/EAP-%VERSION
    fi
