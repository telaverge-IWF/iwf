...
%define BACKUPDIR /opt/IntelliNet/IMS-S6A_BKP
%define RELDIR /tmp/RPM_S6A
%define VERSION 3.1
...
OLDVERSION=%OLDVERSION
BACKUPDIR=%BACKUPDIR
echo "S6A Pre Removal..."
if [ -d $BASEDIR/config  ]
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/S6A-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/S6A-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/S6A-%VERSION
    fi
