...
%define BACKUPDIR /opt/IntelliNet/IMS-JTY_BKP
%define RELDIR /tmp/RPM_ty
%define VERSION 2.2
...
OLDVERSION=%OLDVERSION
BACKUPDIR=%BACKUPDIR
echo "JTY Pre Removal..."
if [ -d $BASEDIR/config  ]
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/JTY-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/JTY-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/JTY-%VERSION
    fi
