...
%define BACKUPDIR /opt/IntelliNet/IMS-MIP_BKP
%define RELDIR /tmp/RPM_mip
%define VERSION 2.1
...
OLDVERSION=%OLDVERSION
BACKUPDIR=%BACKUPDIR
echo "MIP Pre Removal..."
if [ -d $BASEDIR/config  ]
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/MIP-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/MIP-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/MIP-%VERSION
    fi
