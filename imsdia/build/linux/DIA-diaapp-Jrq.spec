...
%define BACKUPDIR /opt/IntelliNet/IMS-JRQ_BKP
%define RELDIR /tmp/RPM_rq
%define VERSION 2.2
...
OLDVERSION=%OLDVERSION
BACKUPDIR=%BACKUPDIR
echo "JRQ Pre Removal..."
if [ -d $BASEDIR/config  ]
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/JRQ-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/JRQ-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/JRQ-%VERSION
    fi
