...
%define BACKUPDIR /opt/IntelliNet/IMS-WM_BKP
%define RELDIR /tmp/RPM_wm
%define VERSION 2.1
...
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/WM-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/WM-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/WM-%VERSION
    fi
