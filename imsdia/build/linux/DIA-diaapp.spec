...
%define BACKUPDIR /opt/IntelliNet/IMS_BKP
...
%pre
PKG=%pkg
BASEDIR=%BASEDIR/IMSDIA-%VERSION
VERSION=%VERSION
...
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/IMSDIA-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/IMSDIA-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/IMSDIA-%VERSION
    fi
