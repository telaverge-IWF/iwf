...
%define BACKUPDIR /opt/IntelliNet/IMS-RQ_BKP
...
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/RQ-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/RQ-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/RQ-%VERSION
    fi
