...
%define BACKUPDIR /opt/IntelliNet/IMS-E4_BKP
...
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/E4-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/E4-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/E4-%VERSION
    fi
