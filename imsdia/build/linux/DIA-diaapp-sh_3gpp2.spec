...
%define BACKUPDIR /opt/IntelliNet/IMS-SH-3GPP2_BKP
%define RELDIR /tmp/RPM_sh_3gpp2
%define VERSION 2.1
%define OLDVERSION 2.0
%define user intelli
%define group other 
%define name SH-3GPP2 
...
%pre
PKG=%pkg
BASEDIR=%BASEDIR/SH-3GPP2-%VERSION
VERSION=%VERSION
...
USER=%user
GROUP=%user
echo "sh_3gpp2 Post Installation..."
...
OLDVERSION=%OLDVERSION
BACKUPDIR=%BACKUPDIR
echo "SH-3GPP2 Pre Removal..."
if [ -d $BASEDIR/config  ]
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/SH-3GPP2-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/SH-3GPP2-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/SH-3GPP2-%VERSION
    fi
