%define pkg IMSDIA-BCGI
%define BASEDIR /opt/IntelliNet
%define BACKUPDIR /opt/IntelliNet/IMS-BCGI_BKP
%define RELDIR /tmp/RPM_imsdia
%define VERSION 2.1
%define OLDVERSION 2.0
%define user intelli
%define group other 
%define name IMSDIA-BCGI
...
%pre
PKG=%pkg
BASEDIR=%BASEDIR/IMSDIA-BCGI-%VERSION
VERSION=%VERSION
...
OLDVERSION=%OLDVERSION
BACKUPDIR=%BACKUPDIR
echo "IMS Pre Removal..."
if [ -d $BASEDIR/config  ]
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/IMSDIA-BCGI-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/IMSDIA-BCGI-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/IMSDIA-BCGI-%VERSION
    fi
