...
%define BACKUPDIR /opt/IntelliNet/IMS-S13_BKP
%define RELDIR /tmp/RPM_s13
%define VERSION 3.1
...
OLDVERSION=%OLDVERSION
BACKUPDIR=%BACKUPDIR
echo "S13 Pre Removal..."
if [ -d $BASEDIR/config  ]
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/S13-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/S13-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/S13-%VERSION
    fi
