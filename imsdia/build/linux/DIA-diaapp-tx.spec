...
%define BACKUPDIR /opt/IntelliNet/IMS-TX_BKP
...
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/TX-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/TX-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/TX-%VERSION
    fi
