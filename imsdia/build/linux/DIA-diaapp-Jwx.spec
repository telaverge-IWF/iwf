...
%define BACKUPDIR /opt/IntelliNet/IMS-JWX_BKP
%define RELDIR /tmp/RPM_wx
%define VERSION 2.2
...
OLDVERSION=%OLDVERSION
BACKUPDIR=%BACKUPDIR
echo "JWX Pre Removal..."
if [ -d $BASEDIR/config  ]
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/JWX-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/JWX-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/JWX-%VERSION
    fi
