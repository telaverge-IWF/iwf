...
%define BACKUPDIR /opt/IntelliNet/IMS-E2_BKP
...
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/E2-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/E2-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/E2-%VERSION
    fi
