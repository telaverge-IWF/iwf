...
%define BACKUPDIR /opt/IntelliNet/IMS-JE4_BKP
%define RELDIR /tmp/RPM_e4
%define VERSION 2.2
...
OLDVERSION=%OLDVERSION
BACKUPDIR=%BACKUPDIR
echo "JE4 Pre Removal..."
if [ -d $BASEDIR/config  ]
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/JE4-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/JE4-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/JE4-%VERSION
    fi
