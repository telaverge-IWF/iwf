...
%define BACKUPDIR /opt/IntelliNet/IMS-JGX_BKP
%define RELDIR /tmp/RPM_gx
%define VERSION 2.2
...
OLDVERSION=%OLDVERSION
BACKUPDIR=%BACKUPDIR
echo "JGX Pre Removal..."
if [ -d $BASEDIR/config  ]
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/JGX-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/JGX-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/JGX-%VERSION
    fi
