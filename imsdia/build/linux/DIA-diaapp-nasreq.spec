...
%define BACKUPDIR /opt/IntelliNet/IMS-NASREQ_BKP
%define RELDIR /tmp/RPM_nasreq
%define VERSION 2.1
...
%pre
PKG=%pkg
BASEDIR=%BASEDIR/NASREQ-%VERSION
VERSION=%VERSION
...
OLDVERSION=%OLDVERSION
BACKUPDIR=%BACKUPDIR
echo "NASREQ Pre Removal..."
if [ -d $BASEDIR/config  ]
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/NASREQ-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/NASREQ-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/NASREQ-%VERSION
    fi
