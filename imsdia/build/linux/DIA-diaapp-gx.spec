...
%define BACKUPDIR /opt/IntelliNet/IMS-GX_BKP
...
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/GX-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/GX-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/GX-%VERSION
    fi
