...
%define BACKUPDIR /opt/IntelliNet/IMS-JSCAP_BKP
%define RELDIR /tmp/RPM_scap
%define VERSION 2.2
...
OLDVERSION=%OLDVERSION
BACKUPDIR=%BACKUPDIR
echo "JSCAP Pre Removal..."
if [ -d $BASEDIR/config  ]
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/JSCAP-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/JSCAP-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/JSCAP-%VERSION
    fi
