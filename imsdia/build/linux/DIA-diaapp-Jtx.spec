...
%define BACKUPDIR /opt/IntelliNet/IMS-JTX_BKP
%define RELDIR /tmp/RPM_tx
%define VERSION 2.2
...
OLDVERSION=%OLDVERSION
BACKUPDIR=%BACKUPDIR
echo "JTX Pre Removal..."
if [ -d $BASEDIR/config  ]
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/JTX-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/JTX-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/JTX-%VERSION
    fi
