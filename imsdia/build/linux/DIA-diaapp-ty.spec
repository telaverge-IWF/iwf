...
%define BACKUPDIR /opt/IntelliNet/IMS-TY_BKP
...
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/TY-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/TY-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/TY-%VERSION
    fi
