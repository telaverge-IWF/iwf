...
%define BACKUPDIR /opt/IntelliNet/IMS-JRX_BKP
...
OLDVERSION=%OLDVERSION
BACKUPDIR=%BACKUPDIR
echo "JRX Pre Removal..."
if [ -d $BASEDIR/config  ]
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/JRX-%VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/JRX-%VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/JRX-%VERSION
    fi
