...
%define BACKUPDIR /opt/IntelliNet/JIDC_BKP
%define RELDIR /tmp/RPM_JIDC
%define VERSION 1.0
%define OLDVERSION 0.0
%define user intelli
%define group other
%define name JIDC
...
then
    echo ""
    echo " Backing up $BASEDIR as $BACKUPDIR/JIDC-$VERSION"
    if [ ! -d $BACKUPDIR ]
    then
       mkdir -p $BACKUPDIR
    fi

    if [ -d $BACKUPDIR/JIDC-$VERSION ]
    then
       # Remove the same version backup
       /bin/rm -rf $BACKUPDIR/JIDC-$VERSION
    fi
