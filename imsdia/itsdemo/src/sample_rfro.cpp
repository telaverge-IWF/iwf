 ...
 * LOG: Revision 3.2  2010/06/24 08:19:49  nvijikumar
 * LOG: Updates for rel 8.a.0
 * LOG:
 * LOG: Revision 3.1  2008/03/31 10:33:26  nvijikumar
 * LOG: Start GA 3.0
 * LOG:
 * LOG: Revision 1.7  2006/11/30 16:53:13  hbhatnagar
 * LOG: removed the user input for session index
 * LOG:
 * LOG: Revision 1.6  2006/11/22 11:17:10  hbhatnagar
 * LOG: made changes for the load test and unit test
 * LOG:
 * LOG: Revision 1.5  2006/11/14 07:18:57  hbhatnagar
 * LOG: Revert the modifications for SendCreditControlAnswer.
 * LOG:
 * LOG: Revision 1.4  2006/10/30 15:27:06  hbhatnagar
 * LOG: Made changes for dcca in rfro for menu and CCR and CCA.
 * LOG:
 * LOG: Revision 1.3  2006/10/17 17:26:26  tpanda
 ...
 * LOG: Start GA 1.1
 * LOG:
 * LOG: Revision 1.10  2006/09/07 06:13:00  kamakshilk
 * LOG: Removed the icmptypes set in IPFilterRule
 * LOG:
 * LOG: Revision 1.9  2006/06/30 14:20:03  nvijikumar
 * LOG: Small changes for menu exit function.
 * LOG:
 * LOG: Revision 1.8  2006/06/30 10:28:13  nvijikumar
 * LOG: Changes for Exit function.
 * LOG:
 * LOG: Revision 1.7  2006/06/14 09:54:52  kamakshilk
 * LOG: AVP value corrections and IPFilterRule AVP addition
 * LOG:
 * LOG: Revision 1.6  2006/06/07 15:00:20  tpanda
 * LOG: Code added for load testing
 * LOG:
 * LOG: Revision 1.5  2006/06/07 11:07:54  tpanda
 * LOG: Handling of error responses
 * LOG:
 * LOG: Revision 1.4  2006/06/06 07:47:01  yranade
 * LOG: Get correct values for Origin(Host-Realm) & Destination(Host-Realm)
 * LOG:
 * LOG: Revision 1.3  2006/06/02 12:10:53  kamakshilk
 * LOG: Minor fixes
 * LOG:
 * LOG: Revision 1.2  2006/06/02 09:22:16  yranade
 * LOG: Minor upate
 * LOG:
 * LOG: Revision 1.1  2006/06/02 04:49:27  kamakshilk
 * LOG: RfRo Demo code
 * LOG:
 *
 * ID: $Id: sample_rfro.cpp,v 3.2 2010/06/24 08:19:49 nvijikumar Exp $
 ****************************************************************************/
#ident "$Id: sample_rfro.cpp,v 3.2 2010/06/24 08:19:49 nvijikumar Exp $"
...
int HandleRfRoDiaMessage(its::ITS_Event& ev);
 ...
 ****************************************************************************/
extern "C"
void RfRoThreadFunc(ThreadPoolThread* thr, void* arg)
{
    int ret;
 ...
 *  See Also:
 ****************************************************************************/
int HandleRfRoDiaMessage(ITS_Event& ev)
{
    APP_TRACE_DEBUG(("Received RfRo Command: Event->Length = %d\n",
                   ev.GetLength()));
                     ...
                     case RFRO_CC_MSG_CMD_CODE:
                         ...
                         else
                         {
                             ...
                             else
                             {
                                 ...
                                 {
                                     case CCRequestType::INITIAL_REQUEST:
                                         SendCreditControlRequest(
                                                   TERMINATION, impl);
                                         break;
                                   case CCRequestType::TERMINATION_REQUEST:
                                       releaseStackCtx = true;
                                       break;
                                 }
                             }
                             if ( releaseStackCtx              ||
                                (!(diaFrame->IsDccaEnabled()) &&
                                (ccans.getCCRequestType().value()
                                == CCRequestType::EVENT_REQUEST)))
                             {
                                 string sessId = ccans.getSessionId().value();
                                 DiaFrame::GetDiaFrame()->ReleaseSession(sessId.c_str());
                             }
                          }
                         break;
...
int SendRfRoMessage(int input)
{
    ...
    case 3:
    case 4:
        SendCreditControlRequest(input); break;
    case 99:
        {
            DiaFrame *diaFrame = DiaFrame::GetDiaFrame();
            if (diaFrame)
...
{
    return SendCreditControlRequest(INITIAL);
}
 ...
 *  See Also:
 ****************************************************************************/
int SendCreditControlRequest(int requestType,
...
int SendCreditControlAnswer(const diameter::CommandImpl* impl ) 
{
    ...
    try
    {
        ...
        ITS_UINT sIdx1 = ccr.getSessionIndex();
        cca.setSessionIndex(sIdx1);
        ...
        if (releaseStackCtx ||
            (!(DiaFrame::GetDiaFrame()->IsDccaEnabled()) &&
            (ccRequestType.value() == CCRequestType::EVENT_REQUEST)))
        {
            string sessId = cca.getSessionId().value();
            DiaFrame::GetDiaFrame()->ReleaseSession(sessId.c_str());
        }
...
//        multipleServicesCC.addTriggerType(triggerType1);
    ...
    else
    {
        ...
        // [1.11] Populate optional parameter Trigger-Type
        // Set the Trigger-Type AVP.
        // The data type Trigger-Type is Enumerated.
        // This shall be included inthe request only when the Reporting-Reason
        // AVP value is RATING_CONDITION_CHANGE
        ...
        TriggerType triggerType1(TriggerType::CHANGE_IN_QOS);
        Trigger trigger;
        trigger.addTriggerType(triggerType1);
        multipleServicesCC.setTrigger(trigger); 
...
void PopulatePSInformation(PSInformation &psInfo) 
{
    ...
    // [1.1.18] Populate optional parameter Charging-Rule-Base-Name. 
    // Set Charging-Rule-Base-Name AVP.
    // The data type of Charging-Rule-Base-Name is OctetString.
    // This indicates the name of a pre-defined group of charging rules 
    // residing at the TPF.
    // The AVP Code for Charging-Rule-Base-Name is 1004.
    ChargingRuleBaseName ruleBaseName("gaming_service");// not clear
    //psInfo.setChargingRuleBaseName(ruleBaseName);
...
void PopulateIMSInformation(IMSInformation &imsInfo) 
{
    ...
    // [1.1.2] Populate optional parameter Role-Of-Node. 
    // Set Role-Of-Node AVP.
    // The data type of Role-Of-Node is Enumerated.
    // This specifies the role of the AS/CSCF.
    // The AVP Code for Role-Of-Node is 829.
    //RoleOfNode roleOfNode(RoleOfNode::PROXY_ROLE);
    //imsInfo.setRoleOfNode(roleOfNode);
    ...
    // [1.1.5] Populate optional parameter Calling-Party-Address. 
    // Set Calling-Party-Address AVP.
    // The data type of Calling-Party-Address is UTF8String.
    // This holds the address (Public User ID,SIP URL,E.164 etc.) of the 
    // party initiating a session.In IMS charging the address is obtained 
    // from P-Asserted-Identity header.
    // The AVP Code for Calling-Party-Address is 831.
    //CallingPartyAddress callingPartyAddr("sip:bob@example.com"); 
    //imsInfo.setCallingPartyAddress(callingPartyAddr); 
    ...
    // [1.1.18] Populate optional parameter Service-Specific-Data. 
    // Set Service-Specific-Data AVP.
    // The data type of Service-Specific-Data is UTF8String.
    // This holds service specific data if and as provided by an 
    // Application Server.
    // Refer draft-ietf-intserv-data-encoding-01.txt
    // The AVP Code for Service-Specific-Data is 863.
    //ServiceSpecificData serviceSpecData("c51023a64e2097"); // not clear
    //imsInfo.setServiceSpecificData(serviceSpecData);
    ...
    // [1.1.20] Populate optional parameter Cause-Code. 
    // Set Cause-Code AVP.
    // The data type of Cause-Code is Enumerated.
    // This includes the cause code value from IMS node.
    // The AVP Code for Cause-Code is 861.
    //CauseCode causeCode(CauseCode::Successful_transaction);
    //imsInfo.setCauseCode(causeCode); 
