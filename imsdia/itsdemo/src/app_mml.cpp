 ...
 * LOG: Revision 3.12.44.1  2013/02/21 12:01:26  ncshivakumar
 * LOG: removed 3rd parameter from SetLocalHostInfo as it does not accept ip address now
 * LOG:
 * LOG: Revision 3.12  2011/02/15 05:41:49  nvijikumar
 * LOG: dia_msg_router.h should not be included at the application
 * LOG:
 * LOG: Revision 3.11  2010/11/04 09:19:04  nvijikumar
 * LOG: Get Statistics via API
 * LOG:
 * LOG: Revision 3.10  2010/09/30 07:19:22  vkumara
 * LOG: MML Commands Enable/Disable Dscp
 * LOG:
 * LOG: Revision 3.9  2010/09/23 13:09:26  vkumara
 * LOG: getDiaTrace added
 * LOG:
 * LOG: Revision 3.8  2010/08/31 09:32:08  vkumara
 * LOG: Dscp value range validation
 * LOG:
 * LOG: Revision 3.7  2010/08/31 04:36:40  nvijikumar
 * LOG: val not required for getDscp MML command
 * LOG:
 * LOG: Revision 3.6  2010/08/27 12:32:47  nvijikumar
 * LOG: MML commands for DSCP testing - Vasanth
 * LOG:
 * LOG: Revision 3.5  2010/08/19 11:26:59  nvijikumar
 * LOG: Removed DCCA deps
 * LOG:
 * LOG: Revision 3.4  2010/08/19 04:19:33  vkumara
 ...
 * LOG:
 * LOG: Revision 3.3  2008/12/12 13:27:21  sureshj
 * LOG: Updated for Stack statistics support.
 * LOG:
 * LOG: Revision 3.2  2008/12/10 07:11:05  nvijikumar
 * LOG: Added MML command to send Alarm
 * LOG:
 ...
 * LOG:
 *
 * ID: $Id: app_mml.cpp,v 3.12.44.1 2013/02/21 12:01:26 ncshivakumar Exp $
 ****************************************************************************/
...
APPMMLServer::DoCommand(istream& commandLine)
{
    ...
    else if (Cmd("sendAlarm", "Send Alarm for UT Input <AlarmId>"))
    {
        ITS_UINT alarmId;
        commandLine >> dummy >> alarmId;
        if (alarmId >= DIA_ALARM_START && alarmId <= DIA_ALARM_END)
        {
            DIA_Alarm(alarmId, __FILE__, __LINE__, NULL);
        }
        else
        {
            oss<<"Invalid Input"<<endl;
        }
    }
    ...
    else if (Cmd("getDiaMiscStatsViaApi", "Display Miscellaneous Stats"))
    {
        int ret;
        DIA_MISC_STATS_STR miscStats;
        memset(&miscStats, 0, sizeof(DIA_MISC_STATS_STR));
        ret = diaMgmt->GetDiaMiscStats(&miscStats, false);
        if (ret == ITS_SUCCESS)
        {
            printf("DiaMiscStats\n");
        }
