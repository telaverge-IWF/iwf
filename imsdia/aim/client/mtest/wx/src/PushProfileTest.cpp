 ...
 * LOG:
 * LOG:
 * ID: $Id: PushProfileTest.cpp,v 3.1 2008/03/31 10:33:03 nvijikumar Exp $
 ****************************************************************************/
#ident "$Id: PushProfileTest.cpp,v 3.1 2008/03/31 10:33:03 nvijikumar Exp $"
...
unsigned int sIdxWxPp = 0;
string sessIdStringWxPp;
IDSCLApi apiWxPp;
...
int SendWxPushProfileRequest() 
{
    ...
    apiWxPp.AllocateSessionIndex(10000, sIdxWxPp,sessIdStringWxPp);
    ppReq.setSessionId(diameter::base::SessionId(sessIdStringWxPp));
   ...
   // Create a Vendor-Specific-Application-Id AVP object
    ...
    ppReq.setVendorSpecificApplicationId(vendorSpecificApplicationId1);
    ...
    ppReq.setAuthSessionState(authSessionState);
    ...
    ppReq.setOriginHost(originHost);
    ...
    ppReq.setOriginRealm(originRealm);
    ...
    ppReq.setDestinationHost(destinationHost);
    ...
    ppReq.setDestinationRealm(destinationRealm);
    ...
    ppReq.setUserName(userName);

    // ppReq.setWLANUserData(WLANUserData(...));

    // Application not supposed to set these AVPS.
    // ppReq.addProxyInfo(diameter::base::ProxyInfo(...));
    // ppReq.addRouteRecord(diameter::base::RouteRecord(...));
    // add more AVP as ANY values

    //cout<<ppReq;
    //SendToStack(&ppReq);
    //WxStats::GetWxStats()->UpdateSendStats(ppReq.getCommandCode(), ITS_TRUE);

    std::cout << ppReq << std::endl;

    apiWxPp.Send(ppReq, sIdxWxPp, WX_INTERFACE_APP_ID);
    return (ITS_SUCCESS);
}
 ...
 *  See Also:
 ****************************************************************************/
int SendWxPushProfileAnswer(const CommandImpl * impl1)
{
    ...
    // Optional param error - whether this command carries an error indicator
    // Optional param retrans - whether this is a retransmission
    diameter::wx::PushProfileAnswer ppAns;

    // Create an instance of PushProfileRequest class
    PushProfileRequest ppreq(*impl1);

    // Get some AVP values from Request
    const SessionId &sid1 = ppreq.getSessionId();
    const OriginHost &oh1 = ppreq.getOriginHost();
    const OriginRealm &or1 = ppreq.getOriginRealm();
    const DestinationHost &dh1 = ppreq.getDestinationHost();
    const DestinationRealm &dr1 = ppreq.getDestinationRealm();
    const AuthSessionState &authSessState = ppreq.getAuthSessionState();
    const VendorSpecificApplicationId &vsAppId = ppreq.getVendorSpecificApplicationId();
    ...
    //[2.1] Set the  Vendor-Specific-Application-Id AVP.
    ppAns.setVendorSpecificApplicationId(vsAppId);
    ...
    // [2.1] Set the Auth-Session-State AVP.
    AuthSessionState authSessionState(AuthSessionState::STATE_MAINTAINED);
    ppAns.setAuthSessionState(authSessionState);
    ...
    ppAns.setOriginHost(originHost);
    ...
    ppAns.setOriginRealm(originRealm);
    ...
    ppAns.setResultCode(resultCode);
    ...
    // ppAns.setExperimentalResult(experimentalResult);

    // [2.2] Set the Proxy-Info AVP.
    // Application should not set these AVPs.
    // ppAns.addProxyInfo(diameter::base::ProxyInfo(...));
    // ppAns.addRouteRecord(diameter::base::RouteRecord(...));
    // add more AVP as ANY values

    //cout<<ppAns;
    //SendToStack(&ppAns);
    //WxStats::GetWxStats()->UpdateSendStats(ppAns.getCommandCode(), ITS_FALSE);
    std::cout << ppAns << std::endl;

    apiWxPp.Send(ppAns, sIdxWxPp, WX_INTERFACE_APP_ID);
