 ...
 *                                                                          *
 ****************************************************************************
 * LOG: $Log: MultimediaAuthenticationTest.cpp,v $
 ...
 * LOG:
 * LOG:
 * ID: $Id: MultimediaAuthenticationTest.cpp,v 3.1 2008/03/31 10:33:03 nvijikumar Exp $
 ****************************************************************************/
#ident "$Id: MultimediaAuthenticationTest.cpp,v 3.1 2008/03/31 10:33:03 nvijikumar Exp $"
...
unsigned int sIdxWxMa = 0;
string sessIdStringWxMa;
IDSCLApi apiWxMa;
 ...
 *  See Also:
 ****************************************************************************/
int SendWxMultimediaAuthenticationRequest() 
{
    // [1]. Create an instance of MultimediaAuthenticationRequest.
    // Optional param error - whether this command carries an error indicator.
    // Optional param retrans - whether this is a retransmission.
    diameter::wx::MultimediaAuthenticationRequest maReq;
    ...
    apiWxMa.AllocateSessionIndex(10000, sIdxWxMa, sessIdStringWxMa);
    maReq.setSessionId(diameter::base::SessionId(sessIdStringWxMa));
   ...
   // [2.1.2.x] Required AVPs
    ...
    maReq.setVendorSpecificApplicationId(vendorSpecificApplicationId1);
    ...
    maReq.setAuthSessionState(authSessionState);
    ...
    maReq.setOriginHost(originHost);
    ...
    maReq.setOriginRealm(originRealm);
    ...
    maReq.setDestinationRealm(destinationRealm);

    // [2.1.2.6] Set the NASPortType AVP of type OctetString.
    // AS-Port-Type AVP indicates the type of the physical port of the 
    // NAS which is authenticating the user.
    // The Value field witn "Virtual" refers to a connection to the NAS 
    // via some transport protocol, instead of through a physical port 
    NASPortType nasPortTyp(NASPortType::Virtual);
    maReq.setNASPortType(nasPortTyp);
    ...
    maReq.setDestinationHost(destinationHost);
    ...
    maReq.setVisitedNetworkIdentifier(visitedNetworkIdentifier);
    ...
    maReq.setSIPAuthDataItem(sIPAuthDataItem);
    ...
    maReq.setSIPNumberAuthItems(sIPNumberAuthItems);

    // Application should not set this AVP.
    // maReq.addProxyInfo(diameter::base::ProxyInfo(...));
    // maReq.addRouteRecord(diameter::base::RouteRecord(...));
    ...
    //WxStats::GetWxStats()->UpdateSendStats(maReq.getCommandCode(), ITS_TRUE);
    std::cout << maReq << std::endl;

    apiWxMa.Send(maReq, sIdxWxMa, WX_INTERFACE_APP_ID);
    return ITS_SUCCESS;
}
/*.implementation:public
 ****************************************************************************
 ...
 *  See Also:
 ****************************************************************************/
int SendWxMultimediaAuthenticationAnswer(const CommandImpl * impl) 
{
    ...
    // Optional param error - whether this command carries an error indicator.
    // Optional param retrans - whether this is a retransmission.
    diameter::wx::MultimediaAuthenticationAnswer maAns;
    ...
    const SessionId &sid1 = mareq.getSessionId();
    const OriginHost &oh1 = mareq.getOriginHost();
    const OriginRealm &or1 = mareq.getOriginRealm();
    const DestinationHost &dh1 = mareq.getDestinationHost();
    const DestinationRealm &dr1 = mareq.getDestinationRealm();
    const VendorSpecificApplicationId &vsAppId = mareq.getVendorSpecificApplicationId();
    const AuthSessionState &authSessState = mareq.getAuthSessionState();
    const UserName &uname = mareq.getUserName();
    const SIPNumberAuthItems &sipNumAuth = mareq.getSIPNumberAuthItems();
   const SIPAuthDataItem &sipAuthData = mareq.getSIPAuthDataItem();
    ...
    //[2.1.2.1] Set the  Vendor-Specific-Application-Id AVP.
    maAns.setVendorSpecificApplicationId(vsAppId);
    ...
    // The AVP Code of Origin-Host AVP is 264
    // The data type of Origin-Host AVP is DiameterIdentity.
    maAns.setAuthSessionState(authSessState);
    ...
    maAns.setOriginHost(originHost);
    ...
    maAns.setOriginRealm(originRealm);

    // [2.1] Set the User-Name AVP.
    // The User-Name AVP ia a RADIUS AVP and contains the user name.
    // The AVP Code of User-Name AVP is 1.
    // The data type of User-Name AVP is UTF8String.
    maAns.setUserName(uname);
    ...
    maAns.setResultCode(resultCode);
    ...
    // maAns.setExperimentalResult(experimentalResult);
    ...
    // The Data Type of SIP-Number-Auth-Items AVP is Unsigned32
    maAns.setSIPNumberAuthItems(sipNumAuth);
    ...
    // The AVP Code of SIP-Auth-Data-Item AVP is 612
    // The Data Type of SIP-Auth-Data-Item AVP is Grouped
    maAns.setSIPAuthDataItem(sipAuthData);
    ...
    // maAns.setProxyInfo(diameter::base::ProxyInfo(...));
    ...
    // This AVP should not be set by Application.
    // maAns.setRouteRecord(diameter::base::RouteRecord(...));
    // add more AVP as ANY values
    ...
    // Update stats.
    //WxStats::GetWxStats()->UpdateSendStats(maAns.getCommandCode(), ITS_FALSE);
    std::cout << maAns << std::endl;

    apiWxMa.Send(maAns, sIdxWxMa, WX_INTERFACE_APP_ID);
