 ...
 * LOG:
 * LOG:
 * ID: $Id: AATest.cpp,v 3.1 2008/03/31 10:33:02 nvijikumar Exp $
 ****************************************************************************/
#ident "$Id: AATest.cpp,v 3.1 2008/03/31 10:33:02 nvijikumar Exp $"
...
unsigned int sIdxRxAa = 0;
string sessIdStringRxAa;
IDSCLApi apiRxAa;
...
int SendRxAARequest() 
{
    ...
    apiRxAa.AllocateSessionIndex(10000, sIdxRxAa, sessIdStringRxAa);
    aaReq.setSessionId(SessionId(sessIdStringRxAa));
    ...
    std::cout << aaReq << std::endl;

    apiRxAa.Send(aaReq, sIdxRxAa, RX_INTERFACE_APP_ID);
...
int SendRxAAAnswer(const CommandImpl *impl) 
{
    ...
    std::cout << aaAns << std::endl;

    apiRxAa.Send(aaAns, sIdxRxAa, RX_INTERFACE_APP_ID);
