 ...
 * LOG:
 * LOG: Revision 1.3  2006/11/17 07:35:23  kamakshilk
 * LOG: contains API added for optional AVPs
 * LOG:
 ...
 * LOG:
 * LOG:
 * ID: $Id: RATest.cpp,v 3.1 2008/03/31 10:33:03 nvijikumar Exp $
 ****************************************************************************/
#ident "$Id: RATest.cpp,v 3.1 2008/03/31 10:33:03 nvijikumar Exp $"
...
extern unsigned int sIdxTy;
string sessIdStringTyRa;
extern IDSCLApi apiTy;
...
int SendTyRARequest()
{
    ...
    cout << "Please enter Re-Authorization Index" << endl;
    string sId;
    cin >> sId;
    sIdxTy = atoi(sId.c_str());
   ...
   // [2.1.2.1] Set the Origin-Host AVP.
    ...
    raReq.setCCSubSessionId(ccSubSessionId);
   ...
   std::cout << raReq << std::endl;

   apiTy.Send(raReq, sIdxTy, TY_INTERFACE_APP_ID);
 ...
 *  See Also:
 ****************************************************************************/
int SendTyRAAnswer(const CommandImpl *impl)
{
    ...
    if (rareq.containsOriginStateId())
        const OriginStateId& osid = rareq.getOriginStateId();
    if (rareq.containsCCSubSessionId())
        const CCSubSessionId& ccsubId = rareq.getCCSubSessionId();
    ...
    // The data type of Auth-Application-Id is Unsigned32.
    AuthApplicationId authAppId(TY_INTERFACE_APP_ID);
    raAns.setAuthApplicationId(authAppId);
    ...
    // Set the CCSubSessionId AVP.
    CCSubSessionId ccsubId(10);
    raAns.setCCSubSessionId(ccsubId);
    ...
    std::cout << raAns << std::endl;

    apiTy.Send(raAns, sIdxTy, TY_INTERFACE_APP_ID);
    return ITS_SUCCESS;
}
