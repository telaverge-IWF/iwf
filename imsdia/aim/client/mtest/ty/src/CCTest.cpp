 ...
 * LOG:
 * LOG:
 * ID: $Id: CCTest.cpp,v 3.1 2008/03/31 10:33:03 nvijikumar Exp $
 ****************************************************************************/
#ident "$Id: CCTest.cpp,v 3.1 2008/03/31 10:33:03 nvijikumar Exp $"
...
int SendTyCCRequest()
{
    ...
    apiTy.AllocateSessionIndex(10000,sIdxTy,sessIdStringTyCc);
    ccReq.setSessionId(SessionId(sessIdStringTyCc));
    ...
    apiTy.Send(ccReq, sIdxTy, TY_INTERFACE_APP_ID);
...
int SendTyCCAnswer(const diameter::CommandImpl* impl)
{
    ...
    apiTy.Send(ccAns, sIdxTy, TY_INTERFACE_APP_ID);
    ...
    if (!apiTy.IsDccaEnabled())
    {
        switch(ccAns.getCCRequestType().value())
        {
           ...
           case CCRequestType::TERMINATION_REQUEST:
               ...
               apiTy.ReleaseSession(rlsStr, sIdxTy);
               break;
        }
    }
