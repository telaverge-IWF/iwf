...
function check_for_tipc()   
{
    ret=0
    modprobe tipc >/dev/null
    if [ $? -ne 0 ];then
        PLOG_ERROR "TIPC Module not found!" $LINENO
        ret=1
    fi
    if [ ! -e /lib/modules/`uname -r`/kernel/net/tipc/tipc.ko ];then
        PLOG_ERROR "TIPC Module not found for `uname -r`!" $LINENO
        ret=1
    fi
    return $ret 
}
...
if [ "$UPGRADE_FLG" == "yes" ];then
    ...
    if [ -e /etc/${PRODUCT}.cfg ];then
        LOG_DEBUG "Truncating /etc/${PRODUCT}.cfg file" $LINENO
        >  /etc/${PRODUCT}.cfg
    fi
...
rpm -qa | grep "MySQL-Cluster-server" >/dev/null
if [ $? -eq 0 ];then
    chkconfig mysql off 
fi
...
if [ "$UPGRADE_FLG" == "no" ];then
    ...
    #done after dependency check because mysql 
    #has to be started to validate
