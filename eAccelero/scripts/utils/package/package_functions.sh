CUR_DRE_VER=dre-4.3.2-0
CUR_IWF_VER=iwf-4.1.0-08
PRODUCT=iwf
...
function validate_filled()
{
    ...
    do
	    ...
	    if [ "$2" == "-i" ];then
		LOG_ERROR "\"$item\" not configured" $LINENO
	    else
...
#$1 is the transport type variable
...
function validate_install_mode_cnf()
{
   ...
   if [ "$PRODUCT" == "dre" ];then
	   ...
	   validate_possible_values "install_mode" "1 3 4"
	   if [ $? -ne 0 ];then
		exit 1
	   fi
       fi
...
function validate_tipc_cnf()
{
	...
	if [ $? -ne 0 ];then
        ...
        if [ "$bonding" == "yes" ];then
            for (( i=0; i<${len}; i++ ));
            do
                LOG_DEBUG "Checking if ${bond_name[$i]} is configured as bearer" $LINENO
                echo ${bond_name[$i]}|grep `echo "$bearer_name"|cut -d':' -f2` >/dev/null
                if [ $? -eq 0 ];then
                    LOG_DEBUG "${bond_name[$i]} match with tipc bearer name!" $LINENO
                    valid_flag=1
                    break
                fi
            done
	...
	#check if
        if [ $valid_flag -eq 0 ];then
	        ...
	        PLOG_ERROR "Interface `echo "$bearer_name"|cut -d':' -f2` is not present in the system/not part of bonding configuration" $LINENO
	        exit 1
        fi
	fi
    fi
...
{
    mysql_heartbeat_cfg__list="mysql_local_ip mysql_mate_ip mysql_interface_name"  
    ...
    ip_address_list="mysql_local_ip  mysql_mate_ip "
       ...
       tmp_vip=`get_dre_cnf_val "mysql_virtual_ip"`

       if [ -n "$tmp_vip" ];then
           validate_ip_list "mysql_virtual_ip"
           if [ $? -ne 0 ];then
                exit 1
           fi
           ...
           LOG_DEBUG "Assigning default IP as 127.0.0.1 for mysql" $LINENO
           mysql_virtual_ip="127.0.0.1/8"
       fi
    #if [ "$PRODUCT" == "iwf" ];then
...
function parse_sigtran_config()
{
    ...
    asp_id1=`get_dre_cnf_val "asp_id1"`
    host_name1=`get_dre_cnf_val "host_name1"`

    #parse the host-aspid configuration only if dre_install_mode == 4
    if [ "$dre_install_mode" == "4" ];then
        parse_host_aspid_config
    fi

}

function parse_host_aspid_config()
{
    #sigtran nodes information
    host_name2=`get_dre_cnf_val "host_name2"`
    asp_id2=`get_dre_cnf_val "asp_id2"`
}
...
function convert_yes_no_to_1_0()
{
    if [ "$1" == "yes" ];then
        echo "1"
    else
...
function update_sigtrancfg_in_iwfcfg()
{
    ...
    set_dre_cfg "`format_cfg "asp_id1"`" "$asp_id1"
    set_dre_cfg "`format_cfg "host_name1"`" "$host_name1"

    #update the host-aspid configuration only if dre_install_mode == 4
    if [ "$dre_install_mode" == "4" ];then
        set_dre_cfg "`format_cfg "host_name2"`" "$host_name2"
        set_dre_cfg "`format_cfg "asp_id2"`" "$asp_id2"
    fi
...
{
    local RET=0
    local host_name=`get_dre_cnf_val "$1"`
    LOG_DEBUG "Validating HostName" $LINENO
...
function validate_hostname_config()
{
    local RET=0
    local hostname1=`get_dre_cnf_val "$1"`
    local hostname2=`get_dre_cnf_val "$2"`
    LOG_DEBUG "Validating both HostNames's $1 != $2 " $LINENO

    if [ "$hostname1" == "$hostname2" ];then
        RET=1
        PLOG_ERROR "Invalid HostNames's configuration given, HostName2 : \"$hostname2\" matching with HostName1 : \"$hostname1\" " $LINENO
    fi
    
    return $RET
}

function validate_aspid_config()
{
    local RET=0
    local aspid1=`get_dre_cnf_val "$1"`
    local aspid2=`get_dre_cnf_val "$2"`
    LOG_DEBUG "Validating both AspId's $1 != $2 " $LINENO

    if [[ $aspid1 = *[[:digit:]]* ]];then
    	LOG_DEBUG "$aspid1 is Number" $LINENO
    
    fi
    if [[ $aspid2 = *[[:digit:]]* ]];then
	    LOG_DEBUG "$aspid2 is Number" $LINENO
    fi

    if [ "$aspid1" == "$aspid2" ];then
        RET=1
        PLOG_ERROR "Invalid AspId's configuration given, AspId2 : \"$aspid2\" matching with AspId1 : \"$aspid1\" " $LINENO
    fi
...
function validate_sigtran_config()
{
    ...
    sigtran_cfg_list="point_code ssn tcap_invoke_timer global_title_indicator routing_indicator selection_criteria asp_id1"
    ...
    validate_possible_values "selection_criteria" "1 4 5 8 9 16 17 24 25"
    if [ $? -ne 0 ];then
        return 1
    fi
...
#    fi
    ...
    validate_filled "asp_id1 host_name1"
    if [ $? -ne 0 ];then
        PLOG_ERROR "Configure proper values for AspId1/HostName1 !" $LINENO
        return 1
    fi
    
    validate_range "asp_id1" 1 65535
    if [ $? -ne 0 ];then
        return 1
    fi
    
    if [ "$dre_install_mode" == "1" ];then
        validate_hostname "host_name1"
        if [ $? -ne 0 ];then
            PLOG_ERROR "Configured HostName1 is not matching with Machine HostName !" $LINENO
            return 1
        fi
    fi

    #validate the aspid's && hostname's configuration only if dre_install_mode == 4
    if [ "$dre_install_mode" == "4" ];then

        validate_filled "asp_id2 host_name2"
        if [ $? -ne 0 ];then
            PLOG_ERROR "Configure proper values for AspId2/HostName2 for HA configuration !" $LINENO
            return 1
        fi

        validate_range "asp_id2" 1 65535
        if [ $? -ne 0 ];then
            return 1
        fi
        ...
        validate_hostname "host_name1"
        if [ $? -ne 0 ];then
            validate_hostname "host_name2"
            if [ $? -ne 0 ];then
                PLOG_ERROR "Configured HostName1 && HostName2 are not matching with Machine HostName !" $LINENO
                return 1
            fi
        fi

        validate_hostname_config "host_name1" "host_name2"
        if [ $? -ne 0 ];then
            return 1
        fi
    fi
...
function parse_install_mode()
{
    ...
    if [ "$PRODUCT" == "iwf" ];then
        ...
        dre_install_mode=`get_dre_cnf_val "dre_install_mode"`
    fi
...
function parse_user_dre_cnf_file()
{
	...
	if ([  "`GetDreInstallMode`" == "1" ] && [ "$dre_install_mode" == "2" ]) ||
        ([  "`GetDreInstallMode`" == "1" ] && [ "$dre_install_mode" == "3" ]) ||
        ([  "`GetDreInstallMode`" == "1" ] && [ "$dre_install_mode" == "4" ]);then
	    LOG_DEBUG "User trying to upgrade from standalone DRE to Active-standby DRE by passing configFile" $LINENO
...
function check_and_stop_3rdparty_processes()
{
...
function install_mode_check()
{
	...
	while [ "$ch" != "1" ] &&  [ "$ch" != "2" ] && [ "$ch" != "3" ] && [ "$ch" != "4" ] ||  [ -z "$ch" ]
	do
		read -e -i "$default" -p "$2" ch
		actualvalue=${ch:-$default}
	done
...
'
    ...
    #if its previous version is active-standby then the upgrade must be to active-standby mode only
    if ([ "$1" == "--upgrade" ] && [ "`GetDreInstallMode`" == "2" ]) ||
        ([ "$1" == "--upgrade" ] && [ "`GetDreInstallMode`" == "3" ]) ||
        ([ "$1" == "--upgrade" ] && [ "`GetDreInstallMode`" == "4" ]);then
	LOG_INFO "Upgrade from HA to HA" $LINENO
	PLOG_INFO "Upgrade from HA to HA" $LINENO
	return 0
...
function set_dre_mode()
{
    ...
    "1") sed -i "s/^export BASICOPTS=.*/export BASICOPTS=\"-i 13 -l 3 -m 1\"/g" /etc/sysconfig/${S_PRODUCT_NAME}
	 LOG_INFO "-m 1" $LINENO
	 ;;
    "2") sed -i "s/^export BASICOPTS=.*/export BASICOPTS=\"-i 13 -l 3 -m 2\"/g" /etc/sysconfig/${S_PRODUCT_NAME}
	 LOG_INFO "-m 2" $LINENO
	 ;;
...
function enable_backup_recovery()
{
    PLOG_INFO "enabling backup and recovery..." $LINENO
    /opt/diametriq/dre/scripts/cron_edit_back.sh --add
}


function disable_backup_recovery()
{
    PLOG_INFO "disabling backup and recovery..." $LINENO
    /opt/diametriq/dre/scripts/cron_edit_back.sh --del
}
...
function modprobe_arptables()
{
    sed -i '/^modprobe arp_tables/d' /etc/rc.modules 2>/dev/null
...
function post_rpm_install()
{
    ...
    if [ "$PRODUCT" == "dre" ];then
        ...
        if [ "`GetDreInstallMode`" == "1" ];then
            update_webgui_cfg "virtual_ip" "`GetLocalIpAddress`"
        else
            update_webgui_cfg "virtual_ip" "`GetVirtualIP`"
        fi
        update_webgui_cfg "port_no" "8080"
    fi
...
#disable unwanted processs for iwf
    disable_processes
...
function revert_mysql_replication_cfg()
{
	...
	VAL=`${my} "show binary logs \G" | grep "Log_name" | tail -1  | tr -s " " | cut -d " " -f 3`
	LOG_DEBUG "Mysql binary logs Purged upto $VAL" $LINENO
	${my} "purge binary logs to '$VAL'" >/dev/null 
	...
	local value=`grep "^binlog_format" $MYSQL_CNFPATH`
	    /bin/sed -i "s/^binlog_format.*/#$value/g"  $MYSQL_CNFPATH
...
function uncommand_my_cnf_for_activestandby()
{
    ...
    if [ "`GetDreInstallMode`" == "2" ] || [ "`GetDreInstallMode`" == "3" ] || [ "`GetDreInstallMode`" == "4" ];then
	    ...
	    /bin/sed -i "s/^#relay-log.*/$value/g"  $MYSQL_CNFPATH
	    local value=`grep "^#binlog_format" $MYSQL_CNFPATH | tr -d "#"`
	    /bin/sed -i "s/^#binlog_format.*/$value/g"  $MYSQL_CNFPATH
    fi
...
function create_mysql_dquser()
{
    ...
    local unset_rep="set @@session.sql_log_bin=0;"
	...
	mysql -uroot -p$ROOT_P -se "${unset_rep} SELECT 1 FROM mysql.user WHERE user = '$USER';"| grep 1 >/dev/null
	if [ $? -ne 0 ];then
        	mysql -uroot -p$ROOT_P -se "${unset_rep} CREATE USER '$USER'@'localhost' IDENTIFIED BY '$PASS';"
	fi
    mysql -uroot -p$ROOT_P -se "${unset_rep} GRANT CREATE,RELOAD,EXECUTE,GRANT OPTION,LOCK TABLES,DROP,REFERENCES,ALTER,DELETE,INDEX,INSERT,SELECT,UPDATE,TRIGGER,ALTER ROUTINE,CREATE ROUTINE,CREATE USER,REPLICATION CLIENT,REPLICATION SLAVE,REFERENCES,SUPER ON *.* TO \"$USER\"@\"%\" IDENTIFIED BY '$PASS';"
    mysql -uroot -p$ROOT_P -se "${unset_rep} GRANT CREATE,RELOAD,EXECUTE,GRANT OPTION,LOCK TABLES,DROP,REFERENCES,ALTER,DELETE,INDEX,INSERT,SELECT,UPDATE,TRIGGER,ALTER ROUTINE,CREATE ROUTINE,CREATE USER,REPLICATION CLIENT,REPLICATION SLAVE,REFERENCES,SUPER ON *.* TO \"$USER\"@'localhost' IDENTIFIED BY '$PASS';"
        #####################
		...
		#disable replication in the mate mysql
    		...
    		mysql -uroot -p$ROOT_P -h $mate_mysql_ip -se "set @@session.sql_log_bin=0"
       	        #mysql -uroot -p$ROOT_P -h $mate_mysql_ip -se "${unset_rep} DROP USER '$USER'@'localhost'" 2>/dev/null
		mysql -uroot -p$ROOT_P -h $mate_mysql_ip -se "${unset_rep} SELECT 1 FROM mysql.user WHERE user = '$USER';"| grep 1 >/dev/null
		if [ $? -ne 0 ];then
    	        	mysql -uroot -p$ROOT_P -h $mate_mysql_ip -se "${unset_rep} CREATE USER '$USER'@'localhost' IDENTIFIED BY '$PASS';" 2>/dev/null
		fi
    	mysql -uroot -p$ROOT_P -h $mate_mysql_ip -se "${unset_rep} GRANT CREATE,RELOAD,EXECUTE,GRANT OPTION,LOCK TABLES,DROP,REFERENCES,ALTER,DELETE,INDEX,INSERT,SELECT,UPDATE,TRIGGER,ALTER ROUTINE,CREATE ROUTINE,CREATE USER,REPLICATION CLIENT,REPLICATION SLAVE,REFERENCES,SUPER ON *.* TO \"$USER\"@\"%\" IDENTIFIED BY '$PASS';"  2>/dev/null
    	mysql -uroot -p$ROOT_P -h $mate_mysql_ip -se "${unset_rep} GRANT CREATE,RELOAD,EXECUTE,GRANT OPTION,LOCK TABLES,DROP,REFERENCES,ALTER,DELETE,INDEX,INSERT,SELECT,UPDATE,TRIGGER,ALTER ROUTINE,CREATE ROUTINE,CREATE USER,REPLICATION CLIENT,REPLICATION SLAVE,REFERENCES,SUPER ON *.* TO \"$USER\"@'localhost' IDENTIFIED BY '$PASS';"  2>/dev/null
    	mysql -uroot -p$ROOT_P -h $mate_mysql_ip mysql -se "${unset_rep} update user set Select_priv='Y', Insert_priv='Y', Update_priv='Y', Delete_priv='Y', Create_priv='Y', Drop_priv='Y', Reload_priv='Y',Grant_priv='Y', References_priv='Y', Index_priv='Y', Alter_priv='Y', Super_priv='Y', Lock_tables_priv='Y', Execute_priv='Y', Repl_slave_priv='Y', Repl_client_priv='Y', Create_routine_priv='Y', Alter_routine_priv='Y', Create_user_priv='Y', Trigger_priv='Y' where Host='localhost' and User='dquser' "
		#enable replication in the mate mysql
    		mysql -uroot -p$ROOT_P -h $mate_mysql_ip -se "${unset_rep} flush privileges"
            fi
        fi
    #####################
        #enable replication
    	mysql -uroot -p$ROOT_P -se "${unset_rep} flush privileges"
...
buckets="DiameterSession"
#SessionBindingMap IPV4CalledStationIdMap IPV6CalledStationIdMap THFSubscriberContextTable THFSessionContextTable IMSISessionBindingMap"
...
function calculate_ram_for_couchbase()
{
    ...
    if [ -z "$ram_size" ];then
        ram_size=$(expr $ram_available / 2|bc)
    fi
    diameter_session_bucket_ram=$ram_size
    #other_bucket_ram=$(expr $diameter_session_bucket_ram / 6|bc)
}
...
function create_buckets()
{
    ...
    local cb_version=`rpm -qa|grep couchbase-server`
        local CLI=/opt/couchbase/bin/couchbase-cli

    #if the local couchbase node is already in the cluster(added by peer node) then skip creating bucket
    local cluster_size=`curl -s -u $ADMIN_NAME:$ADMIN_PASS http://localhost:8091/pools/nodes|tr "," "\n"|grep "hostname" 2>/dev/null|wc -l`
    if [ $cluster_size -ge 2 ];then
        PLOG_INFO "Couchbase Node is already in the cluster" $LINENO
        return 0
    fi
    #delete buckets before creating them
    delete_buckets
    if [ $? -ne 0 ];then
	#retry deletion of buckets
	local retry_count=0
	while [ $retry_count -lt $retry_count_max ]
	do
	    delete_buckets
	    if [ $? -eq 0 ];then
		break
	    fi
	    PLOG_INFO "Retrying deletion..." $LINENO
	    ((retry_count++))
	    sleep 7
	done
	#if it comes here bucket deletion failed even after retry!
    fi
...
function delete_buckets()
{
    ...
    local bucket_list=`$CLI  bucket-list -c $IP:$PORT -u $ADMIN_NAME -p $ADMIN_PASS|grep -v " "`
...
#this will update the /opt/diametriq/dre/config/webgui.cfg file with the details
# $1= name $2 = value
function update_webgui_cfg()
{
    WEBGUI_CFG=/opt/diametriq/${PRODUCT}/config/webgui.cfg
    if [ ! -e $WEBGUI_CFG ];then
	touch $WEBGUI_CFG
    fi
    LOG_DEBUG "update_webgui_cfg $1=$2" $LINENO
    grep "$1" $WEBGUI_CFG  >/dev/null 2>&1
    if [ $? -eq 0 ];then
        sed -i "s/^$1=.*/$1=$2/g" $WEBGUI_CFG
    else
        echo "$1=$2" >> $WEBGUI_CFG
        #append the value
    fi
...
function configure_couchbase()
{
    ...
    if [ "$1" == "--upgrade" ];then
        LOG_DEBUG "Dont configure couchbase during upgrade.." $LINENO
        return 0
    fi
    PLOG_DEBUG "Configuring couchbase..." $LINENO
    check_and_start_couchbase

    op=`$CLI cluster-init -c "$IP" --cluster-init-port="$PORT" --cluster-init-username="$ADMIN_NAME" --cluster-init-password="$ADMIN_PASS" --cluster-init-ramsize="$RAM_SIZE" --service='data;index;query'`
    if [ $? -ne 0 ];then
        ...
        if [ $? -eq 0 ];then
            ...
            op=`$CLI cluster-init -u $ADMIN_NAME -p $ADMIN_PASS -c "$IP" --cluster-init-port="$PORT" --cluster-init-username="$ADMIN_NAME" --cluster-init-password="$ADMIN_PASS" --cluster-init-ramsize="$RAM_SIZE" --service='data;index;query'`
        else
            LOG_ERROR "Couchbase initialization failed!" $LINENO
	    local retry_count=0
            while [ $retry_count -lt $retry_count_max ]
	    do
		sleep 5
	        PLOG_DEBUG "Retrying..." $LINENO
		op=`$CLI cluster-init -u $ADMIN_NAME -p $ADMIN_PASS -c "$IP" --cluster-init-port="$PORT" --cluster-init-username="$ADMIN_NAME" --cluster-init-password="$ADMIN_PASS" --cluster-init-ramsize="$RAM_SIZE" --service='data;index;query'`
	        echo $op|grep "ERROR" >>/var/log/$PRODUCT/package.log
        	if [ $? -ne 0 ];then
		    break
		fi
		((retry_count++))
	...
	do
	    ...
	    PLOG_INFO "Retrying creation of buckets..." $LINENO
	    sleep 7
	done
        #if it comes here, bucket creation failed even after retry!
    fi
...
#to avoid couchbase going into deadlock while stopping(rarely happens) we call kill_couchbase
#which will wait for 15 seconds and kill
function kill_cb()
{
    local process_pids=`ps -eaf|grep couchbase|grep couchbase|grep -v "couchbase_monitor"|grep -v "grep"|tr -s ' '|cut -d' ' -f2`
    if [ -z "$process_pids" ];then
       LOG_DEBUG "couchbase is not running exiting...." $LINENO
       return 0
    fi
    ...
    while [ 1 ]
    do
        local process_pid1=`ps -eaf|grep couchbase|grep couchbase|grep -v "couchbase_monitor"|grep -v "grep"|tr -s ' '|cut -d' ' -f2`
        LOG_DEBUG "couchbase is pid $process_pid1 $process_pids" $LINENO
        if [ -z "$process_pid1" ];then
       	    LOG_DEBUG "couchbase is stopped so exiting...." $LINENO
            return 0
        fi

      #  if [ $process_pid1 != $process_pid ];then
      # 	    LOG_DEBUG "couchbase pid changed so exiting, assuming that heartbeat restarted...." $LINENO
      #      exit
      #  fi
        ((count++))
        sleep 1
        if [ $count == 60 ];then
       	    PLOG_DEBUG "Killing couchbase...$process_pids count 15 reached" $LINENO
            kill -9 $process_pids 2>/dev/null
	    post_kill=`ps -eaf|grep couchbase|grep couchbase|grep -v "couchbase_monitor"|grep -v "grep"|tr -s ' '|cut -d' ' -f2`
	    PLOG_DEBUG "Post_kill=$post_kill" $LINENO
	    while [ ! -z $post_kill ]
	    do
		#check again , if it has not gone down retry
		LOG_DEBUG "Killing couchbase pids $post_kill..." $LINENO
	        kill -9 $process_pids 2>/dev/null
	    done

       	    LOG_DEBUG "kill_couchbase exiting...." $LINENO
            exit
        fi
    done
}
...
{
    #kill_cb will bring down couchbase if it does not go down.
    kill_cb &
    if [ -e /opt/couchbase/var/lib/couchbase/couchbase-server.pid ];then
    ...
    if [ $? -eq 0 ];then
        ...
        LOG_INFO "Couchbase state=`cat /etc/.${PRODUCT}/couchbase_state`" $LINENO
	        ...
	        LOG_WARNING "Couchbase-server is not running!" $LINENO
	echo "0" >/etc/.${PRODUCT}/couchbase_state
    fi
    else
        LOG_WARNING "Couchbase-server is not running!" $LINENO
...
function wait_for_couchbase_to_comeup()
{
    ...
    local flag=1
    local cb_version=`rpm -qa|grep couchbase-server`
    if [ "$cb_version" == "couchbase-server-2.1.1-764.x86_64" ];then
    ...
    elif [ "$cb_version" == "couchbase-server-3.0.0-973.x86_64" ];then
    	local list="epmd beam.smp memsup cpu_sup i386-linux-godu moxi /opt/couchbase/bin/memcached  inet_gethost portsigar"
    fi
...
function clean_couchbase()
{
    ...
    #remove self from the cluster, if this is the last node in the cluster then delete the buckets
    op=`$CLI rebalance-stop -c $IP:$PORT -u $ADMIN_NAME -p $ADMIN_PASS`
    echo "CLI rebalance-stop: $op"
    op=`$CLI failover --cluster=$MATE_IP:$PORT    -u $ADMIN_NAME -p $ADMIN_PASS    --server-failover=$IP:$PORT`
    if [ $? -eq 0 ];then
	echo "CLI failover: $op"
        PLOG_DEBUG "Removed $IP from the cluster" $LINENO
        op=`$CLI rebalance -c $MATE_IP:$PORT     -u $ADMIN_NAME -p $ADMIN_PASS  --server-remove=$IP:$PORT`
	echo "CLI rebalance: $op"
    else
	delete_buckets
        if [ $? -ne 0 ];then
	    #retry deletion of buckets
    ...
    delete_buckets
		...
		((retry_count++))
	        ...
	        PLOG_INFO "Retrying deletion of buckets..." $LINENO
		sleep 7
	    done
	    #if it comes here bucket deletion failed even after max retries
        fi
    fi
...
# $1=process name
# function to disable on boot for upstart services
function disable_on_boot()
{
    local upstart_script=/etc/init/$1.conf
    if [ -e $upstart_script ];then
        ps -eaf|grep $1|grep -v grep|grep -v couchbase >/dev/null
        if [ $? -eq 0 ];then
            stop $1
        fi
        local value=`grep "^start on runlevel \[2345\]" $upstart_script`
        /bin/sed -i "s/^start on runlevel \[2345\].*/#$value/g"  $upstart_script
        LOG_DEBUG "Disabling $1" $LINENO
    fi
}

function enable_on_boot()
{
    echo ""    
}
...
function disable_processes()
{
    if [ "$PRODUCT" == "iwf" ];then
        disable_on_boot "apache-tomcat"
        disable_on_boot "memcached"
    fi
...
{
    LOG_INFO "function : drop_databases" $LINENO
    local ip=`GetMysqlIpAddress`    
    local username=`GetMysqlUsername`
    local password=`GetMysqlPassword`
    local port=`GetMysqlPort`
    local unset_sess_repl="set @@session.sql_log_bin=0;"
    check_and_start_mysql
    my="mysql  --host=$ip --user=$username --password=$password --port=$port -se"
    DATABASES='DIAMETRIQ'
    for db in $DATABASES
    do
        ${my} "${unset_sess_repl} drop database $db" 2>/dev/null
        if [ $? -ne 0 ];then
            LOG_INFO "$db database not available to drop" $LINENO
        else
            LOG_INFO "$db database dropped!" $LINENO
        fi
    done    
}
