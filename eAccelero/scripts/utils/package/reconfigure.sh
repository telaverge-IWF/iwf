	...
	if [ `check_if_cnf_modified` == "no" ];then
		...
		3)  parse_validate $BONDING
	   	...
	   	4) 
		    ...
		    cd $DRE_BASE_PATH/lib/tipc* && ./install.sh
...
do
	...
	"--filesync")
			if [ "`GetDreInstallMode`" == "1" ];then
			    PLOG_INFO "File Sync not supported in standalone mode!" $LINENO
			    exit 0
			else
			    configure_rsync
			fi
