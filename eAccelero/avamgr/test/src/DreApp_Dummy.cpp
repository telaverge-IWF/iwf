...
#include "DreApp_Dummy.h"

using namespace eAccelero;

class DreApp_Dummy : public HAMeSSComponent {
public:

    //For the realistic component,
    //replace COMP_DRE_APP_DUMMY with COMP_DRE_APP (defined in ComponentDefines.h) &
    //replace I_DRE_APP_DUMMY with I_DRE_APP (defined in InterfaceDefines.h).
    DreApp_Dummy()
    : HAMeSSComponent("DreApp_Dummy", COMP_DRE_APP_DUMMY) {
    }
    ...
    void Initialize() {
        ...
        m_objRecurringTimerId = this->GetDispatcher()->AddEventHandler(
                EventSlot<TimerEvent, DreApp_Dummy > (this,
                &DreApp_Dummy::OnRecurringTimerExpiry),
...
int main(int argc, char *argv[]) {
    DreApp_Dummy *app = NULL;
    try {
        app = new DreApp_Dummy();
