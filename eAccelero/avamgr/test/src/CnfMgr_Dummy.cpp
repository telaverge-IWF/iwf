#include "CnfMgr_Dummy.h"

using namespace eAccelero;

class CnfMgr_Dummy : public HAMeSSComponent {
public:

    //For the realistic component,
    //replace COMP_CONFIG_MANAGER_DUMMY with COMP_CONFIG_MANAGER (defined in ComponentDefines.h) &
    //replace I_CONFIG_MANAGER_DUMMY with I_CONFIG_MANAGER (defined in InterfaceDefines.h).
    CnfMgr_Dummy()
    : HAMeSSComponent("CnfMgr_Dummy", COMP_CONFIG_MANAGER_DUMMY) {
    }
    ...
    void Initialize() {
        ...
        m_objRecurringTimerId = this->GetDispatcher()->AddEventHandler(
                EventSlot<TimerEvent, CnfMgr_Dummy > (this,
                &CnfMgr_Dummy::OnRecurringTimerExpiry),
...
int main(int argc, char *argv[]) {
    CnfMgr_Dummy *app = NULL;
    try {
        app = new CnfMgr_Dummy();
