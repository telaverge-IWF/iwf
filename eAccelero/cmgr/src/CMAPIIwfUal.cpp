...
int CMAPIIwfCMgr::HandleGetIwfUalManagerConfigReqs(IwfSigtranUalManagerConfig *req,IWFCfgResp *resp)
{
    int iOperID;
    ...
    {
       case GET_UAL_CONFIG:
                iReturn = m_IwfdbInterface->mysql_IwfUal_Get_Config_Reqs(req, resp);
                break;
    }
    ...
    DLOG_DEBUG("UAL-Manager resp = %s",resp->DebugString().c_str());
    DLOG_DEBUG("UAL-Manager req = %s",req->DebugString().c_str());
    DLOG_NONE(" < %s ",__FUNCTION__);
...
int CMAPIIwfCMgr::HandleSetIwfUalManagerConfigReqs(IwfSigtranUalManagerConfig *req,IWFCfgResp *resp)
{
    int iOperID;
    DbStatus_e iReturn = IWF_DB_FAIL;
    DLOG_NONE(" > %s ",__FUNCTION__);
    ...
    {
        case SET_UAL_CONFIG:
            iReturn = m_IwfdbInterface->mysql_IwfUal_Add_Config_Reqs(req, resp);
            break;

        case MOD_UAL_CONFIG:
            iReturn = m_IwfdbInterface->mysql_IwfUal_Mod_Config_Reqs(req, resp);
            break;
    }
    ...
    {
        IwfSigtranUalManagerConfig *lReq = new IwfSigtranUalManagerConfig();
        lReq->CopyFrom(*req);
        ...
        m_iwfSigtranMethods->HandleUalManagerConfigReqs(*lReq, (void*)cookie);
        resp->set_respid(CMAPI_SUCCESS);
    }
...
#if 1
int CMAPIIwfCMgr::HandleUalManagerConfigReqsRespCbk(IWFCfgResp* resp)
{
    int retVal = 0;
    std::string StrVal;
    IWFCfgResp ErrorResponse;
    DLOG_NONE(">HandleUalManagerConfigReqsRespCbk: ");
    ...
    IwfSigtranUalManagerConfig *req = (IwfSigtranUalManagerConfig*)cookie->req;
    ...
    else
    {
        if (MeSS::Handle().m_state == MESS_STATUS_SUCCESS)
        {
            if (resp->respid() == CMAPI_SUCCESS)
            {
                ...
                m_standbyiwfSigtranMethods->HandleUalManagerConfigReqs(*req, (void*)NULL);
            }
            else {
    ...
    deleteCookie(cookie,SET_UAL_CONFIG);
