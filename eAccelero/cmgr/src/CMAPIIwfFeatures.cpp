...
int CMAPIIwfCMgr::HandleGetIwfFeaturesConfigReqs(IwfFeaturesConfig *req,IWFCfgResp *resp)
{
    int iOperID;
...
int CMAPIIwfCMgr::HandleSetIwfFeaturesConfigReqs(IwfFeaturesConfig *req,IWFCfgResp *resp)
{
    int iOperID;
    DbStatus_e iReturn = IWF_DB_FAIL;
    DLOG_NONE(" > %s ",__FUNCTION__);
    ...
    switch(iOperID)
    {
		...
		iReturn = m_IwfdbInterface->mysql_IwfFeature_Add_Config_Reqs(req,resp);
            break;
        case MOD_IWF_FEATURES:
		iReturn = m_IwfdbInterface->mysql_IwfFeature_Mod_Config_Reqs(req,resp);
            break;
        case DEL_IWF_FEATURES:
            break;

        case DUMP_SYSTEM_INFO:
            DLOG_DEBUG("Passing System Info Dump to IWF");
            break;
    }
    ...
    else
    {
        ...
        IwfFeaturesConfig *lReq = new IwfFeaturesConfig();
        lReq->CopyFrom(*req);
        ...
        m_iwfSigtranMethods->HandleFeaturesConfigReqs(*lReq, (void*)cookie);
        resp->set_respid(CMAPI_SUCCESS);
    }
...
int CMAPIIwfCMgr::HandleFeaturesConfigReqsRespCbk(IWFCfgResp* resp)
{
    int retVal = 0;
    std::string StrVal;
    IWFCfgResp ErrorResponse;
    DLOG_NONE(">HandleFeaturesConfigReqsRespCbk: ");
    ...
    IwfFeaturesConfig *req = (IwfFeaturesConfig*)cookie->req;
    ...
    else
    {
        if (MeSS::Handle().m_state == MESS_STATUS_SUCCESS)
        {
            if (resp->respid() == CMAPI_SUCCESS)
            {
                ...
                m_standbyiwfSigtranMethods->HandleFeaturesConfigReqs(*req, (void*)NULL);
            }
            else {
    ...
    deleteCookie(cookie,DEL_IWF_FEATURES);
